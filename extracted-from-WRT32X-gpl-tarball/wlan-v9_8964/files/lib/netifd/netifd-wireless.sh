...
_wdev_common_device_config() {
	config_add_string channel hwmode htmode txantenna rxantenna rts optlevel beacon_int greenfield tx_stbc short_gi_20 rifs \
		preamble regioncode agingtime gprotect htprotect ratectl rateb rateg raten ratea rateac ratemu ratema dhenable dmode \
		hdfsmode htpc hcsamode hcsacount hnoptout hcactout intolerant40 beamforming opmode
}
...
_wdev_common_iface_config() {
	config_add_string mode ssid encryption 'key:wpakey' key1 key2 key3 key4 hidden dtim_period auth_server auth_port auth_secret \
		...
		amsdu ampdutx wpsenable wpsappin bssid fltmode maclist1 maclist2 maclist3 maclist4 maclist5 index bandsteer bandsteerintf pmfmode
}
