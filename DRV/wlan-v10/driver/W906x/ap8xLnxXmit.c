...
extern wltxdesc_t *wlSkbToCfhDl(struct net_device *netdev, struct sk_buff *skb,
				wltxdesc_t * txcfg,
				extStaDb_StaInfo_t * pStaInfo, int qid,
				int type);
		...
		if (!wlpptr->wlpd_p->dol.disable) {
			...
			if (!is_aligned) {
				if (((wlpptr->devid == SC5) &&
					...
					skb_align->data =
						PTR_ALIGN(skb_align->data,
							  TXBUF_ALIGN);
					memcpy(skb_align->data, skb->data,
					       skb->len);
			...
			if ((ieee80211_hdr->FrmCtl.Subtype ==
			     IEEE_MSG_AUTHENTICATE) ||
			    (ieee80211_hdr->FrmCtl.Subtype ==
			     IEEE_MSG_ASSOCIATE_RSP) ||
			    (ieee80211_hdr->FrmCtl.Subtype ==
			     IEEE_MSG_REASSOCIATE_RSP)) {
				printk("Host->Mgmt(%x): BBTX_TMR_FREE_TSF=0x%08x, BBTX_TMR_FREE_TSF_HI=0x%08x\n", ieee80211_hdr->FrmCtl.Subtype, readl(wlpptr->ioBase1 + BBTX_TMR_FREE_TSF), readl(wlpptr->ioBase1 + BBTX_TMR_FREE_TSF_HI));
			}
#endif
			dol_xmit(wlpptr, skb, &txcfg, qm_txq_id);
		} else {
#endif
			if (txqid_msg == 1) {
				show_pkt_txqid_msg(ieee80211_hdr, skb);
			}
			cfh_dl = wlSkbToCfhDl(netdev, skb, &txcfg, pStaInfo,
					      qm_txq_id, IEEE_TYPE_MANAGEMENT);
#ifdef WIFI_DATA_OFFLOAD
		}
#endif
	} /* if(type == IEEE_TYPE_MANAGEMENT)  */
...
#ifdef WIFI_DATA_OFFLOAD
		if (!wlpptr->wlpd_p->dol.disable) {
			int is_aligned, is_cloned;

			qm_txq_id = pbqm_args->txq_start_index;
			InitCFHDL(netdev, pbqm_args, &txcfg, skb,
				  (void *)pStaInfo, eap, nullpkt);
			is_aligned = IS_ALIGNED(((long)skb->data), TXBUF_ALIGN);
			is_cloned = skb_cloned(skb);

			if (!is_aligned || is_cloned) {
				if (((wlpptr->devid == SC5) &&
				     ((wlpptr->hwData.chipRevision == REV_Z1) ||
			...
			if (!pkt_q_per_sta) {
				...
				InitCFHDL(netdev, pbqm_args, &txcfg, skb,
					  (void *)pStaInfo, eap, nullpkt);
				cfh_dl = wlSkbToCfhDl(netdev, skb, &txcfg,
						      pStaInfo, qm_txq_id,
						      IEEE_TYPE_DATA);
			}
#ifdef WIFI_DATA_OFFLOAD
