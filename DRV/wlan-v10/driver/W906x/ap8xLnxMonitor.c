...
//actively check wfo cmd timeout or not
#define WFO_DOL_CMDTO_CHECK_INTERVAL   3

static int
wlmon_dol_cmdTO_detect(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;

	if (wlpd_p->smon.active == 0 || (wlpd_p->dol.disable == true))
		return 0;

	if (wlpd_p->smon.exceptionDolHangCause)	//already cmd timeout
	{
		//reset the counter
		wlpd_p->smon.DolCmdTOChkIvl = 0;
		return 1;

	}

	if (++(wlpd_p->smon.DolCmdTOChkIvl) == WFO_DOL_CMDTO_CHECK_INTERVAL) {	//check per 3 HM wakeup.
		bool active = 0;

		wlpd_p->smon.DolCmdTOChkIvl = 0;
		dol_check_active(wlpptr, wlpptr->wlpd_p->ipc_session_id,
				 &active);
		//printk("dol_check_active:%u\n", active);
	}
...
//actively monitor wfo(PE) driver rx buffers
#define WFO_RX_BUFF_LEAK_RECOVERY_THRESHOLD  5
#define WFO_RX_BUFF_LEAK_CHECK_INTERVAL      4
static int
wlmon_dol_bm_leak_detect(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct pkt_status pkt_status;
	struct dbg_pkt_cnt pkt_cnt;
	static u32 rxbm_leak[3] = { 0 };	//only monitor 3 bmq ([10~12])
	u8 flag_chk_alloc_cnt = 0;
	int need_recovery = 0, more = 0;
	u32 i;
	...
	{
		//reset the counter
		wlpd_p->smon.WFOBmLeakCnt = 0;
		return 0;

	}

	if (++(wlpd_p->smon.WFOBmLeakCnt) < WFO_RX_BUFF_LEAK_CHECK_INTERVAL)
		return 0;

	wlpd_p->smon.WFOBmLeakCnt = 0;

	if (!dol_get_stats
	    (wlpptr, wlpptr->wlpd_p->ipc_session_id,
	     wlpptr->vmacSta_p->VMacEntry.macId, GET_STATS_PKT_STATUS,
	     &pkt_status, sizeof(struct pkt_status), 0, &more)) {
		//no error
		//printk("pkt_status:%u: %u %u %u\n", 
		//pkt_status.pkt_hdr_free_num, pkt_status.pkt_bmq_free_num[0],pkt_status.pkt_bmq_free_num[1],pkt_status.pkt_bmq_free_num[2]);

		for (i = 0; i < 3; i++) {
			if (pkt_status.pkt_bmq_free_num[i])
				rxbm_leak[i] = 0;
			else
				rxbm_leak[i]++;
		}
		...
		for (i = 0; i < 3; i++) {
			if (rxbm_leak[i] >= WFO_RX_BUFF_LEAK_RECOVERY_THRESHOLD)
				//if(rxbm_leak[i])
			{
				//printk("[HM]: WFO rx Buff might leak, bmq:%u %u %u\n",
				//        rxbm_leak[0],rxbm_leak[1],rxbm_leak[2]);

				//memset(rxbm_leak, 0, sizeof(rxbm_leak));
				flag_chk_alloc_cnt = 1;
				break;
			}
		}
	}

	if (flag_chk_alloc_cnt && !dol_get_stats(wlpptr, wlpptr->wlpd_p->ipc_session_id, wlpptr->vmacSta_p->VMacEntry.macId, GET_STATS_DBG_PKT_CNT, &pkt_cnt, sizeof(struct dbg_pkt_cnt), 1, &more))	//reset counters after get.
	{
		for (i = 0; i < 3; i++) {
			//consider dead only when detecting rx bmq leak and no new alloc cnt increase on the bmq
			if (pkt_cnt.pkt_bmq_free[i] == 0 &&
			    rxbm_leak[i] >=
			    WFO_RX_BUFF_LEAK_RECOVERY_THRESHOLD) {
				printk("Detect BMQ[10+%u] buffer leak: %u\n", i,
				       rxbm_leak[i]);
				need_recovery = 1;
			}
		}
			 ...
			 SMAC_STATUS_st * pCurSmacStatus, void **buf)
{
	...
	if ((wlpptr->wlpd_p->inResetQ == WL_TRUE) ||
		...
		memset(wlpd_p->smon.bcnstucktimestamp, 0x0,
		       sizeof(wlpd_p->smon.bcnstucktimestamp));
		return 0;
	}
...
wlmon_kthread(void *arg)
{
	...
	for (;;) {
		...
		if (wlpd_p->smon.exceptionEvt_rcvd ||
			...
			if (wlpd_p->smon.exceptionEvt_rcvd &&
				...
				if (wlpd_p->smon.active == 0) {
					...
					wldbgCoreMonitor(netdev,
							 WLMON_DEFAULT_ENABLE,
							 WLMON_DEFAULT_HMMASK,
							 SMAC_STATUS_FORMAT_RAW);
				}
		...
		if (wlpd_p->smon.ActiveIf && pCurSmacStatus) {
...
#ifdef WIFI_DATA_OFFLOAD
			//monitor WFO driver cmd timeout
			if (wlmon_dol_cmdTO_detect(netdev)) {

				printk("[HM]:%s Dol Cmd Timeout...\n",
				       wlpd_p->rootdev->name);
			...
			//monitor WFO bmq memory leak
			if (wlmon_dol_bm_leak_detect(netdev)) {
				printk("[HM]:%s WFO Rx Buf Leak...\n",
				       wlpd_p->rootdev->name);
				wlpd_p->smon.exceptionDolHangCause |=
					WFO_DOL_BMQ_LEAK;
