...
*/
void
set_tsf(struct net_device *netdev, uint64_t new_tsf)
{
	...
	if (!wlFwGetTsf(netdev, &tsf_all)) {
		...
		memcpy_fromio(l_Shal_BcnBuff, pShal_BcnBuff,
			      offsetof(HAL_BEACON_st, body));
		start_tsf =
			((uint64_t) (pl_bcnbuf->startTsf[1])) << 32 |
			pl_bcnbuf->startTsf[0];
...
*/
void
adjust_tsf(struct net_device *netdev, int64_t delta)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate *parent_wlpptr = GET_PARENT_PRIV(wlpptr);
	U32 Shal_BcnBuff_val = (parent_wlpptr->bcnBasePtr +
...
*/
void
set_cust_ie(struct net_device *netdev, UINT8 * buf, UINT8 len)
{
	struct wlprivate *priv = NETDEV_PRIV_P(struct wlprivate, netdev);
