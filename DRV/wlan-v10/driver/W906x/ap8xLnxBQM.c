...
void wl_txdatpkt_stat_per_sta(extStaDb_StaInfo_t * pStaInfo,
			      struct sk_buff *skb);
void wl_txdatpkt_bcast_mcast(struct net_device *netdev, ether_hdr_t * eh,
			     struct sk_buff *skb);
...
wlSkbToCfhDl(struct net_device * netdev, struct sk_buff * skb,
	     wltxdesc_t * txcfg, extStaDb_StaInfo_t * pStaInfo, int qid,
	     int type)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
		...
		*((struct sk_buff_head **)skb->cb) =
		...
		if (wlRQIndexPut(qid, &wlqm->rq) == TRUE) {
			...
			if (type == IEEE_TYPE_DATA) {
				...
				wl_txdatpkt_bcast_mcast(netdev,
							(ether_hdr_t *) data_p,
							skb);
			}
...
wlPeToSkb(struct net_device *netdev, bm_pe_hw_t * pe_hw)
{
	...
	if (unlikely(!virt_addr_valid(skb_virt - SKB_INFO_SIZE))) {
		...
		if ((dbg_invalid_skb & (dbg_ivalskb_tx | dbg_ivalskb_rx)) &&
		    (dbg_invalid_skb & dbg_ivalskb_class_1)) {
			WLDBG_ERROR(DBG_LEVEL_0,
	...
	if (bmq_flag) {
		...
		if ((skb->next !=
		     (struct sk_buff *)&wlpd_p->pend_skb_trace[PENDSKB_RX] &&
		     !virt_addr_valid(skb->next)) ||
		    (skb->prev !=
		     (struct sk_buff *)&wlpd_p->pend_skb_trace[PENDSKB_RX] &&
		     !virt_addr_valid(skb->prev))) {
			wlpd_p->except_cnt.skb_notlinked_cnt++;
			if (unlikely
			    ((dbg_invalid_skb & dbg_ivalskb_rx) &&
			     (dbg_invalid_skb & dbg_ivalskb_class_2))) {
				if (!virt_addr_valid(skb->data) ||
				    !virt_addr_valid(skb->data -
						     SKB_INFO_SIZE)) {
					WLDBG_ERROR(DBG_LEVEL_0,
...
#ifdef WIFI_DATA_OFFLOAD
	dol_stop_radio(wlpptr, wlpptr->wlpd_p->ipc_session_id);
#endif
...
wlQMInit(struct net_device *netdev)
{
	...
	for (qid = SC5_BMQ_START_INDEX;
		...
		if (!wlpptr->wlpd_p->dol.disable)
			if (qid != (SC5_BMQ_START_INDEX + SC5_BMQ_NUM - 1)) {
				wlpptr->smacconfig.bpReqInfo[qid -
							     SC5_BMQ_START_INDEX].
					size =
					buf_pool_size[qid -
						      SC5_BMQ_START_INDEX] / 16;
				wlpptr->smacconfig.bpReqInfo[qid -
							     SC5_BMQ_START_INDEX].
					bpid = qid;
				wlpptr->smacconfig.bpReqInfo[qid -
...
#ifdef WIFI_DATA_OFFLOAD
	dol_start_radio(wlpptr, wlpptr->wlpd_p->ipc_session_id);
#endif
...
*/
void
wl_txdatpkt_bcast_mcast(struct net_device *netdev, ether_hdr_t * eh,
			struct sk_buff *skb)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
...
#define PKT_SIGNATURE              0x424B5324
	...
	if (!wlpd_p->dol.disable) {
		u32 aligned_offset;
		u64 pkthdr, *pre_pkthdr;
		static u64 prom_pre_pkthdr = 0, mgmt_pre_pkthdr = 0;

		do {
			if ((rxQid != SC5_RXQ_PROMISCUOUS_INDEX) &&
			    (rxQid != SC5_RXQ_MGMT_INDEX)) {
				printk("%s: not promiscuous/mangement rx queue: %d\n", __func__, rxQid);
				goto destroy;
			}
			aligned_offset =
				((u64) (skb_addr - PKT_INFO_SIZE) & 0x3);
#ifndef DISABLE_SIGNATURE_CHECK_MGMT
			if (*
			    ((u32 *) (skb_addr - PKT_INFO_SIZE -
				      aligned_offset)) != PKT_SIGNATURE) {
				printk("%s: invalid signature %08x\n", __func__,
				       *((u32 *) (skb_addr - PKT_INFO_SIZE -
						  aligned_offset)));
				goto destroy;
			} else
#endif
			{
				skb = wl_alloc_skb(cfh_ul->hdr.length);
				if (!skb) {
					printk("%s: no skbuff available\n",
					       __func__);
					goto destroy;
				}
				memcpy(skb->data, skb_addr, cfh_ul->hdr.length);
				if (rxQid == SC5_RXQ_PROMISCUOUS_INDEX)
					pre_pkthdr = &prom_pre_pkthdr;
				else
					pre_pkthdr = &mgmt_pre_pkthdr;
				pkthdr = *((u64 *) (skb_addr -
						    PKT_POINTER_OFFSET -
						    aligned_offset));
				if (*pre_pkthdr) {
					if (pkthdr != *pre_pkthdr) {
						dol_radio_return_buffer(wlpptr,
									wlpptr->
									wlpd_p->
									ipc_session_id,
									*pre_pkthdr);
...
destroy:
...
#endif
		if (cfh_ul->fpkt == 1)
			wlUnmapBuffer(netdev, (u8 *) skb_addr,
				      wlqm->rq.bm.buf_size);
	...
	if (!wlpd_p->dol.disable)
		...
		if (skb_c->tail + (cfh_ul->hdr.length - offset) > skb_c->end) {
			...
			if (unlikely(skb_c->data_len)) {
				if ((dbg_invalid_skb & dbg_ivalskb_rx) &&
				    (dbg_invalid_skb & dbg_ivalskb_class_3)) {
					WLDBG_ERROR(DBG_LEVEL_0,
	...
	if (cfh_ul->lpkt == 1) {	// free bit is on, last packet
		...
		if (!wlpd_p->dol.disable)
			...
			wl_clr_last_rxskb(netdev,
					  (wlqm_id - SC5_BMQ_START_INDEX));
			pe->skb = NULL;
#ifdef WIFI_DATA_OFFLOAD
		}
#endif
...
//              pskb = skb_dequeue(&txQTrace);
		...
		cfh_dl = wlSkbToCfhDl(netdev, pskb, &cfg, pStaInfo, qid,
				      IEEE_TYPE_DATA);
