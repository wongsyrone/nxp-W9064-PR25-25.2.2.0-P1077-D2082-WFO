...
mwl_send_vendor_assoc_event(struct net_device *netdev, uint8_t * macaddr)
{
	...
	if (!ret) {
		...
		if (nla_put(skb, MWL_VENDOR_ATTR_IE_MAC, ETH_ALEN, macaddr) ||
		    ...
		    nla_put_u8(skb, MWL_VENDOR_ATTR_IE_REASSOC, IEReq.reassoc)
		    || nla_put(skb, MWL_VENDOR_ATTR_IE_DATA, IEReq.IELen,
			       IEReq.IE)) {
			kfree_skb(skb);
			return -EMSGSIZE;
		}
...
int
mwl_send_vendor_neighbor_event(struct net_device *netdev, void *buf,
			       size_t len, uint8_t add)
{
...
int
mwl_send_vendor_assoc_notification(struct net_device *netdev, uint8_t * macaddr,
				   void *assocReqMsg, uint16_t reason_code)
{
	struct wlprivate *vif = mwl_netdev_get_priv(netdev);
	...
	skb = cfg80211_vendor_event_alloc(vif->wiphy, &vif->wdev, (size + 20),
	...
	if (nla_put(skb, MWL_VENDOR_ATTR_MAC_ADDRESS, ETH_ALEN, macaddr) ||
	    nla_put(skb, MWL_VENDOR_ATTR_ASSOC_REQ_FRAME, size, assocReqFrame)
	    || nla_put_u16(skb, MWL_VENDOR_ATTR_REASON_CODE, reason_code)) {
		kfree_skb(skb);
		return -EMSGSIZE;
	}
...
int
mwl_send_vendor_disassoc_notification(struct net_device *netdev,
				      uint8_t * macaddr,
				      ...
				      uint16_t reason_code)
{
	...
	if (nla_put(skb, MWL_VENDOR_ATTR_MAC_ADDRESS, ETH_ALEN, macaddr) ||
	    nla_put(skb, MWL_VENDOR_ATTR_STATION_INFO, size, station_info) ||
	    nla_put_u16(skb, MWL_VENDOR_ATTR_REASON_CODE, reason_code)) {
		kfree_skb(skb);
		return -EMSGSIZE;
	}
...
int
mwl_send_vendor_unassocsta_event(struct net_device *netdev, void *resp)
{
	struct wlprivate *vif = mwl_netdev_get_priv(netdev);
	...
	skb = cfg80211_vendor_event_alloc(vif->wiphy, &vif->wdev,
					  (UNASSOC_RESP_SIZE + 20),
	...
	if (nla_put(skb, MWL_VENDOR_ATTR_UNASSOCIATED_STA_LINK_METRICS_RESP,
		    UNASSOC_RESP_SIZE, unassociated_resp)) {
		kfree_skb(skb);
		return -EMSGSIZE;
	}
...
int
mwl_send_vendor_multiapIE_event(struct net_device *netdev, uint8_t * macaddr,
				IEEEtypes_MultiAP_Element_t * multiapIE,
				UINT16 iesize)
{
	struct wlprivate *vif = mwl_netdev_get_priv(netdev);
	struct sk_buff *skb;
	...
	skb = cfg80211_vendor_event_alloc(vif->wiphy, &vif->wdev, size,
	...
	if (nla_put(skb, MWL_VENDOR_ATTR_MAC_ADDRESS, ETH_ALEN, macaddr) ||
	    nla_put(skb, MWL_VENDOR_ATTR_MULTIAP_IE, iesize, multiapIE)) {
		kfree_skb(skb);
		return -EMSGSIZE;
	}
