...
int wl_bad_mic_handler(struct wlprivate *wlpptr, vmacApInfo_t *vmacSta_p)
{
	...
	else if (vmacSta_p->OpMode == WL_OP_MODE_VAP || vmacSta_p->OpMode == WL_OP_MODE_AP)
	{ // for 11n-cert 4.2.13
...
#endif
{
...
#endif
	{
		...
		wmb();
		pRxDesc =
			&wlpptr->wlpd_p->descData[0].pRxRing[pRxRingDone->User & 0x3fff];
		if (pRxDesc->Data == 0)
			wlTxDescriptorDump(netdev);
		pRxSkBuff =
			wlpptr->wlpd_p->descData[0].Rx_vBufList[pRxRingDone->User & 0x3fff];
		...
		{
			u_int16_t pktLen =
				(pRxSkBuff->data[12] << 8 | pRxSkBuff->data[13]);
		...
		default:
		{
			...
			if (pRxSkBuff != NULL)
			{
				...
				pHdr = (IEEEtypes_GenHdr_t *)&pRxInfo->Hdr[0];
				frmLen = ENDIAN_SWAP16(pHdr->FrmBodyLen);

				if (pHdr->FrmCtl.Wep && removePN)
				{
					keyType =
						(ENDIAN_SWAP32(pRxRingDone->Ctrl) >>
						 rxring_Ctrl_KEYshift) &
						rxring_Ctrl_KEYmask;

					if ((keyType == key_type_WEP40) || (keyType ==
														key_type_WEP104))
					{
						frmLen -= 4;
						weplenadjust = 4;
