			 ...
			 struct cfg80211_chan_def *chandef)
{
	...
	if (wlpptr->cardindex == 0) {
		...
		for (j = 0; j < mwl_band_50.n_channels; j++) {
			...
			if (wlpptr->channel.hw_value ==
			    mwl_channels_50[j].hw_value) {
			  ...
			  u8 * mac, struct station_info *sinfo)
{
	...
	if (entries) {
		...
		if (NULL ==
		    (pStaInfo =
		     extStaDb_GetStaInfo(vmacSta_p,
					 (IEEEtypes_MacAddr_t *) show_buf, 0)))
			return -ENODATA;
			      ...
			      struct cfg80211_chan_def *chandef)
{
	...
	return mwl_drv_set_channel(priv->netDev, chandef->chan->hw_value);
}
		      ...
		      struct cfg80211_ap_settings *params)
{
	...
	if (params->beacon_interval)
		mwl_drv_set_bcninterval(priv->master, params->beacon_interval);

	if (params->ssid && params->ssid_len && params->ssid_len <= 32)
		mwl_drv_set_ssid(dev, params->ssid, params->ssid_len);
		     ...
		     struct cfg80211_connect_params *sme)
{
	...
	if (sme->ssid && sme->ssid_len && sme->ssid_len <= 32)
		mwl_drv_set_ssid(dev, sme->ssid, sme->ssid_len);

	if (sme->channel && sme->channel->center_freq) {
		u8 channel =
			ieee80211_frequency_to_channel(sme->channel->
						       center_freq);
		mwl_drv_set_channel(priv->master, channel);
	}
			...
			u16 reason_code)
{
	...
	if (vmacEntry_p != NULL) {
		...
		return mwl_drv_send_mlme(dev, &mlme);
	} else
		return 0;
}
		     ...
		     struct key_params *params)
{
	...
	u64 key_xmit_seq = 0;
	u16 key_idx = 0;
	u8 key_pn[6] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
	u8 macaddr[6] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
	u8 key_data[WLAN_MAX_KEY_LEN] = { 0x00 };
	...
	key_flag = MWL_KEY_RECV | MWL_KEY_XMIT;
	if (mac_addr == NULL || is_broadcast_ether_addr(mac_addr)) {
		memset(macaddr, 0xff, ETH_ALEN);
		key_flag |= MWL_KEY_DEFAULT;
		key_idx = (u16) key_index;
	} else {
		memcpy(macaddr, mac_addr, ETH_ALEN);
		key_idx = MWL_KEYIX_NONE;
	}

	memset(key_data, 0, WLAN_MAX_KEY_LEN);
	memcpy(key_data, params->key, params->key_len);
	memcpy(key_pn, params->seq, params->seq_len);
	memcpy(&key_xmit_seq, params->seq, params->seq_len);

	return mwl_drv_set_key(netdev, cipher, key_idx, params->key_len,
			       key_flag, macaddr, key_recv_seq, key_xmit_seq,
			       key_data, key_pn);
}
		     ...
		     u8 key_index, bool pairwise, const u8 * mac_addr)
{
	...
	return mwl_drv_del_key(netdev, key_index, macaddr);
}
						     ...
						     struct key_params *))
{
	...
	ret = mwl_drv_get_seqnum(dev, seqnum);
	if (!ret) {
		params.seq = seqnum;
...
int
mwl_cfg80211_create(struct wlprivate *wlpptr)
{
	int ret;
	void *wdev_priv;
	struct wiphy *wiphy;
	...
	if (wlpptr->cardindex == 0) {
		...
		wiphy->bands[NL80211_BAND_5GHZ]->ht_cap.cap = (1 << 1);
		wiphy->bands[NL80211_BAND_5GHZ]->ht_cap.ht_supported = 1;
		wiphy->bands[NL80211_BAND_5GHZ]->vht_cap.cap = (1 << 2);
		wiphy->bands[NL80211_BAND_5GHZ]->vht_cap.vht_supported = 1;
	} else {
		wiphy->bands[NL80211_BAND_2GHZ] = &mwl_band_24;
		/* Set parameters according to the requirement for iwinfo */
		wiphy->bands[NL80211_BAND_2GHZ]->ht_cap.cap = (1 << 1);
		wiphy->bands[NL80211_BAND_2GHZ]->ht_cap.ht_supported = 1;
		/* Treat any nonzero capability as 11ac */
		wiphy->bands[NL80211_BAND_2GHZ]->vht_cap.cap = (1 << 1);
		wiphy->bands[NL80211_BAND_2GHZ]->vht_cap.vht_supported = 1;
	}
	...
	*(unsigned long *)wdev_priv = (unsigned long)wlpptr;
	...
	set_wiphy_dev(wiphy, (struct device *)&wlpptr->pPciDev->dev);
...
mwl_cfg80211_assoc_event(struct net_device *netdev, uint8_t * macaddr)
{
	...
	if (!ret) {
		...
		sinfo.assoc_req_ies = ie;
		sinfo.assoc_req_ies_len = ie_len;
		cfg80211_new_sta(netdev, macaddr, &sinfo, GFP_KERNEL);
	}
...
mwl_cfg80211_disassoc_event(struct net_device *netdev, uint8_t * macaddr)
{
	...
	cfg80211_del_sta(netdev, macaddr, GFP_KERNEL);
