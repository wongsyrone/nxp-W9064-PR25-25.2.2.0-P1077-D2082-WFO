...
int
wlTxRingAlloc(struct net_device *netdev)
{
	...
	wlpptr->wlpd_p->descData[0].pPhysTxRingDone =
		(dma_addr_t) ((UINT32) wlpptr->wlpd_p->descData[0].
			      pPhysTxRingDone);
...
#ifdef NEW_DP
int
wlTxRingInit(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	int currDescr, i;
		...
		if (wlpptr->wlpd_p->descData[num].pTxRing != NULL) {
			for (currDescr = 0; currDescr < MAX_NUM_TX_DESC;
				...
				CURR_TXD(num).pPhysNext =
					ENDIAN_SWAP32((u_int32_t) wlpptr->
						      wlpd_p->descData[num].
						      pPhysTxRing +
						      ((currDescr +
							1) *
				...
				WLDBG_INFO(DBG_LEVEL_12,
					   ...
					   CURR_TXD(num).pNext,
					   ENDIAN_SWAP32(CURR_TXD(num).
							 pPhysNext));
			}
			LAST_TXD(num).pNext = &FIRST_TXD(num);
			LAST_TXD(num).pPhysNext =
				ENDIAN_SWAP32((u_int32_t) wlpptr->wlpd_p->
					      descData[num].pPhysTxRing);
...
void
wlTxRingCleanup(struct net_device *netdev)
{
#ifdef NEW_DP
	wlTxRingInit(netdev);
#else
...
void
wlRxDescriptorDump(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
...
void
wlTxDescriptorDump(struct net_device *netdev)
{
#ifndef NEW_DP
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	int currDescr;
		...
		if (wlpptr->wlpd_p->descData[num].pTxRing != NULL) {
			for (currDescr = 0; currDescr < MAX_NUM_TX_DESC;
				...
				if ((UINT32) & CURR_TXD(num) ==
				    (UINT32) wlpptr->wlpd_p->descData[num].
				    pStaleTxDesc) {
					p1 = str1;
				}
				if ((UINT32) & CURR_TXD(num) ==
...
int
wlRxRingAlloc(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
...
int
wlRxRingInit(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	int currDescr;
...
wlRxRingInit(struct net_device *netdev)
{
	...
	if (wlpptr->wlpd_p->descData[0].pRxRing != NULL) {
		...
		LAST_RXD.pPhysNext =
			ENDIAN_SWAP32((u_int32_t) wlpptr->wlpd_p->descData[0].
				      pPhysRxRing);
...
void
wlRxRingCleanup(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	int currDescr;
	...
	if (wlpptr->wlpd_p->descData[0].pRxRing != NULL) {
		for (currDescr = 0; currDescr < MAX_NUM_RX_DESC; currDescr++) {
#ifdef NEW_DP
			if (wlpptr->wlpd_p->descData[0].
			    Rx_vBufList[currDescr] != NULL) {
				pci_unmap_single(wlpptr->pPciDev,
						 ENDIAN_SWAP32(CURR_RXD.Data),
						 wlpptr->wlpd_p->descData[0].
						 rxBufSize, PCI_DMA_FROMDEVICE);
				wl_free_skb(wlpptr->wlpd_p->descData[0].
					    Rx_vBufList[currDescr]);
			}
#else
			if (CURR_RXD.pSkBuff != NULL) {
				...
				if (skb_shinfo(CURR_RXD.pSkBuff)->frag_list) {
					skb_shinfo(CURR_RXD.pSkBuff)->
						frag_list = NULL;
				}
				pci_unmap_single(wlpptr->pPciDev,
						 ENDIAN_SWAP32(CURR_RXD.
							       pPhysBuffData),
						 wlpptr->wlpd_p->descData[0].
						 rxBufSize, PCI_DMA_FROMDEVICE);
				wl_free_skb(CURR_RXD.pSkBuff);
...
void
wlTxRingFree(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	int num;
	WLDBG_ENTER(DBG_LEVEL_12);
...
void
wlRxRingFree(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
