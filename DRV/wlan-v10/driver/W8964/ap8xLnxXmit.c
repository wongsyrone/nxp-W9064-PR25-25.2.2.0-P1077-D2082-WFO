	...
	// #endif /* NEW_DP */
	{
			 ...
			 VMAC_MODE_CLNT_INFRA))
		{
			...
			if ((pEth->ether_type ==
				 ...
				 IS_GROUP((UINT8 *)&(pEth->ether_dhost)) && (ipheader->protocol != IPPROTO_IGMP)) ||
				(pEth->ether_type ==
					 (UINT16)ETHERTYPE_IP_NW &&
				 IS_GROUP((UINT8 *)&(pEth->ether_dhost)) && (ipheader->protocol == IPPROTO_IGMP) &&
				 igmpheader->type ==
					 IGMP_HOST_MEMBERSHIP_QUERY))
			{
				...
				dIPAddr =
					WORD_SWAP(*((UINT32 *)IPv4_p->dst_IP_addr));
				 ...
				 i++)
			{
					...
					*(mib->mib_IPMFilteredAddress[i]))
				{
					...
					WLDBG_INC_TX_OK_CNT(skb,
										Priority);
					WLDBG_REC_TX_Q_DEPTH(wlpptr->wlpd_p->txQ
											 [Priority]
												 .qlen,
										 Priority);
					skb->dev = netdev;
				...
				if (skb != NULL)
				{
						 ...
						 i++)
					{
						...
						if (((extStaDb_GetStaInfo(vmacSta_p, &(mib->mib_IPMcastGrpTbl[IPMcastGrpIndex]->mib_UCastAddr[i]), 1)) != NULL) &&
							!(memcmp((char *)&mib->mib_IPMcastGrpTbl
										 [IPMcastGrpIndex]
											 ->mib_UCastAddr[i],
									 (char *)
										 pEthHdrMcast->ether_shost,
									 6) == 0))
						{
							...
							skbCopy->dev =
								netdev;
							skb_queue_tail(&wlpptr->wlpd_p->txQ
												[Priority],
										   skbCopy);
							schedulepkt =
								TRUE;
...
#endif /* MPRXY */
		{
			...
			WLDBG_REC_TX_Q_DEPTH(wlpptr->wlpd_p->txQ[Priority].qlen,
								 Priority);
			skb->dev = netdev;
#ifdef TCP_ACK_ENHANCEMENT
			{
		...
		int stadb_flag =
			...
			pStaInfo =
				extStaDb_GetStaInfo(wlpptr->vmacSta_p,
									...
									stadb_flag);
#ifdef MULTI_AP_SUPPORT
			if (vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_AP && (mib->multi_ap_attr & MAP_ATTRIBUTE_BACKHAUL_BSS) && pStaInfo == NULL)
			{
				UINT8 bssId[6];
				UINT8 found = 0;
				MultiAP_4Addr_Entry_t *entry;
		...
		if (pStaInfo != NULL)
		{
			...
			if (vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_AP)
			{
				...
				if ((mib->Privacy->RSNEnabled) &&
					(pStaInfo->keyMgmtStateInfo.RSNDataTrafficEnabled == 0) && (eapolPkt == FALSE))
					goto error1;
			}
		}
#endif
	...
	if (vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_CLNT_INFRA)
	{
		pStaInfo =
			extStaDb_GetStaInfo(vmacSta_p,
								(IEEEtypes_MacAddr_t *)
									GetParentStaBSSID(vmacSta_p->VMacEntry.phyHwMacIndx),
								1);
#ifdef MRVL_WPS_CLIENT
#ifdef MRVL_WAPI
		eapolPkt = ((pEth->ether_type == IEEE_ETHERTYPE_PAE) ||
					(pEth->ether_type == ETH_P_WAPI))
					   ? TRUE
					   : FALSE;
#else
		eapolPkt =
...
void wlTxDone(struct net_device *netdev)
{
	...
	txDoneTail =
		wlpptr->wlpd_p->descData[0].TxDoneTail & (MAX_TX_RING_DONE_SIZE - 1);
				...
				if (netdev_v)
				{
					...
					if (mbss_idx < MAX_VMAC_INSTANCE_AP)
					{
							 ...
							 wds_idx++)
						{
							struct net_device
								*netdev_v_wds =
									wlpptr_v->vmacSta_p->wdsPort
										[wds_idx]
											.netDevWds;

							if (netdev_v_wds &&
								netif_running(netdev_v_wds) &&
								netif_queue_stopped(netdev_v_wds))
								netif_wake_queue(netdev_v_wds);
						}
					}
#endif
				}
...
void wlTxDone(struct net_device *netdev)
{
	...
	SPIN_LOCK_IRQSAVE(&wlpptr->wlpd_p->locks.xmitLock, flags);
	{
		for (num = 0; num < NUM_OF_DESCRIPTOR_DATA; num++)
		{
			...
			while ((CURR_TXD(num)) && (CURR_TXD(num)->Status & ENDIAN_SWAP32(EAGLE_TXD_STATUS_OK)) &&
				   (!(CURR_TXD(num)->Status &
				...
				if (CURR_TXD(num)->multiframes)
				{
						 ...
						 i++)
					{
						pci_unmap_single(wlpptr->pPciDev,
										 ENDIAN_SWAP32(CURR_TXD(num)->PktPtrArray
														   [i]),
								...
								IEEE_TYPE_DATA)
						{
							...
							pStaInfo->RateInfo =
								*(dbRateInfo_t
									  *)&(CURR_TXD(num)->RateInfo);

							if (!pStaInfo->RateInfo.Format ||
								(pStaInfo->RateInfo.Format &&
								 pStaInfo->RateInfo.RateIDMCS == 0))
						...
						if (CURR_TXD(num)->pSkBuffArray[i])
							wl_free_skb(CURR_TXD(num)->pSkBuffArray
											[i]);
						...
						pStaInfo =
							(extStaDb_StaInfo_t *)
								CURR_TXD(num)
									->staInfo;
						pStaInfo->RateInfo =
							*(dbRateInfo_t *)&(CURR_TXD(num)->RateInfo);
						if (!pStaInfo->RateInfo.Format || (pStaInfo->RateInfo.Format &&
														   pStaInfo->RateInfo.RateIDMCS == 0))
							pStaInfo->aggr11n.threshold = 0;
						else
							pStaInfo->aggr11n.threshold =
...
int wlxmit(struct net_device *netdev, struct sk_buff *skb, UINT8 typeAndBits,
		   extStaDb_StaInfo_t *pStaInfo, UINT32 bcast, BOOLEAN eap)
{
	...
	if (type == IEEE_TYPE_DATA)
	{
		...
		if (ieee80211_hdr->FrmCtl.Subtype == QoS_DATA)
			*(UINT16 *)&QosControl =
				*(UINT16 *)&skb->data[NBR_BYTES_IEEE80211HEADER];
		else
			*(UINT16 *)&QosControl = 0;
			...
			if (mib->Privacy->RSNEnabled /*&& pStaInfo */)
			{
				...
				if (bcast)
				{
					if ((!(mib->RSNConfigWPA2->WPA2Enabled ||
						 ...
						 mib_RSNConfig_p->MulticastCipher[3] == 4) ||
						((mib->RSNConfigWPA2->WPA2Enabled ||
						  mib->RSNConfigWPA2->WPA2OnlyEnabled) &&
						 mib->RSNConfigWPA2->MulticastCipher[3] == 4))
					{
				...
				else
				{
					...
					if (pStaInfo)
					{
						if ((pStaInfo->keyMgmtStateInfo.RsnIEBuf[0] == 48 &&
							 pStaInfo->keyMgmtStateInfo.RsnIEBuf[13] == 4) ||
							(pStaInfo->keyMgmtStateInfo.RsnIEBuf[0] == 221 &&
							 pStaInfo->keyMgmtStateInfo.RsnIEBuf[17] == 4))
						{
...
#define SRC_ADD_OFFSET 16
			...
			else if (bcast)
			{
				...
				if ((payload =
						 macMgmtBIP(vmacSta_p,
									(macmgmtQ_MgmtMsg2_t *)&skb->data[0], payload)) != 0)
				{
					printk("%s:\n", vmacSta_p->dev->name);
	...
	if (type == IEEE_TYPE_DATA)
	{
		if (!eap)
		{
			if (!bcast)
			{
				if (ucastCipherCCMP)
				{
					InsertCCMPHdr((UINT8 *)&skb->data
									  [NBR_BYTES_IEEE80211HEADER + 2 + 6],
								  0,
			...
			if ((wep_padding == 18) && mib->Privacy->WAPIEnabled)
			{
				...
				INSERT_WAPIHDR((UINT32 *)&skb->data[NBR_BYTES_IEEE80211HEADER +
													2 + 6 + 2],
							   pn);
			}
#endif
	...
	else
	{
		if (type == IEEE_TYPE_MANAGEMENT)
			...
			{
				InsertCCMPHdr((UINT8 *)&skb->data[NBR_BYTES_IEEE80211HEADER +
												  2 + 6],
							  0,
							  pStaInfo->keyMgmtStateInfo.TxIV16,
							  pStaInfo->keyMgmtStateInfo.TxIV32);
...
#define BA_QUEUE 5
		...
		{
			if ((memcmp((UINT8 *)&wlpptr->wlpd_p->Ampdu_tx[i].MacAddr[0], ampduMacAddr, 6) == 0) && wlpptr->wlpd_p->Ampdu_tx[i].InUse &&
				(QosControl.tid ==
	...
	if ((memcmp((UINT8 *)&wlpptr->wlpd_p->Ampdu_tx[0].MacAddr[0], ampduMacAddr,
				6) == 0) &&
	...
	else if ((memcmp((UINT8 *)&wlpptr->wlpd_p->Ampdu_tx[1].MacAddr[0],
					 ampduMacAddr, 6) == 0) &&
...
newdp_data:
	...
	if (type == IEEE_TYPE_MANAGEMENT)
	{
		...
		else
		{
			...
			if ((macid == NUMOFAPS &&
				...
				(pStaInfo == NULL) || (ieee80211_hdr->FrmCtl.Subtype == IEEE_MSG_ASSOCIATE_RSP) || (ieee80211_hdr->FrmCtl.Subtype == IEEE_MSG_REASSOCIATE_RSP) || (ieee80211_hdr->FrmCtl.Subtype == IEEE_MSG_AUTHENTICATE))
			{
		...
		else
		{
			...
			pNextTxDesc->u.RateCode =
				RateCode_Type_Legacy +
				(RateCode_Legacy_1MBPS_Short << RateCode_MCS_SHIFT) + RateCode_BW_20MHZ;
		}
	...
	{ // no 802.11 header
				...
				if (isDmsFlag)
				{
...
#endif
					{
						...
						else if (mib->Privacy->RSNEnabled) // tkip/ccmp/gcmp
						{
							...
							if (((pDmsStaInfo->keyMgmtStateInfo.RsnIEBuf[0] == 48) && (pDmsStaInfo->keyMgmtStateInfo.RsnIEBuf[13] ==
																					   2)) ||
								((pDmsStaInfo->keyMgmtStateInfo.RsnIEBuf[0] ==
								  221) &&
								 (pDmsStaInfo->keyMgmtStateInfo.RsnIEBuf[17] ==
								  2)))
								mic_pedding = 12; // tkip
							else if (((pDmsStaInfo->keyMgmtStateInfo.RsnIEBuf[0] == 48) &&
									  (pDmsStaInfo->keyMgmtStateInfo.RsnIEBuf[13] == 4)) ||
									 ((pDmsStaInfo->keyMgmtStateInfo.RsnIEBuf[0] == 221) &&
									  (pDmsStaInfo->keyMgmtStateInfo.RsnIEBuf[17] == 4)))
								mic_pedding = 8; // ccmp
...
#ifdef MRVL_WAPI
						else if (mib->Privacy->WAPIEnabled)
						{
							insert_iv = 18;
							mic_pedding = 16;
						}
		...
		if (skb->cb[1] == 2) // TCP ack frame, might be drop in fw
		{
			...
			ctrl |= (txring_Ctrl_TAG_TCP_ACK << txring_Ctrl_TAGshift);
		}
#endif
		...
		pNextTxDesc->Data =
			ENDIAN_SWAP32(pci_map_single(wlpptr->pPciDev, skb->data, skb->len,
										 PCI_DMA_TODEVICE));
		pNextTxDesc->Ctrl =
			ENDIAN_SWAP32(((TxQueuePriority & txring_Ctrl_QIDmask)
						   << txring_Ctrl_QIDshift) |
...
#ifdef ZERO_COPY
int wlxmitmfs(struct net_device *netdev, struct sk_buff *skb[],
...
#define SRC_ADD_OFFSET 16
	...
	*pFwLen =
	...
	if (type == IEEE_TYPE_DATA)
	{
		if (!eap)
		{
			if (!bcast)
			{
				if (ucastCipherCCMP)
				{
					InsertCCMPHdr((UINT8 *)&skb[0]->data
									  [NBR_BYTES_IEEE80211HEADER + 2 + 6],
								  0,
								  pStaInfo->keyMgmtStateInfo.TxIV16,
								  pStaInfo->keyMgmtStateInfo.TxIV32);
	...
	for (i = 0; i < nof; i++)
	{
		wlpptr->wlpd_p->descData[TxDesIndex].pNextTxDesc->PktLenArray[i] = ENDIAN_SWAP16(skb[i]->len);
...
int wlDataTxUnencr(struct sk_buff *skb, struct net_device *netdev,
				   extStaDb_StaInfo_t *pStaInfo)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	unsigned long flags;
