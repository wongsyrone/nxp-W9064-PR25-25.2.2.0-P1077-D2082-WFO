...
SMAC_RX_DISABLE(struct wlprivate *wlpptr, MIB_802DOT11 *mib, UINT32 macId)
{
	...
	if (mib && (*(mib->mib_rx_enable) == 0) &&
		(*(u32 *)(&((SMAC_CTRL_BLK_st *)wlpptr->ioBase0)->config.opMode) == 0))
	{
...
//    printk("  -->pPhysAcntRing   = %pad\n", &wlpptr->wlpd_p->descData[0].pPhysAcntRing);      //0316
	...
	{
		pCmd->AcntBaseAddr[m] =
			ENDIAN_SWAP32(wlpptr->wlpd_p->descData[0].pPhysAcntRing[m]);
	}
...
int wlFwGetEncrInfo(struct net_device *netdev, unsigned char *addr)
{
	...
	else if (p->EnHwEncr == 0)
	{
		...
		printk("%4s	%s	%s	%s	%s	%s	%s	%s\n", encrnames[p->EnHwEncr & 7], p->key.gWepKeyData.LoopBackOn ? "ena" : "-",
		...
		{
			printk("[%2d]",
				   p->key.gWepKeyData.WepDefaultKeys[i].WepDefaultKeyIdx);
			for (j = 0; j < 13; j++)
				printk(" %02x",
					   p->key.gWepKeyData.WepDefaultKeys[i].WepDefaultKeyValue[j]);
			printk("\n");
		}
	}
	else
	{
		...
		printk("%4s	%s	%s		%s	%s		%d\n", encrnames[p->EnHwEncr & 7], p->key.gKeyData.UnicastKeyEnabled ? "ena" : "-",
			   p->key.gKeyData.UnicastKeyEnabled ? ciphernames[p->key.gKeyData.UnicastKeyType & 7] : "-", p->key.gKeyData.MulticastKeyEnabled ? "ena" : "-",
...
int wlFwSetCoexConfig(struct net_device *netdev, u8 *enable, u8 *gpioLevelDetect,
		...
		for (rate_id = 0; rate_id < 14; rate_id++)
		{
			if (rateMask & 0x01)
				pCmd->BasicRate[basic_rate_id] =
					mib->StationConfig->OpRateSet[rate_id] & 0x7F;
			basic_rate_id++;
...
#ifdef SOC_W906X
int wlFwSetChannelSwitchIE(struct net_device *netdev, UINT16 nextChannel,
						   UINT16 secChan, UINT32 mode, UINT32 count,
						   CHNL_FLAGS Chanflag)
#else
int wlFwSetChannelSwitchIE(struct net_device *netdev, UINT32 nextChannel,
...
#endif
{
	...
	if ((pCmd->ChannelFlags.ChnlWidth == CH_40_MHz_WIDTH) ||
		...
		pCmd->NextHTExtChnlOffset =
			macMgmtMlme_Get40MHzExtChannelOffset(pCmd->Next11hChannel);
	else
	...
	if (pCmd->ChannelFlags.ChnlWidth == CH_80_MHz_WIDTH)
		pCmd->ChannelFlags.ActPrimary =
			macMgmtMlme_Get80MHzPrimaryChannelOffset(pCmd->Next11hChannel);

	if (pCmd->ChannelFlags.ChnlWidth == CH_160_MHz_WIDTH)
		pCmd->ChannelFlags.ActPrimary =
			macMgmtMlme_Get160MHzPrimaryChannelOffset(pCmd->Next11hChannel);
#ifdef MV_CPU_BE
...
int wlFwSetPowerConstraint(struct net_device *netdev, UINT32 powerConstraint)
{
	...
	{
		macMgmtMlme_UpdateProbeRspBasicIes(wlpptr->vmacSta_p,
										   (UINT8 *)&pCmd->VHTTransmitPowerEnvelopeElement,
										   pCmd->VHTTransmitPowerEnvelopeElement.Len + 2);
	}
...
int wlFwSetQuiet(struct net_device *netdev, UINT8 enable, UINT8 period,
...
int wlFwSetRate(struct net_device *netdev, wlrate_e rate)
{
	...
	if (*(mib->mib_enableFixedRateTx) == 2)
	{
		...
		if (*(mib->mib_FixedRateTxType) == 0x4)
		{ // use rateinfo directly
	...
	else if (*(mib->mib_enableFixedRateTx) == 1)
	{
		...
		pCmd->FixedRateTable[0].FixedRate =
			ENDIAN_SWAP32(*(mib->mib_FixedRateTxType) ? *(mib->mib_txDataRateN) : *(mib->mib_txDataRate));
		...
		for (i = 0; i < 4; i++)
		{
			...
			pCmd->FixedRateTable[i].FixRateTypeFlags.RetryCountValid =
...
int wlFwSetTxPower(struct net_device *netdev, UINT8 flag, UINT32 powerLevel)
{
	...
	for (i = 0; i < TX_POWER_LEVEL_TOTAL; i++)
	{
		if (found)
		{
			if ((mibS->PhyTXPowerTable[index]->setcap) && (mibS->PhyTXPowerTable[index]->TxPower[i] >
														   mibS->PhyDSSSTable->maxTxPow[i]))
...
int wlFwSetTxPowerClientScan(struct net_device *netdev, UINT8 flag,
							 UINT32 powerLevel, UINT16 channel)
{
	...
	else if (flag == HostCmd_ACT_GEN_SET_LIST)
	{
		...
		if (chan <= 14)
		{ /* BG case and Channel 14 has been forced to 20M in wlset_freq() */
					...
					(ChanBw == CH_AUTO_WIDTH))
				{
					if (chan > 9)
						pCmd->PowerLevelList[i] =
							mib->PowerTagetRateTable40M[(chan - 5) * 4 + i];
					else
						pCmd->PowerLevelList[i] =
							mib->PowerTagetRateTable40M[(chan - 1) * 4 + i];
				}
				else
					pCmd->PowerLevelList[i] =
						mib->PowerTagetRateTable20M[(chan -
													 1) *
														4 +
													i];
#ifdef PWRFRAC
						   ...
						   UINT8 keyType, UINT8 keyIndex)
{
	...
	if (*(mib->mib_STAMode))
		macAddr =
			GetParentStaBSSID(wlpptr->vmacSta_p->VMacEntry.phyHwMacIndx);
...
int wlFwSetWpaWpa2PWK(struct net_device *netdev, extStaDb_StaInfo_t *StaInfo_p)
{
		 ...
		 StaInfo_p->keyMgmtStateInfo.RsnIEBuf[13] == 2))
	{
		...
		tkipParam.Tsc.high = StaInfo_p->keyMgmtStateInfo.TxIV32;
			 ...
			 (StaInfo_p->keyMgmtStateInfo.RsnIEBuf[0] == 48 &&
			  isAes4RsnValid(StaInfo_p->keyMgmtStateInfo.RsnIEBuf[13])))
	{
...
#ifndef SOC_W906X
#ifdef MRVL_WAPI
int wlFwSetWapiKey(struct net_device *netdev, struct wlreq_wapi_key *wapi_key,
				   int groupkey)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
...
int wlFwSetApBeacon(struct net_device *netdev)
{
	...
	if (*(mib->mib_broadcastssid) && (ssid_len <= IEEEtypes_SSID_SIZE))
	{
		ssid_p = (IEEEtypes_SsIdElement_t *)
			wl_kmalloc(strlen(&(mib->StationConfig->DesiredSsId[0])) + sizeof(IEEEtypes_InfoElementHdr_t),
					   GFP_ATOMIC);
		...
		macMgmtMlme_UpdateProbeRspBasicIes(vmacSta_p, (UINT8 *)ssid_p,
										   sizeof(IEEEtypes_InfoElementHdr_t) + ssid_p->Len);
		...
		if (mib->Privacy->RSNEnabled)
		{
			...
			if (!mib->RSNConfigWPA2->WPA2OnlyEnabled)
			{
				if (vmacSta_p->Mib802dot11->thisStaRsnIE->PwsKeyCnt[0] > 1)
				{
					...
					AddRSN_IE(vmacSta_p,
							  (IEEEtypes_RSN_IE_t *)&pCmd->RsnMixedIE);
#ifdef SOC_W906X
					macMgmtMlme_UpdateProbeRspBasicIes(vmacSta_p,
	...
	if (mib->PhyDSSSTable->Chanflag.FreqBand == FREQ_BAND_2DOT4GHZ)
	{
		...
		*((UINT8 *)&ErpInfo.ERPInfo) = wlpptr->wlpd_p->BcnErpVal;
		macMgmtMlme_UpdateProbeRspBasicIes(vmacSta_p,
										   (UINT8 *)&ErpInfo,
										   sizeof(IEEEtypes_ERPInfoElement_t));
	}
#ifdef AP_WPA2
...
#ifdef SOC_W906X
			macMgmtMlme_UpdateProbeRspBasicIes(vmacSta_p,
											   (UINT8 *)&pCmd->StartCmd.Rsn48IE,
											   pCmd->StartCmd.Rsn48IE.Len + 2);
#endif
		}
		else
		{
			AddRSN_IEWPA2(vmacSta_p,
						  (IEEEtypes_RSN_IE_WPA2_t *)&pCmd->StartCmd.Rsn48IE);
			macMgmtMlme_UpdateProbeRspBasicIes(vmacSta_p,
											   (UINT8 *)&pCmd->StartCmd.Rsn48IE,
											   pCmd->StartCmd.Rsn48IE.Len + 2);
		}
	...
	case AP_MODE_N_ONLY:
		...
		else
		{
			...
			for (currRate = 0; currRate < 8; currRate++)
			{
				...
				{
					*opRates++ =
						mib->StationConfig->OpRateSet[currRate + 4] & 0x7f;
				}
			}
		}
		break;
...
void BFMRconfigChanBw(struct wlprivate *wlpptr, UINT8 ChnlWidth, UINT8 ExtChnlOffset,
					  UINT8 channel)
{
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
...
int wlFwSetChannel(struct net_device *netdev, u_int8_t channel, u_int8_t secchannel,
				   CHNL_FLAGS Chanflag, u_int8_t initRateTable)
#else
int wlFwSetChannel(struct net_device *netdev, u_int8_t channel, CHNL_FLAGS Chanflag,
...
#endif
{
	...
	if (pCmd->ChannelFlags.ChnlWidth == CH_80_MHz_WIDTH)
	{
		if (Chanflag.radiomode == RADIO_MODE_7x7p1x1)
			pCmd->ChannelFlags.ActPrimary =
				macMgmtMlme_Get80MHzPrimaryChannelOffset(secchannel);
		else
			pCmd->ChannelFlags.ActPrimary =
...
#ifdef SOC_W906X
int wlFwCreateBAStream(struct net_device *dev, u_int32_t BarThrs,
...
int wlFwCheckBAStream(struct net_device *dev, u_int32_t BarThrs,
					  u_int32_t WindowSize, u_int8_t *Macaddr,
					  u_int8_t DialogToken, u_int8_t Tid, u_int32_t ba_type,
...
int wlFwGetSeqNoBAStream(struct net_device *dev, u_int8_t *Macaddr,
...
int wlFwUpdateDestroyBAStream(struct net_device *dev, u_int32_t ba_type,
...
int wlFwUpdateUpdateBAStream(struct net_device *dev, u_int32_t ba_type,
...
int wlFwFlushBAStream(struct net_device *dev, u_int32_t ba_type,
					  u_int32_t direction)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
	HostCmd_FW_BASTREAM *pCmd =
...
int wlFwAcMaxTolerableDelay(struct net_device *netdev, u_int8_t action, u_int8_t ac,
							u_int32_t *maxdelay)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
...
int wlFwSetIEs(struct net_device *netdev)
{
	...
	if ((*(mib->mib_mbssid) == 1) && (mbssidGID < MAX_MBSSID_SET) && (*(mib->mib_ApMode) & AP_MODE_11AX) && (vmacSta_p->VMacEntry.macId == wlpptr->wlpd_p->mbssSet[mbssidGID].primbss))
	{ // primary bssid
		...
		while (bitmap)
		{
			...
			{
				if (idx !=
					wlpptr->wlpd_p->mbssSet[mbssidGID].primbss)
				{
	...
	if (*(mib_s->QoSOptImpl))
	{
		...
		if (*(mib_s->mib_HT40MIntoler) &&
			!((*(mib_s->mib_ApMode) & AP_MODE_BAND_MASK) >=
			  ...
			  (*(mib_s->mib_ApMode) & AP_MODE_BAND_MASK) <=
				  AP_MODE_MIXED))
		{
			ie_len = AddOverlap_BSS_Scan_Parameters_IE(wlpptr->vmacSta_p,
		...
		{
			IEEEtypes_Extended_Cap_Element_t *ext_cap_ie =
				(IEEEtypes_Extended_Cap_Element_t *)(tail -
													 ie_len);
	...
	{
		ie_len = Add_VendorSpec_VHT_IE(wlpptr->vmacSta_p,
									   (IEEEtypes_HT_Element_t *)tail);
		tail += ie_len;
	}
#ifdef MULTI_AP_SUPPORT
	...
	setFwIes->CmdHdr.Length = (tail - setFwIes->beacon_buf) + sizeof(FWCmdHdr) + sizeof(UINT16); // Action
	...
	if (*(mib->QoSOptImpl))
	{
		...
		else
		{
			pCmd->IeListLenHT +=
				AddHT_IE(wlpptr->vmacSta_p,
						 (IEEEtypes_HT_Element_t *)&(pCmd->IeListHT
														 [retval]));
			retval = pCmd->IeListLenHT;
			pCmd->IeListLenHT +=
				AddAddHT_IE(wlpptr->vmacSta_p,
							(IEEEtypes_Add_HT_Element_t *)&(pCmd->IeListHT[retval]));
			retval = pCmd->IeListLenHT;
		}
		...
		// #ifdef INTOLERANT40
		{
			...
			pCmd->IeListLenHT +=
				AddChanReport_IE(wlpptr->vmacSta_p,
								 (IEEEtypes_ChannelReportEL_t *)&(pCmd->IeListHT[retval]));
			retval = pCmd->IeListLenHT;
		}
		// #endif
			...
			AP_MODE_5GHZ_11AC_ONLY)
		{
			...
			IEEEtypes_Extended_Cap_Element_t *pEC =
				FindIEWithinIEs(vmacSta_p->thisbeaconIEs.WSCData,
	...
	}
	{
		...
		pCmd->IeListLenProprietary +=
			AddM_IE(wlpptr->vmacSta_p,
					(IEEEtypes_HT_Element_t *)&(pCmd->IeListProprietary
													[retvalProprietary]));
		retvalProprietary = pCmd->IeListLenProprietary;
	}
	...
	if (mib->multi_ap_attr)
	{
		...
		pCmd->IeListLenProprietary +=
			Add_MultiAP_IE(wlpptr->vmacSta_p,
						   (IEEEtypes_InfoElementHdr_t *)&(pCmd->IeListProprietary
															   [retvalProprietary]),
						   WL_WLAN_TYPE_AP);
		retvalProprietary = pCmd->IeListLenProprietary;
	}
...
int wlexecuteCommand(struct net_device *netdev, unsigned short cmd)
{
...
#endif
	{
		...
		if (wlwaitForComplete(netdev, 0x8000 | cmd))
		{
			...
			{
				u16 tmpcmd =
					(u16)(*(u16 *)(wlpd_p->smon.pLastCmdBuf));
				...
				mwl_hex_dump((UINT8 *)(wlpd_p->smon.pLastCmdBuf),
							 ((dbglen > 64) ? 64 : dbglen));
...
wlwaitForComplete(struct net_device *netdev, u_int16_t cmdCode)
{
	...
	if (currIteration == 0)
	{
		...
		print_hex_dump(KERN_INFO, "", DUMP_PREFIX_OFFSET,
					   16, 4,
					   &((SMAC_CTRL_BLK_st *)wlpptr->ioBase0)->sfw,
					   sizeof(SMAC_SFW_st), false);
	...
	else
	{
		for (i = 0;
			 ...
			 domainGetSizeOfGrpChList80Mhz() /
				 sizeof(GRP_CHANNEL_LIST_80Mhz);
			 i++)
		{
			if (channel_exists(Channel, GrpChList80Mhz[i].channelEntry,
							   4) == FALSE)
			{
...
int wlFwApplyChannelSettings(struct net_device *netdev)
{
		...
		PhyDSSSTable->Chanflag.radiomode == RADIO_MODE_7x7p1x1)
	{
		...
		memcpy(&chanflag_swp, &PhyDSSSTable->Chanflag,
			   sizeof(CHNL_FLAGS));
		...
		retval = wlchannelSet(netdev, PhyDSSSTable->SecChan,
							  PhyDSSSTable->CurrChan, chanflag_swp, 1);
	}
...
int wlFwApplySettings(struct net_device *netdev)
{
...
#ifndef SOC_W906X
	if (wlFwSetBWSignalType(netdev, *mib->mib_bwSignaltype, 0))
	{ // 3rd input of function is for CTS BW signalling test
	...
	if (wlpptr->wlpd_p->ext_scnr_en && wlpptr->wlpd_p->pdfsApMain)
	{
		...
		if (UpdateCurrentChannelInMIB(vmacSta_p, mib->PhyDSSSTable->CurrChan))
		{
			...
			if (wlchannelSet(netdev, dfsDesc_p->CtlChanInfo.channel,
							 dfsDesc_p->currChanInfo.channel,
...
#endif /* CONCURRENT_DFS_SUPPORT */
	{
			...
			PhyDSSSTable->Chanflag.radiomode == RADIO_MODE_7x7p1x1)
		{
			...
			chanflag_swp.FreqBand =
				PhyDSSSTable->Chanflag.FreqBand2;
			chanflag_swp.FreqBand2 =
				PhyDSSSTable->Chanflag.FreqBand;
			chanflag_swp.ChnlWidth =
				PhyDSSSTable->Chanflag.ChnlWidth2;
			chanflag_swp.ChnlWidth2 =
	...
	{
		if (DecideDFSOperation(netdev, bChannelChanged, bBandWidthChanged, currDFSMode,
							   newDFSMode, mib))
		{
	...
	if (*(mib->QoSOptImpl))
	{
		...
		for (i = 0; i < 4; i++)
		{
			 ...
			 */
			if (wlFwSetEdcaParam(netdev, i, mib_QAPEDCATable[i].QAPEDCATblCWmin,
	...
	{
		if (wlFwNewDP_RxSOP(netdev, 2, mib->rxsop_ed_threshold1,
	...
	{ // place at end so fw doesn't overwrite mac 0xd78
...
#endif
int wlFwGetAddrValue(struct net_device *netdev, UINT32 addr, UINT32 startIdx,
...
int wlFwSetMimoPsHt(struct net_device *netdev, UINT8 *addr, UINT8 enable,
					UINT8 mode)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
...
#ifdef SOC_W906X
int wlFwGetRateTable(struct net_device *netdev, UINT8 *addr, UINT8 *pRateInfo,
 ...
 */
#ifdef SOC_W906X
int wlFwSetRateTable(struct net_device *netdev, UINT32 action, UINT8 *addr,
					   ...
					   UINT8 newDFSState, MIB_802DOT11 *mib)
{
	...
	{
		action = DFSGetCurrentRadarDetectionMode(wlpptr->wlpd_p->pdfsApMain,
												 PhyDSSSTable->CurrChan,
												 PhyDSSSTable->SecChan,
...
int wlFwSet11N_BF_Mode(struct net_device *netdev, UINT8 bf_option,
...
int wlFwSetOfdma_Mode(struct net_device *netdev, UINT8 option, UINT8 ru_mode,
...
int wlFwSetNoAck(struct net_device *netdev, UINT8 Enable, UINT8 be_enable,
...
void PrintTxHisto(struct net_device *netdev, UINT16 indx, UINT8 type)
{
	...
	else
	{
		...
		pTxhisto =
			&(wlpptr->wlpd_p->txRateHistogram[indx]->custom_rate[0]);
	}
			...
			for (bw = 0; bw < QS_NUM_SUPPORTED_11AC_BW; bw++)
			{
					 ...
					 mcs++)
				{
					for (sgi = 0; sgi < QS_NUM_SUPPORTED_GI;
						 sgi++)
					{
						cnt = ENDIAN_SWAP32(wlpptr->wlpd_p->txRateHistogram
												[indx]
													->MU_rate[nss]
															 [bw][sgi]
															 [mcs]
													.cnt);
						rateinfo =
							ENDIAN_SWAP32(wlpptr->wlpd_p->txRateHistogram
											  [indx]
												  ->MU_rate
													  [nss][bw]
													  [sgi]
						...
						{
							total += cnt;
							per4 = ENDIAN_SWAP32(wlpptr->wlpd_p->txRateHistogram
													 [indx]
														 ->MU_rate[nss]
																  [bw][sgi][mcs]
														 .per[4]);
							per3 = ENDIAN_SWAP32(wlpptr->wlpd_p->txRateHistogram
													 [indx]
														 ->MU_rate[nss]
																  [bw][sgi][mcs]
														 .per[3]);
							per2 = ENDIAN_SWAP32(wlpptr->wlpd_p->txRateHistogram
													 [indx]
														 ->MU_rate[nss]
																  [bw][sgi][mcs]
														 .per[2]);
							per1 = ENDIAN_SWAP32(wlpptr->wlpd_p->txRateHistogram
													 [indx]
														 ->MU_rate[nss]
																  [bw][sgi][mcs]
														 .per[1]);
							per0 = ENDIAN_SWAP32(wlpptr->wlpd_p->txRateHistogram
													 [indx]
														 ->MU_rate[nss]
																  [bw][sgi][mcs]
														 .per[0]);
							...
							if ((wlpptr->wlpd_p->txRateHistogram
									 [indx]
										 ->CurRateInfo[type] &
								 ratemask) ==
								(rateinfo &
	...
	else
	{
		...
		for (j = 0; j < loopcnt; j++)
		{
			...
			sgi = (rateinfo >> 6) & 0x1;
					...
					if ((wlpptr->wlpd_p->txRateHistogram[indx]->CurRateInfo[type %
																			SU_MU_TYPE_CNT] &
			...
			for (j = 0; j < 65; j++)
			{
				if (wlpptr->wlpd_p->scheHistogram[stnId]->NumAmpdu[i][j])
					Sysfs_Printk("%sampdu[%2d] -  %10u\n",
								 i ? "" : "Non-", j,
								 wlpptr->wlpd_p->scheHistogram[stnId]->NumAmpdu[i][j]);
			}
			Sysfs_Printk("\n");
		}
	}
...
int wlFwGetQueueStats(struct net_device *netdev, int option,
...
#endif
	{
		...
		case QS_GET_TX_SCHEDULER_INFO:
		{
				...
				for (idx = 0; idx < entries; idx++)
				{
												 ...
												 STADB_DONT_UPDATE_AGINGTIME)) != NULL)
					{
							...
							ASSOCIATED)
						{
							Sysfs_Printk("\n###StnId: %d MAC Addr: %s###\n",
										 pStaInfo->StnId,
										 mac_display(pStaInfo->Addr));
							PrintSchHisto(netdev,
										  pStaInfo->StnId,
										  sysfs_buff);
						}
						listBuf +=
			...
			{
				if (vmacSta_p->OpMode == WL_OP_MODE_STA || vmacSta_p->OpMode ==
															   WL_OP_MODE_VSTA)
				{
					Sysfs_Printk("\n");
			...
			for (i = 0; i < NUM_OF_TCQ; i++)
			{
				if (pQS_Counters->TCQxAttempts[i])
				{
					printk("%2d    %10u\t%10u\t    %10u\t\t%10u\n", i, ENDIAN_SWAP32((int)pQS_Counters->TCQxAttempts[i]),
						   ENDIAN_SWAP32((int)
											 pQS_Counters->TCQxSuccesses
												 [i]),
						   (ENDIAN_SWAP32((int)pQS_Counters->TCQxRetrySuccesses[i]) +
							ENDIAN_SWAP32((int)
											  pQS_Counters->TCQxMultipleRetrySuccesses
												  [i])),
						   ENDIAN_SWAP32((int)
											 pQS_Counters->TCQxFailures
												 [i]));
				}
			}
			...
			for (i = 0; i < QS_NUM_STA_SUPPORTED; i++)
			{
				if (pQS_Counters->SwBAStats[i].SwBaPktEnqueued)
				{
					...
					memset((void *)&SBLTS, 0x00,
						   sizeof(SBLTS));
					if (wlFwGetAddrValue(netdev,
										 pQS_Counters->SwBAStats[i].pSBLTS,
										 (sizeof(SWBA_LFTM_STATS_t) >> 2), (UINT32 *)&SBLTS,
										 0) == SUCCESS)
					{
						int k;
...

			{
...
#ifdef RXACNT_REC
				{
					DRV_RATE_HIST *prxRateHistogram =
						&wlpptr->wlpd_p->drvrxRateHistogram;
				...
				{
					{
						if (ENDIAN_SWAP32(wlpptr->wlpd_p->rxRateHistogram.LegacyRates[i]) >
							0)
						{
							if (i == 2)
							{
								...
								IEEEtypes_MacAddr_t * addr)
	{
		...
		pCmd->CmdHdr.Length =
			ENDIAN_SWAP16(sizeof(HostCmd_FW_GET_CONSEC_TXFAIL_ADDR));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
...
#define HAL_ACTSUBCH_BOTH 2
						   ...
						   u32 * sub)
	{
		...
		{
			*primary =
				macMgmtMlme_Get80MHzPrimaryChannelOffset(chan_num);
			if (*primary & BIT(0))
			{
				*sub = HAL_ACTSUBCH_LOWER;
			}
					   ...
					   u8 req_type, struct sk_buff * skb, u16 * result)
	{
		...
		if (skb != NULL)
			pCmd->CmdHdr.Length =
				ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_OFFCHANNEL) -
						   ...
						   u32 * fw_offchan_state)
	{
		...
		HostCmd_FW_NEWDP_OFFCHANNEL_DBG *pCmd =
			(HostCmd_FW_NEWDP_OFFCHANNEL_DBG *)&wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
	...
	int wlFwNewDP_OffChannel_Start(struct net_device * netdev)
	{
		...
		HostCmd_FW_NEWDP_OFFCHANNEL_START *pCmd =
			(HostCmd_FW_NEWDP_OFFCHANNEL_START *)&wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		...
		wlpptr->wlpd_p->descData[0].pOffChReqRing =
			(u_int8_t *)pci_alloc_consistent(wlpptr->pPciDev,
											 MAX_OFF_CHAN_REQ *
												 sizeof(offchan_desc_t),
											 &wlpptr->wlpd_p->descData[0].pPhysOffChReqRing);
		...
		pCmd->CmdHdr.Length =
			ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_OFFCHANNEL_START));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->OffChanReqBase =
			ENDIAN_SWAP32(wlpptr->wlpd_p->descData[0].pPhysOffChReqRing);
		pCmd->OffChanDoneBase =
			ENDIAN_SWAP32(wlpptr->wlpd_p->descData[0].pPhysOffChDoneRing);
		pCmd->pPhyoffchanshared =
			ENDIAN_SWAP32(wlpptr->wlpd_p->descData[0].pPhyoffchanshared);
								...
								DOT11_OFFCHAN_REQ_t *pOffChan)
{
	...
	pOffChDesc =
		(offchan_desc_t *)(wlpptr->wlpd_p->descData[0].pOffChReqRing + nextOff);
...
#else
		pOffChDesc->txDesc.Data =
			ENDIAN_SWAP32(pci_map_single(wlpptr->pPciDev, txSkb_p->data,
										 txSkb_p->len, PCI_DMA_TODEVICE));
		pOffChDesc->txDesc.Ctrl =
			ENDIAN_SWAP32(((0 & txring_Ctrl_QIDmask) << txring_Ctrl_QIDshift) |
						  (((txSkb_p->len -
							 (sizeof(struct ieee80211_frame)) -
							 2) &
							txring_Ctrl_LenMask)
						   << txring_Ctrl_LenShift) |
						  (txring_Ctrl_TAG_MGMT << txring_Ctrl_TAGshift));
#endif /* #ifdef SOC_W906X */
	}
	if ((ReqIdListItem_p =
...
#define OFFCHAN_COOLDOWN_TIME 2 // in 100ms unit. 10 = 1sec
	...
	int wlFwNewDP_handle_OffChan_event(struct net_device * netdev)
	{
		...
		case OFFCHAN_IDLE:
		{
			...
			offChanListItem_p =
				(offChanListItem *)
					ListGetItem(&wlpptr->wlpd_p->offChanList);
			SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->locks.offChanListLock,
								   listflags);
			...
			if (offChanListItem_p != NULL)
			{
				...
				case OFFCHAN_TYPE_RX_NF:
				{
					...
					u32 dwell =
						offchan_current_req_p->offchan_desc.OffChannel.dwell_time;
					u8 req_type =
						offchan_current_req_p->offchan_desc.OffChannel.req_type;

					ret = wlFwOffChannel(netdev, ch, bw,
										 dwell,
										 req_type,
										 offchan_current_req_p->txSkb_p,
										 &result);
					if (ret == SUCCESS)
					{
						switch (result)
		...
		OFFCHAN_PROCESS_FAIL:
			...
			TimerFireIn(&wlpptr->wlpd_p->offChanCooldownTimer, 1,
			...
			i = GetRegionChanIndx(GetDomainIndxIEEERegion(domainGetDomain()),
								  tmp_status.channel);
			if (i >= 0 &&
				i <
...

			{
				...
				for (i = 0; i < 14; i++)
				{
					...
					if ((4096 -
						...
						printk("acs data size exceeded holding memory\n");
				}
				...
				wl_vfree(string_buff);
			}

			/* if ACS mode is NF-reading and auto channel process is on-going, call timeout */
			if (*(vmacSta_p->Mib802dot11->mib_autochannel) && (vmacSta_p->preautochannelfinished == 0) && (vmacSta_p->acs_mode == 1))
			{
				memcpy(vmacSta_p->acs_db[vmacSta_p->ChanIdx].nf_bin, tmp_status.nf_dbm,
...
int wlFwNewDP_handle_OffChan_event(struct net_device *netdev)
{
	...
	UINT32 offchDoneHead =
		((offchan_shared_t *)wlpptr->wlpd_p->descData[0].poffchanshared)->OffChanDoneHead;
	UINT32 offchDoneTail =
		((offchan_shared_t *)wlpptr->wlpd_p->descData[0].poffchanshared)->OffChanDoneTail;
	...
	{
		pOffDone =
			(offchan_done_stat_t *)(wlpptr->wlpd_p->descData[0].pOffChDoneRing +
									offchDoneTail *
										sizeof(offchan_done_stat_t));
		pOffHead =
			(offchan_done_stat_t *)(wlpptr->wlpd_p->descData[0].pOffChDoneRing +
			...
			(ENDIAN_SWAP32(pOffDone->status) == OFFCHAN_FAIL))
		{
			while ((ReqIdListItem_p =
						(ReqIdListItem *)ListGetItem(&wlpptr->wlpd_p->ReqIdList)) != NULL)
			{
...
int wlFwNewDP_setAcntBufSize(struct net_device *netdev, u_int32_t base,
							   ...
							   sensord_init_t * sensordinit, UINT8 action)
	{
		...
		if (action)
		{
			pCmd->SensordInit_ext.mrvl_pri_mailbox =
				(UINT32)ENDIAN_SWAP32((UINT32)wlpptr->wlpd_p->MrvlPriSharedMem.dataPhysicalLoc);
	...
	int wlFwNewDP_wifiarb_post_req_intr(struct net_device * netdev)
	{
		...
		rcu_read_lock();
		t = pid_task(find_pid_ns(wlpptr->wlpd_p->PostReqSiginfo.pid, &init_pid_ns),
					 PIDTYPE_PID);
		if (t == NULL)
		{
			printk("no such pid\n");
									   ...
									   u8 * blankingmask)
	{
		...
		HostCmd_FW_NEWDP_SENSORD_SET_BLANKING *pCmd =
			(HostCmd_FW_NEWDP_SENSORD_SET_BLANKING *)&wlpptr->pCmdBuf[0];
		...
		pCmd->CmdHdr.Length =
			ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_SENSORD_SET_BLANKING));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
								 ...
								 wlcfg_sbf_close_t * WlcfgsbfClose)
	{
		...
		HostCmd_FW_NEWDP_BFMR_SBF_CLOSE *pCmd =
			(HostCmd_FW_NEWDP_BFMR_SBF_CLOSE *)&wlpptr->pCmdBuf[0];
		unsigned long flags;
		int retval = FAIL;
	...
	int wlFwSetPowerPerRate(struct net_device * netdev)
	{
		...
		HostCmd_FW_NEWDP_SET_POWER_PER_RATE *pCmd =
			(HostCmd_FW_NEWDP_SET_POWER_PER_RATE *)&wlpptr->pCmdBuf[0];
		unsigned long flags;
		int retval = FAIL;
#ifdef SOC_W8964
		MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);
		...
		pCmd->CmdHdr.Length =
			ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_SET_POWER_PER_RATE));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->pPhyInfoPwrTbl =
			(UINT32)ENDIAN_SWAP32(wlpptr->wlpd_p->descData[0].pPhyInfoPwrTbl);
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd,
						...
						UINT8 *trpcid, UINT16 *dBm, UINT16 *ant)
{
	...
	pratepower =
		(Info_rate_power_table_t *)wlpptr->wlpd_p->descData[0].pInfoPwrTbl;
									...
									UINT32 action)
	{
		...
		HostCmd_FW_NEWDP_RADIO_STATUS_NOTIFICATION *pCmd =
			(HostCmd_FW_NEWDP_RADIO_STATUS_NOTIFICATION *)&wlpptr->pCmdBuf[0];
		...
		pCmd->CmdHdr.Cmd =
			ENDIAN_SWAP16(HostCmd_CMD_NEWDP_RADIO_STATUS_NOTIFICATION);
		pCmd->CmdHdr.Length =
			ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_RADIO_STATUS_NOTIFICATION));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
...
int wlFwSetMUSet(struct net_device *netdev, UINT8 Option, UINT8 GID,
...
#endif
	{
		...
		{ // if Del Mu Set and Stn is null, set 0xFFFF in first stn.
			pCmd->StnID[0] = 0xFFFF;
		}
#else
	pCmd->StnID[0] = Stn1;
					  ...
					  UINT16 len, UINT8 * tlvData, char *string_buff)
	{
		...
		{
			extern void wlsFTM_configureDevice(struct net_device * netdev, UINT8 act,
											   UINT16 type,
											   UINT16 len,
											   UINT8 * tlvData,
											   char *string_buff);
		...
		if (!act)
		{
			...
			if (type == 3)
			{
				...
				WLS_Distance_INT =
					(pCmd->TlvData[0] << 8) | (pCmd->TlvData[1]);
				WLS_Distance_DEC = pCmd->TlvData[2];
				AoA_Angle_INT =
					(SINT16)((pCmd->TlvData[3] << 8) | (pCmd->TlvData
															[4]));
			...
			// WLS_Distance_DEC = pCmd->TlvData[2];
			AoA_Angle_INT =
				(SINT16)((pCmd->TlvData[10] << 8) | (pCmd->TlvData
														 [11]));
			AoA_Angle_DEC = pCmd->TlvData[12];
			Timestamp =
				(pCmd->TlvData[17] << 24) | (pCmd->TlvData[16] << 16) |
				(pCmd->TlvData[15] << 8) | (pCmd->TlvData[14]);
			LOS_Factor1 =
				(pCmd->TlvData[23] << 8) | (pCmd->TlvData
												[22]);
			LOS_Factor2 =
				(pCmd->TlvData[25] << 8) | (pCmd->TlvData
												[24]);
			// Pass back string to buffer for printing out at user space
			// Print back MAC address of distance / angle data
			sprintf(string_buff,
								 ...
								 UINT8 bitmap, UINT8 channel)
	{
		...
		HostCmd_FW_NEWDP_OFFCHANNEL_PWR *pCmd =
			(HostCmd_FW_NEWDP_OFFCHANNEL_PWR *)&wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
...
#define TEST_REC 8
	...
	DataEntry_t Key[TEST_REC] =
		{{16, {0xC6, 0x7E, 0x81, 0x6B, 0x4B, 0xFB, 0xE2, 0xFB, 0x54, 0xF6, 0xBD, 0xDF, 0x7C, 0x1C, 0xE1, 0x87}},
		 {32, {0xc9, 0x7c, 0x1f, 0x67, 0xce, 0x37, 0x11, 0x85, 0x51, 0x4a, 0x8a, 0x19, 0xf2, 0xbd, 0xd5, 0x2f, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f}},
		 {16, {0x2e, 0xcd, 0x70, 0xd3, 0x44, 0xac, 0xa4, 0x02, 0xfe, 0xf9, 0x90, 0xd5, 0x2a, 0xd4, 0x3e, 0x52}},
		 {32, {0xe9, 0x95, 0xf0, 0x70, 0xc9, 0x26, 0x2f, 0xe9, 0xa9, 0xb9, 0xd9, 0x11, 0xf4, 0xe5, 0x1e, 0xd6, 0x62, 0x4c, 0xb3, 0x67, 0xb1, 0x8c, 0x0c, 0xc4, 0x77, 0x98, 0x39, 0xcd, 0x19, 0x02, 0x31, 0xd7}},
	...
	DataEntry_t Nounce[TEST_REC] =
		{{13, {0x01, 0xBF, 0x31, 0xDE, 0x56, 0x72, 0x0F, 0x47, 0x67, 0x66, 0x87, 0x59, 0xAA}},
		 {13, {0x00, 0x50, 0x30, 0xf1, 0x84, 0x44, 0x08, 0xb5, 0x03, 0x97, 0x76, 0xe7, 0x0c}},
		 {12, {0xc1, 0xcc, 0x07, 0x4b, 0x72, 0xc0, 0x3d, 0x11, 0xe8, 0xa5, 0xa9, 0x8c}},
		 {12, {0xe9, 0x7e, 0x06, 0x80, 0x1f, 0xb7, 0xe5, 0x03, 0xf7, 0x48, 0xa5, 0x0b}},
	...
	DataEntry_t AAD[TEST_REC] =
		{{22, {0xEA, 0x56, 0x13, 0x7B, 0xD2, 0x85, 0xA1, 0xD8, 0x3C, 0x54, 0x55, 0x2F, 0x37, 0xAE, 0x65, 0x5B, 0xDA, 0x02, 0x79, 0x98, 0xCC, 0xE3}},
		 {22, {0x08, 0x40, 0x0f, 0xd2, 0xe1, 0x28, 0xa5, 0x7c, 0x50, 0x30, 0xf1, 0x84, 0x44, 0x08, 0xab, 0xae, 0xa5, 0xb8, 0xfc, 0xba, 0x00, 0x00}},
		 {22, {0x15, 0xf9, 0x51, 0x75, 0x99, 0xe4, 0x06, 0xf0, 0x4b, 0xea, 0xae, 0x73, 0xc1, 0x05, 0x46, 0x15, 0x72, 0x08, 0x18, 0xc4, 0x99, 0x9e}},
		 {22, {0x04, 0x64, 0x33, 0xf8, 0x47, 0xdf, 0x67, 0xb3, 0xb7, 0x85, 0xa6, 0x35, 0x21, 0xf1, 0x4f, 0xee, 0x59, 0x4e, 0xeb, 0xc3, 0x3b, 0x0e}},
	...
	DataEntry_t InData[TEST_REC] = {{2, {0xEE, 0x43}},
									{20, {0xf8, 0xba, 0x1a, 0x55, 0xd0, 0x2f, 0x85, 0xae, 0x96, 0x7b, 0xb6, 0x2f, 0xb6, 0xcd, 0xa8, 0xeb, 0x7e, 0x78, 0xa0, 0x50}},
									{16, {0x1a, 0x4d, 0xa6, 0x30, 0x1e, 0xab, 0x28, 0xc1, 0x9c, 0xdf, 0x22, 0xb1, 0x56, 0xab, 0xb9, 0x90}},
									{16, {0xbe, 0x6f, 0x3d, 0xac, 0xf9, 0xcf, 0x62, 0xe1, 0x7b, 0x82, 0xbc, 0xa3, 0x71, 0xb0, 0xa8, 0x19}},

									{10, {0xF2, 0x40, 0xC9, 0x34, 0x89, 0x8B, 0xB4, 0xEB, 0x7C, 0x27}},
									{36, {0x6d, 0x15, 0x5d, 0x88, 0x32, 0x66, 0x82, 0x56, 0xd6, 0xa9, 0x2b, 0x78, 0xe1, 0x1d, 0x8e, 0x54, 0x49, 0x5d, 0xd1, 0x74, 0x80, 0xaa, 0x56, 0xc9, 0x49, 0x2e, 0x88, 0x2b, 0x97, 0x64, 0x2f, 0x80, 0xd5, 0x0f, 0xe9, 0x7b}},
									{32, {0xeb, 0x13, 0x3b, 0xce, 0x32, 0xc2, 0x8b, 0xf8, 0xce, 0x4c, 0x22, 0xe3, 0x9e, 0xff, 0x69, 0x1b, 0xd2, 0xf1, 0x3c, 0xab, 0x0f, 0xb9, 0xa6, 0x8f, 0xd8, 0xc7, 0x36, 0xa8, 0x7e, 0x82, 0x32, 0x35}},
									{32, {0xbe, 0xa5, 0x16, 0x49, 0xe3, 0xa9, 0x0b, 0xfc, 0x74, 0x3c, 0x4f, 0x40, 0x8d, 0x53, 0x0b, 0xb3, 0x9e, 0x94, 0xc6, 0xd3, 0x37, 0x30, 0x17, 0x8f, 0x1b, 0x56, 0xaf, 0xdb, 0xe3, 0x15, 0x36, 0x57}}};
	...
	int wlFwSendFipsTestAll(struct net_device * netdev)
	{
		...
		for (i = 0; i < TEST_REC; i++)
		{
			...
			if (memcmp(OutData.Data, expData[i].Data,
					   expData[i].Length))
			{
				status |= 1 << i;
...
#include "ap8xLnxSwMimo.h"
						   ...
						   mvl_status_t * radio_status)
	{
		...
		HostCmd_FW_GET_MVL_RADIO_STATUS *pCmd =
			(HostCmd_FW_GET_MVL_RADIO_STATUS *)&wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
