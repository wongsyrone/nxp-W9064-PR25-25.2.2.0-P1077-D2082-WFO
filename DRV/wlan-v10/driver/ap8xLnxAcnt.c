...
wlAcntSetBufSize(struct net_device *netdev, SetAcntBufInfo_t * SetInfo)
{
	...
	if (SetInfo->ActionType == ACNT_PAUSE) {
		...
		if (bAllocFailed) {
			//back to oldsize
			writel(head & (oldSize / ACNT_NCHUNK - 1),
			       wlpptr->ioBase1 + reg_acnt_tail);
			writel(head & (oldSize / ACNT_NCHUNK - 1),
			       wlpptr->ioBase1 + reg_acnt_head);
			for (i = 0; i < ACNT_NCHUNK; i++) {
#ifdef SOC_W906X
			...
			writel(head & (size / ACNT_NCHUNK - 1),
			       wlpptr->ioBase1 + reg_acnt_tail);
			writel(head & (size / ACNT_NCHUNK - 1),
			       wlpptr->ioBase1 + reg_acnt_head);
			wlpptr->wlpd_p->AcntChunkInfo.SizeOfChunk =
	...
	case acnt_code_tx_enqueue:
		{
			...
			memcpy(&acnt_tx_records[(*record_idx)++], acntRecd,
			       sizeof(acnt_tx_t));
	...
	case acnt_code_RA_stats:
		{
			...
			memcpy(&acnt_RA_stats[(*record_idx)++], acntRecd,
...
wl_write_acnt_tx_record(struct net_device *netdev, char *filename)
{
	struct wlprivate *priv = NETDEV_PRIV_P(struct wlprivate, netdev);
	UINT32 idx = 0;
	UINT32 currIdx = (priv->wlpd_p->acnt_tx_record_idx == 0) ?
		(ACNT_TX_RECORD_MAX - 1) : (priv->wlpd_p->acnt_tx_record_idx -
					    1);
