...
void
apmode_integrity(UINT8 * pmib_ApMode)
{
	BOOLEAN In_24g;
		     ...
		     struct iw_request_info *info, void *wrqu, char *extra)
{
	...
	case WL_PARAM_BAND:
...
#endif
		{
			...
			for (i = 0; i < wlp->wlpd_p->NumOfAPs + 2; i++) {
				...
				mib1 = vmac->ShadowMib802dot11;
				*(mib1->mib_ApMode) = (UCHAR) value;
				apmode_integrity(mib1->mib_ApMode);
#ifdef BRS_SUPPORT
				wlset_rateSupport(mib1);
#endif
			}
		}
		break;
		...
		case 0:
			...
			case SCBT:
				...
				if (PhyDSSSTable->Chanflag.FreqBand ==
					...
					PhyDSSSTable->Chanflag.ChnlWidth =
						CH_80_MHz_WIDTH;
				else if (PhyDSSSTable->Chanflag.FreqBand ==
					 FREQ_BAND_2DOT4GHZ)
					PhyDSSSTable->Chanflag.ChnlWidth =
						CH_40_MHz_WIDTH;
			...
			default:
				WLDBG_ERROR(DBG_LEVEL_1,
					    "Not support chip. Consider what's optimal auto BW settting for thsi chip.\n");
				break;
			}
			break;
		case 1:
	...
	case WL_PARAM_AUTOCHANNEL:
		...
		if (vmacSta_p->preautochannelfinished == 0) {
			...
			wait_ret =
				wait_for_completion_timeout(&vmacSta_p->
							    scan_complete,
							    vmacSta_p->
							    scan_timeout);
			if (vmacSta_p->preautochannelfinished == 0)
				printk(KERN_WARNING
				       "Wait scan finish timeout, autochannel, %us, %lu\n",
				       (vmacSta_p->scan_timeout / HZ),
				       wait_ret);
			else
		...
		*(mib->mib_autochannel) = value;
		...
		vmacSta_p->acs_ch_distance_weight = 50;
		vmacSta_p->acs_bss_distance_weight = 50;
	...
	case WL_PARAM_RRM_EN:
		if (wlpd_p->rrm_trigger_time == 0) {
			wlpd_p->rrm_trigger_time = RRM_DEFAULT_TRIGGER_TIME;
		}
		if (wlpd_p->rrm_interval_time == 0) {
			wlpd_p->rrm_interval_time = RRM_DEFAULT_INTERVAL_TIME;
		}
		if (wlpd_p->rrm_dwell_time == 0) {
			wlpd_p->rrm_dwell_time = RRM_DEFAULT_DWELL_TIME;
		}
...
extern void shift_tbtt(struct net_device *netdev, long shift_tm);
UINT32 quiet_dbg[10] = { 0 };

#if defined(SOC_W906X) && defined(WIFI_DATA_OFFLOAD)
	...
	case WL_IOCTL_SETCMD:
		{
			...
			if ((strcmp(param[0], "scanchannels") == 0)) {
				...
				char *tf_type_str[TF_MAX] =
					{ "basic", "bfrp", "mu_bar", "mu_rts",
			    "bsrp" };
					...
					if (vmacSta_p->preautochannelfinished ==
						...
						wait_ret =
							...
							(&vmacSta_p->
							 ...
							 scan_timeout);
						if (vmacSta_p->
						    preautochannelfinished == 0)
							printk(KERN_WARNING
							       "Wait scan finish timeout, intolerant40, %us, %lu\n",
							       (vmacSta_p->
								scan_timeout /
								HZ), wait_ret);
						else
			 ...
			 */
				...
				*ret_len = strlen(bufBack);
				 ...
				 */
					...
					for (i = 0; i < QS_MAX_DATA_RATES_G;
						...
						if (ENDIAN_SWAP32
						    (priv->wlpd_p->
						     ...
						     LegacyRates[i]) > 0) {
							if (i == 2) {
								printk("5.5Mbps \t: %10u\n", ENDIAN_SWAP32((unsigned int)priv->wlpd_p->rxRateHistogram.LegacyRates[i]));
							} else {
								printk("%2dMbps  \t: %10u\n", _11G_rate[i]
								       ,
								       ENDIAN_SWAP32
								       ((unsigned int)priv->wlpd_p->rxRateHistogram.LegacyRates[i]));
							}
						}
					}
					for (bw = 0;
						...
						for (sgi = 0;
							...
							for (i = 0;
							     ...
							     i++) {
								if (ENDIAN_SWAP32(priv->wlpd_p->drvrxRateHistogram.HtRates[bw][sgi][i]) > 0) {
									printk("%4s_%3s_MCS%2d  : %10u\n", bwStr[bw], sgiStr[sgi], i, ENDIAN_SWAP32((unsigned int)priv->wlpd_p->drvrxRateHistogram.HtRates[bw][sgi][i]));
								}
							}
						}
					}
						...
						for (bw = 0;
							...
							for (i = 0;
								...
								for (sgi = 0;
								     ...
								     QS_NUM_SUPPORTED_GI;
								     sgi++) {
									if (ENDIAN_SWAP32(priv->wlpd_p->drvrxRateHistogram.VHtRates[nss][bw][sgi][i]) > 0) {
										printk("%4s_%3s_%1dSS_MCS%2d  : %10u\n", vhtbwStr[bw], sgiStr[sgi], (nss + 1), i, ENDIAN_SWAP32(((int)priv->wlpd_p->drvrxRateHistogram.VHtRates[nss][bw][sgi][i])));
									}
								}
							}
						}
						...
						for (bw = 0;
							...
							for (i = 0;
								...
								for (sgi = 0;
								     ...
								     sgi++) {
									if (ENDIAN_SWAP32(priv->wlpd_p->drvrxRateHistogram.HERates[nss][bw][sgi][i]) > 0) {
										printk("%4s_giltf[%d]_%1dSS_MCS%2d  : %10u\n", hebwStr[bw], sgi, (nss + 1), i, ENDIAN_SWAP32(((int)priv->wlpd_p->drvrxRateHistogram.HERates[nss][bw][sgi][i])));
									}
								}
							}
						}
					...
					printk("============================\n");
#else /* #if defined(SOC_W906X) && defined(WIFI_DATA_OFFLOAD) */
#ifdef SOC_W906X
					if (wlFwGetQueueStats
...
#endif /* #if defined(SOC_W906X) && defined(WIFI_DATA_OFFLOAD) */
...
#include "macmgmtap.h"
				...
				*(mib->mib_quiet) = enable;
				...
				if (strcmp(param[1], "help") == 0) {
					printk("Usage: config_btmreq <abridged> <Disassoc Imm> <BSS Term> <ESS Disassoc Imm> <Disassoc Timer> <Validity Interval>\n");
					return FALSE;
				}
				...
				memcpy(mib1->mib_BSSTMRequest,
				       mib->mib_BSSTMRequest,
				       sizeof(IEEEtypes_BSS_TM_Request_t));
			}
#endif /* IEEE80211K */
			...
			//Use this CLI only after BSS is up
				...
				if (strcmp(param[1], "help") == 0) {
					...
					printk("acs_ch_distance_weight: %d\n",
					       vmacSta_p->
					       acs_ch_distance_weight);
					printk("acs_bss_distance_weight: %d\n",
					       vmacSta_p->
					       acs_bss_distance_weight);
...
#endif /* AUTOCHANNEL */
#ifdef WIFI_DATA_OFFLOAD
			else if (!strcmp(param[0], "dol_get_version")) {
				u8 ver[32];

				dol_get_wfo_version(priv,
						    priv->wlpd_p->
						    ipc_session_id, ver);
				printk("\t WFO version: %s\n", ver);
			} else if (!strcmp(param[0], "dol_stop")) {
				if (priv->wlpd_p->dol.radio_status ==
				    DOL_RADIO_STOP)
					printk("\tRadio is already stopped\n");
				else
					dol_stop_radio(priv,
						       priv->wlpd_p->
						       ipc_session_id);
			} else if (!strcmp(param[0], "dol_dbg_get")) {
				printk("\tdbg_ctrl: 0x%04x\n",
				       priv->wlpd_p->dol.dbg_ctrl);
			} else if (!strcmp(param[0], "dol_dbg_set")) {
				priv->wlpd_p->dol.dbg_ctrl = atohex(param[1]);
				dol_set_dbg_ctrl(priv,
						 priv->wlpd_p->ipc_session_id,
						 priv->wlpd_p->dol.dbg_ctrl);
			} else if (!strcmp(param[0], "dol_dscp_get")) {
				printk("\tdscp_wmm_mapping: %d\n",
				       priv->wlpd_p->dol.dscp_wmm_mapping);
				printk("\t 0: Map DSCP via IP precendence of IP TOS\n");
				printk("\t 1: Map DSCP via NEC vendor specific mapping\n");
			} else if (!strcmp(param[0], "dol_dscp_set")) {
				priv->wlpd_p->dol.dscp_wmm_mapping =
					atoi(param[1]);
				dol_set_dscp_wmm_mapping(priv,
							 priv->wlpd_p->
							 ipc_session_id,
							 priv->wlpd_p->dol.
							 dscp_wmm_mapping);
			} else if (!strcmp(param[0], "dol_iso_grp_get")) {
				int i;

				printk("\tIsolate group id:\n");
				for (i = 0; i < NUMOFAPS; i++)
					printk("\tVIF: %d, isolate group id: %d\n", i, priv->wlpd_p->dol.vif_isolate_grp_id[i]);
			} else if (!strcmp(param[0], "dol_iso_grp_set")) {
				int vif, iso_grp_id, rc;

				vif = atoi(param[1]);
				if ((vif >= 0) && (vif < NUMOFAPS)) {
					iso_grp_id = atoi(param[2]);
					if (iso_grp_id >= 0) {
						if (!priv->wlpd_p->dol.disable) {
							rc = dol_vif_set_isolate_grp_id(priv, priv->wlpd_p->ipc_session_id, vif, iso_grp_id);
							if (rc)
								printk("\tSet isolate group id failed: %d (VIF: %d, ID: %d\n", rc, vif, iso_grp_id);
							else
								priv->wlpd_p->
									dol.
									vif_isolate_grp_id
						...
						printk("\tWrong isolate group id: %d (VIF: %d)\n", iso_grp_id, vif);
				} else
					printk("\tWrong VIF: %d\n", vif);
			} else if (!strcmp(param[0], "dol_cnt_get")) {
				int cnt_type = 0, clear_after_read = 0, more =
					0;

				cnt_type = atoi(param[1]);
				clear_after_read = atoi(param[2]);
				switch (cnt_type) {
				case GET_STATS_NET_DEVICE:
					{
						struct netdev_stats net_stats;
						...
						printk("\trx_length_errors: %d\n", net_stats.rx_length_errors);
						printk("\trx_over_errors: %d\n",
						       net_stats.
						       rx_over_errors);
						printk("\trx_crc_errors: %d\n",
						       net_stats.rx_crc_errors);
						printk("\trx_frame_errors: %d\n", net_stats.rx_frame_errors);
						printk("\trx_fifo_errors: %d\n",
						       net_stats.
						       rx_fifo_errors);
						printk("\trx_missed_errors: %d\n", net_stats.rx_missed_errors);
						printk("\ttx_aborted_errors: %d\n", net_stats.tx_aborted_errors);
						printk("\ttx_carrier_errors: %d\n", net_stats.tx_carrier_errors);
					}
					break;
				case GET_STATS_PKT_STATUS:
					{
						...
						dol_get_stats(priv,
							      ...
							      &more);
						printk("\n\tPacket Status:\n");
						printk("\tpkt_hdr_free_num: %d\n", pkt_status.pkt_hdr_free_num);
						for (i = 0; i < 4; i++)
							printk("\tpkt_bmq_free_num(%d): %d\n", i, pkt_status.pkt_bmq_free_num[i]);
						printk("\tpkt_from_host_list_num: %d\n", pkt_status.pkt_from_host_num);
						for (i = 0; i < 4; i++)
							printk("\tpkt_from_eth_list_num(%d): %d\n", i, pkt_status.pkt_from_eth_num[i]);
					}
					break;
				case GET_STATS_DBG_TX_CNT:
					{
						...
						dol_get_stats(priv,
							      ...
							      &more);
						printk("\n\tTransmit Counters:\n");
						printk("\tdata_pkt_from_host: %d\n", tx_cnt.data_pkt_from_host);
						printk("\tmgmt_pkt_from_host: %d\n", tx_cnt.mgmt_pkt_from_host);
						printk("\tunicast_pkt_from_eth: %d\n", tx_cnt.unicast_pkt_from_eth);
						printk("\tbcmc_pkt_from_eth: %d\n", tx_cnt.bcmc_pkt_from_eth);
						for (i = 0; i < 4; i++)
							printk("\tac_pkt(%d): %d\n", i, tx_cnt.ac_pkt[i]);
						for (i = 0; i < 4; i++)
							printk("\tac_drop(%d): %d\n", i, tx_cnt.ac_drop[i]);
						printk("\ttx_drop_msg_err: %d\n", tx_cnt.tx_drop_msg_err);
						printk("\ttx_drop_vif_err: %d\n", tx_cnt.tx_drop_vif_err);
						printk("\ttx_drop_vif_disable: %d\n", tx_cnt.tx_drop_vif_disable);
						printk("\ttx_drop_sta_err: %d\n", tx_cnt.tx_drop_sta_err);
						printk("\ttx_drop_sta_disable: %d\n", tx_cnt.tx_drop_sta_disable);
						printk("\ttx_drop_no_pkt_hdr: %d\n", tx_cnt.tx_drop_no_pkt_hdr);
						printk("\ttx_queue_full: %d\n",
						       tx_cnt.tx_queue_full);
						printk("\ttx_queue_send: %d\n",
				...
				case GET_STATS_DBG_REL_CNT:
					{
						...
						printk("\ttx_release: %d\n",
						       rel_cnt.tx_release);
						printk("\tbm_release_request_num: %d\n", priv->wlpd_p->bm_release_request_num);
						printk("\tbm_release_request_ipc: %d\n", priv->wlpd_p->bm_release_request_ipc);
						printk("\tbm_release_from_host: %d\n", rel_cnt.bm_release_from_host);
						for (i = 0; i < 4; i++)
							printk("\tbmq_release(%d): %d\n", i, rel_cnt.bmq_release[i]);
						printk("\tbm10_poll: %d\n",
						       rel_cnt.bm10_poll);
						printk("\tbm10_return_non_clone: %d\n", rel_cnt.bm10_return_non_clone);
						printk("\tbm10_to_host: %d\n",
						       rel_cnt.bm10_to_host);
						printk("\tbm10_return_host: %d\n", rel_cnt.bm10_return_host);
						printk("\tbm10_to_eth: %d\n",
						       rel_cnt.bm10_to_eth);
						printk("\tbm10_return_eth: %d\n", rel_cnt.bm10_return_eth);
						printk("\tpe_hw_sig_err: %d\n",
						       rel_cnt.pe_hw_sig_err);
						printk("\tpe_hw_phy_addr_err: %d\n", rel_cnt.pe_hw_phy_addr_err);
						printk("\tpe_hw_bpid_err: %d\n",
						       rel_cnt.pe_hw_bpid_err);
						printk("\tpe_hw_pkt_sig_err: %d\n", rel_cnt.pe_hw_pkt_sig_err);
					}
					break;
				case GET_STATS_DBG_RX_CNT:
					{
						...
						dol_get_stats(priv,
							      ...
							      &more);
						printk("\n\tReceive Counters\n");
						printk("\tmgmt_pkt_to_host: %d\n", rx_cnt.mgmt_pkt_to_host);
						printk("\teapol_pkt_to_host: %d\n", rx_cnt.eapol_pkt_to_host);
						printk("\tdata_pkt_to_host: %d\n", rx_cnt.data_pkt_to_host);
						printk("\tdata_pkt_to_eth: %d\n", rx_cnt.data_pkt_to_eth);
						printk("\trx_drop_mgmt_q_type_err: %d\n", rx_cnt.rx_drop_mgmt_q_type_err);
						printk("\trx_drop_data_q_type_err: %d\n", rx_cnt.rx_drop_data_q_type_err);
						printk("\trx_drop_vif_err: %d\n", rx_cnt.rx_drop_vif_err);
						printk("\trx_drop_vif_disable: %d\n", rx_cnt.rx_drop_vif_disable);
						printk("\trx_drop_sta_err: %d\n", rx_cnt.rx_drop_sta_err);
						printk("\trx_drop_sta_disable: %d\n", rx_cnt.rx_drop_sta_disable);
						printk("\trx_drop_llc_err: %d\n", rx_cnt.rx_drop_llc_err);
						printk("\trx_drop_msdu_err: %d\n", rx_cnt.rx_drop_msdu_err);
						for (i = 0; i < 3; i++)
							printk("\trx_bmq_refill_fail(%d): %d\n", i, rx_cnt.rx_bmq_refill_fail[i]);
						printk("\trx_cfh_ul_sig_err: %d\n", rx_cnt.rx_cfh_ul_sig_err);
						printk("\trx_cfh_ul_bpid_err: %d\n", rx_cnt.rx_cfh_ul_bpid_err);
						printk("\trx_cfh_ul_snap_err: %d\n", rx_cnt.rx_cfh_ul_snap_err);
						printk("\trx_cfh_ul_size_err: %d\n", rx_cnt.rx_cfh_ul_size_err);
						printk("\trx_cfh_ul_war: %d\n",
				...
				case GET_STATS_DBG_PKT_CNT:
					{
						...
						dol_get_stats(priv,
							      ...
							      &more);
						printk("\n\tPacket Counters:\n");
						printk("\tpkt_hdr_alloc: %d\n",
						       pkt_cnt.pkt_hdr_alloc);
						printk("\tpkt_hdr_free: %d\n",
						       pkt_cnt.pkt_hdr_free);
						printk("\tpkt_hdr_lack: %d\n",
						       pkt_cnt.pkt_hdr_lack);
						printk("\tpkt_bm_data_alloc: %d\n", pkt_cnt.pkt_bm_data_alloc);
						printk("\tpkt_bm_data_free: %d\n", pkt_cnt.pkt_bm_data_free);
						for (i = 0; i < 3; i++) {
							printk("\tpkt_bmq_alloc(%d): %d\n", i, pkt_cnt.pkt_bmq_alloc[i]);
							printk("\tpkt_bmq_free(%d): %d\n", i, pkt_cnt.pkt_bmq_free[i]);
							printk("\tpkt_bmq_lack_buf(%d): %d\n", i, pkt_cnt.pkt_bmq_lack_buf[i]);
						}
						printk("\tpkt_bm_data_clone: %d\n", pkt_cnt.pkt_bm_data_clone);
						printk("\tpkt_bm_data_clone_free: %d\n", pkt_cnt.pkt_bm_data_clone_free);
						printk("\tpkt_host_data_free: %d\n", pkt_cnt.pkt_host_data_free);
						printk("\tpkt_eth_data_free: %d\n", pkt_cnt.pkt_eth_data_free);
						printk("\tpkt_local_data_free: %d\n", pkt_cnt.pkt_local_data_free);
						printk("\tpkt_amsdu_free: %d\n",
						       pkt_cnt.pkt_amsdu_free);
						printk("\tpkt_amsdu_alloc: %d\n", pkt_cnt.pkt_amsdu_alloc);
						printk("\tpkt_amsdu_lack: %d\n",
				...
				case GET_STATS_DBG_STA_CNT:
					{
						...
						while (1) {
							...
							printk("\tdrop_cnt: %d\n", sta_cnt.drop_cnt);
							for (i = 0; i < 8; i++)
								printk("\ttxq_pend_cnt(%d): %d\n", i, sta_cnt.txq_pend_cnt[i]);
						};
					}
					break;
				case GET_STATS_HFRMQ_INFO:
					{
						int qid;
						struct dbg_hfrmq_info *hfrm_rq;
						struct dbg_hfrmq_info *hfrm_sq;
						hfrm_rq =
						...
						for (qid = 0; qid < 16; qid++) {
							...
							dol_get_stats(priv,
								      ...
								      (struct
								       dbg_hfrmq_info),
								      0, &more);
							printk("\tRQ%d\tRdPtr %08x\tWrPtr %08x\n", qid, hfrm_rq[qid].rdptr, hfrm_rq[qid].wrptr);
						}
						for (qid = 0; qid < 16; qid++) {
							hfrm_sq[qid].qid = qid;
							hfrm_sq[qid].qoff =
							...
							dol_get_stats(priv,
								      ...
								      (struct
								       dbg_hfrmq_info),
								      0, &more);
							printk("\tSQ%d\tRdPtr %08x\tWrPtr %08x\n", qid, hfrm_sq[qid].rdptr, hfrm_sq[qid].wrptr);
						}
						wl_kfree(hfrm_rq);
						wl_kfree(hfrm_sq);
					}
			...
			} else if (!strcmp(param[0], "dol_del_sta")) {
				u8 del_sta_mac[ETH_ALEN];

				sscanf(param[1],
				       "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx\n",
				       &del_sta_mac[0], &del_sta_mac[1],
				       &del_sta_mac[2], &del_sta_mac[3],
				       &del_sta_mac[4], &del_sta_mac[5]);

				printk("%s(%d): delete sta %pM\n", __func__,
				       __LINE__, del_sta_mac);
				extStaDb_DelSta(vmacSta_p,
						(IEEEtypes_MacAddr_t *) &
						del_sta_mac[0],
						STADB_DONT_UPDATE_AGINGTIME);
			} else if (!strcmp(param[0], "dol_ba_holdtime")) {
				//u8 del_sta_mac[ETH_ALEN];
				u32 holdtime;

				holdtime = atoi(param[1]);

				printk("%s(%d): ba reorder time %u\n", __func__,
				       __LINE__, holdtime);
				dol_set_ba_info(priv,
						priv->wlpd_p->ipc_session_id,
						BA_INFO_CFG_FLUSHTIME,
						(U16) holdtime, 0, 0, 0);
			} else if (!strcmp(param[0], "dol_pci_sw_sem_timeout")) {
				printk("\tpci_sw_sem_timeout: 0x%04x\n",
				       priv->wlpd_p->dol.pci_sw_sem_timeout);
			}
				...
				if (input_cnt < 2) {
					printk("rrm_offchan_time trigger=%dms interval=%dms dwell time=%dms\n", wlpd_p->rrm_trigger_time, wlpd_p->rrm_interval_time, wlpd_p->rrm_dwell_time);
					break;
				}
				if (strcmp(param[1], "help") == 0) {
					...
					printk("If time = 0, set to default time(trigger=%d interval=%d dwell=%d)\n", RRM_DEFAULT_TRIGGER_TIME, RRM_DEFAULT_INTERVAL_TIME, RRM_DEFAULT_DWELL_TIME);
					break;
				}
				time_val = atoi(param[3]);
				if (time_val == 0) {
					wlpd_p->rrm_dwell_time =
						RRM_DEFAULT_DWELL_TIME;
				} else {
					if (time_val > 1000) {
					...
					wlpd_p->rrm_dwell_time = time_val;
				}
				time_val = atoi(param[2]);
				if (time_val == 0) {
					wlpd_p->rrm_interval_time =
						RRM_DEFAULT_INTERVAL_TIME;
				} else {
					wlpd_p->rrm_interval_time = time_val;
				}
				time_val = atoi(param[1]);
				if (time_val == 0) {
					wlpd_p->rrm_trigger_time =
						RRM_DEFAULT_TRIGGER_TIME;
				} else {
					wlpd_p->rrm_trigger_time = time_val;
				}
				...
				if (strcmp(param[1], "help") == 0) {
					printk("Usage: iwpriv <radio> setcmd \"dfs_opt <DFS_params> <value>\"\n\n");
					printk("DFS_params              default min Max\n");
					printk("fcc_min_radar_num_pri[8]      3   1   6\n");
					printk("etsi_min_radar_num_pri[8]     3   1   4\n");
					printk("jpn_w53_min_radar_num_pri[8]  2   1   2\n");
					printk("jpn_w56_min_radar_num_pri[8]  4   1   4\n");
					printk("false_detect_th               9   1  50\n");
					printk("fcc_zc_error_th               3   1   5\n");
					printk("etsi_zc_error_th              1   1   5\n");
					printk("jp_zc_error_th                3   1   5\n\n");
				...
				if (strcmp(param[1], "get") == 0) {
					ret = wlFwDFSParams(vmacSta_p->dev,
							    DFS_GET_ALL,
							    fcc_min_radar_num_pri,
							    etsi_min_radar_num_pri,
							    jpn_w53_min_radar_num_pri,
							    jpn_w56_min_radar_num_pri,
							    &false_detect_th,
							    &fcc_zc_error_th,
							    &etsi_zc_error_th,
							    &jp_zc_error_th);
					printk("fcc_min_radar_num_pri       %d %d %d %d %d %d %d %d\n", fcc_min_radar_num_pri[0], fcc_min_radar_num_pri[1], fcc_min_radar_num_pri[2], fcc_min_radar_num_pri[3], fcc_min_radar_num_pri[4], fcc_min_radar_num_pri[5], fcc_min_radar_num_pri[6], fcc_min_radar_num_pri[7]);
					printk("etsi_min_radar_num_pri      %d %d %d %d %d %d %d %d\n", etsi_min_radar_num_pri[0], etsi_min_radar_num_pri[1], etsi_min_radar_num_pri[2], etsi_min_radar_num_pri[3], etsi_min_radar_num_pri[4], etsi_min_radar_num_pri[5], etsi_min_radar_num_pri[6], etsi_min_radar_num_pri[7]);
					printk("jpn_w53_min_radar_num_pri   %d %d %d %d %d %d %d %d\n", jpn_w53_min_radar_num_pri[0], jpn_w53_min_radar_num_pri[1], jpn_w53_min_radar_num_pri[2], jpn_w53_min_radar_num_pri[3], jpn_w53_min_radar_num_pri[4], jpn_w53_min_radar_num_pri[5], jpn_w53_min_radar_num_pri[6], jpn_w53_min_radar_num_pri[7]);
					printk("jpn_w56_min_radar_num_pri   %d %d %d %d %d %d %d %d\n", jpn_w56_min_radar_num_pri[0], jpn_w56_min_radar_num_pri[1], jpn_w56_min_radar_num_pri[2], jpn_w56_min_radar_num_pri[3], jpn_w56_min_radar_num_pri[4], jpn_w56_min_radar_num_pri[5], jpn_w56_min_radar_num_pri[6], jpn_w56_min_radar_num_pri[7]);
					...
					for (i = 0;
					     i <
					     (sizeof(fcc_min_radar_num_pri) /
					      sizeof(UINT8)); i++) {
						fcc_min_radar_num_pri[i] =
							atoi(param[i + 2]);
						if (fcc_min_radar_num_pri[i] < 1
						    || fcc_min_radar_num_pri[i]
						    > 6) {
							printk("Error: fcc_min_radar_num_pri[%d] %d out of range [min=1] [max=6]\n", i, fcc_min_radar_num_pri[i]);
							ret = -1;
							break;
						}
					}
					if (!ret)
						ret = wlFwDFSParams(vmacSta_p->
								    dev,
								    DFS_SET_FCC_MIN_RADAR_NUM_PRI,
								    fcc_min_radar_num_pri,
								    etsi_min_radar_num_pri,
								    jpn_w53_min_radar_num_pri,
								    jpn_w56_min_radar_num_pri,
					...
					for (i = 0;
					     i <
					     (sizeof(etsi_min_radar_num_pri) /
					      sizeof(UINT8)); i++) {
						etsi_min_radar_num_pri[i] =
							atoi(param[i + 2]);
						if (etsi_min_radar_num_pri[i] <
						    1 ||
						    etsi_min_radar_num_pri[i] >
						    4) {
							printk("Error: etsi_min_radar_num_pri[%d] %d out of range [min=1] [max=4]\n", i, etsi_min_radar_num_pri[i]);
							ret = -1;
							break;
						}
					}
					if (!ret)
						ret = wlFwDFSParams(vmacSta_p->
								    dev,
								    DFS_SET_ETSI_MIN_RADAR_NUM_PRI,
								    fcc_min_radar_num_pri,
								    etsi_min_radar_num_pri,
								    jpn_w53_min_radar_num_pri,
								    jpn_w56_min_radar_num_pri,
					...
					for (i = 0;
					     i <
					     (sizeof(jpn_w53_min_radar_num_pri)
					      / sizeof(UINT8)); i++) {
						jpn_w53_min_radar_num_pri[i] =
							atoi(param[i + 2]);
						if (jpn_w53_min_radar_num_pri[i]
						    < 1 ||
						    jpn_w53_min_radar_num_pri[i]
						    > 2) {
							printk("Error: jpn_w53_min_radar_num_pri[%d] %d out of range [min=1] [max=2]\n", i, jpn_w53_min_radar_num_pri[i]);
							ret = -1;
							break;
						}
					}
					if (!ret)
						ret = wlFwDFSParams(vmacSta_p->
								    dev,
								    DFS_SET_JPN_W53_MIN_RADAR_NUM_PRI,
								    fcc_min_radar_num_pri,
								    etsi_min_radar_num_pri,
								    jpn_w53_min_radar_num_pri,
								    jpn_w56_min_radar_num_pri,
					...
					for (i = 0;
					     i <
					     (sizeof(jpn_w56_min_radar_num_pri)
					      / sizeof(UINT8)); i++) {
						jpn_w56_min_radar_num_pri[i] =
							atoi(param[i + 2]);
						if (jpn_w56_min_radar_num_pri[i]
						    < 1 ||
						    jpn_w56_min_radar_num_pri[i]
						    > 4) {
							printk("Error: jpn_w56_min_radar_num_pri[%d] %d out of range [min=1] [max=4]\n", i, jpn_w56_min_radar_num_pri[i]);
							ret = -1;
							break;
						}
					}
					if (!ret)
						ret = wlFwDFSParams(vmacSta_p->
								    dev,
								    DFS_SET_JPN_W56_MIN_RADAR_NUM_PRI,
								    fcc_min_radar_num_pri,
								    etsi_min_radar_num_pri,
								    jpn_w53_min_radar_num_pri,
								    jpn_w56_min_radar_num_pri,
								    ...
								    &etsi_zc_error_th,
								    &jp_zc_error_th);
				} else if (strcmp(param[1], "false_detect_th")
					   == 0) {
					false_detect_th = atoi(param[2]);
					if (false_detect_th < 1 ||
					    false_detect_th > 50) {
						printk("Error: false_detect_th %d out of range [min=1] [max=50]\n", false_detect_th);
						break;
					}
					ret = wlFwDFSParams(vmacSta_p->dev,
							    DFS_SET_FALSE_DETECT_TH,
							    fcc_min_radar_num_pri,
							    etsi_min_radar_num_pri,
							    jpn_w53_min_radar_num_pri,
							    jpn_w56_min_radar_num_pri,
							    ...
							    &etsi_zc_error_th,
							    &jp_zc_error_th);
				} else if (strcmp(param[1], "fcc_zc_error_th")
					   == 0) {
					fcc_zc_error_th = atoi(param[2]);
					if (fcc_zc_error_th < 1 ||
					    fcc_zc_error_th > 5) {
						printk("Error: fcc_zc_error_th %d out of range [min=1] [max=5]\n", fcc_zc_error_th);
						break;
					}
					ret = wlFwDFSParams(vmacSta_p->dev,
							    DFS_SET_FCC_ZC_ERROR_TH,
							    fcc_min_radar_num_pri,
							    etsi_min_radar_num_pri,
							    jpn_w53_min_radar_num_pri,
							    jpn_w56_min_radar_num_pri,
							    ...
							    &etsi_zc_error_th,
							    &jp_zc_error_th);
				} else if (strcmp(param[1], "etsi_zc_error_th")
					   == 0) {
					etsi_zc_error_th = atoi(param[2]);
					if (etsi_zc_error_th < 1 ||
					    etsi_zc_error_th > 5) {
						printk("Error: etsi_zc_error_th %d out of range [min=1] [max=5]\n", etsi_zc_error_th);
						break;
					}
					ret = wlFwDFSParams(vmacSta_p->dev,
							    DFS_SET_ETSI_ZC_ERROR_TH,
							    fcc_min_radar_num_pri,
							    etsi_min_radar_num_pri,
							    jpn_w53_min_radar_num_pri,
							    jpn_w56_min_radar_num_pri,
							    ...
							    &etsi_zc_error_th,
							    &jp_zc_error_th);
				} else if (strcmp(param[1], "jp_zc_error_th") ==
					   0) {
					jp_zc_error_th = atoi(param[2]);
					if (jp_zc_error_th < 1 ||
					    jp_zc_error_th > 5) {
						printk("Error: jp_zc_error_th %d out of range [min=1] [max=5]\n", jp_zc_error_th);
						break;
					}
					ret = wlFwDFSParams(vmacSta_p->dev,
							    DFS_SET_JP_ZC_ERROR_TH,
							    fcc_min_radar_num_pri,
							    etsi_min_radar_num_pri,
							    jpn_w53_min_radar_num_pri,
							    jpn_w56_min_radar_num_pri,
