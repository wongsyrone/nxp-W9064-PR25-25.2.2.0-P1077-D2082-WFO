...
#define WFO_DOL_CMDTO    (1<< 0)
#define WFO_DOL_BMQ_LEAK (1<< 1)
#endif
...
#ifdef WIFI_DATA_OFFLOAD
	UINT8 exceptionDolHangCause;	//flag to denote WFO hang cause(WFO used only) b0: cmd TO, b1: bmq buff leak
	UINT8 DolCmdTOChkIvl;	//interval counter for check Dol Cmd timeout (WFO used only) 
	UINT8 WFOBmLeakCnt;	//interval counter for checking WFO bmq buffer leak
#endif
