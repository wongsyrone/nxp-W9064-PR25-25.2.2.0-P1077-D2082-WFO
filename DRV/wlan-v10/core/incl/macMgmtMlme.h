...
*
*****************************************************************************/
extern void macMgmtMlme_AssociateReq(vmacApInfo_t * vmacSta_p,
...
*
*****************************************************************************/
extern void macMgmtMlme_ProbeRqst(vmacApInfo_t * vmacSta_p,
				  macmgmtQ_MgmtMsg3_t * MgmtMsg_p);
...
extern WL_STATUS macMgmtMlme_Init(vmacApInfo_t * vmacSta_p, UINT32 maxStns,
				  IEEEtypes_MacAddr_t * stnMacAddr);
extern void macMgmtMlme_PsPollMsg(macmgmtQ_MgmtMsg3_t * MgmtMsg_p,
				  UINT32 msgSize);
#ifdef SOC_W906X
...
extern void macMgmtMlme_SendDisassociateMsg4MAP(vmacApInfo_t * vmacSta_p,
						IEEEtypes_MacAddr_t * Addr,
						UINT16 StnId, UINT16 Reason);
...
int macMgmtMlme_SendAuthenticateMsg(vmacApInfo_t * vmacAP_p,
				    IEEEtypes_MacAddr_t * staMac, UINT16 seq,
				    UINT16 status_code, UINT8 * optie,
				    UINT8 optie_len);
#endif
SINT8 isMacAccessList(vmacApInfo_t * vmacSta_p,
		      IEEEtypes_MacAddr_t * destAddr_p);
...
void macMgmtMlme_SendChannelSwitchCmd(struct net_device *dev,
				      Dfs_ChanSwitchReq_t * pChannelSwitchCmd);
void macMgmtMlme_SwitchChannel(struct net_device *dev, UINT8 channel,
			       UINT8 channel2, CHNL_FLAGS * chanFlag_p);
...
extern WL_STATUS SendChannelSwitchCmd(vmacApInfo_t * vmacSta_p,
...
void IEEEToMrvlRateBitMapConversion(UINT8 SupportedIEEERate,
				    UINT32 * pMrvlLegacySupportedRateBitMap);
...
#ifdef MRVL_WAPI
extern void macMgmtMlme_WAPI_event(struct net_device *dev, int event_type,
				   u16 auth_type,
...
extern int MUchecksta_SetList(vmacApInfo_t * vmac_p,
			      extStaDb_StaInfo_t * pStaInfo);
extern int MUchecksta_StaList(vmacApInfo_t * vmac_p,
			      extStaDb_StaInfo_t * pStaInfo);
extern void MUDisplayMUSetList(vmacApInfo_t * vmac_p);
#ifdef SOC_W906X
extern UINT8 MUCreateMUSet(vmacApInfo_t * vmac_p,
			   extStaDb_StaInfo_t * pStaInfo[]);
#else
extern UINT8 MUCreateMUSet(vmacApInfo_t * vmacSta_p,
...
extern BOOLEAN MUAddStaToMUStaList(vmacApInfo_t * vmac_p,
				   extStaDb_StaInfo_t * StaInfo_p);
extern BOOLEAN MUDelStaFromMUStaList(vmacApInfo_t * vmac_p,
				     MUCapStaNode_t * MUStaNode_p);
extern void MUAutoSet_Hdlr(struct net_device *netdev);
#ifdef SOC_W906X
extern BOOLEAN MUManualSet(vmacApInfo_t * vmac_p,
			   extStaDb_StaInfo_t * pStaInfo[]);
#else
extern BOOLEAN MUManualSet(vmacApInfo_t * vmac_p,
			   extStaDb_StaInfo_t * pStaInfo0,
			   ...
			   extStaDb_StaInfo_t * pStaInfo2);
#endif
extern BOOLEAN MUDel_MUSet(vmacApInfo_t * vmac_p, muset_t * muset_p,
			   UINT8 option);
...
int macMgmtMlme_set_sta_associated(vmacApInfo_t * vmacAP_p,
				   IEEEtypes_MacAddr_t * staMac, UINT8 Aid,
				   PeerInfo_t * PeerInfo, UINT8 QosInfo,
				   UINT8 isQosSta, UINT8 rsnSta, UINT8 * rsnIE);
...
#ifdef MRVL_80211R
void macMgmtMlme_SendAssocMsg(vmacApInfo_t * vmacSta_p,
			      IEEEtypes_MacAddr_t * staMac, UINT8 * optie,
			      UINT8 optie_len);
int macMgmtMlme_SendAuthenticateMsg(vmacApInfo_t * vmacAP_p,
				    IEEEtypes_MacAddr_t * staMac, UINT16 seq,
				    UINT16 status_code, UINT8 * optie,
...
int macMgmtMlme_WNMAct(vmacApInfo_t * vmacSta_p,
		       macmgmtQ_MgmtMsg3_t * MgmtMsg_p);
void macMgmtMlme_RmBeaconRequest(struct net_device *netdev, UINT8 * stamac,
				 UINT8 * bssid, UINT8 RegDomain, UINT8 ch,
				 UINT8 RandInt, UINT8 MeasDur, UINT8 MeasMode,
				 UINT8 * ssid, UINT16 ReportCond,
				 UINT8 ReportDetail, UINT8 MeasDurMand,
				 UINT8 VoWiCase);

#endif
#ifdef CONFIG_IEEE80211W
int isRobustMgmtFrame(UINT16 Subtype);
int macMgmtBIP(vmacApInfo_t * vmac_p, macmgmtQ_MgmtMsg2_t * mgtFrm,
	       int payload_len);
#endif

void macMgmtMlme_UpdateProbeRspInfo(vmacApInfo_t * vmacSta_p,
				    UINT16 bcn_interval, UINT16 capinfo);
void macMgmtMlme_UpdateProbeRspExtraIes(vmacApInfo_t * vmacSta_p, UINT8 * iep,
					UINT16 length);
void macMgmtMlme_UpdateProbeRspBasicIes(vmacApInfo_t * vmacSta_p, UINT8 * iep,
					UINT16 length);
