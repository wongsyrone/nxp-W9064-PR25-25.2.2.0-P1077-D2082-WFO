...
static void
bsstm_disassoc_timeoutHdlr(UINT8 * data)
{
	...
	if (StaInfo_p->State == ASSOCIATED)
		macMgmtMlme_SendDisassociateMsg(vmacSta_p, &StaInfo_p->Addr,
						StaInfo_p->StnId,
						IEEEtypes_REASON_DISASSOC_AP_BUSY);
}

void
bsstm_set_disassoc_timer(vmacApInfo_t * vmacSta_p,
			 IEEEtypes_MacAddr_t * destaddr, UINT16 disassoc_time)
{
	extStaDb_StaInfo_t *StaInfo_p = NULL;
	UINT32 timeout = 0;

	StaInfo_p =
		extStaDb_GetStaInfo(vmacSta_p, destaddr,
	...
	TimerFireInByJiffies(&StaInfo_p->btmreq_disassocTimer, 1,
			     &bsstm_disassoc_timeoutHdlr,
			     (UINT8 *) StaInfo_p, timeout * TIMER_1MS);
}
...
//              printk("BTM Req failed\n");
	...
	if (BSS_TM_Req_p->disassoc_timer) {
		...
		bsstm_set_disassoc_timer(vmacSta_p,
					 (IEEEtypes_MacAddr_t *) destaddr,
					 BSS_TM_Req_p->disassoc_timer);
	}
	return TRUE;
}
