...
#endif
static WL_STATUS SendMREQUESTCmd(vmacApInfo_t * vmacSta_p,
				 IEEEtypes_MacAddr_t * MacAddr_p,
				 UINT8 MeasuredChannel);
...
#endif
static void smeMrequestIndProcess(vmacApInfo_t * vmacSta_p,
				  IEEEtypes_MRequestInd_t * MrequestInd_p);
static void smeMrequestCfrmProcess(vmacApInfo_t * vmacSta_p,
				   IEEEtypes_MRequestCfrm_t * MrequestCfrm_p);
static void smeMeasureCfrmProcess(vmacApInfo_t * vmacSta_p,
				  IEEEtypes_MeasureCfrm_t * MeasureCfrm_p);
static void smeMreportIndProcess(vmacApInfo_t * vmacSta_p,
				 IEEEtypes_MacAddr_t * MacAddr_p,
				 IEEEtypes_MReportInd_t * MreportInd_p);
static void smeMreportCfrmProcess(vmacApInfo_t * vmacSta_p,
				  IEEEtypes_MReportCfrm_t * MreportCfrm_p);
static void smeMTpcAdptCfrmdProcess(vmacApInfo_t * vmacSta_p,
				    IEEEtypes_TPCAdaptCfrm_t * TpcAdptCfrm_p);
static void smeChannelSwitchIndProcess(vmacApInfo_t * vmacSta_p,
				       IEEEtypes_ChannelSwitchInd_t *
				       ChannelSwitchInd_p);
static void smeChannelSwitchCfrmProcess(vmacApInfo_t * vmacSta_p,
					IEEEtypes_ChannelSwitchCfrm_t *
					ChannelSwitchCfrm_p);
...
#endif
static SME_CHANNEL_INFO_t
	semChannelUtilizationInfo[IEEE_80211_MAX_NUMBER_OF_CHANNELS];
...
*****************************************************************************/
extern WL_STATUS
smeMain_Init(vmacApInfo_t * vmacSta_p)
{
	//      WL_STATUS status;
	...
	TimerFireEvery(&vmacSta_p->reqMeasurementTimer, 1,
		       &StartSendMREQUESTCmd, vmacSta_p, 50);
#endif /* IEEE80211H */
#endif
	//    SendStartCmd();
	return (OS_SUCCESS);
...
*****************************************************************************/
WL_STATUS
SendChannelSwitchCmd(vmacApInfo_t * vmacSta_p,
		     Dfs_ChanSwitchReq_t * ChannelSwitchCmd_p)
{
	macmgmtQ_SmeCmd_t SmeCmd;
	macmgmtQ_SmeCmd_t *SmeCmd_p = &SmeCmd;

	if (ChannelSwitchCmd_p) {
		memset(SmeCmd_p, 0, sizeof(macmgmtQ_SmeCmd_t));
		SmeCmd_p->CmdType = SMC_CMD_CHANNELSWITCH_REQ;
		memcpy(&SmeCmd_p->Body.Dfs_ChanSwitchReq, ChannelSwitchCmd_p,
		       sizeof(Dfs_ChanSwitchReq_t));
		macMgmtQ_SmeWriteNoBlock(vmacSta_p, SmeCmd_p);

		return (OS_SUCCESS);
	} else {
		printk("Error: SendChannelSwitchCmd fail because ChannelSwitchCmd_p is NULL\n");
		return (OS_FAIL);
	}
}

extern void
SmeMgmt(vmacApInfo_t * vmacSta_p, UINT32 eventsTriggered, UINT8 * msg)
{
			...
			if (eventsTriggered & smeMain_MGMT_MSG_RCVD) {
				CfrmIndMsg_p = (smeQ_MgmtMsg_t *) msg;
				{
					...
					case SME_NOTIFY_START_CFRM:
						if (CfrmIndMsg_p->Msg.StartCfrm.
						    Result ==
						    START_RESULT_SUCCESS) {
						} else if (CfrmIndMsg_p->Msg.
							   StartCfrm.Result ==
					...
					case SME_NOTIFY_MREQUEST_IND:
						smeMrequestIndProcess(vmacSta_p,
								      &CfrmIndMsg_p->
								      Msg.
								      MrequestInd);
						break;
					...
					case SME_NOTIFY_MEASURE_CFRM:
						smeMeasureCfrmProcess(vmacSta_p,
								      &CfrmIndMsg_p->
								      Msg.
								      MeasureCfrm);
						break;
					...
					case SME_NOTIFY_MREPORT_CFRM:
						smeMreportCfrmProcess(vmacSta_p,
								      &CfrmIndMsg_p->
								      Msg.
								      MreportCfrm);
						break;
...
*****************************************************************************/
extern WL_STATUS
smeQ_MgmtWriteNoBlock(smeQ_MgmtMsg_t * MgmtMsg_p)
{
	SmeMgmt(MgmtMsg_p->vmacSta_p, smeMain_MGMT_MSG_RCVD,
		(UINT8 *) MgmtMsg_p);
	return (SUCCESS);
}
...
*****************************************************************************/
UINT32
CopySsId(UINT8 * SsId1, UINT8 * SsId2)
{
	UINT8 i;
	i = 0;
...
*****************************************************************************/
UINT32
SendStartCmd(vmacApInfo_t * vmacSta_p)
{
	MIB_802DOT11 *mib = vmacSta_p->Mib802dot11;
	MIB_STA_CFG *mib_StaCfg_p = vmacSta_p->Mib802dot11->StationConfig;
	...
	CopySsId(SmeCmd_p->Body.StartCmd.SsId, mib_StaCfg_p->DesiredSsId);
	{
		...
		SmeCmd_p->Body.StartCmd.SsParamSet.CfParamSet.ElementId =
			CF_PARAM_SET;
		SmeCmd_p->Body.StartCmd.SsParamSet.CfParamSet.CfpCnt = 0;	/* HW MAC fills later */
		SmeCmd_p->Body.StartCmd.SsParamSet.CfParamSet.CfpPeriod =
			mib_StaCfg_p->CfPeriod;
		SmeCmd_p->Body.StartCmd.SsParamSet.CfParamSet.CfpMaxDuration =
			mib_StaCfg_p->CfpMax;
		SmeCmd_p->Body.StartCmd.SsParamSet.CfParamSet.CfpDurationRemaining = 0;	/* HW MAC */
		SmeCmd_p->Body.StartCmd.SsParamSet.CfParamSet.Len =
			sizeof(IEEEtypes_CfParamSet_t) -
			sizeof(IEEEtypes_ElementId_t) - sizeof(IEEEtypes_Len_t);

		SmeCmd_p->Body.StartCmd.PhyParamSet.DsParamSet.ElementId =
			DS_PARAM_SET;
		...
		if (PhyDSSSTable_p->CurrChan == 0 || PhyDSSSTable_p->CurrChan2 == 0)	// || g_urAutoCfg)
		{
			...
			if (PhyDSSSTable_p->CurrChan == 0) {
				if ((*(mib->mib_ApMode) & AP_MODE_BAND_MASK) >=
				    AP_MODE_A_ONLY) {
					SmeCmd_p->Body.StartCmd.PhyParamSet.
				...
				SmeCmd_p->Body.StartCmd.PhyParamSet.DsParamSet.
					CurrentChan = PhyDSSSTable_p->CurrChan;
			}
			RfSwitchChanA =
				PhyDSSSTable_p->CurrChan ? PhyDSSSTable_p->
				CurrChan : chsel_a;
			RfSwitchChanG =
				PhyDSSSTable_p->CurrChan2 ? PhyDSSSTable_p->
				CurrChan2 : chsel_g;
		} else
#endif
		{
		...
		SmeCmd_p->Body.StartCmd.CapInfo.CfPollable =
			mib_StaCfg_p->CfPollable ? 1 : 0;
		SmeCmd_p->Body.StartCmd.CapInfo.CfPollRqst = 0;
		if (*(mib->mib_ApMode) != AP_MODE_B_ONLY &&
		    *(mib->mib_ApMode) != AP_MODE_AandG)
			SmeCmd_p->Body.StartCmd.CapInfo.ShortSlotTime =
		...
		if (mib_PrivacyTable_p->PrivInvoked ||
		    mib_PrivacyTable_p->WAPIEnabled)
#else
		if (mib_PrivacyTable_p->PrivInvoked)
#endif
		{
		...
		if (mib_StaCfg_p->mib_preAmble == PREAMBLE_LONG ||
		    mib_StaCfg_p->mib_preAmble == PREAMBLE_AUTO_SELECT ||
...
#ifdef IEEE80211H
		SmeCmd_p->Body.StartCmd.CapInfo.SpectrumMgmt =
			mib_StaCfg_p->SpectrumManagementRequired ? 1 : 0;
#endif /* IEEE80211H */
		...
		while (i < IEEEtypes_MAX_DATA_RATES_G &&
		       mib_StaCfg_p->OpRateSet[i] != '\0') {
			if (rateMask & 0x01) {

				if (basicRateMask & 0x01) {
					SmeCmd_p->Body.StartCmd.
						BssBasicRateSet[j] =
						(mib_StaCfg_p->
						 OpRateSet[i] & 0x7F);
					SmeCmd_p->Body.StartCmd.OpRateSet[k] =
						(mib_StaCfg_p->
						 OpRateSet[i] & 0x7F);
		...
		while (i < IEEEtypes_MAX_DATA_RATES_G &&
			...
			if (mib_StaCfg_p->OpRateSet[i] & 0x80) {
				SmeCmd_p->Body.StartCmd.BssBasicRateSet[j] =
					(mib_StaCfg_p->OpRateSet[i] & 0x7f);
				SmeCmd_p->Body.StartCmd.OpRateSet[k] =
					mib_StaCfg_p->OpRateSet[i] & 0x7f;
				i++;
				j++;
		...
		if (*(mib->mib_ApMode) == AP_MODE_AandG) {
			while (i < IEEEtypes_MAX_DATA_RATES_G &&
			       mib_StaCfg_p->OpRateSet2[i] != '\0') {
				if (mib_StaCfg_p->OpRateSet2[i] & 0x80) {
					SmeCmd_p->Body.StartCmd.
						BssBasicRateSet2[j] =
						(mib_StaCfg_p->
						 OpRateSet2[i] & 0x7f);
					SmeCmd_p->Body.StartCmd.OpRateSet2[k] =
						mib_StaCfg_p->
						OpRateSet2[i] & 0x7f;
					i++;
					j++;
			...
			if (i < IEEEtypes_MAX_DATA_RATES_G) {
				SmeCmd_p->Body.StartCmd.BssBasicRateSet2[j] =
					'\0';
		...
		memcpy(&vmacSta_p->macStaAddr, mib_OpData_p->StaMacAddr,
		       sizeof(IEEEtypes_MacAddr_t));

		if (*(mib->mib_ApMode) == AP_MODE_AandG)
			memcpy(&vmacSta_p->macStaAddr2, &vmacSta_p->macBssId2,
			       sizeof(IEEEtypes_MacAddr_t));

		memcpy(&vmacSta_p->macBssId, mib_OpData_p->StaMacAddr,
		       sizeof(IEEEtypes_MacAddr_t));
...
*****************************************************************************/
void
SendResetCmd(vmacApInfo_t * vmacSta_p, int quiet)
{
	...
	/*-----------------------------------------------------------*/
	{
		...
		macMgmtMlme_ResetCmd(vmacSta_p,
				     (IEEEtypes_ResetCmd_t *) & resetMsg_p->
				     Body);
		return;
	}
...
*****************************************************************************/
void
SendScanCmd(vmacApInfo_t * vmacSta_p, UINT8 * channels)
{
#ifdef AP_MAC_LINUX
	macmgmtQ_SmeCmd_t SmeCmd;
	int i, j;
	...
	memset(&SmeCmd.Body.ScanCmd.ChanList[0], 0,
	       IEEEtypes_MAX_CHANNELS + IEEEtypes_MAX_CHANNELS_A);
	if ((*(vmacSta_p->Mib802dot11->mib_ApMode) & AP_MODE_BAND_MASK) <
	    AP_MODE_A_ONLY) {
		for (i = 0, j = 0; (i < IEEEtypes_MAX_CHANNELS); i++) {
			if (channels[i] != 0) {
				SmeCmd.Body.ScanCmd.ChanList[j] = channels[i];
				j++;
			}
		}
		SmeCmd.Body.ScanCmd.ChanList[j] = 0;
	} else {
		for (i = 0, j = 0; (i < IEEEtypes_MAX_CHANNELS_A); i++) {
			if (channels[i + IEEEtypes_MAX_CHANNELS] != 0) {
				SmeCmd.Body.ScanCmd.ChanList[j] =
					channels[i + IEEEtypes_MAX_CHANNELS];
				j++;
			}
		}
		SmeCmd.Body.ScanCmd.ChanList[j] = 0;
	}
	syncSrv_ScanCmd(vmacSta_p,
			(IEEEtypes_ScanCmd_t *) & SmeCmd.Body.ScanCmd);
#endif
}
...
#ifdef IEEE80211H_NOTWIFI
void
StartSendMREQUESTCmd(void)
{
}
#endif
#define	MAX(a,b) (((a)>(b))?(a):(b))
WL_STATUS
StartSendChannelSwitchCmd(UINT8 Mode, UINT8 channel)
{
...
static WL_STATUS
SendMREQUESTCmd(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * MacAddr_p,
		UINT8 MeasuredChannel)
{
#ifdef AP_BUFFER
	MIB_STA_CFG *mib_StaCfg_p = vmacSta_p->Mib802dot11->StationConfig;
...
#ifdef AP_BUFFER
	if ((SmeCmd_p =
	     (macmgmtQ_SmeCmd_t *) pool_GetBuf(smeCmdPoolId)) != NULL) {
		IEEEtypes_MRequestCmd_t *MrequestCmd_p =
			&SmeCmd_p->Body.MrequestCmd;
		memset(SmeCmd_p, 0, sizeof(macmgmtQ_SmeCmd_t));
		SmeCmd_p->CmdType = SME_CMD_MREQUEST;
		memcpy(SmeCmd_p->Body.MrequestCmd.PeerStaAddr, MacAddr_p,
		       sizeof(IEEEtypes_MacAddr_t));
		...
		MrequestCmd_p->MeasureReqSet[0].MeasurementToken =
			getMeasurementToken();
		...
		MrequestCmd_p->MeasureReqSet[0].Request.Channel =
			MeasuredChannel;
		...
		if (STATE_SUCCESS ==
		    extStaDb_GetMeasurementInfo(vmacSta_p, MacAddr_p,
						&measureInfo)) {
			measureInfo.DiaglogToken = MrequestCmd_p->DiaglogToken;
			measureInfo.mBasic.mToken =
				MrequestCmd_p->MeasureReqSet[0].
				MeasurementToken;
			memcpy(measureInfo.mBasic.measureStartTime,
			       MrequestCmd_p->MeasureReqSet[0].Request.
			       StartTime, 8);
			measureInfo.mBasic.measureDuration =
				MrequestCmd_p->MeasureReqSet[0].Request.
				Duration;

			extStaDb_SetMeasurementInfo(vmacSta_p, MacAddr_p,
						    &measureInfo);
		}
		macMgmtQ_SmeWriteNoBlock(vmacSta_p, SmeCmd_p);
		return (OS_SUCCESS);
...
WL_STATUS
SendMREPORTCmd(vmacApInfo_t * vmacSta_p, IEEEtypes_MReportCmd_t * MReportCmd_p)
{
	//      MIB_802DOT11 *mib=vmacSta_p->Mib802dot11;
	macmgmtQ_SmeCmd_t SmeCmd;
	macmgmtQ_SmeCmd_t *SmeCmd_p = &SmeCmd;
	...
	memcpy(&SmeCmd_p->Body.MreportCmd, MReportCmd_p,
	       sizeof(IEEEtypes_MReportCmd_t));
	macMgmtQ_SmeWriteNoBlock(vmacSta_p, SmeCmd_p);
	return (OS_SUCCESS);
}
...
*/
static void
smeMrequestIndProcess(vmacApInfo_t * vmacSta_p,
		      IEEEtypes_MRequestInd_t * MrequestInd_p)
{
	MIB_802DOT11 *mib = vmacSta_p->Mib802dot11;
	MIB_STA_CFG *mib_StaCfg_p = vmacSta_p->Mib802dot11->StationConfig;
	...
	memcpy(&MreportCmd.PeerStaAddr, &MrequestInd_p->PeerStaAddr,
	       sizeof(IEEEtypes_MacAddr_t));
...
static void
smeMrequestCfrmProcess(vmacApInfo_t * vmacSta_p,
		       IEEEtypes_MRequestCfrm_t * MrequestCfrm_p)
{
	MIB_802DOT11 *mib = vmacSta_p->Mib802dot11;
	MIB_STA_CFG *mib_StaCfg_p = vmacSta_p->Mib802dot11->StationConfig;
...
static void
smeMeasureCfrmProcess(vmacApInfo_t * vmacSta_p,
		      IEEEtypes_MeasureCfrm_t * MeasureCfrm_p)
{
	MIB_802DOT11 *mib = vmacSta_p->Mib802dot11;
	MIB_STA_CFG *mib_StaCfg_p = vmacSta_p->Mib802dot11->StationConfig;
...
static void
semUpdateChannelUtilizationStatus(basic_info * basic_measurement_p)
{
	UINT32 loop = 0;

	do {
		if (semChannelUtilizationInfo[loop].channel ==
		    basic_measurement_p->measuredChannel) {
			semChannelUtilizationInfo[loop].mMap =
				basic_measurement_p->mMap;
			break;
		}
...
static void
smeChannelSelect(UINT8 * targetChannel_p)
{
	UINT32 loop = 0;
...
static void
smeMreportIndProcess(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * MacAddr_p,
		     IEEEtypes_MReportInd_t * MreportInd_p)
{
	MIB_802DOT11 *mib = vmacSta_p->Mib802dot11;
	MIB_STA_CFG *mib_StaCfg_p = vmacSta_p->Mib802dot11->StationConfig;
	...
	if (STATE_SUCCESS !=
	    extStaDb_GetMeasurementInfo(vmacSta_p, MacAddr_p, &measureInfo))
		return;

	/* check the diaglog sequence */
	if ((MreportInd_p->DiaglogToken != 0) &&
	    (MreportInd_p->DiaglogToken != measureInfo.DiaglogToken))
		return;
	...
	for (loop = 0; loop < MreportInd_p->ReportItems; loop++) {
		...
		case TYPE_REP_BASIC:
			if ((MreportInd_p->DiaglogToken == 0) ||
			    (measureInfo.mBasic.mToken ==
			     MreportInd_p->MeasureRepSet[loop].
			     MeasurementToken)) {
				measureInfo.mBasic.capability =
					MreportInd_p->MeasureRepSet[loop].Mode.
					Incapable ? FALSE : TRUE;
				measureInfo.mBasic.mMap =
					MreportInd_p->MeasureRepSet[loop].
					Report.data.Map;
				measureInfo.mBasic.measuredChannel =
					MreportInd_p->MeasureRepSet[loop].
					Report.Channel;
			}
			break;
	 ...
	 */
	if ((measureInfo.mBasic.measuredChannel == RfSwitchChanA) &&
	    (measureInfo.mBasic.mMap.Radar == 1)) {
		UINT8 targetChannel;
		...
		StartSendChannelSwitchCmd(1
					  /* keep quiet before channel switch */
					  , targetChannel);
	}
	return;
}
...
static void
smeMreportCfrmProcess(vmacApInfo_t * vmacSta_p,
		      IEEEtypes_MReportCfrm_t * MreportCfrm_p)
{
	MIB_802DOT11 *mib = vmacSta_p->Mib802dot11;
	MIB_STA_CFG *mib_StaCfg_p = vmacSta_p->Mib802dot11->StationConfig;
...
static void
smeMTpcAdptCfrmdProcess(vmacApInfo_t * vmacSta_p,
			IEEEtypes_TPCAdaptCfrm_t * TpcAdptCfrm_p)
{
	MIB_802DOT11 *mib = vmacSta_p->Mib802dot11;
	MIB_STA_CFG *mib_StaCfg_p = vmacSta_p->Mib802dot11->StationConfig;
...
static void
smeChannelSwitchIndProcess(vmacApInfo_t * vmacSta_p,
			   IEEEtypes_ChannelSwitchInd_t * ChannelSwitchInd_p)
{
	MIB_802DOT11 *mib = vmacSta_p->Mib802dot11;
	MIB_STA_CFG *mib_StaCfg_p = vmacSta_p->Mib802dot11->StationConfig;
	if (((*(mib->mib_ApMode) != AP_MODE_A_ONLY) &&
...
static void
smeChannelSwitchCfrmProcess(vmacApInfo_t * vmacSta_p,
			    IEEEtypes_ChannelSwitchCfrm_t * ChannelSwitchCfrm_p)
{
	MIB_802DOT11 *mib = vmacSta_p->Mib802dot11;
	MIB_STA_CFG *mib_StaCfg_p = vmacSta_p->Mib802dot11->StationConfig;
	if (((*(mib->mib_ApMode) != AP_MODE_A_ONLY) &&
...
#ifdef AP_BUFFER
static UINT8
getDiaglogToken(void)
{
	return D_Token++;
}

static UINT8
getMeasurementToken(void)
{
	return M_Token++;
}
#endif
static void
semChannelUtilizationInit(UINT8 * ChannelList)
{
	UINT32 loop;
	...
	for (loop = 0; loop < nbr_of_channel; loop++) {
		...
		memset(&semChannelUtilizationInfo[loop].mMap, 0,
		       sizeof(IEEEtypes_MeasurementRepMap_t));
	}
...
void
semShowChannelUtilizationStatus(void)
{
	//    UINT32 loop;
#ifdef DEBUG_ENABLE
	...
	for (loop = 0; loop < nbr_of_channel; loop++) {
		etherBugSend
			("Channel %d: BSS %d | OFDM %d | UnidentifiedSignal %d | Radar %d | Unmeasured %d\n\r",
			 semChannelUtilizationInfo[loop].channel,
