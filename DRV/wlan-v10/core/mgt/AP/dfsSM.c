...
UINT16 getCacTimerValue(UINT16 regionCode, UINT16 cacTimeout,
...
**************************************************************************/
UINT32
dfs_updateNOL(DfsApDesc * dfsDesc_p)
{
#ifdef CONCURRENT_DFS_SUPPORT
	if (dfsDesc_p->currChanInfo.chanflag.ChnlWidth == CH_160_MHz_WIDTH) {
		...
		if (UpdateNOL(dfsDesc_p, dfsDesc_p->currChanInfo.from) ==
		    DFS_FAILURE) {
			return DFS_FAILURE;
		}
	}
...
Dfs_Scan_Handle(DfsAp * me, MhsmEvent_t * msg)
{
	...
	case RADAR_EVT:
		...
		mib_shadow->PhyDSSSTable->CurrChan =
			dfsDesc_p->currChanInfo.channel;
		mib_shadow->PhyDSSSTable->SecChan =
			dfsDesc_p->currChanInfo.channel2;
		mib_shadow->PhyDSSSTable->Chanflag.isDfsChan =
			dfsDesc_p->currChanInfo.chanflag.isDfsChan;
		mib_shadow->PhyDSSSTable->Chanflag.isDfsChan2 =
...
Dfs_Operational_Handle(DfsAp * me, MhsmEvent_t * msg)
{
	...
	case RADAR_EVT:
		...
		if (DfsEnabledChannel(me->pNetDev, chanInfo_p)) {
			...
			{
				mdelay(10);
				((SMAC_CTRL_BLK_st *) wlpptr->ioBase0)->config.
					txAcStop = 0xf;
...
DfsPrintNOLChannelDetails(DfsAp * me, char *NOLPrintStr_p, int maxLength)
{
	...
	while (nocListItem_p) {
		len += sprintf(buf + len,
			       ...
			       (jiffies - nocListItem_p->occurance) / HZ);
