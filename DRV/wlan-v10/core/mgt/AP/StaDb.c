...
extStaDb_AddSta(vmacApInfo_t * vmac_p, extStaDb_StaInfo_t * StaInfo_p)
{
	...
	if (tmp) {
		...
		if (vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_AP) {
			...
			dol_add_sta(wlpptr, wlpptr->wlpd_p->ipc_session_id,
...
#ifdef WIFI_DATA_OFFLOAD
	if (vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_AP)
		dol_del_sta(wlpptr, wlpptr->wlpd_p->ipc_session_id,
			    wlpptr->vmacSta_p->VMacEntry.macId, (u8 *) Addr_p);
#endif
