...
void
ipc_fun_deinit(ca_ipc_session_id_t session_id)
{
	if (handle_array)
		wl_kfree(handle_array);
...
void
ipc_check_msg(ca_ipc_session_id_t session_id)
{
	char buf[256];
	...
	while (t2h_fifo_peek(buf, buf_size)) {
		msg_size = t2h_fifo_out(buf, buf_size);
			   ...
			   ca_uint32_t msg_handle_count)
{
	...
	if (msg_handle_count) {
		...
		handle_array = wl_kzalloc(handle_size, GFP_KERNEL);
		if (!handle_array)
			return -ENOMEM;
