...
#endif
#ifdef WIFI_DATA_OFFLOAD
void enableAmpduTx(vmacApInfo_t * vmacSta_p, UINT8 * macaddr, UINT8 tid);
#else
	...
	if (wlpptr->wlpd_p->Ampdu_tx[stream].InUse == 1) {
		...
		if (pStaInfo) {
			...
			dol_sta_tx_ampdu_ctrl(wlpptr,
					      ...
					      wlpptr->vmacSta_p->VMacEntry.
					      macId, (u8 *) pStaInfo->Addr,
					      pStaInfo->aggr11n.threshold,
					      &pStaInfo->aggr11n.startbytid[0]);
#endif
		...
		/** If a blockack req is received, all complete MSDUs with lower sequence numbers than the starting sequence number contained in the
		   ...
		   recipient shall indicate the MSDUs starting with the Starting Sequence number sequentially until there is an incomplete MSDU in the buffer **/
			...
			if (!wlpptr->wlpd_p->dol.disable) {
				...
				dol_set_ba_req(wlpptr,
					       wlpptr->wlpd_p->ipc_session_id,
					       wlpptr->vmacSta_p->VMacEntry.
					       macId, pStaInfo->StnId, Priority,
					       SeqNo);
			} else {
#endif
				baRxInfo =
					&wlpptr->wlpd_p->
					AmpduPckReorder[pStaInfo->StnId];
				SPIN_LOCK(&baRxInfo->ba[Priority].BAreodrLock);

				SeqNo = BaReqFrm->Seq_Ctrl.StartSeqNo;
				winStartB = baRxInfo->ba[Priority].winStartB;
				storedBufCnt =
					baRxInfo->ba[Priority].storedBufCnt;
					...
					if (storedBufCnt != 0)	// Only if there is at least one saved reorder buffer
					{
						...
						baRxInfo->ba[Priority].
							...
							BA_flushAnyData(dev,
									...
									[Priority],
									temp,
									(((baRxInfo->ba[Priority].winSizeB) << 16) | winDelta), &pStaInfo->pn->ucRxQueues[Priority], &wlexcept_p->badPNcntUcast, &wlpptr->wlpd_p->drv_stats_val.rx_data_ucast_pn_pass_cnt, tid);
						storedBufCnt =
							baRxInfo->ba[Priority].
							storedBufCnt;

						BA_TimerActivateCheck(dev,
								      pStaInfo,
								      baRxInfo,
								      Priority,
								      SeqNo, 0);
				...
				SPIN_UNLOCK(&baRxInfo->ba[Priority].
					    BAreodrLock);
#ifdef WIFI_DATA_OFFLOAD
			}
#endif
