...
UINT16
Add_VendorSpec_VHT_IE(vmacApInfo_t * vmacSta_p,
		      IEEEtypes_HT_Element_t * pNextElement)
{
	int len = 0;
	memset(&VendorSpec_VHT_IE, 0,
	       sizeof(IEEEtypes_VendorSpec_VHT_Element_t));
		  ...
		  IEEEtypes_ChannelReportEL_t * elment)
{
	...
	if (elment->RegClass != 32 &&
	    elment->RegClass != 33 &&
	    elment->RegClass != 12 && elment->RegClass != 11)
...
UINT16
Build_IE_191(vmacApInfo_t * vmacSta_p, UINT8 * IE_p, UINT8 isEffective,
	     UINT8 nss)
{
	MIB_802DOT11 *mib = vmacSta_p->Mib802dot11;
	...
	if (wlpptr->devid == SC5) {
		...
		if (isEffective && nss) {
			printk("%s: isEffective vht_cap=0x%x nss=%u\n",
			       __func__, vht_cap, nss);
			vht_cap &= ~(7 << 13);	//bit[13:15]
			vht_cap |= ((nss - 1) << 13);
			printk("After set bit[13:15] vht_cap=0x%x\n", vht_cap);
		}
	}
#endif
	if (*(mib->mib_bfmee) == 1) {
...
#ifdef CB_SUPPORT
UINT16
Build_IE_cb(vmacApInfo_t * vmacSta_p, UINT8 * pBcnBuf)
{
	struct wlprivate *wlpptr =
		NETDEV_PRIV_P(struct wlprivate, vmacSta_p->dev);

	if ((0 < wlpptr->custie_len) &&
	    (wlpptr->custie_len <= sizeof(wlpptr->cust_ie))) {
