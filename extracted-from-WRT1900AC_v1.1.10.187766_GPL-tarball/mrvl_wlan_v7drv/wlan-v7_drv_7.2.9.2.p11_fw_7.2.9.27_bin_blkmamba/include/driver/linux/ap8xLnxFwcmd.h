...
extern int wlFwCreateBAStream(struct net_device *dev, 	u_int32_t BarThrs, u_int32_t WindowSize , u_int8_t *Macaddr,
                              ...
                              u_int8_t *SrcMacaddr, UINT16 seqNo, UINT8 macid);
#endif
extern int wlFwSetMacAddr_Client(struct net_device *netdev, UINT8 *macAddr, u_int8_t);
...
extern int wlFwMediaBusyIdleTime(struct net_device *dev, char *page);
...
int wlFwGet_Device_PwrTbl(struct net_device *netdev, channel_power_tbl_t *EEPROM_CH_PwrTbl, UINT8 *region_code, UINT8 *number_of_channels, UINT32 channel_index);
