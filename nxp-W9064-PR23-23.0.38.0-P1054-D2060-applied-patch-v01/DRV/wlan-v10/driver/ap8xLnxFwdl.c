...
#ifdef SOC_W906X
static void
fw_chunk_transfer(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	unsigned int reg_event_offset =
		wlpptr->wlpd_p->reg.fw_int_event_offeset;
	unsigned int reg_gen_ptr = wlpptr->wlpd_p->reg.gen_ptr;

	/* write location of data */
	wl_util_writel(netdev, wlpptr->wlpd_p->pPhysFwDlBuf, wlpptr->ioBase1 + reg_gen_ptr);

	/* inform bootcode */
	wl_util_writel(netdev, MACREG_H2ARIC_BIT_DOOR_BELL, wlpptr->ioBase1 + reg_event_offset);
}

static inline void
fw_setup_intr(struct wlprivate *wlpptr)
{
	unsigned int trigger = wlpptr->wlpd_p->reg.fw_setup_int_trigger;
	unsigned int reg_a2h_intr_clear_sel =
	...
	unsigned int reg_a2h_status_mask =
		wlpptr->wlpd_p->reg.a2h_int_status_mask;

	wl_util_writel(wlpptr->netDev, MACREG_A2HRIC_BIT_MASK, wlpptr->ioBase1 + reg_a2h_intr_clear_sel);
	wl_util_writel(wlpptr->netDev, 0x00, wlpptr->ioBase1 + reg_a2h_intr_cause);
	wl_util_writel(wlpptr->netDev, 0x00, wlpptr->ioBase1 + reg_a2h_intr_mask);
	wl_util_writel(wlpptr->netDev, trigger, wlpptr->ioBase1 + reg_a2h_status_mask);
}
...
wlFwDownload(struct net_device *netdev)
{
	...
	if (IS_BUS_TYPE_MCI(wlpptr)) {
		...
		wl_util_writel(netdev, MACREG_A2HRIC_BIT_MASK_MSI, wlpptr->ioBase1 + MACREG_REG_A2H_INTERRUPT_CLEAR_SEL);
		wl_util_writel(netdev, 0x00, wlpptr->ioBase1 + MACREG_REG_A2H_INTERRUPT_CAUSE);
		wl_util_writel(netdev, 0x00, wlpptr->ioBase1 + MACREG_REG_A2H_INTERRUPT_MASK);
		wl_util_writel(netdev, MACREG_A2HRIC_BIT_MASK_MSI, wlpptr->ioBase1 + MACREG_REG_A2H_INTERRUPT_STATUS_MASK);
	...
	while (wl_util_readl(netdev, wlpptr->ioBase1 + reg_len_offset) == 0) ;

	while (sizeFwDownloaded < wlpptr->FwSize) {
		len = wl_util_readl(netdev, wlpptr->ioBase1 + reg_len_offset);
	...
	wl_util_writel(netdev, 0x100,wlpptr->ioBase1+0xE0E4);

	/* make sure SCRATCH2 C40 is clear, in case we are too quick */
	while (wl_util_readl(netdev, wlpptr->ioBase1 + 0xc40) == 0);
#ifdef NEW_DP
	/* download ddr init code */
	printk("init ddr...\n");

	while (sizeDdrInitDownloaded < sizeDdrInit) {
		len = wl_util_readl(netdev, wlpptr->ioBase1 + 0xc40);
		if (!len)
			break;
		...
		do {
			intCode = wl_util_readl(netdev, wlpptr->ioBase1 + 0xc1c);
			if (intCode != 0)
				break;
			currIteration--;
	...
	while (sizeFwDownloaded < wlpptr->FwSize) {
		...
		/* NOTE: the following back to back checks on C1C is time sensitive, hence
		   ...
		   can alternatively tweak this routines to fit your platform */
		do {
			intCode = wl_util_readl(netdev, wlpptr->ioBase1 + reg_cause_offset);
			if (intCode != 0)
				break;
			currIteration--;
		} while (currIteration);
	...
	do {
		currIteration--;
		wl_util_writel(netdev, OpMode, wlpptr->ioBase1 + reg_gen_ptr);
#ifdef SOC_W906X
		if (IS_BUS_TYPE_MCI(wlpptr))
			mdelay(FW_CHECK_MSECS);
		else
		...
		intCode = wl_util_readl(netdev, wlpptr->ioBase1 + reg_int_code);
#ifndef SOC_W906X
...
complete:
	...
	printk("wlFwDownload complete\n");
#endif /* #ifdef SOC_W906X */
	wl_util_writel(netdev, 0x00, wlpptr->ioBase1 + reg_int_code);
...
#ifndef SOC_W906X
static void
wltriggerPciCmd(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	...
	wl_util_writel(netdev, MACREG_H2ARIC_BIT_DOOR_BELL, 		wlpptr->ioBase1 + MACREG_REG_H2A_INTERRUPT_EVENTS);
}
#endif /* #ifndef SOC_W906X */

static void
wltriggerPciCmd_bootcode(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);

#ifdef SOC_W906X
	/* write location of data to c10 */
	wl_util_writel(netdev, wlpptr->wlpd_p->pPhysFwDlBuf, wlpptr->ioBase1 + MACREG_REG_GEN_PTR);
#else
	/* write location of data to c10 */
	wl_util_writel(netdev, wlpptr->wlpd_p->pPhysCmdBuf, wlpptr->ioBase1 + MACREG_REG_GEN_PTR);
#endif
...
#define EXTERNAL_MFG_FILE_NAME_FOR_SC5 "/lib/firmware/marvell/W9068-mfg.bin"
#define EXTERNAL_MFG_FILE_NAME_FOR_SCBT "/lib/firmware/marvell/W9064-mfg.bin"
...
#define EXTERNAL_FILE_NAME_FOR_SCBT "/lib/firmware/marvell/W9064.bin"
#define EXTERNAL_FILE_NAME_FOR_SCBT_A0 "/lib/firmware/marvell/W9064.bin"
#define EXTERNAL_FILE_NAME_FOR_SCBT_STA "/lib/firmware/marvell/W9064-STA.bin"
#define EXTERNAL_FILE_NAME_FOR_SCBT_AP "/lib/firmware/marvell/W9064-AP.bin"
#endif
#else
...
static int
wlGetCalFile(char *filename, char *kbuf)
{
	struct file *filp;
	int i = 0;
	int skipCount = 6;
