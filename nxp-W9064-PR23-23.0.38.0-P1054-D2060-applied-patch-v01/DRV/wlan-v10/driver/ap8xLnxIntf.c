...
int rddelay = 0;
module_param(rddelay, int, S_IRUGO);
MODULE_PARM_DESC(rddelay, "read delay");

int wrdelay = 0;
module_param(wrdelay, int, S_IRUGO);
MODULE_PARM_DESC(wrdelay, "write delay");

int intmode = 0;
module_param(intmode, int, S_IRUGO);
MODULE_PARM_DESC(intmode, "interrupt mode, 0: MSI-X, 1: MSI, 2: Legacy");
...
/* external functions for process mgmt work queue */
extern void macMgmt_txMgmtWaitQInit(struct net_device *netdev);
extern void macMgmt_txMgmtWaitQReset(struct net_device *netdev);
extern void macMgmt_txMgmtWaitQDeInit(struct net_device *netdev);
...
wl_init_intr(struct wlprivate *wlpptr)
{
	...
	if (!IS_BUS_TYPE_MCI(wlpptr)) {
		if (wlpptr->intr_type == INTR_TYPE_MSIX) {
			...
			//0x38
			wl_util_writel(wlpptr->netDev, SC5_PCIE_MODE_GIC,
			...
			wl_util_writel(wlpptr->netDev, PCI_REG_DOORBELL_ADDR,
			...
			wl_util_writel(wlpptr->netDev, SMAC_CTRLBASE_NSS_PCIE_HI_VAL_NOINTR,
			       wlpptr->ioBase1 +
			       SC5_REG_SMAC_CTRLBASE_NSS_PCIE_HI);
			wl_util_writel(wlpptr->netDev, PCI_REG_HOST_ITR_ADDR,
			       wlpptr->ioBase1 + SC5_REG_FRAME_0);
			wl_util_writel(wlpptr->netDev, PCI_REG_HOST_ITR_ADDR,
			       wlpptr->ioBase1 + SC5_REG_FRAME_1);
			wl_util_writel(wlpptr->netDev, PCI_REG_HOST_ITR_ADDR,
		...
		wl_util_writel(wlpptr->netDev, wlpptr->wlpd_p->sysintr_frm.frm_base,
		       wlpptr->ioBase1 + SC5_REG_FRAME_0);
		...
		wl_util_writel(wlpptr->netDev, wlpptr->hframe_phy_addr,
		...
		wl_util_writel(wlpptr->netDev, regval,
		...
		wl_util_writel(wlpptr->netDev, wlpptr->wlpd_p->sysintr_frm.spi_num,
...
wl_hook_intr(struct wlprivate *wlpptr)
{
	...
	if (wlpptr->intr_type == INTR_TYPE_MSIX) {
		...
		for (idx = 0; idx < wlpptr->wlpd_p->msix_num; idx++) {
			...
			wl_util_writel(wlpptr->netDev, 0, wlpptr->ioBase1 + SC5_REG_FRM_SEL(qid, ((int)(issq)&0x1)));
			wl_util_writel(wlpptr->netDev, BIT(idx), wlpptr->ioBase1 + SC5_REG_EFF_ID(qid, ((int)(issq)&0x1)));
		}
...
#define WIFIARB_POST_REQUEST_INTR_DEV_1 "wifiarb_post_request_dev1"
	...
	/* Set hframe register base */
	wl_util_writel(wlpptr->netDev, wlpptr->hframe_phy_addr, wlpptr->ioBase1 + SC5_REG_HFRAME_BASE);
		   ...
		   BOOLEAN issq)
{
	...
	//      ref: wl_init_intr()
	wl_util_writel(wlpptr->netDev, 0, wlpptr->ioBase1 + SC5_REG_FRM_SEL(qid, ((int)(issq)&0x1)));
	WLDBG_INFO(DBG_LEVEL_0, "[FRM_SEL] wl_util_writel(ioBase1+%xh) = %xh\n",
		   SC5_REG_FRM_SEL(qid, ((int)(issq) & 0x1)), 0);
	// Set the interrupt
	wl_util_writel(wlpptr->netDev, intr_id+wlpptr->wlpd_p->sysintr_frm.spi_num,
...
#define WIFIARB_POST_REQUEST_INTR_DEV_1 "wifiarb_post_request_dev1"
	...
	if (!intmode && pci_find_capability(pdev, PCI_CAP_ID_MSIX)) {
		int msi_err = 0;
...
try_msi:
	...
	if (!wlpptr->msix_entries) {
		...
		if ((intmode == 1) && (msi_err = pci_enable_msi(pdev)) == 0) {
			wlpptr->netDev->irq = pdev->irq;
...
mfg_send_cmd(struct net_device *netdev)
{
	...
	wl_util_writel(netdev, wlpptr->wlpd_p->pPhysCmdBuf, wlpptr->ioBase1 + 0xcd0);
	wl_util_writel(netdev, 0x00, wlpptr->ioBase1 + 0xcd4);
	wl_util_writel(netdev, 0x00, wlpptr->ioBase1 + MACREG_REG_INT_CODE);
	wl_util_writel(netdev, len + 4, wlpptr->ioBase1 + 0xc40);
#endif
#if 0				//For MOCHI
	...
	wl_util_writel(netdev, wlpptr->wlpd_p->pPhysCmdBuf,
...
wlInit(struct net_device *netdev, u_int16_t devid)
{
	...
	spin_lock_init(&wlpptr->wlpd_p->pcie_iolock);
	smeCmdBuf_q =
...
//A390/A385 platform supports only 16 interrupts => Use polling function instead
	...
	if (wlPrepareFwFile(netdev) == FAIL) {
		...
		if (SMAC_HAL_VERSION >= 0x0080) {
			...
			wl_util_writel(netdev, SMAC_HAL_VERSION, &wlpptr->smacCfgAddr->magic);
			//wlpptr->smacCfgAddr->magic = SMAC_HAL_VERSION;
		}

		printk("=> %s(), MAC_STATUS_st->verCtrl[3] = %xh\n", __func__,
		       0xF0000000);
		wl_util_writel(netdev, 0xF0000000, &wlpptr->smacStatusAddr->verCtrl[3]);
...
#ifndef NEW_DP
	wl_util_writel(netdev, (wlpptr->wlpd_p->descData[0].pPhysTxRing),
	       wlpptr->ioBase0 + wlpptr->wlpd_p->descData[0].wcbBase);
#if NUM_OF_DESCRIPTOR_DATA >3
	int i;
	for (i = 1; i < TOTAL_TX_QUEUES; i++)
		wl_util_writel(netdev, (wlpptr->wlpd_p->descData[i].pPhysTxRing),
		       wlpptr->ioBase0 + wlpptr->wlpd_p->descData[i].wcbBase);
#endif
	wl_util_writel(netdev, (wlpptr->wlpd_p->descData[0].pPhysRxRing),
...
wlDeinit(struct net_device *netdev)
{
	...
	wl_util_writel(netdev, 0x0, wlpptr->ioBase1 + SC5_REG_PCIE_INTR_MODE_SEL);
...
wlSC5MSIX_r7(int irq, void *dev_id)
{
	...
	if ((msg_id & 0x01) == 0) {	//RQ = 0
		...
		if ((intStatus & SC5_EVENT_FW) && (wlpptr->event_bufq_vaddr)) {
			UINT32 wrptr, rdptr;
			rdptr = wl_util_readl(netdev, wlpptr->ioBase1 + reg_evt_rdptr);
			wrptr = wl_util_readl(netdev, wlpptr->ioBase1 + reg_evt_wrptr);
			...
			while (rdptr != wrptr) {
				...
				wl_util_writel(netdev, rdptr, wlpptr->ioBase1 + reg_evt_rdptr);
			}
	...
	do {
		...
		if (intStatus != 0x00000000) {
			...
			if (!wlpptr->wlpd_p->pPciDev->msi_enabled) {
				...
				tmpval = readl(wlpptr->ioBase1 + SC5_MACREG_REG_INTERRUPT_CAUSE);
			}
		}
		wl_util_unlock(netdev);
...
#endif
	do {
		intStatus = (wl_util_readl(netdev, wlpptr->ioBase1 + MACREG_REG_A2H_INTERRUPT_CAUSE));
#ifdef NAPI
		mask = (wl_util_readl(netdev, wlpptr->ioBase1 + MACREG_REG_A2H_INTERRUPT_MASK));
#endif
		if (intStatus != 0x00000000) {
			if (intStatus == 0xffffffff) {
				WLDBG_INFO(DBG_LEVEL_2, "card plugged out???");
			...
			intStatus &= mask;
#endif
			wl_util_writel(netdev, (MACREG_A2HRIC_BIT_MASK & ~intStatus),
			       wlpptr->ioBase1 +
			       MACREG_REG_A2H_INTERRUPT_CAUSE);
		}
		if ((intStatus & ISR_SRC_BITS) ||
			...
			if (intStatus & MACREG_A2HRIC_BIT_QUEUE_EMPTY) {
				...
				if (extStaDb_AggrFrameCk(vmacSta_p, 1)) {
					...
					wl_util_writel(netdev, MACREG_H2ARIC_BIT_PPA_READY,
					       wlpptr->ioBase1 +
					       MACREG_REG_H2A_INTERRUPT_EVENTS);
				}
				retVal = IRQ_HANDLED;
...
wlInterruptEnable(struct net_device *netdev)
{
	...
	if (wlChkAdapter(netdev)) {
		...
		wl_util_writel(netdev, 0x00, wlpptr->ioBase1 + reg_a2h_intr_mask);
		//writel((MACREG_A2HRIC_BIT_MASK),
		printk("%s(), w_reg(%p)=%xh\n", __func__,
		       (wlpptr->ioBase1 + reg_a2h_intr_mask), hfctrl);
		wl_util_writel(netdev, hfctrl, wlpptr->ioBase1 + reg_a2h_intr_mask);
#else
		wl_util_writel(netdev, 0x00, wlpptr->ioBase1 + MACREG_REG_A2H_INTERRUPT_MASK);

		wl_util_writel(netdev, (MACREG_A2HRIC_BIT_MASK),
...
wlInterruptUnMask(struct net_device *netdev, int mask)
{
	...
	if (wlChkAdapter(netdev)) {
#ifdef SOC_W906X
		wl_util_writel(netdev, (wl_util_readl(netdev, wlpptr->ioBase1 + reg_a2h_intr_mask) | (mask)),
		       wlpptr->ioBase1 + reg_a2h_intr_mask);
#else
		wl_util_writel(netdev, (wl_util_readl(netdev, wlpptr->ioBase1 + MACREG_REG_A2H_INTERRUPT_MASK) | (mask)),
				wlpptr->ioBase1 + MACREG_REG_A2H_INTERRUPT_MASK);
#endif /* SOC_W906X */
	}
}
...
wlInterruptMask(struct net_device *netdev, int mask)
{
	...
	if (wlChkAdapter(netdev)) {
		...
		wl_util_writel(netdev, (wl_util_readl(netdev, wlpptr->ioBase1 + reg_a2h_intr_mask) & (~mask)),
		       wlpptr->ioBase1 + reg_a2h_intr_mask);
#else
		wl_util_writel(netdev, (wl_util_readl(netdev, wlpptr->ioBase1 + MACREG_REG_A2H_INTERRUPT_MASK)& (~mask)), 			wlpptr->ioBase1 + MACREG_REG_A2H_INTERRUPT_MASK);
#endif /* SOC_W906X */
	}
}
	...
	if (!IS_BUS_TYPE_MCI(wlpptr)) {
		if (wlChkAdapter(netdev)) {
			WLDBG_INFO(DBG_LEVEL_2, "write ISR_RESET to %u\n",
				   MACREG_REG_H2A_INTERRUPT_EVENTS);
			wl_util_writel(netdev, ISR_RESET, wlpptr->ioBase1 + MACREG_REG_H2A_INTERRUPT_EVENTS);
		} else {
...
#ifdef SOC_W906X
	regval = wl_util_readl(netdev, wlpptr->ioBase1 + reg_int_code);
#else
	regval = wl_util_readl(netdev, wlpptr->ioBase1 + MACREG_REG_INT_CODE);
#endif
...
wltxwait_txdone(struct net_device *netdev)
{
	...
	do {
		mdelay(10);
		wlTxDone(netdev);
		txDoneHeadnew = wl_util_readl(netdev, priv->ioBase1 + MACREG_REG_TxDoneHead);
		txDoneTail =
...
#ifdef SOC_W906X
		txSendTail = wl_util_readl(netdev, priv->ioBase1 + priv->wlpd_p->reg.tx_send_tail);
#else
		txSendTail = wl_util_readl(netdev, priv->ioBase1 + MACREG_REG_TxSendTail);
#endif /* SOC_W906X */
	}
#ifdef SOC_W906X
...
*/
	...
	wl_util_writel(netdev, priv->hframe_phy_addr, priv->ioBase1 + SC5_REG_HFRAME_BASE);
...
wlReinit(struct net_device *netdev)
{
	...
	printk("=> %s(), MAC_STATUS_st->verCtrl[3] = %xh\n", __func__,
	       0xF0000000);
	wl_util_writel(netdev, 0xF0000000, &parent_wlpptr->smacStatusAddr->verCtrl[3]);
	if (wlInitChkSmacRdy(netdev) == FALSE) {
...
wlFwHardResetAndReInit(struct net_device *netdev, U8 halt)
{
	...
	printk("Stopping RX path\n");
	{
		...
		*(u32 *) (&((SMAC_CTRL_BLK_st *) priv->ioBase0)->config.
			  rxEnable) = 0;
		regval = wl_util_readl(netdev, priv->ioBase1 + BBRX_CFG);
		if (regval & 0x01)
			wl_util_writel(netdev, (regval & 0xFFFFFFFE), priv->ioBase1 + BBRX_CFG);
	}
	/* disable all interupts from hframe */
...
wlopen_mbss(struct net_device *netdev)
{
	...
	macMgmt_txMgmtWaitQInit(wlpptr->netDev);
...
wlstop_mbss(struct net_device *netdev)
{
	...
	if (netdev->flags & IFF_RUNNING) {
		...
		macMgmt_txMgmtWaitQReset(netdev);
...
wlDeinit_mbss(struct net_device *netdev)
{
...
#endif
	{
		...
		macMgmt_txMgmtWaitQDeInit(wlpptr->vdev[i]);
		wlp->vmacSta_p = NULL;
