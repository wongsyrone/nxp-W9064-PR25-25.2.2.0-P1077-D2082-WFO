...
extern UINT32 acnt_tx_record_idx;
				 ...
				 UINT8 * macAddr, char *buf)
{
	...
	if ((!strcmp(mode, "ax") || !strcmp(mode, "ac")) &&
		...
		if ((acnt_tx_record_dump =
			...
			currIdx = (acnt_tx_record_idx == 0) ?
				(ACNT_TX_RECORD_MAX - 1) : (acnt_tx_record_idx -
							    1);
			...
			for (idx = 0; idx < ACNT_TX_RECORD_MAX; idx++) {
				if (StaInfo_p->StnId ==
					...
					tx_sgi_mode =
						(acnt_tx_record_dump[currIdx].
						 rateInfo & 0x000000c0) >> 6;
					bw = (acnt_tx_record_dump[currIdx].
					      rateInfo & 0x00000030) >> 4;
					tx_dcm = (acnt_tx_record_dump[currIdx].
						  rateInfo & 0x00000008) >> 3;
