...
typedef struct _rxdbg_intf {
	...
	void (*rxdbg_push_errcfhul) (void *prxdbg,
				     wl_cfhul_amsdu * pcfhul_amsdu,
				     wlrxdesc_t * pcfhul);
	wlrxdesc_t *(*rxdbg_pull_errcfhul) (void *prxdbg);
	void (*rxdbg_push) (void *prxdbg, wlrxdesc_t * pcfhul, int qid,
			    u16 rdinx, u16 wrinx);
...
void wl_free_cfhul_lo(u_int32_t tmp_lodword_addr, struct net_device *netdev,
		      u32 bpid);
void wl_update_cfhul_buf_rec(cfhul_buf_pool * pcfhul_buf_pool,
			     struct net_device *netdev, int qid, u32 rdinx,
			     wlrxdesc_t * cfh_ul);
#if 1
void wl_clr_cfhul_buf_rec(cfhul_buf_pool * pcfhul_buf_pool, int qid, u32 rdinx,
			  wlrxdesc_t * pcfh_ul);
#else
void wl_clr_cfhul_buf_rec(struct net_device *netdev, int qid,
			  wlrxdesc_t * cfh_ul);
#endif

void wl_save_last_rxskb(struct net_device *netdev, wlrxdesc_t * cfh_ul,
			struct sk_buff *skb);
void wl_clr_last_rxskb(struct net_device *netdev, u32 wlqm_aryid);
extern UINT32 wl_ch_load(vmacApInfo_t * vmacSta_p, UINT32 delta_time,
			 UINT32 slotTickCnt, UINT8 scale_mapping);
...
void *wl_util_kmalloc(size_t size, gfp_t flags, const char *func,
		      const int line);
void wl_util_kfree(const void *ptr, const char *func, const int line);
void *wl_util_dma_alloc_coherent(struct device *dev, size_t size,
				 dma_addr_t * dma_handle, int flag,
				 const char *func, const int line);
void wl_util_dma_free_coherent(struct device *dev, size_t size, void *cpu_addr,
...
extern SINT16 wl_util_get_nf(struct net_device *netdev,
			     NfPathInfo_t * NF_path_p, SINT16 * s_value);
extern SINT16 wl_util_get_rssi(struct net_device *netdev,
			       RssiPathInfo_t * RSSI_path_p, SINT16 * s_value);
