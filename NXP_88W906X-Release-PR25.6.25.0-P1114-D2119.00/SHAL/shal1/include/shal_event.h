...
#define EVT_MAP_SW_TRIG_MBA_PROC            185
...
static SHAL_INLINE void EVT_sendSingle(const U32 eventMapNum, U32 val)
{
	EVT_write32((SMAC_EVENT_BASE_ADDR + eventMapNum * 4), val);
}

static SHAL_INLINE U32 EVT_wakeOnGroup(const U32 grpNum)
{
	U32 evtGrpBits;
	evtGrpBits = EVT_read32((EVT_STAT_GRP_0 + grpNum * sizeof(EVT_GRP_REG_BASE_st)));
	return evtGrpBits;
}

static SHAL_INLINE void EVT_clrGroup(const U32 eventNum, U32 eventBits)
{
	EVT_write32((EVT_CLR_GRP_0 + (eventNum / EVT_NUM_GRP_SIZE) * sizeof(EVT_GRP_REG_BASE_st)), eventBits);
}

static SHAL_INLINE void EVT_clrSingle(const U32 eventNum)
{
	EVT_write32((SMAC_EVENT_BASE_ADDR + eventNum * 4), 0);
}

static SHAL_INLINE U32 EVT_peekGroup(const U32 eventNum)
{
	return (EVT_read32((EVT_STAT_SHDW_GRP_0 + (eventNum / EVT_NUM_GRP_SIZE) * sizeof(EVT_GRP_REG_BASE_st))));
}

static SHAL_INLINE U32 EVT_peekSingle(const U32 eventNum)
{
	return (EVT_read32((EVT_STAT_SHDW_GRP_0 + (eventNum / EVT_NUM_GRP_SIZE) * sizeof(EVT_GRP_REG_BASE_st))) & EVT_BIT(eventNum));
}

static SHAL_INLINE void EVT_wakeOnSingle(const U32 eventNum)
{
	EVT_read32((SMAC_EVENT_BASE_ADDR + eventNum * 4));
	__dsb(0xF);
}

static SHAL_INLINE void EVT_unmaskPingPong(const U32 eventNum)
{
	U32 grpAddr = (EVT_SHIELD_GRP_0 + (eventNum / EVT_NUM_GRP_SIZE) * sizeof(EVT_GRP_REG_BASE_st));
	EVT_write32(grpAddr, (EVT_read32(grpAddr) & ~(0x3 << EVT_BITNUM(eventNum))));
}

static SHAL_INLINE void EVT_maskPingPong(const U32 eventNum)
{
	U32 grpAddr = (EVT_SHIELD_GRP_0 + (eventNum / EVT_NUM_GRP_SIZE) * sizeof(EVT_GRP_REG_BASE_st));
	EVT_write32(grpAddr, (EVT_read32(grpAddr) | (0x3 << EVT_BITNUM(eventNum))));
}

static SHAL_INLINE void EVT_unmaskSingle(const U32 eventNum)
{
	U32 grpAddr = (EVT_SHIELD_GRP_0 + (eventNum / EVT_NUM_GRP_SIZE) * sizeof(EVT_GRP_REG_BASE_st));
	EVT_write32(grpAddr, (EVT_read32(grpAddr) & ~(EVT_BIT(eventNum))));
}

static SHAL_INLINE void EVT_maskSingle(const U32 eventNum)
{
	U32 grpAddr = (EVT_SHIELD_GRP_0 + (eventNum / EVT_NUM_GRP_SIZE) * sizeof(EVT_GRP_REG_BASE_st));
	EVT_write32(grpAddr, (EVT_read32(grpAddr) | (EVT_BIT(eventNum))));
}
...
#define  EVT_RX_BBIF_WATCH_DOG          EVT_REMAP_NUM(0, 20)
...
#define  EVT_SW_TRIG_MBA_PROC           EVT_REMAP_NUM(1, 13)
#if defined(RXINFO_WAR_EN)
#define  EVT_RX_BBIF_RX_INFO_DMA        EVT_REMAP_NUM(1, 14)
#define  EVT_RX_BBIF_END_PPDU_1         EVT_REMAP_NUM(1, 15)
#endif
