...
#define MAX_MED_TIME (500000 / 32)	/* We allow max 50% bw for ACM, in 32us unit = 15625 */

UINT16 MediumTimeTotal = 0;
UINT16 MediumTimeAccCat[4] = { 0 };	//In 32us unit
UINT16 MediumTimeUserPri[8] = { 0 };	//In 32us unit
#endif				/* WMM_AC_EDCA */
...
#ifdef SOC_W8964
struct qos_ts_info QosTsData[8];
#endif
//=============================================================================
//                         PRIVATE FUNCTIONS
//============================================================================ 
...
{
#ifdef WMM_AC_EDCA
	return (MAX_MED_TIME - MediumTimeTotal);
#else
...
void CopyTSpecElems(Mrvl_TSPEC_t * pTSpecDst, IEEEtypes_TSPEC_t * pTSpecSrc)
{
	...
	if (pTSpecSrc->ElementId == PROPRIETARY_IE) {
		...
		pTSpecDst->nom_msdu_size = ENDIAN_SWAP16(pWsmTspecSrc->nom_msdu_size);
		pTSpecDst->max_msdu_size = ENDIAN_SWAP16(pWsmTspecSrc->max_msdu_size);
		pTSpecDst->min_SI = ENDIAN_SWAP32(pWsmTspecSrc->min_SI);	//convert to msec
		pTSpecDst->max_SI = ENDIAN_SWAP32(pWsmTspecSrc->max_SI);	//convert to msec

		if (pWsmTspecSrc->inactive_intrvl)
			pTSpecDst->inactive_intrvl = ENDIAN_SWAP32(pWsmTspecSrc->inactive_intrvl);
		else
			pTSpecDst->inactive_intrvl = ENDIAN_SWAP32(0xffffff);

		pTSpecDst->suspen_intrvl = ENDIAN_SWAP32(pWsmTspecSrc->suspen_intrvl);
		pTSpecDst->serv_start_time = ENDIAN_SWAP32(pWsmTspecSrc->serv_start_time);
		pTSpecDst->min_data_rate = ENDIAN_SWAP32(pWsmTspecSrc->min_data_rate);
		pTSpecDst->mean_data_rate = ENDIAN_SWAP32(pWsmTspecSrc->mean_data_rate);
		pTSpecDst->peak_data_rate = ENDIAN_SWAP32(pWsmTspecSrc->peak_data_rate);
		pTSpecDst->max_burst_size = ENDIAN_SWAP32(pWsmTspecSrc->max_burst_size);
		pTSpecDst->delay_bound = ENDIAN_SWAP32(pWsmTspecSrc->delay_bound);
		pTSpecDst->min_phy_rate = ENDIAN_SWAP32(pWsmTspecSrc->min_phy_rate);
		pTSpecDst->srpl_bw_allow = ENDIAN_SWAP16(pWsmTspecSrc->srpl_bw_allow);
		pTSpecDst->med_time = ENDIAN_SWAP16(pWsmTspecSrc->med_time);
	} else {
		...
		pTSpecDst->nom_msdu_size = ENDIAN_SWAP16(pTSpecSrc->nom_msdu_size);
		pTSpecDst->max_msdu_size = ENDIAN_SWAP16(pTSpecSrc->max_msdu_size);
		...
		if (pTSpecSrc->inactive_intrvl)
			pTSpecDst->inactive_intrvl = ENDIAN_SWAP32(pTSpecSrc->inactive_intrvl);
		else
			pTSpecDst->inactive_intrvl = ENDIAN_SWAP32(0xffffff);

		pTSpecDst->suspen_intrvl = ENDIAN_SWAP32(pTSpecSrc->suspen_intrvl);
		pTSpecDst->serv_start_time = ENDIAN_SWAP32(pTSpecSrc->serv_start_time);
		pTSpecDst->min_data_rate = ENDIAN_SWAP32(pTSpecSrc->min_data_rate);
		pTSpecDst->mean_data_rate = ENDIAN_SWAP32(pTSpecSrc->mean_data_rate);
		pTSpecDst->peak_data_rate = ENDIAN_SWAP32(pTSpecSrc->peak_data_rate);
		pTSpecDst->max_burst_size = ENDIAN_SWAP32(pTSpecSrc->max_burst_size);
		pTSpecDst->delay_bound = ENDIAN_SWAP32(pTSpecSrc->delay_bound);
		pTSpecDst->min_phy_rate = ENDIAN_SWAP32(pTSpecSrc->min_phy_rate);
		pTSpecDst->med_time = ENDIAN_SWAP16(pTSpecSrc->med_time);
	}
}
...
* 
*****************************************************************************/
inline UINT32 AddTSpecEntry(IEEEtypes_TSPEC_t * pTSpec, IEEEtypes_MacAddr_t * pAddr)
{
	UINT32 indx;
...
Status_e DelTSpecEntry(UINT32 Index)
{
	...
	for (i = Index + 1; (i < MAX_QOS_STRMS) && (TSpecEntryDb[i].not_free == TRUE); i++) {
		UpdateAssocTblGtid(i);	//update the GTid element in the TClas Db.
	}
	i--;
	if (Index < (MAX_QOS_STRMS - 1)) {
...
* 
*****************************************************************************/
inline Status_e ProcessADDTSReqTCLAS(IEEEtypes_ADDTS_Req_t * AddTSReq_p, IEEEtypes_MacAddr_t * pStaAddr, UINT32 Aid, UINT32 gTID)
{
	UINT8 index;
	TCLAS_t *pTCLAS;
	...
	index = 0;
	while ((index < MAX_TCLAS_PER_STN) && (AssocTable[Aid].Qos_Stn_Data.TCLASEntry[index].gTID != 0xffffffff))
		index++;
	...
	if ((pTCLAS->ElementId == TCLAS) || (pTCLAS->ElementId == PROPRIETARY_IE)) {
		...
		while (((pTCLAS->ElementId == TCLAS) || (pTCLAS->ElementId == PROPRIETARY_IE))
			...
			if ((pTCLAS->ElementId == TCLAS)) {
				...
				memcpy(&(AssocTable[Aid].Qos_Stn_Data.TCLASEntry[index].TCLAS), pTCLAS, pTCLAS->Len + 2);
		...
		if (pTCLAS->ElementId == TCLAS_PROCESSING) {
			memcpy(&(AssocTable[Aid].Qos_Stn_Data.TCLAS_Processing), pTCLAS, sizeof(TCLAS_Processing_t));
		}
	} else {
...
/******************************************************************************
* 
* Name: Qos_ProcessADDTSReqTCLAS_WFA
* 
* Description: 1> Will add a TCLAS entry for that ADDTS Req.
*     
* 
...
* 
*****************************************************************************/
inline Status_e ProcessADDTSReqTCLAS_WFA(IEEEtypes_WFA_ADDTS_Req_t * AddTSReq_p, IEEEtypes_MacAddr_t * pStaAddr, UINT32 Aid, UINT32 gTID)
{
	UINT8 index;
#if 0
	TCLAS_t *pTCLAS;
...
#ifdef WMM_AC_EDCA
#ifdef QOS_WSM_FEATURE
BOOLEAN AdmissionCntrl_EDCA(vmacApInfo_t * vmacSta_p, WSM_TSPEC_t * pTSpec, UINT16 * med_time)
#else
BOOLEAN AdmissionCntrl_EDCA(vmacApInfo_t * vmacSta_p, IEEEtypes_TSPEC_t * pTSpec, UINT16 * med_time)
#endif
{
	UINT16 nom_msdu_size = ENDIAN_SWAP16(pTSpec->nom_msdu_size) & 0x7FFF;	//bit[15] = Fixed subfield, ignore it
	UINT32 mean_data_rate = ENDIAN_SWAP32(pTSpec->mean_data_rate);
	UINT32 min_phy_rate = ENDIAN_SWAP32(pTSpec->min_phy_rate);
	UINT16 srpl_bw_allow = ENDIAN_SWAP16(pTSpec->srpl_bw_allow);
	UINT8 acc_cat = AccCategoryQ[pTSpec->ts_info.usr_priority];
	UINT32 pps = 0;
	UINT32 MPDUExchangeTime = 0;
	UINT32 med_time_tmp = 0;	//32bit med_time for the multiply reulst
	Direction_e dir = pTSpec->ts_info.direction;

	//By IEEE Std 802.11-2012 N.2.2
	pps = CEIL((mean_data_rate / 8), nom_msdu_size);
	// Data packet duration
	// 802.11 hdr = 24 bytes, Security hdr = 8 bytes, FCS = 4 bytes
	// We ignore preamble for now
	MPDUExchangeTime = CEIL(((nom_msdu_size + 24 + 8 + 4) * 8) * 1000000, min_phy_rate);
	// SIFS
	MPDUExchangeTime += IEEE80211_SIFS;
	// ACK, assume 39 bytes
	if (min_phy_rate < 6000000) {
		//11b = 1Mbps
		MPDUExchangeTime += CEIL((39 * 8) * 1000000, 1000000);
	} else {
		//11a/g/n/ac = 6Mbps
		MPDUExchangeTime += CEIL((39 * 8) * 1000000, 6000000);
	}

	// Linux kernel space discourage of using floating point numbers, so we use integer arithmatic for srpl_bw_allow
	med_time_tmp = CEIL(((UINT32) srpl_bw_allow * pps * MPDUExchangeTime), (UINT32) (1 << 13));	//srpl_bw_allow's b[0~12] are decimal part
	*med_time = (UINT16) CEIL(med_time_tmp, 32);	//med_time is in the 32us unit

#if 0				//dbg
	printk("med_time_tmp = 0x%x\n", med_time_tmp);
	printk("med_time = %d, srpl_bw_allow = 0x%04x, pps = %d, MPDUExchangeTime = %d\n", *med_time, srpl_bw_allow, pps, MPDUExchangeTime);
#endif

	if ((dir == BIDIRLINK) && ((MAX_MED_TIME - MediumTimeTotal) > (*med_time * 2))) {	//bi-dir, reserve medium time for both up and down link
		MediumTimeUserPri[pTSpec->ts_info.usr_priority] += (*med_time * 2);
		MediumTimeAccCat[acc_cat] += (*med_time * 2);
		MediumTimeTotal += (*med_time * 2);

		//Save the med_time to pTSpec which will be stored in TSpecEntryDb[]. DelTS will need it.
		pTSpec->med_time = ENDIAN_SWAP16((*med_time * 2));
	} else if ((dir != BIDIRLINK) && ((MAX_MED_TIME - MediumTimeTotal) > *med_time)) {	//uni-dir
		MediumTimeUserPri[pTSpec->ts_info.usr_priority] += *med_time;
		MediumTimeAccCat[acc_cat] += *med_time;
		MediumTimeTotal += *med_time;

		//Save the med_time to pTSpec which will be stored in TSpecEntryDb[]. DelTS will need it.
		pTSpec->med_time = ENDIAN_SWAP16(*med_time);
	} else {
		printk("Not enough medium time, MediumTimeTotal = %d\n", MediumTimeTotal);
		*med_time = 0;
	...
	wlFwSetBssLoadAac(vmacSta_p->dev, (MAX_MED_TIME - MediumTimeTotal));
...
UINT16 ProcessADDTSRequest(vmacApInfo_t * vmacSta_p, IEEEtypes_ADDTS_Req_t * AddTSReq_p,
			   IEEEtypes_MacAddr_t * pStaAddr, UINT16 Aid, UINT32 * TspecIndx, UINT8 ClientMode, UINT16 * pMediumTime)
{
	UINT32 index;

	//Send the Tspec entry to the Scheduler function.
	if ((AddTSReq_p->TSpec.ts_info.access_policy == HCCA) || (AddTSReq_p->TSpec.ts_info.access_policy == BOTH)) {

		if (!AdmissionCntrl(ENDIAN_SWAP32(AddTSReq_p->TSpec.mean_data_rate))) {	//check for admission control
			return IEEEtypes_STATUS_QOS_INSUFFICIENT_BW;
		}
		//Add entry into the Tspec Database
		index = AddTSpecEntry((IEEEtypes_TSPEC_t *) & AddTSReq_p->TSpec, pStaAddr);
		if (index == 0xFFFFFFFF) {	//Tspec DB is full!!!
			return IEEEtypes_STATUS_QOS_REFUSED;
		}
		//Add entry tinto the Txop Data Base
		if (AddTxOpEntry(vmacSta_p, index, Aid, ClientMode) == FAIL) {	//Txop DataBase full!!!
			DelTSpecEntry(index);
			return IEEEtypes_STATUS_QOS_REFUSED;
		}
		//Update TCLAS Table.
...
#ifdef WMM_AC_EDCA
	else if (AddTSReq_p->TSpec.ts_info.access_policy == EDCA) {
		UINT8 acc_cat = AccCategoryQ[AddTSReq_p->TSpec.ts_info.usr_priority];

		if (mib_QStaEDCATable[acc_cat].QStaEDCATblMandatory == FALSE) {
			return IEEEtypes_STATUS_QOS_NOT_QOSAP;
		}

		if (!AdmissionCntrl_EDCA(vmacSta_p, &(AddTSReq_p->TSpec), pMediumTime)) {	//check for admission control
			return IEEEtypes_STATUS_QOS_INSUFFICIENT_BW;
		}
		//Add entry into the Tspec Database
		index = AddTSpecEntry((IEEEtypes_TSPEC_t *) & AddTSReq_p->TSpec, pStaAddr);
		if (index == 0xFFFFFFFF) {	//Tspec DB is full!!!
			printk("ProcessADDTSRequest(): Tspec DB is full!\n");
			return IEEEtypes_STATUS_QOS_REFUSED;
		}
		//Update TCLAS Table.
		ProcessADDTSReqTCLAS(AddTSReq_p, pStaAddr, Aid, index);
...
#endif				/* WMM_AC_EDCA */
	else {			//Reject EDCA Tspecs. WME mandates it
...
/******************************************************************************
* 
* Name: ProcessADDTSRequest_WFA
* 
* Description: Will return ADDTS Response in WFA format.
*     
* 
* Conditions For Use: 
*    When Action frame with category value = 17 && action code == 1 comes in. 
* 
* Arguments: 
*    QoS Action Frame containing ADDTS Request. 
*    QoS Resp Frame containing ADDTS Response                             
* Return Value: 
*    Status indicating whether the stream was accepted.
* 
* Notes: 
*    None. 
* 
...
* 
*****************************************************************************/
UINT16 ProcessADDTSRequest_WFA(vmacApInfo_t * vmacSta_p, IEEEtypes_WFA_ADDTS_Req_t * AddTSReq_p,
			       IEEEtypes_MacAddr_t * pStaAddr, UINT16 Aid, UINT32 * TspecIndx, UINT8 ClientMode, UINT16 * pMediumTime)
{
	UINT32 index;
	...
	if ((AddTSReq_p->TSpec.ts_info.access_policy == HCCA) || (AddTSReq_p->TSpec.ts_info.access_policy == BOTH)) {
		...
		//Update TCLAS Table.
		ProcessADDTSReqTCLAS_WFA(AddTSReq_p, pStaAddr, Aid, index);
		//Also reset the Delts Counter.
		TSpecEntryDb[index].DelTS_PktCnt = 0;
		...
		*TspecIndx = index;
		...
		if (!AdmissionCntrl_EDCA(vmacSta_p, &(AddTSReq_p->TSpec), pMediumTime)) {	//check for admission control
			printk("ProcessADDTSRequest_WFA(): admission control failed!\n");
			return IEEEtypes_STATUS_QOS_INSUFFICIENT_BW;
		}
		//Add entry into the Tspec Database
		index = AddTSpecEntry((IEEEtypes_TSPEC_t *) & AddTSReq_p->TSpec, pStaAddr);
		if (index == 0xFFFFFFFF) {	//Tspec DB is full!!!
			printk("ProcessADDTSRequest_WFA(): Tspec DB is full!\n");
			return IEEEtypes_STATUS_QOS_REFUSED;
		}
		//Update TCLAS Table.
		ProcessADDTSReqTCLAS_WFA(AddTSReq_p, pStaAddr, Aid, index);
...
* 
*****************************************************************************/
UINT8 ProcessADDTSRequestSchedule(IEEEtypes_ADDTS_Rsp_t * pAddTsRspFrm, UINT32 TspecDBindx)
{
	...
	if (pTxOp)
		pWSM_Schedule->serv_intrvl = ENDIAN_SWAP32(pTxOp->mySI);	//in usec
	else
		pWSM_Schedule->serv_intrvl = ENDIAN_SWAP32(DEFAULT_SPEC_INTERVAL);
	pWSM_Schedule->serv_start_time = ENDIAN_SWAP32(tsflow);
	pWSM_Schedule->spec_intrvl = ENDIAN_SWAP16(DEFAULT_SPEC_INTERVAL);
...
UINT32 FindTspecIndx(IEEEtypes_MacAddr_t * pStaAddr, UINT32 TsId)
{
	...
	for (i = 0; (i < MAX_QOS_STRMS) && (TSpecEntryDb[i].not_free); i++) {
		if (!MACADDR_CMP(pStaAddr, TSpecEntryDb[i].src_addr) && TSpecEntryDb[i].TSpec.ts_info.tsid == TsId) {
			return i;
		}
	}
	return 0xFFFFFFFF;
...
Status_e ProcessDELTSRequest(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * pStaAddr, UINT32 TsId)
{
	...
	pStaInfo = extStaDb_GetStaInfo(vmacSta_p, pStaAddr, STADB_UPDATE_AGINGTIME);

	TspecIndx = FindTspecIndx(pStaAddr, TsId);
	if (TspecIndx >= MAX_QOS_STRMS) {
		return FAIL;
	}
	if (pStaInfo) {
...
#ifdef WMM_AC_EDCA
	if (TSpecEntryDb[TspecIndx].TSpec.ts_info.access_policy == EDCA) {	//Decrease MediumTime
		if (TSpecEntryDb[TspecIndx].TSpec.med_time != 0) {
			UINT8 acc_cat = AccCategoryQ[TSpecEntryDb[TspecIndx].TSpec.ts_info.usr_priority];

			MediumTimeTotal -= TSpecEntryDb[TspecIndx].TSpec.med_time;
			MediumTimeUserPri[TSpecEntryDb[TspecIndx].TSpec.ts_info.usr_priority] -= TSpecEntryDb[TspecIndx].TSpec.med_time;
			MediumTimeAccCat[acc_cat] -= TSpecEntryDb[TspecIndx].TSpec.med_time;
	...
	for (i = 0; i <= MAX_AID; i++) {
		...
		for (j = 0; j < MAX_TCLAS_PER_STN; j++) {
			if (j == 0) {
				...
				AssocTable[i].Qos_Stn_Data.TCLASEntry[j].TCLAS.frm_classifier.classif_params.classif_0.type = 0x08;
...
UINT8 QoS_AppendWMEInfoElem(vmacApInfo_t * vmacSta_p, UINT8 * pBcnBuf)
{
	...
	if (mib_StaCfg_p->WSMQoSOptImpl) {
		...
		pWMEInfoElem->QoS_info.EDCA_param_set_update_cnt = vmacSta_p->VMacEntry.edca_param_set_update_cnt;
#endif
...
void InitWMEParamElem(vmacApInfo_t * vmacSta_p)
{
	...
	WMEParamElem.QoS_info.EDCA_param_set_update_cnt = vmacSta_p->VMacEntry.edca_param_set_update_cnt;;
	WMEParamElem.rsvd = 0;
	//Updae EDCA for BE
	...
	WMEParamElem.AC_BE.ECW_min_max.ECW_min = GetLog(mib_QStaEDCATable[0].QStaEDCATblCWmin);
	WMEParamElem.AC_BE.ECW_min_max.ECW_max = GetLog(mib_QStaEDCATable[0].QStaEDCATblCWmax);
	WMEParamElem.AC_BE.TXOP_lim = ENDIAN_SWAP16(mib_QStaEDCATable[0].QStaEDCATblTXOPLimit);
	//Update EDCA for BK
	...
	WMEParamElem.AC_BK.ECW_min_max.ECW_min = GetLog(mib_QStaEDCATable[1].QStaEDCATblCWmin);
	WMEParamElem.AC_BK.ECW_min_max.ECW_max = GetLog(mib_QStaEDCATable[1].QStaEDCATblCWmax);
	WMEParamElem.AC_BK.TXOP_lim = ENDIAN_SWAP16(mib_QStaEDCATable[1].QStaEDCATblTXOPLimit);
	//Update EDCA for VI
	...
	WMEParamElem.AC_VI.ECW_min_max.ECW_min = GetLog(mib_QStaEDCATable[2].QStaEDCATblCWmin);
	WMEParamElem.AC_VI.ECW_min_max.ECW_max = GetLog(mib_QStaEDCATable[2].QStaEDCATblCWmax);
	WMEParamElem.AC_VI.TXOP_lim = ENDIAN_SWAP16(mib_QStaEDCATable[2].QStaEDCATblTXOPLimit);
	//Update EDCA for VO
	...
	WMEParamElem.AC_VO.ECW_min_max.ECW_min = GetLog(mib_QStaEDCATable[3].QStaEDCATblCWmin);
	WMEParamElem.AC_VO.ECW_min_max.ECW_max = GetLog(mib_QStaEDCATable[3].QStaEDCATblCWmax);
	WMEParamElem.AC_VO.TXOP_lim = ENDIAN_SWAP16(mib_QStaEDCATable[3].QStaEDCATblTXOPLimit);

	if (*(mib->mib_ApMode) == AP_MODE_B_ONLY)
	{
		WMEParamElem.AC_BE.TXOP_lim = ENDIAN_SWAP16(mib_QStaEDCATable[0].QStaEDCATblTXOPLimitBSta);
		WMEParamElem.AC_BK.TXOP_lim = ENDIAN_SWAP16(mib_QStaEDCATable[1].QStaEDCATblTXOPLimitBSta);
		WMEParamElem.AC_VI.TXOP_lim = ENDIAN_SWAP16(mib_QStaEDCATable[2].QStaEDCATblTXOPLimitBSta);
		WMEParamElem.AC_VO.TXOP_lim = ENDIAN_SWAP16(mib_QStaEDCATable[3].QStaEDCATblTXOPLimitBSta);
...
UINT16 QoS_AppendWMEParamElem(vmacApInfo_t * vmacSta_p, UINT8 * pBcnBuf)
{
	...
	if (mib_StaCfg_p->WSMQoSOptImpl) {
		...
		pWMEParamElem->QoS_info.EDCA_param_set_update_cnt = vmacSta_p->VMacEntry.edca_param_set_update_cnt;
#else
		...
		pWMEParamElem->AC_BE.ECW_min_max.ECW_min = GetLog(mib_QStaEDCATable[0].QStaEDCATblCWmin);
		pWMEParamElem->AC_BE.ECW_min_max.ECW_max = GetLog(mib_QStaEDCATable[0].QStaEDCATblCWmax);
		pWMEParamElem->AC_BE.TXOP_lim = ENDIAN_SWAP16(mib_QStaEDCATable[0].QStaEDCATblTXOPLimit);
		//Update EDCA for BK
		...
		pWMEParamElem->AC_BK.ECW_min_max.ECW_min = GetLog(mib_QStaEDCATable[1].QStaEDCATblCWmin);
		pWMEParamElem->AC_BK.ECW_min_max.ECW_max = GetLog(mib_QStaEDCATable[1].QStaEDCATblCWmax);
		pWMEParamElem->AC_BK.TXOP_lim = ENDIAN_SWAP16(mib_QStaEDCATable[1].QStaEDCATblTXOPLimit);
		//Update EDCA for VI
		...
		pWMEParamElem->AC_VI.ECW_min_max.ECW_min = GetLog(mib_QStaEDCATable[2].QStaEDCATblCWmin);
		pWMEParamElem->AC_VI.ECW_min_max.ECW_max = GetLog(mib_QStaEDCATable[2].QStaEDCATblCWmax);
		pWMEParamElem->AC_VI.TXOP_lim = ENDIAN_SWAP16(mib_QStaEDCATable[2].QStaEDCATblTXOPLimit);
		//Update EDCA for VO
		...
		pWMEParamElem->AC_VO.ECW_min_max.ECW_min = GetLog(mib_QStaEDCATable[3].QStaEDCATblCWmin);
		pWMEParamElem->AC_VO.ECW_min_max.ECW_max = GetLog(mib_QStaEDCATable[3].QStaEDCATblCWmax);
		pWMEParamElem->AC_VO.TXOP_lim = ENDIAN_SWAP16(mib_QStaEDCATable[3].QStaEDCATblTXOPLimit);

		if (*(mib->mib_ApMode) == AP_MODE_B_ONLY)
		{
			pWMEParamElem->AC_BE.TXOP_lim = ENDIAN_SWAP16(mib_QStaEDCATable[0].QStaEDCATblTXOPLimitBSta);
			pWMEParamElem->AC_BK.TXOP_lim = ENDIAN_SWAP16(mib_QStaEDCATable[1].QStaEDCATblTXOPLimitBSta);
			pWMEParamElem->AC_VI.TXOP_lim = ENDIAN_SWAP16(mib_QStaEDCATable[2].QStaEDCATblTXOPLimitBSta);
			pWMEParamElem->AC_VO.TXOP_lim = ENDIAN_SWAP16(mib_QStaEDCATable[3].QStaEDCATblTXOPLimitBSta);
...
UINT16 Qos_UpdateQosCapElem(vmacApInfo_t * vmacSta_p, UINT8 * pBcnBuf)
{
	...
	if (*(mib->QoSOptImpl)) {
		...
		pQosCapElem->QoS_info.EDCA_param_set_update_cnt = vmacSta_p->VMacEntry.edca_param_set_update_cnt;
...
Mrvl_TSPEC_t *GetTspec(IEEEtypes_MacAddr_t * pAddr, UINT8 req_tsid)
{
	UINT32 i;
	for (i = 0; i < MAX_QOS_STRMS; i++) {
		if (TSpecEntryDb[i].not_free && !MACADDR_CMP(pAddr, TSpecEntryDb[i].src_addr) && (TSpecEntryDb[i].TSpec.ts_info.tsid == req_tsid)) {
...
uint WMM_Customer_spec = 0;
module_param(WMM_Customer_spec, uint, 0644);
MODULE_PARM_DESC(WMM_Customer_spec, "WMM Mapping Customer spec");

static inline UINT8 ipv6_get_dsfield(IEEEtypes_IPv6_Hdr_t * ipv6h)
{
	return ntohs(*(const UINT16 *)ipv6h) >> 4;
}

UINT8 WMMmappingTable(UINT8 dsfield)
{
	UINT8 Pri = (dsfield & 0xFC) >> 2;
	UINT8 tid = 0;
	// Default WFA spec. DSCP tid values
	static UINT8 CS0_tid = 0;
	static UINT8 CS1_tid = 1;
	static UINT8 CS2_tid = 2;
	static UINT8 CS3_tid = 3;
	static UINT8 CS4_tid = 4;
	static UINT8 CS5_tid = 5;
	static UINT8 CS6_tid = 6;
	static UINT8 CS7_tid = 7;
	/*static UINT8 EF_tid = 5; */// Expedited forwarding

	if (WMM_Customer_spec) {
		switch (Pri) {
			// AccCategoryQ[8] = {AC_BE_Q, AC_BK_Q, AC_BK_Q, AC_BE_Q, AC_VI_Q, AC_VI_Q, AC_VO_Q, AC_VO_Q};
		case 0x00:	// 000000
			tid = CS0_tid;	// WMM_AC_BE
			break;
		case 0x08:	// 001000
			tid = CS1_tid;	// WMM_AC_BK
			break;
		case 0x10:	// 010000
			tid = CS2_tid;	// WMM_AC_BK
			break;
		case 0x18:	// 011000
			tid = CS3_tid;	// WMM_AC_BE
			break;
		case 0x20:	// 100000
			tid = CS4_tid;	// WMM_AC_VI
			break;
		case 0x28:	// 101000
			tid = CS5_tid;	// WMM_AC_VI
			break;
		case 0x30:	// 110000
			tid = CS6_tid;	// WMM_AC_VO
			break;
		case 0x38:	// 111000
			tid = CS7_tid;	// WMM_AC_VO
			break;
		case 0x2E:	// 101110
			tid = CS6_tid;	// WMM_AC_VO
			break;
...
UINT8 Qos_GetDSCPPriority(UINT8 * Databufptr)
{
	...
	if (type == IEEE802_11Q_TYPE) {
		...
		if (typelen <= MAX_ETHER_PKT_SIZE) {
			//pkt has LLC Header
			llc_p = (llc_snap_hdr_t *) ((UINT8 *) DataEtherFrame_p + sizeof(ether_hdr_t));
			type = SHORT_SWAP(llc_p->Type);
			IPv4_p = (IEEEtypes_IPv4_Hdr_t *) ((UINT8 *) DataEtherFrame_p + sizeof(ether_hdr_t) + sizeof(llc_snap_hdr_t));

		} else {
			type = SHORT_SWAP(DataEtherFrame_p->Hdr.type);
			IPv4_p = (IEEEtypes_IPv4_Hdr_t *) ((UINT8 *) DataEtherFrame_p + sizeof(ether_hdr_t));
		}
		if (type == ETH_P_IP)	//IP Pkt
		{
			AC_prio = WMMmappingTable(IPv4_p->tos);
		} else if (type == ETH_P_IPV6) {
			IEEEtypes_IPv6_Hdr_t *IPv6_p = (IEEEtypes_IPv6_Hdr_t *) IPv4_p;
			UINT8 dsfield = ipv6_get_dsfield(IPv6_p);
			if (IPv6_p->ver == 6) {
				AC_prio = WMMmappingTable(dsfield);
			}
		} else {
			AC_prio = 0;
		}
