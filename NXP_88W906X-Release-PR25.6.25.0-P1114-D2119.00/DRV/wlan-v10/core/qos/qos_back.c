...
void SendAddBAReq(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t StaAddrA, UINT8 tsid,
		  IEEEtypes_QoS_BA_Policy_e BaPolicy, UINT32 SeqNo, UINT8 DialogToken, u16 bufsize, UINT16 * pBufSizeInReq)
{
	...
	if ((txSkb_p =
	     mlmeApiPrepMgtMsg2(IEEE_MSG_QOS_ACTION, (IEEEtypes_MacAddr_t *) StaAddrA, (IEEEtypes_MacAddr_t *) vmacSta_p->macBssId,
				sizeof(IEEEtypes_ADDBA_Req_t2))) == NULL) {
		WLDBG_INFO(DBG_LEVEL_0, "No more buffer !!!!!!!!!!!!\n");
		return;
	}

	if ((pStaInfo = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) StaAddrA, STADB_DONT_UPDATE_AGINGTIME)) == NULL) {
		printk("sta:%02x%02x%02x%02x%02x%02x not in StaDb\n", StaAddrA[0], StaAddrA[1], StaAddrA[2], StaAddrA[3], StaAddrA[4], StaAddrA[5]);
		return;
	}
#ifdef SOC_W906X
	...
	if (!STADB_IS_HE(pStaInfo) && !STADB_IS_VHT(pStaInfo) && STADB_IS_HT(pStaInfo))
		pAddBaReqFrm->ParamSet.amsdu = 0;
	else
		pAddBaReqFrm->ParamSet.amsdu = (*(vmacSta_p->Mib802dot11->pMib_11nAggrMode) & WL_MODE_AMSDU_TX_MASK) ? 1 : 0;
	if (!((pStaInfo->aggr11n.amsducfg.priority_aggr) & (0x1 << tsid)) || !(master->amsducfg.priority_aggr & (0x1 << tsid))) {
		pAddBaReqFrm->ParamSet.amsdu = 0;
	}
	...
	//sba++
	if (he_capable) {
		if ((*(vmacSta_p->Mib802dot11->mib_superBA) == 1) || (*(vmacSta_p->Mib802dot11->mib_superBA) == 2))
	...
	if (bufsize != 0)
		pAddBaReqFrm->ParamSet.BufSize = bufsize;

	*pBufSizeInReq = pAddBaReqFrm->ParamSet.BufSize;
...
void SendDelBA(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t StaAddr, UINT8 tsid)
{
	...
	if ((txSkb_p =
	     mlmeApiPrepMgtMsg2(IEEE_MSG_QOS_ACTION, (IEEEtypes_MacAddr_t *) StaAddr, (IEEEtypes_MacAddr_t *) vmacSta_p->macBssId,
...
void SendGroupIDMgmtframe(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t StaAddr, UINT8 gid, UINT8 userposition)
{
	...
	if (gid < 64) {
		...
		pGroupIDMgmt->UserPositionArray[gid / 4] = (userposition) << (2 * (gid % 4));
	}
