...
{ \
	pBuf = wl_kmalloc_autogfp(buf_size); \
...
BOOLEAN WL_FUN_BssStart(void *info)
{
	...
	WL_FUN_SETUP_APICMDBUF(info, pBuf, sizeof(APICMDBUF), APCMD_BSS_START, 0);
...
BOOLEAN WL_FUN_SetESSID(void *info, char *essid)
{
	...
	WL_FUN_SETUP_APICMDBUF(info, pBuf, sizeof(APICMDBUF) + 32, APCMD_SET_SSID, 32);
...
BOOLEAN WL_FUN_SetAuthType(void *info, int authtype)
{
	...
	WL_FUN_SETUP_APICMDBUF(info, pBuf, sizeof(APICMDBUF_SET_AUTH_MODE), APCMD_SET_AUTH_MODE, sizeof(APICMDBUF_SET_AUTH_MODE) - sizeof(APICMDBUF));
...
BOOLEAN WL_FUN_SetBand(void *info, int band)
{
	...
	WL_FUN_SETUP_APICMDBUF(info, pBuf, sizeof(APICMDBUF_SET_AP_MODE), APCMD_SET_AP_MODE, sizeof(APICMDBUF_SET_AP_MODE) - sizeof(APICMDBUF));
...
BOOLEAN WL_FUN_SetBCAConfig(void *info, int mode, ULONG wlTxPri0, ULONG wlTxPri1, ULONG wlRxPri0, ULONG wlRxPri1)
{
	...
	WL_FUN_SETUP_APICMDBUF(info, pBuf, sizeof(APICMDBUF_BCA_CONFIG), APCMD_SET_BCA_CONFIG, sizeof(APICMDBUF_BCA_CONFIG) - sizeof(APICMDBUF));
...
BOOLEAN WL_FUN_SetWMM(void *info, int wmm)
{
	...
	BOOLEAN result;
#ifdef QOS_FEATURE
	WL_FUN_SETUP_APICMDBUF(info, pBuf, sizeof(APICMDBUF_SET_QOS_MODE), APCMD_SET_QOS_MODE, sizeof(APICMDBUF_SET_QOS_MODE) - sizeof(APICMDBUF));
...
BOOLEAN WL_FUN_SetMACFilter(void *info, int no, UCHAR * mac)
{
	...
	WL_FUN_SETUP_APICMDBUF(info, pBuf, sizeof(APICMDBUF) + (32 * 6) + 2, APCMD_SET_WLAN_FILTER_LIST, (32 * 6) + 2);
...
BOOLEAN WL_FUN_GetSTAList(void *info, char *sta)
{
	...
	WL_FUN_SETUP_APICMDBUF(info, pBuf, sizeof(APICMDBUF) + 2, APCMD_GET_STA_LIST, 2);
	apctl(pBuf);
	mdelay(1000);
	len = le16_to_cpu(pBuf->cmdBodyLen);
	wl_kfree(pBuf);
	WL_FUN_SETUP_APICMDBUF(info, pBuf, sizeof(APICMDBUF) + len, APCMD_GET_STA_LIST, len);
...
BOOLEAN WL_FUN_GetRfReg(void *info, USHORT reg, UCHAR * value)
{
	...
	WL_FUN_SETUP_APICMDBUF(info, pBuf, sizeof(APICMDBUF_GET_RF_REG), APCMD_GET_RF_REG, sizeof(APICMDBUF_GET_RF_REG) - sizeof(APICMDBUF));
...
BOOLEAN WL_FUN_SetRfReg(void *info, USHORT reg, UCHAR value)
{
	...
	WL_FUN_SETUP_APICMDBUF(info, pBuf, sizeof(APICMDBUF_SET_RF_REG), APCMD_SET_RF_REG, sizeof(APICMDBUF_SET_RF_REG) - sizeof(APICMDBUF));
