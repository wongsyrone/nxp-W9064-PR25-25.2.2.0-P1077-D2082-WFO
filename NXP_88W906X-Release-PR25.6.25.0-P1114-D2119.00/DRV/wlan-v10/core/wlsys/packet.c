...
UINT32 dbg_BAredr_tag[7] =
    { DBG_BAREORDER_TAG_TMO, DBG_BAREORDER_TAG_SN, DBG_BAREORDER_TAG_OOR, DBG_BAREORDER_TAG_OOR_RPT, DBG_BAREORDER_TAG_STORE, DBG_BAREORDER_TAG_WEND,
DBG_BAREORDER_TAG_BAR };
UINT8 dbg_BAredr_log_en_mask = (DBG_BAREORDER_TMO_EN_BIT | DBG_BAREORDER_OOR_EN_BIT | DBG_BAREORDER_WEND_EN_BIT);
...
static UINT32 BA_send2host(struct net_device *dev, struct sk_buff *skb, extStaDb_StaInfo_t * pStaInfo, UINT8 tid);
...
void dbg_replay_attack_trace(UINT32 flag, UINT32 data0, UINT32 data1, UINT32 data2, UINT32 data3, UINT32 data4, UINT32 data5, UINT32 data6)
{
	if (dbg_pn_idx >= DBG_REPLAY_ATTACK_MAX)
		return;

	if (pDbgPnRec == NULL) {
		pDbgPnRec = wl_kmalloc(sizeof(dbg_replay_attack_t) * DBG_REPLAY_ATTACK_MAX, GFP_ATOMIC);
	}
	if (pDbgPnRec) {
		...
		if (dbg_pn_idx >= DBG_REPLAY_ATTACK_MAX)
			dbg_pn_idx = 0;
		memset((void *)&pDbgPnRec[dbg_pn_idx], 0xff, sizeof(dbg_replay_attack_t));
	}
}

void dbg_replay_attack_print(void)
{
	int i;
	...
	for (i = 0; i < DBG_REPLAY_ATTACK_MAX; i++) {
		printk("flag=%08x   state=%08x   prev=%08x-%08x   curr=%08x-%08x   next=%08x-%08x\n",
		       pDbgPnRec[i].data[0],
		       pDbgPnRec[i].data[1],
		       pDbgPnRec[i].data[2], pDbgPnRec[i].data[3],
		       pDbgPnRec[i].data[4], pDbgPnRec[i].data[5], pDbgPnRec[i].data[6], pDbgPnRec[i].data[7]);
	}
}

void dbg_replay_attack_release(void)
{
	if (pDbgPnRec) {
		wl_kfree(pDbgPnRec);
	}
...
#ifdef SOC_W906X
extern UINT8 decomposeMBSSID(struct net_device *dev, struct sk_buff *skb, struct sk_buff_head *skbList);
#endif
...
void start_log(void);
UINT32 BA_flushSequencialData(struct net_device *dev, extStaDb_StaInfo_t * pStaInfo, BA_RX_st * baRxInfo,
			      UINT32 winStartB_BufCnt, rx_queue_t * rq, UINT32 * badPNcnt, UINT32 * passPNcnt, UINT8 tid);
...
int send_11n_aggregation_skb(struct net_device *netdev, extStaDb_StaInfo_t * pStaInfo, int force)
{
	...
	for (i = 0; i < MAX_AGG_QUE; i++) {
		if (CurrentRdFrag(i).status
		    && (force || (jiffies >= (CurrentRdFrag(i).jiffies + HZ / 100) /*CurrentRdFrag(i).status == CurrentRdFrag(i).status_pre */ ))) {
			previous_aggregation_index = i;

			skb = CurrentRdFrag(i).skb;
#ifdef SOC_W8964
			if (SUCCESS == wlxmit(netdev, skb, IEEE_TYPE_DATA, pStaInfo, 0, FALSE)) {
#else
			if (SUCCESS == wlxmit(netdev, skb, IEEE_TYPE_DATA, pStaInfo, 0, FALSE, 0)) {
#endif
				retval++;
				wlpptr->netDevStats.tx_bytes += skb->len;
				NextFrag(i);
...
void AddbaTimerProcess(UINT8 * data)
{
	...
	pStaInfo = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) & (Ampdu_p->MacAddr), STADB_UPDATE_AGINGTIME);
	if (pStaInfo) {
...
#ifdef SOC_W906X
extern void Ampdu_Flush_All_Pck_in_Reorder_queue(struct net_device *dev, extStaDb_StaInfo_t * pStaInfo, u_int8_t Priority);
#else
...
#ifdef SOC_W906X
	Ampdu_Flush_All_Pck_in_Reorder_queue(ro_p->dev, ro_p->pStaInfo, ro_p->tid);
	wlpptr->wlpd_p->AmpduPckReorder[ro_p->pStaInfo->StnId].ReOrdering[ro_p->tid] = FALSE;
...
void AddbaTimerProcessSta(UINT8 * data)
{
	...
	pStaInfo = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) GetParentStaBSSID(vmacEntry_p->phyHwMacIndx), STADB_UPDATE_AGINGTIME);
	if (pStaInfo) {
		pStaInfo->aggr11n.onbytid[Ampdu_p->AccessCat] = 0;
		pStaInfo->aggr11n.startbytid[Ampdu_p->AccessCat] = 0;
	}
...
static void __inline ieee80211_txampdu_update_pps(Ampdu_tx_t * tap)
{
 ...
 */
void __inline ieee80211_txampdu_count_packet(Ampdu_tx_t * tap)
{
...
static void __inline tx_update_pps(txACInfo * tap)
{
	/* NB: scale factor of 2 was picked heuristically */
 ...
 * Count a packet towards the pps estimate.
 */
void __inline tx_count_packet(txACInfo * tap)
{
	if (jiffies - tap->txa_lastsample >= MAX_SECS_TO_RESET_PPS * HZ) {
		tap->txa_pkts = 1;
		tap->txa_avgpps = 0;
 ...
 */
static int __inline ieee80211_txampdu_getpps(Ampdu_tx_t * tap)
{
	if (jiffies - tap->txa_lastsample >= MAX_SECS_TO_RESET_PPS * HZ) {
		tap->txa_avgpps = 0;
		tap->txa_lastsample = jiffies;
...
static inline void enableAmpduTx(vmacApInfo_t * vmacSta_p, UINT8 * macaddr, UINT8 tid)
{
	...
	if (vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_CLNT_INFRA) {
		if (!smeGetStaLinkInfo(vmacSta_p->VMacEntry.id, &AssociatedFlag, &bssId[0])) {
			return;
		}
		if (AssociatedFlag) {
...
#ifdef AMPDU_SUPPORT_TX_CLIENT
						if (vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_CLNT_INFRA)
							SendDelBASta(vmacSta_p, (UINT8 *) & wlpptr->wlpd_p->Ampdu_tx[0].MacAddr[0],
								     wlpptr->wlpd_p->Ampdu_tx[0].AccessCat);
						else
#endif
							SendDelBA(vmacSta_p, (UINT8 *) & wlpptr->wlpd_p->Ampdu_tx[0].MacAddr[0],
								  wlpptr->wlpd_p->Ampdu_tx[0].AccessCat);
						wlFwUpdateDestroyBAStream(vmacSta_p->dev, 0, 0, 0, wlpptr->wlpd_p->Ampdu_tx[0].AccessCat,
									  wlpptr->wlpd_p->Ampdu_tx[0].MacAddr, pStaInfo ? pStaInfo->StnId : 0);
						if (pStaInfo) {
...
#ifdef AMPDU_SUPPORT_TX_CLIENT
						if (vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_CLNT_INFRA)
							SendDelBASta(vmacSta_p, (UINT8 *) & wlpptr->wlpd_p->Ampdu_tx[1].MacAddr[0],
								     wlpptr->wlpd_p->Ampdu_tx[1].AccessCat);
						else
#endif
							SendDelBA(vmacSta_p, (UINT8 *) & wlpptr->wlpd_p->Ampdu_tx[1].MacAddr[0],
								  wlpptr->wlpd_p->Ampdu_tx[1].AccessCat);
						wlFwUpdateDestroyBAStream(vmacSta_p->dev, 0, 0, 1, wlpptr->wlpd_p->Ampdu_tx[1].AccessCat,
									  wlpptr->wlpd_p->Ampdu_tx[1].MacAddr, pStaInfo ? pStaInfo->StnId : 0);
						if (pStaInfo) {
							pStaInfo->aggr11n.onbytid[tid] = 0;
							pStaInfo->aggr11n.startbytid[tid] = 0;
							pStaInfo->aggr11n.type &= ~WL_WLAN_TYPE_AMPDU;
	...
	for (stream = AMPDU_STREAM_NO_START; stream < AMPDU_STREAM_NO_END; stream++)
		...
		if (wlpptr->wlpd_p->Ampdu_tx[stream].InUse != 1) {
			...
			wlFwGetSeqNoBAStream(vmacSta_p->dev, macaddr, tid, (UINT16 *) & (wlpptr->wlpd_p->Ampdu_tx[stream].start_seqno));

#ifdef AMPDU_SUPPORT_TX_CLIENT
			if (vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_CLNT_INFRA) {
				TimerFireIn(&wlpptr->wlpd_p->Ampdu_tx[stream].timer, 1, &AddbaTimerProcessSta,
					    (UINT8 *) & wlpptr->wlpd_p->Ampdu_tx[stream], ADDBA_PERIOD_1SEC);
				SendAddBAReqSta(vmacSta_p, macaddr, tid, 1, wlpptr->wlpd_p->Ampdu_tx[stream].start_seqno,
						wlpptr->wlpd_p->Ampdu_tx[stream].DialogToken);
			} else
#endif
			{
				TimerFireIn(&wlpptr->wlpd_p->Ampdu_tx[stream].timer, 1, &AddbaTimerProcess,
					    (UINT8 *) & wlpptr->wlpd_p->Ampdu_tx[stream], ADDBA_PERIOD_1SEC);
				if (pStaInfo && (pStaInfo->he_cap_ie == HE_CAPABILITIES_IE) && (tid == 6)) {
					if (pStaInfo->aggr11n.winsize[tid])
						winsize = pStaInfo->aggr11n.winsize[tid];
					else if (vmacSta_p->winsize[tid])
						winsize = vmacSta_p->winsize[tid];
					else
						winsize = 1;
				}

				SendAddBAReq(vmacSta_p, macaddr, tid, 1, wlpptr->wlpd_p->Ampdu_tx[stream].start_seqno, wlpptr->wlpd_p->Ampdu_tx[stream].DialogToken, winsize, &(wlpptr->wlpd_p->Ampdu_tx[stream].BufSizeInReq));/** Only support immediate ba **/
			}
			if (pStaInfo) {
				pStaInfo->aggr11n.type |= WL_WLAN_TYPE_AMPDU;
	...
	for (i = AMPDU_STREAM_NO_START; i < AMPDU_STREAM_NO_END; i++) {
		//if(wlpptr->wlpd_p->Ampdu_tx[i].InUse == 1)
		{
			if (!MACADDR_CMP(wlpptr->wlpd_p->Ampdu_tx[i].MacAddr, ampduMacAddr)) {
				...
				wlFwUpdateDestroyBAStream(vmacSta_p->dev, 0, 0, i, tid, wlpptr->wlpd_p->Ampdu_tx[i].MacAddr,
...
#endif
						SendDelBA(vmacSta_p, ampduMacAddr, tid);
					wlFwUpdateDestroyBAStream(vmacSta_p->dev, 0, 0, i, tid,
								  wlpptr->wlpd_p->Ampdu_tx[i].MacAddr, pStaInfo ? pStaInfo->StnId : 0);
					if (pStaInfo) {
						pStaInfo->aggr11n.onbytid[tid] = 0;
						pStaInfo->aggr11n.startbytid[tid] = 0;
						pStaInfo->aggr11n.type &= ~WL_WLAN_TYPE_AMPDU;
...
void disableAmpduTxAll(vmacApInfo_t * vmacSta_p)
{
	...
	for (i = AMPDU_STREAM_NO_START; i < AMPDU_STREAM_NO_END; i++) {
		if (wlpptr->wlpd_p->Ampdu_tx[i].InUse == 1) {
			...
			pStaInfo = extStaDb_GetStaInfo(wlpptr->wlpd_p->Ampdu_tx[i].vmacSta_p,
						       (IEEEtypes_MacAddr_t *) & wlpptr->wlpd_p->Ampdu_tx[i].MacAddr[0], STADB_DONT_UPDATE_AGINGTIME);

			SendDelBA(wlpptr->wlpd_p->Ampdu_tx[i].vmacSta_p, wlpptr->wlpd_p->Ampdu_tx[i].MacAddr, wlpptr->wlpd_p->Ampdu_tx[i].AccessCat);
			wlFwUpdateDestroyBAStream(wlpptr->wlpd_p->Ampdu_tx[i].vmacSta_p->dev, 0, 0, i, wlpptr->wlpd_p->Ampdu_tx[i].AccessCat,
						  wlpptr->wlpd_p->Ampdu_tx[i].MacAddr, pStaInfo ? pStaInfo->StnId : 0);
			wlpptr->wlpd_p->Ampdu_tx[i].InUse = 0;
			wlpptr->wlpd_p->Ampdu_tx[i].TimeOut = 0;
...
void disableAmpduTxstream(vmacApInfo_t * vmacSta_p, int stream)
{
	...
	if (wlpptr->wlpd_p->Ampdu_tx[stream].InUse == 1) {
		...
		pStaInfo = extStaDb_GetStaInfo(wlpptr->wlpd_p->Ampdu_tx[stream].vmacSta_p,
					       (IEEEtypes_MacAddr_t *) & wlpptr->wlpd_p->Ampdu_tx[stream].MacAddr[0], STADB_DONT_UPDATE_AGINGTIME);

		SendDelBA(wlpptr->wlpd_p->Ampdu_tx[stream].vmacSta_p, wlpptr->wlpd_p->Ampdu_tx[stream].MacAddr,
			  wlpptr->wlpd_p->Ampdu_tx[stream].AccessCat);
		wlFwUpdateDestroyBAStream(wlpptr->wlpd_p->Ampdu_tx[stream].vmacSta_p->dev, 0, 0, stream, wlpptr->wlpd_p->Ampdu_tx[stream].AccessCat,
					  wlpptr->wlpd_p->Ampdu_tx[stream].MacAddr, pStaInfo ? pStaInfo->StnId : 0);
...
#ifdef SOC_W906X
struct sk_buff *ieee80211_encap(struct sk_buff *skb, struct net_device *netdev, BOOLEAN eap, void *pStaInfoIn)
#else
...
#endif
{
...
#else
	int stadb_flag = STADB_FIND_IN_CACHE | STADB_UPDATE_CACHE | STADB_UPDATE_AGINGTIME;
#endif
		...
		if (!vmacEntry_p->active) {
			if (*(mib->mib_STAMacCloneEnable) == 1) {
...
#ifdef SOC_W906X
#ifdef OPENWRT
				if ((dev2 = dev_get_by_name(&init_net, "br-lan")) != NULL) {
#else
				...
				for_each_netdev_rcu(&init_net, dev2) {
					if (dev2->priv_flags & IFF_EBRIDGE && memcmp(eh.ether_shost, dev2->dev_addr, 6) == 0) {
#endif
				...
				WLSNDEVT(netdev, IWEVCUSTOM, (IEEEtypes_MacAddr_t *) & vmacEntry_p->vmacAddr[0], WLSYSLOG_MSG_CLIENT_CLONED);
			...
			if ((pStaInfo = extStaDb_GetStaInfo(vmacSta_p, &(eh.ether_shost), stadb_flag)) == NULL) {
				...
				child_VMacEntry_p = smeStartChildSession(vmacEntry_p->phyHwMacIndx,
									 ...
									 1, sme_GetParentPrivInfo(vmacEntry_p->phyHwMacIndx));
				goto bad;
			}
#endif
		}
	...
	if (!IS_GROUP((UINT8 *) & (eh.ether_dhost)) || wds || (vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_CLNT_INFRA)) {
		if (wds) {
			pStaInfo = extStaDb_GetStaInfo(vmacSta_p, &(pWdsPort->wdsMacAddr), stadb_flag);
		}
#ifdef MULTI_AP_SUPPORT
		else if ((vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_AP) &&
			...
			if (pStaInfoIn == NULL) {
#endif				/* SOC_W906X */
				if ((pStaInfo = extStaDb_GetStaInfo(vmacSta_p, &(eh.ether_dhost), stadb_flag)) == NULL) {
					found = FourAddr_SearchHashEntry((IEEEtypes_MacAddr_t *) eh.ether_dhost, &entry, 0);

					if (found == 1) {
						MACADDR_CPY(bssId, entry->tar);
						pStaInfo_multiAP = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) & bssId[0], stadb_flag);
						...
						WLDBG_INFO(DBG_LEVEL_9, "ieee80211_encap: target mac not found!!!\n");
				}
#ifdef SOC_W906X
			} else
...
#ifdef CLIENT_SUPPORT
			if (vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_CLNT_INFRA)
				pStaInfo = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) & bssId[0], stadb_flag);
			else
#endif
#ifdef SOC_W906X
			if (pStaInfoIn == NULL) {
			...
			if (pStaInfo && pStaInfo->aggr11n.threshold) {
				...
				if (!eap) {
					...
					case 3:
						...
						if (!((pStaInfo->aggr11n.ampducfg) & (0x1 << tid)) || !((master->ampducfg) & (0x1 << tid))) {
							if (pStaInfo->aggr11n.startbytid[tid & 0x07]) {
	...
	if ((pStaInfo && (pStaInfo->StaType & 0x02) == 0x02)
		...
		llc_rptr->llc.llc_un.type_snap.ether_type = htons(RPTR_ETHERTYPE);
...
#endif
		if (llc->llc_dsap != 0x0 || llc->llc_ssap != 0x1 || llc->llc_un.type_u.control != 0xAF) {
...
#ifdef SOC_W8964
	{
		...
		p = (struct ether_header *)skb_push(skb, sizeof(struct ether_header));
	...
	if (!IS_GROUP((UINT8 *) & (eh.ether_dhost)) || wds
		...
		if (((pStaInfo == NULL) || ((vmacSta_p->VMacEntry.modeOfService != VMAC_MODE_CLNT_INFRA) && (pStaInfo->State != ASSOCIATED)))
		    && !wds)
#else
	if (!IS_GROUP((UINT8 *) & (eh.ether_dhost))) {
...
#endif
		{
			...
			wlpptr->wlpd_p->drv_stats_val.txq_ac_stats[AccCategoryQ[tid]].drop_skb++;
			wl_free_skb(skb);
			return NULL;
		}
		if (*(mib->QoSOptImpl)) {
	...
	if (!IS_GROUP((UINT8 *) & (eh.ether_dhost))) {
		...
		if (*(mib->QoSOptImpl)) {
			...
			if (pStaInfo->IsStaQSTA) {
				...
				*(UINT16 *) & wh->seq[0] = ENDIAN_SWAP16(EdcaSeqNum[pStaInfo->StnId][Priority] << 4);
				EdcaSeqNum[pStaInfo->StnId][Priority]++;
...
#endif
static struct sk_buff *ieee80211_decap(struct net_device *dev, struct sk_buff *skb, extStaDb_StaInfo_t * pStaInfo, u_int16_t stnid)
{
	...
	if (skb->protocol & WL_WLAN_TYPE_RX_FAST_DATA) {
		...
		if (*(mib->mib_wdsEnable)) {
			...
			struct wlprivate *myPriv = NETDEV_PRIV_P(struct wlprivate, vDev);

			for (k = 0; k < 6; k++) {
				if (myPriv->vmacSta_p->wdsPort[k].active) {
					pWdsPort = &myPriv->vmacSta_p->wdsPort[k];
					WLDBG_INFO(DBG_LEVEL_6, "pWdsPort: %02x%02x%02x%02x%02x%02x\n",
						   pWdsPort->wdsMacAddr[0], pWdsPort->wdsMacAddr[1], pWdsPort->wdsMacAddr[2],
						   pWdsPort->wdsMacAddr[3], pWdsPort->wdsMacAddr[4], pWdsPort->wdsMacAddr[5]);

					if ((pStaInfo = extStaDb_GetStaInfo(vmacSta_p, &(pWdsPort->wdsMacAddr), STADB_UPDATE_AGINGTIME)) == NULL) {
						WLDBG_ERROR(DBG_LEVEL_6, "drop1, extStaDb_GetStaInfo() == NULL\n");
						goto dropPacket;
					}
					if ((pStaInfo->StnId == stnid) || (skb->protocol & WL_WLAN_TYPE_WDS)) {
						IEEE80211_ADDR_COPY(&wh.addr4[0], &skb->data[6]);
						IEEE80211_ADDR_COPY(&wh.addr2[0], &pWdsPort->wdsMacAddr[0]);
						skb->protocol |= WL_WLAN_TYPE_WDS;
						skb->dev = (struct net_device *)pStaInfo->wdsInfo;
						WLDBG_INFO(DBG_LEVEL_6, "ieee80211_decap: stnid:%xh\n", stnid);
						break;
					}
				}
			}
...
#ifdef MULTI_AP_SUPPORT
		else if (pStaInfo && ((pStaInfo->MultiAP_4addr == 1) || ((pStaInfo->MultiAP_4addr == 2) && (skb->protocol & WL_WLAN_TYPE_WDS)))) {
...
#endif
		{
			if (*(mib->mib_STAMode) && !pStaInfo) {	/** check need to change for dual STA and AP mode **/
				IEEE80211_ADDR_COPY(&wh.addr1[0], &skb->data[0]);
				IEEE80211_ADDR_COPY(&wh.addr3[0], &skb->data[6]);
				...
				if ((pStaInfo =
				     extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) & priv->wlpd_p->mac_addr_sta_ta /*&(wh.addr2) */ ,
							 STADB_UPDATE_AGINGTIME | STADB_NO_BLOCK)) == NULL) {
					goto dropPacket;
				}
			}
		}
	...
	if (pStaInfo && pStaInfo->MultiAP_4addr && (skb->protocol & WL_WLAN_TYPE_WDS) && !pStaInfo->Client) {
		UINT8 found = 0;
		MultiAP_4Addr_Entry_t *entry;
		found = FourAddr_SearchHashEntry((IEEEtypes_MacAddr_t *) wh.addr4, &entry, 0);

		if (found == 1) {
			MACADDR_CPY(entry->tar, wh.addr2);
		} else {
			FourAddr_AddHashEntry(&entry, (IEEEtypes_MacAddr_t *) wh.addr2, (IEEEtypes_MacAddr_t *) wh.addr4);
		}
	}
#endif				/* MULTI_AP_SUPPORT */
...
#endif
	{
...
#ifdef SOC_W906X
			macMgmtMlme_SendDeauthenticateMsg(vmacSta_p, &(wh.addr2), 0, IEEEtypes_REASON_CLASS3_NONASSOC, TRUE);
#else
...
#endif
		/* fragattack -- CVE-2020-26139 AP forwards EAPOL attack tests */
		if ((mib->Privacy->RSNEnabled == 1) || (mib->RSNConfigWPA2->WPA2Enabled == 1)) {
			...
			if ((flagPsk == 0) && memcmp(&(wh.addr3), &(wh.addr1), sizeof(IEEEtypes_MacAddr_t))) {
				/* If handshake not in "end" state and RA is not the same as RA, skip the packet */
				wl_free_skb(skb);
				return NULL;
			}
		}
	...
	if (skb->len >= sizeof(struct llc) &&
	    ...
	    llc->llc_un.type_u.control == LLC_UI &&
	    ((llc->llc_un.type_snap.org_code[0] == 0x00 && llc->llc_un.type_snap.org_code[1] == 0x40 && llc->llc_un.type_snap.org_code[2] == 0x96) ||
	     (llc->llc_un.type_snap.org_code[0] == 0x00 && llc->llc_un.type_snap.org_code[1] == 0x00 && llc->llc_un.type_snap.org_code[2] == 0x00))) {
	...
	if ((skb->protocol & WL_WLAN_TYPE_RX_FAST_DATA) == 0) {
...
#endif
		{
			...
			if (pStaInfo->AP
#ifdef MULTI_AP_SUPPORT
			    || ((pStaInfo && pStaInfo->MultiAP_4addr) && (wh.FrmCtl.FromDs == 1 && wh.FrmCtl.ToDs == 1))
#endif
			    ) {
...
static inline void DOT11_SETPN(u8 * dst, u8 * src)
{
	(*(u16 *) & dst[0]) = (*(u16 *) & src[0]);
	(*(u32 *) & dst[2]) = (*(u32 *) & src[2]);
}

static inline int DOT11_CMPPN_GE(u8 * dst, u8 * src)
{
...
static inline void DOT11_INCPN(u8 * dst)
{
	if (++(*(u16 *) & dst[0]) == 0)
		(*(u32 *) & dst[2])++;
}

static inline UINT32 GET_SEC_HDR_PN(UINT8 secMode, UINT8 * pSecHdr, UINT8 * PN, UINT8 * keyIdx)
{
	UINT32 retCode = 0;
...
static int pn_replay_detection(UINT8 frameType, UINT8 secMode, UINT8 * pSecHdr, rx_queue_t * rq, UINT16 seqNum, UINT32 * badPNcnt, UINT32 * passPNcnt)
{
	UINT8 keyIdx;
	...
	if (frameType == IEEE_TYPE_DATA) {
		...
		if ((secMode == SHAL_EU_MODE_CCMP) || (secMode == SHAL_EU_MODE_GCMP)) {
 ...
 Check_Legacy_PN:
	...
	if ((pSecHdr[3] & 0x20) != 0x20) {
		...
		dbg_replay_attack_trace(0x10000000 | rq->InxPN,
					slot->setFlag,
					(rq->prevSeqNum << 16) | (*(u16 *) & rq->RxPN[rq->InxPN][4]),
					*(u32 *) & rq->RxPN[rq->InxPN][0],
					(rq->SeqNr << 16) | (*(u16 *) & slot->PN[4]),
					*(u32 *) & slot->PN[0], *(u32 *) & pSecHdr[4], *(u32 *) & pSecHdr[0]);
		dbg_pn_badCnt[0]++;
		slot->setFlag = 2;
		return 1;	//bad_PN                                                                              
	}
...
static UINT32 mgmt_pn_replay_detect(struct net_device *dev, struct sk_buff *skb,
				    struct ieee80211_frame *wh, UINT16 SeqNo, UINT8 machdrLen, UINT8 secMode, UINT8 * pn)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	MIB_802DOT11 *mib = vmacSta_p->Mib802dot11;
	extStaDb_StaInfo_t *pStaInfo = NULL;
	struct except_cnt __maybe_unused *wlexcept_p = &wlpptr->wlpd_p->except_cnt;
	...
	if (*(mib->mib_wdsEnable)) {
		...
		if (pWdsPort)
			pStaInfo = extStaDb_GetStaInfo(vmacSta_p, &(pWdsPort->wdsMacAddr), STADB_UPDATE_AGINGTIME);
	} else
#endif
#ifdef CLIENT_SUPPORT
	if (*(mib->mib_STAMode)) {
		pStaInfo = extStaDb_GetStaInfo(vmacSta_p,
					       (IEEEtypes_MacAddr_t *) GetParentStaBSSID(vmacSta_p->VMacEntry.phyHwMacIndx), STADB_UPDATE_AGINGTIME);
	} else
#endif
	{
		pStaInfo = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) & (wh->addr2), STADB_UPDATE_AGINGTIME);
	}
	...
	if (pStaInfo && pStaInfo->Ieee80211wSta) {
		if (pn_replay_detection(IEEE_TYPE_MANAGEMENT, secMode, pn,
					...
					&wlexcept_p->badPNcntMgmtcast, &wlpptr->wlpd_p->drv_stats_val.rx_mgmt_ucast_pn_pass_cnt)) {
...
static UINT32 data_pn_replay_detect(struct net_device *dev, struct sk_buff *skb,
				    IEEEtypes_FrameCtl_t * frame_ctlp, UINT16 ampdu_qos, UINT16 SeqNo, UINT8 machdrLen, UINT8 secMode, UINT8 * pn)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	MIB_802DOT11 *mib = vmacSta_p->Mib802dot11;
...
#endif
	{
		pStaInfo = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) & wlpptr->wlpd_p->mac_addr_sta_ta /*&skb->data[6] */ ,
	...
	if (pStaInfo) {
		...
		if ((tid == MAX_TID) || (baRxInfo->AddBaReceive[tid] == FALSE)) {
			...
			passPNcnt = &wlpptr->wlpd_p->drv_stats_val.rx_data_ucast_pn_pass_cnt;
			if (*(mib->mib_STAMode)) {
				if (IS_GROUP((UINT8 *) & skb->data[0])) {
					...
					passPNcnt = &wlpptr->wlpd_p->drv_stats_val.rx_data_mcast_pn_pass_cnt;
				}
			}

			if (pn_replay_detection(IEEE_TYPE_DATA, secMode, pn, rq, SeqNo, badPNcnt, passPNcnt)) {
...
static inline UINT8 pn_aggregate_check(extStaDb_StaInfo_t * pStaInfo, BA_RX_st * baRxInfo, rx_queue_t * rq,
				       UINT32 index, UINT32 * badPNcnt, UINT32 * passPncnt)
{
	if (baRxInfo->pn_check_enabled) {
		...
		printk("state = %d-%d-%d flag=%d-%d-%d\n",
		       baRxInfo->AmsduQ[prevIdx].state, baRxInfo->AmsduQ[index].state, baRxInfo->AmsduQ[tmpNext].state,
		       rq->Slots[prevIdx].setFlag, rq->Slots[index].setFlag, rq->Slots[tmpNext].setFlag);
		...
		if (rq->Slots[index].setFlag == 2) {
#if 0
			printk
			    ("pn_aggregate_check with self drop :: index=%d - %d - %d [%d]  seq=0x%04x - 0x%04x - 0x%04x [0x%04x] state=%d-%d-%d flag=%d-%d-%d\n",
			     prevIdx, index, tmpNext, nextIdx, rq->Slots[prevIdx].SeqNr, rq->Slots[index].SeqNr, rq->Slots[tmpNext].SeqNr, rq->SeqNr,
			     baRxInfo->AmsduQ[prevIdx].state, baRxInfo->AmsduQ[index].state, baRxInfo->AmsduQ[tmpNext].state,
			     rq->Slots[prevIdx].setFlag, rq->Slots[index].setFlag, rq->Slots[tmpNext].setFlag);
			mwl_hex_dump(rq->RxPN[rq->InxPN], 16);
			...
			mwl_hex_dump(rq->Slots[tmpNext].PN, 16);
#endif
			dbg_replay_attack_trace(0x30000000 | (rq->InxPN << 16) | (index << 8) | tmpNext,
						((baRxInfo->AmsduQ[prevIdx].state << 20) | (rq->Slots[prevIdx].setFlag << 16)) |
						((baRxInfo->AmsduQ[index].state << 12) | (rq->Slots[index].setFlag << 8)) |
						((baRxInfo->AmsduQ[tmpNext].state << 4) | rq->Slots[tmpNext].setFlag),
						(rq->Slots[prevIdx].SeqNr << 16) | (*(u16 *) & rq->Slots[prevIdx].PN[4]),
						*(u32 *) & rq->Slots[prevIdx].PN[0],
						(rq->Slots[index].SeqNr << 16) | (*(u16 *) & rq->Slots[index].PN[4]),
						*(u32 *) & rq->Slots[index].PN[0],
						(rq->Slots[tmpNext].SeqNr << 16) | (*(u16 *) & rq->Slots[tmpNext].PN[4]),
						*(u32 *) & rq->Slots[tmpNext].PN[0]);
		...
		if ((rq->Slots[prevIdx].setFlag == 1) &&
		    (rq->Slots[index].setFlag == 1) && DOT11_CMPPN_GE(rq->Slots[prevIdx].PN, rq->Slots[index].PN)) {
#if 0
			printk
			    ("pn_aggregate_check with previous :: index=%d - %d - %d [%d]  seq=0x%04x - 0x%04x - 0x%04x [0x%04x] state=%d-%d-%d flag=%d-%d-%d\n",
			     prevIdx, index, tmpNext, nextIdx, rq->Slots[prevIdx].SeqNr, rq->Slots[index].SeqNr, rq->Slots[tmpNext].SeqNr, rq->SeqNr,
			     baRxInfo->AmsduQ[prevIdx].state, baRxInfo->AmsduQ[index].state, baRxInfo->AmsduQ[tmpNext].state,
			     rq->Slots[prevIdx].setFlag, rq->Slots[index].setFlag, rq->Slots[tmpNext].setFlag);
			mwl_hex_dump(rq->RxPN[rq->InxPN], 16);
			...
			mwl_hex_dump(rq->Slots[tmpNext].PN, 16);
#endif
			dbg_replay_attack_trace(0x40000000 | (rq->InxPN << 16) | (index << 8) | tmpNext,
						((baRxInfo->AmsduQ[prevIdx].state << 20) | (rq->Slots[prevIdx].setFlag << 16)) |
						((baRxInfo->AmsduQ[index].state << 12) | (rq->Slots[index].setFlag << 8)) |
						((baRxInfo->AmsduQ[tmpNext].state << 4) | rq->Slots[tmpNext].setFlag),
						(rq->Slots[prevIdx].SeqNr << 16) | (*(u16 *) & rq->Slots[prevIdx].PN[4]),
		...
		if (nextIdx < MAX_BA_REORDER_BUF_SIZE) {
			...
			if (rq->Slots[index].SeqNr > rq->Slots[nextIdx].SeqNr) {
				offset = rq->Slots[index].SeqNr - rq->Slots[nextIdx].SeqNr + 1;
			} else {
				offset = 0x1000 - rq->Slots[nextIdx].SeqNr + rq->Slots[index].SeqNr + 1;
			}
			...
			if ((rq->Slots[nextIdx].setFlag == 1) && (DOT11_CMPPN_GE(rq->Slots[prevIdx].PN, rq->Slots[nextIdx].PN))) {
			...
			if ((rq->Slots[index].setFlag == 1) &&
			    (rq->Slots[nextIdx].setFlag == 1) && DOT11_CMPPN_GE(rq->Slots[index].PN, rq->Slots[nextIdx].PN)) {
#if 0
				printk
				    ("pn_aggregate_check with next :: index=%d - %d - %d  seq=0x%04x - 0x%04x - 0x%04x [0x%04x]  state=%d-%d-%d flag=%d-%d-%d\n",
				     prevIdx, index, nextIdx, rq->Slots[prevIdx].SeqNr, rq->Slots[index].SeqNr, rq->Slots[nextIdx].SeqNr, rq->SeqNr,
				     baRxInfo->AmsduQ[prevIdx].state, baRxInfo->AmsduQ[index].state, baRxInfo->AmsduQ[nextIdx].state,
				     rq->Slots[prevIdx].setFlag, rq->Slots[index].setFlag, rq->Slots[tmpNext].setFlag);
				mwl_hex_dump(rq->RxPN[rq->InxPN], 16);
				mwl_hex_dump(rq->Slots[prevIdx].PN, 16);
				mwl_hex_dump(rq->Slots[index].PN, 16);
...
#endif
				dbg_replay_attack_trace(0x50000000 | (rq->InxPN << 16) | (index << 8) | nextIdx,
							((baRxInfo->AmsduQ[prevIdx].state << 20) | (rq->Slots[prevIdx].setFlag << 16)) |
							((baRxInfo->AmsduQ[index].state << 12) | (rq->Slots[index].setFlag << 8)) |
							((baRxInfo->AmsduQ[nextIdx].state << 4) | rq->Slots[nextIdx].setFlag),
							(rq->Slots[prevIdx].SeqNr << 16) | (*(u16 *) & rq->Slots[prevIdx].PN[4]),
							*(u32 *) & rq->Slots[prevIdx].PN[0],
							(rq->Slots[index].SeqNr << 16) | (*(u16 *) & rq->Slots[index].PN[4]),
							*(u32 *) & rq->Slots[index].PN[0],
							(rq->Slots[nextIdx].SeqNr << 16) | (*(u16 *) & rq->Slots[nextIdx].PN[4]),
							*(u32 *) & rq->Slots[nextIdx].PN[0]);
				rq->Slots[index].setFlag = 0;
				pStaInfo->pn->ucastBadCnt++;
				(*badPNcnt)++;
				dbg_pn_badCnt[4]++;
...
void ampdu_Init(struct net_device *dev)
{
	...
	for (i = 0; i < sta_num; i++) {
		memset((UINT8 *) wlpptr->wlpd_p->AmpduPckReorder[i].ba, 0, (MAX_UP * sizeof(BA_RX_st)));
		memset((UINT8 *) wlpptr->wlpd_p->AmpduPckReorder[i].AddBaReceive, 0, (MAX_UP * sizeof(UINT8)));

		for (j = 0; j < MAX_UP; j++) {
			for (k = 0; k < MAX_BA_REORDER_BUF_SIZE; k++)
				skb_queue_head_init(&wlpptr->wlpd_p->AmpduPckReorder[i].ba[j].AmsduQ[k].skbHead);
		}
	}
#else
	for (i = 0; i < MAX_AID; i++) {
...
#endif				/* SOC_W906X */
	memset(&wlpptr->wlpd_p->Ampdu_tx[0], 0, sizeof(Ampdu_tx_t) * MAX_SUPPORT_AMPDU_TX_STREAM_RUNNING);
}

#ifdef SOC_W906X
void ampdu_ReInit(struct net_device *dev, u_int16_t StnId)
#else
...
#endif				/* SOC_W906X */
{
	...
	for (j = 0; j < MAX_UP; j++) {
		for (k = 0; k < MAX_AMPDU_REORDER_BUFFER; k++) {
			skb = wlpptr->wlpd_p->AmpduPckReorder[StnId].pFrame[j][i];
			if (skb != NULL) {
				wlpptr->wlpd_p->AmpduPckReorder[StnId].pFrame[j][i] = NULL;
				wl_free_skb(skb);
			}

			wlpptr->wlpd_p->AmpduPckReorder[StnId].pFrame[j][k] = NULL;
			wlpptr->wlpd_p->AmpduPckReorder[StnId].ExpectedSeqNo[j][k] = 0;
		}
	...
	for (j = 0; j < MAX_AMPDU_REORDER_BUFFER - offset; j++) {
		if (wlpptr->wlpd_p->AmpduPckReorder[Aid].pFrame[Priority][j + offset] != NULL) {	//** I don't think we need to do this **/
			if (wlpptr->wlpd_p->AmpduPckReorder[Aid].pFrame[Priority][j] != NULL)
				wl_free_skb(wlpptr->wlpd_p->AmpduPckReorder[Aid].pFrame[Priority][j]);
			wlpptr->wlpd_p->AmpduPckReorder[Aid].pFrame[Priority][j] = wlpptr->wlpd_p->AmpduPckReorder[Aid].pFrame[Priority][j + offset];
			wlpptr->wlpd_p->AmpduPckReorder[Aid].ExpectedSeqNo[Priority][j] =
			    wlpptr->wlpd_p->AmpduPckReorder[Aid].ExpectedSeqNo[Priority][j + offset];
...
#ifdef SOC_W906X
int flush_blockack_pck(struct net_device *dev, int i, extStaDb_StaInfo_t * pStaInfo, u_int8_t Priority)
{
	struct sk_buff *skb;
	...
	if (skb != NULL) {
		if (skb->protocol & WL_WLAN_TYPE_RX_FAST_DATA) {
			if ((skb->protocol & WL_WLAN_TYPE_STA) || (skb->protocol & WL_WLAN_TYPE_WDS)) {
				if (skb->protocol & WL_WLAN_TYPE_STA)
...
#ifdef SOC_W906X
void Ampdu_Flush_All_Pck_in_Reorder_queue(struct net_device *dev, extStaDb_StaInfo_t * pStaInfo, u_int8_t Priority)
#else
...
#ifdef SOC_W906X
void free_any_pending_ampdu_pck(struct net_device *dev, u_int16_t StnId)
#else
...
void flush_any_pending_ampdu_pck(struct net_device *dev, extStaDb_StaInfo_t * pStaInfo)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
	int i, j;
	Ampdu_Pck_Reorder_t *baRxInfo;
	...
	for (i = 0; i < MAX_UP; i++) {
		if (baRxInfo->ba[i].storedBufCnt != 0) {
			for (j = 0; j < MAX_BA_REORDER_BUF_SIZE; j++) {
				//skb_queue_purge(&baRxInfo->ba[i].AmsduQ[j].skbHead);
				while (skb_queue_len(&baRxInfo->ba[i].AmsduQ[j].skbHead)) {
					struct sk_buff *skb = NULL;
					skb = skb_dequeue(&baRxInfo->ba[i].AmsduQ[j].skbHead);
...
#ifdef SOC_W906X
#ifdef DEBUG_BAREORDER
void BA_DebugLog(UINT8 tid, UINT16 winStartB, UINT16 SeqNo, UINT32 cardIndex_Id, UINT32 tag, UINT8 info)
{
	if (((dbg_BAredr_cardindex << 16) | dbg_BAredr_id) == cardIndex_Id) {
		...
		if (tag == DBG_BAREORDER_TAG_OOR) {
			...
			log = (tid << 28) | ((info & 0xF) << 24) | (winStartB << 12) | (SeqNo);
		...
		//Enable log
		if ((dbg_BAredr_log_en_mask & (tag & 0xFF)) && (dbg_BAredr_log_cnt < (DBG_BAREORDER_LOG_MASK - 4))) {
			UINT32 i, index;

			dbg_BAredr_log[dbg_BAredr_log_cnt++ & DBG_BAREORDER_LOG_MASK] = tag & 0xFFFFFF00;
			dbg_BAredr_log[dbg_BAredr_log_cnt++ & DBG_BAREORDER_LOG_MASK] = log;

			index = dbg_BAredr_SN_cnt;	//get current seqno history index

			//get previous seqno history log
			for (i = 0; i < historyNum; i++) {
				if (dbg_BAredr_log_cnt > (DBG_BAREORDER_LOG_MASK - 4)) {
					break;
				}

				index = (index - 1) & DBG_BAREORDER_SN_MASK;
				//skip history log without data
				if ((dbg_BAredr_SN[index] == 0) || (dbg_BAredr_SN[index] == 0xdeaddead)) {
					break;
				}

				//if history log shows this pkt was stored
				if ((dbg_BAredr_SN[index] & 0x0F000000) == 0x09000000) {
					UINT8 loop = 0;
					dbg_BAredr_log[dbg_BAredr_log_cnt++ & DBG_BAREORDER_LOG_MASK] = dbg_BAredr_SN[index];

					//Only save first and last of pkt store evt, skip all consecutive store in between. 
					while ((i < (historyNum - 1))
					       && ((dbg_BAredr_SN[(index - 1) & DBG_BAREORDER_SN_MASK] & 0x0F000000) == 0x09000000)) {
						index = (index - 1) & DBG_BAREORDER_SN_MASK;
						i++;
						loop = 1;
					}
		...
		dbg_BAredr_log[dbg_BAredr_log_cnt & DBG_BAREORDER_LOG_MASK] = 0xdeadbeef;
	}
}
#endif

//Function to handle tasklet call for BA reorder
void BA_TimerProcess(UINT8 * data)
{
	struct reorder_t *ro_p = (struct reorder_t *)data;
	...
	if (ro_p == NULL) {
		printk("[%s], Invalid ro_p\n", __func__);
		goto exit;
	}
	if (ro_p->dev == NULL) {
		printk("[%s], Invalid ro_p->dev\n", __func__);
		goto exit;
	}
	...
	if ((mib = vmacSta_p->Mib802dot11) == NULL) {
		printk("[%s], Invalid mib, dev: %s\n", __func__, ro_p->dev->name);
		goto exit;
	}
	if ((wlexcept_p = &wlpptr->wlpd_p->except_cnt) == NULL) {
		printk("[%s], Invalid wlexcept_p, dev: %s\n", __func__, ro_p->dev->name);
		goto exit;
	}
	...
	if (wlpptr->master) {
		cardindex = ((NETDEV_PRIV_P(struct wlprivate, wlpptr->master)))->cardindex;
	} else {
	...
	if (jiffies - baRxInfo->ba[tid].minTime >= *(mib->mib_BAReorder_holdtime)) {
		leastSeqNo = baRxInfo->ba[tid].leastSeqNo & 0xFFFF;
		BufCnt = baRxInfo->ba[tid].storedBufCnt;

		if (BufCnt != 0) {
			if (baRxInfo->ba[tid].winStartB <= leastSeqNo) {
				dropCnt = leastSeqNo - baRxInfo->ba[tid].winStartB;
				vmacSta_p->BA_RodrTMODropCnt += dropCnt;
				pStaInfo->rxBaStats[tid].BA_RodrTMODropCnt += dropCnt;
				pStaInfo->rxBaStats[tid].BA_RodrLastDropReason = BA_RODR_DROP_TMO;
			} else {
				dropCnt = ((BA_MAX_SEQ_NUM + 1) - baRxInfo->ba[tid].winStartB) + leastSeqNo;
				vmacSta_p->BA_RodrTMODropCnt += dropCnt;
				pStaInfo->rxBaStats[tid].BA_RodrTMODropCnt += dropCnt;
				pStaInfo->rxBaStats[tid].BA_RodrLastDropReason = BA_RODR_DROP_TMO;
			}
			...
			BA_DebugLog(tid, baRxInfo->ba[tid].winStartB, dropCnt, (UINT32) (cardindex << 16 | id), DBG_BAREORDER_TAG_TMO, BufCnt);
#endif

			//Flush starting from least seqno in buf until 1st hole
			baRxInfo->ba[tid].winStartB = BA_flushSequencialData(ro_p->dev, pStaInfo, &baRxInfo->ba[tid],
									     ((leastSeqNo << 16) | BufCnt), &pStaInfo->pn->ucRxQueues[tid],
									     &wlexcept_p->badPNcntUcast,
									     &wlpptr->wlpd_p->drv_stats_val.rx_data_ucast_pn_pass_cnt, tid);

			DBG_BAREORDER_SN(((cardindex << 16) | id), tid, 12, baRxInfo->ba[tid].winStartB, leastSeqNo)
				...
				TimerFireInByJiffies(&baRxInfo->timer[tid], 1, &BA_TimerHdlr, (UINT8 *) ro_p, *(mib->mib_BAReorder_holdtime));
...
void BA_TimerHdlr(UINT8 * data)
{
	...
	tasklet_init(&wlpptr->wlpd_p->AmpduPckReorder[ro_p->pStaInfo->StnId].ba[ro_p->tid].BArodertask, (void *)BA_TimerProcess, (unsigned long)ro_p);
	tasklet_schedule(&wlpptr->wlpd_p->AmpduPckReorder[ro_p->pStaInfo->StnId].ba[ro_p->tid].BArodertask);
}

//Decide whether to add timer after a flush
//StoreFlag: 0-This function is called after a flush. 1-After a fresh skb store
void BA_TimerActivateCheck(struct net_device *dev, extStaDb_StaInfo_t * pStaInfo, Ampdu_Pck_Reorder_t * baRxInfo, UINT8 tid, UINT32 SeqNo,
			   UINT8 StoreFlag)
{
...
//Return 0 to drop
INLINE UINT32 BA_getSeqDelta(UINT32 wEnd_endExt, UINT32 winStartB, UINT32 seqNum, UINT32 winSizeB_minus1)
{
...
static UINT32 BA_send2host(struct net_device *dev, struct sk_buff *skb, extStaDb_StaInfo_t * pStaInfo, UINT8 tid)
{
	...
	int cnt = 0;
	/* skb might be freed after ForwardFrame, use skb_len to save skb->len */
...
INLINE UINT8 BA_CheckAmsduDup(BA_RX_st * baRxInfo, UINT32 bufIndex, struct sk_buff * skb,
			      UINT8 LMFbit, rx_queue_t * rq, UINT8 secMode, UINT16 SeqNo, UINT8 * pn)
{
...
void BA_flushAmsduQ(struct net_device *dev, extStaDb_StaInfo_t * pStaInfo, AmsduQ_st * AmsduQ, UINT8 fwdPkt, UINT8 tid)
{
	struct sk_buff *skb;
			      ...
			      UINT32 winStartB_BufCnt, rx_queue_t * rq, UINT32 * badPNcnt, UINT32 * passPNcnt, UINT8 tid)
{
	...
	if (storedBufCnt != 0) {
		...
		while (baRxInfo->AmsduQ[index].state >= 2) {
#ifdef RX_REPLAY_DETECTION
			fwdPkt = pn_aggregate_check(pStaInfo, baRxInfo, rq, index, badPNcnt, passPNcnt);
#endif
			...
			BA_flushAmsduQ(dev, pStaInfo, &baRxInfo->AmsduQ[index], fwdPkt, tid);
			sendMpduNum++;
...
UINT32 BA_flushAnyData(struct net_device * dev, extStaDb_StaInfo_t * pStaInfo, BA_RX_st * baRxInfo,
		       UINT32 BufCnt_least_winStartB, UINT32 winSizeB_Delta, rx_queue_t * rq, UINT32 * badPNcnt, UINT32 * passPNcnt, UINT8 tid)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	...
	index = ((winStartB + advanceDelta) & BA_MAX_SEQ_NUM) % MAX_BA_REORDER_BUF_SIZE;
	...
	if ((storedMpduNum - 1) == 0)	// Only one
	{
		//if(baRxInfo->AmsduQ[index].state != 0)      //Not needed, just dequeue and flush
		{
			...
			if (winDelta > winSizeB) {
				vmacSta_p->BA_RodrFlushDropCnt += (winSizeB - 1);
				...
				vmacSta_p->BA_RodrFlushDropCnt += (winDelta - 1);
	...
	if (advanceDelta < winDelta)	// there may be available buffers before New winStartB
	{
		...
		pStaInfo->rxBaStats[tid].BA_RodrFlushDropCnt += (advanceDelta - 1);
		while (scanNum--) {
			...
			if (baRxInfo->AmsduQ[index].state != 0) {
				...
				pStaInfo->rxBaStats[tid].BA_RodrLastDropReason = BA_RODR_DROP_FLUSH;
			}
			if (index == (MAX_BA_REORDER_BUF_SIZE - 1))
				index = 0;
			else
...
INLINE UINT8 BA_chkSnValid(UINT32 SN, UINT32 winStartB)
{
	if ((winStartB + 2048) > BA_MAX_SEQ_NUM)	// Round up
	{
		if ((SN >= ((winStartB + 2048) & BA_MAX_SEQ_NUM)) && (SN < winStartB))
			return 1;	// Drop
...
#endif
static struct sk_buff *ProcessEAPoL(struct sk_buff *skb, vmacApInfo_t * vmacSta_p, vmacEntry_t * vmacEntry_p)
{
	...
	if (skb->protocol & WL_WLAN_TYPE_WDS) {
		pStaInfo = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) & wlpptr->wlpd_p->mac_addr_sta_ta, STADB_DONT_UPDATE_AGINGTIME);
		if (pStaInfo && pStaInfo->Client) {
			struct net_device *dev_tmp = (struct net_device *)vmacEntry_p->privInfo_p;
			struct wlprivate *wlpptr_tmp = NETDEV_PRIV_P(struct wlprivate, dev_tmp);
			MIB_802DOT11 *mib_tmp = wlpptr_tmp->vmacSta_p->Mib802dot11;
			if (mib_tmp->multi_ap_attr & MAP_ATTRIBUTE_BACKHAUL_STA) {
				skb->dev = dev_tmp;
				wlpptr = wlpptr_tmp;
				mib = mib_tmp;
			}
...
*/
inline U8 *get_cfhul_template(struct sk_buff * skb, wlrxdesc_t * pCfhul)
{
	...
	if ((pCfhul->fpkt != 1) || ((frame_ctlp->Type != IEEE_TYPE_DATA) && (frame_ctlp->Type != IEEE_TYPE_MANAGEMENT))) {
	...
	if (skb->protocol & WL_WLAN_TYPE_RX_FAST_DATA)
		...
		cfhul_template = skb->data - SMAC_MGMT_EXTRA_BYTE + pCfhul->cfh_offset;

	mac_hdr = (IEEEtypes_fullHdr_t *) (cfhul_template + SMAC_MAC_HDR_OFFSET);
...
int ieee80211_input(struct net_device *dev, struct sk_buff *skb, u_int32_t rssi, RssiPathInfo_t * prssiPaths, wlrxdesc_t * pCfhul,
		    u_int8_t isFirstMsdu, u_int16_t stnid, u_int8_t LMFbit)
#else
int ieee80211_input(struct net_device *dev, struct sk_buff *skb, u_int32_t rssi, u_int32_t rssiPaths, u_int8_t ampdu_qos, u_int32_t status1,
		    u_int16_t stnid)
#endif				/* SOC_W906X */
{
	struct ieee80211_frame *wh;
...
#endif
	if ((frame_ctlp->Type == IEEE_TYPE_DATA) && (frame_ctlp->Retry == 1)) {
		wlpptr->cbinfo_bss.rx_dup_cnt++;
		// remove the re-transmite packets for CB function
		//wl_free_skb(skb);
		//return 0;
	}
	if ((cfhul_template != NULL) && (frame_ctlp->Order == 1)) {
		...
		mac_hdr = (IEEEtypes_fullHdr_t *) (cfhul_template + SMAC_MAC_HDR_OFFSET);
		if (!memcmp(&pCfhul->frame_ctrl, &mac_hdr->FrmCtl, sizeof(IEEEtypes_FrameCtl_t))) {
			if (mac_hdr->FrmCtl.ToDs && mac_hdr->FrmCtl.FromDs) {
				if ((frame_ctlp->Type == IEEE_TYPE_DATA) && (mac_hdr->FrmCtl.Subtype & BIT(3)))
					memcpy(&htc, &mac_hdr->wds_qos_htc.htc, sizeof(IEEEtypes_htcField_t));
				else
					memcpy(&htc, &mac_hdr->wds_htc.htc, sizeof(IEEEtypes_htcField_t));
			} else if ((frame_ctlp->Type == IEEE_TYPE_DATA) && (mac_hdr->FrmCtl.Subtype & BIT(3)))
			...
			if (shift_bits) {
				acontrol_om.om_control = (htc.he_variant.a_control >> shift_bits) & 0x1FF;
				if (wfa_11ax_pf)	/* Only print for WFA tests */
					printk
					    ("qos_pkt, acontrol_om.chbw %d acontrol_om.rxnss %d, acontrol_om.tx_nsts %d, acontrol_om.ulmu_disable %d\n",
					     acontrol_om.chbw, acontrol_om.rxnss, acontrol_om.tx_nsts, acontrol_om.ulmu_disable);
				if ((pStaInfo =
				     extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) & mac_hdr->Addr2,
							 STADB_SKIP_MATCH_VAP | STADB_NO_BLOCK)) != NULL) {
					...
					if (wfa_11ax_pf && (vap_wlpptr->vmacSta_p->ul_ofdma.period_tmr != 0)) {
						...
						if (pStaInfo->operating_mode.tx_nsts != acontrol_om.tx_nsts) {
							...
							wlFwSetMib(dev, HostCmd_ACT_GEN_SET, MIB_TF_NSS, &mib_val, &ul_usr_num);
							wlFwSetMib(dev, HostCmd_ACT_GEN_GET, MIB_TF_DATA_LEN, data_len, &ul_usr_num);
							data_len[0] =
							    data_len[0] * (acontrol_om.tx_nsts + 1) / (pStaInfo->operating_mode.tx_nsts + 1);
							ul_usr_num = 1;
							wlFwSetMib(dev, HostCmd_ACT_GEN_SET, MIB_TF_DATA_LEN, data_len, &ul_usr_num);
						}
						if (pStaInfo->operating_mode.ulmu_disable != acontrol_om.ulmu_disable) {
							if (acontrol_om.ulmu_disable == 1) {
								memcpy((void *)&tf_cfg, (void *)&vap_wlpptr->vmacSta_p->ul_ofdma,
								       sizeof(sched_cfg_ul_ofdma_t));
								tf_cfg.period_tmr = 0;
								wlFwSetSchedMode(pStaInfo->dev, HostCmd_ACT_GEN_SET, MODE_SELECT_UL_OFDMA, &tf_cfg,
										 sizeof(sched_cfg_ul_ofdma_t), &cmd_status);
							} else {
								wlFwSetSchedMode(pStaInfo->dev, HostCmd_ACT_GEN_SET, MODE_SELECT_UL_OFDMA,
										 (void *)&vap_wlpptr->vmacSta_p->ul_ofdma,
										 sizeof(sched_cfg_ul_ofdma_t), &cmd_status);
							}
						}
					...
					if ((pStaInfo->operating_mode.rxnss != acontrol_om.rxnss)
					    || (pStaInfo->operating_mode.chbw != acontrol_om.chbw)) {
					...
					if (wfa_11ax_pf && (vap_wlpptr->vmacSta_p->ul_ofdma.period_tmr != 0)) {
						...
						if (pStaInfo->operating_mode.chbw != acontrol_om.chbw) {
							...
							case 3:
								ul_usr_num = 1;
								mib_val = 68;
								wlFwSetMib(pStaInfo->dev, HostCmd_ACT_GEN_SET, MIB_TF_RU_ALLOC, &mib_val,
									   &ul_usr_num);
								break;
							case 2:
								ul_usr_num = 1;
	...
	if ((skb->protocol & WL_WLAN_TYPE_STA) && (prssiPaths != NULL)) {
		...
		memcpy(&vmacSta_p->RSSI_path, prssiPaths, sizeof(RssiPathInfo_t));
	}
#endif
#endif				/* SOC_W906X */
	...
	if (skb->protocol & WL_WLAN_TYPE_RX_FAST_DATA) {
		...
		if (pCfhul->fpkt == 1 && pCfhul->lpkt == 1) {
			memcpy(&skb->data[0], (u_int32_t *) & pCfhul->nss_hdr[0], 14);
		}
#ifdef MULTI_AP_SUPPORT
		if (skb->protocol & WL_WLAN_TYPE_WDS) {
#if 0
			if (cfhul_template != NULL) {
				mac_hdr = (IEEEtypes_fullHdr_t *) (cfhul_template + SMAC_MAC_HDR_OFFSET);
				pStaInfo_multiAP = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) & mac_hdr->Addr2, 0);
			}
#endif
			pStaInfo_multiAP = extStaDb_GetStaInfoStn(vmacSta_p, stnid);
			if (pStaInfo_multiAP && pStaInfo_multiAP->MultiAP_4addr) {
				...
				pStaInfo = pStaInfo_multiAP;
#ifdef MBSS
				if ((vmactem_p = vmacGetMBssByAddr(vmacSta_p, pStaInfo->Bssid)) != NULL) {
					vmacSta_p = vmactem_p;
					dev = vmacSta_p->dev;
					skb->dev = dev;
				}
		...
		WLDBG_INFO(DBG_LEVEL_9, "mib->mib_wdsEnable:%x, mib->mib_STAMode:%x, stnid == rxring_Ctrl_STAfromDS:%x\n",
			   *(mib->mib_wdsEnable), *(mib->mib_STAMode), (stnid == rxring_Ctrl_STAfromDS) ? 1 : 0);
		...
		if (*(mib->mib_wdsEnable)
			...
			if (*(mib->mib_wdsEnable)) {
				...
				vmacApInfo_t *vmactem_p;
				if (skb->protocol & WL_WLAN_TYPE_WDS)
					pStaInfo = extStaDb_GetStaInfoStn(vmacSta_p, stnid);
				else
					pStaInfo =
					    extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) & skb->data[6],
								STADB_SKIP_MATCH_VAP | STADB_NO_BLOCK);
				if (pStaInfo) {
					if ((vmactem_p = vmacGetMBssByAddr(vmacSta_p, pStaInfo->Bssid)) != NULL) {
						vmacSta_p = vmactem_p;
						dev = vmacSta_p->dev;
			...
			if ((pStaInfo = extStaDb_GetStaInfo(vmacSta_p,
							    (IEEEtypes_MacAddr_t *) & wlpptr->wlpd_p->mac_addr_sta_ta,
							    STADB_SKIP_MATCH_VAP | STADB_NO_BLOCK)) == NULL) {
				goto err;
			}
			vmactem_p = vmacGetMBssByAddr(vmacSta_p, pStaInfo->Bssid);
			if (vmactem_p)
				vmacSta_p = vmactem_p;
			dev = vmacSta_p->dev;
			skb->dev = dev;
		...
		if (!wlpptr->wlpd_p->fastdata_reordering_disable) {
			isfastdatareorder = 1;
			memset((UINT8 *) & wh_s, 0, sizeof(struct ieee80211_frame));
			wh_s.FrmCtl = *frame_ctlp;
			if (stnid == rxring_Ctrl_STAfromDS) {
				wh_s.FrmCtl.FromDs = 1;
				stationPacket = TRUE;
				if (*(mib->mib_STAMode))
					isBcastPkt = IS_GROUP((UINT8 *) & skb->data[0]);
			}
			IEEE80211_ADDR_COPY(&wh_s.addr1[0], &vmacSta_p->macStaAddr[0]);
			IEEE80211_ADDR_COPY(&wh_s.addr3[0], &skb->data[0]);
			IEEE80211_ADDR_COPY(&wh_s.addr2[0], &skb->data[6]);
#ifdef MULTI_AP_SUPPORT
			if (pStaInfo && pStaInfo->MultiAP_4addr && ((frame_ctlp->FromDs == 1) && (frame_ctlp->ToDs == 1))) {
				IEEE80211_ADDR_COPY(&wh_s.addr4[0], &skb->data[6]);
				IEEE80211_ADDR_COPY(&wh_s.addr2[0], pStaInfo->Addr);
				wh_s.FrmCtl.FromDs = 1;
				wh_s.FrmCtl.ToDs = 1;
				if (pStaInfo->Client) {
					stationPacket = TRUE;
					if (*(mib->mib_STAMode))
						isBcastPkt = IS_GROUP((UINT8 *) & wh_s.addr1[0]);
				}
				snap_aaaa = (u16 *) pCfhul->nss_hdr;
				if (snap_aaaa[7] == 0x0100) {
					if (skb->data[14] == 0x00 && skb->data[15] == 0x01 &&
					    skb->data[16] == 0xAF && skb->data[17] == 0x81 && skb->data[18] == 0x01) {
						if ((pStaInfo = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) wh_s.addr4,
										    STADB_SKIP_MATCH_VAP | STADB_NO_BLOCK))) {
							...
							if (memcmp(&vmacSta_p->macBssId, &pStaInfo->Bssid, sizeof(IEEEtypes_MacAddr_t))) {
								/* StaInfo_p and vmacSta_p current NOT point to the same vap */
								if ((vmacSta_temp = vmacGetMBssByAddr(vmacSta_p, pStaInfo->Bssid)) == NULL) {
									WLDBG_ENTER_INFO(DBG_LEVEL_11, "not found the VAP to which the STA point\n");
								}
							}

							if (vmacSta_temp)
								macMgmtRemoveSta(vmacSta_temp, pStaInfo);
							goto out;
						}
					}
				}
			...
			if (frame_ctlp->Wep) {
				if ((pCfhul->fpkt == 1) && cfhul_template) {
					pn = (UINT8 *) cfhul_template + SMAC_CFHUL_MAC_HDR_OFFSET + machdrLen;
				}
			}
#endif
		...
		if (frame_ctlp->Wep && (!wfa_11ax_pf)) {
			if ((pCfhul->fpkt == 1) && cfhul_template) {
				pn = (UINT8 *) cfhul_template + SMAC_CFHUL_MAC_HDR_OFFSET + machdrLen;
				if (data_pn_replay_detect(dev, skb, frame_ctlp, ampdu_qos, SeqNo, machdrLen, secMode, pn))
 ...
 fastdatareordercontinue:
		if (*(mib->mib_wdsEnable)
			...
			if (prssiPaths != NULL && pStaInfo != NULL) {	// valid rssiPath
				pStaInfo->RSSI = rssi;
				memcpy(&pStaInfo->RSSI_path, prssiPaths, sizeof(RssiPathInfo_t));
			}
#endif				/* SOC_W906X */
			skb = ieee80211_decap(dev, skb, pStaInfo, stnid);
			if (skb == NULL) {
	...
	case IEEE_TYPE_DATA:
...
#ifdef MBSS
		{
			...
			if (!MACADDR_CMP(wh->addr1, brdcastAddr)) {
				vmactem_p = vmacGetMBssByAddr(vmacSta_p, (IEEEtypes_MacAddr_t *) & vmacEntry_p->vmacAddr);
				if (vmactem_p)
					vmacSta_p = vmactem_p;
			} else
#endif
...
#ifdef MULTI_AP_SUPPORT
			pStaInfo = extStaDb_GetStaInfo(vmacSta_p, &(wh->addr2), STADB_UPDATE_AGINGTIME);
			if (!pStaInfo) {
...
#endif
				if (!*(mib->mib_wdsEnable) ||
				    ((pStaInfo = extStaDb_GetStaInfo(vmacSta_p, &(wh->addr2), STADB_UPDATE_AGINGTIME)) == NULL)) {
 ...
 fastdatareorder:
		...
		if (!stationPacket) {
#endif
			if (*(mib->mib_wdsEnable) && (skb->protocol & WL_WLAN_TYPE_WDS))
				// WDS enable and SA adress is bridge Mac address, the bridge Mac address is different with wdevX Mac address
				pStaInfo = extStaDb_GetStaInfoStn(vmacSta_p, stnid);
			else
				pStaInfo =
				    extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) & wlpptr->wlpd_p->mac_addr_sta_ta /*&(wh->addr2) */ ,
			...
			if ((pStaInfo =
			     extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) GetParentStaBSSID((vmacEntry_p)->phyHwMacIndx),
						 STADB_UPDATE_AGINGTIME | STADB_NO_BLOCK)) == NULL)
		...
		*****************************************************************************************************************/
		{
			...
			if ((tid < MAX_TID) && (baRxInfo->AddBaReceive[tid] == TRUE)) {
				SPIN_LOCK(&baRxInfo->ba[tid].BAreodrLock);
				//Unicast and QoS pkt
				if (!isBcastPkt && ((wh->FrmCtl.Subtype == QoS_DATA) ||
						    (wh->FrmCtl.Subtype == QoS_DATA_CF_ACK) ||
						    (wh->FrmCtl.Subtype == QoS_DATA_CF_POLL) || (wh->FrmCtl.Subtype == QoS_DATA_CF_ACK_CF_POLL))) {
					...
					//Start of PPDU
					if (pCfhul->hdr.startPPDU) {
		     /*************************************************************************************************************************
                                    *** SFW: overWExtCnt reset at EOF if no sn>wExt evt in that current rx PPDU
                                    *** DRV: overWExtCnt reset at next rx PPDU or after > threshold
                                    ***
                                    *** Two places overWExtCnt is reset:
                                    *** 1)PPDU_0[200,3001],overWExtCnt=1 |   PPDU_1[3001], overWExtCnt=2   |  PPDU_2[3001:move wStart & reset overWExtCnt], overWExtCnt=0 
                                    *** 2)PPDU_0[200,3001],overWExtCnt=1 |   PPDU_1[201], overWExtCnt=1     |  PPDU_2[4001: reset overWExtCnt], overWExtCnt=1 
                                    **************************************************************************************************************************/
						if (baRxInfo->ba[tid].overWExtNotFound)	//previous PPDU has no sn>wExt evt
						{
							baRxInfo->ba[tid].overWExtCnt = 0;
						}

						baRxInfo->ba[tid].overWExtNotFound = pCfhul->hdr.startPPDU;
					}
					...
					if (winStartB == SeqNo) {
...
#ifdef DEBUG_BAREORDER
						BA_DebugLog(tid, winStartB, SeqNo, (UINT32) (cardindex << 16 | id), DBG_BAREORDER_TAG_SN, 0);
#endif

						//Check duplicate and store skb to AmsduQ if needed
						if (BA_CheckAmsduDup(&baRxInfo->ba[tid], bufIndex, skb, LMFbit, &pStaInfo->pn->ucRxQueues[tid],
								     secMode, SeqNo, pn)) {
							vmacSta_p->BA_RodrDupDropCnt++;
							pStaInfo->rxBaStats[tid].BA_RodrDupDropCnt++;
							pStaInfo->rxBaStats[tid].BA_RodrLastDropReason = BA_RODR_DROP_DUP;
						...
						if (baRxInfo->ba[tid].AmsduQ[bufIndex].state == 2) {
							baRxInfo->ba[tid].winStartB = BA_flushSequencialData(dev, pStaInfo, &baRxInfo->ba[tid],
													     ((winStartB << 16) | storedBufCnt),
													     &pStaInfo->pn->ucRxQueues[tid],
													     &wlexcept_p->badPNcntUcast,
													     &wlpptr->wlpd_p->drv_stats_val.
													     rx_data_ucast_pn_pass_cnt, tid);
							storedBufCnt = baRxInfo->ba[tid].storedBufCnt;

							BA_TimerActivateCheck(dev, pStaInfo, baRxInfo, tid, SeqNo, 0);

							DBG_BAREORDER_SN(((cardindex << 16) | id), tid, 1, baRxInfo->ba[tid].winStartB, SeqNo)
						} else {
							if (storedBufCnt == 1) {
								baRxInfo->ba[tid].leastSeqNo = SeqNo;
							}
					...
					} else	//(winStartB != SeqNo)
					{
						...
						winSizeB_minus1 = baRxInfo->ba[tid].winSizeB - 1;
						wEnd = (winStartB + winSizeB_minus1) & BA_MAX_SEQ_NUM;
						endExt = (winStartB + 2048) & BA_MAX_SEQ_NUM;	// winStartB+2^11

						seqDelta = BA_getSeqDelta(((wEnd << 16) | endExt), winStartB, SeqNo, winSizeB_minus1);
						...
						if (seqDelta == 0) {
							...
							if (wfa_11ax_pf) {
								...
								*(volatile unsigned int *)(wlpptr->ioBase0 + dmemaddr) = le32_to_cpu(val + 1);
							}

							pStaInfo->rxBaStats[tid].BA_RodrLastDropReason = BA_RODR_DROP_OOR;

							wEnd = (winStartB - 1) & BA_MAX_SEQ_NUM;
							endExt = (wEnd - (baRxInfo->ba[tid].winSizeB << 1)) & BA_MAX_SEQ_NUM;
							val = 0;	//0: follow through, 1: retry (drop), 2: out of range (drop)
							if (wEnd > endExt) {
								if ((SeqNo > endExt) && (SeqNo <= wEnd))
									val = 1;
							} else {
								if ((SeqNo <= wEnd) || (SeqNo > endExt))
									val = 1;
							}
							if (val)	//retry
							{
								vmacSta_p->BA_RodrRetryDropCnt++;
								pStaInfo->rxBaStats[tid].BA_RodrRetryDropCnt++;
							} else {	//Out of range
								val = 2;
								...
								if (LMFbit & 0x1)	//only increase counter for 1st MSDU
								{
									if (baRxInfo->ba[tid].overWExtNotFound) {
										baRxInfo->ba[tid].overWExtNotFound = 0;	//set 0 to indicate there is sn>wExt event in this PPDU
										baRxInfo->ba[tid].overWExtCnt++;
										if (baRxInfo->ba[tid].overWExtCnt > RX_BA_SN_OVER_2K_CNT_THRES) {
											val = 0;
											baRxInfo->ba[tid].overWExtCnt = 0;
											seqDelta = (SeqNo - winStartB) & BA_MAX_SEQ_NUM;	//follow up seqno jump
										}
									}
								}
#ifdef DEBUG_BAREORDER
								BA_DebugLog(tid, winStartB, SeqNo, (UINT32) (cardindex << 16 | id),
									    DBG_BAREORDER_TAG_OOR, 0);
#endif
							}
							...
							if (wfa_11ax_pf) {
								...
								ba_debug_buf[temp_index].winStartB = winStartB;
								ba_debug_buf[temp_index].SeqNo = SeqNo;
								ba_debug_buf[temp_index].StnId = pStaInfo->StnId;
								IEEE80211_ADDR_COPY(ba_debug_buf[temp_index].Addr, pStaInfo->Addr);
								ba_debug_buf[temp_index].lo_dword_addr = pCfhul->hdr.lo_dword_addr;
...
#ifdef DEBUG_BAREORDER
						BA_DebugLog(tid, winStartB, SeqNo, (UINT32) (cardindex << 16 | id), DBG_BAREORDER_TAG_STORE, 0);
#endif
...
#ifdef DEBUG_BAREORDER
							BA_DebugLog(tid, winStartB, SeqNo, (UINT32) (cardindex << 16 | id), DBG_BAREORDER_TAG_WEND,
								    0);
#endif
							if (storedBufCnt != 0) {
								temp =
								    ((storedBufCnt & 0xFF) << 24) | ((leastSeqNo & 0xFFF) << 12) | (winStartB &
																    0xFFF);
								baRxInfo->ba[tid].winStartB =
								    BA_flushAnyData(dev, pStaInfo, &baRxInfo->ba[tid], temp,
										    (((baRxInfo->ba[tid].winSizeB) << 16) | winDelta),
								...
								if (baRxInfo->ba[tid].winStartB == SeqNo) {
									bufIndex = SeqNo % MAX_BA_REORDER_BUF_SIZE;

									//Check duplicate and store skb to AmsduQ if needed
									if (BA_CheckAmsduDup
									    (&baRxInfo->ba[tid], bufIndex, skb, LMFbit,
									     &pStaInfo->pn->ucRxQueues[tid], secMode, SeqNo, pn)) {
										vmacSta_p->BA_RodrDupDropCnt++;
										pStaInfo->rxBaStats[tid].BA_RodrDupDropCnt++;
										pStaInfo->rxBaStats[tid].BA_RodrLastDropReason = BA_RODR_DROP_DUP;

										DBG_BAREORDER_SN(((cardindex << 16) | id), tid, 3,
												 baRxInfo->ba[tid].winStartB, SeqNo)
										    SPIN_UNLOCK(&baRxInfo->ba[tid].BAreodrLock);
										goto err;	// Drop packet due to duplicate   
									}
									storedBufCnt = baRxInfo->ba[tid].storedBufCnt;
									...
									if (baRxInfo->ba[tid].AmsduQ[bufIndex].state == 2) {
										baRxInfo->ba[tid].winStartB =
										    BA_flushSequencialData(dev, pStaInfo, &baRxInfo->ba[tid],
													   ((baRxInfo->ba[tid].
													     winStartB << 16) | storedBufCnt),
													   &pStaInfo->pn->ucRxQueues[tid],
													   &wlexcept_p->badPNcntUcast,
									...
									cnt = 1;
									DBG_BAREORDER_SN(((cardindex << 16) | id), tid, 4,
											 baRxInfo->ba[tid].winStartB, SeqNo)
									    SPIN_UNLOCK(&baRxInfo->ba[tid].BAreodrLock);
									skb = NULL;
									goto err;
								} else {
									DBG_BAREORDER_SN(((cardindex << 16) | id), tid, 5,
											 baRxInfo->ba[tid].winStartB, SeqNo)
								}
							} else {
								baRxInfo->ba[tid].winStartB =
								    (baRxInfo->ba[tid].winStartB + winDelta) & BA_MAX_SEQ_NUM;

								vmacSta_p->BA_RodrFlushDropCnt += baRxInfo->ba[tid].winSizeB;
								pStaInfo->rxBaStats[tid].BA_RodrFlushDropCnt += baRxInfo->ba[tid].winSizeB;

								vmacSta_p->BA_RodrWinEndJumpCnt++;
								pStaInfo->rxBaStats[tid].BA_RodrWinEndJumpCnt++;

								DBG_BAREORDER_SN(((cardindex << 16) | id), tid, 6, baRxInfo->ba[tid].winStartB, SeqNo)
							}
						}
						...
						if (storedBufCnt == 0) {
							...
							pStaInfo->rxBaStats[tid].BA_RodrLastDropReason = BA_RODR_DROP_AMSDUENQ;
						...
						} else if (storedBufCnt == 1)	//only 1 amsdu is stored
						{
							...
							if ((UINT32) (baRxInfo->ba[tid].leastSeqNo - SeqNo) < baRxInfo->ba[tid].winSizeB) {
								baRxInfo->ba[tid].leastSeqNo = SeqNo;
							}
						}
						DBG_BAREORDER_SN(((cardindex << 16) | id), tid, 9, baRxInfo->ba[tid].winStartB, SeqNo)

						    BA_TimerActivateCheck(dev, pStaInfo, baRxInfo, tid, SeqNo, 1);
					...
					if (!(wh->addr1[0] & 0x01)) {
						...
						if ((wh->FrmCtl.Subtype == QoS_DATA) ||
							...
							printk("** 11 blk ack pck received , SeqNo = %d ExpectedSqNo = %d ampdu_qos = %x\n", SeqNo,
							       wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].CurrentSeqNo[tid], ampdu_qos);
#endif
							...
							if (wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].ReOrdering[tid] == FALSE) {			/** previous pck is in order **/
								...
								*****************************************************************************************************************/
								if (SeqNo == wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].CurrentSeqNo[tid]) {	/** found the right one **/
									...
									if (jiffies - wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].Time[tid] >
									...
									if (wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].CurrentSeqNo[tid] < SeqNo) {	/** Valid case **/
										if (SeqNo -
											...
											wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].
										...
										/** valid condition **/
										{
											...
											if (wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].
											    pFrame[tid][tempvalue] != NULL) {
												DEBUG_REORDER();
												wl_free_skb(skb);
											} else
												wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].
												    pFrame[tid][tempvalue] = skb;
										}
										//AmpduPckReorder[pStaInfo->Aid].pFrame[Priority][SeqNo-AmpduPckReorder[pStaInfo->Aid].CurrentSeqNo[Priority]]= skb;
										wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].
										    ExpectedSeqNo[tid][SeqNo -
												       wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].
												       CurrentSeqNo[tid]] = SeqNo;
										{
											struct reorder_t *ro_p =
											    wl_kmalloc_autogfp(sizeof(struct reorder_t));
											ro_p->dev = dev;
#ifdef SOC_W906X
											ro_p->pStaInfo = pStaInfo;
#else
											...
											ro_p->tid = tid;
											ro_p->SeqNo = SeqNo;
											TimerFireInByJiffies(&wlpptr->wlpd_p->
													     AmpduPckReorder[pStaInfo->Aid].
													     timer[tid], 1, &ReorderingTimerProcess,
													     (UINT8 *) ro_p,
													     *(mib->mib_BAReorder_holdtime));
										}
										skb = NULL;
										...
										if (tempvalue < MAX_AMPDU_REORDER_BUFFER) {
											if (wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].
												...
												wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].
												    pFrame[tid][tempvalue] = skb;
											wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].
											    ExpectedSeqNo[tid][tempvalue] = SeqNo;

										} else {
											/** treat it as most likely overflow condition **/
											/** too many pck is missing, time to reset the cnt **/
#ifdef SOC_W906X
											Ampdu_Flush_All_Pck_in_Reorder_queue(dev, pStaInfo, tid);
#else
											...
											wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].
											    ReOrdering[tid] = FALSE;
											wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].CurrentSeqNo[tid] = (SeqNo + 1) % MAX_AC_SEQNO;	/**  assuming next pck **/
											TimerDisarmByJiffies(&wlpptr->wlpd_p->
													     AmpduPckReorder[pStaInfo->Aid].
													     timer[tid], 1);
											goto blkackoutcontinue;
										}
								...
								if (SeqNo == wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].CurrentSeqNo[tid]) {
									if (wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].pFrame[tid][0] != NULL) {
										wl_free_skb(wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].
											    pFrame[tid][0]);
										DEBUG_REORDER();
									}
									...
									for (i = 0; i < MAX_AMPDU_REORDER_BUFFER; i++) {
										...
										if (flush_blockack_pck(dev, i, pStaInfo->Aid, tid) == 0) {
											...
											wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].
											    CurrentSeqNo[tid] =
											    (wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].
											     CurrentSeqNo[tid] + i) % MAX_AC_SEQNO;
									...
									if (Ampdu_Check_Valid_Pck_in_Reorder_queue(dev, pStaInfo->Aid, tid) == FALSE) {
										wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].ReOrdering[tid] =
										    FALSE;
										TimerDisarmByJiffies(&wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].
												     timer[tid], 1);
									}
									goto err;
									...
									if (wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].CurrentSeqNo[tid] < SeqNo) {	/** Valid case **/
										...
										if (SeqNo -
										    wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].
										    CurrentSeqNo[tid] >= MAX_AMPDU_REORDER_BUFFER) {
#ifdef SOC_W906X
											Ampdu_Flush_All_Pck_in_Reorder_queue(dev, pStaInfo, tid);
#else
											Ampdu_Flush_All_Pck_in_Reorder_queue(dev, pStaInfo->Aid, tid);
 ...
 blkackoutcontinue:
...
#endif
			{
				...
				skb = ieee80211_decap(vmacSta_p->dev, skb, pStaInfo, 0);
				if (pStaInfo) {
					pStaInfo->rx_packets++;
	...
	case IEEE_TYPE_MANAGEMENT:
		...
		if (frame_ctlp->Wep && cfhul_template && (!wfa_11ax_pf)) {
			...
			if (mgmt_pn_replay_detect(dev, skb, wh, SeqNo, machdrLen, secMode, pn))
				goto out;	//drop packet
		}
#endif
#ifdef AUTOCHANNEL
		if (*(vmacSta_p->Mib802dot11->mib_autochannel) && ((!vmacSta_p->preautochannelfinished) || vmacSta_p->acs_cur_bcn)) {
			struct IEEEtypes_Frame_t *wlanMsg_p = (IEEEtypes_Frame_t *) ((UINT8 *) skb->data - 2);

			wlanMsg_p->Hdr.FrmBodyLen = skb->len;
			if (wh->FrmCtl.Type == IEEE_TYPE_MANAGEMENT && wlanMsg_p->Hdr.FrmCtl.Subtype == IEEE_MSG_BEACON) {
				MSAN_neighbor_bcnproc(dev, wlanMsg_p, skb->len, prssiPaths, SCAN_BY_ACS);
			}
		}
		...
		if (!IS_GROUP((UINT8 *) & (wh->addr1))) {
			...
			if (wh->FrmCtl.Subtype == IEEE_MSG_QOS_ACTION) {
				...
				if (vmactem_p) {
					...
					pStaInfo = extStaDb_GetStaInfo(vmactem_p, &(wh->addr2), STADB_DONT_UPDATE_AGINGTIME);
				} else {
					...
					if (vmacwds_p)
						pStaInfo = extStaDb_GetStaInfo(vmacwds_p, &(wh->addr2), STADB_DONT_UPDATE_AGINGTIME);
					else {
						/*If vmac is not found, we just use parent */
						pStaInfo = extStaDb_GetStaInfo(vmacSta_p, &(wh->addr2), STADB_DONT_UPDATE_AGINGTIME);
					}
				}
...
#ifdef DUPLICATED_MGMT_DBG
			if (stationPacket)
				pStaInfo = extStaDb_GetStaInfo(vmacSta_p, &(wh->addr1), STADB_SKIP_MATCH_VAP);

			if (pStaInfo) {
				wlanMsg_p = (IEEEtypes_Frame_t *) ((UINT8 *) skb->data - 2);
				if (pStaInfo->pre_mgmt_seq == wlanMsg_p->Hdr.SeqCtl) {
					if (wlanMsg_p->Hdr.FrmCtl.Retry) {
						wlpptr->rx_retry_mgmt_cnt++;
						WLDBG_ERROR(DBG_LEVEL_0, "RX %d duplicated subtype: %x MGMT packets\n", wlpptr->rx_retry_mgmt_cnt,
							    wh->FrmCtl.Subtype);
					}
				} else if (pStaInfo->pre_mgmt_seq < wlanMsg_p->Hdr.SeqCtl) {
					pStaInfo->pre_mgmt_seq = wlanMsg_p->Hdr.SeqCtl;
				}
			}
#endif
		...
		if (dev != vmacSta_p->dev) {
			 ...
			 */
			((NETDEV_PRIV_P(struct wlprivate, vmacSta_p->dev)))->netDevStats.rx_packets++;
			((NETDEV_PRIV_P(struct wlprivate, vmacSta_p->dev)))->netDevStats.rx_bytes += skb->len;
		} else {
			wlpptr->netDevStats.rx_packets++;
			wlpptr->netDevStats.rx_bytes += skb->len;
		}
		...
		//convert mbssid beacon to legacy beacon
		{
			...
			if (num) {
				while ((skb2 = skb_dequeue(&skbList))) {
					receiveWlanMsg(vmacSta_p->dev, skb2, rssi, stationPacket);
				}
			}
		}
#else
	...
	case IEEE_TYPE_CONTROL:
		...
		/** Assume this is a BAR now, since this is the only one we pass up from the firmware **/
			...
			if (BaReqFrm->FrmCtl.Subtype != 8) {
							/** it is not a bar frame **/
				goto err;		/** free the pck **/
			}

			pStaInfo = extStaDb_GetStaInfo(vmacSta_p, &(BaReqFrm->SrcAddr), STADB_SKIP_MATCH_VAP | STADB_NO_BLOCK);
			if (pStaInfo) {
		...
		/** If a blockack req is received, all complete MSDUs with lower sequence numbers than the starting sequence number contained in the
		   ...
		   recipient shall indicate the MSDUs starting with the Starting Sequence number sequentially until there is an incomplete MSDU in the buffer **/
			...
			BaReqFrm->Seq_Ctrl.u16_data = le16_to_cpu(BaReqFrm->Seq_Ctrl.u16_data);
#endif
				...
				id = pStaInfo->StnId;
				if ((SeqNo != winStartB) && (BA_chkSnValid(SeqNo, winStartB) == 0)) {
					UINT32 cardindex;
					if (wlpptr->master) {
						cardindex = ((NETDEV_PRIV_P(struct wlprivate, wlpptr->master)))->cardindex;
					} else {
...
#ifdef DEBUG_BAREORDER
						BA_DebugLog(tid, winStartB, SeqNo, (UINT32) (cardindex << 16 | id), DBG_BAREORDER_TAG_BAR, 0);

#endif

						if (winStartB > SeqNo)	// rollover
						{
							winDelta = (BA_MAX_SEQ_NUM + 1) - winStartB + SeqNo;
						} else {
							winDelta = SeqNo - winStartB;
						}
						// Any complete Data Flush from winStartB to SSN(winDelta)                
						temp =
						    ((baRxInfo->ba[Priority].storedBufCnt & 0xFF) << 24) | ((baRxInfo->ba[Priority].
													     leastSeqNo & 0xFFF) << 12) | (baRxInfo->
																	   ba
																	   [Priority].
																	   winStartB &
																	   0xFFF);
						baRxInfo->ba[Priority].winStartB =
						    BA_flushAnyData(dev, pStaInfo, &baRxInfo->ba[Priority], temp,
								    (((baRxInfo->ba[Priority].winSizeB) << 16) | winDelta),
								    &pStaInfo->pn->ucRxQueues[Priority], &wlexcept_p->badPNcntUcast,
								    &wlpptr->wlpd_p->drv_stats_val.rx_data_ucast_pn_pass_cnt, tid);
						storedBufCnt = baRxInfo->ba[Priority].storedBufCnt;

						BA_TimerActivateCheck(dev, pStaInfo, baRxInfo, Priority, SeqNo, 0);

						DBG_BAREORDER_SN(((cardindex << 16) | id), Priority, 10, baRxInfo->ba[Priority].winStartB, SeqNo)
					} else {
						baRxInfo->ba[Priority].winStartB = SeqNo;

						DBG_BAREORDER_SN(((cardindex << 16) | id), Priority, 11, baRxInfo->ba[Priority].winStartB, SeqNo)
					}
				}
			...
			/** Make sure it is not a rollover condition **/
				...
				if (BaReqFrm->Seq_Ctrl.StartSeqNo - wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].CurrentSeqNo[Priority] <
				...
				/** Need to start flushing from here till the next hole **/
					...
					for (i = 0;
					     i <
					     BaReqFrm->Seq_Ctrl.StartSeqNo - wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].CurrentSeqNo[Priority];
					     i++) {
					...
					/** flush all subsequent pck until the next hole **/
#ifdef SOC_W906X
						if (flush_blockack_pck(dev, i, pStaInfo, Priority) == 0) {
#else
				...
				/** check for error condition here **/
					...
					tempvalue =
					    MAX_AC_SEQNO - BaReqFrm->Seq_Ctrl.StartSeqNo +
					    wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].CurrentSeqNo[Priority];
			...
			//
				...
				if (wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].CurrentSeqNo[Priority] - BaReqFrm->Seq_Ctrl.StartSeqNo <
					...
					tempvalue =
					    MAX_AC_SEQNO - wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].CurrentSeqNo[Priority] +
					...
					if (tempvalue < MAX_AMPDU_REORDER_BUFFER) {
						...
						for (i = 0;
						     i <
						     (MAX_AC_SEQNO - wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].CurrentSeqNo[Priority] +
						      BaReqFrm->Seq_Ctrl.StartSeqNo); i++) {
						/** Need to continue flushing until next pck **/
#ifdef SOC_W906X
							flush_blockack_pck(dev, i, pStaInfo, Priority);
												     /** there is a hole in the pck **/
						...
						wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].CurrentSeqNo[Priority] = BaReqFrm->Seq_Ctrl.StartSeqNo;
						wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].ExpectedSeqNo[Priority][0] =
						    BaReqFrm->Seq_Ctrl.StartSeqNo;
						wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].CurrentSeqNo[Priority] = BaReqFrm->Seq_Ctrl.StartSeqNo;
						wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].ExpectedSeqNo[Priority][0] =
						    BaReqFrm->Seq_Ctrl.StartSeqNo;
...
struct sk_buff *ieee80211_getmgtframe_undefine_len(UINT8 ** frm, unsigned int pktlen)
{
	const unsigned int align = sizeof(u_int32_t);
	struct sk_buff *skb;
	unsigned int len;
...
void sendLlcExchangeID(struct net_device *dev, IEEEtypes_MacAddr_t * src)
{
	...
	if (skb) {
		...
		*(bufptr++) = 0x03;	/* Ieee802.3 length */
		*(bufptr++) = 0x81;	/* LLC data */
		*(bufptr++) = 0x01;	/* LLC data */
		*(bufptr++) = 0x00;	/* LLC data */
		memset(bufptr, 0, 20 - (bufptr - skb->data));	/* pad */
		skb->dev = dev;
		skb->protocol = eth_type_trans(skb, dev);
		skb->cb[16] = 0x00;	/* Null DSAP */
		skb->cb[17] = 0x01;	/* Null SSAP */
		skb->cb[18] = 0xaf;	/* exchange ID */
				...
				if (memcmp((char *)&(mib->mib_IPMcastGrpTbl[i]->mib_UCastAddr[j]), addr, 6) == 0) {
					...
					if (mib->mib_IPMcastGrpTbl[i]->mib_MAddrCount == 0) {
						...
						memset((char *)&mib->mib_IPMcastGrpTbl[i]->mib_UCastAddr[mib->mib_IPMcastGrpTbl[i]->mib_MAddrCount],
						       0, 6);
						/* Update shadow MIB */
						memset((char *)&mibShadow->mib_IPMcastGrpTbl[i]->
						       mib_UCastAddr[mibShadow->mib_IPMcastGrpTbl[i]->mib_MAddrCount], 0, 6);
...
int ForwardFrame(struct net_device *dev, struct sk_buff *skb)
{
	...
	if (!(skb->protocol & WL_WLAN_TYPE_STA)) {
		...
		if (IS_GROUP((UINT8 *) & (eh->ether_dhost))) {
			...
			if (is_broadcast_ether_addr((UINT8 *) & (eh->ether_dhost)))
				wlpptr->wlpd_p->privNdevStats.rx_bcast_bytes += skb->len;
			else
				wlpptr->wlpd_p->privNdevStats.rx_mcast_bytes += skb->len;
			/* Intrabss packets get txed AP mode only */
			if (*(mib->mib_intraBSS) && (vmacSta_p->VMacEntry.modeOfService != VMAC_MODE_CLNT_INFRA)) {
				newskb = wl_alloc_skb(skb->len + NUM_EXTRA_RX_BYTES);
				if (newskb) {
	...
	pStaInfo = extStaDb_GetStaInfo(vmacSta_p, &(eh->ether_dhost), STADB_DONT_UPDATE_AGINGTIME);
...
#ifdef MULTI_AP_SUPPORT
	if ((!pStaInfo) && (!IS_GROUP((UINT8 *) & (eh->ether_dhost))) &&
	    (mib->multi_ap_attr & MAP_ATTRIBUTE_BACKHAUL_BSS) && (skb->protocol & WL_WLAN_TYPE_WDS)) {
		UINT8 bssId[6];
		UINT8 found = 0;
		MultiAP_4Addr_Entry_t *entry;
		...
		if (found == 1) {
			MACADDR_CPY(bssId, entry->tar);
			pStaInfo = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) & bssId[0], STADB_DONT_UPDATE_AGINGTIME);
		}
	}
#endif				/* MULTI_AP_SUPPORT */
...
#endif				/*CLIENT_SUPPORT */
	{
		if (pStaInfo->State == ASSOCIATED) {
			...
			if (*(mib->mib_intraBSS) && (vmacSta_p->VMacEntry.modeOfService != VMAC_MODE_CLNT_INFRA)) {
				...
				skb->priority = (skb->priority & 0xfffffff8) | 0x5;
	...
	if (!(skb->protocol & WL_WLAN_TYPE_STA)) {
		...
		WLDBG_INFO(DBG_LEVEL_9, "1B.skb->head,data,tail,end,len:%p %p %p %p %d\n", skb->head, skb->data, skb->tail, skb->end, skb->len);

		skb->protocol = eth_type_trans(skb, skb->dev);

		WLDBG_INFO(DBG_LEVEL_9, "1A.skb->head,data,tail,end,len:%p %p %p %p %d\n", skb->head, skb->data, skb->tail, skb->end, skb->len);
#endif
#ifdef MPRXY_SNOOP
		if (*(mib->mib_MCastPrxy)) {
			...
			if ((igmpheader->type != IGMP_HOST_MEMBERSHIP_REPORT) &&
			    ...
			    (igmpheader->type != IGMPV3_HOST_MEMBERSHIP_REPORT) && (igmpheader->type != IGMP_HOST_LEAVE_MESSAGE))
				goto mprxycontinue;
			...
			case IGMPV3_HOST_MEMBERSHIP_REPORT:
				...
				WLDBG_INFO(DBG_LEVEL_9, "IGMP Report:%u.%u.%u.%u", IPQUAD(igmp_addr_host));
				WLDBG_INFO(DBG_LEVEL_9, "   %02x%02x%02x%02x%02x%02x\n",
					...
					if (mib->mib_IPMcastGrpTbl[i]->mib_McastIPAddr == igmp_addr_host) {
						...
						if (mib->mib_IPMcastGrpTbl[i]->mib_MAddrCount < MAX_UCAST_MAC_IN_GRP) {
							...
							if (UcMACEntryExists == FALSE) {
								...
								memcpy((char *)&mibShadow->mib_IPMcastGrpTbl[i]->
								       mib_UCastAddr[mibShadow->mib_IPMcastGrpTbl[i]->mib_MAddrCount],
								       (char *)&eh->ether_shost, 6);

								/* update the timestamp corresponding to the unicast entry */
								mib->mib_IPMcastGrpTbl[i]->mib_UcEntryTS[mib->mib_IPMcastGrpTbl[i]->mib_MAddrCount] =
								    jiffies;
								...
								if (!IGMPQueryEntry) {
									...
									memcpy((char *)
									       &(mib->mib_IPMcastGrpTbl[0]->
										 mib_UCastAddr[mib->mib_IPMcastGrpTbl[0]->mib_MAddrCount]),
									       (char *)&eh->ether_shost, 6);
									/* Update shadow MIB */
									memcpy((char *)
									       &(mibShadow->mib_IPMcastGrpTbl[0]->
										 mib_UCastAddr[mibShadow->mib_IPMcastGrpTbl[0]->mib_MAddrCount]),
									       (char *)&eh->ether_shost, 6);
				...
				if (IPMcEntryExists == FALSE) {
					...
					if (*(mib->mib_IPMcastGrpCount) < MAX_IP_MCAST_GRPS) {
						...
						memcpy((char *)
							 ...
							 mib_UCastAddr[mibShadow->mib_IPMcastGrpTbl[i]->mib_MAddrCount]), (char *)&eh->ether_shost,
						       6);
						...
						*(mibShadow->mib_IPMcastGrpCount) = *(mibShadow->mib_IPMcastGrpCount) + 1;
						...
						if (!IGMPQueryEntry) {
							/* Add the unicast entry in the IPM Grp table [0] */
							memcpy((char *)
							       &(mib->mib_IPMcastGrpTbl[0]->mib_UCastAddr[mib->mib_IPMcastGrpTbl[0]->mib_MAddrCount]),
							       (char *)&eh->ether_shost, 6);
			...
			case IGMP_HOST_LEAVE_MESSAGE:
				WLDBG_INFO(DBG_LEVEL_9, "IGMP Leave:%u.%u.%u.%u", IPQUAD(igmp_addr_host));
				WLDBG_INFO(DBG_LEVEL_9, "   %02x%02x%02x%02x%02x%02x\n",
					   eh->ether_shost[0], eh->ether_shost[1], eh->ether_shost[2],
					   eh->ether_shost[3], eh->ether_shost[4], eh->ether_shost[5]);
									...
									if (mib->mib_IPMcastGrpTbl[i]->mib_MAddrCount == 0) {
										...
										if (i > 0) {
											...
											*(mibShadow->mib_IPMcastGrpCount) =
											    *(mibShadow->mib_IPMcastGrpCount) - 1;
											...
											if (*(mib->mib_IPMcastGrpCount) > 1) {
												...
												memcpy((char *)mib->mib_IPMcastGrpTbl[i],
												       ...
												       (*(mib->mib_IPMcastGrpCount) - i));
												/* Update shadow MIB */
												memcpy((char *)mibShadow->mib_IPMcastGrpTbl[i],
												       (char *)mibShadow->mib_IPMcastGrpTbl[i + 1],
												       sizeof(MIB_IPMCAST_GRP_TBL) *
												       (*(mibShadow->mib_IPMcastGrpCount) - i));

												/* clear out the last instance of the IPM Grp table */
												memset((char *)mib->
												       mib_IPMcastGrpTbl[*(mib->mib_IPMcastGrpCount)],
												       0, sizeof(MIB_IPMCAST_GRP_TBL));
												/* update shadow MIB */
												memset((char *)mibShadow->
												       mib_IPMcastGrpTbl[*
															 (mibShadow->
															  mib_IPMcastGrpCount)], 0,
												       sizeof(MIB_IPMCAST_GRP_TBL));
		...
		if (!*(mib->mib_STAMacCloneEnable)) {
			...
			for_each_netdev_rcu(&init_net, dev) {
				if (dev->priv_flags & IFF_EBRIDGE && memcmp(eh->ether_shost, dev->dev_addr, 6) == 0) {
		...
		skb->dev = staDev;
#ifndef MV_NSS_SUPPORT
		WLDBG_INFO(DBG_LEVEL_9, "2B.skb->head,data,tail,end,len:%p %p %p %p %d\n", skb->head, skb->data, skb->tail, skb->end, skb->len);

		skb->protocol = eth_type_trans(skb, staDev);

		WLDBG_INFO(DBG_LEVEL_9, "2A.skb->head,data,tail,end,len:%p %p %p %p %d\n", skb->head, skb->data, skb->tail, skb->end, skb->len);
#endif
	}
#endif				/* CLIENT_SUPPORT */
#ifdef MPRXY_SNOOP
		...
		if (pRxSkBuff) {
			...
			if (skb_tailroom(pRxSkBuff) >= (length + headerlength)) {
				...
				skb_put(pRxSkBuff, length + headerlength);
				{
					...
					pRxSkBuff = ieee80211_decap(netdev, pRxSkBuff, pStaInfo, 0);
...
#ifdef ENABLE_MONIF
#ifdef NEW_DP
int monif_handle_recv(struct wlprivate *wlpptr, rx_info_aux_t * prxinfo_aux, struct sk_buff *skb, int promisc_data)
#else
int monif_handle_recv(struct wlprivate *wlpptr, wlrxdesc_t * pCurrent, struct sk_buff *skb)
#endif
{
