...
vmacApInfo_t *Mac_Init(struct wlprivate * wlp, struct net_device * dev, char *addr, UINT32 mode, int phyMacId)
{
	...
	if (wlp) {
...
#ifdef SOC_W906X
		if ((wlp->wlpd_p->vmacIndex == wlp->wlpd_p->NumOfAPs) && (mode == WL_OP_MODE_VSTA)) {
		...
		memcpy((void *)vmacSta_p->VMacEntry.mib_QAP_MUEDCA_Table,
		       (void *)mib_QAP_MUEDCA_Table_def, sizeof(vmacSta_p->VMacEntry.mib_QAP_MUEDCA_Table));
#endif
	} else {
	...
	memset(&vmacSta_p->thisprobeRespIEsWAPI, 0, sizeof(WAPI_ProbeRespIEs_t));
#endif
#ifdef MRVL_DFS
	if (wlp)
