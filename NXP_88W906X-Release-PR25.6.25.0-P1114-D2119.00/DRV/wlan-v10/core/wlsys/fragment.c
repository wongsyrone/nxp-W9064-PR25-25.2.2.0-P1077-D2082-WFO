...
struct sk_buff *DeFragPck(struct net_device *dev, struct sk_buff *skb, extStaDb_StaInfo_t ** pStaInfo)
{
	...
	*pStaInfo = extStaDb_GetStaInfo(vmacSta_p, &(wh->addr2), STADB_UPDATE_AGINGTIME);
	...
	if (((*pStaInfo)->DeFragBufInfo.SeqNo == SeqNo) && ((*pStaInfo)->DeFragBufInfo.FragNo == FragNo)) {
	...
	if (FragNo == 0) {	//first fragment
		...
		if (BuffNo != 0xFFFFFFFF) {
			...
			if (wh->FrmCtl.MoreFrag) {
				if (skb_is_nonlinear(skb)) {
					...
					if ((*pStaInfo)->DeFragBufInfo.pFrame == NULL) {
						wl_free_skb(skb);
						return NULL;
					}
					...
					wl_free_skb(skb);
				} else if ((unsigned long)skb->end - (unsigned long)skb->head < dev->mtu + ieee80211HdrLen) {
					(*pStaInfo)->DeFragBufInfo.pFrame = skb_copy_expand(skb, 0, (dev->mtu + ieee80211HdrLen)
		...
		if (BuffNo != 0xFFFFFFFF) {
			if ((*pStaInfo)->DeFragBufInfo.FragNo + 1 == FragNo) {
				if ((skb_tail_pointer((*pStaInfo)->DeFragBufInfo.pFrame) == NULL) || ((skb->data + ieee80211HdrLen) == NULL)) {
					wl_free_skb(skb);
					return NULL;
				}
				...
				memcpy(skb_tail_pointer((*pStaInfo)->DeFragBufInfo.pFrame), skb->data + ieee80211HdrLen, skb->len - ieee80211HdrLen);
				/* Update tail and length */
				skb_put((*pStaInfo)->DeFragBufInfo.pFrame, skb->len - ieee80211HdrLen);
