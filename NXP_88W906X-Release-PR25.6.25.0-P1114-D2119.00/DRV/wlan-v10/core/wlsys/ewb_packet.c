...
static void *ewb_net_hdr(struct sk_buff *skb, unsigned short *pEthType, unsigned short *pNetOffset)
{
	...
	if (etherType == ETH_8021Q_TYPE && (netOffset + sizeof(vlan8021q_hdr_t)) <= skb_headlen(skb)) {
 ...
 *
 **************************************************************************/
int ewbIpToDs(unsigned char *packet, void *post_eth, unsigned char *rfAddr)
{
	eth_hdr_t *eth;
	ewb_ip_hdr *ip;
	UINT32 srcAddr;
 ...
 *
 **************************************************************************/
int ewbIpFromDs(unsigned char *packet, void *post_eth)
{
	eth_hdr_t *eth;
	ewb_ip_hdr *ip;
 ...
 *
 **************************************************************************/
int ewbArpToDs(unsigned char *packet, void *post_eth, unsigned char *rfAddr)
{
	eth_hdr_t *eth;
 ...
 *
 **************************************************************************/
int ewbArpFromDs(unsigned char *packet, void *post_eth)
{
	eth_hdr_t *eth;
	arp_eth_ip_addr *arpAddr;
	unsigned char *clntMac;
...
extern int ewbWlanRecv(struct sk_buff *skb, unsigned char *rfAddr)
{
	...
	case ETH_IP_TYPE:
		...
		if (!(memcmp(eth->dest, rfAddr, HW_ADDR_LEN))
		    && iphp->protocol == IPPROTO_UDP && udphp->source == htons(67) && (udphp->dest == htons(67) || udphp->dest == htons(68))) {
...
extern int ewbLanRecv(struct sk_buff *skb, unsigned char *rfAddr)
{
	...
	case ETH_IP_TYPE:
		...
		if (iphp->protocol == IPPROTO_UDP && bootp->op == BOOTP_REQUEST && (bootp->exten[0] == ic_bootp_cookie[0])
			...
			if (bootp->exten[6] == DHCPDISCOVER || bootp->exten[6] == DHCPREQUEST) {
				...
				udphp->check = 0;
				skb->csum = skb_checksum(skb, iphp->ihl * 4, skb->len - iphp->ihl * 4, 0);
				udphp->check = csum_tcpudp_magic(iphp->saddr, iphp->daddr, skb->len - iphp->ihl * 4, IPPROTO_UDP, skb->csum);
