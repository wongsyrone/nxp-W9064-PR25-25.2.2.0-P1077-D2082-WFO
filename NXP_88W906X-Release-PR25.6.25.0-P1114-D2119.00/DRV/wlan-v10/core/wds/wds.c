...
void setStaPeerInfoApMode(struct wlprivate *wlpptr, extStaDb_StaInfo_t * pStaInfo, PeerInfo_t * pWdsPeerInfo, UINT8 ApMode,
...
void setWdsPeerInfo(PeerInfo_t * pWdsPeerInfo, UINT8 Mode)
{
	...
	case AP_MODE_B_ONLY:
		...
		pWdsPeerInfo->LegacyRateBitMap = ENDIAN_SWAP32(0x0000000F);	// Set for b rates.
	...
	default:		// case AP_MODE_G_ONLY:
		...
		pWdsPeerInfo->CapInfo.ShortPreamble = 1;
		pWdsPeerInfo->CapInfo.ShortSlotTime = 1;
		pWdsPeerInfo->LegacyRateBitMap = ENDIAN_SWAP32(0x00001FFF);	// Set for g rates.
		pWdsPeerInfo->MrvlSta = 0;
		break;
	}
...
void wlprobeInitWds(struct wlprivate *wlpptr)
{
	...
	for (i = 0; i < MAX_WDS_PORT; i++) {
		...
		wlpptr->vmacSta_p->wdsPort[i].pWdsDevInfo = (void *)&wlpptr->vmacSta_p->wdsPeerInfo[i];
		snprintf(wlpptr->vmacSta_p->wdsPort[i].netDevWds->name, IFNAMSIZ, DRV_NAME_WDS, wlpptr->netDev->name, (int)i);
...
extStaDb_StaInfo_t *updateWds(struct net_device * netdev)
{
	...
	//if ((pStaInfo = extStaDb_GetStaInfo(vmacSta_p,(IEEEtypes_MacAddr_t *)&netdev->dev_addr, STADB_UPDATE_AGINGTIME)) == NULL)
	if ((pStaInfo = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) & pWdsPort->wdsMacAddr, STADB_UPDATE_AGINGTIME)) == NULL) {
		...
		if ((pStaInfo =
		     macMgtStaDbInit(vmacSta_p, (IEEEtypes_MacAddr_t *) & pWdsPort->wdsMacAddr, (IEEEtypes_MacAddr_t *) & vmacSta_p->macBssId))) {
			pStaInfo->State = UNAUTHENTICATED;
			pStaInfo->PwrMode = PWR_MODE_ACTIVE;
			if (pStaInfo->Aid == 0)
				pStaInfo->Aid = AssignAid(vmacSta_p);
#ifdef SOC_W906X
			if (pStaInfo->StnId >= sta_num) {
				staIdx = AssignStnId(vmacSta_p);
				if (staIdx >= sta_num) {
					//printk("Error[%s:%d]: No Available resource AssignStnId Failed!!!\n", __func__, __LINE__);
					extStaDb_DelSta(vmacSta_p, &pStaInfo->Addr, STADB_DONT_UPDATE_AGINGTIME);
					return NULL;
				}
				pStaInfo->StnId = staIdx;
			}
#else
			if (pStaInfo->StnId == 0)
...
#endif
		pStaInfo->FwStaPtr = wlFwSetNewStn(netdev, (u_int8_t *) & pWdsPort->wdsMacAddr, pStaInfo->Aid, pStaInfo->StnId,
						   StaInfoDbActionAddEntry, pPeerInfo, 0, *(wlpptr->vmacSta_p->Mib802dot11->QoSOptImpl), 1);
...
{
	//      extStaDb_StaInfo_t *pStaInfo = NULL;
	WLDBG_INFO(DBG_LEVEL_6, "setWdsPortMacAddr %x:%x:%x:%x:%x:%x\n", pMacAddr[0], pMacAddr[2], pMacAddr[3], pMacAddr[4], pMacAddr[5]);
...
BOOLEAN wlCreateWdsPort(struct wlprivate * wlpptr, UINT8 * macAddr, UINT8 wdsMode)
{
	...
	setWdsPeerInfo((PeerInfo_t *) wlpptr->vmacSta_p->wdsPort[wlpptr->vmacSta_p->CurrFreeWdsPort].pWdsDevInfo,
		       *(wlpptr->vmacSta_p->Mib802dot11->mib_ApMode));
	ether_setup(wlpptr->vmacSta_p->wdsPort[wlpptr->vmacSta_p->CurrFreeWdsPort].netDevWds);
	snprintf(wlpptr->vmacSta_p->wdsPort[wlpptr->vmacSta_p->CurrFreeWdsPort].netDevWds->name, IFNAMSIZ, DRV_NAME_WDS, wlpptr->netDev->name,
		 (int)wlpptr->vmacSta_p->CurrFreeWdsPort);
...
void setStaPeerInfoApMode(struct wlprivate *wlpptr, extStaDb_StaInfo_t * pStaInfo, PeerInfo_t * pWdsPeerInfo, UINT8 ApMode, struct wds_port *pWdsPort)
{
	...
	case AP_MODE_5GHZ_NandACand11AX:
		...
		pWdsPeerInfo->HTRateBitMap = ENDIAN_SWAP32((0xff | (0xff << 8) | (0xff << 16) | (0xff << 24)));
		if (pWdsPort) {
			...
			pStaInfo->aggr11n.cap = amsdu_bitmap;
		    ...
		    /** 1x1 configuration **/
				...
				if ((mib->PhyDSSSTable->Chanflag.ChnlWidth == CH_160_MHz_WIDTH)
					...
					pWdsPeerInfo->vht_RxChannelWidth = 3;
					vht_cap |= (1 << 2);	//set bit2 for 160Mhz support

					//If LGI
					if (*(wlpptr->vmacSta_p->Mib802dot11->mib_guardInterval) == 2)
						vht_cap &= ~(1 << 6);
					else
	...
	case AP_MODE_A_ONLY:
		...
		pWdsPeerInfo->CapInfo.ShortPreamble = 1;
		pWdsPeerInfo->CapInfo.ShortSlotTime = 1;
		pWdsPeerInfo->LegacyRateBitMap = ENDIAN_SWAP32(0x00001FE0);	// Set for A rates.
		pWdsPeerInfo->MrvlSta = 0;
		if (pWdsPort) {
...
BOOLEAN setWdsStaPeerInfoWdsPortMode(struct wlprivate *wlpptr, extStaDb_StaInfo_t * pStaInfo, PeerInfo_t * pWdsPeerInfo, UINT8 wdsPortMode)
{
	...
	case AC_3SS_MODE:
		...
		if ((PhyDSSSTable->Chanflag.ChnlWidth == CH_160_MHz_WIDTH) || (PhyDSSSTable->Chanflag.ChnlWidth == CH_AUTO_WIDTH)) {
			pWdsPeerInfo->vht_RxChannelWidth = 3;
			vht_cap |= (1 << 2);	//set bit2 for 160Mhz support
		...
		if ((*(wlpptr->vmacSta_p->Mib802dot11->pMib_11nAggrMode) & WL_MODE_AMSDU_TX_MASK) == WL_MODE_AMSDU_TX_8K)
			vht_cap |= 1;
		else if ((*(wlpptr->vmacSta_p->Mib802dot11->pMib_11nAggrMode) & WL_MODE_AMSDU_TX_MASK) == WL_MODE_AMSDU_TX_11K)
			vht_cap |= 2;
...
BOOLEAN wlSetWdsPort(struct wlprivate * wlpptr, UINT8 * macAddr, UINT8 wdsIndex, UINT8 wdsPortMode)
{
	...
	if (!wlpptr->vmacSta_p->wdsPort[wdsIndex].active) {
		snprintf(wlpptr->vmacSta_p->wdsPort[wdsIndex].netDevWds->name, IFNAMSIZ, DRV_NAME_WDS, wlpptr->netDev->name, (int)wdsIndex);
	}
	...
	if (*(wlpptr->vmacSta_p->Mib802dot11->mib_wdsEnable)) {
		...
		for (i = 0; i < MAX_WDS_PORT; i++) {
			WLDBG_INFO(DBG_LEVEL_6, "Mib wds Mac addr %x:%x:%x:%x:%x:%x\n",
				   ...
				   wlpptr->vmacSta_p->Mib802dot11->mib_WdsMacAddr[i][0][4], wlpptr->vmacSta_p->Mib802dot11->mib_WdsMacAddr[i][0][5]);
		...
		for (i = 0; i < MAX_WDS_PORT; i++) {
			...
			WLDBG_INFO(DBG_LEVEL_6, "Delete Mib wds Mac addr %x:%x:%x:%x:%x:%x\n",
				   ...
				   mib->mib_WdsMacAddr[i][0][3], mib->mib_WdsMacAddr[i][0][4], mib->mib_WdsMacAddr[i][0][5]);
