...
*
*****************************************************************************/
WL_STATUS SendChannelSwitchCmd(vmacApInfo_t * vmacSta_p, Dfs_ChanSwitchReq_t * ChannelSwitchCmd_p)
{
	...
	if (ChannelSwitchCmd_p) {
		...
		memcpy(&SmeCmd_p->Body.Dfs_ChanSwitchReq, ChannelSwitchCmd_p, sizeof(Dfs_ChanSwitchReq_t));
			...
			if (eventsTriggered & smeMain_MGMT_MSG_RCVD) {
				CfrmIndMsg_p = (smeQ_MgmtMsg_t *) msg;
				{
					...
					case SME_NOTIFY_START_CFRM:
						if (CfrmIndMsg_p->Msg.StartCfrm.Result == START_RESULT_SUCCESS) {
						} else if (CfrmIndMsg_p->Msg.StartCfrm.Result == START_RESULT_BSS_ALREADY_STARTED_OR_JOINED) {
						} else if (CfrmIndMsg_p->Msg.StartCfrm.Result == START_RESULT_INVALID_PARAMETERS) {
						}
						break;
...
#endif				/* DEBUG_ENABLE */
								if ((staInfo = extStaDb_GetStaInfo(vmacSta_p, &CfrmIndMsg_p->Msg.AssocInd.PeerStaAddr,
...
#endif				/* DEBUG_ENABLE */
								if ((staInfo =
								     extStaDb_GetStaInfo(vmacSta_p, &CfrmIndMsg_p->Msg.ReassocInd.PeerStaAddr,
											 STADB_DONT_UPDATE_AGINGTIME))) {
									/* Station IS capable of doing spectrum management */
									if (staInfo->IsSpectrumMgmt == TRUE) {
#ifdef DEBUG_ENABLE
					...
					case SME_NOTIFY_CHANNELSWITCH_CFRM:
						{
							...
							if (CfrmIndMsg_p->Msg.ChanSwitchCfrm.result) {
								...
								memcpy(&dfsCmd.Body.chInfo,
								       &CfrmIndMsg_p->Msg.ChanSwitchCfrm.chInfo, sizeof(DfsChanInfo));
					...
					case SME_NOTIFY_RADAR_DETECTION_IND:
						{
							...
							memcpy(&dfsCmd.Body.chInfo, &CfrmIndMsg_p->Msg.RadarDetectionInd.chInfo, sizeof(DfsChanInfo));
							evtDFSMsg(vmacSta_p->dev, (UINT8 *) & dfsCmd);
						}
						break;
#endif				//MRVL_DFS
					...
					case SME_NOTIFY_MREPORT_IND:
						{
							...
							if (((*(mib->mib_ApMode) == AP_MODE_A_ONLY) || (*(mib->mib_ApMode) == AP_MODE_AandG)) &&
							    (mib_StaCfg_p->SpectrumManagementRequired == TRUE)) {
								staInfo = extStaDb_GetStaInfo(vmacSta_p, &CfrmIndMsg_p->Msg.MreportInd.PeerStaAddr,
											      STADB_DONT_UPDATE_AGINGTIME);
#ifdef DEBUG_ENABLE
...
#endif				/* DEBUG_ENABLE */
										smeMreportIndProcess(vmacSta_p,
												     (IEEEtypes_MacAddr_t *) & staInfo->Addr,
												     &CfrmIndMsg_p->Msg.MreportInd);
									}
								} else {
#ifdef DEBUG_ENABLE
									printf("Oops! Dest station not found\n\r");
...
UINT32 SendStartCmd(vmacApInfo_t * vmacSta_p)
{
	...
	CopySsId(SmeCmd_p->Body.StartCmd.SsId, mib_StaCfg_p->DesiredSsId);
	{
		...
		if (mib_StaCfg_p->mib_preAmble == PREAMBLE_LONG || mib_StaCfg_p->mib_preAmble == PREAMBLE_AUTO_SELECT
	...
	if ((*(vmacSta_p->Mib802dot11->mib_ApMode) & AP_MODE_BAND_MASK) < AP_MODE_A_ONLY) {
		...
		for (i = 0, j = 0; (i < IEEEtypes_MAX_CHANNELS_A); i++) {
			if (channels[i + IEEEtypes_MAX_CHANNELS] != 0) {
				SmeCmd.Body.ScanCmd.ChanList[j] = channels[i + IEEEtypes_MAX_CHANNELS];
				j++;
			}
		}
		SmeCmd.Body.ScanCmd.ChanList[j] = 0;
...
void semShowChannelUtilizationStatus(void)
{
	...
	for (loop = 0; loop < nbr_of_channel; loop++) {
		etherBugSend("Channel %d: BSS %d | OFDM %d | UnidentifiedSignal %d | Radar %d | Unmeasured %d\n\r",
			     semChannelUtilizationInfo[loop].channel, semChannelUtilizationInfo[loop].mMap.BSS,
			     semChannelUtilizationInfo[loop].mMap.OFDM, semChannelUtilizationInfo[loop].mMap.UnidentifiedSignal,
			     semChannelUtilizationInfo[loop].mMap.Radar, semChannelUtilizationInfo[loop].mMap.Unmeasured);
	}
#endif
	return;
}
