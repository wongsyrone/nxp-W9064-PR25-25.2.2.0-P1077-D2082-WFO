...
#endif
/* Fix XBOX can't recv LDPC frame issue */
#define XBOX_OUI_NUM      3
UINT8 XBOX_MAC_OUI[XBOX_OUI_NUM][3] = { {0x60, 0x45, 0xBD}
,
{0x50, 0x1A, 0xC5}
,
{0x28, 0x18, 0x78}
...
#ifdef SOC_W906X
extern int wlchannelSet(struct net_device *netdev, int channel, int Channel2, CHNL_FLAGS * chanflag, UINT8 initRateTable);
//twt
#ifdef AP_TWT
void ProcessTWTsetup(vmacApInfo_t * vmacSta_p, macmgmtQ_MgmtMsg_t * MgmtMsg_p);
void ProcessTWTteardown(vmacApInfo_t * vmacSta_p, macmgmtQ_MgmtMsg_t * MgmtMsg_p);
#endif
#else
...
UINT8 NXP_OUI[3] = { 0x00, 0x50, 0x43 };
UINT8 MTK_OUI[3] = { 0x00, 0x0c, 0xe7 };

#ifdef MULTI_AP_SUPPORT
UINT8 M_COMP_OUI[3] = { 0x00, 0x0c, 0xe7 };
...
#endif
extern bool acs_IsTheSameGroup(UINT16 ch1, UINT16 ch2, UINT8 bw);
...
/*============================================================================= */
WL_STATUS isSsIdMatch(IEEEtypes_SsIdElement_t * SsId1, IEEEtypes_SsIdElement_t * SsId2);

WL_STATUS isCapInfoSupported(IEEEtypes_CapInfo_t * CapInfo1, IEEEtypes_CapInfo_t * CapInfo2);
...
WL_STATUS isBasicRatesSupported(IEEEtypes_SuppRatesElement_t * SuppRates, IEEEtypes_ExtSuppRatesElement_t * ExtSuppRates, BOOLEAN gRatePresent);
UINT32 SetSuppRateSetRegMap(vmacApInfo_t * vmacSta_p, IEEEtypes_SuppRatesElement_t * SuppRates, UINT32 * SuppRateSetRegMap);
...
extern Status_e NotifyPwrModeChange(IEEEtypes_MacAddr_t * mac, IEEEtypes_PwrMgmtMode_e mode, UINT8 flag, UINT8 flag)
{
	return FAIL;
};
#else
extern Status_e NotifyPwrModeChange(IEEEtypes_MacAddr_t * mac, IEEEtypes_PwrMgmtMode_e mode)
{
	return FAIL;
};
#endif
...
#ifdef AUTOCHANNEL
extern void wlClrSecChan(vmacApInfo_t * vmacSta_p, UINT32 chan, UINT8 shadowMIB);
...
#ifdef SOC_W906X
extern int wlFwSetChannelSwitchIE(struct net_device *netdev, UINT16 nextChannel, UINT16 secChan, UINT32 mode, UINT32 count, CHNL_FLAGS * Chanflag);
#else
extern int wlFwSetChannelSwitchIE(struct net_device *netdev, UINT32 nextChannel, UINT32 mode, UINT32 count, UINT32 freq, UINT32 bw);
#endif
...
extern void wlAcntCopyRateTbl(struct net_device *netdev, UINT8 * sta_addr, UINT32 sta_id, UINT8 type);
...
void macMgmtMlme_UpdateRateTable4AllSta(vmacApInfo_t * vmac_p);
...
void fillStaInfo(struct net_device *netdev, struct station_info *pSinfo, struct extStaDb_StaInfo_t *pStaInfo);
void fillApCapInfo(struct net_device *netdev, int mode, u8 * pApCap, u32 * pLen);
void fillApRadioInfo(struct net_device *netdev, struct ap_radio_basic_capa_rpt_t *pRpt);
#endif				/* SOC_W906X */
...
static unsigned int syncSrv_channel_score_calibration(vmacApInfo_t * vmacSta_p, unsigned int channel_idx, unsigned int avg_score);
/*============================================================================= */
...
extern void *FindIEWithinIEs(UINT8 * data_p, UINT32 lenPacket, UINT8 attrib, UINT8 * OUI);
...
int add_MMIE(vmacApInfo_t * vmac_p, UINT8 * buf)
{
	int len = 0;
	...
	if (vmac_p->igtksaInstalled == WL_CIPHER_IGTK) {
		...
		len = sizeof(IEEEtypes_MMIE_Element_t) - 8;
		   ...
		   vmac_p->igtksaInstalled == WL_CIPHER_AES_GMAC_256 || vmac_p->igtksaInstalled == WL_CIPHER_AES_CMAC_256) {
		mmie_p->Len = 24;
...
int add_MMIET(vmacApInfo_t * vmac_p, UINT8 * buf)
{
	int len = 0;
	IEEEtypes_MMIE_Element_t *mmie_p = (IEEEtypes_MMIE_Element_t *) buf;
	mmie_p->ElementId = 76;
...
{
	if ((Subtype != IEEE_MSG_DISASSOCIATE) && (Subtype != IEEE_MSG_DEAUTHENTICATE) && (Subtype != IEEE_MSG_QOS_ACTION))
		return 0;
	return 1;
}

int isRobustQoSFrame(UINT16 qosCategory)
{
	switch (qosCategory) {
	...
	case 6:		/* Fast BSS Transition */
	case 8:		/* SA Query */
	case 9:		/* Protected Dual of Public Action */
	case 10:		/* WNM */
	case 13:		/* Mesh */
	case 14:		/* Multihop */
	case 16:		/* DMG */
	case 18:		/* Fast Session Transfer */
	case 19:		/* Robust AV Streaming */
	case 31:		/* Protected HE */
	case 126:		/* Vendor-specific Protected */
		return 1;
	}
	return 0;
}

int validateFrameContents(macmgmtQ_MgmtMsg3_t * mgtFrm)
{
	...
	case IEEE_MSG_DEAUTHENTICATE:
		...
		printk("subtype: %x reason: %x mmie_p->ElementId: %x\n",
		       mgtFrm->Hdr.FrmCtl.Subtype, mgtFrm->Body.Deauth.ReasonCode, mmie_p->ElementId);
		...
		if (totallen != mgtFrm->Hdr.FrmBodyLen + 2)
			ret = 1;
		/* invalid reason code */
		if (mgtFrm->Body.Deauth.ReasonCode > IEEEtypes_REASON_8021X_AUTH_FAIL)
			ret = 1;
		break;
...
int validateRobustManagementframe(vmacApInfo_t * vmac_p, extStaDb_StaInfo_t * StaInfo_p, macmgmtQ_MgmtMsg3_t * mgtFrm)
{
	if (!StaInfo_p)
		return 0;
...
extern int aes_gmac(const u8 * key, size_t key_len, const u8 * iv, size_t iv_len, const u8 * aad, size_t aad_len, u8 * tag);
extern struct crypto_cipher *ieee80211_aes_cmac_key_setup(const u8 * key, size_t key_len);
extern void ieee80211_aes_cmac_256(struct crypto_cipher *tfm, const u8 * aad, const u8 * data, size_t data_len, u8 * mic);
extern void ieee80211_aes_cmac_key_free(struct crypto_cipher *tfm);

#define MMIE_AAD_LEN	sizeof(IEEE80211w_AAD_t)	//20

int mgmtBipMicHandler(UINT8 bipType, UINT8 * igtk, macmgmtQ_MgmtMsg2_t * mgtFrm, IEEEtypes_MMIE_Element_t * mmie_p, int payload_len, BOOLEAN isEnc)
{
	...
	if (bipType == WL_CIPHER_IGTK) {
		omac1_aes_128((const UINT8 *)igtk, (const UINT8 *)aad_p, (size_t) (payload_len + MMIE_AAD_LEN), mic);
		memcpy((UINT8 *) mmie_p->MIC, mic, mic_len);
	} else if (bipType == WL_CIPHER_AES_GMAC || bipType == WL_CIPHER_AES_GMAC_256 || bipType == WL_CIPHER_AES_CMAC_256) {
		u8 nonce[12], *npos;
		size_t igtk_len = 32;
		...
		if (bipType == WL_CIPHER_AES_CMAC_256) {
			...
			tfm = ieee80211_aes_cmac_key_setup((const UINT8 *)igtk, igtk_len);
			if (tfm == 0) {
				printk("error: key setup fail for WL_CIPHER_AES_CMAC256");
				return 0;
			}
			ieee80211_aes_cmac_256(tfm, (const UINT8 *)aad_p, (const u8 *)mgtFrm->Body.data, (size_t) payload_len, mmie_p->MIC);
			...
			*npos++ = mmie_p->IPN[0];
			...
			aes_gmac((const UINT8 *)igtk, igtk_len, nonce, sizeof(nonce), (const UINT8 *)aad_p,
				 (size_t) (payload_len + MMIE_AAD_LEN), mmie_p->MIC);
		}
	} else {
...
int macMgmtBIP(vmacApInfo_t * vmac_p, macmgmtQ_MgmtMsg2_t * mgtFrm, int payload_len)
{
	...
	mmie_p = (IEEEtypes_MMIE_Element_t *) ((UINT8 *) & mgtFrm->Body + payload_len);
	payload_len += add_MMIE(vmac_p, (UINT8 *) mmie_p);
	return mgmtBipMicHandler(vmac_p->igtksaInstalled, vmac_p->igtk, mgtFrm, mmie_p, payload_len, TRUE);
}
#endif

#ifdef AUTOCHANNEL
static inline int syncSrv_wifi_noise(vmacApInfo_t * vmacSta_p)
{
	int i = 0;
	while (vmacSta_p->ScanParams.ChanList[i]) {
		if (vmacSta_p->acs_db[i].bss_num && vmacSta_p->acs_db[i].raw_max_rssi >= -(rssi_threshold))
			return 1;
		i++;
	}
	return 0;
...
static int syncSrv_adjacent_avg_nf(vmacApInfo_t * vmacSta_p, int ch_idx)
{
	int cal_nf_sum = 0;
	int cal_count = 0;

	cal_nf_sum += vmacSta_p->acs_db[ch_idx].noise_floor;
	cal_count += 1;
	if (domainChannelValid(vmacSta_p->ScanParams.ChanList[ch_idx - 1], FREQ_BAND_2DOT4GHZ)) {
		cal_nf_sum += vmacSta_p->acs_db[ch_idx - 1].noise_floor;
		cal_count++;
	}
	if (domainChannelValid(vmacSta_p->ScanParams.ChanList[ch_idx + 1], FREQ_BAND_2DOT4GHZ)) {
		cal_nf_sum += vmacSta_p->acs_db[ch_idx + 1].noise_floor;
	...
	return cal_nf_sum / cal_count;
}

void ACS_set_unusable_channels(vmacApInfo_t * vmacSta_p, UINT8 band, s32 avg_nf)
{
	UINT8 i = 0;
	UINT8 worst_nf_idx1 = 0, worst_nf_idx2 = 0, worst_nf_idx3 = 0, worst_nf_idx4 = 0;

	if (!vmacSta_p || vmacSta_p->acs_mode != 0 || band != FREQ_BAND_2DOT4GHZ)
		return;

	if (avg_nf > -rssi_threshold) {
		/* first round filter the worst score / most bss / bad NF channels */
		while (vmacSta_p->ScanParams.ChanList[i]) {
			if (i == vmacSta_p->worst_channel_idx[0] || i == vmacSta_p->worst_channel_idx[1] ||
			    i == vmacSta_p->bss_channel_idx[0] || i == vmacSta_p->bss_channel_idx[1] || vmacSta_p->acs_db[i].noise_floor >= avg_nf) {
				vmacSta_p->acs_db[i].unusable = 1;
			}
			i++;
		}

		/* second round filter more bad channels according to 4 worst NF channels */
		if (vmacSta_p->NumScanChannels >= 1)
			worst_nf_idx1 = vmacSta_p->acs_2g_sorted_nf_idx[vmacSta_p->NumScanChannels - 1];
		if (vmacSta_p->NumScanChannels >= 2)
			worst_nf_idx2 = vmacSta_p->acs_2g_sorted_nf_idx[vmacSta_p->NumScanChannels - 2];
		if (vmacSta_p->NumScanChannels >= 3)
			worst_nf_idx3 = vmacSta_p->acs_2g_sorted_nf_idx[vmacSta_p->NumScanChannels - 3];
		if (vmacSta_p->NumScanChannels >= 4)
			worst_nf_idx4 = vmacSta_p->acs_2g_sorted_nf_idx[vmacSta_p->NumScanChannels - 4];

		i = 0;
		while (vmacSta_p->ScanParams.ChanList[i]) {
			/* filter the 2 max NF channel and it's neighbor channel */
			if ((i == worst_nf_idx1 - 2 || i == worst_nf_idx1 - 1 || i == worst_nf_idx1 + 2 || i == worst_nf_idx1 + 1 ||
			     i == worst_nf_idx2 - 2 || i == worst_nf_idx2 - 1 || i == worst_nf_idx2 + 2 || i == worst_nf_idx2 + 1)) {
				vmacSta_p->acs_db[i].unusable = 1;
			}
			/* filter the bss_num <= min bss_num but NF is big channel */
			if ((i == worst_nf_idx3 || i == worst_nf_idx4) && vmacSta_p->acs_db[i].bss_num == 0)
				vmacSta_p->acs_db[i].unusable = 1;
...
static void syncSrv_channel_score(vmacApInfo_t * vmacSta_p)
{
	int i = 0, j = 0;
	unsigned int avg_score = 0;
	s32 avg_nf = 0;
	unsigned char band = 0, channel_num_2g = 0;
	unsigned int tmp_swap_value = 0, local_score_ref[IEEEtypes_MAX_CHANNELS], local_nf_ref[IEEEtypes_MAX_CHANNELS];

	if ((vmacSta_p->acs_mode == 1) && vmacSta_p->acs_mode_nf_worst_score)
		vmacSta_p->acs_mode_nf_normalize_factor = 10000 / vmacSta_p->acs_mode_nf_worst_score;

	band = (*(vmacSta_p->Mib802dot11->mib_ApMode) & AP_MODE_BAND_MASK) >= AP_MODE_A_ONLY ? FREQ_BAND_5GHZ : FREQ_BAND_2DOT4GHZ;
	...
	while (vmacSta_p->ScanParams.ChanList[i]) {
		...
		if (vmacSta_p->acs_mode == 0) {
			...
			if (vmacSta_p->acs_db[i].bss_num)
				rssi_bonus = abs(rssi_threshold - vmacSta_p->acs_db[i].rssi_ls);

			if ((vmacSta_p->acs_db[i].noise_floor <= -85) && (vmacSta_p->acs_db[i].ch_load > 50))
				vmacSta_p->acs_db[i].ch_load /= 10;

			ch_load = min((unsigned int)100, (unsigned int)vmacSta_p->acs_db[i].ch_load);
			ch_nf = syncSrv_adjacent_avg_nf(vmacSta_p, i);

			/* Compute the score baesd on weighted channel load and noise floor */
			if (!syncSrv_wifi_noise(vmacSta_p))
				vmacSta_p->acs_db[i].score = (abs(ch_nf) * vmacSta_p->acs_ch_nf_weight) + max_rssi * vmacSta_p->acs_rssi_weight;
			else
				vmacSta_p->acs_db[i].score =
				    rssi_bonus * vmacSta_p->acs_rssi_weight + (abs(ch_nf) * (vmacSta_p->acs_ch_nf_weight + 50));

			if (vmacSta_p->ScanParams.ChanList[i] == 36) {
				printk("extra weight %d for chan: %u\n", 2 * ext_weight_1611, vmacSta_p->ScanParams.ChanList[i]);
				vmacSta_p->acs_db[i].score += (2 * ext_weight_1611);
			}

			/* Print the score info */
			if (!syncSrv_wifi_noise(vmacSta_p))
				printk("Channel %d NF: %d Load: %d Rssi: %d (%d), Score %d \n", vmacSta_p->ScanParams.ChanList[i], ch_nf,
				       ch_load, vmacSta_p->acs_db[i].max_rssi, max_rssi, vmacSta_p->acs_db[i].score);
			else
				printk("Channel %d NF: %d Load: %d SUM Rssi: %d, Bss Num: %u, Score: %u (%u)\n",
				       vmacSta_p->ScanParams.ChanList[i], ch_nf,
				       ch_load, vmacSta_p->acs_db[i].rssi_ls, vmacSta_p->acs_db[i].bss_num, vmacSta_p->acs_db[i].score, rssi_bonus);
		} else if (vmacSta_p->acs_mode == 1) {
			printk("Pre Channel %d Score: %u %u (%u)\n",
			       vmacSta_p->ScanParams.ChanList[i], vmacSta_p->acs_db[i].score,
			       100 - ((vmacSta_p->acs_db[i].score * vmacSta_p->acs_mode_nf_normalize_factor) / 100),
			       vmacSta_p->acs_mode_nf_worst_score);
			/* range is in 0 to 100, higher is better */
			vmacSta_p->acs_db[i].score = 100 - ((vmacSta_p->acs_db[i].score * vmacSta_p->acs_mode_nf_normalize_factor) / 100);
			printk("Pre Channel %d Score: %u \n", vmacSta_p->ScanParams.ChanList[i], vmacSta_p->acs_db[i].score);
		}

		if ((vmacSta_p->acs_mode == 0) && (band == FREQ_BAND_2DOT4GHZ)) {
			avg_score += vmacSta_p->acs_db[i].score;
			avg_nf += vmacSta_p->acs_db[i].noise_floor;
			channel_num_2g++;
			vmacSta_p->acs_2g_sorted_score_idx[i] = i;
			local_score_ref[i] = vmacSta_p->acs_db[i].score;
			vmacSta_p->acs_2g_sorted_nf_idx[i] = i;
			local_nf_ref[i] = ch_nf;
		}
		...
		if (vmacSta_p->acs_mode == 0) {
			...
			else if (vmacSta_p->acs_db[i].score < vmacSta_p->acs_db[vmacSta_p->worst_channel_idx[0]].score) {
				vmacSta_p->worst_channel_idx[1] = vmacSta_p->worst_channel_idx[0];
				vmacSta_p->worst_channel_idx[0] = i;
			} else if ((vmacSta_p->worst_channel_idx[1] == 0xFF) ||
				   (vmacSta_p->acs_db[i].score < vmacSta_p->acs_db[vmacSta_p->worst_channel_idx[1]].score))
				vmacSta_p->worst_channel_idx[1] = i;
		} else {
			u32 worst_idx0_diff = abs((vmacSta_p->worst_channel_idx[0] == 0xFF ? 0 : vmacSta_p->acs_db[i].score)
						  - (vmacSta_p->worst_channel_idx[0] ==
						     0xFF ? 0 : vmacSta_p->acs_db[vmacSta_p->worst_channel_idx[0]].score));
			u32 worst_idx1_diff = abs((vmacSta_p->worst_channel_idx[1] == 0xFF ? 0 : vmacSta_p->acs_db[i].score)
						  - (vmacSta_p->worst_channel_idx[1] ==
						     0xFF ? 0 : vmacSta_p->acs_db[vmacSta_p->worst_channel_idx[1]].score));
			WLDBG_INFO(DBG_LEVEL_7, "%d %d %d score %d:%d %d:%d\n", i, worst_idx0_diff, worst_idx1_diff, vmacSta_p->worst_channel_idx[0],
				   vmacSta_p->acs_db[vmacSta_p->worst_channel_idx[0]].score, vmacSta_p->worst_channel_idx[1],
				   vmacSta_p->acs_db[vmacSta_p->worst_channel_idx[1]].score);
			if (vmacSta_p->worst_channel_idx[0] == 0xFF) {
				vmacSta_p->worst_channel_idx[0] = i;
			} else if (vmacSta_p->acs_db[i].score < vmacSta_p->acs_db[vmacSta_p->worst_channel_idx[0]].score) {
				if (band == FREQ_BAND_5GHZ) {
					if (false == acs_IsTheSameGroup(vmacSta_p->acs_db[i].channel,
									vmacSta_p->acs_db[vmacSta_p->worst_channel_idx[0]].channel, CH_80_MHz_WIDTH))
						vmacSta_p->worst_channel_idx[1] = vmacSta_p->worst_channel_idx[0];
					vmacSta_p->worst_channel_idx[0] = i;
				} else {
					if ((abs(i - vmacSta_p->worst_channel_idx[0]) > 1) || worst_idx0_diff > 5) {
						vmacSta_p->worst_channel_idx[1] = vmacSta_p->worst_channel_idx[0];
						vmacSta_p->worst_channel_idx[0] = i;
					}
				}
			...
			} else if (vmacSta_p->acs_db[i].score <= vmacSta_p->acs_db[vmacSta_p->worst_channel_idx[1]].score) {
				if ((abs(i - vmacSta_p->worst_channel_idx[1]) > 1) || worst_idx1_diff > 5)
					vmacSta_p->worst_channel_idx[1] = i;
			}
		}
		i++;
	...
	if ((vmacSta_p->acs_mode == 0) && (band == FREQ_BAND_2DOT4GHZ)) {
		avg_score = avg_score / (channel_num_2g == 0 ? 1 : channel_num_2g);
		avg_nf = avg_nf / (channel_num_2g == 0 ? 1 : channel_num_2g);
		i = 0;
		while (vmacSta_p->ScanParams.ChanList[i]) {
			vmacSta_p->acs_db[i].noise_floor = local_nf_ref[i];
			i++;
		}

		for (i = 0; i < channel_num_2g - 1; i++) {
			for (j = 0; j < channel_num_2g - 1 - i; j++) {
				if (local_score_ref[j] > local_score_ref[j + 1]) {
					tmp_swap_value = local_score_ref[j];
					local_score_ref[j] = local_score_ref[j + 1];
					local_score_ref[j + 1] = tmp_swap_value;
					tmp_swap_value = vmacSta_p->acs_2g_sorted_score_idx[j];
					vmacSta_p->acs_2g_sorted_score_idx[j] = vmacSta_p->acs_2g_sorted_score_idx[j + 1];
					vmacSta_p->acs_2g_sorted_score_idx[j + 1] = tmp_swap_value;
				}

				if (local_nf_ref[j] > local_nf_ref[j + 1]) {
					tmp_swap_value = local_nf_ref[j];
					local_nf_ref[j] = local_nf_ref[j + 1];
					local_nf_ref[j + 1] = tmp_swap_value;
					tmp_swap_value = vmacSta_p->acs_2g_sorted_nf_idx[j];
					vmacSta_p->acs_2g_sorted_nf_idx[j] = vmacSta_p->acs_2g_sorted_nf_idx[j + 1];
					vmacSta_p->acs_2g_sorted_nf_idx[j + 1] = tmp_swap_value;
				}
			}
		}

		vmacSta_p->worst_channel_idx[0] = vmacSta_p->acs_2g_sorted_score_idx[0];
		vmacSta_p->worst_channel_idx[1] = vmacSta_p->acs_2g_sorted_score_idx[1];
		for (i = 0; i < 6; i++) {
			if ((vmacSta_p->acs_db[vmacSta_p->acs_2g_sorted_score_idx[i + 1]].bss_num != 0
			     || (abs(vmacSta_p->acs_2g_sorted_score_idx[i + 1] - vmacSta_p->acs_2g_sorted_score_idx[0]) > 2
				 && abs(vmacSta_p->acs_db[vmacSta_p->acs_2g_sorted_score_idx[i + 1]].noise_floor -
					vmacSta_p->acs_db[vmacSta_p->acs_2g_sorted_score_idx[1]].noise_floor) <= chld_nf_delta))
			    && vmacSta_p->acs_db[vmacSta_p->acs_2g_sorted_score_idx[i + 1]].score <= avg_score) {
				vmacSta_p->worst_channel_idx[1] = vmacSta_p->acs_2g_sorted_score_idx[i + 1];
				break;
			}
		}

		ACS_set_unusable_channels(vmacSta_p, band, avg_nf);
	}
	printk("worst channel (score): %u, %u, avg_score=%u, avg_nf=%d\n", vmacSta_p->worst_channel_idx[0], vmacSta_p->worst_channel_idx[1],
	       avg_score, avg_nf);

	i = 0;
	while (vmacSta_p->ScanParams.ChanList[i]) {
		if (acs_cal)
			vmacSta_p->acs_db[i].score += syncSrv_channel_score_calibration(vmacSta_p, i, avg_score);

		if ((vmacSta_p->ScanParams.ChanList[i] == 1 ||
		     vmacSta_p->ScanParams.ChanList[i] == 6 || vmacSta_p->ScanParams.ChanList[i] == 11) && (vmacSta_p->acs_db[i].bss_num == 0)) {
			if (vmacSta_p->acs_mode == 1) {
				printk("extra weight %d (+0.5) for chan: %u\n", vmacSta_p->acs_db[i].score >> 1, vmacSta_p->ScanParams.ChanList[i]);
				vmacSta_p->acs_db[i].score += (vmacSta_p->acs_db[i].score >> 1);
			} else {
				if (vmacSta_p->acs_db[i].noise_floor < avg_nf) {
					printk("extra weight %d for chan: %u\n", ext_weight_1611, vmacSta_p->ScanParams.ChanList[i]);
	...
	if ((vmacSta_p->acs_mode == 0) && (band == FREQ_BAND_2DOT4GHZ)) {
		...
		printk("\nsorted nf list:\n");
		for (i = 0; i < channel_num_2g; i++) {
			printk("%d(%d)\n", vmacSta_p->acs_2g_sorted_nf_idx[i], vmacSta_p->acs_db[vmacSta_p->acs_2g_sorted_nf_idx[i]].noise_floor);
		}
		printk("sorted final score list:\n");
		for (i = 0; i < channel_num_2g; i++) {
			printk("%d(%d)\n", vmacSta_p->acs_2g_sorted_score_idx[i], vmacSta_p->acs_db[vmacSta_p->acs_2g_sorted_score_idx[i]].score);
		}
		printk("unusable ch list:\n");
		for (i = 0; i < channel_num_2g; i++) {
			if (vmacSta_p->acs_db[i].unusable)
				printk("ch%d(%d)\n", vmacSta_p->ScanParams.ChanList[i], vmacSta_p->acs_db[i].noise_floor);
		}
	}
...
void syncSrv_RestorePreScanSettings(vmacApInfo_t * vmacSta_p)
{
	...
	if (*(vmacSta_p->ShadowMib802dot11->mib_autochannel)) {
		MSAN_get_ACS_db(vmacSta_p, vmacSta_p->NumScanChannels, 0, 0);
...
#endif
		/* reset secondary channel after ACS */
		wlClrSecChan(vmacSta_p, cur_channel, 1);
		...
		if (wlpptr->wlpd_p->pdfsApMain && DfsEnabledChannel(vmacSta_p->dev, &wlpptr->wlpd_p->pdfsApMain->dfsApDesc.currChanInfo)) {
			dfs_sme_channel_switch(vmacSta_p->dev, vmacSta_p->ShadowMib802dot11->PhyDSSSTable->CurrChan, 0, 0);
		}
	} else if (Is2GNeedCoexScan(vmacSta_p->dev)) {
		vmacSta_p->autochannelstarted = 1;	/* break setup scan after reset */
		vmacSta_p->preautochannelfinished = 1;
		wlUpdateAutoChan(vmacSta_p, vmacSta_p->ShadowMib802dot11->PhyDSSSTable->CurrChan, 1);
		wlResetTask(vmacSta_p->dev);
		DisableBlockTrafficMode(vmacSta_p);
		complete(&vmacSta_p->scan_complete);
	}
...
static unsigned int syncSrv_channel_score_calibration(vmacApInfo_t * vmacSta_p, unsigned int channel_idx, unsigned int avg_score)
{
	...
	unsigned int no_cal_gap = (vmacSta_p->acs_mode == 1) ? 1 : 2;

	/* get distance to the worst 2 channel index (worst score) */
	ch_a = vmacSta_p->worst_channel_idx[0];
	ch_b = vmacSta_p->worst_channel_idx[1];

	m = (((channel_idx < ch_a) && (channel_idx < ch_b)) || ((channel_idx > ch_a) && (channel_idx > ch_b))) ? 0 : 1;

	if ((vmacSta_p->acs_mode && vmacSta_p->acs_db[channel_idx].score > avg_score
	     && abs(channel_idx - ch_a) > no_cal_gap && abs(channel_idx - ch_b) > no_cal_gap)
	    || (vmacSta_p->acs_mode == 0 && !vmacSta_p->acs_db[channel_idx].unusable))
		calibrated_ch_bonus = vmacSta_p->acs_ch_distance_weight *
		    (abs(channel_idx - ch_a) + abs(channel_idx - ch_b) - m * abs(abs(channel_idx - ch_a) - abs(channel_idx - ch_b)));
	else
		printk("calibrated_ch_bonus ignore ch%d\n", channel_idx);

	printk("w0: %u, w1: %u, ch_idx: %u ch_dist_bonus: %u\n", ch_a, ch_b, channel_idx, calibrated_ch_bonus);
	...
	if ((vmacSta_p->acs_mode && vmacSta_p->acs_db[channel_idx].score > avg_score
	     && abs(channel_idx - vmacSta_p->bss_channel_idx[0]) > no_cal_gap
	     && abs(channel_idx - vmacSta_p->bss_channel_idx[1]) > no_cal_gap
	     && abs(channel_idx - vmacSta_p->worst_channel_idx[0]) > no_cal_gap && abs(channel_idx - vmacSta_p->worst_channel_idx[1]) > no_cal_gap)
	    || (vmacSta_p->acs_mode == 0 && !vmacSta_p->acs_db[channel_idx].unusable)) {
		...
		if (vmacSta_p->bss_channel_idx[0] != 0xFF) {
			calibrated_bss_bonus += (vmacSta_p->acs_bss_num_weight *
						 abs(vmacSta_p->acs_db[vmacSta_p->bss_channel_idx[0]].bss_num -
						     vmacSta_p->acs_db[channel_idx].bss_num));
			bssb =
			    vmacSta_p->acs_bss_num_weight * abs(vmacSta_p->acs_db[vmacSta_p->bss_channel_idx[0]].bss_num -
								vmacSta_p->acs_db[channel_idx].bss_num);
		}
	} else
		printk("calibrated_bss_bonus ignore ch_idx %d ch%d\n", channel_idx, vmacSta_p->acs_db[channel_idx].channel);

	printk("most bss: %u, ch_idx: %u, bssb: %u, calscore: %u\n", vmacSta_p->bss_channel_idx[0],
	       channel_idx, bssb, calibrated_bss_bonus + calibrated_ch_bonus);

	/* For acs_mode 1, check if there is bss in adjacent channel, */
	if (vmacSta_p->acs_mode && !vmacSta_p->acs_db[channel_idx].bss_num) {
		if (!channel_idx) {
			/* for first channel */
			if (!vmacSta_p->acs_db[channel_idx + 1].bss_num)
				calibrated_bss_bonus += vmacSta_p->acs_adjacent_bss_weight;
		} else if (channel_idx < vmacSta_p->NumScanChannels - 1) {
			/* for non-first and non-last channel */
			if ((!vmacSta_p->acs_db[channel_idx - 1].bss_num) && (!vmacSta_p->acs_db[channel_idx + 1].bss_num)) {
				/* for channel n, check n-1 and n+1 */
				calibrated_bss_bonus += vmacSta_p->acs_adjacent_bss_weight;
				if ((channel_idx > 1) && (channel_idx < vmacSta_p->NumScanChannels - 2)) {
					/* for non-first-2 and non-last-2 channel */
					if ((!vmacSta_p->acs_db[channel_idx - 2].bss_num) && (!vmacSta_p->acs_db[channel_idx + 2].bss_num)) {
						/* for channel n, check n-2 and n+2 */
						calibrated_bss_bonus += vmacSta_p->acs_adjacent_bss_weight_plus;
					}
				}
			}
		} else {
			/* for last channel */
			if (!vmacSta_p->acs_db[channel_idx - 1].bss_num)
				calibrated_bss_bonus += vmacSta_p->acs_adjacent_bss_weight / 2;
		}
	}
	return calibrated_bss_bonus + calibrated_ch_bonus;
}

extern unsigned int ACS_channel_score_from_nf_reading(unsigned int *NF_map, u32 * ch_load, u32 * ch_nf);
extern SINT32 syncSrv_ScanActTimeOut(UINT8 * data)
{
	...
	int channel = 0;
	UINT32 ch_load;
	u8 *mib_autochannel_p = vmacSta_p->ShadowMib802dot11->mib_autochannel;
	u8 scan_20_40_coex = 0;

	if ((*mib_autochannel_p == 0) && Is2GNeedCoexScan(vmacSta_p->dev))
		scan_20_40_coex = 1;
	...
	if (channel) {
		if (scan_20_40_coex) {
			/* scan for 20/40 coex */
			printk("finished scanning channel %d\n", channel);
		} else if (vmacSta_p->acs_mode == 0) {
			/* Compute the score in Legacy Mode */
			t_cnt = Rx_Traffic_Cnt(vmacSta_p);
			bbu_cnt = Rx_Traffic_BBU(vmacSta_p);
			ch_load = wl_ch_load(vmacSta_p, ktime_us_delta(ktime_get_real(), vmacSta_p->acs_scantime), vmacSta_p->acs_chload, 100);
			vmacSta_p->acs_db[vmacSta_p->ChanIdx].channel = channel;
#ifdef SOC_W906X
			vmacSta_p->acs_db[vmacSta_p->ChanIdx].ch_load = ch_load;
			vmacSta_p->acs_db[vmacSta_p->ChanIdx].noise_floor = (SINT32) wl_util_get_nf_from_fw(vmacSta_p->dev);
			if (vmacSta_p->acs_db[vmacSta_p->ChanIdx].noise_floor < -95)
				vmacSta_p->acs_db[vmacSta_p->ChanIdx].noise_floor = -95;
#else				/* SOC_W906X */
			vmacSta_p->acs_db[vmacSta_p->ChanIdx].score = t_cnt * 100 + bbu_cnt;
#endif				/* SOC_W906X */
			//        printk( "finished scanning channel %d, traffic cnt %d, bbu cnt %d\n", channel, (int)t_cnt, (int)bbu_cnt);
			printk("finished scanning channel %d, noise %ddb, ch_load %d\n", channel, vmacSta_p->acs_db[vmacSta_p->ChanIdx].noise_floor,
			       ch_load);
		} else if (vmacSta_p->acs_mode == 1) {
			/* Compute the score in NF-reading Mode */
			vmacSta_p->acs_db[vmacSta_p->ChanIdx].channel = channel;
			vmacSta_p->acs_db[vmacSta_p->ChanIdx].score =
			    ACS_channel_score_from_nf_reading(vmacSta_p->acs_db[vmacSta_p->ChanIdx].nf_bin,
							      &vmacSta_p->acs_db[vmacSta_p->ChanIdx].ch_load,
							      &vmacSta_p->acs_db[vmacSta_p->ChanIdx].noise_floor);
			vmacSta_p->acs_mode_nf_worst_score =
			    max((unsigned int)vmacSta_p->acs_mode_nf_worst_score, (unsigned int)vmacSta_p->acs_db[vmacSta_p->ChanIdx].score);
			printk("finished scanning channel %d, nf=%ddb, score %d\n", channel, vmacSta_p->acs_db[vmacSta_p->ChanIdx].noise_floor,
			       vmacSta_p->acs_db[vmacSta_p->ChanIdx].score);
		}
	}
#endif				/* AUTOCHANNEL */
...
extern void syncSrv_SetNextChannel(vmacApInfo_t * vmacSta_p)
{
	...
	if (vmacSta_p->ChanIdx < vmacSta_p->NumScanChannels) {
		if (vmacSta_p->acs_mode == 0) {
			...
			if (wlSetRFChan(vmacSta_p, vmacSta_p->ScanParams.ChanList[vmacSta_p->ChanIdx]) == FALSE) {
				vmacSta_p->busyScanning = 0;
				syncSrv_RestorePreScanSettings(vmacSta_p);
				/* Reset Mac and Start AP Services */
#ifdef AP_MAC_LINUX
...
#ifdef AUTOCHANNEL
			vmacSta_p->acs_chload = wl_ch_load(vmacSta_p, 0, 0, 0);
...

		{
			...
			offchan.channel = vmacSta_p->ScanParams.ChanList[vmacSta_p->ChanIdx];
			offchan.id = OFFCHAN_GET_ID_FROM_FEATURE(OFFCHAN_BY_ACS, 1);
			offchan.req_type = OFFCHAN_TYPE_RX_NF;	/* NF-reading feedback */
			if (!scan_20_40_coex)
				offchan.dwell_time = SCAN_TIME_MS;	/* 500ms */
			else	/* 20 40 coex scan */
				offchan.dwell_time = 0;	/* Not do NF-reading */
			printk("Offchan ch:%u, id:%u, dwell:%u\n", (unsigned int)offchan.channel, (unsigned int)offchan.id,
			       (unsigned int)offchan.dwell_time);
			wlFwNewDP_queue_OffChan_req(vmacSta_p->dev, &offchan);
		}
	}
}
...
#ifdef AUTOCHANNEL
	memset(vmacSta_p->acs_db, 0, sizeof(acs_data_t) * (IEEEtypes_MAX_CHANNELS + IEEEtypes_MAX_CHANNELS_A));
#endif				/* AUTOCHANNEL */
#ifdef MRVL_DFS
	if (wlpptr->wlpd_p->pdfsApMain != NULL && (Is5GBand(*(vmacSta_p->Mib802dot11->mib_ApMode)))) {
		DfsApDesc *dfsDesc_p = &wlpptr->wlpd_p->pdfsApMain->dfsApDesc;
		dfsDesc_p->cac_complete = 0;
		// Disarm CAC
		TimerDisarm(&dfsDesc_p->CACTimer);
		wlpptr->wlpd_p->bCACTimerFired = FALSE;
		// Switch off radar detection
		macMgmtMlme_StopRadarDetection(vmacSta_p->dev, DFS_NORMAL_MODE);
...
extern int channelSelected(vmacApInfo_t * vmacSta_p, int mode)
{
	...
	int channel = 0;
	UINT32 traffic = 0;
	UINT8 band = mode ? FREQ_BAND_5GHZ : FREQ_BAND_2DOT4GHZ, channel_num = 0;
	/* for AUTOCHANNEL_G_BAND_1_6_11 */
	MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
	SINT32 avg_nf = 0;

	/* 20/40 coex scan done, ch will not change */
	if ((*mib->mib_autochannel == 0) && Is2GNeedCoexScan(vmacSta_p->dev) && vmacSta_p->autochannelstarted)
		return mib->PhyDSSSTable->CurrChan;
#ifdef AP_URPTR
	if (mib_urMode == 1) {
	...
	i = 0;
	while (vmacSta_p->ScanParams.ChanList[i]) {
		if (*(mib->mib_autochannel) == 2 && ACS_OpChanCheck(vmacSta_p, vmacSta_p->ScanParams.ChanList[i]) == FAIL) {
			i++;
			continue;
		}
		if (vmacSta_p->ScanParams.ChanList[i] && domainChannelValid(vmacSta_p->ScanParams.ChanList[i], band)) {
			...
			vmacSta_p->autochannel[i] = vmacSta_p->acs_db[i].score;

			/* move to syncSrv_channel_score
			   if (acs_cal)
			   vmacSta_p->autochannel[i] += syncSrv_channel_score_calibration(vmacSta_p, i);
			 */
			if (vmacSta_p->autochannel[i] > traffic) {
				if (band == FREQ_BAND_5GHZ) {
					u32 ch_idx_1, ch_idx_2;
					ch_idx_1 = vmacSta_p->worst_channel_idx[0];
					ch_idx_2 = vmacSta_p->worst_channel_idx[1];
					if ((true ==
					     acs_IsTheSameGroup(vmacSta_p->acs_db[i].channel, vmacSta_p->acs_db[ch_idx_1].channel, CH_80_MHz_WIDTH))
					    || (true ==
						acs_IsTheSameGroup(vmacSta_p->acs_db[i].channel, vmacSta_p->acs_db[ch_idx_2].channel,
								   CH_80_MHz_WIDTH))) {
						printk("channel %d is in same group with channel %d or %d. skip\n", vmacSta_p->acs_db[i].channel,
						       vmacSta_p->acs_db[ch_idx_1].channel, vmacSta_p->acs_db[ch_idx_2].channel);
					} else {
	...
	if (band == FREQ_BAND_2DOT4GHZ) {
		avg_nf = avg_nf / (channel_num == 0 ? 1 : channel_num);
		if (*(mib->mib_autochannel)) {
			printk("%s pre_channel %d(%d dbm) selected before G-Band optimization, total %d channels avg_nf=%d\n", mode ? "A" : "G",
			       channel, vmacSta_p->acs_db[channel - 1].noise_floor, channel_num, avg_nf);
		}

		/* ACS mode 0 ignore unusable channel */
		if (vmacSta_p->acs_mode == 0) {
			for (i = 0; i < vmacSta_p->NumScanChannels - 1; i++) {
				if (!vmacSta_p->acs_db[vmacSta_p->acs_2g_sorted_score_idx[vmacSta_p->NumScanChannels - 1 - i]].unusable) {
					channel =
					    vmacSta_p->ScanParams.ChanList[vmacSta_p->acs_2g_sorted_score_idx[vmacSta_p->NumScanChannels - 1 - i]];
					if (*(mib->mib_autochannel)) {
						printk("select channel %d\n", channel);
					}
					break;
				} else {
					printk("ignore channel %d\n",
					       vmacSta_p->ScanParams.ChanList[vmacSta_p->
									      acs_2g_sorted_score_idx[vmacSta_p->NumScanChannels - 1 - i]]);
				}
			}
		}
/*
...
*/
	...
	/*
	 * If channel is not selected for below case in 5Ghz
	 *    1. Only 1 80Mhz band.
	 *    2. Have 2 80Mhz band and 1st and 2nd worst channels are on different 80Mhz Band.
	 */
	if (!channel && (band == FREQ_BAND_5GHZ)) {
		u32 ch_idx_1, ch_idx_2;
		ch_idx_1 = vmacSta_p->worst_channel_idx[0];
		ch_idx_2 = vmacSta_p->worst_channel_idx[1];
		if (true == acs_IsTheSameGroup(vmacSta_p->acs_db[ch_idx_1].channel, vmacSta_p->acs_db[ch_idx_2].channel, CH_80_MHz_WIDTH)) {
			/* if 1st and 2nd worst channel are in same band */
			traffic = 0;
			i = 0;
			while (vmacSta_p->ScanParams.ChanList[i]) {
				if (vmacSta_p->autochannel[i] > traffic) {
			...
			/* try to find highest scored channel in the band where 2nd worst channel is in */
			traffic = 0;
			i = 0;
			while (vmacSta_p->ScanParams.ChanList[i]) {
				if (vmacSta_p->autochannel[i] > traffic) {
	...
	/* if channel is still 0 for unknown reason, just set it to 1st scan channel */
	if (!channel) {
		channel = vmacSta_p->ScanParams.ChanList[0];
	}

	if (*(mib->mib_autochannel)) {
		printk("%s channel %d selected\n", mode ? "A" : "G", channel);
	}
...
#ifdef ERP
extern void macMgmtMlme_IncrBarkerPreambleStnCnt(vmacApInfo_t * vmacSta_p)
{
	MIB_802DOT11 *mib;
	...
	if (*(mib->mib_ApMode) != AP_MODE_B_ONLY && !(*(mib->mib_ApMode) & AP_MODE_A_ONLY)) {
		...
		if (*(mib->mib_ErpProtEnabled) && (vmacSta_p->BarkerPreambleStnCnt == 1)) {
...
extern void macMgmtMlme_DecrBarkerPreambleStnCnt(vmacApInfo_t * vmacSta_p)
{
	...
	if (*(mib->mib_ApMode) != AP_MODE_B_ONLY && !(*(mib->mib_ApMode) & AP_MODE_A_ONLY)) {
		...
		if (*(mib->mib_ErpProtEnabled) && !vmacSta_p->BarkerPreambleStnCnt) {
...
#ifdef CONFIG_IEEE80211W
int parsing_rsn_ie(UINT8 * pData, IEEEtypes_RSN_IE_WPA2_t * pRSNE, UINT8 * mfpc, UINT8 * mfpr)
#else
...
#ifdef CONFIG_IEEE80211W
	memcpy(pRSNE->GrpMgtKeyCipher, ptr, sizeof(pRSNE->GrpMgtKeyCipher));
#endif
...
#ifdef CONFIG_IEEE80211W
BOOLEAN RsnBIPcap(IEEEtypes_RSN_IE_WPA2_t * ie_p, UINT8 * mfpc, UINT8 * mfpr)
{
...
void SAQueryMgmt_TimeoutHdlr(UINT8 * data)
{
	...
	if (StaInfo_p->sa_query_count++ >= 5) {
		...
		macMgmtMlme_SendDisassociateMsg(vmacSta_p, &StaInfo_p->Addr, StaInfo_p->StnId, IEEEtypes_REASON_PRIOR_AUTH_INVALID);
		extStaDb_RemoveSta(vmacSta_p, (IEEEtypes_MacAddr_t *) & StaInfo_p->Addr);
#ifdef CFG80211
		mwl_cfg80211_disassoc_event(vmacSta_p->dev, (uint8_t *) & StaInfo_p->Addr);
#endif
	} else {
		macMgmtMlme_SAQuery(vmacSta_p, &StaInfo_p->Addr, NULL, 0);
		// send SAQuery per 0.2 second as the same hostapd SAQuery implematation
		TimerFireIn(&StaInfo_p->SA_Query_Timer, 1, SAQueryMgmt_TimeoutHdlr, (UINT8 *) StaInfo_p, 2);
	}

}

static BOOLEAN macMgmtMlme_WPA2PMF_Verify(vmacApInfo_t * vmacSta_p, struct sk_buff *txSkb_p, extStaDb_StaInfo_t * StaInfo_p,
					  IEEEtypes_RSN_IE_WPA2_t * RsnIEWPA2_p, UINT8 * reconfig_rsn_ie)
{
	...
	int ret, ret1;
	macmgmtQ_MgmtMsg2_t *MgmtRsp_p = (macmgmtQ_MgmtMsg2_t *) txSkb_p->data;
	...
	*reconfig_rsn_ie = FALSE;
	...
	if ((RsnIEWPA2_p->Len + 2) > MAX_SIZE_RSN_IE_BUF) {
		printk("%s[line %d] out of boundary (%d %d)\n", __FUNCTION__, __LINE__, (RsnIEWPA2_p->Len + 2), MAX_SIZE_RSN_IE_BUF);
		return FALSE;
	} else {
		memcpy(StaInfo_p->keyMgmtStateInfo.RsnIEBuf, RsnIEWPA2_p, RsnIEWPA2_p->Len + 2);
	}
	ret1 = parsing_rsn_ie((UINT8 *) mib->thisStaRsnIEWPA2, &MibRsnIEWPA2, &mfpc, &mfpr);
	ret = parsing_rsn_ie((UINT8 *) RsnIEWPA2_p, &RsnIEWPA2, &mfpc, &mfpr);

	if (RsnIEWPA2.AuthKeyList[3] == 0x08 || RsnIEWPA2.AuthKeyList1[3] == 0x08) {
		if (mfpc == 0 && mfpr == 0) {
			MgmtRsp_p->Body.AssocRsp.StatusCode = IEEEtypes_STATUS_ASSOC_DENIED_INVALID_IE;
	...
	if ((ret > 1) && (ret1 > 1) && (memcmp(mib->thisStaRsnIEWPA2->GrpKeyCipher, RsnIEWPA2.GrpKeyCipher, sizeof(RsnIEWPA2.GrpKeyCipher)))) {
	...
	if ((ret > 2) && (ret1 > 2)
	    && (memcmp(mib->thisStaRsnIEWPA2->PwsKeyCipherList, RsnIEWPA2.PwsKeyCipherList, sizeof(RsnIEWPA2.PwsKeyCipherList)))) {
		MgmtRsp_p->Body.AssocRsp.StatusCode = IEEEtypes_STATUS_ASSOC_DENIED_INVALID_PAIRWISE_CIPHER;
		if (txMgmtMsg(vmacSta_p->dev, txSkb_p) != OS_SUCCESS) {
			wl_free_skb(txSkb_p);
		}
		return FALSE;
	...
	if ((ret > 4) && (ret1 > 4)) {
		if ((MibRsnIEWPA2.AuthKeyCnt[0] == 1) && (RsnIEWPA2.AuthKeyCnt[0] == 1)) {
			if (memcmp(MibRsnIEWPA2.AuthKeyList, RsnIEWPA2.AuthKeyList, sizeof(RsnIEWPA2.AuthKeyList))) {
				MgmtRsp_p->Body.AssocRsp.StatusCode = IEEEtypes_STATUS_ASSOC_DENIED_INVALID_AKMP;
				if (txMgmtMsg(vmacSta_p->dev, txSkb_p) != OS_SUCCESS) {
					wl_free_skb(txSkb_p);
				}
				return FALSE;
	...
	if ((ret == 9) && (ret1 == 9)) {
		if (memcmp(MibRsnIEWPA2.GrpMgtKeyCipher, RsnIEWPA2.GrpMgtKeyCipher, sizeof(RsnIEWPA2.GrpMgtKeyCipher))) {
			MgmtRsp_p->Body.AssocRsp.StatusCode = IEEEtypes_STATUS_ASSOC_DENIED_CIPHER_SUITE_REJECTED;
			if (txMgmtMsg(vmacSta_p->dev, txSkb_p) != OS_SUCCESS) {
				wl_free_skb(txSkb_p);
			}
			return FALSE;
	...
	if ((mfpr > mfpc) || ((mfpc != vmacSta_p->ieee80211w) && (mfpr != vmacSta_p->ieee80211wRequired))) {
		MgmtRsp_p->Body.AssocRsp.StatusCode = IEEEtypes_STATUS_ROBUST_MGMT_FRAME_POLICY_VIOLATION;
		if (txMgmtMsg(vmacSta_p->dev, txSkb_p) != OS_SUCCESS) {
			wl_free_skb(txSkb_p);
		}
		return FALSE;
...
void macMgmtMlme_AssocReAssocReqHandler(vmacApInfo_t * vmacSta_p, macmgmtQ_MgmtMsg3_t * MgmtMsg_p, UINT32 msgSize, UINT32 flag)
{
...
#endif
	IEEEtypes_RSNX_IE_t *RsnxIE_p = NULL;
#ifdef MRVL_80211R
	...
	if (((vmacSta_p->Mib802dot11->WPA2AuthSuites->AuthSuites[3]) == 0x12)) {
//              extra_size += (sizeof(StaInfo_p->AP_DHIEBuf) + sizeof(StaInfo_p->EXT_RsnIE));
	...
	if (wlpptr->wlpd_p->bStopBcnProbeResp && macMgmtMlme_DfsEnabled(vmacSta_p->dev))
		goto OUT;

	if (flag) {
		if ((txSkb_p =
		     mlmeApiPrepMgtMsg2(IEEE_MSG_REASSOCIATE_RSP, &MgmtMsg_p->Hdr.SrcAddr, &MgmtMsg_p->Hdr.DestAddr,
					sizeof(IEEEtypes_AssocRsp_t) - 4 + extra_size)) == NULL)
			goto OUT;
	} else {
		if ((txSkb_p =
		     mlmeApiPrepMgtMsg2(IEEE_MSG_ASSOCIATE_RSP, &MgmtMsg_p->Hdr.SrcAddr, &MgmtMsg_p->Hdr.DestAddr,
					sizeof(IEEEtypes_AssocRsp_t) - 4 + extra_size)) == NULL)
			goto OUT;
	}
	...
	SPIN_LOCK_IRQSAVE(&wlpptr->wlpd_p->locks.probeRspIEsLock, flags);
	...
	if (vmacSta_p->probeRspBody.basic_ies) {
		...
		vmacSta_p->macCapInfo.ShortPreamble = ((IEEEtypes_ProbeRsp_t *) vmacSta_p->probeRspBody.basic_ies)->CapInfo.ShortPreamble;
	}
	SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->locks.probeRspIEsLock, flags);
	...
	frameSize = 30 /* sizeof(struct ieee80211_frame) */  +
	    ...
	    sizeof(IEEEtypes_StatusCode_t) + sizeof(IEEEtypes_AId_t) + sizeof(IEEEtypes_InfoElementHdr_t) + MgmtRsp_p->Body.AssocRsp.SuppRates.Len;
	...
	if (ExtSuppRateSet_p->Len > 0) {
		...
		memcpy((char *)&TxExtRates_p->Rates, (char *)&ExtSuppRateSet_p->Rates, ExtSuppRateSet_p->Len);
	...
	if ((StaInfo_p = extStaDb_GetStaInfo(vmacSta_p, &(MgmtMsg_p->Hdr.SrcAddr), STADB_UPDATE_AGINGTIME)) == NULL ||
	    !(StaInfo_p->State == AUTHENTICATED || StaInfo_p->State == ASSOCIATED) || vmacSta_p->MIC_ErrordisableStaAsso != 0) {
		...
		MgmtRsp_p->Body.AssocRsp.StatusCode = IEEEtypes_STATUS_UNSPEC_FAILURE;
		...
		if (StaInfo_p->pDefragSkBuff) {
			struct except_cnt *wlexcept_p = &wlpptr->wlpd_p->except_cnt;

			wl_free_skb(StaInfo_p->pDefragSkBuff);
			StaInfo_p->pDefragSkBuff = NULL;
			wlexcept_p->cnt_defrag_drop++;
			wlexcept_p->cnt_defrag_drop_x[8]++;
		}
	}
#ifdef MRVL_80211R
...
#endif
#ifdef AP_STEERING_SUPPORT
	memset(&(StaInfo_p->ExtCapElem), 0, sizeof(IEEEtypes_Extended_Cap_Element_t));
#endif				/* AP_STEERING_SUPPORT */

#ifdef IEEE80211H
	StaInfo_p->ListenInterval = ENDIAN_SWAP16(MgmtMsg_p->Body.AssocRqst.ListenInterval);
#endif
#ifdef MRVL_WSC			//MRVL_WSC_IE
...
//      cleanupAmpduTx(vmacSta_p,(UINT8 *)&StaInfo_p->Addr);
	...
	if (flag) {
		...
		VarLen = msgSize -
		    sizeof(IEEEtypes_MgmtHdr3_t) - sizeof(IEEEtypes_CapInfo_t) - sizeof(IEEEtypes_ListenInterval_t) + 2 - sizeof(IEEEtypes_MacAddr_t);
	} else {
		VariableElements_p = (UINT8 *) & MgmtMsg_p->Body.AssocRqst.SsId;
		/* Add 2 bytes to make up for BodyLen, ptr is adjusted to include but
		   message size does not include this. */
		VarLen = msgSize - sizeof(IEEEtypes_MgmtHdr3_t) - sizeof(IEEEtypes_CapInfo_t) - sizeof(IEEEtypes_ListenInterval_t) + 2;
	}
#ifdef MULTI_AP_SUPPORT
	IEBuf = VariableElements_p;
	IELen = VarLen;
	capInfo_p = (UINT8 *) & MgmtMsg_p->Body.AssocRqst.CapInfo;
	listenInterval_p = (UINT8 *) & MgmtMsg_p->Body.AssocRqst.ListenInterval;
#endif
#ifdef ERP
	ExtRates_p = NULL;
#endif
	...
	if (macMgmtMlme_AssocReAssocIESanityCheck(VariableElements_p, VarLen) == FALSE) {
		...
		/* below is to debug assoc problem, will remove if issue is clarified */
		{
			...
			if (dbg_class & 0x80000000)
				print_hex_dump(KERN_INFO, "", DUMP_PREFIX_ADDRESS, 16, 1, MgmtMsg_p, msgSize, true);
		}
		MgmtRsp_p->Body.AssocRsp.StatusCode = IEEEtypes_STATUS_UNSPEC_FAILURE;
		if (txMgmtMsg(vmacSta_p->dev, txSkb_p) != OS_SUCCESS)
			wl_free_skb(txSkb_p);
	...
	if (!flag) {
		/* Assoc Req */
		memcpy(&StaInfo_p->assocReqMsg.Body.AssocRqst, &MgmtRsp_p->Body.AssocRqst, sizeof(IEEEtypes_AssocRqst_t));
		StaInfo_p->assocReqMsg.len = sizeof(IEEEtypes_AssocRqst_t);
	} else {
		/* ReAssoc Req */
		memcpy(&StaInfo_p->assocReqMsg.Body.ReassocRqst, &MgmtRsp_p->Body.ReassocRqst, sizeof(IEEEtypes_ReassocRqst_t));
		StaInfo_p->assocReqMsg.len = sizeof(IEEEtypes_ReassocRqst_t);
	}
	memcpy(&StaInfo_p->assocReqMsg.Hdr, &MgmtRsp_p->Hdr, sizeof(IEEEtypes_MgmtHdr_t));
	StaInfo_p->ClientMode_mix_ext = 0;
	while (VarLen > 0) {
		...
		case SSID:
			...
			VariableElements_p += (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + SsId_p->Len);
			VarLen -= (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + SsId_p->Len);
			break;
		case SUPPORTED_RATES:
			Rates_p = (IEEEtypes_SuppRatesElement_t *) VariableElements_p;
			VariableElements_p += (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + Rates_p->Len);
			VarLen -= (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + Rates_p->Len);
			break;
#ifdef ERP
		case EXT_SUPPORTED_RATES:
			ExtRates_p = (IEEEtypes_ExtSuppRatesElement_t *) VariableElements_p;
			VariableElements_p += (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + ExtRates_p->Len);
			VarLen -= (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + ExtRates_p->Len);
			break;
#endif
#ifdef QOS_FEATURE
		case QOS_CAPABILITY:
			QosCapElem_p = (QoS_Cap_Elem_t *) VariableElements_p;
			VariableElements_p += (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + QosCapElem_p->Len);
			VarLen -= (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + QosCapElem_p->Len);
			break;
		...
		case PROPRIETARY_IE:
			...
			if (!memcmp(&((IEEEtypes_Generic_HT_Element_t *) VariableElements_p)->OUI, B_COMP_OUI, 3)) {
				// B_COMP VHT Cap
				if (((IEEEtypes_Generic_HT_Element_t *) VariableElements_p)->OUIType == 4) {
					if (isVendorSpecVHTIE(VariableElements_p)) {
						IEEEtypes_VhtCap_t *tmp_p =
						    (IEEEtypes_VhtCap_t *) ((IEEEtypes_VendorSpec_VHT_Element_t *) VariableElements_p)->VHTData;
						StaInfo_p->vhtCap.id = tmp_p->id;
						StaInfo_p->vhtCap.len = tmp_p->len;
#ifdef MV_CPU_BE
						StaInfo_p->vhtCap.cap.u32_data = ENDIAN_SWAP32(tmp_p->cap.u32_data);
#else
						memcpy((void *)&StaInfo_p->vhtCap, &tmp_p->id, sizeof(IEEEtypes_VhtCap_t));
#endif
						StaInfo_p->vhtCap.SupportedRxMcsSet = ENDIAN_SWAP32(tmp_p->SupportedRxMcsSet);
						StaInfo_p->vhtCap.SupportedTxMcsSet = ENDIAN_SWAP32(tmp_p->SupportedTxMcsSet);
						...
						vht_RxChannelWidth_IE191 = tmp_p->cap.SupportedChannelWidthSet;

						VendorSpecVHTPresent = 1;
					}
...
#ifdef EXPLICIT_BF
					PeerInfo.TxBFCapabilities = ENDIAN_SWAP32(tmp_p->TxBFCapabilities);
#endif
					if (*(mib->mib_3x3Rate) == 1 && ((*(mib->mib_rxAntenna) == 0) || (*(mib->mib_rxAntenna) == 3))) {
						...
						if (MCSCapEnable) {	/*Enabled through mcscap debug cmd */
							...
							printk("MCSSet0:0x%x MCSSet1:0x%x MCSSet2:0x%x MCSSet3:0x%x \n", tmp_p->SupportedMCSset[0],
							       tmp_p->SupportedMCSset[1], tmp_p->SupportedMCSset[2]
							       , tmp_p->SupportedMCSset[3]);
						}
#endif

						PeerInfo.HTRateBitMap = ENDIAN_SWAP32((tmp_p->SupportedMCSset[0] | (tmp_p->SupportedMCSset[1] << 8) |
										       (tmp_p->SupportedMCSset[2] << 16) | (tmp_p->
															    SupportedMCSset[3] <<
															    24)));
					} else if (!((*(mib->mib_rxAntenna) == 0) || (*(mib->mib_rxAntenna) == 3) || (*(mib->mib_rxAntenna) == 2)))
					{
						...
						PeerInfo.HTRateBitMap = ENDIAN_SWAP32((tmp_p->SupportedMCSset[0]));
					} else {
#ifdef CAP_MAX_RATE
						if (MCSCapEnable) {
							if ((MCSCap < 8) && (tmp_p->SupportedMCSset[0] != 0)) {
					...
					StaInfo_p->HtElem.ExtHTCapabilitiesInfo = ENDIAN_SWAP16(tmp_p->ExtHTCapabilitiesInfo);
#ifdef EXPLICIT_BF
					StaInfo_p->HtElem.TxBFCapabilities = ENDIAN_SWAP32(tmp_p->TxBFCapabilities);
#else
					StaInfo_p->HtElem.TxBFCapabilities = ENDIAN_SWAP16(tmp_p->ExtHTCapabilitiesInfo);
#endif
					...
					amsdu_bitmap = (*(vmacSta_p->Mib802dot11->pMib_11nAggrMode) & WL_MODE_AMSDU_TX_MASK);
					...
					if (StaInfo_p->aggr11n.cap > amsdu_bitmap) {
						...
						WLDBG_INFO(DBG_LEVEL_7, "Mismatched Sta HTCapabilitiesInfo.MaxAMSDUSize=%x",
							   tmp_p->HTCapabilitiesInfo.MaxAMSDUSize);
					}
			...
			if (!memcmp(&((IEEEtypes_Generic_HT_Element_t2 *) VariableElements_p)->OUI, NXP_OUI, 3)) {
				StaInfo_p->nxp_sta = 1;
				VarLen -= (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + *(VariableElements_p + 1));	/* value in the length field */
				VariableElements_p += (sizeof(IEEEtypes_ElementId_t));
				VariableElements_p += *VariableElements_p;
			...
			if (!memcmp(&((IEEEtypes_Generic_HT_Element_t2 *) VariableElements_p)->OUI, MTK_OUI, 3)) {
				StaInfo_p->mtk_sta = 1;
				VarLen -= (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + *(VariableElements_p + 1));	/* value in the length field */
				VariableElements_p += (sizeof(IEEEtypes_ElementId_t));
				VariableElements_p += *VariableElements_p;
...
#ifdef MULTI_AP_SUPPORT
			if (!memcmp(&((IEEEtypes_Generic_HT_Element_t2 *) VariableElements_p)->OUI, M_COMP_OUI, 3)) {
				isMComp_sta = 1;
				VarLen -= (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + *(VariableElements_p + 1));	/* value in the length field */
				VariableElements_p += (sizeof(IEEEtypes_ElementId_t));
				VariableElements_p += *VariableElements_p;
			...
			if (!memcmp(&((IEEEtypes_Generic_HT_Element_t2 *) VariableElements_p)->OUI, MultiAP_OUI, 3)
			    && ((IEEEtypes_Generic_HT_Element_t2 *) VariableElements_p)->OUIType == MultiAP_OUI_type) {
				MultiAP_IE_p = (IEEEtypes_MultiAP_Element_t *) VariableElements_p;

				/* Set MultiAP 4-address flag for STA
				 * 0: normal STA connected to normal AP or fBSS.
				 * 1: bSTA (bSTA bit is 1) connected to bBSS, bSTA will send 4-address data frame.
				 * 2: bSTA (bSTA bit is 1) connected to fBSS, bSTA will send either 3-address or 4 address,
				 *    it depends on STA design.
				 */
				if (MultiAP_IE_p->attributes.BackSTA) {
					if (mib->multi_ap_attr & MAP_ATTRIBUTE_BACKHAUL_BSS)
						StaInfo_p->MultiAP_4addr = 1;
					else if (mib->multi_ap_attr & MAP_ATTRIBUTE_FRONTHAUL_BSS)
						StaInfo_p->MultiAP_4addr = 2;
				}
			...
			if (!memcmp(&((WSM_QoS_Cap_Elem_t *) VariableElements_p)->OUI, WiFiOUI, 3)) {
				if (((WSM_QoS_Cap_Elem_t *) VariableElements_p)->OUI.Type == 0x2) {
					if (((WSM_QoS_Cap_Elem_t *) VariableElements_p)->OUI.Subtype == 0) {
						...
						VariableElements_p += (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + pWMEInfoElem->Len);
						VarLen -= (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + pWMEInfoElem->Len);
						...
						VariableElements_p += (sizeof(IEEEtypes_ElementId_t) +
								       sizeof(IEEEtypes_Len_t) + WsmQosCapElem_p->Len);
						VarLen -= (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + WsmQosCapElem_p->Len);
					} else {
						InfoElemHdr_p = (IEEEtypes_InfoElementHdr_t *) VariableElements_p;
						VariableElements_p += (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + InfoElemHdr_p->Len);
						VarLen -= (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + InfoElemHdr_p->Len);
					}
					break;
					...
					VariableElements_p += (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + RsnIE_p->Len);
					VarLen -= (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + RsnIE_p->Len);
					break;
				} else {
					InfoElemHdr_p = (IEEEtypes_InfoElementHdr_t *) VariableElements_p;
					VariableElements_p += (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + InfoElemHdr_p->Len);
			...
			if (memcmp(((IEEEtypes_RSN_IE_t *) VariableElements_p)->OuiType, WPA_OUItype, 4) == 0) {
				...
				VariableElements_p += (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + RsnIE_Len);
				VarLen -= (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + RsnIE_Len);
			}
		...
		case RSN_IEWPA2:
			...
			VariableElements_p += (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + RsnIEWPA2_p->Len);
			VarLen -= (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + RsnIEWPA2_p->Len);
			break;
		case RSNX_IE:
			RsnxIE_p = (IEEEtypes_RSNX_IE_t *) VariableElements_p;
			VariableElements_p += (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + RsnxIE_p->Len);
			VarLen -= (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + RsnxIE_p->Len);
			break;
#ifdef MRVL_80211R
		case MD_IE:
			MDIE_p = (IEEEtypes_MOBILITY_DOMAIN_IE_t *) VariableElements_p;
			VariableElements_p += (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + MDIE_p->Len);
			VarLen -= (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + MDIE_p->Len);
			break;

		case FT_IE:
			FTIE_p = (UINT8 *) VariableElements_p;
			VariableElements_p += (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + FTIE_p[1]);
			VarLen -= (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + FTIE_p[1]);
			break;
#endif
#endif
		...
		case WAPI_IE:
			...
			VariableElements_p += (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + WAPI_IE_p->Len);
			VarLen -= (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + WAPI_IE_p->Len);
			break;
#endif
		...
		case PWR_CAP:
			...
			VariableElements_p += (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + PowerCapability_p->Len);
			VarLen -= (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + PowerCapability_p->Len);
			break;

		case SUPPORTED_CHANNEL:
			SupportedChanne_p = (IEEEtypes_SupportedChannelElement_t *) VariableElements_p;
			VariableElements_p += (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + SupportedChanne_p->Len);
			VarLen -= (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + SupportedChanne_p->Len);
			break;
#endif				/* IEEE80211H */
		case HT:
			{
		...
		case VHT_CAP:
			if (*(vmacSta_p->Mib802dot11->mib_ApMode) & AP_MODE_11AC) {
				...
				for (i = 0; i < 8; i++) {
					if (((tmp_p->SupportedRxMcsSet >> i * 2) & 0x3) == 0x3) {
						tmp_p->SupportedRxMcsSet = (tmp_p->SupportedRxMcsSet | (0xFFFF << i * 2)) & 0xFFFF;
						break;
					}
				}
		...
		case VHT_OPERATION:
			{
				...
				vht_center_freq0_IE192 = tmp_p->center_freq0;
				vht_center_freq1_IE192 = tmp_p->center_freq1;
			}
			IE192Present = 1;
			VarLen -= (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + *(VariableElements_p + 1));	/* value in the length field */
			VariableElements_p += (sizeof(IEEEtypes_ElementId_t));
		...
		case EXT_IE:
			{
				IEEEtypes_InfoElementExtHdr_t *tmp_p = (IEEEtypes_InfoElementExtHdr_t *) VariableElements_p;
				u8 ie_length;

				ie_length = tmp_p->Len + sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t);
				...
				case HE_CAPABILITIES_IE:
					if (*(vmacSta_p->Mib802dot11->mib_ApMode) & AP_MODE_11AX) {
						...
						if (ie_length <= sizeof(PeerInfo.he_cap))
							memcpy((void *)&PeerInfo.he_cap, (void *)tmp_p, ie_length);

						StaInfo_p->he_cap_ie = tmp_p->ext;

						ptr = (u8 *) & tmp_p->ext;
						ptr += sizeof(tmp_p->ext);
						memcpy((u8 *) & StaInfo_p->he_mac_cap, ptr, sizeof(HE_Mac_Capabilities_Info_t));
						ptr += sizeof(HE_Mac_Capabilities_Info_t);
						memcpy((u8 *) & StaInfo_p->he_phy_cap, ptr, sizeof(HE_Phy_Capabilities_Info_t));
						//printk("found HE_CAPABILITIES_IE\n");

						//ptr = &StaInfo_p->he_phy_cap;
						if (wfa_11ax_pf) {
							if (WFA_PeerInfo_HE_CAP_1NSS) {
								PeerInfo.he_cap.rx_he_mcs_80m.max_mcs_set =
								    PeerInfo.he_cap.rx_he_mcs_80m.max_mcs_set | 0xfffc;
								PeerInfo.he_cap.tx_he_mcs_80m.max_mcs_set =
								    PeerInfo.he_cap.tx_he_mcs_80m.max_mcs_set | 0xfffc;
							}
						}
						memcpy((u8 *) & StaInfo_p->heCap, (u8 *) & PeerInfo.he_cap, sizeof(PeerInfo.he_cap));
					}
					he_cap_present = 1;
					break;
				case HE_OPERATION_IE:
					if (*(vmacSta_p->Mib802dot11->mib_ApMode) & AP_MODE_11AX) {
						if (ie_length <= sizeof(PeerInfo.he_op))
		...
		case EXT_CAP_IE:
			{
				IEEEtypes_Extended_Cap_Element_t *tmp_p = (IEEEtypes_Extended_Cap_Element_t *) VariableElements_p;

				memcpy(&(StaInfo_p->ExtCapElem), tmp_p, sizeof(IEEEtypes_Extended_Cap_Element_t));
				VarLen -= (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + *(VariableElements_p + 1));	/* value in the length field */
				VariableElements_p += (sizeof(IEEEtypes_ElementId_t));
				VariableElements_p += *VariableElements_p;
				VariableElements_p += sizeof(IEEEtypes_Len_t);
		...
		case RRM_CAP_IE:
			{
				struct IEEEtypes_RM_Enable_Capabilities_Element_t *tmp_p =
				    (IEEEtypes_RM_Enable_Capabilities_Element_t *) VariableElements_p;

				memcpy(&(StaInfo_p->RRM_Cap_IE), tmp_p, sizeof(IEEEtypes_RM_Enable_Capabilities_Element_t));
				VarLen -= (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + *(VariableElements_p + 1));	/* value in the length field */
				VariableElements_p += (sizeof(IEEEtypes_ElementId_t));
				VariableElements_p += *VariableElements_p;
				VariableElements_p += sizeof(IEEEtypes_Len_t);
...
#ifdef CONFIG_IEEE80211W
	if ((StaInfo_p->Ieee80211wSta && StaInfo_p->Aid) && !((FTIE_p != NULL) && (flag))) {
		if (!StaInfo_p->sa_query_timed_out) {
			IEEEtypes_TimeoutInterval_Element_t *TIe_p = (IEEEtypes_TimeoutInterval_Element_t *) ((UINT8 *) MgmtRsp_p + frameSize);

			MgmtRsp_p->Body.AssocRsp.StatusCode = IEEEtypes_STATUS_ASSOC_REJECTED_TEMPORARILY;
			TIe_p->ElementId = 56;
			TIe_p->Len = 5;
			TIe_p->TIType = 3;
			TIe_p->TIValue = vmacSta_p->assoc_sa_query_max_timeout;
			frameSize += sizeof(IEEEtypes_InfoElementHdr_t) + TIe_p->Len;
			/* Send for tx */
			skb_trim(txSkb_p, frameSize);
			...
			if (StaInfo_p->sa_query_count == 0) {
				...
				StaInfo_p->sa_query_start_time = xxGetTimeStamp();
				TimerInit(&StaInfo_p->SA_Query_Timer);
				// send SAQuery per 0.2 second as the same hostapd SAQuery implematation
				SAQueryMgmt_TimeoutHdlr((UINT8 *) StaInfo_p);
				goto OUT;
			} else {
				UINT32 now = xxGetTimeStamp();
				UINT32 delta_time;

				delta_time = (now > StaInfo_p->sa_query_start_time) ?
				    (now - StaInfo_p->sa_query_start_time) : ((0xFFFFFFFF - StaInfo_p->sa_query_start_time) + now);
				TIe_p->TIValue = vmacSta_p->assoc_sa_query_max_timeout - delta_time / 1024;

				if (txMgmtMsg(vmacSta_p->dev, txSkb_p) != OS_SUCCESS)
					wl_free_skb(txSkb_p);

				WLDBG_INFO(DBG_LEVEL_7, "SAQueryMgmt_TimeoutHdlr is working and need to wait for timeout (%p)\n", StaInfo_p);
				goto OUT;
			}
		} else {
			if (StaInfo_p->ptkCipherOuiType != CIPHER_OUI_TYPE_NONE) {
				macMgmtMlme_SendDisassociateMsg(vmacSta_p, &(MgmtMsg_p->Hdr.SrcAddr), StaInfo_p->StnId,
	...
	if ((mib->multi_ap_attr & MAP_ATTRIBUTE_BACKHAUL_BSS) && !(mib->multi_ap_attr & MAP_ATTRIBUTE_FRONTHAUL_BSS)) {
		if ((MultiAP_IE_p == NULL) || ((MultiAP_IE_p != NULL) && !MultiAP_IE_p->attributes.BackSTA)) {
			skb_trim(txSkb_p, frameSize);
	...
	if (mib->multi_ap_attr & MAP_ATTRIBUTE_BACKHAUL_BSS) {
		if (mib->multi_ap_attr & MAP_ATTRIBUTE_R1BSTA_DISALLOWED) {
			if ((MultiAP_IE_p == NULL) ||
			    ((MultiAP_IE_p != NULL) &&
			     (MultiAP_IE_p->attributes.BackSTA == 1) &&
			     ((MultiAP_IE_p->Len == MAP_R1_IE_LEN) ||
			      ((MultiAP_IE_p->Len >= (MAP_R1_IE_LEN + 3)) &&
			       (((IEEEtypes_MultiAP_Version_t *) MultiAP_IE_p->variable)->ElementId == 0x07) &&
			       (((IEEEtypes_MultiAP_Version_t *) MultiAP_IE_p->variable)->Len == 0x01) &&
			       (((IEEEtypes_MultiAP_Version_t *) MultiAP_IE_p->variable)->value == 0x01))))) {
				skb_trim(txSkb_p, frameSize);
		...
		if (mib->multi_ap_attr & MAP_ATTRIBUTE_R2BSTA_DISALLOWED) {
			if ((MultiAP_IE_p == NULL) ||
			    ((MultiAP_IE_p != NULL) &&
			     (MultiAP_IE_p->attributes.BackSTA == 1) &&
			     ((MultiAP_IE_p->Len >= (MAP_R1_IE_LEN + 3)) &&
			      (((IEEEtypes_MultiAP_Version_t *) MultiAP_IE_p->variable)->ElementId == 0x07) &&
			      (((IEEEtypes_MultiAP_Version_t *) MultiAP_IE_p->variable)->Len == 0x01) &&
			      (((IEEEtypes_MultiAP_Version_t *) MultiAP_IE_p->variable)->value >= 0x02)))) {
				skb_trim(txSkb_p, frameSize);
	...
	if (IE199Present) {
		...
		if (IE192Present) {
			...
			case 1:
				...
				if (1 ==
				    (ret =
				     isSupport160MhzByCenterFreq(wlpptr, VHT_EXTENDED_NSS_BW_CAPABLE, vht_center_freq0_IE192, vht_center_freq1_IE192,
								 PeerInfo.AddHtInfo.OpMode.center_freq2))) {
			...
			if (PhyDSSSTable->Chanflag.FreqBand == FREQ_BAND_2DOT4GHZ) {
				...
				if ((vht_RxChannelWidth_IE191 == 1) || (vht_RxChannelWidth_IE191 == 2)) {
					PeerInfo.vht_RxChannelWidth = 3;

					/* peer STA doesn't support 80+80MHz */
					if ((PhyDSSSTable->Chanflag.ChnlWidth != CH_160_MHz_WIDTH) && (vht_RxChannelWidth_IE191 == 1)
					    && !(PeerInfo.vht_cap >> 30))
						PeerInfo.vht_RxChannelWidth = 2;
				} else
	...
	if (IE191Present) {
		...
		StaInfo_p->vht_RxChannelWidth = PeerInfo.vht_RxChannelWidth;
		StaInfo_p->vht_peer_RxNss = vht_peer_RxNss;
	...
	if (he_cap_present) {
		vht_peer_RxNss = 1;
		if ((PeerInfo.he_cap.rx_he_mcs_80m.max_mcs_set & 0xc000) != 0xc000)
			vht_peer_RxNss = 8;
		else if ((PeerInfo.he_cap.rx_he_mcs_80m.max_mcs_set & 0x3000) != 0x3000)
			vht_peer_RxNss = 7;
		else if ((PeerInfo.he_cap.rx_he_mcs_80m.max_mcs_set & 0xc00) != 0xc00)
			vht_peer_RxNss = 6;
		else if ((PeerInfo.he_cap.rx_he_mcs_80m.max_mcs_set & 0x300) != 0x300)
			vht_peer_RxNss = 5;
		else if ((PeerInfo.he_cap.rx_he_mcs_80m.max_mcs_set & 0xc0) != 0xc0)
			vht_peer_RxNss = 4;
		else if ((PeerInfo.he_cap.rx_he_mcs_80m.max_mcs_set & 0x30) != 0x30)
			vht_peer_RxNss = 3;
		else if ((PeerInfo.he_cap.rx_he_mcs_80m.max_mcs_set & 0xc) != 0xc)
			vht_peer_RxNss = 2;
		StaInfo_p->vht_peer_RxNss = vht_peer_RxNss;
	}
	...
	if (!allow_ht_tkip && ((HTpresent) && ((*(vmacSta_p->Mib802dot11->mib_wpaWpa2Mode) & 0x0F) == 1))) {
	...
	if (!allow_ht_tkip && ((HTpresent) &&
			       (((*(vmacSta_p->Mib802dot11->mib_wpaWpa2Mode) & 0x0F) == 3)
				|| ((*(vmacSta_p->Mib802dot11->mib_wpaWpa2Mode) & 0x0F) == 6)) && (RsnIE_p != NULL)
			       && (RsnIE_p->PwsKeyCipherList[3] == RSN_TKIP_ID))) {
		MgmtRsp_p->Body.AssocRsp.StatusCode = IEEEtypes_STATUS_CAPS_UNSUPPORTED;
		/* Send for tx */
		if (txMgmtMsg(vmacSta_p->dev, txSkb_p) != OS_SUCCESS)
			wl_free_skb(txSkb_p);
	...
	if (!(isSsIdMatch(SsId_p, &vmacSta_p->macSsId) == OS_SUCCESS || isSsIdMatch(SsId_p, &vmacSta_p->macSsId2) == OS_SUCCESS)) {
	...
	if (isCapInfoSupported(&MgmtMsg_p->Body.AssocRqst.CapInfo, &vmacSta_p->macCapInfo) != OS_SUCCESS) {
...
#ifdef SOC_W906X
	if (*(vmacSta_p->Mib802dot11->mib_ApMode) == AP_MODE_5GHZ_11AX_ONLY) {
		if (!he_cap_present) {
			/* Build ASSOC_RSP msg with REFUSED status code */
			MgmtRsp_p->Body.AssocRsp.StatusCode = IEEEtypes_STATUS_ASSOC_DENIED_UNSPEC;
			/* Send for tx */
	...
	if (Rates_p) {
		PeerInfo.LegacyRateBitMap = ENDIAN_SWAP32(GetLegacyRateBitMap(vmacSta_p, Rates_p, ExtRates_p));
		...
		MgmtRsp_p->Body.AssocRsp.StatusCode = IEEEtypes_STATUS_ASSOC_DENIED_RATES;
	...
	if (StaInfo_p->ClientMode == NONLY_MODE) {
		...
		} else if ((*(mib->mib_ApMode) == AP_MODE_BandGandN) || (*(mib->mib_ApMode) == AP_MODE_2_4GHZ_11AC_MIXED) ||
#ifdef SOC_W906X
			   (*(mib->mib_ApMode) == AP_MODE_2_4GHZ_Nand11AX) || (*(mib->mib_ApMode) == AP_MODE_2_4GHZ_11AX_MIXED) ||
#endif
			   (*(mib->mib_ApMode) == AP_MODE_MIXED) || (*(mib->mib_ApMode) == AP_MODE_GandN)) {
			StaInfo_p->ClientMode = GONLY_MODE;
		} else if ((*(mib->mib_ApMode) == AP_MODE_AandN) ||
			   (*(mib->mib_ApMode) == AP_MODE_5GHZ_Nand11AC) || (*(mib->mib_ApMode) == AP_MODE_5GHZ_11AC_ONLY)
#ifdef SOC_W906X
	...
	if (!gRatePresent && (*(mib->mib_ApMode) == AP_MODE_G_ONLY ||
			      ...
			      *(mib->mib_ApMode) == AP_MODE_5GHZ_NandACand11AX || *(mib->mib_ApMode) == AP_MODE_5GHZ_11AX_ONLY))
#else
	if (!gRatePresent) {	/* only 11b supported rate */
		if (*(mib->mib_ApMode) == AP_MODE_G_ONLY ||
		    *(mib->mib_ApMode) == AP_MODE_A_ONLY ||
		    *(mib->mib_ApMode) == AP_MODE_N_ONLY || *(mib->mib_ApMode) == AP_MODE_GandN || *(mib->mib_ApMode) == AP_MODE_AandN ||
#ifdef SOC_W8864
		    *(mib->mib_ApMode) == AP_MODE_5GHZ_Nand11AC || *(mib->mib_ApMode) == AP_MODE_5GHZ_11AC_ONLY
#endif
		    ) {
		...
		if (*(mib->mib_ApMode) == AP_MODE_B_ONLY ||
		    *(mib->mib_ApMode) == AP_MODE_N_ONLY || *(mib->mib_ApMode) == AP_MODE_5GHZ_11AC_ONLY || *(mib->mib_ApMode) == AP_MODE_11AC) {
			/* Build ASSOC_RSP msg with basic rates not supported */
			MgmtRsp_p->Body.AssocRsp.StatusCode = IEEEtypes_STATUS_ASSOC_DENIED_RATES;
		...
		if (*(mib->mib_ApMode) == AP_MODE_B_ONLY ||
		    ...
		    *(mib->mib_ApMode) == AP_MODE_A_ONLY || *(mib->mib_ApMode) == AP_MODE_AandG || *(mib->mib_ApMode) == AP_MODE_5GHZ_11AC_ONLY) {
			/* Build ASSOC_RSP msg with basic rates not supported */
			MgmtRsp_p->Body.AssocRsp.StatusCode = IEEEtypes_STATUS_ASSOC_DENIED_RATES;
		...
		if (*(mib->mib_ApMode) == AP_MODE_A_ONLY || *(mib->mib_ApMode) == AP_MODE_N_ONLY)
#endif				/* SOC_W906X */
		{
			/* Build ASSOC_RSP msg with basic rates not supported */
			MgmtRsp_p->Body.AssocRsp.StatusCode = IEEEtypes_STATUS_ASSOC_DENIED_RATES;
...
#endif
#ifdef IEEE80211H
	if ((*(mib->mib_ApMode) == AP_MODE_A_ONLY) || ((*(mib->mib_ApMode) == AP_MODE_AandG) && (StaInfo_p->ApMode == AONLY_MODE)))
	...
	if ((mib_PrivacyTable_p->RSNEnabled && !mib_RSNConfigWPA2_p->WPA2Enabled && !mib_RSNConfigWPA2_p->WPA2OnlyEnabled
	     && (RsnIE_p != NULL) && (memcmp(RsnIE_p->OuiType, &(mib->thisStaRsnIE->OuiType), mib->thisStaRsnIE->Len) != 0))
	...
	if (mib_PrivacyTable_p->RSNEnabled) {
		...
		if (RsnxIE_p != NULL) {
			if ((sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + RsnxIE_p->Len) > MAX_SIZE_RSNX_IE_BUF)
				OUT_OF_BOUNDARDY_MESSAGE((RsnxIE_p->Len + 2), MAX_SIZE_RSNX_IE_BUF);
			else
				memcpy(StaInfo_p->RsnxIE_Buf, RsnxIE_p, (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + RsnxIE_p->Len));
			/* mwl_cfg80211_hex_dump("Assoc Req: rsnx_ie", StaInfo_p->RsnxIE_Buf, RsnxIE_p->Len + 2); */
		}
		...
		if (mib_RSNConfigWPA2_p->WPA2OnlyEnabled) {
			if (RsnIEWPA2_p != NULL) {
				...
				ret = parsing_rsn_ie((UINT8 *) RsnIEWPA2_p, &RsnIEWPA2, &mfpc, &mfpr);
#else
				ret1 = parsing_rsn_ie((UINT8 *) mib->thisStaRsnIEWPA2, &MibRsnIEWPA2);
				...
				if ((ret > 1) && (ret1 > 1) && (memcmp(mib->thisStaRsnIEWPA2->GrpKeyCipher, RsnIEWPA2.GrpKeyCipher,
				...
				if ((ret > 2) && (ret1 > 2) && (memcmp(mib->thisStaRsnIEWPA2->PwsKeyCipherList, RsnIEWPA2.PwsKeyCipherList,
				...
				if ((ret > 1) &&
				    (RsnIEWPA2_p->GrpKeyCipher[3] != RSN_AES_ID) &&
				    (RsnIEWPA2_p->GrpKeyCipher[3] != RSN_GCMP_ID) &&
				    (RsnIEWPA2_p->GrpKeyCipher[3] != RSN_GCMP_256_ID) && (RsnIEWPA2_p->GrpKeyCipher[3] != RSN_CCMP_256_ID)) {
					MgmtRsp_p->Body.AssocRsp.StatusCode = IEEEtypes_STATUS_ASSOC_DENIED_INVALID_GRP_CIPHER;
			...
			if (RsnIEWPA2_p != NULL) {
				...
				if (HTpresent == 0 && RsnIEWPA2_p->GrpKeyCipher[3] != RSN_TKIP_ID) {
					MgmtRsp_p->Body.AssocRsp.StatusCode = IEEEtypes_STATUS_ASSOC_DENIED_INVALID_GRP_CIPHER;
					if (txMgmtMsg(vmacSta_p->dev, txSkb_p) != OS_SUCCESS)
						wl_free_skb(txSkb_p);
					goto OUT;
				}
				if (macMgmtMlme_WPA2PMF_Verify(vmacSta_p, txSkb_p, StaInfo_p, RsnIEWPA2_p, &reconfig_rsn_ie) == FALSE) {
					goto OUT;
				}
				...
				if (HTpresent == 0 && RsnIE_p->GrpKeyCipher[3] != RSN_TKIP_ID) {
					MgmtRsp_p->Body.AssocRsp.StatusCode = IEEEtypes_STATUS_ASSOC_DENIED_INVALID_GRP_CIPHER;
					if (txMgmtMsg(vmacSta_p->dev, txSkb_p) != OS_SUCCESS)
						wl_free_skb(txSkb_p);
					goto OUT;
		...
		if (MDIE_p != NULL) {
			if ((MDIE_p->Len + 2) > MAX_SIZE_MDIE_BUF)
				...
				memcpy(StaInfo_p->keyMgmtStateInfo.mdie_buf, MDIE_p, (MDIE_p->Len + 2));
			if (FTIE_p != NULL) {
				if ((FTIE_p[1] + 2) > MAX_SIZE_FTIE_BUF)
					OUT_OF_BOUNDARDY_MESSAGE((FTIE_p[1] + 2), MAX_SIZE_FTIE_BUF);
				else
					memcpy(StaInfo_p->keyMgmtStateInfo.ftie_buf, FTIE_p, (FTIE_p[1] + 2));
			}
			assoc_pending = 1;
		}
		if (RsnIEWPA2_p != NULL) {
			...
			memset((void *)&RsnIEWPA2, 0, sizeof(IEEEtypes_RSN_IE_WPA2_t));
			memcpy((void *)&RsnIEWPA2, (void *)RsnIEWPA2_p, (RsnIEWPA2_p->Len + 2));
			if (RsnIEWPA2.PMKIDCnt[0] != 0 && RsnIEWPA2.PMKIDCnt[1] != 0)
				assoc_pending = 1;
		}
#endif
		...
		if (DH_IE_p) {
			...
			memcpy(StaInfo_p->STA_DHIEBuf, DH_IE_p, (sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + *(DH_IE_p + 1)));
		}
#endif				/* OWE_SUPPORT */
	}
#ifdef SOC_W906X

	tempbuffer = wl_kzalloc(tempbuffer_len, GFP_ATOMIC);
	if (!tempbuffer)
		goto OUT;
	...
	if (vmacSta_p->macCapInfo.Rrm) {
		...
		memset(&RRM_ie, 0, sizeof(struct IEEEtypes_RM_Enable_Capabilities_Element_t));
	...
	if (staIdx >= sta_num) {
		skb_trim(txSkb_p, frameSize);
		MgmtRsp_p->Body.AssocRsp.StatusCode = IEEEtypes_STATUS_ASSOC_DENIED_BUSY;
	...
	for (i = 0; i < MAX_UP; i++)
		...
		wlpptr->wlpd_p->AmpduPckReorder[StaInfo_p->StnId].AddBaReceive[i] = FALSE;/** clear Ba flag **/
#else
...
/** we are at 20MHz mode **/
	...
	if (HTpresent) {
		...
		if (skb_tailroom(txSkb_p) > 22 /** len of add generic ht **/  + 2) {
			...
			StaInfo_p->ClientMode_mix_ext = NONLY_MODE;
		...
		if (*(vmacSta_p->ShadowMib802dot11->mib_HT40MIntoler)) {
			if (PhyDSSSTable->Chanflag.FreqBand == FREQ_BAND_2DOT4GHZ
				...
				if (skb_tailroom(txSkb_p) > 14 /** len of OBSS Scan Parameter IE **/  + 2) {
					...
					extern UINT16 AddOverlap_BSS_Scan_Parameters_IE(vmacApInfo_t * vmacSta_p,
											IEEEtypes_OVERLAP_BSS_SCAN_PARAMETERS_Element_t *
											pNextElement);

					pOverLapBSS =
					    (IEEEtypes_OVERLAP_BSS_SCAN_PARAMETERS_Element_t *) skb_put(txSkb_p,
													14 /** len of OBSS Scan Parameter IE **/  +
													2);
				...
				if (skb_tailroom(txSkb_p) > sizeof(IEEEtypes_Extended_Cap_Element_t)) {
					...
					pExtCap = (IEEEtypes_Extended_Cap_Element_t *) skb_put(txSkb_p, sizeof(IEEEtypes_Extended_Cap_Element_t));
					memset((void *)pExtCap, 0, sizeof(IEEEtypes_Extended_Cap_Element_t));
					AddExtended_Cap_IE(vmacSta_p, pExtCap);
					addedExtcap = TRUE;

				} else if (skb_headroom(txSkb_p) > sizeof(IEEEtypes_Extended_Cap_Element_t)) {
					IEEEtypes_Extended_Cap_Element_t *pExtCap;
					extern UINT16 AddExtended_Cap_IE(vmacApInfo_t * vmacSta_p, IEEEtypes_Extended_Cap_Element_t * pNextElement);
					...
					AddExtended_Cap_IE(vmacSta_p, pExtCap);
					memset(&txSkb_p->data[0], 0, txSkb_p->len);
					skb_push(txSkb_p, sizeof(IEEEtypes_Extended_Cap_Element_t));
					memcpy(&txSkb_p->data[0], &tempbuffer[0], txSkb_p->len);
					addedExtcap = TRUE;
				} else {
		...
		if (*(vmacSta_p->ShadowMib802dot11->wls_ftm_enable) == 1) {
			extern UINT16 AddRRM_Cap_IE(vmacApInfo_t * vmacSta_p, IEEEtypes_RM_Enable_Capabilities_Element_t * pNextElement);
			IEEEtypes_RM_Enable_Capabilities_Element_t *pRMElement;
			if (skb_tailroom(txSkb_p) > sizeof(IEEEtypes_RM_Enable_Capabilities_Element_t)) {
				pRMElement =
				    (IEEEtypes_RM_Enable_Capabilities_Element_t *) skb_put(txSkb_p,
											   sizeof(IEEEtypes_RM_Enable_Capabilities_Element_t));
				AddRRM_Cap_IE(vmacSta_p, pRMElement);
			} else if (skb_headroom(txSkb_p) > sizeof(IEEEtypes_RM_Enable_Capabilities_Element_t)) {
				//we have enough at the head
				memcpy(&tempbuffer[0], &txSkb_p->data[0], txSkb_p->len);
				pRMElement = (IEEEtypes_RM_Enable_Capabilities_Element_t *) & tempbuffer[txSkb_p->len];
				AddRRM_Cap_IE(vmacSta_p, pRMElement);
				memset(&txSkb_p->data[0], 0, txSkb_p->len);
				skb_push(txSkb_p, sizeof(IEEEtypes_RM_Enable_Capabilities_Element_t));
				memcpy(&txSkb_p->data[0], &tempbuffer[0], txSkb_p->len);
			} else
		...
		if (!addedExtcap && Is5GBand(*(vmacSta_p->Mib802dot11->mib_ApMode)) &&
		    ((*(vmacSta_p->Mib802dot11->mib_ApMode) & AP_MODE_11AX) || (*(vmacSta_p->Mib802dot11->mib_ApMode) & AP_MODE_11AC))) {
			/** add Extended Cap IE here **/
			if (skb_tailroom(txSkb_p) > sizeof(IEEEtypes_Extended_Cap_Element_t)) {
				extern UINT16 AddExtended_Cap_IE(vmacApInfo_t * vmacSta_p, IEEEtypes_Extended_Cap_Element_t * pNextElement);
				IEEEtypes_Extended_Cap_Element_t *pExtCap;
	...
	if (!he_cap_present || !(*(mib->mib_ApMode) & AP_MODE_11AX)) {
		UINT8 ie_len = 0;
		UINT32 len_before_supprates = sizeof(MgmtRsp_p->Body.AssocRsp.CapInfo) +
		    sizeof(MgmtRsp_p->Body.AssocRsp.StatusCode) + sizeof(MgmtRsp_p->Body.AssocRsp.AId);
		UINT8 *ie = FindIEWithinIEs(txSkb_p->data + sizeof(MgmtRsp_p->Hdr) + len_before_supprates,
					    txSkb_p->len - sizeof(MgmtRsp_p->Hdr) - len_before_supprates, EXT_CAP_IE, NULL);
		if (ie) {
			ie_len = *((UINT8 *) (ie + 1));
			if (ie_len == 11) {	/* 11 = sizeof(IEEEtypes_Extended_Cap_Element_t) - 2 */
				UINT8 *ie_end = ie + 2 + ie_len;
				UINT8 *ie_end_new = ie_end - (11 - 8);
				memmove(ie_end_new, ie_end, skb_tail_pointer(txSkb_p) - ie_end);
				*(ie + 1) = 8;
				txSkb_p->tail -= (11 - 8);
				txSkb_p->len -= (11 - 8);
			}
	...
	if (*(vmacSta_p->Mib802dot11->mib_ApMode) & AP_MODE_11AC) {
		if (IE191Present) {
			if (skb_tailroom(txSkb_p) > sizeof(IEEEtypes_VhtCap_t)) {
				...
				if (IE_p != NULL) {
					UINT8 vhtRxNss = StaInfo_p->vht_peer_RxNss;
					UINT8 isEffective = 0;

					if (IE199Present)
						vhtRxNss = min(StaInfo_p->vht_peer_RxNss, vht_RxNss_IE199);
					...
					isEffective = (!he_cap_present && vhtRxNss);
					Build_IE_191(vmacSta_p, (UINT8 *) IE_p, isEffective, vhtRxNss);
				}
			} else {
		...
		if ((IE199Present || VendorSpecVHTPresent) && !IE191Present) {
			if (skb_tailroom(txSkb_p) > sizeof(IEEEtypes_VhtCap_t)) {
				//We have enough room at the tail
				IE_p = skb_put(txSkb_p, sizeof(IEEEtypes_VhtCap_t));
				if (IE_p != NULL) {
					...
					if (!VendorSpecVHTPresent) {
						...
						memcpy(&StaInfo_p->vhtCap, (UINT8 *) IE_p, sizeof(IEEEtypes_VhtCap_t));
						vhtRxNss = min(vhtRxNss, vht_RxNss_IE199);
						switch (vhtRxNss) {
						case 1:
							StaInfo_p->vhtCap.SupportedTxMcsSet = 0xfffe;
							StaInfo_p->vhtCap.SupportedRxMcsSet = 0xfffe;
							break;
						case 2:
							StaInfo_p->vhtCap.SupportedTxMcsSet = 0xfffa;
							StaInfo_p->vhtCap.SupportedRxMcsSet = 0xfffa;
							break;
						case 3:
							StaInfo_p->vhtCap.SupportedTxMcsSet = 0xffea;
							StaInfo_p->vhtCap.SupportedRxMcsSet = 0xffea;
							break;
						case 4:
							StaInfo_p->vhtCap.SupportedTxMcsSet = 0xffaa;
							StaInfo_p->vhtCap.SupportedRxMcsSet = 0xffaa;
							break;
						case 5:
							StaInfo_p->vhtCap.SupportedTxMcsSet = 0xfeaa;
							StaInfo_p->vhtCap.SupportedRxMcsSet = 0xfeaa;
							break;
						case 6:
							StaInfo_p->vhtCap.SupportedTxMcsSet = 0xfaaa;
							StaInfo_p->vhtCap.SupportedRxMcsSet = 0xfaaa;
							break;
						case 7:
							StaInfo_p->vhtCap.SupportedTxMcsSet = 0xeaaa;
							StaInfo_p->vhtCap.SupportedRxMcsSet = 0xeaaa;
							break;
						case 8:
							StaInfo_p->vhtCap.SupportedTxMcsSet = 0xaaaa;
							StaInfo_p->vhtCap.SupportedRxMcsSet = 0xaaaa;
							break;
						default:
							printk("Invalid RxNSS: %u\n", vhtRxNss);
							break;
						}
						PeerInfo.vht_MaxRxMcs = StaInfo_p->vhtCap.SupportedRxMcsSet;
						memcpy((UINT8 *) & PeerInfo.vht_cap, (UINT8 *) & StaInfo_p->vhtCap.cap,
		...
		if (IE191Present || IE192Present) {
			if (skb_tailroom(txSkb_p) > sizeof(IEEEtypes_VhOpt_t)) {
				...
				StaInfo_p->ClientMode_mix_ext = AC_MODE;
			} else {
	...
	if (*(vmacSta_p->Mib802dot11->mib_ApMode) & AP_MODE_11AX) {
		if (he_cap_present) {
			...
			if (!IE192Present) {
				if (PhyDSSSTable->Chanflag.FreqBand == FREQ_BAND_2DOT4GHZ) {
					if (PeerInfo.he_cap.phy_cap.channel_width_set & HE_SUPPORT_40MHZ_BW_24G)
						PeerInfo.vht_RxChannelWidth = 1;
					else
						PeerInfo.vht_RxChannelWidth = 0;
				} else {
					if (PeerInfo.he_cap.phy_cap.channel_width_set & HE_SUPPORT_160MHZ_BW_5G) {
						PeerInfo.vht_RxChannelWidth = 3;

						/* peer STA doesn't support 80+80MHz */
						if ((PhyDSSSTable->Chanflag.ChnlWidth != CH_160_MHz_WIDTH)
						    && !(PeerInfo.he_cap.phy_cap.channel_width_set & HE_SUPPORT_80P80MHZ_BW_5G)) {
							PeerInfo.vht_RxChannelWidth = 2;
						}
					} else if (PeerInfo.he_cap.phy_cap.channel_width_set & HE_SUPPORT_40_80MHZ_BW_5G)
						PeerInfo.vht_RxChannelWidth = 2;
					else
						PeerInfo.vht_RxChannelWidth = 0;
				}
			...
			if (skb_tailroom(txSkb_p) > sizeof(HE_Capabilities_IE_t)) {
				/* We have enough room at the tail */
				IE_p = skb_tail_pointer(txSkb_p);

				if (IE_p != NULL) {
					memcpy((void *)IE_p, (void *)&vmacSta_p->he_cap, vmacSta_p->he_cap.hdr.Len +
					       sizeof(IEEEtypes_InfoElementHdr_t));
					skb_put(txSkb_p, vmacSta_p->he_cap.hdr.Len + sizeof(IEEEtypes_InfoElementHdr_t));
				}
			} else {
			...
			if (skb_tailroom(txSkb_p) > sizeof(HE_Operation_IE_t)) {
				/* We have enough room at the tail */
				IE_p = skb_tail_pointer(txSkb_p);

				if (IE_p != NULL)
					skb_put(txSkb_p, Build_IE_HE_OP(vmacSta_p, (UINT8 *) IE_p, IE192Present));
			} else {
			...
			if (vmacSta_p->VMacEntry.muedcaEnable) {
				...
				if (IE_p != NULL)
					skb_put(txSkb_p, Build_IE_MU_EDCA(vmacSta_p, (UINT8 *) IE_p));
				else
					printk("Error: no space in skb for mu edca ie\n");
			}

			if (skb_tailroom(txSkb_p) > sizeof(SRP_param_set_t)) {
				/* We have enough room at the tail */
				IE_p = skb_tail_pointer(txSkb_p);

				if (IE_p != NULL)
					skb_put(txSkb_p, Build_IE_SRP(vmacSta_p, (UINT8 *) IE_p));
			} else {
				printk("Assoc Response not enough space for Spatial Reuse Parameter Set IE\n");
				WLDBG_INFO(DBG_LEVEL_7, "panic!!!!");
			}

			if (vmacSta_p->he_cap.rx_he_mcs_80m.max_mcs_set > PeerInfo.he_cap.tx_he_mcs_80m.max_mcs_set) {
				PeerInfo.he_cap.tx_he_mcs_80m.max_mcs_set = vmacSta_p->he_cap.rx_he_mcs_80m.max_mcs_set;
			}

			if ((vmacSta_p->he_cap.phy_cap.channel_width_set & HE_SUPPORT_160MHZ_BW_5G) &&
			    (PeerInfo.he_cap.phy_cap.channel_width_set & HE_SUPPORT_160MHZ_BW_5G)) {
				if ((&vmacSta_p->he_cap.rx_he_mcs_80m + 2)->max_mcs_set > (&PeerInfo.he_cap.tx_he_mcs_80m + 2)->max_mcs_set) {
					(&PeerInfo.he_cap.tx_he_mcs_80m + 2)->max_mcs_set = (&vmacSta_p->he_cap.rx_he_mcs_80m + 2)->max_mcs_set;
				}

				if ((vmacSta_p->he_cap.phy_cap.channel_width_set & HE_SUPPORT_80P80MHZ_BW_5G) &&
				    (PeerInfo.he_cap.phy_cap.channel_width_set & HE_SUPPORT_80P80MHZ_BW_5G)) {
					if ((&vmacSta_p->he_cap.rx_he_mcs_80m + 4)->max_mcs_set > (&PeerInfo.he_cap.tx_he_mcs_80m + 4)->max_mcs_set) {
						(&PeerInfo.he_cap.tx_he_mcs_80m + 4)->max_mcs_set =
						    (&vmacSta_p->he_cap.rx_he_mcs_80m + 4)->max_mcs_set;
					}
					if (PeerInfo.vht_RxChannelWidth == 3) {
						StaInfo_p->operating_mode.rxnss = get_he_peer_nss(&PeerInfo.he_cap.rx_he_mcs_80m + 4);
						StaInfo_p->operating_mode.tx_nsts = get_he_peer_nss(&PeerInfo.he_cap.tx_he_mcs_80m + 4);
					} else {
						StaInfo_p->operating_mode.rxnss = get_he_peer_nss(&PeerInfo.he_cap.rx_he_mcs_80m);
						StaInfo_p->operating_mode.tx_nsts = get_he_peer_nss(&PeerInfo.he_cap.tx_he_mcs_80m);
					}

				} else {
					if (PeerInfo.vht_RxChannelWidth == 3) {
						StaInfo_p->operating_mode.rxnss = get_he_peer_nss(&PeerInfo.he_cap.rx_he_mcs_80m + 2);
						StaInfo_p->operating_mode.tx_nsts = get_he_peer_nss(&PeerInfo.he_cap.tx_he_mcs_80m + 2);
					} else {
						StaInfo_p->operating_mode.rxnss = get_he_peer_nss(&PeerInfo.he_cap.rx_he_mcs_80m);
						StaInfo_p->operating_mode.tx_nsts = get_he_peer_nss(&PeerInfo.he_cap.tx_he_mcs_80m);
					}
			...
			StaInfo_p->operating_mode.chbw = PeerInfo.vht_RxChannelWidth;
			StaInfo_p->operating_mode.ulmu_disable = 0;
			StaInfo_p->ClientMode_mix_ext = AX_MODE;
		}
	}
#endif				/* SOC_W906X */
	...
	if ((MultiAP_IE_p != NULL) && mib->multi_ap_attr) {
		...
		if (skb_tailroom(txSkb_p) > MultiAP_size) {
			//We have enough room at the tail
			InfoElemHdr_p = (IEEEtypes_InfoElementHdr_t *) skb_put(txSkb_p, MultiAP_size);
			Add_MultiAP_IE(vmacSta_p, InfoElemHdr_p, WL_WLAN_TYPE_AP);
...
#ifdef SOC_W906X
#ifdef MULTI_AP_SUPPORT
	if (((StaInfo_p->MultiAP_4addr == 1) || ((StaInfo_p->MultiAP_4addr == 2) && isMComp_sta)) && mib->multi_ap_attr) {
		StaInfo_p->MultiAP_4addr = 1;
		wds = 4;
	}

	if (StaInfo_p->MultiAP_4addr == 0)
		FourAddr_IfDelHashEntry(&StaInfo_p->Addr);
#endif				/* MULTI_AP_SUPPORT */

	if (HTpresent && (PeerInfo.HTRateBitMap == 0x00))
...
#endif				//CB_SUPPORT
	if (!he_cap_present && IE191Present) {
		IEEEtypes_VHT_Cap_Info_t *ptr = (IEEEtypes_VHT_Cap_Info_t *) (&(PeerInfo.vht_cap));
		if ((StaInfo_p->vht_peer_RxNss == 1) && (ptr->MaximumMPDULength == 1)) {
			ptr->MaximumMPDULength = 0;
		}
	}
	//workaround:recognise 2ss 11ax mtk solution as vivo z1, to extend the AMSDU size to 8k for tcptx tp improvement purpose
	if (he_cap_present && (StaInfo_p->mtk_sta)
	    && (StaInfo_p->vht_peer_RxNss == 2)) {
		((IEEEtypes_VHT_Cap_Info_t *) & PeerInfo.vht_cap)->MaximumMPDULength = 1;
	}
	//Fix Xbox can't recv LDPC frame issue, disable LDPC for the devices with Xbox vendor OUT,1ss,11N,2.4GHz
	if ((!he_cap_present && !IE191Present && HTpresent) &&
	    (StaInfo_p->vht_peer_RxNss == 0) && (PeerInfo.HTRateBitMap == 0xFF) && (PhyDSSSTable->Chanflag.FreqBand == FREQ_BAND_2DOT4GHZ)) {
		for (i = 0; i < XBOX_OUI_NUM; i++) {
			if (memcmp(&XBOX_MAC_OUI[i][0], &MgmtMsg_p->Hdr.SrcAddr, 3) == 0) {
				printk("disable LDPC for Xbox device\n");
				PeerInfo.HTCapabilitiesInfo.AdvCoding = 0;
			}
		}
	}
	...
	{
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, ((struct net_device *)vmacSta_p->dev));
		if (wlpptr->cb_enable == TRUE) {
			// Inherit enable_tx from cbinfo_bss
			printk("1-[%s], cbinfo_sta[%u]=%u\n", ((struct net_device *)vmacSta_p->dev)->name, StaInfo_p->StnId,
			       wlpptr->cbinfo_bss.enable_tx);
			//wlpd_p->cbinfo_sta[StaInfo_p->StnId].enable_tx = wlpptr->cbinfo_bss.enable_tx;
			StaInfo_p->cbinfo_sta.enable_tx = wlpptr->cbinfo_bss.enable_tx;
		} else {
			printk("1-[%s], cb_disabled\n", ((struct net_device *)vmacSta_p->dev)->name);
		}
	}
#else
#ifdef MULTI_AP_SUPPORT
	if ((StaInfo_p->MultiAP_4addr == 1) && mib->multi_ap_attr) {
	...
	WLSYSLOG(vmacSta_p->dev, WLSYSLOG_CLASS_ALL, WLSYSLOG_MSG_MLME_ASSOC_SUCCESS "%02x%02x%02x%02x%02x%02x StnId=%d\n",
		 MgmtMsg_p->Hdr.SrcAddr[0],
		 MgmtMsg_p->Hdr.SrcAddr[1], MgmtMsg_p->Hdr.SrcAddr[2], MgmtMsg_p->Hdr.SrcAddr[3],
		 MgmtMsg_p->Hdr.SrcAddr[4], MgmtMsg_p->Hdr.SrcAddr[5], StaInfo_p->StnId);

#ifdef MBO_SUPPORT
	if ((mib->mib_mbo_enabled) && (skb_tailroom(txSkb_p) > StaInfo_p->AP_MBOIEBuf[1] + 2)) {
		//We have enough room at the tail
		IE_p = skb_put(txSkb_p, StaInfo_p->AP_MBOIEBuf[1] + 2);
		if (IE_p != NULL) {
			memcpy(IE_p, &StaInfo_p->AP_MBOIEBuf[0], StaInfo_p->AP_MBOIEBuf[1] + 2);
		}
	}
#endif				/* MBO_SUPPORT */
	...
	/* OWE mode, adds the Diffie-Hellman Parameter element to association response. */
	if (((vmacSta_p->Mib802dot11->WPA2AuthSuites->AuthSuites[3]) == 0x12)) {
		if (skb_tailroom(txSkb_p) > StaInfo_p->AP_DHIEBuf[1] + 2) {
			//We have enough room at the tail
			IE_p = skb_put(txSkb_p, StaInfo_p->AP_DHIEBuf[1] + 2);
			if (IE_p != NULL) {
				memcpy(IE_p, &StaInfo_p->AP_DHIEBuf[0], StaInfo_p->AP_DHIEBuf[1] + 2);
			}
		}

		if (StaInfo_p->EXT_RsnIE[0] == RSN_IEWPA2 &&
		    (StaInfo_p->EXT_RsnIE[1] + 2) <= 64 && skb_tailroom(txSkb_p) > (StaInfo_p->EXT_RsnIE[1] + 2)) {
			IE_p = skb_put(txSkb_p, StaInfo_p->EXT_RsnIE[1] + 2);
			if (IE_p != NULL)
				memcpy(IE_p, &StaInfo_p->EXT_RsnIE[0], StaInfo_p->EXT_RsnIE[1] + 2);
		}
	}
#endif				/* OWE_SUPPORT */

	/* adds rsnx to association response. */
	if (RsnxIE_p) {
		if (skb_tailroom(txSkb_p) > StaInfo_p->RsnxIE_Buf[1] + 2) {
			/* We have enough room at the tail */
			IE_p = skb_put(txSkb_p, StaInfo_p->RsnxIE_Buf[1] + 2);
			if (IE_p != NULL) {
				memcpy(IE_p, &StaInfo_p->RsnxIE_Buf[0], StaInfo_p->RsnxIE_Buf[1] + 2);
			}
		}
	}
	...
	if (wfa_11ax_pf && vmacSta_p->master) {
		if (vmacSta_p->master->dl_ofdma_para.postpone_time) {
			wlFwSetOfdma_Mode(vmacSta_p->master->dev,
					  0,
					  vmacSta_p->master->dl_ofdma_para.ru_mode,
					  vmacSta_p->master->dl_ofdma_para.max_delay, vmacSta_p->master->dl_ofdma_para.max_sta);
	...
	if ((flag) && (StaInfo_p->keyMgmtStateInfo.pending_assoc))
		mwl_send_vendor_assoc_event(vmacSta_p->dev, (uint8_t *) & MgmtMsg_p->Hdr.SrcAddr);
	else
#endif				/* MRVL_80211R */
		mwl_cfg80211_assoc_event(vmacSta_p->dev, (uint8_t *) & MgmtMsg_p->Hdr.SrcAddr);
#else
#ifdef SOC_W906X
#ifdef MRVL_80211R
	if ((flag) && (StaInfo_p->keyMgmtStateInfo.pending_assoc))
	...
	if (reconfig_rsn_ie == TRUE) {
		memcpy(StaInfo_p->keyMgmtStateInfo.RsnIEBuf, mib->thisStaRsnIEWPA2, 14);
		StaInfo_p->keyMgmtStateInfo.RsnIEBuf[1] = 0x0C;
	}
#ifdef MULTI_AP_SUPPORT
	if ((mib->multi_ap_attr & MAP_ATTRIBUTE_FRONTHAUL_BSS) || (mib->multi_ap_attr & MAP_ATTRIBUTE_BACKHAUL_BSS)) {
		snprintf(buf, sizeof(buf), "%s", tag);
		if (MgmtMsg_p->Hdr.FrmCtl.Subtype == IEEE_MSG_ASSOCIATE_RQST) {
			event_len = strlen(tag) + sizeof(struct multiap_sta_assoc_event) + IELen;
			if (event_len <= IW_CUSTOM_MAX) {
				multiap_sta_assoc_p = (struct multiap_sta_assoc_event *)(buf + strlen(tag));
				multiap_sta_assoc_p->ie_len = IELen + sizeof(struct multiap_sta_assoc_event)
				    - sizeof(multiap_sta_assoc_p->ie_len);
				multiap_sta_assoc_p->FrmCtl = MgmtMsg_p->Hdr.FrmCtl;
				multiap_sta_assoc_p->Duration = MgmtMsg_p->Hdr.Duration;
				memcpy(multiap_sta_assoc_p->DestAddr, MgmtMsg_p->Hdr.DestAddr, sizeof(IEEEtypes_MacAddr_t));
				memcpy(multiap_sta_assoc_p->SrcAddr, MgmtMsg_p->Hdr.SrcAddr, sizeof(IEEEtypes_MacAddr_t));
				memcpy(multiap_sta_assoc_p->BssId, MgmtMsg_p->Hdr.BssId, sizeof(IEEEtypes_MacAddr_t));
				multiap_sta_assoc_p->SeqCtl = MgmtMsg_p->Hdr.SeqCtl;
				multiap_sta_assoc_p->CapInfo = MgmtMsg_p->Body.AssocRqst.CapInfo;
				multiap_sta_assoc_p->ListenInterval = MgmtMsg_p->Body.AssocRqst.ListenInterval;
				memcpy(multiap_sta_assoc_p->more_ies, IEBuf, IELen);
				memset(&wreq, 0, sizeof(wreq));
				wreq.data.length = event_len;
				if (vmacSta_p->dev->flags & IFF_RUNNING)
					wireless_send_event(vmacSta_p->dev, IWEVCUSTOM, &wreq, buf);
			}
		} else if (MgmtMsg_p->Hdr.FrmCtl.Subtype == IEEE_MSG_REASSOCIATE_RQST) {
			event_len = strlen(tag) + sizeof(struct multiap_sta_reassoc_event) + IELen;
			if (event_len <= IW_CUSTOM_MAX) {
				multiap_sta_reassoc_p = (struct multiap_sta_reassoc_event *)(buf + strlen(tag));
				multiap_sta_reassoc_p->ie_len = IELen + sizeof(struct multiap_sta_reassoc_event)
				    - sizeof(multiap_sta_reassoc_p->ie_len);
				multiap_sta_reassoc_p->FrmCtl = MgmtMsg_p->Hdr.FrmCtl;
				multiap_sta_reassoc_p->Duration = MgmtMsg_p->Hdr.Duration;
				memcpy(multiap_sta_reassoc_p->DestAddr, MgmtMsg_p->Hdr.DestAddr, sizeof(IEEEtypes_MacAddr_t));
				memcpy(multiap_sta_reassoc_p->SrcAddr, MgmtMsg_p->Hdr.SrcAddr, sizeof(IEEEtypes_MacAddr_t));
				memcpy(multiap_sta_reassoc_p->BssId, MgmtMsg_p->Hdr.BssId, sizeof(IEEEtypes_MacAddr_t));
				multiap_sta_reassoc_p->SeqCtl = MgmtMsg_p->Hdr.SeqCtl;
				multiap_sta_reassoc_p->CapInfo = MgmtMsg_p->Body.ReassocRqst.CapInfo;
				multiap_sta_reassoc_p->ListenInterval = MgmtMsg_p->Body.ReassocRqst.ListenInterval;
				memcpy(multiap_sta_reassoc_p->CurrentApAddr, MgmtMsg_p->Body.ReassocRqst.CurrentApAddr, sizeof(IEEEtypes_MacAddr_t));
				memcpy(multiap_sta_reassoc_p->more_ies, IEBuf, IELen);
				memset(&wreq, 0, sizeof(wreq));
				wreq.data.length = event_len;
				if (vmacSta_p->dev->flags & IFF_RUNNING)
					wireless_send_event(vmacSta_p->dev, IWEVCUSTOM, &wreq, buf);
...
#if defined(SOC_W906X) && defined(CFG80211)
		mwl_send_vendor_assoc_notification(vmacSta_p->dev, (uint8_t *) & MgmtMsg_p->Hdr.SrcAddr,
						   (void *)&StaInfo_p->assocReqMsg, MgmtRsp_p->Body.AssocRsp.StatusCode);
#endif				/* SOC_W906X */
	}
#endif				/* MULTI_AP_SUPPORT */
	...
	if (!StaInfo_p->CapInfo.ShortPreamble && ChangeAssocParam == TRUE)
...
#ifdef CFG80211
#ifdef CFG80211_COMPATIABLE
		mwl_cfg80211_assoc_event(vmacSta_p->dev, &MgmtMsg_p->Hdr.SrcAddr);
#else
#ifdef SOC_W906X
		mwl_cfg80211_assoc_event(vmacSta_p->dev, &MgmtMsg_p->Hdr.SrcAddr);
#else
		mwl_send_vendor_assoc_event(vmacSta_p->dev, &MgmtMsg_p->Hdr.SrcAddr);
#endif				//SOC_W906X
#endif				/* CFG80211_COMPATIABLE */
#endif				/* CFG80211 */
	}
	...
	if (*(vmacSta_p->Mib802dot11->mib_HT40MIntoler)) {
		if (HTpresent) {
			...
			/* handle 40-20 switch */
			if ((PeerInfo.HTCapabilitiesInfo.FortyMIntolerant) || (!StaInfo_p->HtElem.HTCapabilitiesInfo.SupChanWidth)) {
	...
	if (StaInfo_p->StnId < sta_num) {
		if (wlpptr->wlpd_p->txRateHistogram[StaInfo_p->StnId] == NULL) {
			wlpptr->wlpd_p->txRateHistogram[StaInfo_p->StnId] = (WLAN_TX_RATE_HIST *) wl_kmalloc_autogfp(sizeof(WLAN_TX_RATE_HIST));
		}
		if (wlpptr->wlpd_p->txRateHistogram[StaInfo_p->StnId]) {
			memset(wlpptr->wlpd_p->txRateHistogram[StaInfo_p->StnId], 0, sizeof(WLAN_TX_RATE_HIST));
		...
		if (wlpptr->wlpd_p->scheHistogram[StaInfo_p->StnId] != NULL)
			memset(wlpptr->wlpd_p->scheHistogram[StaInfo_p->StnId], 0, sizeof(WLAN_SCHEDULER_HIST));
		else {
			if ((sch_histo_p = (WLAN_SCHEDULER_HIST *) wl_kmalloc_autogfp(sizeof(WLAN_SCHEDULER_HIST))) != NULL) {
				memset(sch_histo_p, 0, sizeof(WLAN_SCHEDULER_HIST));
				wlpptr->wlpd_p->scheHistogram[StaInfo_p->StnId] = sch_histo_p;
			} else
	...
	if ((StaInfo_p->StnId > 0) && (StaInfo_p->StnId <= sta_num)) {
		if ((txrate_histo_p = (WLAN_TX_RATE_HIST *) wl_kmalloc_autogfp(sizeof(WLAN_TX_RATE_HIST))) != NULL) {
			memset(txrate_histo_p, 0, sizeof(WLAN_TX_RATE_HIST));

			SPIN_LOCK_IRQSAVE(&wlpptr->wlpd_p->txRateHistoLock[StaInfo_p->StnId - 1], txRateHistoflags);

			if (wlpptr->wlpd_p->txRateHistogram[StaInfo_p->StnId - 1] != NULL) {
				wl_kfree(wlpptr->wlpd_p->txRateHistogram[StaInfo_p->StnId - 1]);
				wlpptr->wlpd_p->txRateHistogram[StaInfo_p->StnId - 1] = NULL;
			}

			wlpptr->wlpd_p->txRateHistogram[StaInfo_p->StnId - 1] = txrate_histo_p;
			wlAcntCopyRateTbl(vmacSta_p->dev, (UINT8 *) & MgmtMsg_p->Hdr.SrcAddr, StaInfo_p->StnId, SU_MIMO);	//SU for now

			SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->txRateHistoLock[StaInfo_p->StnId - 1], txRateHistoflags);
		} else
			printk("staid:%d alloc WLAN_TX_RATE_HIST memory FAIL\n", StaInfo_p->StnId);
	}
#endif				/* SOC_W906X */
...
#if 0
	if (ofdma_autogrp == 1) {
		printk("MUBeamformeeCapable:%u, Nss:%u\n", StaInfo_p->vhtCap.cap.MUBeamformeeCapable, StaInfo_p->vht_peer_RxNss);
	}
#endif
	...
	if (ofdma_autogrp && StaInfo_p) {
		if ((is_he_capable_sta(StaInfo_p)) ||	/* add HE STA into list */
		    (*((UINT32 *) & StaInfo_p->vhtCap.cap) &&	/* add VHT MU BFmer capable STA into list */
		     StaInfo_p->vhtCap.cap.MUBeamformeeCapable && (StaInfo_p->vht_peer_RxNss < 3)))
...
#endif				/* CCK_DESENSE */
#else
	if (StaInfo_p && *((UINT32 *) & StaInfo_p->vhtCap.cap) && StaInfo_p->vhtCap.cap.MUBeamformeeCapable && (StaInfo_p->vht_peer_RxNss < 3)) {

		MUAddStaToMUStaList(vmacSta_p, StaInfo_p);
	}
#endif				/* SOC_W906X */

	atf_check_setting(vmacSta_p->dev, &MgmtMsg_p->Hdr.SrcAddr, staIdx);
	wl_kfree(tempbuffer);
	return;
...
#ifdef SOC_W906X
void macMgmtMlme_SendDeauthenticateMsg(vmacApInfo_t * vmacAP_p, IEEEtypes_MacAddr_t * Addr, UINT16 StnId, UINT16 Reason, UINT8 sendCmd)
#else
...
#endif
{
	...
	/*
	   If CB enabled => BSSID of APs may be identical that it may get packets from STA which is not associated with it
	   => Should not send deauth
	 */
	if ((wlpptr->cb_enable == TRUE) && ((Reason == IEEEtypes_REASON_CLASS3_NONASSOC) || (Reason == IEEEtypes_REASON_CLASS2_NONAUTH))
	    ) {
		return;
	}
	...
	if (net_ratelimit()) {
		...
		printk(WLSYSLOG_MSG_MLME_DEAUTH_TOSTA "%02x%02x%02x%02x%02x%02x Reason %d\n", ((unsigned char *)Addr)[0],
		       ((unsigned char *)Addr)[1], ((unsigned char *)Addr)[2], ((unsigned char *)Addr)[3],
		       ((unsigned char *)Addr)[4], ((unsigned char *)Addr)[5], ENDIAN_SWAP16(Reason));
	}
	...
	if ((StaInfo_p = extStaDb_GetStaInfo(vmacSta_p, Addr, STADB_UPDATE_AGINGTIME)) != NULL) {
		...
		mwl_cfg80211_disassoc_event(vmacSta_p->dev, (uint8_t *) Addr);
#else
		mwl_send_vendor_disassoc_event(vmacSta_p->dev, (uint8_t *) Addr);
#endif				//SOC_W906X
#endif				/* CFG80211_COMPATIABLE */
#endif				/* CFG80211 */
#endif
		...
		if ((mib->multi_ap_attr & MAP_ATTRIBUTE_FRONTHAUL_BSS) || (mib->multi_ap_attr & MAP_ATTRIBUTE_BACKHAUL_BSS)) {
#if defined(SOC_W906X) && defined(CFG80211)
			sinfo = wl_kzalloc(sizeof(struct station_info), GFP_ATOMIC);
			if (sinfo)
				fillStaInfo(vmacSta_p->dev, sinfo, StaInfo_p);
#endif				/* SOC_W906X */

			snprintf(buf, sizeof(buf), "%s", tag);
			disassoc_event_p = (struct multiap_sta_disassoc_event *)(buf + strlen(tag));
			memcpy(disassoc_event_p->bssid, vmacSta_p->macBssId, sizeof(IEEEtypes_MacAddr_t));
			memcpy(disassoc_event_p->sta_mac, Addr, sizeof(IEEEtypes_MacAddr_t));
			disassoc_event_p->reason = Reason;
#ifdef CFG80211
			disassoc_event_p->tx_packets = (sinfo) ? sinfo->tx_packets : StaInfo_p->tx_packets;
			disassoc_event_p->rx_packets = (sinfo) ? sinfo->rx_packets : StaInfo_p->rx_packets;
			disassoc_event_p->tx_bytes = (sinfo) ? sinfo->tx_bytes : StaInfo_p->tx_bytes;
			disassoc_event_p->rx_bytes = (sinfo) ? sinfo->rx_bytes : StaInfo_p->rx_bytes;
			disassoc_event_p->tx_failed = (sinfo) ? sinfo->tx_failed : 0;
			disassoc_event_p->tx_retries = (sinfo) ? sinfo->tx_retries : 0;
			disassoc_event_p->rx_dropped_misc = (sinfo) ? sinfo->rx_dropped_misc : 0;
#endif
			memset(&wreq, 0, sizeof(wreq));
			wreq.data.length = strlen(tag) + sizeof(struct multiap_sta_disassoc_event);
			...
			if (sinfo) {
#if defined(SOC_W906X) && defined(CFG80211)
				mwl_send_vendor_disassoc_notification(vmacSta_p->dev, (uint8_t *) Addr, sinfo, Reason);
#endif				/* SOC_W906X */
				wl_kfree(sinfo);
			}
		}
	...
	{
#ifdef SOC_W906X
		skb_put(txSkb_p, sizeof(MgmtMsg_p->Body.Deauth.ReasonCode));
#endif
...
#ifdef SOC_W906X
			if (sendCmd && StaInfo_p && StaInfo_p->State != UNAUTHENTICATED) {
#else
			if (StaInfo_p && StaInfo_p->State != UNAUTHENTICATED) {
#endif
				wlFwSetNewStn(vmacSta_p->dev, (u_int8_t *) Addr, 0, 0, StaInfoDbActionRemoveEntry, NULL, 0, 0, 0);
...
void macMgmtMlme_DeauthenticateMsg(vmacApInfo_t * vmacSta_p, macmgmtQ_MgmtMsg3_t * MgmtMsg_p, UINT32 msgSize)
{
	...
	union iwreq_data wreq;
#endif
	if ((StaInfo_p = extStaDb_GetStaInfo(vmacSta_p, &(MgmtMsg_p->Hdr.SrcAddr), STADB_DONT_UPDATE_AGINGTIME)) == NULL) {
	...
	if ((mib->multi_ap_attr & MAP_ATTRIBUTE_FRONTHAUL_BSS) || (mib->multi_ap_attr & MAP_ATTRIBUTE_BACKHAUL_BSS)) {
		...
		disassoc_event_p = (struct multiap_sta_disassoc_event *)(buf + strlen(tag));
		memcpy(disassoc_event_p->bssid, vmacSta_p->macBssId, sizeof(IEEEtypes_MacAddr_t));
		memcpy(disassoc_event_p->sta_mac, StaInfo_p->Addr, sizeof(IEEEtypes_MacAddr_t));
		disassoc_event_p->reason = MgmtMsg_p->Body.Deauth.ReasonCode;
#ifdef CFG80211
		disassoc_event_p->tx_packets = (sinfo) ? sinfo->tx_packets : StaInfo_p->tx_packets;
		disassoc_event_p->rx_packets = (sinfo) ? sinfo->rx_packets : StaInfo_p->rx_packets;
		disassoc_event_p->tx_bytes = (sinfo) ? sinfo->tx_bytes : StaInfo_p->tx_bytes;
		...
		if (sinfo) {
#if defined(SOC_W906X) && defined(CFG80211)
			mwl_send_vendor_disassoc_notification(vmacSta_p->dev, (uint8_t *) & MgmtMsg_p->Hdr.SrcAddr,
							      sinfo, MgmtMsg_p->Body.Deauth.ReasonCode);
#endif				/* SOC_W906X */
			wl_kfree(sinfo);
		}
	}
#endif				/* MULTI_AP_SUPPORT */
	{
		...
		if (wlpptr->cb_enable == TRUE) {
			// Inherit enable_tx from cbinfo_bss
			printk("(%s)-[%s], cbinfo_sta[%u]=%u\n", __func__, netdev->name, StaInfo_p->StnId, wlpptr->cbinfo_bss.enable_tx);
		}
	}
	//   FreePowerSaveQueue(StaInfo_p->StnId);
	FreeStnId(vmacSta_p, StaInfo_p->StnId);
	extStaDb_DelSta(vmacSta_p, &(MgmtMsg_p->Hdr.SrcAddr), STADB_DONT_UPDATE_AGINGTIME);
	wlFwSetNewStn(vmacSta_p->dev, (u_int8_t *) & MgmtMsg_p->Hdr.SrcAddr, StaInfo_p->Aid, StaInfo_p->StnId,
		      StaInfoDbActionRemoveEntry, NULL, 0, 0, 0);
	/* Send event to user space */
	WLSNDEVT(vmacSta_p->dev, IWEVEXPIRED, &MgmtMsg_p->Hdr.SrcAddr, NULL);
#ifdef CFG80211
#ifdef CFG80211_COMPATIABLE
	mwl_cfg80211_disassoc_event(vmacSta_p->dev, (uint8_t *) & MgmtMsg_p->Hdr.SrcAddr);
#else
#ifdef SOC_W906X
	mwl_cfg80211_disassoc_event(vmacSta_p->dev, (uint8_t *) & MgmtMsg_p->Hdr.SrcAddr);
#else
	mwl_send_vendor_disassoc_event(vmacSta_p->dev, (uint8_t *) & MgmtMsg_p->Hdr.SrcAddr);
...
void macMgmtMlme_DisassociateCmd(vmacApInfo_t * vmacSta_p, IEEEtypes_DisassocCmd_t * DisassocCmd_p)
{
	...
	if ((StaInfo_p = extStaDb_GetStaInfo(vmacSta_p, &DisassocCmd_p->PeerStaAddr, STADB_DONT_UPDATE_AGINGTIME)) == NULL) {
		return;
	}
...
#ifdef MULTI_AP_SUPPORT
void macMgmtMlme_SendDisassociateMsg4MAP(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * Addr, UINT16 StnId, UINT16 Reason)
{
	macmgmtQ_MgmtMsg2_t *MgmtMsg_p;
...
void macMgmtMlme_SendDisassociateMsg(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * Addr, UINT16 StnId, UINT16 Reason)
{
	...
	{
#ifdef SOC_W906X
		skb_put(txSkb_p, sizeof(MgmtMsg_p->Body.DisAssoc.ReasonCode));
#endif
		MgmtMsg_p->Body.DisAssoc.ReasonCode = Reason;
		if (txMgmtMsg(vmacSta_p->dev, txSkb_p) != OS_SUCCESS)
			wl_free_skb(txSkb_p);
...
{
	TimerFireIn(&sta_info->mgtAssoc.timer, 1, &pending_assoc_timeout_handler, (UINT8 *) sta_info, ticks);
}
...
void pending_assoc_start_timer(extStaDb_StaInfo_t * sta_info)
{
	...
	pending_assoc_timer_add(sta_info, 100);
}
#endif				/* defined(MRVL_80211R) || defined(OWE_SUPPORT) || defined(MBO_SUPPORT) */

#ifdef MRVL_80211R
void macMgmtMlme_SendAssocMsg(vmacApInfo_t * vmacAP_p, IEEEtypes_MacAddr_t * staMac, UINT8 * optie, UINT8 optie_len)
{
	extStaDb_StaInfo_t *StaInfo_p;
	UINT8 *addr = (char *)staMac;
	struct sk_buff *skb, *newskb;

	if ((StaInfo_p = extStaDb_GetStaInfo(vmacAP_p, staMac, STADB_DONT_UPDATE_AGINGTIME)) == NULL) {
	...
	if (skb_tailroom(skb) < optie_len) {
		newskb = wl_alloc_skb(skb->len + optie_len + MIN_BYTES_HEADROOM);
		if (newskb == NULL) {
				    ...
				    UINT16 status_code, UINT8 * optie, UINT8 optie_len)
{
	...
	if ((skb = mlmeApiPrepMgtMsg2(IEEE_MSG_AUTHENTICATE, (IEEEtypes_MacAddr_t *) staMac,
				      (IEEEtypes_MacAddr_t *) vmacAP_p->macBssId, optie_len + 6)) == NULL) {
		WLDBG_INFO(DBG_LEVEL_8, "Send Auth Failed, skb=NULL...\n");
		return -1;
	}
	...
	memcpy((UINT8 *) & MgmtMsg_p->Body.Auth.ChallengeText, optie, optie_len);
...
extern void macMgmtMlme_DisassociateMsg(vmacApInfo_t * vmacSta_p, macmgmtQ_MgmtMsg3_t * MgmtMsg_p, UINT32 msgSize)
{
	...
	union iwreq_data wreq;
#endif
	StaInfo_p = extStaDb_GetStaInfo(vmacSta_p, &(MgmtMsg_p->Hdr.SrcAddr), STADB_DONT_UPDATE_AGINGTIME);
	if (!StaInfo_p)
		return;
	...
	if ((mib->multi_ap_attr & MAP_ATTRIBUTE_FRONTHAUL_BSS) || (mib->multi_ap_attr & MAP_ATTRIBUTE_BACKHAUL_BSS)) {
		...
		memcpy(disassoc_event_p->bssid, vmacSta_p->macBssId, sizeof(IEEEtypes_MacAddr_t));
		memcpy(disassoc_event_p->sta_mac, StaInfo_p->Addr, sizeof(IEEEtypes_MacAddr_t));
		disassoc_event_p->reason = MgmtMsg_p->Body.DisAssoc.ReasonCode;
#ifdef CFG80211
		disassoc_event_p->tx_packets = (sinfo) ? sinfo->tx_packets : StaInfo_p->tx_packets;
		disassoc_event_p->rx_packets = (sinfo) ? sinfo->rx_packets : StaInfo_p->rx_packets;
		disassoc_event_p->tx_bytes = (sinfo) ? sinfo->tx_bytes : StaInfo_p->tx_bytes;
...
#if defined(SOC_W906X) && defined(CFG80211)
			mwl_send_vendor_disassoc_notification(vmacSta_p->dev, (uint8_t *) & MgmtMsg_p->Hdr.SrcAddr,
							      sinfo, MgmtMsg_p->Body.DisAssoc.ReasonCode);
#endif				/* SOC_W906X */
			wl_kfree(sinfo);
		}
	}
	...
	cleanupAmpduTx(vmacSta_p, (UINT8 *) & StaInfo_p->Addr);
#ifndef SOC_W906X
	wlFwSetNewStn(vmacSta_p->dev, (u_int8_t *) & StaInfo_p->Addr, StaInfo_p->Aid, StaInfo_p->StnId, 2, NULL, 0, 0, 0);
#endif
	...
	if (wfa_11ax_pf) {
		...
		if ((vmacSta_master_p->dl_ofdma_para.max_sta > 0) && (vmacSta_master_p->dl_ofdma_para.sta_cnt > 0)) {
			for (i = 0; i < MAX_OFDMADL_STA; i++) {
				if (!memcmp(StaInfo_p->Addr, vmacSta_master_p->ofdma_mu_sta_addr[i], IEEEtypes_ADDRESS_SIZE))
					break;
			}
			if (i < MAX_OFDMADL_STA) {
				...
				memset(vmacSta_master_p->ofdma_mu_sta_addr[i], 0, IEEEtypes_ADDRESS_SIZE);
			...
			if (!vmacSta_master_p->dl_ofdma_para.sta_cnt) {
				vmacSta_master_p->dl_ofdma_para.option = 0;
				vmacSta_master_p->dl_ofdma_para.ru_mode = 0;
				vmacSta_master_p->dl_ofdma_para.max_delay = 0;
				printk("Set ofdma fw cmd: option=%d ru_mode=%d max_delay=%d max_sta=%d sta_cnt=%d\n",
				       vmacSta_master_p->dl_ofdma_para.option,
				       vmacSta_master_p->dl_ofdma_para.ru_mode,
				       vmacSta_master_p->dl_ofdma_para.max_delay,
				       vmacSta_master_p->dl_ofdma_para.max_sta, vmacSta_master_p->dl_ofdma_para.sta_cnt);
				wlFwSetOfdma_Mode(vmacSta_master_p->dev,
						  vmacSta_master_p->dl_ofdma_para.option,
						  vmacSta_master_p->dl_ofdma_para.ru_mode,
						  vmacSta_master_p->dl_ofdma_para.max_delay, vmacSta_master_p->dl_ofdma_para.max_sta);
				vmacSta_master_p->dl_ofdma_para.started = 0;
			}
		}
		if (vmacSta_master_p->dl_ofdma_para.all_connected
		    && (vmacSta_master_p->dl_ofdma_para.sta_cnt < vmacSta_master_p->dl_ofdma_para.max_sta))
			vmacSta_master_p->dl_ofdma_para.all_connected = 0;
	}
	/* Send indication to SME */
...
#ifdef CONFIG_IEEE80211W
extern void *probeRequest_ParseAttribWithinFrame(macmgmtQ_MgmtMsg3_t * mgtFrame_p, UINT8 * data_p, UINT8 attrib)
{
...
void macMgmtMlme_UpdateProbeRspInfo(vmacApInfo_t * vmacSta_p, UINT16 bcn_interval, UINT16 capinfo)
{
	...
	SPIN_LOCK_IRQSAVE(&wlpptr->wlpd_p->locks.probeRspIEsLock, flags);
	if (!vmacSta_p->probeRspBody.basic_ies)
		vmacSta_p->probeRspBody.basic_ies = wl_kmalloc(MAX_BEACON_SIZE, GFP_ATOMIC);
	...
	probersp_p = (IEEEtypes_ProbeRsp_t *) vmacSta_p->probeRspBody.basic_ies;

	memcpy((void *)&probersp_p->BcnInterval, (void *)&bcn_interval, sizeof(bcn_interval));
	memcpy((void *)&probersp_p->CapInfo, (void *)&capinfo, sizeof(capinfo));
	SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->locks.probeRspIEsLock, flags);
}

void macMgmtMlme_UpdateProbeRspExtraIes(vmacApInfo_t * vmacSta_p, UINT8 * probeRspIe, UINT16 length)
{
	unsigned long flags;
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, vmacSta_p->dev);
	if (!vmacSta_p->master || !length)
		return;

	SPIN_LOCK_IRQSAVE(&wlpptr->wlpd_p->locks.probeRspIEsLock, flags);
	if (!vmacSta_p->probeRspBody.extra_ies)
		vmacSta_p->probeRspBody.extra_ies = wl_kmalloc(MAX_BEACON_SIZE, GFP_ATOMIC);
	...
	memcpy((void *)vmacSta_p->probeRspBody.extra_ies, (void *)probeRspIe, length);
...
void macMgmtMlme_UpdateProbeRspBasicIes(vmacApInfo_t * vmacSta_p, UINT8 * iep, UINT16 length)
{
	...
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, vmacSta_p->dev);
	IEEEtypes_ProbeRsp_t *probersp_p;
	IEEEtypes_InfoElementHdr_t *ie_found = NULL;
	IEEEtypes_InfoElementHdr_t *ie_added = (IEEEtypes_InfoElementHdr_t *) iep;
	u8 dummy_oui[4] = { 0xff, 0xff, 0xff, 0xff };
	...
	SPIN_LOCK_IRQSAVE(&wlpptr->wlpd_p->locks.probeRspIEsLock, flags);
	if (!vmacSta_p->probeRspBody.basic_ies)
		vmacSta_p->probeRspBody.basic_ies = wl_kzalloc(MAX_BEACON_SIZE, GFP_ATOMIC);

	if (!vmacSta_p->probeRspBody.basic_ies) {
		SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->locks.probeRspIEsLock, flags);
		return;
	}
	if (MAX_BEACON_SIZE < (vmacSta_p->probeRspBody.basic_len + length)) {
		SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->locks.probeRspIEsLock, flags);
		return;
	}

	probersp_p = (IEEEtypes_ProbeRsp_t *) vmacSta_p->probeRspBody.basic_ies;
	/* search if updated IE presented */
	ie_found = (IEEEtypes_InfoElementHdr_t *) FindIEWithinIEs((u8 *) & probersp_p->SsId,
								  (u32) vmacSta_p->probeRspBody.basic_len, ie_added->ElementId, dummy_oui);

	if (ie_found) {
		/* found and replace */
		memcpy((u8 *) ie_found, (u8 *) iep, length);
	} else {
		/* add if this is new added */
		memcpy((void *)((UINT8 *) & probersp_p->SsId + vmacSta_p->probeRspBody.basic_len), (void *)iep, length);
...
#ifdef SOC_W906X
void macMgmtMlme_UpdateProbeRspCsaIes(vmacApInfo_t * vmacSta_p, UINT8 * iep, UINT16 length)
{
	unsigned long flags;
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, vmacSta_p->dev);
	if (!vmacSta_p->master || !length)
	...
	if (!vmacSta_p->probeRspBody.csa_ies)
		vmacSta_p->probeRspBody.csa_ies = wl_kmalloc(MAX_BEACON_SIZE, GFP_ATOMIC);
...
void macMgmtMlme_UpdateExistedProbeRspIE(vmacApInfo_t * vmacSta_p, UINT8 * iep, UINT16 length)
{
	...
	/* If the specific ie is not in basic, search from extra_ies */
	if (!vmacSta_p->probeRspBody.extra_ies) {
		SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->locks.probeRspIEsLock, flags);
		return;
	}
	ie_found = (IEEEtypes_InfoElementHdr_t *) FindIEWithinIEs((u8 *) vmacSta_p->probeRspBody.extra_ies,
								  (u32) vmacSta_p->probeRspBody.extra_len, ie_added->ElementId, dummy_oui);
	if (ie_found) {
		/* found and replace */
		memcpy((u8 *) ie_found, (u8 *) iep, length);
		SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->locks.probeRspIEsLock, flags);
...
int skb_check(struct sk_buff *txSkb_p, int len, u8 * dump_p, int dump_len)
{
	if (len > skb_tailroom(txSkb_p)) {
		printk("Len:%d, skb_tail:%d, skb_end:%d\n", len, (unsigned int)txSkb_p->tail, (unsigned int)txSkb_p->end);
...
*
*****************************************************************************/
void macMgmtMlme_ProbeRqst1(vmacApInfo_t * vmacSta_p, macmgmtQ_MgmtMsg3_t * MgmtMsg_p)
{
#ifdef SOC_W906X
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, vmacSta_p->dev);
...
#define IW_CUSTOM_MAX2 512
	...
	if (isSsIdMatch(&MgmtMsg_p->Body.ProbeRqst.SsId, &vmacSta_p->macSsId) != SUCCESS) {
		if (*(mib->mib_broadcastssid) == TRUE) {
			if ((MgmtMsg_p->Body.ProbeRqst.SsId.Len != 0) && (*(mib->mib_mbssid) == 0)) {
...
#endif
	{
		...
		MgmtMsg_p->Hdr.FrmBodyLen = MgmtMsg_p->Hdr.FrmBodyLen - sizeof(IEEEtypes_MacAddr_t);
		memcpy(&buf[strlen(tag)], (char *)MgmtMsg_p, sizeof(IEEEtypes_MgmtHdr3_t) - sizeof(IEEEtypes_MacAddr_t));
		MgmtMsg_p->Hdr.FrmBodyLen = MgmtMsg_p->Hdr.FrmBodyLen + sizeof(IEEEtypes_MacAddr_t);
		memcpy(&buf[strlen(tag) + sizeof(IEEEtypes_MgmtHdr3_t) - sizeof(IEEEtypes_MacAddr_t)], (char *)&MgmtMsg_p->Body,
		       MgmtMsg_p->Hdr.FrmBodyLen - sizeof(IEEEtypes_MgmtHdr3_t) + sizeof(UINT16));

		memset(&wreq, 0, sizeof(wreq));
		wreq.data.length = strlen(tag) + MgmtMsg_p->Hdr.FrmBodyLen + sizeof(UINT16) - sizeof(IEEEtypes_MacAddr_t);
		...
		if (vmacSta_p->WPSOn) {
			...
			macmgmtQ_MgmtMsg3_t *Msg_p = (macmgmtQ_MgmtMsg3_t *) (buf + strlen(tag));
			mwl_send_vendor_wps_req_event(vmacSta_p->dev, ((UINT8 *) Msg_p) + 2, Msg_p->Hdr.FrmBodyLen, 0);
#endif				/* CFG80211_COMPATIABLE */
#endif				/* CFG80211 */
		}
	} else
	...
	if (((vmacSta_p->dev->flags & IFF_RUNNING) == 0) ||
	    (vmacSta_p->probeRspBody.basic_ies == NULL) || (vmacSta_p->probeRspBody.extra_ies == NULL)) {
		SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->locks.probeRspIEsLock, flags);
		return;
	}
#endif
	if ((txSkb_p = mlmeApiPrepMgtMsg(IEEE_MSG_PROBE_RSP, &MgmtMsg_p->Hdr.SrcAddr, &vmacSta_p->macBssId)) == NULL) {
		SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->locks.probeRspIEsLock, flags);
		return;
	}
...
#else
	len_before_ssid = sizeof(IEEEtypes_TimeStamp_t) + sizeof(IEEEtypes_BcnInterval_t) + sizeof(IEEEtypes_CapInfo_t);

	if (skb_check(txSkb_p, len_before_ssid, (u8 *) vmacSta_p->probeRspBody.basic_ies, len_before_ssid)) {
		SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->locks.probeRspIEsLock, flags);
		printk("[%s] not enough space for TimeStamp,BcnInterval,CapInfo\n", __func__);
		return;
	}

	memcpy((void *)&MgmtRsp_p->Body.ProbeRsp, (void *)vmacSta_p->probeRspBody.basic_ies, len_before_ssid);

	ssid_p = (IEEEtypes_SsIdElement_t *) (vmacSta_p->probeRspBody.basic_ies + len_before_ssid);

//Need it or not?
//if (vmacSta_p->VMacEntry.macId == wlpptr->wlpd_p->mbssSet[0].primbss) {
	if (*(mib->mib_broadcastssid) && (ssid_p->ElementId == SSID)) {
		out_ssid_len = in_ssid_len = sizeof(IEEEtypes_InfoElementHdr_t) + ssid_p->Len;

		if (skb_check(txSkb_p, len_before_ssid + out_ssid_len, (u8 *) vmacSta_p->probeRspBody.basic_ies + len_before_ssid, out_ssid_len)) {
			SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->locks.probeRspIEsLock, flags);
			printk("[%s] not enough space for SSID\n", __func__);
			return;
		}

		memcpy((void *)&MgmtRsp_p->Body.ProbeRsp + len_before_ssid,
		       (void *)vmacSta_p->probeRspBody.basic_ies + len_before_ssid, out_ssid_len);
	} else if ((MgmtMsg_p->Body.ProbeRqst.SsId.Len != 0)
		   && (MgmtMsg_p->Body.ProbeRqst.SsId.ElementId == SSID)) {
		out_ssid_len = sizeof(IEEEtypes_InfoElementHdr_t) + MgmtMsg_p->Body.ProbeRqst.SsId.Len;

		if (skb_check(txSkb_p, len_before_ssid + out_ssid_len, (u8 *) & MgmtMsg_p->Body.ProbeRqst.SsId, out_ssid_len)) {
			SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->locks.probeRspIEsLock, flags);
			printk("[%s] not enough space for SSID\n", __func__);
			return;
		}

		memcpy((void *)&MgmtRsp_p->Body.ProbeRsp + len_before_ssid, (void *)&MgmtMsg_p->Body.ProbeRqst.SsId, out_ssid_len);
	}
//} else {
...
//}
	...
	supprates_p = (UINT8 *) & MgmtRsp_p->Body.ProbeRsp + len_before_ssid + out_ssid_len;

	if (vmacSta_p->SuppRateSet.Len) {
		supprates_len = sizeof(IEEEtypes_InfoElementHdr_t) + vmacSta_p->SuppRateSet.Len;

		if (skb_check(txSkb_p, supprates_len, (u8 *) & vmacSta_p->SuppRateSet, supprates_len)) {
			SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->locks.probeRspIEsLock, flags);
			printk("[%s] not enough space for SuppRateSet\n", __func__);
			return;
		}

		memcpy((void *)supprates_p, (void *)&vmacSta_p->SuppRateSet, supprates_len);
	...
	if (skb_check
	    (txSkb_p, vmacSta_p->probeRspBody.basic_len - in_ssid_len, (u8 *) ssid_p + in_ssid_len,
	...
	memcpy((void *)(supprates_p + supprates_len), (void *)((UINT8 *) ssid_p + in_ssid_len), vmacSta_p->probeRspBody.basic_len - in_ssid_len);

	skb_put(txSkb_p, vmacSta_p->probeRspBody.basic_len - in_ssid_len);

	extsupprates_p = supprates_p + supprates_len + vmacSta_p->probeRspBody.basic_len - in_ssid_len;

	if (vmacSta_p->ExtSuppRateSet.Len) {
		extsupprates_len = sizeof(IEEEtypes_InfoElementHdr_t) + vmacSta_p->ExtSuppRateSet.Len;

		if (skb_check(txSkb_p, extsupprates_len, (u8 *) & vmacSta_p->ExtSuppRateSet, extsupprates_len)) {
			SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->locks.probeRspIEsLock, flags);
			printk("[%s] not enough space for ExtSuppRateSet\n", __func__);
			return;
		}

		memcpy((void *)extsupprates_p, (void *)&vmacSta_p->ExtSuppRateSet, extsupprates_len);
...
#endif				/* WMM_AC_EDCA */
		QBSSLoad_ie.channel_util = MSAN_get_channel_util(vmacSta_p);

		if (skb_check(txSkb_p, sizeof(QBSSLoad_ie), (u8 *) & QBSSLoad_ie, sizeof(QBSSLoad_ie))) {
			SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->locks.probeRspIEsLock, flags);
			printk("[%s] not enough space for QBSSLoad_ie\n", __func__);
			return;
		}

		memcpy((void *)((UINT8 *) extsupprates_p + extsupprates_len), (void *)&QBSSLoad_ie, sizeof(QBSSLoad_ie));
		skb_put(txSkb_p, sizeof(QBSSLoad_ie));
		extsupprates_len += sizeof(QBSSLoad_ie);
	}
#endif				/* IEEE80211K */

	if (skb_check(txSkb_p, vmacSta_p->probeRspBody.extra_len, (u8 *) vmacSta_p->probeRspBody.extra_ies, vmacSta_p->probeRspBody.extra_len)) {
		SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->locks.probeRspIEsLock, flags);
	...
	if (wlpptr->devid == SCBT || wlpptr->devid == SC5) {
		...
		SINT32 VarLen = MgmtMsg_p->Hdr.FrmBodyLen + sizeof(UINT16) - sizeof(IEEEtypes_MgmtHdr3_t);
		...
		IEEEtypes_VendorSpec_VHT_Element_t *pVendorSpecVht = NULL;
		...
		while (VarLen > 0) {
			...
			case EXT_IE:
				pExtIE = (IEEEtypes_InfoElementExtHdr_t *) pIE;
				if (pExtIE->ext == HE_CAPABILITIES_IE) {
					Has_HE = 1;
					pHeCap = (HE_Capabilities_IE_t *) (&pExtIE->ext);
				}
				break;
			case VHT_CAP:
				pVhtCap = (IEEEtypes_VhtCap_t *) pIE;
				if ((pVhtCap->SupportedRxMcsSet & 0xc000) != 0xc000)
					vhtCapNss = 8;
				else if ((pVhtCap->SupportedRxMcsSet & 0x3000) != 0x3000)
					vhtCapNss = 7;
				else if ((pVhtCap->SupportedRxMcsSet & 0xc00) != 0xc00)
					vhtCapNss = 6;
				else if ((pVhtCap->SupportedRxMcsSet & 0x300) != 0x300)
					vhtCapNss = 5;
				else if ((pVhtCap->SupportedRxMcsSet & 0xc0) != 0xc0)
					vhtCapNss = 4;
				else if ((pVhtCap->SupportedRxMcsSet & 0x30) != 0x30)
					vhtCapNss = 3;
				else if ((pVhtCap->SupportedRxMcsSet & 0xc) != 0xc)
					vhtCapNss = 2;
				break;
			case OP_MODE_NOTIFICATION:
				pVhtOmn = (IEEEtypes_VHT_op_mode_t *) pIE;
				vhtOmnNss = pVhtOmn->OperatingMode.RxNss + 1;
				break;
			case PROPRIETARY_IE:
				pIEbody = (UINT8 *) pIE + sizeof(IEEEtypes_InfoElementHdr_t);
				if ((pIE->Len >= 5) && (memcmp(pIEbody, &oui[0], 3) == 0) &&	// Epigram
				    (pIEbody[3] == 0x4) &&	// VENDOR_VHT_TYPE
				    (pIEbody[4] == 0x8))	// VENDOR_VHT_SUBTYPE
					pVendorSpecVht = (IEEEtypes_VendorSpec_VHT_Element_t *) pIE;
				break;
			default:
				break;
			}
			pData += sizeof(IEEEtypes_InfoElementHdr_t) + pIE->Len;
			VarLen -= sizeof(IEEEtypes_InfoElementHdr_t) + pIE->Len;
		}
		...
		while (VarLen > 0) {
			...
			switch (pIE->ElementId) {
			case ADD_HT:
				add_ht = (IEEEtypes_Add_HT_Element_t *) pIE;
				/* Update the operating channel for the case of channel switch
				 */
				add_ht->ControlChan = PhyDSSSTable->CurrChan;

				/* for 5G, if STA Channel Width is 1, set the Secondary Channel Offset */
				if ((PhyDSSSTable->Chanflag.FreqBand == FREQ_BAND_5GHZ) && add_ht->AddChan.STAChannelWidth) {
					add_ht->AddChan.ExtChanOffset = PhyDSSSTable->Chanflag.ExtChnlOffset;
				}
#ifdef COEXIST_20_40_SUPPORT
				if (*(vmacSta_p->ShadowMib802dot11->mib_HT40MIntoler)) {
					if (PhyDSSSTable->Chanflag.FreqBand == FREQ_BAND_2DOT4GHZ &&
						...
						if (wlpptr->wlpd_p->BcnAddHtAddChannel == 0) {
							add_ht->AddChan.STAChannelWidth = 0;
							add_ht->AddChan.ExtChanOffset = 0;
						} else {
							add_ht->AddChan.STAChannelWidth = 1;
							add_ht->AddChan.ExtChanOffset =
							    vmacSta_p->ShadowMib802dot11->PhyDSSSTable->Chanflag.ExtChnlOffset;
						}
					}
				} else {
					if ((PhyDSSSTable->Chanflag.FreqBand == FREQ_BAND_2DOT4GHZ) && add_ht->AddChan.STAChannelWidth) {
						add_ht->AddChan.ExtChanOffset = PhyDSSSTable->Chanflag.ExtChnlOffset;
					}
				}
#endif
				if (PhyDSSSTable->Chanflag.FreqBand == FREQ_BAND_2DOT4GHZ)
					add_ht->OpMode.OpMode = (wlpptr->wlpd_p->BcnAddHtOpMode & 0x3);
				break;
			case VHT_CAP:
				if (!pHeCap && pVhtCap) {
					pVhtcapProbeRsp = (IEEEtypes_VhtCap_t *) ((UINT8 *) extsupprates_p + extsupprates_len);
				}
				break;
			case PROPRIETARY_IE:
				pIEbody = (UINT8 *) pIE + sizeof(IEEEtypes_InfoElementHdr_t);
				if (!pVendorSpecVht && (*(mib->mib_ApMode) & AP_MODE_11AC))
					if ((pIE->Len >= 5) && (memcmp(pIEbody, &oui[0], 3) == 0) &&	// Epigram
					    (pIEbody[3] == 0x4) &&	// VENDOR_VHT_TYPE
					    (pIEbody[4] == 0x8))	// VENDOR_VHT_SUBTYPE
						skip = TRUE;
			...
			if (!skip) {
				memcpy((void *)((UINT8 *) extsupprates_p + extsupprates_len),
				       (void *)pIE, sizeof(IEEEtypes_InfoElementHdr_t) + pIE->Len);
				extsupprates_len += sizeof(IEEEtypes_InfoElementHdr_t) + pIE->Len;
				skb_put(txSkb_p, sizeof(IEEEtypes_InfoElementHdr_t) + pIE->Len);
			}
			pData += sizeof(IEEEtypes_InfoElementHdr_t) + pIE->Len;
			VarLen -= sizeof(IEEEtypes_InfoElementHdr_t) + pIE->Len;
		}
		...
		if (pVhtcapProbeRsp) {
			...
			if (peerVhtNss < 3)
				peerVhtNss = 3;
			pVhtcapProbeRsp->cap.CompressedSteeringNumberofBeamformerAntennaSupported = 0x3;
		}
	} else {
		memcpy((void *)((UINT8 *) extsupprates_p + extsupprates_len),
		       (void *)vmacSta_p->probeRspBody.extra_ies, vmacSta_p->probeRspBody.extra_len);
		skb_put(txSkb_p, vmacSta_p->probeRspBody.extra_len);
		extsupprates_len += vmacSta_p->probeRspBody.extra_len;
	}

#ifdef SOC_W906X
	if (vmacSta_p->probeRspBody.csa_ies && vmacSta_p->probeRspBody.csa_len) {
		if (skb_check(txSkb_p, vmacSta_p->probeRspBody.csa_len, (u8 *) vmacSta_p->probeRspBody.csa_ies, vmacSta_p->probeRspBody.csa_len)) {
			SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->locks.probeRspIEsLock, flags);
			printk("[%s] not enough space for csa_ies %d bytes\n", __func__, vmacSta_p->probeRspBody.csa_len);
			return;
		}
		memcpy((void *)((UINT8 *) extsupprates_p + extsupprates_len), vmacSta_p->probeRspBody.csa_ies, vmacSta_p->probeRspBody.csa_len);
	...
	/** add Extended Cap IE here **/
	if ((*(vmacSta_p->Mib802dot11->mib_ApMode) & AP_MODE_11AC) && !Is5GBand(*(vmacSta_p->Mib802dot11->mib_ApMode))) {
		if ((skb_tailroom(txSkb_p) > sizeof(IEEEtypes_Extended_Cap_Element_t)) &&
		    FindIEWithinIEs(txSkb_p->data + sizeof(MgmtRsp_p->Hdr) + len_before_ssid,
				    txSkb_p->len - sizeof(MgmtRsp_p->Hdr) - len_before_ssid, EXT_CAP_IE, NULL) == NULL) {
			extern UINT16 AddExtended_Cap_IE(vmacApInfo_t * vmacSta_p, IEEEtypes_Extended_Cap_Element_t * pNextElement);
			IEEEtypes_Extended_Cap_Element_t *pExtCap;
			...
			memset((void *)pExtCap, 0, sizeof(IEEEtypes_Extended_Cap_Element_t));
			AddExtended_Cap_IE(vmacSta_p, pExtCap);
			extsupprates_len += sizeof(IEEEtypes_Extended_Cap_Element_t);
		}
	}
#ifdef SOC_W906X
	...
	if (!(*(mib->mib_ApMode) & AP_MODE_11AX) || !Has_HE) {
		UINT8 ie_len = 0;
		UINT8 *ie = FindIEWithinIEs(txSkb_p->data + sizeof(MgmtRsp_p->Hdr) + len_before_ssid,
					    txSkb_p->len - sizeof(MgmtRsp_p->Hdr) - len_before_ssid, EXT_CAP_IE, NULL);
		if (ie) {
			ie_len = *((UINT8 *) (ie + 1));
			if (ie_len == 11) {	/* 11 = sizeof(IEEEtypes_Extended_Cap_Element_t) - 2 */
				UINT8 *ie_end = ie + 2 + ie_len;
	...
	memset(&MgmtRsp_p->Body.ProbeRsp.TimeStamp[0], 0, sizeof(IEEEtypes_TimeStamp_t));
	...
	memcpy(&MgmtRsp_p->Body.ProbeRsp.TimeStamp[0], &bsstsf64, sizeof(bsstsf64));
	//printk("probResp: hwtsf:%08llx, bsstsf:%08llx, bsstsfbase:%08llx\n",hwtsf64,bsstsf64,vmacSta_p->BssTsfBase);

#endif				/* SOC_W906X */
#else
	if ((TxMsg_p = mlmeApiPrepMgtMsg(IEEE_MSG_PROBE_RSP, &MgmtMsg_p->Hdr.SrcAddr, &vmacSta_p->macBssId)) == NULL)
	{
		return;
	}
#endif
	...
	if (memcmp(MgmtMsg_p->Hdr.DestAddr, bctAddr, 6) == 0) {
		...
		if (vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_AP) {
			...
			if ((isMacAccessList(vmacSta_p, &(MgmtMsg_p->Hdr.SrcAddr)) == SUCCESS) ||
			    ((*(mib->mib_broadcastssid) == FALSE) && (MgmtMsg_p->Body.ProbeRqst.SsId.Len &&
								      !memcmp((mib->StationConfig->DesiredSsId), MgmtMsg_p->Body.ProbeRqst.SsId.SsId,
									      MgmtMsg_p->Body.ProbeRqst.SsId.Len))))
...
void macMgmtMlme_WAPI_event(struct net_device *dev, int event_type, u16 auth_type, IEEEtypes_MacAddr_t * sta_addr, IEEEtypes_MacAddr_t * ap_addr,
			    char *info)
{
	...
	*(UINT32 *) & (asso_mt_info.gsn[12]) = *(UINT32 *) & (vmacSta_p->wapiPN_mc[0]);
	...
	if (info) {
		...
		WLDBG_INFO(DBG_LEVEL_7, "### Driver (%s), extra len %d, ap mac %s,  len1=%d, len2=%d\n", __FUNCTION__, extra_len,
			   mac_display((const UINT8 *)ap_addr), sizeof(asso_mt_info), sizeof(asso_mt_info.wie));
	}
...
void ProcessTWTsetup(vmacApInfo_t * vmacSta_p, macmgmtQ_MgmtMsg_t * MgmtMsg_p)
{
	...
	if ((pStaInfo = extStaDb_GetStaInfo(vmacSta_p, &(MgmtMsg_p->Hdr.SrcAddr), STADB_UPDATE_AGINGTIME)) == NULL)
		goto done;

	ptwt = (IEEEtypes_TWT_Element_t *) & (MgmtMsg_p->Body.Action.Data.TwtElement);
	...
	if (ptwt->ctrl.NegoType & 0x2) {
		...
		pIdvl = (Individual_set_fd_t *) & ptwt->info.IndvParamSet;
		...
		if (pIdvl->ReqType.SetupCmd < TWT_SETUP_CMD_GROUPING) {
			...
			if (flowid == 0) {
				...
				for (idx = 1; idx < ITWT_AGREEMENT_PER_STA; idx++) {
					if (!(map & (1 << idx)))
						break;
				}
				...
				flowid = idx;
				pIdvl->ReqType.FlowID_or_Bcast_Recom = idx;
			...
			memcpy((void *)pflow, (void *)ptwt,
			       ((ptwt->Len > sizeof(IEEEtypes_TWT_Element_t) - 2) ? sizeof(IEEEtypes_TWT_Element_t) : (ptwt->Len + 2)));
			...
			case TWT_SETUP_CMD_REQUEST:
				{	//sta did not provide twt, accept and replay a twt to client
					...
					if (wfa_11ax_pf) {
						...
						pIdvlDB->NomMinWakeDur -= wfa_itwt_wakedur_early_end;
					}
				}
				break;
			case TWT_SETUP_CMD_SUGGEST:
			case TWT_SETUP_CMD_DEMAND:
				{
					...
					if (1) {
						...
						pIdvlDB->TargetWakeTime = curtsf + 200000;
						CmdReply = TWT_SETUP_CMD_ALTERNATE;
					}
		...
		twteLen = ((ptwt->ctrl.NDP_PagingIndicator) ? sizeof(Individual_set_fd_t) : (sizeof(Individual_set_fd_t) - 4)) + 3;
		actLen = twteLen + 3;	//total length, action hdr (cat(1), action(1), token(1)))

		//prepare Reply cmd 
		if ((txSkb_p = mlmeApiPrepMgtMsg2(IEEE_MSG_QOS_ACTION, &MgmtMsg_p->Hdr.SrcAddr, &vmacSta_p->macBssId, actLen)) == NULL) {
		...
		MgmtRsp_p = (macmgmtQ_MgmtMsg2_t *) txSkb_p->data;
		ptwtResp = (IEEEtypes_TWT_Element_t *) & (MgmtRsp_p->Body.Action.Data.TwtElement);
		MgmtRsp_p->Body.Action.Category = S1G;
		MgmtRsp_p->Body.Action.Action = TWT_SETUP;
						    /** same enum as addba_resp **/
		MgmtRsp_p->Body.Action.DialogToken = MgmtMsg_p->Body.Action.DialogToken;
		switch (CmdReply) {
		case TWT_SETUP_CMD_ACCEPT:
			{
				param.Twt = pIdvlDB->TargetWakeTime;
				param.MinWakeDur = pIdvlDB->NomMinWakeDur;
				param.WakeInvlMantissa = pIdvlDB->WakeInvlMantissa;
				param.WakeInvlExpo = pIdvlDB->ReqType.WakeIvlExp;
				param.Trigger = pIdvlDB->ReqType.Trigger;
				param.FlowType = pIdvlDB->ReqType.FlowType;
			}
		case TWT_SETUP_CMD_DICTATE:
		case TWT_SETUP_CMD_ALTERNATE:
		case TWT_SETUP_CMD_REJECT:
			{
				Individual_set_fd_t *pidv = &ptwtResp->info.IndvParamSet;

				memcpy((void *)ptwtResp, pflow, twteLen);
		...
		if (txMgmtMsg(vmacSta_p->dev, txSkb_p) != OS_SUCCESS) {
			...
			if (CmdReply == TWT_SETUP_CMD_ACCEPT) {
				...
				if (!wlFwTwtParam(netdev, WL_SET, (MgmtMsg_p->Hdr.SrcAddr), flowid, &param)) {

					pStaInfo->TwtAgree.ActFlowID |= (1 << flowid);

					printk("Config PFW StaDB:%02X%02X%02X%02X%02X%02X TWT param success\n",
					       MgmtMsg_p->Hdr.SrcAddr[0], MgmtMsg_p->Hdr.SrcAddr[1], MgmtMsg_p->Hdr.SrcAddr[2],
					...
					printk("twt:0x%llx, MinWakeDur:%u (256us unit), wakeivl:%u, trigger:%u, FlowType:%u, FlowId:%u\n", param.Twt,
					       param.MinWakeDur, ivl, param.Trigger, param.FlowType, flowid);
				}
			}
...
void ProcessTWTteardown(vmacApInfo_t * vmacSta_p, macmgmtQ_MgmtMsg_t * MgmtMsg_p)
{
	...
	if (!wlFwTwtParam(netdev, WL_RESET, MgmtMsg_p->Hdr.SrcAddr, flowid, NULL)) {
		...
		if (pStaInfo) {
			pStaInfo->TwtAgree.ActFlowID &= (~(1 << flowid));
			memset(&pStaInfo->TwtAgree.twtE[flowid], 0, sizeof(IEEEtypes_TWT_Element_t));
		}
...
//twt
void ProcessTWTInformation(vmacApInfo_t * vmacSta_p, macmgmtQ_MgmtMsg_t * MgmtMsg_p)
{
	//struct wlprivate    *wlpptr   = NETDEV_PRIV_P(struct wlprivate, vmacSta_p->dev);
	//struct net_device *netdev = (wlpptr->master) ?  wlpptr->master : wlpptr->netDev; 
	//u8 *p = (MgmtMsg_p->Hdr.SrcAddr); 
	...
	ptwtinfo = (IEEEtypes_TWT_Information_t *) & (((u8 *) & MgmtMsg_p->Body)[2]);
	flowid = ptwtinfo->FlowID;

	printk("TWT Info: flowid:%u, RspReq:%u, NextReq:%u, NextSubfieldSize:%u, AllTWT:%u, NextTwt:0x%llx",
	       flowid, ptwtinfo->Rsp_Req, ptwtinfo->NextTWT_Req, ptwtinfo->NextTWT_Subfd_Size, ptwtinfo->AllTWT, ptwtinfo->NextTWT);
...
/*
	...
	return:
		...
		- FALSE: not exist
*/
BOOLEAN _is_mac_exist(IEEEtypes_MacAddr_t chk_macaddr, mac_pool * macpool_p)
{
	BOOLEAN res = false;
	UINT8 i;
	for (i = 0; i < MAC_POOL_SIZE; i++) {
		if (!memcmp(macpool_p->mac_pool[i], chk_macaddr, sizeof(IEEEtypes_MacAddr_t))) {
			res = TRUE;
			break;
		}
	}
...
UINT8 get_ampdu_rx_disable_flag(vmacApInfo_t * vmacSta_p, macmgmtQ_MgmtMsg3_t * MgmtMsg_p)
{
	UINT8 Ampdu_Rx_Disable_Flag_l = vmacSta_p->Ampdu_Rx_Disable_Flag;
	...
	BOOLEAN is_except = _is_mac_exist(MgmtMsg_p->Hdr.SrcAddr,
					  ((Ampdu_Rx_Disable_Flag_l == TRUE) ? (&vmacSta_p->ampdu_acpt_pool) : (&vmacSta_p->ampdu_rejt_pool)));
	...
	Ampdu_Rx_Disable_Flag_l = (is_except == TRUE) ? (!Ampdu_Rx_Disable_Flag_l) : Ampdu_Rx_Disable_Flag_l;
...
extern void macMgmtMlme_QoSAct(vmacApInfo_t * vmacSta_p, macmgmtQ_MgmtMsg3_t * MgmtMsg_p)
{
	...
	case WFA:
		...
		case ADDTS_REQ:
			{
				IEEEtypes_WFA_ADDTS_Rsp_t *pWfaAddTsRspFrm;
				IEEEtypes_WFA_ADDTS_Req_t *pWfaAddTsReqFrm;
				...
				if (pStaInfo) {
					...
					if ((txSkb_p = mlmeApiPrepMgtMsg2(IEEE_MSG_QOS_ACTION,
									  &MgmtMsg_p->Hdr.SrcAddr, &vmacSta_p->macBssId,
									  sizeof(IEEEtypes_WFA_ADDTS_Rsp_t))) == NULL) {
						printk("macMgmtMlme_QoSAct(): mlmeApiPrepMgtMsg failed\n");
						return;
					}
					MgmtRsp_p = (macmgmtQ_MgmtMsg2_t *) txSkb_p->data;
#else
					if ((TxMsg_p = mlmeApiPrepMgtMsg2(IEEE_MSG_QOS_ACTION,
									  &MgmtMsg_p->Hdr.SrcAddr, &vmacSta_p->macBssId,
									  sizeof(IEEEtypes_WFA_ADDTS_Rsp_t))) == NULL) {
						printk("macMgmtMlme_QoSAct(): mlmeApiPrepMgtMsg failed\n");
						return;
					...
					pWfaAddTsReqFrm = (IEEEtypes_WFA_ADDTS_Req_t *) & MgmtMsg_p->Body;
					pWfaAddTsRspFrm = (IEEEtypes_WFA_ADDTS_Rsp_t *) & MgmtRsp_p->Body;
#ifdef MV_CPU_BE
					pWfaAddTsReqFrm->TSpec.ts_info.u16_data = ENDIAN_SWAP16(pWfaAddTsReqFrm->TSpec.ts_info.u16_data);
#endif
					...
					Status = ProcessDELTSRequest(vmacSta_p, (IEEEtypes_MacAddr_t *) MgmtMsg_p->Hdr.SrcAddr,
								     pWfaAddTsReqFrm->TSpec.ts_info.tsid);
					//Process ADDTS Request. Return a ADDTS Response.
					Status = ProcessADDTSRequest_WFA(vmacSta_p, (IEEEtypes_WFA_ADDTS_Req_t *) & MgmtMsg_p->Body,
									 &MgmtMsg_p->Hdr.SrcAddr, pStaInfo->Aid,
									 &TspecDBindex, pStaInfo->ClientMode, &MediumTime);

#if 0				//dbg
					printk("pWfaAddTsReqFrm\n");
					printk
					    ("ts_info: traffic_type 0x%x, tsid 0x%x, direction 0x%x, access_policy 0x%x, aggregation 0x%x, apsd 0x%x, usr_priority 0x%x, ts_info_ack_policy 0x%x, sched 0x%x, rsvd 0x%x\n",
					     pWfaAddTsReqFrm->TSpec.ts_info.traffic_type, pWfaAddTsReqFrm->TSpec.ts_info.tsid,
					     pWfaAddTsReqFrm->TSpec.ts_info.direction, pWfaAddTsReqFrm->TSpec.ts_info.access_policy,
					     pWfaAddTsReqFrm->TSpec.ts_info.aggregation, pWfaAddTsReqFrm->TSpec.ts_info.apsd,
					     pWfaAddTsReqFrm->TSpec.ts_info.usr_priority, pWfaAddTsReqFrm->TSpec.ts_info.ts_info_ack_policy,
					     pWfaAddTsReqFrm->TSpec.ts_info.sched, pWfaAddTsReqFrm->TSpec.ts_info.rsvd);
					printk
					    ("nom_msdu_size 0x%x, max_msdu_size 0x%x, min_SI 0x%x, max_SI 0x%x, inactive_intrvl 0x%x, suspen_intrvl 0x%x\n",
					     pWfaAddTsReqFrm->TSpec.nom_msdu_size, pWfaAddTsReqFrm->TSpec.max_msdu_size,
					     pWfaAddTsReqFrm->TSpec.min_SI, pWfaAddTsReqFrm->TSpec.max_SI, pWfaAddTsReqFrm->TSpec.inactive_intrvl,
					     pWfaAddTsReqFrm->TSpec.suspen_intrvl);
					printk
					    ("serv_start_time 0x%x, min_data_rate 0x%x, mean_data_rate 0x%x, peak_data_rate 0x%x, max_burst_size 0x%x\n",
					     pWfaAddTsReqFrm->TSpec.serv_start_time, pWfaAddTsReqFrm->TSpec.min_data_rate,
					     pWfaAddTsReqFrm->TSpec.mean_data_rate, pWfaAddTsReqFrm->TSpec.peak_data_rate,
					     pWfaAddTsReqFrm->TSpec.max_burst_size);
					printk("delay_bound 0x%x, min_phy_rate 0x%x, srpl_bw_allow 0x%x, med_time 0x%x\n",
					       pWfaAddTsReqFrm->TSpec.delay_bound, pWfaAddTsReqFrm->TSpec.min_phy_rate,
					       pWfaAddTsReqFrm->TSpec.srpl_bw_allow, pWfaAddTsReqFrm->TSpec.med_time);
#endif

					pWfaAddTsRspFrm->Category = pWfaAddTsReqFrm->Category;
					pWfaAddTsRspFrm->Action = ADDTS_RSP;
					pWfaAddTsRspFrm->DialogToken = pWfaAddTsReqFrm->DialogToken;
					if (Status != IEEEtypes_STATUS_SUCCESS) {
					...
					if (pWfaAddTsReqFrm->TSpec.ts_info.access_policy == EDCA) {
						if (Status == IEEEtypes_STATUS_SUCCESS) {
							if (pWfaAddTsReqFrm->TSpec.ts_info.direction != DOWNLINK) {
								pWfaAddTsRspFrm->TSpec.med_time = ENDIAN_SWAP16(MediumTime);
							} else {	//DOWNLINK
								pWfaAddTsRspFrm->TSpec.med_time = 0;
							}
						} else {
							pWfaAddTsRspFrm->TSpec.med_time = 0;
						}
					...
					else	//HCCA / BOTH
					{
						...
						ProcessADDTSRequestSchedule(pWfaAddTsRspFrm, TspecDBindex);
					}
#endif
#ifdef MV_CPU_BE
					pWfaAddTsRspFrm->TSpec.ts_info.u16_data = ENDIAN_SWAP16(pWfaAddTsRspFrm->TSpec.ts_info.u16_data);
#endif
#if 0				//dbg
					printk("pWfaAddTsRspFrm\n");
					printk
					    ("ts_info: traffic_type 0x%x, tsid 0x%x, direction 0x%x, access_policy 0x%x, aggregation 0x%x, apsd 0x%x, usr_priority 0x%x, ts_info_ack_policy 0x%x, sched 0x%x, rsvd 0x%x\n",
					     pWfaAddTsRspFrm->TSpec.ts_info.traffic_type, pWfaAddTsRspFrm->TSpec.ts_info.tsid,
					     pWfaAddTsRspFrm->TSpec.ts_info.direction, pWfaAddTsRspFrm->TSpec.ts_info.access_policy,
					     pWfaAddTsRspFrm->TSpec.ts_info.aggregation, pWfaAddTsRspFrm->TSpec.ts_info.apsd,
					     pWfaAddTsRspFrm->TSpec.ts_info.usr_priority, pWfaAddTsRspFrm->TSpec.ts_info.ts_info_ack_policy,
					     pWfaAddTsRspFrm->TSpec.ts_info.sched, pWfaAddTsRspFrm->TSpec.ts_info.rsvd);
					printk
					    ("nom_msdu_size 0x%x, max_msdu_size 0x%x, min_SI 0x%x, max_SI 0x%x, inactive_intrvl 0x%x, suspen_intrvl 0x%x\n",
					     pWfaAddTsRspFrm->TSpec.nom_msdu_size, pWfaAddTsRspFrm->TSpec.max_msdu_size,
					     pWfaAddTsRspFrm->TSpec.min_SI, pWfaAddTsRspFrm->TSpec.max_SI, pWfaAddTsRspFrm->TSpec.inactive_intrvl,
					     pWfaAddTsRspFrm->TSpec.suspen_intrvl);
					printk
					    ("serv_start_time 0x%x, min_data_rate 0x%x, mean_data_rate 0x%x, peak_data_rate 0x%x, max_burst_size 0x%x\n",
					     pWfaAddTsRspFrm->TSpec.serv_start_time, pWfaAddTsRspFrm->TSpec.min_data_rate,
					     pWfaAddTsRspFrm->TSpec.mean_data_rate, pWfaAddTsRspFrm->TSpec.peak_data_rate,
					     pWfaAddTsRspFrm->TSpec.max_burst_size);
					printk("delay_bound 0x%x, min_phy_rate 0x%x, srpl_bw_allow 0x%x, med_time 0x%x\n",
					       pWfaAddTsRspFrm->TSpec.delay_bound, pWfaAddTsRspFrm->TSpec.min_phy_rate,
					       pWfaAddTsRspFrm->TSpec.srpl_bw_allow, pWfaAddTsRspFrm->TSpec.med_time);
#endif
#ifdef AP_MAC_LINUX
					if (txMgmtMsg(vmacSta_p->dev, txSkb_p) != OS_SUCCESS) {
						wl_free_skb(txSkb_p);
		...
		case DELTS:
			{
				...
				pDelTSFrm = (IEEEtypes_WFA_DELTS_Req_t *) & MgmtMsg_p->Body;
#ifdef MV_CPU_BE
				pDelTSFrm->TSpec.ts_info.u16_data = ENDIAN_SWAP16(pDelTSFrm->TSInfo.ts_info.u16_data);
#endif
#if 0				//dbg
				printk("DELTS tsid 0x%x\n", pDelTSFrm->TSpec.ts_info.tsid);
#endif
				ProcessDELTSRequest(vmacSta_p, &MgmtMsg_p->Hdr.SrcAddr, (UINT32) pDelTSFrm->TSpec.ts_info.tsid);
			}
			break;
	...
	case QoS:
		...
		case ADDTS_REQ:
			...
			if (pStaInfo) {
				/* Allocate space for response message */
#ifdef AP_MAC_LINUX
				if ((txSkb_p = mlmeApiPrepMgtMsg(IEEE_MSG_QOS_ACTION, &MgmtMsg_p->Hdr.SrcAddr, &vmacSta_p->macBssId)) == NULL)
					return;
				MgmtRsp_p = (macmgmtQ_MgmtMsg2_t *) txSkb_p->data;
#else
				if ((TxMsg_p = mlmeApiPrepMgtMsg(IEEE_MSG_QOS_ACTION, &MgmtMsg_p->Hdr.SrcAddr, &vmacSta_p->macBssId)) == NULL) {
					return;
				}
#endif
...
#ifdef MV_CPU_BE
				pAddTsReqFrm->TSpec.ts_info.u16_data = ENDIAN_SWAP16(pAddTsReqFrm->TSpec.ts_info.u16_data);
#endif
				/*If a TSpec with the same MAC Addr and TsId exists then
				   first delete it and then add a new one */
				Status = ProcessDELTSRequest(vmacSta_p, (IEEEtypes_MacAddr_t *) MgmtMsg_p->Hdr.SrcAddr,
				...
				pAddTsRspFrm->StatusCode = ENDIAN_SWAP16(Status);
...
#ifdef WMM_AC_EDCA
				if (pAddTsReqFrm->TSpec.ts_info.access_policy == EDCA) {
					if (Status == IEEEtypes_STATUS_SUCCESS) {
						if (pAddTsReqFrm->TSpec.ts_info.direction != DOWNLINK) {
							pAddTsRspFrm->TSpec.med_time = ENDIAN_SWAP16(MediumTime);
						} else {	//DOWNLINK
							pAddTsRspFrm->TSpec.med_time = 0;
						}
					} else {
						pAddTsRspFrm->TSpec.med_time = 0;
					}
...
#endif				/* WMM_AC_EDCA */
#ifdef MV_CPU_BE
				pAddTsRspFrm->TSpec.ts_info.u16_data = ENDIAN_SWAP16(pAddTsRspFrm->TSpec.ts_info.u16_data);
#endif
#ifdef AP_MAC_LINUX
				if (txMgmtMsg(vmacSta_p->dev, txSkb_p) != OS_SUCCESS) {
					wl_free_skb(txSkb_p);
...
#ifdef MV_CPU_BE
			pDelTSFrm->TSInfo.ts_info.u16_data = ENDIAN_SWAP16(pDelTSFrm->TSInfo.ts_info.u16_data);
#endif

			ProcessDELTSRequest(vmacSta_p, &MgmtMsg_p->Hdr.SrcAddr, (UINT32) pDelTSFrm->TSInfo.ts_info.tsid);
			break;
		default:
			break;
	...
	case BlkAck:
		...
		case ADDBA_REQ:
			...
			if (pStaInfo && (pStaInfo->StnId < max_sta_num)) {
				...
				UINT8 Ampdu_Rx_Disable_Flag_l = get_ampdu_rx_disable_flag(vmacSta_p, MgmtMsg_p);

				/* Allocate space for response message */
				if ((txSkb_p =
				     mlmeApiPrepMgtMsg2(IEEE_MSG_QOS_ACTION, &MgmtMsg_p->Hdr.SrcAddr, &MgmtMsg_p->Hdr.DestAddr,
							sizeof(IEEEtypes_ADDBA_Rsp_t))) == NULL) {
				...
				if ((Ampdu_Rx_Disable_Flag_l) || (tid > 7)) {
					if (tid > 7)
						printk("Reject ADDBA REQ (tid %d StnId %d mac %pM\n", tid, pStaInfo->StnId, pStaInfo->Addr);
				...
				if (*(vmacSta_p->Mib802dot11->pMib_11nAggrMode) & WL_MODE_AMSDU_TX_MASK)
					amsdu_bitmap =
					    (pAddBaReqFrm->ParamSet.
					     amsdu) ? (*(vmacSta_p->Mib802dot11->pMib_11nAggrMode) & WL_MODE_AMSDU_TX_MASK) : 0;

#ifdef MV_CPU_BE
				pAddBaReqFrm->ParamSet.u16_data = ENDIAN_SWAP16(pAddBaReqFrm->ParamSet.u16_data);
				pAddBaReqFrm->SeqControl.u16_data = ENDIAN_SWAP16(pAddBaReqFrm->SeqControl.u16_data);
#endif
...
#ifdef SOC_W906X
				if ((*(vmacSta_p->Mib802dot11->mib_superBA) == 1) || (*(vmacSta_p->Mib802dot11->mib_superBA) == 3))
					pAddBaRspFrm->ParamSet.BufSize =
					    (pAddBaReqFrm->ParamSet.BufSize < MAX_BA_REORDER_BUF_SIZE) ? 64 : MAX_BA_REORDER_BUF_SIZE;
				else
					// mib_superBA == 0 || 2
				...
				if ((!Ampdu_Rx_Disable_Flag_l) && (tid < 8)) {
					...
					wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].ExpectedSeqNo[tid][0] =
					    wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].CurrentSeqNo[tid];

					baRxInfo = &wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid];
#endif				/* SOC_W906X */
...
#ifdef SOC_W906X
					baRxInfo->ba[tid].winSizeB = pAddBaRspFrm->ParamSet.BufSize;
#else
...
#ifdef MV_CPU_BE
				pAddBaRspFrm->ParamSet.u16_data = ENDIAN_SWAP16(pAddBaRspFrm->ParamSet.u16_data);
#endif
				if (txMgmtMsg(vmacSta_p->dev, txSkb_p) != OS_SUCCESS) {
					wl_free_skb(txSkb_p);
					return;
				...
				if (pAddBaRspFrm->StatusCode == IEEEtypes_STATUS_SUCCESS) {
					if (wlpptr->cb_enable == TRUE) {
						/*
						   Save the BA status so that the information can be hand-over
						 */
						cbba_info_t *pcbba_info = &(pStaInfo->cbinfo_sta.cbba_info[0][pAddBaReqFrm->ParamSet.tid % MAX_TID]);
						pcbba_info->valid = true;
						pcbba_info->BarThrs = pcbba_info->WindowSize = pAddBaRspFrm->ParamSet.BufSize;
						pcbba_info->DialogToken = 10;
						pcbba_info->ba_type = amsdu_bitmap;
						pcbba_info->direction = 1;
						pcbba_info->ParamInfo = pStaInfo->HtElem.MacHTParamInfo;
						pcbba_info->seqNo = pAddBaReqFrm->SeqControl.Starting_Seq_No;
						pcbba_info->vhtrxfactor = pStaInfo->vhtCap.cap.MaximumAmpduLengthExponent;
						pcbba_info->queueid = 0;
						pcbba_info->staid = pStaInfo->StnId;
					}
					wlFwCreateBAStream(vmacSta_p->dev, pAddBaRspFrm->ParamSet.BufSize, pAddBaRspFrm->ParamSet.BufSize,
				...
				wlFwCreateBAStream(vmacSta_p->dev, pAddBaReqFrm->ParamSet.BufSize, pAddBaReqFrm->ParamSet.BufSize,
						   (u_int8_t *) & (MgmtMsg_p->Hdr.SrcAddr), 10, pAddBaReqFrm->ParamSet.tid, amsdu_bitmap, 1,
						   pStaInfo->HtElem.MacHTParamInfo, (u_int8_t *) & (MgmtMsg_p->Hdr.DestAddr),
						   pAddBaReqFrm->SeqControl.Starting_Seq_No, pStaInfo->vhtCap.cap.MaximumAmpduLengthExponent, 0);
#endif				/* SOC_W906X */
			}
#endif
			break;
...
/** Initiator want to stop ampdu **/
					...
					wlFwUpdateDestroyBAStream(vmacSta_p->dev, 0, 1, i, pDelBaReqFrm->ParamSet.tid,
								  (u_int8_t *) & (MgmtMsg_p->Hdr.SrcAddr), pStaInfo->StnId);
					...
					*(UINT32 *) & baRxInfo->ba[tid].winStartB = 0;
					*(UINT32 *) & baRxInfo->ba[tid].storedBufCnt = 0;
					baRxInfo->ba[tid].minTime = 0;
				} else {
/** Receiver want to stop us from doing ampdu **/
					...
					for (i = 0; i < MAX_SUPPORT_AMPDU_TX_STREAM_RUNNING; i++) {
						if (!MACADDR_CMP(wlpptr->wlpd_p->Ampdu_tx[i].MacAddr, &(MgmtMsg_p->Hdr.SrcAddr))) {
							/** they are equal **/
							if (wlpptr->wlpd_p->Ampdu_tx[i].AccessCat == pDelBaReqFrm->ParamSet.tid
								...
								wlFwUpdateDestroyBAStream(vmacSta_p->dev, 0, 0, i,
											  ...
											  wlpptr->wlpd_p->Ampdu_tx[i].MacAddr, pStaInfo->StnId);
					...
					if (vmacSta_master_p->dl_ofdma_para.max_sta > 0) {
						...
						for (i = 0; i < MAX_OFDMADL_STA; i++) {
							if (!memcmp(vmacSta_master_p->ofdma_mu_sta_addr[i], val, IEEEtypes_ADDRESS_SIZE)) {
							    ...
							    if (!memcmp
								(pStaInfo->Addr, vmacSta_master_p->ofdma_mu_sta_addr[i], IEEEtypes_ADDRESS_SIZE))
								break;
						}
						if (i < vmacSta_master_p->dl_ofdma_para.max_sta) {
							vmacSta_master_p->dl_ofdma_para.sta_cnt--;
		...
		case ADDBA_RESP:
			...
			if (pStaInfo) {
				...
				WLDBG_INFO(DBG_LEVEL_0, "Value of ParamSet.BlockAckPolicy = %x, ParamSet.Tid = %x, BufferSize = %x\n",
					   pAddBaRspFrm->ParamSet.BA_policy, pAddBaRspFrm->ParamSet.tid, pAddBaRspFrm->ParamSet.BufSize);
				WLDBG_INFO(DBG_LEVEL_0, "Value of Reason = %x\n", pAddBaRspFrm->Timeout_val);

				if (pAddBaRspFrm->ParamSet.amsdu)
					pStaInfo->addba_rxamsdu = TRUE;
				else
					pStaInfo->addba_rxamsdu = FALSE;

				/* If the STA is 11N, disable AMSDU */
				if (!STADB_IS_HE(pStaInfo) && !STADB_IS_VHT(pStaInfo) && STADB_IS_HT(pStaInfo)) {
					pAddBaRspFrm->ParamSet.amsdu = 0;
					pStaInfo->addba_rxamsdu = FALSE;
				}
...
/** either stream 0 or 1 is equal **/
					...
					if (pAddBaRspFrm->StatusCode == 0)	//success
					{
						if (pAddBaRspFrm->ParamSet.tid == wlpptr->wlpd_p->Ampdu_tx[i].AccessCat) {
...
#ifdef SOC_W906X
							if ((*(vmacSta_p->Mib802dot11->mib_ApMode) & AP_MODE_11AX)
							    && (pStaInfo->he_cap_ie == HE_CAPABILITIES_IE)
							    && ((*(vmacSta_p->Mib802dot11->mib_superBA) == 1)
								|| (*(vmacSta_p->Mib802dot11->mib_superBA) == 3))
							    && (pAddBaRspFrm->ParamSet.BufSize >= MAX_BA_REORDER_BUF_SIZE))
								pAddBaRspFrm->ParamSet.BufSize = MAX_BA_REORDER_BUF_SIZE;
							...
							pAddBaRspFrm->ParamSet.BufSize =
							    MIN(pAddBaRspFrm->ParamSet.BufSize, wlpptr->wlpd_p->Ampdu_tx[i].BufSizeInReq);
							...
							if (wlpptr->cb_enable == TRUE) {
								cbba_info_t *pcbba_info =
								    &(pStaInfo->cbinfo_sta.
								      cbba_info[1][wlpptr->wlpd_p->Ampdu_tx[i].AccessCat % MAX_TID]);
								pcbba_info->valid = true;
								pcbba_info->BarThrs = pcbba_info->WindowSize = pAddBaRspFrm->ParamSet.BufSize;
								pcbba_info->DialogToken = 10;
								pcbba_info->ba_type = amsdu_bitmap;
								pcbba_info->direction = 0;
								pcbba_info->ParamInfo = pStaInfo->HtElem.MacHTParamInfo;
								pcbba_info->seqNo = wlpptr->wlpd_p->Ampdu_tx[i].start_seqno;
								pcbba_info->vhtrxfactor = pStaInfo->vhtCap.cap.MaximumAmpduLengthExponent;
								pcbba_info->queueid = i;
								pcbba_info->staid = pStaInfo->StnId;
							}
							if (wlFwCreateBAStream
							    (vmacSta_p->dev, pAddBaRspFrm->ParamSet.BufSize, pAddBaRspFrm->ParamSet.BufSize,
							     (u_int8_t *) & (MgmtMsg_p->Hdr.SrcAddr), 10, wlpptr->wlpd_p->Ampdu_tx[i].AccessCat,
							     amsdu_bitmap, 0, pStaInfo->HtElem.MacHTParamInfo, NULL,
							     wlpptr->wlpd_p->Ampdu_tx[i].start_seqno, pStaInfo->vhtCap.cap.MaximumAmpduLengthExponent,
...
#endif
							{
								...
								SendDelBA(vmacSta_p, (UINT8 *) & wlpptr->wlpd_p->Ampdu_tx[i].MacAddr[0],
									  wlpptr->wlpd_p->Ampdu_tx[i].AccessCat);
								pStaInfo->aggr11n.type &= ~WL_WLAN_TYPE_AMPDU;
								wlpptr->wlpd_p->Ampdu_tx[i].InUse = 0;
								wlpptr->wlpd_p->Ampdu_tx[i].TimeOut = 0;
							...
							if (wfa_11ax_pf) {
								...
								if (vmacSta_master_p->dl_ofdma_para.max_sta > 0) {
									...
									for (i = 0; i < MAX_OFDMADL_STA; i++) {
										if (!memcmp
										    (vmacSta_master_p->ofdma_mu_sta_addr[i], val,
										     IEEEtypes_ADDRESS_SIZE)) {
											idx = i;
											continue;
										} else
										    if (!memcmp
											(pStaInfo->Addr, vmacSta_master_p->ofdma_mu_sta_addr[i],
											 IEEEtypes_ADDRESS_SIZE))
											break;
									}
									if (i >= MAX_OFDMADL_STA) {
										...
										if (vmacSta_master_p->dl_ofdma_para.sta_cnt <=
										    vmacSta_master_p->dl_ofdma_para.max_sta) {
											memcpy(vmacSta_master_p->ofdma_mu_sta_addr[idx],
											       pStaInfo->Addr, IEEEtypes_ADDRESS_SIZE);
									...
									if (vmacSta_master_p->dl_ofdma_para.sta_cnt ==
									    vmacSta_master_p->dl_ofdma_para.max_sta) {
										printk
										    ("Set ofdma fw cmd: option=%d ru_mode=%d max_delay=%d max_sta=%d sta_cnt=%d\n",
										     vmacSta_master_p->dl_ofdma_para.option,
										     vmacSta_master_p->dl_ofdma_para.ru_mode,
										     vmacSta_master_p->dl_ofdma_para.max_delay,
										     vmacSta_master_p->dl_ofdma_para.max_sta,
										...
										vmacSta_master_p->dl_ofdma_para.all_connected = jiffies;

										if (!vmacSta_master_p->dl_ofdma_para.started &&
										    vmacSta_master_p->dl_ofdma_para.postpone_time == 0) {
											printk("%s\n", __FUNCTION__);
											wlFwSetOfdma_Mode(vmacSta_master_p->dev,
													  vmacSta_master_p->dl_ofdma_para.option,
													  vmacSta_master_p->dl_ofdma_para.ru_mode,
													  vmacSta_master_p->dl_ofdma_para.max_delay,
													  vmacSta_master_p->dl_ofdma_para.max_sta);
											vmacSta_master_p->dl_ofdma_para.started = jiffies;
...
#ifdef SOC_W906X
				wlFwSetVHTOpMode(vmacSta_p->dev, pStaInfo->StnId, vht_NewRxChannelWidth, vht_NewRxNss);
#else
	...
	case S1G:		//process twt
		...
		case TWT_SETUP:
			{
				ProcessTWTsetup(vmacSta_p, (macmgmtQ_MgmtMsg_t *) MgmtMsg_p);
			}
			break;
		case TWT_TEARDOWN:
			{
				ProcessTWTteardown(vmacSta_p, (macmgmtQ_MgmtMsg_t *) MgmtMsg_p);
			}
			break;
		case TWT_INFO:
			{
				ProcessTWTInformation(vmacSta_p, (macmgmtQ_MgmtMsg_t *) MgmtMsg_p);
			}
			break;
		default:
...
int macMgmtMlme_RrmAct(vmacApInfo_t * vmacSta_p, macmgmtQ_MgmtMsg3_t * MgmtMsg_p)
{
	...
	if (MgmtMsg_p->Body.Action.Category != AC_RADIO_MEASUREMENT || !*(mib->mib_rrm))
		return FALSE;
	...
	if (mib->mib_conf_capab->rrm) {
		...
		if ((msgbuf = (macmgmtQ_MgmtMsg3_t *) wl_kmalloc(MgmtMsg_p->Hdr.FrmBodyLen + 2, GFP_ATOMIC)) == NULL) {
			...
			memcpy(&(msgbuf->Hdr.Rsrvd), &(MgmtMsg_p->Body), MgmtMsg_p->Hdr.FrmBodyLen - sizeof(IEEEtypes_MgmtHdr3_t) + sizeof(UINT16));
			msgbuf->Hdr.FrmBodyLen -= ETH_ALEN;	// Remove Addr4 bssid length here.
#ifdef CFG80211
			mwl_cfg80211_rx_mgmt(vmacSta_p->dev, &(msgbuf->Hdr.FrmCtl), msgbuf->Hdr.FrmBodyLen, 0);
#else				/* CFG80211 */
	...
	case AF_RM_NEIGHBOR_REQUEST:
		...
		StaInfo = extStaDb_GetStaInfo(vmacSta_p, &(MgmtMsg_p->Hdr.SrcAddr), 1);
		if (StaInfo == NULL) {
		...
		if (MgmtMsg_p->Hdr.FrmBodyLen > sizeof(IEEEtypes_MgmtHdr2_t) + 3) {
			ie = (struct IEEEtypes_InfoElementHdr_t *)&MgmtMsg_p->Body.Action.Data;
			if (ie != NULL) {
				switch (ie->ElementId) {
				case SSID:
					{
						...
						if (ssid->Len >= IEEEtypes_SSID_SIZE) {
							break;
						}
						memset(&scannedSSID[0], 0, IEEEtypes_SSID_SIZE);
						memcpy(&scannedSSID[0], &ssid->SsId[0], ssid->Len);
						nb_num = MSAN_get_neighbor_bySSID(vmacSta_p->dev, ssid);
					}
					break;
				case PROPRIETARY_IE:
					printk("Neighbor Report_Req Proprietary_IE...\n");
				default:
					nb_num = MSAN_get_neighbor_byDefault(vmacSta_p->dev);
					break;
				}
			}
		}
		if ((skb = mlmeApiPrepMgtMsg2(IEEE_MSG_QOS_ACTION,
					      ...
					      &MgmtMsg_p->Hdr.DestAddr, 3 + (sizeof(struct IEEEtypes_Neighbor_Report_Element_t) * nb_num)))
		    == NULL) {
			WLDBG_INFO(DBG_LEVEL_7, "Failed to allocate buffer ...\n");
			return FALSE;
		}
		...
		MgmtResp_p->Body.Action.DialogToken = MgmtMsg_p->Body.Action.DialogToken;
		if (nb_num > 0) {
			NeighborRpt_Element = (struct IEEEtypes_Neighbor_Report_Element_t *)&MgmtResp_p->Body.Action.Data.NeighborReqportElemes[0];
			memcpy(NeighborRpt_Element, &(wlpptr->wlpd_p->nb_info.nb_elem[0]), sizeof(IEEEtypes_Neighbor_Report_Element_t) * nb_num);
		}
	...
	case AF_RM_MEASUREMENT_REPORT:
		{
			...
			ie_p = (struct IEEEtypes_MeasurementReportElement_t *)&MgmtMsg_p->Body.Action.Data.MeasurementReport[0];
			if (ie_p == NULL) {
				break;
			}
...
#ifdef MULTI_AP_SUPPORT
			MAP_tlv_Resp_process(vmacSta_p, (void *)MgmtMsg_p, (IEEEtypes_MacAddr_t *) & (MgmtMsg_p->Hdr.SrcAddr), 0);
#endif				/* MULTI_AP_SUPPORT */
			...
			while (msg_len < MgmtMsg_p->Hdr.FrmBodyLen) {
				...
				case TYPE_REP_BCN:
					{
						...
						printk("bcnreport bssid=0x%2x:0x%2x:0x%2x:0x%2x:0x%2x:0x%2x channel:%d RCPI:%d, RSNI:%d\n",
						       ...
						       bcn_rep->Bssid[3],
						       bcn_rep->Bssid[4],
						       bcn_rep->Bssid[5], bcn_rep->Channel, bcn_rep->RCPI_value, bcn_rep->RSNI_value);
					}
					break;
				default:
					break;
				...
				msg_len += ie_p->Len + 2;
				ie_p = (struct IEEEtypes_MeasurementReportElement_t *)((UINT8 *) ie_p + ie_p->Len + 2);
			}
		}
		break;
	default:
...
int macMgmtMlme_WNMAct(vmacApInfo_t * vmacSta_p, macmgmtQ_MgmtMsg3_t * MgmtMsg_p)
{
	...
	case AF_WNM_BTM_QUERY:
		{
			struct IEEEtypes_BSS_TM_Query_t *bss_tm_query_p = NULL;
			...
			StaInfo = extStaDb_GetStaInfo(vmacSta_p, &MgmtMsg_p->Hdr.SrcAddr, STADB_DONT_UPDATE_AGINGTIME);
			if (StaInfo == NULL) {
				printk("AF_WNM_BTM_QUERY Failed to get StaInfo...\n");
				break;
			}

			if (StaInfo->ExtCapElem.ExtCap.BSSTransition == 0) {
				printk("AF_WNM_BTM_QUERY STA does not support BTM Transition, skip...\n");
				break;
			}

			bss_tm_query_p = (struct IEEEtypes_BSS_TM_Query_t *)&MgmtMsg_p->Body.Action.Data;
			if (bss_tm_query_p->QueryReason < 20) {
				...
				UINT8 *var_ptr = bss_tm_query_p->variable;
				...
				MSAN_clean_neighbor_list(vmacSta_p->dev);
				var_ptr = bss_tm_query_p->variable;
				while (msg_len >= sizeof(struct IEEEtypes_Neighbor_Report_Element_t) && (PrefList_p->ElementId == NEIGHBOR_REPORT)) {
					nb_num = MSAN_get_neighbor_byAddr(vmacSta_p->dev, (IEEEtypes_MacAddr_t *) PrefList_p->Bssid);
					if (!nb_num) {
			...
			if (!nb_num) {
				nb_num = MSAN_get_neighbor_bySSID(vmacSta_p->dev, &vmacSta_p->macSsId);
				if (!nb_num) {
			...
			BSS_TM_Req.Abridged = mib->mib_BSSTMRequest->Abridged;
			BSS_TM_Req.DisassocImm = mib->mib_BSSTMRequest->DisassocImm;
			BSS_TM_Req.BSSTermiInc = mib->mib_BSSTMRequest->BSSTermiInc;
			BSS_TM_Req.ESSDisassocImm = mib->mib_BSSTMRequest->ESSDisassocImm;
			...
			if (bsstm_send_request(vmacSta_p->dev, (UINT8 *) & MgmtMsg_p->Hdr.SrcAddr, &BSS_TM_Req) == FALSE) {
	...
	case AF_WNM_BTM_RESPONSE:
		{
			...
			bss_tm_resp_p = (struct IEEEtypes_BSS_TM_Resp_t *)&MgmtMsg_p->Body.Action.Data;
			msg_len = sizeof(struct IEEEtypes_BSS_TM_Resp_t);
			...
			if (bss_tm_resp_p->StatusCode == 0) {
				...
				printk("Target BSS:%s", mac_display(bss_tm_resp_p->variable));
			}
			printk("\n");

			/* send BTM response to al_entity */
			snprintf(buf, sizeof(buf), "%s", tag);
			memcpy(&buf[strlen(tag)], &(MgmtMsg_p->Hdr.SrcAddr), sizeof(IEEEtypes_MacAddr_t));
			memcpy(&buf[strlen(tag) + sizeof(IEEEtypes_MacAddr_t)], bss_tm_resp_p, msg_len);
			memset(&wreq, 0, sizeof(wreq));
			wreq.data.length = strlen(tag) + sizeof(IEEEtypes_MacAddr_t) + msg_len;
			if (vmacSta_p->dev->flags & IFF_RUNNING) {
				wireless_send_event(vmacSta_p->dev, IWEVCUSTOM, &wreq, buf);
			}
		}
...
void PrepareRateElements(vmacApInfo_t * vmacSta_p, IEEEtypes_StartCmd_t * StartCmd_p)
{
	...
	if (StartCmd_p->sae_pwe == 1) {
		if (k < IEEEtypes_MAX_DATA_RATES) {
			SuppRateSet_p->Rates[k++] = 0x80 | BSS_MEMBERSHIP_SELECTOR_SAE_H2E_ONLY;
			SuppRateSet_p->Len = k;
		} else if (l < IEEEtypes_MAX_DATA_RATES) {
			ExtSuppRateSet_p->Rates[l++] = 0x80 | BSS_MEMBERSHIP_SELECTOR_SAE_H2E_ONLY;
			ExtSuppRateSet_p->Len = l;
		}
	}
}
...
extern void macMgmtMlme_ResetCmd(vmacApInfo_t * vmacSta_p, IEEEtypes_ResetCmd_t * ResetCmd_p)
{
	...
	if (!ResetCmd_p->quiet) {
#ifdef SOC_W906X
		macMgmtMlme_SendDeauthenticateMsg(vmacSta_p, &bcast, MCBC_STN_ID, IEEEtypes_REASON_DEAUTH_LEAVING, FALSE);
#else
...
extern void macMgmtMlme_StartCmd(vmacApInfo_t * vmacSta_p, IEEEtypes_StartCmd_t * StartCmd_p)
{
	...
	if (mibshadow->RSNConfigWPA2->WPA2Enabled || mibshadow->RSNConfigWPA2->WPA2OnlyEnabled) {
		if (vmacSta_p->RSNXIE[1] != 0) {
			StartCmd_p->sae_pwe = 1;
		}
	}
	...
	if (StartCmd_p->SsParamSet.CfParamSet.ElementId == CF_PARAM_SET && StartCmd_p->SsParamSet.CfParamSet.Len == 6) {
	}
...
{
	if (SsId1 && SsId2) {
		if (SsId1->Len == SsId2->Len && !memcmp(SsId1->SsId, SsId2->SsId, SsId1->Len)) {
			return (OS_SUCCESS);
		} else {
			return (OS_FAIL);
		}
...
WL_STATUS isRateSetValid(UINT8 * RateSet)
{
	int i;
	i = 0;
	while (i < IEEEtypes_MAX_DATA_RATES && RateSet[i] > 1 && RateSet[i] <= 127) {
		i++;
	}
	...
	*gRatePresent = FALSE;
	...
	for (i = 0; i < SuppRates->Len && SuppRates->Rates[i]; i++) {
		if ((Index = hw_GetPhyRateIndex(vmacSta_p, (SuppRates->Rates[i] & IEEEtypes_BASIC_RATE_MASK)))
			...
			/* valid rate for the Phy */
		{
			if (isBandA || (Index > HIGHEST_11B_RATE_REG_INDEX))
				*gRatePresent = TRUE;
...
void macMgmtRemoveSta(vmacApInfo_t * vmacSta_p, extStaDb_StaInfo_t * StaInfo_p)
{
	...
	StaInfo_p->Aid = 0;
	FreeStnId(vmacSta_p, StaInfo_p->StnId);
	extStaDb_DelSta(vmacSta_p, (IEEEtypes_MacAddr_t *) StaInfo_p->Addr, STADB_DONT_UPDATE_AGINGTIME);
	wlFwSetNewStn(vmacSta_p->dev, (u_int8_t *) StaInfo_p->Addr, StaInfo_p->Aid, StaInfo_p->StnId, StaInfoDbActionRemoveEntry, NULL, 0, 0, 0);
}
...
int macMgmtMlme_set_sta_authorized(vmacApInfo_t * vmacAP_p, IEEEtypes_MacAddr_t * staMac)
{
	...
	if ((StaInfo_p = extStaDb_GetStaInfo(vmacAP_p, staMac, STADB_DONT_UPDATE_AGINGTIME)) == NULL) {
		...
		if ((StaInfo_p = extStaDb_GetStaInfo(vmacAP_p, staMac, STADB_SKIP_MATCH_VAP))) {
			macMgmtRemoveSta(vmacAP_p, StaInfo_p);
		}
		if ((StaInfo_p = macMgtStaDbInit(vmacAP_p, staMac, &vmacAP_p->macStaAddr)) == NULL) {
			WLDBG_ENTER_INFO(DBG_LEVEL_11, "Init DB in Ioctl APi failed ...\n");
			return -EUSERS;
		}
		StaInfo_p->State = AUTHENTICATED;	//bypass auth alg check for now.
		PRINT1(IOCTL, "STA-%02x:%02x:%02x:%02x:%02x:%02x set authicanted ...\n", addr[0], addr[1], addr[2], addr[3], addr[4], addr[5]);
	} else {
		//Do nothing if the STA already exists...
		PRINT1(IOCTL, "STA-%02x:%02x:%02x:%02x:%02x:%02x already exists ...\n", addr[0], addr[1], addr[2], addr[3], addr[4], addr[5]);
	}
	return 0;
}
				   ...
				   UINT8 Aid, PeerInfo_t * PeerInfo, UINT8 QosInfo, UINT8 isQosSta, UINT8 rsnSta, UINT8 * rsnIE)
{
	...
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, ((struct net_device *)vmacAP_p->dev));
#endif
	if ((StaInfo_p = extStaDb_GetStaInfo(vmacAP_p, staMac, STADB_DONT_UPDATE_AGINGTIME)) == NULL) {
		PRINT1(IOCTL, "STA-%02x:%02x:%02x:%02x:%02x:%02x not found in StaDB ...\n", addr[0], addr[1], addr[2], addr[3], addr[4], addr[5]);
		return -ENXIO;
	} else {
		if (StaInfo_p->State != AUTHENTICATED) {
			PRINT1(IOCTL, "STA-%02x:%02x:%02x:%02x:%02x:%02x Station has not authenticated! State = %x\n",
			       addr[0], addr[1], addr[2], addr[3], addr[4], addr[5], StaInfo_p->State);
			return -EPERM;
		}
		if (Aid)
			StaInfo_p->Aid = Aid;
...
#endif
		if (rsnSta) {
			memcpy(StaInfo_p->keyMgmtStateInfo.RsnIEBuf, rsnIE, rsnIE[1] + 2);
		}
		cleanupAmpduTx(vmacAP_p, (UINT8 *) & StaInfo_p->Addr);
		if (((PeerInfo->LegacyRateBitMap & 0xFFFFFFF0) == 0) || (*(mib->mib_ApMode) == AP_MODE_B_ONLY))
			StaInfo_p->ClientMode = BONLY_MODE;
		else if ((PeerInfo->HTRateBitMap) && (*(mib->mib_ApMode) & 0x4)) {
			StaInfo_p->ClientMode = NONLY_MODE;
			StaInfo_p->aggr11n.threshold = AGGRTHRESHOLD;
			StaInfo_p->aggr11n.thresholdBackUp = StaInfo_p->aggr11n.threshold;
			StaInfo_p->aggr11n.cap = (PeerInfo->HTCapabilitiesInfo.MaxAMSDUSize) ? 2 : 1;

			amsdu_bitmap = (*(mib->pMib_11nAggrMode) & WL_MODE_AMSDU_TX_MASK);
			if (amsdu_bitmap == WL_MODE_AMSDU_TX_11K)
				amsdu_bitmap = WL_MODE_AMSDU_TX_8K;
			...
			StaInfo_p->PeerHTCapabilitiesInfo = PeerInfo->HTCapabilitiesInfo;
			StaInfo_p->HtElem.HTCapabilitiesInfo = PeerInfo->HTCapabilitiesInfo;
			StaInfo_p->HtElem.MacHTParamInfo = PeerInfo->MacHTParamInfo;
			StaInfo_p->HtElem.TxBFCapabilities = PeerInfo->TxBFCapabilities;
		} else {
...
#ifdef SOC_W906X
		if (staIdx >= sta_num) { /*if only can get valid stnid */ ;
#else
		...
		StaInfo_p->StnId = staIdx;
		StaInfo_p->FwStaPtr = wlFwSetNewStn(vmacAP_p->dev, (UINT8 *) staMac, StaInfo_p->Aid, StaInfo_p->StnId,
						    StaInfoDbActionAddEntry, PeerInfo, QosInfo, isQosSta, 0);
...
{
	extStaDb_StaInfo_t *StaInfo_p;
	if ((StaInfo_p = extStaDb_GetStaInfo(vmacSta_p, staMac, STADB_DONT_UPDATE_AGINGTIME)) == NULL) {
		return;		//Do nothing if STA is not in StaDB.
	}
#ifdef QOS_FEATURE_REMOVE
	if (*(mib->QoSOptImpl) && extStaDb_GetQoSOptn(vmacSta_p, staMac))
		ClearQoSDB(staMac);
#endif
	wlFwSetAPUpdateTim(vmacSta_p->dev, StaInfo_p->Aid, RESETBIT);
	FreeAid(vmacSta_p, StaInfo_p->Aid);
	...
	wlFwSetNewStn(vmacSta_p->dev, (u_int8_t *) staMac, StaInfo_p->Aid, StaInfo_p->StnId, StaInfoDbActionRemoveEntry, NULL, 0, 0, 0);
	return;
}
...
/** powersave bit is still 1 **/
			...
			if (*(mib->QoSOptImpl) && ((pStaInfo->Qosinfo.Uapsd_ac_vo &&
						    ...
						    pStaInfo->Qosinfo.Uapsd_ac_be && pStaInfo->Qosinfo.Uapsd_ac_bk) ||
				/** mixed case **/
...
#ifdef SOC_W906X
	macMgmtMlme_SendDeauthenticateMsg(vmacSta_p, &bcast, 0, IEEEtypes_REASON_DEAUTH_LEAVING, FALSE);
#else
...
void macMgmtMlme_SAQuery(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * Addr, IEEEtypes_MacAddr_t * SrcAddr, UINT32 stamode)
{
	...
	if ((txSkb_p = mlmeApiPrepMgtMsg2(IEEE_MSG_QOS_ACTION, Addr, &vmacSta_p->macStaAddr, sizeof(IEEEtypes_SAQuery_Req_t))) == NULL)
		return;
	...
	if (vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_CLNT_INFRA) {
		...
		MACADDR_CPY(MgmtMsg_p->Hdr.SrcAddr, vmacSta_p->macStaAddr);

		if ((pbssid = GetParentStaBSSID(vmacSta_p->VMacEntry.phyHwMacIndx)) != NULL) {
	...
	memcpy(MgmtMsg_p->Body.SAQuery_Req.trans_id, trans_id, WLAN_SA_QUERY_TR_ID_LEN);
	...
	if (stamode == 0) {
		...
		if (wlxmit(vmacSta_p->dev, txSkb_p, IEEE_TYPE_MANAGEMENT | (key << 7) | (1 << 6), NULL, 0, FALSE, 0))
#else
		SPIN_LOCK_IRQSAVE(&wlpptr->wlpd_p->locks.xmitLock, flags);
		if (wlxmit(vmacSta_p->dev, txSkb_p, IEEE_TYPE_MANAGEMENT | (key << 7) | (1 << 6), NULL, 0, FALSE))
#endif
		{
			wl_free_skb(txSkb_p);
		}
...
void macMgmtMlme_SAQuery_Rsp(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * Addr, IEEEtypes_MacAddr_t * SrcAddr, UINT8 * trans_id, UINT32 stamode)
{
	macmgmtQ_MgmtMsg2_t *MgmtMsg_p;
	BOOLEAN sendResult = FALSE;
	struct sk_buff *txSkb_p;
	...
	if ((txSkb_p = mlmeApiPrepMgtMsg2(IEEE_MSG_QOS_ACTION, Addr, SrcAddr, sizeof(IEEEtypes_SAQuery_Rsp_t))) == NULL)
		return;
	MgmtMsg_p = (macmgmtQ_MgmtMsg2_t *) txSkb_p->data;

	MgmtMsg_p->Body.SAQuery_Rsp.Category = WLAN_ACTION_SA_QUERY;
	MgmtMsg_p->Body.SAQuery_Rsp.Action = WLAN_SA_QUERY_RESPONSE;
	memcpy(MgmtMsg_p->Body.SAQuery_Rsp.trans_id, trans_id, WLAN_SA_QUERY_TR_ID_LEN);
...
static WL_STATUS macMgmtMlme_asso_ind(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * addr)
{
	...
	WLDBG_ENTER(DBG_LEVEL_7);
	if ((toSmeMsg = (smeQ_MgmtMsg_t *) wl_kmalloc_autogfp(sizeof(smeQ_MgmtMsg_t))) == NULL) {
...
#endif
	{
		...
		for (loop = 0; loop < MreportCmd_p->ReportItems; loop++) {
			...
			if ((MreportCmd_p->MeasureRepSet[loop].Mode.Incapable) ||	/* problem to produce report result */
				...
				memcpy(&IE_p->Report.DefReport, &MreportCmd_p->MeasureRepSet[loop].Report, sizeof(IEEEtypes_MeasurementRep_t));
			}
			/* this IE's length */
			IE_p->Len = ielen[loop];
			subLen = ielen[loop] + 2;
				 ...
				 UINT8 MeasMode, UINT8 * ssid, UINT16 ReportCond, UINT8 ReportDetail, UINT8 MeasDurMand, UINT8 VoWiCase)
{
	...
	skb = mlmeApiPrepMgtMsg2(IEEE_MSG_QOS_ACTION, (IEEEtypes_MacAddr_t *) stamac, (IEEEtypes_MacAddr_t *) netdev->dev_addr, 200);
	if (skb == NULL) {
		return;
	}
	...
	RequestElement = (IEEEtypes_RadioMeasurementRequestElement_t *)
	    & MgmtMsg_p->Body.Action.Data.RadioMeasReq.RequestElement[0];
	{
		MgmtMsg_p->Body.Action.Category = AC_RADIO_MEASUREMENT;
		MgmtMsg_p->Body.Action.Action = AF_RM_MEASUREMENT_REQUEST;
		MgmtMsg_p->Body.Action.DialogToken = wlpptr->wlpd_p->Global_DialogToken;
		wlpptr->wlpd_p->Global_DialogToken = (wlpptr->wlpd_p->Global_DialogToken + 1) % 63;
		MgmtMsg_p->Body.Action.Data.RadioMeasReq.NoOfRepetitions = 0;
		...
		*(UINT8 *) & RequestElement->Mode = 0;
		...
		bcn_req = (IEEEtypes_BeaconRequest_t *) & RequestElement->BcnReq;
...
/***** Reporting Detail End *******/
		...
		RequestElement->Len = 3 + sizeof(IEEEtypes_BeaconRequest_t) + len;
		skb_trim(skb, sizeof(struct ieee80211_frame) + 23 + len);

		WLDBG_INFO(DBG_LEVEL_7, "macMgmtMlme_RmBeaconRequest: Tx action frame ... \n");
...
void getVhtBwFreqInfo(UINT16 FreqBand, UINT16 Chan, UINT16 Chan2, UINT16 ChnlWidth, UINT8 radiomode,
		      UINT8 * ch_width, UINT8 * center_freq0, UINT8 * center_freq1)
{
	extern UINT32 GetCenterFreq(UINT32 ch, UINT32 bw);
	extern UINT32 ie192_version;

	if ((ChnlWidth == CH_40_MHz_WIDTH) || (ChnlWidth == CH_20_MHz_WIDTH) || (FreqBand == FREQ_BAND_2DOT4GHZ)) {
		*ch_width = 0;
	}
#ifdef SOC_W8964		//deprecated
	else if ((ChnlWidth == CH_160_MHz_WIDTH) || (ChnlWidth == CH_AUTO_WIDTH)) {
		*ch_width = 2;	//160MHz
	}
#endif
	else {
		...
		*center_freq1 = 0;
		...
		if (ie192_version == 2) {
			...
			if (ChnlWidth == CH_80_MHz_WIDTH) {
				*center_freq0 = GetCenterFreq(Chan, CH_80_MHz_WIDTH);
			...
			{
				*center_freq0 = GetCenterFreq(Chan, CH_160_MHz_WIDTH);
				*center_freq1 = 0;
			}
#endif
		} else {
...
#ifdef SOC_W906X
void macMgmtMlme_ChannelSwitchReq(vmacApInfo_t * vmacSta_p, Dfs_ChanSwitchReq_t * pChanSwitch)
{
	MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
	macmgmtQ_MgmtMsg2_t *MgmtMsg_p;
	UINT16 extraLen = sizeof(IEEEtypes_WideBWCS_Element_t) + sizeof(IEEEtypes_SecondaryChannelOffsetElement_t);
	UINT16 FrameLen = sizeof(IEEEtypes_Category_t) +
	    sizeof(IEEEtypes_ActionFieldType_t) + sizeof(IEEEtypes_ChannelSwitchAnnouncementElement_t) + extraLen;
	...
	if (ChnlWidth == CH_AUTO_WIDTH) {
		if (FreqBand == FREQ_BAND_2DOT4GHZ) {
			...
			if ((*(mib->mib_ApMode)) & (AP_MODE_11AC | AP_MODE_11AX))
...
#endif
	{
...
#endif
		{
			...
			ChannelSwitchAnnouncementElement_p->Mode = pChanSwitch->ChannelSwitchCmd.Mode;
			ChannelSwitchAnnouncementElement_p->Count = pChanSwitch->ChannelSwitchCmd.ChannelSwitchCount;
			ChannelSwitchAnnouncementElement_p->Channel = pChanSwitch->ChannelSwitchCmd.ChannelNumber;
			if (*(mib->mib_ApMode) & (AP_MODE_N_ONLY | AP_MODE_11AC | AP_MODE_11AX)) {
			    ...
			    **/
				...
				if (ChnlWidth < CH_40_MHz_WIDTH)
					...
					pIE->Offset = macMgmtMlme_Get40MHzExtChannelOffset(ChannelSwitchAnnouncementElement_p->Channel);

				if (ChnlWidth > CH_40_MHz_WIDTH) {
					pWideIE = (IEEEtypes_WideBWCS_Element_t *) (pIE + 1);
				...
				/** VHT: Wide Bandwidth Channel Switch Element [5]
			        ...
			        **/
					...
					getVhtBwFreqInfo(PhyDSSSTable->Chanflag.FreqBand,
							 ...
							 pChanSwitch->chInfo.chanflag.radiomode,
							 &pWideIE->ch_width, &pWideIE->center_freq0, &pWideIE->center_freq1);
				} else
					skb_trim(txSkb_p, txSkb_p->len - sizeof(IEEEtypes_WideBWCS_Element_t));
			} else {
	...
	if (1)			//sendResult == TRUE)        
	{
...
#ifdef MRVL_DFS
		{
			...
			mib_SpectrumMagament_p->csaChannelNumber = pChanSwitch->chInfo.channel;
			mib_SpectrumMagament_p->csaCount = pChanSwitch->ChannelSwitchCmd.ChannelSwitchCount;
			mib_SpectrumMagament_p->csaMode = pChanSwitch->ChannelSwitchCmd.Mode;
		}
#ifdef 	BARBADOS_DFS_TEST
...
#endif
		{
			if (pChanSwitch->chInfo.channel2) {
				extern BOOLEAN UpdateSecondChannelInMIB(vmacApInfo_t * vmacSta_p, UINT32 channel);

				UpdateSecondChannelInMIB(vmacSta_p, pChanSwitch->chInfo.channel2);
			}
...
#ifdef DEBUG_PRINT
		WLDBG_INFO(DBG_LEVEL_0, "RECEIVED A CHANNEL SWITCH ANNOUNCEMENT COMMAND :%d:%d:%d chan2=%d radiomode=%d\n",
			   pChanSwitch->chInfo.channel,
			   pChanSwitch->ChannelSwitchCmd.Mode,
			   pChanSwitch->ChannelSwitchCmd.ChannelSwitchCount, pChanSwitch->chInfo.channel2, PhyDSSSTable->Chanflag.radiomode);
#endif
#endif				//MRVL_DFS
		...
		wlFwSetChannelSwitchIE(vmacSta_p->dev,
				       pChanSwitch->chInfo.channel,
				       pChanSwitch->chInfo.channel2,
				       pChanSwitch->ChannelSwitchCmd.Mode, pChanSwitch->ChannelSwitchCmd.ChannelSwitchCount, &PhyDSSSTable->Chanflag);
...
void macMgmtMlme_ChannelSwitchReq(vmacApInfo_t * vmacSta_p, IEEEtypes_ChannelSwitchCmd_t * ChannelSwitchtCmd_p)
{
	MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
	macmgmtQ_MgmtMsg2_t *MgmtMsg_p;
	UINT16 FrameLen = sizeof(IEEEtypes_Category_t) + sizeof(IEEEtypes_ActionFieldType_t) + sizeof(IEEEtypes_ChannelSwitchAnnouncementElement_t);
#ifndef SOC_W906X
	UINT16 extraLen = sizeof(IEEEtypes_WideBWCS_Element_t) + sizeof(IEEEtypes_SecondaryChannelOffsetElement_t);
	IEEEtypes_SecondaryChannelOffsetElement_t *pIE;
	IEEEtypes_WideBWCS_Element_t *pWideIE;
...
#ifdef AP_MAC_LINUX
#ifndef SOC_W906X
		if ((txSkb_p = mlmeApiPrepMgtMsg2(IEEE_MSG_QOS_ACTION, &bcast, &vmacSta_p->macStaAddr, FrameLen + extraLen)) == NULL)
#else
		if ((txSkb_p = mlmeApiPrepMgtMsg2(IEEE_MSG_QOS_ACTION, &bcast, &vmacSta_p->macStaAddr, FrameLen)) == NULL)
#endif				/* SOC_W906X */
			return;
			...
			if (!(*(mib->mib_ApMode) == AP_MODE_B_ONLY
				...
				**/
				...
				if (ChnlWidth < CH_40_MHz_WIDTH)
					...
					pIE->Offset = macMgmtMlme_Get40MHzExtChannelOffset(ChannelSwitchtCmd_p->ChannelNumber);
	...
	if (1)			//sendResult == TRUE)        
	{
...
#ifdef MRVL_DFS
		{
			MIB_SPECTRUM_MGMT *mib_SpectrumMagament_p = vmacSta_p->ShadowMib802dot11->SpectrumMagament;
			/*Store the CSA Parameters in Shadow MIB */
			mib_SpectrumMagament_p->csaChannelNumber = ChannelSwitchtCmd_p->ChannelNumber;
			mib_SpectrumMagament_p->csaCount = ChannelSwitchtCmd_p->ChannelSwitchCount;
			mib_SpectrumMagament_p->csaMode = ChannelSwitchtCmd_p->Mode;
		}
#ifdef 	BARBADOS_DFS_TEST
		if (!dfs_monitor)	/* DFS monitor mode - do not update channel list, stay in current channel. */
#else
		...
		WLDBG_INFO(DBG_LEVEL_0, "RECEIVED A CHANNEL SWITCH ANNOUNCEMENT COMMAND :%d:%d:%d\n",
			   ChannelSwitchtCmd_p->ChannelNumber, ChannelSwitchtCmd_p->Mode, ChannelSwitchtCmd_p->ChannelSwitchCount);
#endif
#endif				//MRVL_DFS
		wlFwSetChannelSwitchIE(vmacSta_p->dev, ChannelSwitchtCmd_p->ChannelNumber,
#ifdef SOC_W906X
				       pChanSwitch->chInfo.channel2,
#endif
				       ChannelSwitchtCmd_p->Mode, ChannelSwitchtCmd_p->ChannelSwitchCount,
#ifdef SOC_W906X
...
static void msgMreportIndPack(IEEEtypes_MReportInd_t * MReportInd_p, macmgmtQ_MgmtMsg_t * MgmtMsg_p)
{
	...
	while (remainLen > 0) {
		...
		MReportInd_p->MeasureRepSet[idx].Report = MeasurementReport_p->Report.DefReport;
		/* remaining IE's total length */
		remainLen -= ieLen;
...
extern BOOLEAN macMgmtMlme_80211hAct(vmacApInfo_t * vmacSta_p, macmgmtQ_MgmtMsg3_t * MgmtMsg_p)
{
	...
	if (((*(mib->mib_ApMode) != AP_MODE_A_ONLY) && (*(mib->mib_ApMode) != AP_MODE_AandG)) || (mib_StaCfg_p->SpectrumManagementRequired != TRUE)) {
	...
	case TPC_REQUEST:
		{
			 ...
			 */
			macMgmtMlme_TPCReport(vmacSta_p, &MgmtMsg_p->Hdr.SrcAddr, MgmtMsg_p->Body.Action.DialogToken,
...
extern BOOLEAN macMgmtMlme_80211PublicAction(vmacApInfo_t * vmacSta_p, macmgmtQ_MgmtMsg3_t * MgmtMsg_p)
{
	...
	if (CoexistAction->Coexist_Report.Coexist.FortyMhz_Intorant == 1 || CoexistAction->Coexist_Report.Coexist.TwentyMhz_BSS_Width_Request == 1) {
	...
	for (i = 0; i < CoexistAction->Intolerant_Report.Len; i++) {
		...
		if (CoexistAction->Intolerant_Report.ChanList[i] != 0) {
			extern int Check_40MHz_Affected_range(vmacApInfo_t *, int, int);
			/* revert to 20MHz only when intolerant channel report_pri different with our own_pri */
			if (Check_40MHz_Affected_range(vmacSta_p, CoexistAction->Intolerant_Report.ChanList[i], 0)
			    && CoexistAction->Intolerant_Report.ChanList[i] != PhyDSSSTable->CurrChan)
				Handle20_40_Channel_switch(vmacSta_p, 0);
			break;
		}
...
void MonitorTimerProcess(UINT8 * data)
{
	...
	extern int protect_rx_rate_thres_max;
...
#ifdef SOC_W906X
	{
		...
		if (protect_dynamic) {
			...
			if (wlpptr->vdev[ivap] != NULL && (wlpptr->vdev[ivap]->flags & IFF_RUNNING)) {
				wlpptr_v = NETDEV_PRIV_P(struct wlprivate, wlpptr->vdev[ivap]);
				if (wlpptr_v->vmacSta_p->OpMode == WL_OP_MODE_VSTA) {
					entries_total = 1;
					do_check = 1;
				}
			} else {
				for (ivap = 0; ivap < wlpptr->wlpd_p->NumOfAPs; ivap++) {
					if (wlpptr->vdev[ivap] != NULL && (wlpptr->vdev[ivap]->flags & IFF_RUNNING)) {
						wlpptr_v = NETDEV_PRIV_P(struct wlprivate, wlpptr->vdev[ivap]);
			...
			if (do_check) {
				u32 rx_threshold = protect_rx_rate_thres * entries_total;

				rx_threshold = (rx_threshold < protect_rx_rate_thres_max) ? rx_threshold : protect_rx_rate_thres_max;
				txbps = (wlpptr->wlpd_p->wl_tpprofile.tx.bytes - vmacSta_p->tx_bytes_last) * 8;
				tcp_txbps = (wlpptr->wlpd_p->tpkt_type_cnt.tcp_byte_cnt - vmacSta_p->tcp_tx_bytes_last) * 8;;
				rxbps = (wlpptr->wlpd_p->wl_tpprofile.rx.bytes - vmacSta_p->rx_bytes_last) * 8;

				vmacSta_p->tx_bytes_last = wlpptr->wlpd_p->wl_tpprofile.tx.bytes;
				vmacSta_p->tcp_tx_bytes_last = wlpptr->wlpd_p->tpkt_type_cnt.tcp_byte_cnt;
				vmacSta_p->rx_bytes_last = wlpptr->wlpd_p->wl_tpprofile.rx.bytes;

				if ((tcp_txbps > (txbps * 8) / 10) && (rxbps > 100000)) {
					/* if tcp tx is dorminate(80% of overall tx traffic) and the rxbps is over 100kpbs, protect rts for TCP */
					mode = FORCE_PROTECT_RTS;
					wlFwSetProtectMode(wlpptr->netDev, HostCmd_ACT_GEN_SET, &mode);
				} else if ((txbps >= protect_tx_rate_thres) && (rxbps < rx_threshold)) {
					/* protect disable  for UDP */
					mode = FORCE_PROTECT_NONE;
					wlFwSetProtectMode(wlpptr->netDev, HostCmd_ACT_GEN_SET, &mode);
				} else if ((txbps >= protect_tx_rate_thres) && (rxbps >= rx_threshold)) {
					/* protect rts for bidirectional traffice */
					mode = FORCE_PROTECT_RTS;
					wlFwSetProtectMode(wlpptr->netDev, HostCmd_ACT_GEN_SET, &mode);
				}
			}
...
int getMBSSID_index(IEEEtypes_Mbssid_Element_t * pmb)
{
	...
	pNonTxPf = (IEEEtypes_NonTransmitted_BSSID_Profile_t *) & pmb->NonTxPf;

	if (pNonTxPf->subElementId != NONTRANSMITTED_BSSID_PROFILE_SUBELM_ID)
		goto exit;
	...
	while (len > 0) {
		...
		if (subie[0] == MBSSID_INDEX) {	//found 
			return (((IEEEtypes_mbssid_idx_t *) subie)->BssidIdx);
		}
...
u8 extract_mbssid_ies(void *attrib_p, int len, mbssid_ie_db_t ** phead)
{
	...
	do {
		...
		if ((entry = (mbssid_ie_db_t *) wl_kmalloc(sizeof(mbssid_ie_db_t), GFP_ATOMIC)) == NULL) {
			printk("Fail to allocating mbssid_ie_db_t entry\n");
			goto exit;
		}

		memset((void *)entry, 0, sizeof(mbssid_ie_db_t));
		memcpy((u8 *) entry->iebuf, (u8 *) attrib_p, (pmb->Len + 2));
	...
	for (i = 0; i < num; i++) {
		...
		while (pnode) {
			...
			while (length > len1) {
				...
				for (k = 0; k < 4 && pnode->ietable[subie[0]][k] != 0; k++) ;
...
UINT8 decomposeMBSSID(struct net_device * dev, struct sk_buff * skb, struct sk_buff_head * skbList)
{
	...
	if (wlanMsg_p->Hdr.FrmCtl.Subtype != IEEE_MSG_BEACON && wlanMsg_p->Hdr.FrmCtl.Subtype != IEEE_MSG_PROBE_RSP)
		goto exit;

	wlanMsg_p->Hdr.FrmBodyLen = skb->len;

	if ((attrib_p = syncSrv_ParseAttrib((macmgmtQ_MgmtMsg_t *) wlanMsg_p, EXT_CAP_IE, len)) != NULL) {
		IEEEtypes_Extended_Cap_Element_t *pextie = (IEEEtypes_Extended_Cap_Element_t *) attrib_p;

		if (!(pextie->Len > 0 && pextie->ExtCap.MultipleBSSID == 1)) {
	...
	if ((attrib_p = syncSrv_ParseAttrib((macmgmtQ_MgmtMsg_t *) wlanMsg_p, MULTI_BSSID, len)) != NULL) {
		//mbssid_ie_db_t  *pentry;
		int len2 = (skb->len - (u32) ((u8 *) attrib_p - skb->data));

		//printk("MULTI_BSSID: len2:%u\n", len2);

		//mwl_hex_dump(attrib_p, len2);

		if ((num = extract_mbssid_ies(attrib_p, len2, &phead)) == 0) {
	...
	while (num > cnt) {
		...
		pmgmtf_src = (macmgmtQ_MgmtMsg_t *) ((UINT8 *) skb->data - 2);

		pmgmtf = (macmgmtQ_MgmtMsg_t *) ((UINT8 *) skb2->data - 2);
		pmgmtf->Hdr.FrmBodyLen = skb->len;

		len = skb2->len + 2;
		len1 = ((u8 *) & (pmgmtf->Body.Bcn.SsId) - (u8 *) pmgmtf);
		...
		pmgmtf->Hdr.SrcAddr[5] = (pmgmtf->Hdr.SrcAddr[5] + pnode->mbssid_index) & mask;
		pmgmtf->Hdr.BssId[5] = (bssid_A | pmgmtf->Hdr.SrcAddr[5]);
		...
		while (len > len1) {
			...
			do {
				if (pnode->ietable[iesrc[0]][0] == 0) {	//no need to update
					...
					for (i = 0; i < 4 && pnode->ietable[iesrc[0]][i]; i++) {
...
extern void RxBeacon(vmacApInfo_t * vmacSta_p, void *BssData_p, UINT16 len, UINT32 rssi)
{
	...
	if ((attrib_p = syncSrv_ParseAttrib((macmgmtQ_MgmtMsg_t *) BssData_p, ADD_HT, len)) != NULL) {
		if (((IEEEtypes_Add_HT_Element_t *) attrib_p)->OpMode.NonHTStaPresent == 1)
			wlpptr->wlpd_p->legAPCount = 1;
	} else {
		wlpptr->wlpd_p->legAPCount = 1;
	}
	 ...
	 */
	if ((vmacSta_p->preautochannelfinished == 0) && (*(mib->mib_autochannel) != 0)) {
		return;
	}
	...
	if (PhyDSSSTable->Chanflag.FreqBand == FREQ_BAND_2DOT4GHZ && (vmacSta_p->busyScanning ||	/* Recv beacon during coex scan */
								      PhyDSSSTable->Chanflag.ChnlWidth == CH_AUTO_WIDTH ||
								      PhyDSSSTable->Chanflag.ChnlWidth ==
								      CH_40_MHz_WIDTH) /*&& (rssi <= FortyMIntolerantRSSIThres) */ ) {
...
/** 40MHz channel capable AP found **/
		...
		if ((attrib_p = syncSrv_ParseAttrib((macmgmtQ_MgmtMsg_t *) BssData_p, DS_PARAM_SET, len)) != NULL) {
			...
			if (dsPSetIE_p->CurrentChan != PhyDSSSTable->CurrChan) {
				...
				if (!HTCap || AddHtChannelOffset == 0 || AddHtChannelOffset == PhyDSSSTable->Chanflag.ExtChnlOffset) {
					/* (20MHz AP) or (40MHz AP. ExtCh is same) */
				} else {
					/* 40Mhz AP. ExtCh is not the same */
					if (AddHtChannelOffset == EXT_CH_ABOVE_CTRL_CH)
						RxSecChan = dsPSetIE_p->CurrentChan + 4;
					else
						RxSecChan = dsPSetIE_p->CurrentChan - 4;
					if (Check_40MHz_Affected_range(vmacSta_p, RxSecChan, 0)) {
						Handle20_40_Channel_switch(vmacSta_p, 1);
...
{
	TimerRemoveSync(&vmacSta_p->AgingTimer);
	TimerRemoveSync(&vmacSta_p->monTimer);
	TimerRemoveSync(&vmacSta_p->KeepAliveTimer);
	TimerRemoveSync(&vmacSta_p->GrpKeytimer);
#ifdef AUTOCHANNEL
	TimerRemoveSync(&vmacSta_p->scaningTimer);
#endif
}
...
void IEEEToMrvlRateBitMapConversion(UINT8 SupportedIEEERate, UINT32 * pMrvlLegacySupportedRateBitMap)
{
	...
	while (IEEEToMrvlRateBitMapConversionTbl[i].IEEERate != ENDOFTBL) {
		if ((IEEEToMrvlRateBitMapConversionTbl[i].IEEERate == SupportedIEEERate)) {
			*pMrvlLegacySupportedRateBitMap = *pMrvlLegacySupportedRateBitMap | (IEEEToMrvlRateBitMapConversionTbl[i].MrvlRateBitMap);
		}
		i++;
	}
}
...
void Coexist_TimeoutHdlr(void *pData)
{
	...
	if (*(vmacSta_p->ShadowMib802dot11->mib_HT40MIntoler) && (PhyDSSSTable->Chanflag.ChnlWidth != CH_20_MHz_WIDTH)) {
		if (*(vmacSta_p->Mib802dot11->mib_ApMode) == AP_MODE_N_ONLY
		    ...
		    || *(vmacSta_p->Mib802dot11->mib_ApMode) == AP_MODE_2_4GHZ_Nand11AX
		    || *(vmacSta_p->Mib802dot11->mib_ApMode) == AP_MODE_2_4GHZ_11AX_MIXED) {
			int mode = 1;

			if (PhyDSSSTable->Chanflag.ExtChnlOffset) {
				if (PhyDSSSTable->CurrChan > 9)
					PhyDSSSTable->Chanflag.ExtChnlOffset = EXT_CH_BELOW_CTRL_CH;
				else if (PhyDSSSTable->CurrChan < 5)
			...
			macMgmtMlme_UpdateRateTable4AllSta(vmacSta_p);
...
void StartCoexistTimer(vmacApInfo_t * vmacSta_p)
{
	...
	TimerFireIn(&vmacSta_p->CoexistTimer, 1, Coexist_TimeoutHdlr, (unsigned char *)vmacSta_p,
		    *(mib->mib_Channel_Width_Trigger_Scan_Interval) * (*(mib->mib_Channel_Transition_Delay_Factor)) * 10);

}

void Disable_StartCoexisTimer(vmacApInfo_t * vmacSta_p)
{
	TimerRemoveSync(&vmacSta_p->CoexistTimer);
}
...
int Check_40MHz_Affected_range(vmacApInfo_t * vmacSta_p, int curchannel, int extchan)
{
	...
	else if ((PhyDSSSTable->Chanflag.ExtChnlOffset == EXT_CH_BELOW_CTRL_CH) && (PhyDSSSTable->CurrChan > 4))
		fs = channel[PhyDSSSTable->CurrChan - 4 - 1];
...
void macMgmtMlme_UpdateRateTable4AllSta(vmacApInfo_t * vmac_p)
{
	vmacApInfo_t *vmacSta_p = NULL, *vmacAp_p = NULL;
	ExtStaInfoItem_t *Curr_p = NULL, *Item_p = NULL;
	extStaDb_StaInfo_t *StaInfo_p = NULL;
	struct wlprivate *wlpptr = NULL;
	unsigned long dbflags = 0, count = 0, i = 0;
	...
	while (Curr_p != NULL) {
		...
		if ((StaInfo_p = extStaDb_GetStaInfo(vmacSta_p, &(Item_p->StaInfo.Addr), STADB_NO_BLOCK | STADB_SKIP_MATCH_VAP)) != NULL
		    && StaInfo_p->AP == FALSE && StaInfo_p->State == ASSOCIATED) {
			if ((vmacAp_p = vmacGetMBssByAddr(vmacSta_p, StaInfo_p->Bssid)) != NULL) {
				/* Clear old txrate_histogram statics */
				if (wlpptr->wlpd_p->txRateHistogram[StaInfo_p->StnId] != NULL) {
					memset(wlpptr->wlpd_p->txRateHistogram[StaInfo_p->StnId]->CurRateInfo, 0, sizeof(UINT32) * SU_MU_TYPE_CNT);
					memset(wlpptr->wlpd_p->txRateHistogram[StaInfo_p->StnId]->TotalTxCnt, 0, sizeof(UINT32) * SU_MU_TYPE_CNT);
					for (i = 0; i < RATE_ADAPT_MAX_SUPPORTED_RATES; i++) {
						wlpptr->wlpd_p->txRateHistogram[StaInfo_p->StnId]->SU_rate[i].cnt = 0;
						memset(wlpptr->wlpd_p->txRateHistogram[StaInfo_p->StnId]->SU_rate[i].per, 0,
						       sizeof(UINT32) * TX_RATE_HISTO_PER_CNT);
					}
				}
				wlAcntCopyRateTbl(vmacAp_p->dev, StaInfo_p->Addr, StaInfo_p->StnId, SU_MIMO);
			}
		}
		Curr_p = Curr_p->nxt;
	}
...
void Handle20_40_Channel_switch(vmacApInfo_t * vmacSta_p, UINT8 Scanning)
{
	...
	if (*(vmacSta_p->ShadowMib802dot11->mib_HT40MIntoler) && (mib->PhyDSSSTable->Chanflag.ChnlWidth != CH_20_MHz_WIDTH)) {
		extern int wlFwSet11N_20_40_Switch(struct net_device *netdev, UINT8 mode);
		extern BOOLEAN macMgmtMlme_SendNotifyChannelWidthManagementAction(vmacApInfo_t * vmacSta_p, UINT8 mode);
...
/** we are already at 20MHz mode **/
		...
		printk("%s: HT2040 BW Switch: %u -> %u\n", vmacSta_p->dev->name, wlpptr->wlpd_p->BcnAddHtAddChannel, BcnAddHtAddChannel);
	...
	if ((legacyAPCount[HWIndex] != wlpptr->wlpd_p->legAPCount) || (counterHtProt[HWIndex]++ % 10 == 0)) {	/* 30s check */
		...
		while (i <= bss_num) {
			if (wlpptr->vdev[i]) {
				dev = wlpptr->vdev[i];
				wlpptr1 = NETDEV_PRIV_P(struct wlprivate, dev);
				if (wlpptr1->vmacSta_p != NULL && wlpptr1->vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_AP)
...
BOOLEAN macMgmtMlme_SendMimoPsHtManagementAction(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * Addr, UINT8 mode)
{
	...
	UINT16 FrameLen = sizeof(IEEEtypes_Category_t) + sizeof(IEEEtypes_ActionFieldType_t) + sizeof(IEEEtypes_SM_PwrCtl_t);
...
BOOLEAN macMgmtMlme_SendNotifyChannelWidthManagementAction(vmacApInfo_t * vmacSta_p, UINT8 mode)
{
	...
	UINT16 FrameLen = sizeof(IEEEtypes_Category_t) + sizeof(IEEEtypes_ActionFieldType_t) + sizeof(IEEEtypes_BWCtl_t);
...
BOOLEAN macMgmtMlme_SendInfoExchHtManagementAction(struct net_device * dev, IEEEtypes_MacAddr_t * Addr)
{
	...
	UINT16 FrameLen = sizeof(IEEEtypes_Category_t) + sizeof(IEEEtypes_ActionFieldType_t) + sizeof(IEEEtypes_InfoExch_t);
...
BOOLEAN macMgmtMlme_SendBeaconReqMeasureReqAction(struct net_device * dev, IEEEtypes_MacAddr_t * Addr)
{
	...
	UINT16 FrameLen = sizeof(IEEEtypes_Category_t) +
	    ...
	    sizeof(UINT8) + sizeof(UINT16) + sizeof(IEEEtypes_MeasurementRequestEl_t) - (34 - strlen(&(mib->StationConfig->DesiredSsId[0])));
...
BOOLEAN macMgmtMlme_SendExtChanSwitchAnnounceAction(struct net_device * dev, IEEEtypes_MacAddr_t * Addr)
{
	...
	UINT16 FrameLen = sizeof(IEEEtypes_Category_t) + sizeof(IEEEtypes_ActionFieldType_t) + sizeof(IEEEtypes_ExtendChanSwitchAnnounceEl_t);
...
#ifdef SOC_W906X
BOOLEAN UpdateSecondChannelInMIB(vmacApInfo_t * vmacSta_p, UINT32 channel)
{
	extern BOOLEAN force_5G_channel;
	MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
	MIB_PHY_DSSS_TABLE *PhyDSSSTable = mib->PhyDSSSTable;

	if (domainChannelValid(channel, force_5G_channel ? FREQ_BAND_5GHZ : (channel <= 14 ? FREQ_BAND_2DOT4GHZ : FREQ_BAND_5GHZ))) {
...
BOOLEAN UpdateCurrentChannelInMIB(vmacApInfo_t * vmacSta_p, UINT32 channel)
{
...
#endif
	{
		...
		if (PhyDSSSTable->CurrChan == 14) {
			if ((PhyDSSSTable->Chanflag.ChnlWidth == CH_AUTO_WIDTH) || (PhyDSSSTable->Chanflag.ChnlWidth == CH_40_MHz_WIDTH))
...
#endif
	MIB_PHY_DSSS_TABLE *PhyDSSSTable = mib->PhyDSSSTable;
	IEEEtypes_DsParamSet_t ds_param_ie = { DS_PARAM_SET, 1, 0 };
	IEEEtypes_VhOpt_t vht_op_ie;

	/* copy to buffer of running mbss under this radio interface */
	for (i = 0; i <= bss_num; i++) {
		struct wlprivate *wlpptr_mbss;

		if (wlpptr->vdev[i]->flags & IFF_RUNNING) {
			wlpptr_mbss = NETDEV_PRIV_P(struct wlprivate, wlpptr->vdev[i]);
			/* update the DSSS Parameter Set element */
			ds_param_ie.CurrentChan = channel;
			/* update VHT OPERATION IE */
			Build_IE_192(wlpptr->vmacSta_p, (UINT8 *) & vht_op_ie);
			if (PhyDSSSTable->Chanflag.FreqBand == FREQ_BAND_2DOT4GHZ)
				macMgmtMlme_UpdateProbeRspBasicIes(wlpptr_mbss->vmacSta_p, (u8 *) & ds_param_ie, sizeof(ds_param_ie));
			macMgmtMlme_UpdateExistedProbeRspIE(wlpptr_mbss->vmacSta_p, (UINT8 *) & vht_op_ie, sizeof(vht_op_ie));
		}
	}

	{
#ifdef MRVL_DFS
		if ((toSmeMsg = (smeQ_MgmtMsg_t *) wl_kmalloc(sizeof(smeQ_MgmtMsg_t), GFP_ATOMIC)) == NULL) {
		...
		toSmeMsg->Msg.ChanSwitchCfrm.chInfo.channel2 = PhyDSSSTable->SecChan;

		memcpy(&toSmeMsg->Msg.ChanSwitchCfrm.chInfo.chanflag, &PhyDSSSTable->Chanflag, sizeof(CHNL_FLAGS));
#ifdef RADAR_SCANNER_SUPPORT
		toSmeMsg->Msg.ChanSwitchCfrm.chInfo.no_cac = PhyDSSSTable->no_cac;
...
void macMgmtMlme_SwitchChannel(struct net_device *dev, UINT8 channel, UINT8 channel2, CHNL_FLAGS * chanFlag_p)
{
	...
	if (wlpptr->wlpd_p->ext_scnr_en) {
		extern DFS_STATUS dfs_set_op_ch(struct net_device *dev, UINT16 channel);
	...
	if (UpdateCurrentChannelInMIB(syscfg, channel)) {
		...
		if (wlchannelSet(dev, channel, channel2, &PhyDSSSTable->Chanflag, 0))
#else
...
void macMgmtMlme_StopDataTraffic(struct net_device *dev)
{
	...
	dfsTrace(dev, 0xe, 0x1, NULL, (0x10000000 | ((dev->flags & IFF_RUNNING) << 4) | (pdfsApMain ? 1 : 0)));
...
void macMgmtMlme_RestartDataTraffic(struct net_device *dev)
{
	...
	dfsTrace(dev, 0xe, 0x2, NULL, (0x10000000 | ((dev->flags & IFF_RUNNING) << 4) | (pdfsApMain ? 1 : 0)));
...
static void HT40MIntolerantHandler(vmacApInfo_t * vmacSta_p, UINT8 soon)
{
	...
	if (((mib->PhyDSSSTable->Chanflag.ChnlWidth == CH_40_MHz_WIDTH) || (mib->PhyDSSSTable->Chanflag.ChnlWidth == CH_80_MHz_WIDTH)
	     || (mib->PhyDSSSTable->Chanflag.ChnlWidth == CH_AUTO_WIDTH))
	    && ((*(mib->USER_ChnlWidth) == CH_40_MHz_WIDTH) || (*(mib->USER_ChnlWidth) == CH_80_MHz_WIDTH)
...
void RecHTIntolerant(vmacApInfo_t * vmacSta_p, UINT8 enable)
{
	...
	if (enable) {
		...
		if (((mib->PhyDSSSTable->Chanflag.ChnlWidth == CH_40_MHz_WIDTH) || (mib->PhyDSSSTable->Chanflag.ChnlWidth == CH_80_MHz_WIDTH)
		     || (mib->PhyDSSSTable->Chanflag.ChnlWidth == CH_AUTO_WIDTH)) && ((*(mib->USER_ChnlWidth) == CH_40_MHz_WIDTH)
...
int MUchecksta_SetList(vmacApInfo_t * vmac_p, extStaDb_StaInfo_t * pStaInfo)
{
	...
	while (tempnode != NULL) {
		...
		for (i = 0; i < 3; i++) {
			...
			if (tempnode->StaInfo[i] != NULL) {
				if (memcmp(tempnode->StaInfo[i]->Addr, pStaInfo->Addr, 6) == 0)
					return 1;
			}
		}
...
int MUchecksta_StaList(vmacApInfo_t * vmac_p, extStaDb_StaInfo_t * pStaInfo)
{
	MUCapStaNode_t *tempnode = NULL;
	u8 index;

	index = is_he_capable_sta(pStaInfo) * 4 + pStaInfo->vht_RxChannelWidth;
	...
	while (tempnode != NULL) {
		if (tempnode->StaInfo_p != NULL) {
			if (memcmp(tempnode->StaInfo_p->Addr, pStaInfo->Addr, 6) == 0)
				return 1;
		}
...
#ifdef SOC_W906X
int MUinsert_node(vmacApInfo_t * vmac_p, extStaDb_StaInfo_t * MUUsr[], int index)
#else
int MUinsert_node(vmacApInfo_t * vmac_p, extStaDb_StaInfo_t * pStaInfo0, extStaDb_StaInfo_t * pStaInfo1, extStaDb_StaInfo_t * pStaInfo2, int index)
#endif
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, vmac_p->dev);
	muset_t *tempnode;
	...
	ListPutItem((List *) & wlpptr->wlpd_p->MUSetList, (ListItem *) tempnode);
...
*/
#ifdef SOC_W906X
UINT8 MUCreateMUSet(vmacApInfo_t * vmac_p, extStaDb_StaInfo_t * pStaInfo[])
#else
UINT8 MUCreateMUSet(vmacApInfo_t * vmac_p, extStaDb_StaInfo_t * pStaInfo0, extStaDb_StaInfo_t * pStaInfo1, extStaDb_StaInfo_t * pStaInfo2)
#endif
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, vmac_p->dev);
...
#ifdef SOC_W906X
	for (i = 0; i < 63; i++) {
		if (!MUcheck_index((muset_t *) wlpptr->wlpd_p->MUSetList.head, i))
			break;
...
#else
	if (MUinsert_node(vmac_p, pStaInfo0, pStaInfo1, pStaInfo2, i) == 0)
#endif
	{
	...
	if (myGid != 0) {
		...
		for (i = 0; i < MU_MAX_USERS; i++) {
			if (pStaInfo[i] != NULL) {
				if (!is_he_capable_sta(pStaInfo[i]))
					SendGroupIDMgmtframe(vmac_p, pStaInfo[i]->Addr, myGid, i);
				else
...
#ifdef SOC_W906X
		if (wlFwSetMUSet(vmac_p->dev, cmd_option, myGid, myGid - 1, stnid))
#else
		if (wlFwSetMUSet(vmac_p->dev, cmd_option, myGid, myGid - 1, stnid[0], stnid[1], stnid[2]))
#endif
		{
#ifdef SOC_W906X
			printk("Setting %s MU Set GID=%d Index=%d Stnid=", (cmd_option == 2) ? "HE" : "VHT", myGid, myGid - 1);
			for (i = 0; (i < MU_MAX_USERS) && (stnid[i] != 0xffff); i++)
				printk("%d ", stnid[i]);
			printk("\n");
#else
		...
		/*If fw cmd to create MU Set fail, remove MU set node created earlier in this function */
			...
			for (i = 0; i < MU_MAX_USERS; i++) {
				if ((pStaInfo[i] != NULL) && (pStaInfo[i]->MUset != NULL)) {
					muset_node = pStaInfo[i]->MUset;
					break;
				}
			}
#ifdef SOC_W906X
			MUDel_MUSet(vmac_p, muset_node, MUSet_BLOCK | MUSet_NO_STA_BLOCK);
#else
...
*/
BOOLEAN MUAddStaToMUStaList(vmacApInfo_t * vmac_p, extStaDb_StaInfo_t * StaInfo_p)
{
...
#ifdef SOC_W906X
			MUDel_MUSet(vmac_p, StaInfo_p->MUset, (MUSet_BLOCK | MUSet_NO_STA_BLOCK));
#else
			MUDel_MUSet(vmac_p, StaInfo_p->MUset, MUSet_FW_DEL | MUSet_NO_GID_FRAME);
#endif
	...
	if (tmp) {
		...
		item_p->MUStaList_idx = is_he_capable_sta(StaInfo_p) * 4 + StaInfo_p->vht_RxChannelWidth;
		if (wfa_11ax_pf) {
			if ((StaInfo_p->vht_RxChannelWidth == 3) && !(vmacSta_master_p->he_cap.phy_cap.channel_width_set & HE_SUPPORT_160MHZ_BW_5G))
				item_p->MUStaList_idx = is_he_capable_sta(StaInfo_p) * 4 + 2;
		}

		StaInfo_p->MUStaListNode = item_p;
		ListPutItem((List *) & vmac_p->MUStaList[item_p->MUStaList_idx], (ListItem *) tmp);
...
* IMPORTANT: Make sure caller to this function has locked MUStaListLock and MUSetflags
*/
BOOLEAN MUDelStaFromMUStaList(vmacApInfo_t * vmac_p, MUCapStaNode_t * MUStaNode_p)
{
	MUCapStaNode_t *item_p = NULL;
	//ListItem *tmp;
	vmacApInfo_t *vmacSta_master_p;
	...
	if (MUStaNode_p != NULL) {
		...
		ListPutItem(&vmacSta_master_p->StaCtl->FreeMUStaList, ListRmvItem((List *) & vmac_p->MUStaList[index], (ListItem *) item_p));
...
*/
#ifdef SOC_W906X
BOOLEAN MURemStaFromMUSet(vmacApInfo_t * vmac_p, extStaDb_StaInfo_t * pStaInfo, UINT8 option)
#else
BOOLEAN MURemStaFromMUSet(vmacApInfo_t * vmac_p, extStaDb_StaInfo_t * pStaInfo)
#endif
{
	UINT8 index;
#ifdef SOC_W906X
...
*/
muset_t *MUPrefer2Usr(struct net_device * netdev, MU_Sta_List * MUStaList, UINT32 * MUUsr_total, UINT8 index)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		...
		if (free_MUUsr_cnt == 1) {
			...
			if (free_sta_p != NULL) {
				...
				if (((muset_p->antcnt == 8) && (muset_p->cnt == 2)) ||
				    ((muset_p->cnt == 2) && ((free_sta_p->StaInfo_p->vht_peer_RxNss + muset_p->antcnt) > 8))) {
			 ...
			 */
			if (wlpptr->wlpd_p->MUSetList.total_sta && ((wlpptr->wlpd_p->MUSetList.total_sta % 2) != 0)) {
				muset_p = (muset_t *) wlpptr->wlpd_p->MUSetList.tail;
...
* If pointer return is NULL, then create new group.
*/
muset_t *MUPrefer3Usr(struct net_device * netdev, MU_Sta_List * MUStaList, UINT32 * MUUsr_total, UINT8 index)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	vmacApInfo_t *vmac_p = (vmacApInfo_t *) wlpptr->vmacSta_p;
	muset_t *muset_p = NULL;
	...
	if (free_MUUsr_cnt) {
		...
		if (free_MUUsr_cnt == 1) {
			 ...
			 */
			if (wlpptr->wlpd_p->MUSetList.total_sta && ((wlpptr->wlpd_p->MUSetList.total_sta % 3) != 0)) {
				muset_p = (muset_t *) wlpptr->wlpd_p->MUSetList.tail;
...
* If pointer return is NULL, then create new group.
*/
muset_t *MUPreferNUsr(struct net_device * netdev, MU_Sta_List * MUStaList, UINT32 * MUUsr_total, UINT8 index, UINT32 n_user)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	vmacApInfo_t *vmac_p = (vmacApInfo_t *) wlpptr->vmacSta_p;
	muset_t *muset_p = NULL;
	...
	if (free_MUUsr_cnt) {
		...
		if (free_MUUsr_cnt == 1) {
			...
			if (free_sta_p != NULL) {
#ifdef SOC_W906X
				if (((muset_p->antcnt == mu_max_user) && (muset_p->cnt == (n_user - 1))) ||
				    ((muset_p->cnt == (n_user - 1)) && ((free_sta_p->StaInfo_p->vht_peer_RxNss + muset_p->antcnt) > mu_max_user))) {
					//((muset_p->antcnt==2) &&(muset_p->cnt==2) && (free_sta_p->StaInfo_p->vht_peer_RxNss==2))){
#else
				if (((muset_p->antcnt == 3) && (muset_p->cnt == 2)) ||
				    ((muset_p->antcnt == 2) && (muset_p->cnt == 2) && (free_sta_p->StaInfo_p->vht_peer_RxNss == 2))) {
			 ...
			 * Find this muset with 2 usrs and decide to break this muset or create new muset without breaking a muset
			 */
			if (wlpptr->wlpd_p->MUSetList.total_sta && ((wlpptr->wlpd_p->MUSetList.total_sta % n_user) != 0)) {
				muset_p = (muset_t *) wlpptr->wlpd_p->MUSetList.tail;
			 ...
			 * If 2 usrs muset + free MU sta give multiple of 3, break group to form 3usrs muset. Otherwise, just form new muset
			 */
			if ((muset_p != NULL) && (muset_p->cnt == (n_user - 1))) {
...
void MUAutoSet_Hdlr(struct net_device *netdev)
{
	...
	for (i = 0; i < ARRAY_SIZE(vmac_p->MUStaList); i++) {
		...
		if (vmac_p->MUSet_Prefer_UsrCnt == 2)
			del_muset[del_muset_cnt] = MUPrefer2Usr(netdev, &vmac_p->MUStaList[i], &MUUsr_total, i);
		else {
			if ((i < 4) && (vmac_p->MUSet_Prefer_UsrCnt > 4))
				del_muset[del_muset_cnt] = MUPreferNUsr(netdev, &vmac_p->MUStaList[i], &MUUsr_total, i, 4);
			else
				del_muset[del_muset_cnt] = MUPreferNUsr(netdev, &vmac_p->MUStaList[i], &MUUsr_total, i, vmac_p->MUSet_Prefer_UsrCnt);
		}
...
#ifdef SOC_W906X
			if (!item_p->MUSet_created_flag && (item_p->StaInfo_p->vht_peer_RxNss <= 4)) {
#else
			if (!item_p->MUSet_created_flag && (item_p->StaInfo_p->vht_peer_RxNss < 3)) {
#endif

#ifdef MUGRP_DELAY
				if (((item_p->StaInfo_p->aggr11n.type & WL_WLAN_TYPE_AMPDU) == 0) || (item_p->StaInfo_p->rx_packets == 0)) {
...
#ifdef SOC_W906X
					if ((nooftxantenna + item_p->StaInfo_p->vht_peer_RxNss) <= 8) {
#else
					if ((nooftxantenna + item_p->StaInfo_p->vht_peer_RxNss) <= 3) {
#endif
						nooftxantenna += item_p->StaInfo_p->vht_peer_RxNss;
						MUUsr[MUUsr_cnt] = item_p->StaInfo_p;
						MUUsr_cnt++;
					}
				}
			}
...
#else
			if (MUCreateMUSet(vmac_p, MUUsr[0], MUUsr[1], MUUsr[2])) {
#endif
				...
				while (cnt) {
					if (MUUsr[cnt - 1] != NULL) {
						if (MUUsr[cnt - 1]->MUStaListNode != NULL)
							MUUsr[cnt - 1]->MUStaListNode->MUSet_created_flag = 1;
...
#ifdef SOC_W906X
BOOLEAN MUManualSet(vmacApInfo_t * vmac_p, extStaDb_StaInfo_t * pStaInfo[])
#else
BOOLEAN MUManualSet(vmacApInfo_t * vmac_p, extStaDb_StaInfo_t * pStaInfo0, extStaDb_StaInfo_t * pStaInfo1, extStaDb_StaInfo_t * pStaInfo2)
#endif
{
	...
	free_MUUsr_cnt = vmac_p->MUStaList[index].cnt - vmac_p->MUStaList[index].taken_musta_cnt;
	...
	if (free_MUUsr_cnt >= 2) {
...
#endif
		{
			...
			if (MUCreateMUSet(vmac_p, MUUsr[0], MUUsr[1], MUUsr[2])) {
#endif
				vmac_p->MUStaList[index].taken_musta_cnt += MUUsr_cnt;
...
* IMPORTANT: Make sure caller to this function has locked MUStaListLock and MUSetflags
*/
BOOLEAN MUDel_MUSet(vmacApInfo_t * vmac_p, muset_t * muset_p, UINT8 option)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, vmac_p->dev);
	muset_t *delnode;
	UINT8 i;
			...
			for (i = 0; i < MU_MAX_USERS; i++) {
				...
				if (muset_p->StaInfo[i] != NULL) {
					if (option & MUSet_NO_STA_BLOCK)
						MURemStaFromMUSet(vmac_p, muset_p->StaInfo[i], MUSta_NO_BLOCK);
					else
						MURemStaFromMUSet(vmac_p, muset_p->StaInfo[i], MUSta_BLOCK);
					muset_p->StaInfo[i] = NULL;	//set to NULL only at the end after all been reset
				}
			}

			delnode = (muset_t *) ListRmvItem((List *) & wlpptr->wlpd_p->MUSetList.head, (ListItem *) muset_p);
	...
	if (muset_p != NULL) {
		...
		if (((option & MUSet_FW_DEL) == 0) || wlFwSetMUSet(vmac_p->dev, 0, muset_p->index + 1, muset_p->index, 0, 0, 0)) {	//when del, sta id not needed in fw
			...
			for (i = 0; i < 3; i++) {
				if (muset_p->StaInfo[i] != NULL) {
					if ((option & MUSet_NO_GID_FRAME) == 0) {
						...
						SendGroupIDMgmtframe(vmac_p, muset_p->StaInfo[i]->Addr, 0, 0);
					}
			...
			if (delnode != NULL) {
				if (wlpptr->wlpd_p->MUSetList.total_sta >= delnode->cnt)
...
void MUDel_MUSetIndex(vmacApInfo_t * vmac_p, UINT8 muset_index)
{
		...
		if (tempnode->index == muset_index) {
			...
			if (wlFwSetMUSet(vmac_p->dev, 0, muset_index + 1, muset_index, 0, 0, 0)) {	//when del, sta id not needed in fw
				...
				for (i = 0; i < MU_MAX_USERS; i++) {
					...
					if (tempnode->StaInfo[i] != NULL) {
#ifdef SOC_W906X
						MURemStaFromMUSet(vmac_p, tempnode->StaInfo[i], MUSta_BLOCK);
#else
						BUG_ON(SPIN_LOCK_IS_LOCKED_BY_SAME_CORE(&vmacSta_master_p->StaCtl->dbLock));
						SendGroupIDMgmtframe(vmac_p, tempnode->StaInfo[i]->Addr, 0, 0);
	...
	if (foundnode != NULL) {
		delnode = (muset_t *) ListRmvItem((List *) & wlpptr->wlpd_p->MUSetList.head, (ListItem *) foundnode);
		if (delnode) {
			if (wlpptr->wlpd_p->MUSetList.total_sta >= delnode->cnt)
				wlpptr->wlpd_p->MUSetList.total_sta -= delnode->cnt;
	...
	for (i = 0; i < ARRAY_SIZE(vmac_p->MUStaList); i++) {
		...
		if ((vmac_p->MUStaList[i].head != NULL)) {
			...
			printk("MUStaList: %s, %dMHz, total cnt:%d, isgrouped cnt:%d\n\n", (item_p->MUStaList_idx > 3) ? "HE" : "VHT",
			       bw[item_p->MUStaList_idx % 4], vmac_p->MUStaList[i].cnt, vmac_p->MUStaList[i].taken_musta_cnt);
			...
			while (item_p != NULL) {
				if (item_p->StaInfo_p != NULL) {
					...
					printk("MUBeamformeeCapable=%x VHTBW=%dMHz RxNss=%x\n", pStaInfo->vhtCap.cap.MUBeamformeeCapable,
					       bw[pStaInfo->vht_RxChannelWidth], pStaInfo->vht_peer_RxNss);
...
void fillStaInfo(struct net_device *netdev, struct station_info *pSinfo, struct extStaDb_StaInfo_t *pStaInfo)
{
	...
	rssi = pStaInfo->RSSI ? -(pStaInfo->RSSI) : -(pStaInfo->assocRSSI);
	...
	pSinfo->txrate.legacy = getPhyRate((dbRateInfo_t *) & (pStaInfo->RateInfo));
	pSinfo->txrate.nss = getNss((dbRateInfo_t *) & (pStaInfo->RateInfo));

	memcpy(&RateInfo, &(pStaInfo->rx_info_aux.rate_info), sizeof(dbRateInfo_t));
	if (pStaInfo->rx_info_aux.nss > 0) {
		RateInfo.RateIDMCS |= ((pStaInfo->rx_info_aux.nss - 1) & 0x7) << 4;
	}
...
#ifdef OPENWRT
	pSinfo->connected_time = ktime_get_seconds() - pStaInfo->last_connected;
#else
	pSinfo->connected_time = 0;
#endif
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,10,0)
	pSinfo->inactive_time = jiffies_to_msecs(jiffies - netdev->last_rx);
#endif

	memset(&StaStatsTbl, 0, sizeof(SMAC_STA_STATISTICS_st));
	if (wlFwGetStaStats(netdev, pStaInfo->StnId, &StaStatsTbl) != SUCCESS) {
		WLDBG_INFO(DBG_LEVEL_7, "cannot get StnId %d stats from fw%d\n", pStaInfo->StnId);
		return;
	}

	pSinfo->tx_retries = StaStatsTbl.dot11RetryCount;
	pSinfo->tx_failed = StaStatsTbl.dot11MPDUCount - StaStatsTbl.dot11SuccessCount - StaStatsTbl.dot11RetryCount;
	...
	if (pSinfo->assoc_req_ies) {
		...
		htCap = (struct IEEEtypes_HT_Element_t *)pSinfo->assoc_req_ies;
		len += sizeof(IEEEtypes_HT_Element_t);

		vhtCap = (struct IEEEtypes_VhtCap_t *)(pSinfo->assoc_req_ies + len);
		len += sizeof(IEEEtypes_VhtCap_t);

		heCap = (struct HE_Capabilities_IE_t *)(pSinfo->assoc_req_ies + len);
		len += sizeof(HE_Capabilities_IE_t);

		memcpy(htCap, &pStaInfo->HtElem, sizeof(IEEEtypes_HT_Element_t));
		memcpy(vhtCap, &pStaInfo->vhtCap, sizeof(IEEEtypes_VhtCap_t));
		memcpy(heCap, &pStaInfo->heCap, sizeof(HE_Capabilities_IE_t));
...
void fillApCapInfo(struct net_device *netdev, int mode, u8 * pApCap, u32 * pLen)
{
	extern UINT16 AddHT_IE(vmacApInfo_t * vmacSta_p, IEEEtypes_HT_Element_t * pNextElement);
	...
	struct wlprivate *wlpriv = NETDEV_PRIV_P(struct wlprivate, netdev);
...
void fillApRadioInfo(struct net_device *netdev, struct ap_radio_basic_capa_rpt_t *pRpt)
{
	...
	memcpy(&pRpt->ruid, &vmacSta_p->VMacEntry.vmacAddr, IEEEtypes_ADDRESS_SIZE);
	...
	if (*(vmacSta_p->Mib802dot11->mib_ApMode) & AP_MODE_G_ONLY) {
		...
		memcpy(&pRpt->operatingClass, &pOpc->op_class_tab, sizeof(op_class_tab_t) * i);
	} else {
		pRpt->num_of_operating_class = pOpc->op_class_nums - i;
		memcpy(&pRpt->operatingClass, &pOpc->op_class_tab[i], sizeof(op_class_tab_t) * (pOpc->op_class_nums - i));
	}
