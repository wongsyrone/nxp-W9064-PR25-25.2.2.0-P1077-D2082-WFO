...
static void wep_decrypt(vmacApInfo_t * vmacSta_p, macmgmtQ_MgmtMsg3_t * MgmtMsg_p);
...
void mlmeAuthError(vmacApInfo_t * vmacSta_p, IEEEtypes_StatusCode_t statusCode, UINT16 arAlg_in, UINT8 * Addr)
{
	...
	if ((txSkb_p =
	     mlmeApiPrepMgtMsg2(IEEE_MSG_AUTHENTICATE, (IEEEtypes_MacAddr_t *) Addr, (IEEEtypes_MacAddr_t *) vmacSta_p->macStaAddr,
...
static UINT32 mlmeAuthValidateMsg(vmacApInfo_t * vmacSta_p, macmgmtQ_MgmtMsg3_t * MgmtMsg_p, BOOLEAN txAuth)
{
	...
	if ((MgmtMsg_p->Body.Auth.AuthTransSeq != ENDIAN_SWAP16(1) &&
	    ...
	    (MgmtMsg_p->Body.Auth.AuthTransSeq == ENDIAN_SWAP16(3) && StaInfo_p->State != EXT_INIT_AUTHENTICATING)) {
		/* Send response message */
		WLDBG_INFO(DBG_LEVEL_8, "mlmeAuthValidateMsg Error  = %x \n", MgmtMsg_p);
		if (txAuth) {
			mlmeAuthError(vmacSta_p, IEEEtypes_STATUS_RX_AUTH_NOSEQ, MgmtMsg_p->Body.Auth.AuthAlg, (UINT8 *) & MgmtMsg_p->Hdr.SrcAddr);
	...
	if (isStrictShareMode(vmacSta_p, bType)) {
		if (!isAuthAlgTypeMatch(vmacSta_p, ENDIAN_SWAP16(MgmtMsg_p->Body.Auth.AuthAlg), bType) ||
		    (ENDIAN_SWAP16(MgmtMsg_p->Body.Auth.AuthAlg) == AUTH_SHARED_KEY && !isWepRequired(vmacSta_p, bType))) {
			/* Send response message */
			...
			if (txAuth) {
				mlmeAuthError(vmacSta_p, IEEEtypes_STATUS_UNSUPPORTED_AUTHALG,
					      MgmtMsg_p->Body.Auth.AuthAlg, (UINT8 *) & MgmtMsg_p->Hdr.SrcAddr);
			}
			return (1);
		}
	}
...
SINT32 mlmeAuthDoOpenSys(vmacApInfo_t * vmacSta_p, AuthRspSrvApMsg * authRspMsg_p)
{
	...
	if ((StaInfo_p = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) authRspMsg_p->rspMac, STADB_UPDATE_AGINGTIME)) == NULL) {
		return (MLME_FAILURE);
	}
	...
	if ((txSkb_p =
	     mlmeApiPrepMgtMsg2(IEEE_MSG_AUTHENTICATE, (IEEEtypes_MacAddr_t *) MgmtMsg_p->Hdr.SrcAddr,
				(IEEEtypes_MacAddr_t *) MgmtMsg_p->Hdr.DestAddr, AUTH_BODY_LEN_WITH_NO_CHAL_TEXT)) == NULL) {
		WLDBG_INFO(DBG_LEVEL_8, "AUTH fail txSkb_p == NULL \n");
		return (MLME_FAILURE);
	}
...
SINT32 mlmeAuthDoSharedKeySeq1(vmacApInfo_t * vmacSta_p, AuthRspSrvApMsg * authRspMsg_p)
{
	...
	if ((txSkb_p =
	     mlmeApiPrepMgtMsg2(IEEE_MSG_AUTHENTICATE, &MgmtMsg_p->Hdr.SrcAddr, &SrcMacAddr,
				(AUTH_BODY_LEN_WITH_NO_CHAL_TEXT + 2 + IEEEtypes_CHALLENGE_TEXT_LEN))) == NULL) {
		WLDBG_INFO(DBG_LEVEL_8, "AUTH fail txSkb_p == NULL \n");
		return (MLME_FAILURE);
	}
...
static UINT8 rc4_byte(struct rc4context *rc4WS)
{
	UINT32 x;
	UINT32 y;
	UINT32 sx, sy;
...
static void wep_decrypt(vmacApInfo_t * vmacSta_p, macmgmtQ_MgmtMsg3_t * MgmtMsg_p)
{
	...
	memcpy(&wepkey[3], &mib->WepDefaultKeys[*(mib->mib_defaultkeyindex)].WepDefaultKeyValue[0], keylength);
