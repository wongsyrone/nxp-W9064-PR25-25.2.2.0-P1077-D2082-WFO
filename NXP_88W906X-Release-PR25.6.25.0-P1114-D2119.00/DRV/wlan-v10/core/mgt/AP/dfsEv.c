...
void dfsTrace(struct net_device *netdev, UINT8 state, UINT8 event, DfsChanInfo * pInfo, UINT32 tag)
{
	...
	if (intf < NUM_OF_WLMACS) {
		...
		if (pInfo) {
			...
			dfsTraceData[intf][dfsTraceLogIdx[intf]].radiomode = pInfo->chanflag.radiomode;
			dfsTraceData[intf][dfsTraceLogIdx[intf]].bw = pInfo->chanflag.ChnlWidth;
			dfsTraceData[intf][dfsTraceLogIdx[intf]].chan = pInfo->channel;
			dfsTraceData[intf][dfsTraceLogIdx[intf]].isDfs = pInfo->chanflag.isDfsChan;
			dfsTraceData[intf][dfsTraceLogIdx[intf]].bw2 = pInfo->chanflag.ChnlWidth2;
			dfsTraceData[intf][dfsTraceLogIdx[intf]].chan2 = pInfo->channel2;
			dfsTraceData[intf][dfsTraceLogIdx[intf]].isDfs2 = pInfo->chanflag.isDfsChan2;
		} else {
...
void dfsTraceLogPrint(UINT8 idx, DfsAp * pdfsApMain)
{
	UINT32 id, i, buffLen;
	UINT8 *pBuff;
	dfsTraceLog_t *temp;
	...
	char eventStr[16][20] = {
		"CHANNEL_CHANGE_EVT", "RADAR_EVT", "CAC_EXPIRY_EVT", "WL_RESET_EVT",
	...
	if (idx < NUM_OF_WLMACS) {
		...
		if (pBuff) {
			...
			memcpy((void *)temp, (void *)&dfsTraceData[idx], sizeof(dfsTraceLog_t) * MAX_DFS_TRACE_LOG);
			for (i = 0; i < MAX_DFS_TRACE_LOG; i++) {
				if ((temp[id].state == 0) && (temp[id].event == 0) && (temp[id].tag == 0)) {
					id++;
				...
				if (temp[id].state < DFS_STATE_MAX) {
					printk("[DFS-%d] ts=0x%08x state=%s event=%s tag=0x%08x\n", id, temp[id].timestamp,
					       stateStr[temp[id].state], eventStr[temp[id].event], temp[id].tag);
				} else {
					printk("[DFS-%d] ts=0x%08x state=0x%x event=0x%x tag=0x%08x\n", id, temp[id].timestamp, temp[id].state,
					       temp[id].event, temp[id].tag);
				}

				if (temp[id].chanInfo) {
					printk("[DFS-%d]     radiomode=%d bw=%d ch=%d isDfs=%d bw2=%d ch2=%d isDfs2=%d\n", id,
					       temp[id].radiomode,
					       temp[id].bw, temp[id].chan, temp[id].isDfs, temp[id].bw2, temp[id].chan2, temp[id].isDfs2);
				}
				printk("\n");
				id++;
				if (id >= MAX_DFS_TRACE_LOG)
...
void DfsInit(struct net_device *pNetDev, struct wlprivate_data *wlpd_p)
{
	...
	if ((pdfsApMain = (DfsAp *) wl_kmalloc(sizeof(DfsAp), GFP_KERNEL)) == NULL) {
...
void verify_chan_bw(MIB_PHY_DSSS_TABLE * PhyDSSSTable)
{
...
#ifdef CONCURRENT_DFS_SUPPORT
void dfs_proc_aux(struct net_device *dev, SCANNER_CTL_EVENT event, DFS_STATE dfs_status, UINT8 IsFromAux)
{
	...
	if (dfsDesc_p->currChanInfo.chanflag.radiomode == RADIO_MODE_80p80) {
		extern int wlRadarDetection(struct net_device *netdev, UINT8 from);
...
#ifdef RADAR_SCANNER_SUPPORT
void dfs_send_iwevcustom_event(struct net_device *dev, char *str)
{
...
void dfs_controller_send_event(struct net_device *dev, SCANNER_CTL_EVENT event, DFS_STATE dfs_status)
{
	...
	sprintf(tmp, "mrvl,w8964,%s,ch,%d,stat,%s,bw,%d", evt_str, PhyDSSSTable->CurrChan, stat_str, PhyDSSSTable->Chanflag.ChnlWidth);
	dfs_send_iwevcustom_event(dev, tmp);
}

DFS_STATUS dfs_sme_channel_switch(struct net_device *dev, UINT16 channel, UINT8 no_cac, UINT8 do_csa)
{
	...
	sme_msg = (smeQ_MgmtMsg_t *) wl_kmalloc(sizeof(smeQ_MgmtMsg_t), GFP_ATOMIC);
	if (!sme_msg) {
	...
	memcpy(&sme_msg->Msg.ChanSwitchCfrm.chInfo.chanflag, &PhyDSSSTable->Chanflag, sizeof(CHNL_FLAGS));
...
DFS_STATE get_dfs_ctl_status(DfsApDesc * dfsDesc_p, UINT8 path)
{
...
void set_dfs_ctl_status(DfsApDesc * dfsDesc_p, UINT8 path, DFS_STATE status)
{
	if (path == DFS_PATH_NORMAL)
...
void add_dfs_ctl_ready_channel(DfsApDesc * dfsDesc_p, UINT16 channel)
{
	dfsDesc_p->CtlReadyCh = channel;
	//printk("[DFS] Ready Ch: %d\n", dfsDesc_p->CtlReadyCh);
}

UINT16 pop_dfs_ctl_ready_channel(DfsApDesc * dfsDesc_p)
{
	UINT16 readyCh = 0;
...
void remove_dfs_ctl_ready_channel(DfsApDesc * dfsDesc_p, UINT16 channel, UINT8 bw)
{
	if (IsTheSameGroup(channel, dfsDesc_p->CtlReadyCh, bw) == DFS_SUCCESS)
		dfsDesc_p->CtlReadyCh = 0;
}

DFS_STATUS dfs_set_aux_ch(struct net_device *dev, UINT16 channel)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
	...
	extern BOOLEAN UpdateCurrentChannelInMIB(vmacApInfo_t * vmacSta_p, UINT32 channel);

	dfsDesc_p = &wlpptr->wlpd_p->pdfsApMain->dfsApDesc;
#if 1
	mib->PhyDSSSTable->Chanflag.radiomode = RADIO_MODE_7x7p1x1;
	mib->PhyDSSSTable->Chanflag.FreqBand = dfsDesc_p->CtlChanInfo.chanflag.FreqBand;
	mib->PhyDSSSTable->Chanflag.ChnlWidth = dfsDesc_p->CtlChanInfo.chanflag.ChnlWidth;
	mib->PhyDSSSTable->Chanflag.FreqBand2 = dfsDesc_p->currChanInfo.chanflag.FreqBand;
	mib->PhyDSSSTable->Chanflag.ChnlWidth2 = dfsDesc_p->currChanInfo.chanflag.ChnlWidth;
	mib->PhyDSSSTable->CurrChan = dfsDesc_p->currChanInfo.channel;
	mib->PhyDSSSTable->SecChan = channel;
#endif
	if (UpdateCurrentChannelInMIB(vmacSta_p, dfsDesc_p->currChanInfo.channel)) {
		mib_Update();

		if (wlchannelSet(dev, channel, dfsDesc_p->currChanInfo.channel, &dfsDesc_p->CtlChanInfo.chanflag, 0)) {
...
DFS_STATUS dfs_set_op_ch(struct net_device * dev, UINT16 channel)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
	...
	if (UpdateCurrentChannelInMIB(vmacSta_p, channel)) {
		...
		if (wlchannelSet(dev, dfsDesc_p->CtlChanInfo.channel, channel, &dfsDesc_p->CtlChanInfo.chanflag, 0)) {
			WLDBG_EXIT_INFO(DBG_LEVEL_15, "setting channel failed");
			return DFS_FAILURE;
		}
	}
...
void dfs_set_channel_switch(struct net_device *dev, UINT16 channel, UINT8 no_cac, UINT8 do_csa, UINT8 IsFromAux)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
	UINT16 set_ch = 0;
	DfsApDesc *dfsDesc_p;
	...
	if (IsFromAux == DFS_PATH_DEDICATED) {
		if (channel == 0) {
			...
			set_ch = DfsDecideNewTargetChannel(dev, dfsDesc_p, FALSE, IsFromAux);
...
void DFSCtlInit(struct net_device *pNetDev)
{
	...
	if (wlpptr->wlpd_p->ext_scnr_en) {
		...
		memcpy(&dfsDesc_p->CtlChanInfo.chanflag, &mib->PhyDSSSTable->Chanflag, sizeof(CHNL_FLAGS));
		memcpy(&dfsDesc_p->currChanInfo.chanflag, &mib->PhyDSSSTable->Chanflag, sizeof(CHNL_FLAGS));

		channel = DfsDecideNewTargetChannel(pNetDev, dfsDesc_p, FALSE, TRUE);
		mib->PhyDSSSTable->SecChan = channel;
...
void concurrent_dfs_proc(struct net_device *dev, SCANNER_CTL_EVENT event, DFS_STATE dfs_status, UINT8 IsFromAux)
									//DfsApDesc *dfsDesc_p)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
	//vmacApInfo_t *vmacSta_p=(vmacApInfo_t *)wlpptr->vmacSta_p;
	//MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
	//MIB_PHY_DSSS_TABLE *PhyDSSSTable = mib->PhyDSSSTable;
	extern void macMgmtMlme_StopAuxRadarDetection(struct net_device *dev, UINT8 detectionMode);
	DfsApDesc *dfsDesc_p;
	UINT8 bw, path;
	UINT16 ch;
	...
	if (dfsDesc_p->CtlChanInfo.chanflag.ChnlWidth != dfsDesc_p->currChanInfo.chanflag.ChnlWidth) {

		// if new bw is larger than old one, the ready channel is not meaningful
		if (dfsDesc_p->CtlChanInfo.chanflag.ChnlWidth != 0
		    && dfsDesc_p->currChanInfo.chanflag.ChnlWidth > dfsDesc_p->CtlChanInfo.chanflag.ChnlWidth) {
			remove_dfs_ctl_ready_channel(dfsDesc_p, dfsDesc_p->CtlReadyCh, dfsDesc_p->currChanInfo.chanflag.ChnlWidth);
		}
		memcpy(&dfsDesc_p->CtlChanInfo.chanflag, &dfsDesc_p->currChanInfo.chanflag, sizeof(CHNL_FLAGS));
	...
	case ScnrCtl_Radar_Detected:
		...
		if (path == DFS_PATH_DEDICATED) {
			...
			// Stop monitor first 
			macMgmtMlme_StopAuxRadarDetection(dev, DR_SCANNER_SERVICE_STOP);
			...
			if (dfsDesc_p->EMCACState == DFS_STATE_EM_SCAN || dfsDesc_p->EMCACState == DFS_STATE_EM_SCAN_ALL) {
				EM_update_cac_status(wlpptr->vmacSta_p, ch, 1 /*radar channel */ , 1 /*indecation */ );
			} else {
				dfs_set_channel_switch(dev, 0, 0, 0, DFS_PATH_DEDICATED);
			}
			...
			if ((get_dfs_ctl_status(dfsDesc_p, DFS_PATH_DEDICATED) == DFS_STATE_OPERATIONAL) &&
			    (!IsTheSameGroup(dfsDesc_p->currChanInfo.channel, dfsDesc_p->CtlChanInfo.channel, bw))) {
				UINT16 readyCh = 0;
				if ((readyCh = pop_dfs_ctl_ready_channel(dfsDesc_p)) != 0) {
					dfs_set_channel_switch(dev, readyCh, 1, 0, DFS_PATH_NORMAL);
					dfs_set_channel_switch(dev, 0, 0, 0, DFS_PATH_DEDICATED);
				} else {	// OK channel is empty 
					dfs_set_channel_switch(dev, 0, 0, 1, DFS_PATH_NORMAL);
					dfs_set_channel_switch(dev, 0, 0, 0, DFS_PATH_DEDICATED);
				}
			} else {	// path A scanning, cac not done
				if (get_dfs_ctl_status(dfsDesc_p, DFS_PATH_NORMAL) == DFS_STATE_OPERATIONAL)
					dfs_set_channel_switch(dev, 0, 0, 1, DFS_PATH_NORMAL);
				else
					dfs_set_channel_switch(dev, 0, 0, 0, DFS_PATH_NORMAL);
			}
		}
