...
extern Status_e NotifyNewStn(IEEEtypes_MacAddr_t * MacAddr, UINT16 StnId)
{
	return FAIL;
};
...
#endif
extern int wlFwSetNewStn(struct net_device *dev, u_int8_t * staaddr, u_int16_t assocId, u_int16_t stnId, u_int16_t action, PeerInfo_t * pPeerInfo,
...
extern void setStaPeerInfoApMode(struct wlprivate *wlpptr, extStaDb_StaInfo_t * pStaInfo, PeerInfo_t * pPeerInfo, UINT8 ApMode,
				 struct wds_port *pWdsPort);
#ifdef MBSS
extern vmacApInfo_t *vmacGetMBssByAddr(vmacApInfo_t * vmacSta_p, UINT8 * macAddr_p);
#endif
#ifdef CONFIG_IEEE80211W
extern int validateRobustManagementframe(vmacApInfo_t * vmac_p, extStaDb_StaInfo_t * StaInfo_p, macmgmtQ_MgmtMsg3_t * mgtFrm);
#endif
...
extern extStaDb_StaInfo_t *macMgtStaDbInit(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * staMacAddr, IEEEtypes_MacAddr_t * apMacAddr)
{
...
#endif
	{
		...
		wl_kfree(StaInfo);
		StaInfo = NULL;
		if ((StaInfo_p = extStaDb_GetStaInfo(vmacSta_p, staMacAddr, STADB_UPDATE_AGINGTIME)) == NULL) {
			return NULL;
		}
...
#endif
{
...
#endif
	{
		WLDBG_ENTER_INFO(DBG_LEVEL_11, "mgt frame %d rxved %2x-%2x-%2x-%2x-%2x-%2x\n", MgmtMsg_p->Hdr.FrmCtl.Subtype,
				 ...
				 MgmtMsg_p->Hdr.DestAddr[2], MgmtMsg_p->Hdr.DestAddr[3], MgmtMsg_p->Hdr.DestAddr[4], MgmtMsg_p->Hdr.DestAddr[5]);
		return 1;
	}
	...
	if ((MgmtMsg_p->Hdr.FrmCtl.Subtype != IEEE_MSG_AUTHENTICATE) && (isMacAccessList(vmacSta_p, &(MgmtMsg_p->Hdr.SrcAddr)) != SUCCESS)) {
		return 1;
	}
	...
	if ((StaInfo_p = extStaDb_GetStaInfo(vmacSta_p, &MgmtMsg_p->Hdr.SrcAddr, STADB_DONT_UPDATE_AGINGTIME)) == NULL) {
		if (MgmtMsg_p->Hdr.FrmCtl.Subtype == IEEE_MSG_AUTHENTICATE) {
			...
			//added call to check other band's StaInfo_p
			for (i = 0; i < MAX_CARDS_SUPPORT; i++) {
				if (i == cardindex)	/* Ignore the same band */
					continue;
				if (global_private_data[i]->rootdev == NULL)
					continue;
				wlpptr = NETDEV_PRIV_P(struct wlprivate, global_private_data[i]->rootdev);
				if (wlpptr == NULL)
					continue;
				if ((StaInfo_p = extStaDb_GetStaInfo(wlpptr->vmacSta_p, &MgmtMsg_p->Hdr.SrcAddr, STADB_SKIP_MATCH_VAP))) {
					macMgmtRemoveSta(wlpptr->vmacSta_p, StaInfo_p);
				}
			}

			//added call to check other VAP's StaInfo_p
			if ((StaInfo_p = extStaDb_GetStaInfo(vmacSta_p, &MgmtMsg_p->Hdr.SrcAddr, STADB_SKIP_MATCH_VAP))) {
				vmacApInfo_t *vmacSta_temp = vmacSta_p;
#ifdef MBSS
				//StaInfo_p might locat at other VAP, need to find the correct VAP before del the STA
				if (memcmp(&vmacSta_p->macBssId, &StaInfo_p->Bssid, sizeof(IEEEtypes_MacAddr_t))) {
					//StaInfo_p and vmacSta_p current NOT point to the same vap
					if ((vmacSta_temp = vmacGetMBssByAddr(vmacSta_p, StaInfo_p->Bssid)) == NULL) {
		...
		{
			if ((MgmtMsg_p->Hdr.FrmCtl.Subtype != IEEE_MSG_DEAUTHENTICATE) && (MgmtMsg_p->Hdr.FrmCtl.Subtype != IEEE_MSG_PROBE_RSP))
			{
				if (is_unicast_ether_addr((UINT8 *) & MgmtMsg_p->Hdr.SrcAddr)) {
#ifdef SOC_W906X
					macMgmtMlme_SendDeauthenticateMsg(vmacSta_p, &MgmtMsg_p->Hdr.SrcAddr,
									  0, IEEEtypes_REASON_CLASS2_NONAUTH, TRUE);
#else
					macMgmtMlme_SendDeauthenticateMsg(vmacSta_p, &MgmtMsg_p->Hdr.SrcAddr, 0, IEEEtypes_REASON_CLASS2_NONAUTH);
#endif
		...
		if (MgmtMsg_p->Hdr.FrmCtl.Subtype != IEEE_MSG_ACTION_NO_ACK) {
			...
			if ((!vmacSta_p->ieee80211w) && (StaInfo_p->mgmt_seqNum == (MgmtMsg_p->Hdr.SeqCtl >> 4)))
				return 1;
		}
...
#ifdef CCK_DESENSE
			cck_desense_ctrl(vmacSta_p->master->dev, CCK_DES_AUTH_REQ);
#endif				/* CCK_DESENSE */
...
#ifdef CONFIG_IEEE80211W
			if (MgmtMsg_p->Body.Action.Category == WLAN_ACTION_SA_QUERY) {
				extern void macMgmtMlme_SAQuery_Rsp(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * Addr,
								    IEEEtypes_MacAddr_t * SrcAddr, UINT8 * trans_id, UINT32 stamode);
				...
				case WLAN_SA_QUERY_REQUEST:
					WLDBG_INFO(DBG_LEVEL_0, "IEEE_MSG_QOS_ACTION::: Rx WLAN_SA_QUERY_REQUEST trans_id=%02x-%02x\n",
						   SARsp_p->trans_id[0], SARsp_p->trans_id[1]);
					macMgmtMlme_SAQuery_Rsp(vmacSta_p, &(MgmtMsg_p->Hdr.SrcAddr), &(MgmtMsg_p->Hdr.DestAddr), SARsp_p->trans_id,
								0);
					break;
				case WLAN_SA_QUERY_RESPONSE:
					WLDBG_INFO(DBG_LEVEL_0, "IEEE_MSG_QOS_ACTION::: Rx WLAN_SA_QUERY_RESPONSE trans_id=%02x-%02x\n",
						   SARsp_p->trans_id[0], SARsp_p->trans_id[1]);
...
#endif
#ifdef IEEE80211K
			if (TRUE == macMgmtMlme_RrmAct(vmacSta_p, (macmgmtQ_MgmtMsg3_t *) MgmtMsg_p))
				break;

			if (TRUE == macMgmtMlme_WNMAct(vmacSta_p, (macmgmtQ_MgmtMsg3_t *) MgmtMsg_p))
				break;
#endif
#ifdef QOS_FEATURE
	...
	case IEEE_MSG_ACTION_NO_ACK:
		{
			...
			extern int wlDspTrigMu(struct net_device *netdev, UINT8 index, UINT8 priority, U8 * msg, int len);
			macmgmtQ_MgmtActionNoAck_t *MgmtActionNoAck_p = (macmgmtQ_MgmtActionNoAck_t *) MgmtMsg_p;
#ifdef DSP_TRIG_CMD
			U8 i;
			U32 tempNc;
			U8 *helpPtr;
			muset_t *pMUset;
			U8 muGID;
			IEEEtypes_HE_MimoControl_t *HEMimoCtrl_p = &(MgmtActionNoAck_p->Body.CbfHEReport.Mimo);
#endif
			IEEEtypes_VHT_MimoControl_t *MimoCtrl_p = &(MgmtActionNoAck_p->Body.CbfReport.Mimo);
			...
			case VHT_CATEGORY:
				switch (MgmtActionNoAck_p->Body.CbfReport.Action) {
				case 0:
					if (MimoCtrl_p->FbType == 1) {	//MU
						...
						for (i = 0; i < pMUset->cnt; i++) {
							if (!memcmp(pMUset->StaInfo[i]->Addr, MgmtActionNoAck_p->Hdr.SrcAddr, 6)) {
								muGID = StaInfo_p->mu_index + 1;
								muBitmap[muGID] = muBitmap[muGID] | (1 << i);
								tempNc = MgmtActionNoAck_p->Body.CbfReport.Mimo.NcIdx + 1;
								helpPtr = (U8 *) MgmtActionNoAck_p->Hdr.SrcAddr;
								//printk("User: %d, Nc:%d, MAC: %02x:%02x:%02x:%02x:%02x:%02x\n",i,tempNc,helpPtr[0],helpPtr[1],helpPtr[2],helpPtr[3],helpPtr[4],helpPtr[5]);                                                                           
							}
						}

						if (pMUset->cnt == (w81_count_num_ones(muBitmap[muGID]) & 0xff)) {
							muConfig.Num_Users = pMUset->cnt;
							...
							wlDspTrigMu(vmacSta_p->dev, 0xff, 0, (U8 *) & muConfig, sizeof(mu_config_t));
							muBitmap[muGID] = 0;
						}
#endif
					} else {	//SU
						if ((!StaInfo_p->nxp_sta) && (MimoCtrl_p->NcIdx == 1)) {	/* 2SS */
							if ((MgmtActionNoAck_p->Body.CbfReport.report.snr[0] == StaInfo_p->cbf_stat.pre_snr) &&
							    (MgmtActionNoAck_p->Body.CbfReport.report.snr[1] == StaInfo_p->cbf_stat.pre_snr1))
								StaInfo_p->cbf_stat.snr_unchange_cnt++;
							else
								StaInfo_p->cbf_stat.snr_unchange_cnt = 0;

							if (StaInfo_p->cbf_stat.snr_unchange_cnt >= 10)
								wlFwSetEbfMaxSs(vmacSta_p->dev, (u_int8_t *) & MgmtMsg_p->Hdr.SrcAddr,
										StaInfo_p->vht_peer_RxNss);

							StaInfo_p->cbf_stat.pre_snr = MgmtActionNoAck_p->Body.CbfReport.report.snr[0];
							StaInfo_p->cbf_stat.pre_snr1 = MgmtActionNoAck_p->Body.CbfReport.report.snr[1];
						}
					}
					break;
...
#ifdef DSP_TRIG_CMD
			case HE_CATEGORY:
				switch (MgmtActionNoAck_p->Body.CbfHEReport.Action) {
				case 0:
...
u32 w81_count_num_ones(u32 bitmap)
{
	u32 num_ones = 0;
	while (bitmap) {
		num_ones++;
	...
	case SME_CMD_DISASSOCIATE:
		{
			...
			if ((StaInfo_p = extStaDb_GetStaInfo(vmacSta_p, &smeCmd_p->Body.AssocCmd.PeerStaAddr, STADB_UPDATE_AGINGTIME)) == NULL) {
				return 1;
			}
	...
	case SME_CMD_MREQUEST:
		{
			...
			if (!IS_BROADCAST(&smeCmd_p->Body.MrequestCmd.PeerStaAddr)) {
				if ((StaInfo_p = extStaDb_GetStaInfo(vmacSta_p, &smeCmd_p->Body.MrequestCmd.PeerStaAddr,
					...
					WLDBG_INFO(DBG_LEVEL_11, "evtSmeCmdMsg: SME_CMD_MREQUEST - no station found %x:%x:%x:%x:%x:%x] \n",
						   ...
						   smeCmd_p->Body.MrequestCmd.PeerStaAddr[4], smeCmd_p->Body.MrequestCmd.PeerStaAddr[5]);
					return 1;
				}
			}
	...
	case SME_CMD_MREPORT:
		{
			...
			if ((StaInfo_p = extStaDb_GetStaInfo(vmacSta_p, &smeCmd_p->Body.MrequestCmd.PeerStaAddr, STADB_UPDATE_AGINGTIME)) == NULL) {
