...
#ifdef SOC_W906X
	if ((skb = ieee80211_getmgtframe_undefine_len(&frm, sizeof(struct ieee80211_frame) + 1024)) != NULL)
#else
...
UINT8 setClientPeerInfo(vmacEntry_t * vmacEntry_p, dot11MgtFrame_t * MgmtMsg_p, PeerInfo_t * pPeerInfo, extStaDb_StaInfo_t * StaInfo_p,
...
#ifdef SOC_W906X
int get_he_peer_nss(he_mcs_nss_support_t * mcs_nss_set_remote_p)
{
	int nss;
...
void InitClientInfo(UINT8 * macAddr_p, dot11MgtFrame_t * MgmtMsg_p, vmacEntry_t * clientVMacEntry_p, BOOLEAN isApMrvl, BOOLEAN isConnectbBSS)
#else
...
#endif
{
	...
	if ((StaInfoRmAp_p = extStaDb_GetStaInfo(wlpptrSta->vmacSta_p, (IEEEtypes_MacAddr_t *) macAddr_p, STADB_UPDATE_AGINGTIME)) == NULL) {
		if ((StaInfo_p = wl_kmalloc(sizeof(extStaDb_StaInfo_t), GFP_ATOMIC)) == NULL) {
			printk("%s: Fail to allocate memory\n", __FUNCTION__);
			return;
		}

		memset(StaInfo_p, 0, sizeof(extStaDb_StaInfo_t));
		memcpy(&StaInfo_p->Addr, macAddr_p, sizeof(IEEEtypes_MacAddr_t));
	...
	} else if ((*(wlpptrSta->vmacSta_p->Mib802dot11->pMib_11nAggrMode) & WL_MODE_AMSDU_TX_MASK) == WL_MODE_AMSDU_TX_4K) {
	...
	wlFwSetNewStn(pStaDev, macAddr_p, StaInfo_p->Aid, StaInfo_p->StnId,
		      StaInfoDbActionModifyEntry, pPeerInfo, QosInfo, StaInfo_p->IsStaQSTA, wds);
	...
	if (StaInfo_p->StnId < sta_num) {
		if (wlpptrSta->wlpd_p->txRateHistogram[StaInfo_p->StnId] == NULL) {
			wlpptrSta->wlpd_p->txRateHistogram[StaInfo_p->StnId] = (WLAN_TX_RATE_HIST *) wl_kmalloc_autogfp(sizeof(WLAN_TX_RATE_HIST));
		}
		if (wlpptrSta->wlpd_p->txRateHistogram[StaInfo_p->StnId]) {
			memset(wlpptrSta->wlpd_p->txRateHistogram[StaInfo_p->StnId], 0, sizeof(WLAN_TX_RATE_HIST));
		...
		if (wlpptrSta->wlpd_p->scheHistogram[StaInfo_p->StnId] != NULL)
			memset(wlpptrSta->wlpd_p->scheHistogram[StaInfo_p->StnId], 0, sizeof(WLAN_SCHEDULER_HIST));
		else {
			if ((sch_histo_p = (WLAN_SCHEDULER_HIST *) wl_kmalloc_autogfp(sizeof(WLAN_SCHEDULER_HIST))) != NULL) {
				...
				wlpptrSta->wlpd_p->scheHistogram[StaInfo_p->StnId] = sch_histo_p;
			} else
...
#endif
{
...
#ifdef MULTI_AP_SUPPORT
	if (isConnectbBSS)
		wlFwSetNewStn(pStaDev, (u_int8_t *) macAddr_p, parentApAid, parentApStnId, 2, pPeerInfo, QosInfo, StaInfo_p->IsStaQSTA, 4);
	else
#endif
		wlFwSetNewStn(pStaDev, (u_int8_t *) macAddr_p, parentApAid, parentApStnId, 2, pPeerInfo, QosInfo, StaInfo_p->IsStaQSTA, 0);
#ifdef V6FW
#ifdef MULTI_AP_SUPPORT
	if (isConnectbBSS)
		wlFwSetNewStn(pStaDev, (u_int8_t *) MgmtMsg_p->Hdr.DestAddr, ClientAid, ClientStnId, 2, pPeerInfo, QosInfo,
...
#ifdef MULTI_AP_SUPPORT
	if (isConnectbBSS)
		wlFwSetNewStn(pStaDev, (u_int8_t *) macAddr_p, parentApAid, parentApStnId, 0, pPeerInfo, QosInfo, StaInfo_p->IsStaQSTA, 4);
	else
#endif
		wlFwSetNewStn(pStaDev, (u_int8_t *) macAddr_p, parentApAid, parentApStnId, 0, pPeerInfo, QosInfo, StaInfo_p->IsStaQSTA, 0);
#ifdef V6FW
#ifdef MULTI_AP_SUPPORT
	if (isConnectbBSS)
	...
	if (parentApStnId) {
		if ((txrate_histo_p = (WLAN_TX_RATE_HIST *) wl_kmalloc_autogfp(sizeof(WLAN_TX_RATE_HIST))) != NULL) {
			...
			if (wlpptrSta->wlpd_p->txRateHistogram[parentApStnId - 1] != NULL) {
				wl_kfree(wlpptrSta->wlpd_p->txRateHistogram[parentApStnId - 1]);
				wlpptrSta->wlpd_p->txRateHistogram[parentApStnId - 1] = NULL;
			}
			wlpptrSta->wlpd_p->txRateHistogram[parentApStnId - 1] = txrate_histo_p;
...
void RemoveRemoteAPFw(UINT8 * apMacAddr_p, vmacEntry_t * clientVMacEntry_p)
{
	...
	if ((StaInfo_p = extStaDb_GetStaInfo(wlpptrSta->vmacSta_p, (IEEEtypes_MacAddr_t *) apMacAddr_p, STADB_UPDATE_AGINGTIME)) != NULL) {
		extStaDb_DelSta(wlpptrSta->vmacSta_p, (IEEEtypes_MacAddr_t *) apMacAddr_p, STADB_DONT_UPDATE_AGINGTIME);
		wlFwSetNewStn(wlpptrSta->master, (u_int8_t *) apMacAddr_p, StaInfo_p->Aid, StaInfo_p->StnId,
...
UINT8 setClientPeerInfo(vmacEntry_t * vmacEntry_p,
			dot11MgtFrame_t * MgmtMsg_p, PeerInfo_t * pPeerInfo, extStaDb_StaInfo_t * StaInfo_p, UINT8 * QosInfo_p, MIB_802DOT11 * mib)
{
	UINT8 *attrib_p;
	...
	if (vmacSta_p->VMacEntry.modeOfService == VMAC_MODE_CLNT_INFRA) {
		...
		if ((vmacSta_p->RsnIESetByHost == 1) && (vmacSta_p->RsnIE)) {
			parsing_rsn_ie((UINT8 *) (vmacSta_p->RsnIE), &RsnIEWPA2_local, &mfpc_local, &mfpr_local);
			printk("%s: Local RSN mfpc=%d mfpr=%d\n", __func__, mfpc_local, mfpr_local);
		}

		/* If Local support PMF, check Peer PMF capa */
		if ((mfpc_local + mfpr_local) >= 1) {
			if (vStaInfo_p) {
				parsing_rsn_ie((UINT8 *) & (vStaInfo_p->bssDescProfile_p->Wpa2Element), &RsnIEWPA2, &mfpc, &mfpr);
				if ((mfpc + mfpr) >= 1)
					StaInfo_p->Ieee80211wSta = 1;
				printk("%s: Peer RSN mfpc=%d mfpr=%d\n", __func__, mfpc, mfpr);
			}
		}
		printk("%s StaInfo_p->Ieee80211wSta=%d\n", __func__, StaInfo_p->Ieee80211wSta);
	}
#endif
#endif
	...
	if ((pVhtCap = (IEEEtypes_VhtCap_t *) syncSrv_ParseAttribWithinFrame(MgmtMsg_p, attrib_p, VHT_CAP))) {
		...
		printk("VHT AP: vht_cap=%x, vht_RxMcs=%x, vht_TxMcs=%x\n",
		       (u_int32_t) vhtcap, (u_int32_t) pVhtCap->SupportedRxMcsSet, (u_int32_t) pVhtCap->SupportedTxMcsSet);
		...
		memcpy((UINT8 *) & StaInfo_p->vhtCap.cap, (UINT8 *) & pVhtCap->cap, sizeof(IEEEtypes_VHT_Cap_Info_t));
		...
		if ((pVhtOp = (IEEEtypes_VhOpt_t *) syncSrv_ParseAttribWithinFrame(MgmtMsg_p, attrib_p, VHT_OPERATION))) {
			...
			case 1:
				...
				if (1 ==
				    (ret =
				     isSupport160MhzByCenterFreq(priv, VHT_EXTENDED_NSS_BW_CAPABLE, pVhtOp->center_freq0, pVhtOp->center_freq1,
								 (pHTAdd) ? pHTAdd->OpMode.center_freq2 : 0))) {
				...
				if (pVhtOp->center_freq1 == 0) {
					...
					UINT8 diff;
					if (pVhtOp->center_freq1 > pVhtOp->center_freq0) {
						diff = pVhtOp->center_freq1 - pVhtOp->center_freq0;
					} else {
						diff = pVhtOp->center_freq0 - pVhtOp->center_freq1;
					}
					if (diff == 8) {
						printk
			...
			if (mib->PhyDSSSTable->Chanflag.FreqBand == FREQ_BAND_2DOT4GHZ) {
				...
				if ((pVhtCap->cap.SupportedChannelWidthSet == 1) || (pVhtCap->cap.SupportedChannelWidthSet == 2))
					pPeerInfo->vht_RxChannelWidth = 3;
				else
					pPeerInfo->vht_RxChannelWidth = 2;
			}
...
#ifdef SOC_W906X
	while ((pIeExt = (IEEEtypes_InfoElementExtHdr_t *) syncSrv_ParseAttribWithinFrame(MgmtMsg_p, attrib_p, EXT_IE)) != NULL) {

		switch (pIeExt->ext) {
		case HE_CAPABILITIES_IE:
			memcpy((void *)&pPeerInfo->he_cap, (void *)pIeExt, pIeExt->Len + 2);
			prx_he_mcs_80m = (he_mcs_nss_support_t *) ((void *)&vmacSta_p->he_cap + sizeof(IEEEtypes_InfoElementExtHdr_t)
			...
			if ((vmacSta_p->he_cap.phy_cap.channel_width_set & HE_SUPPORT_160MHZ_BW_5G) &&
			    (pPeerInfo->he_cap.phy_cap.channel_width_set & HE_SUPPORT_160MHZ_BW_5G)) {

				if ((&vmacSta_p->he_cap.rx_he_mcs_80m + 2)->max_mcs_set > (&pPeerInfo->he_cap.tx_he_mcs_80m + 2)->max_mcs_set) {
					(&pPeerInfo->he_cap.tx_he_mcs_80m + 2)->max_mcs_set = (prx_he_mcs_80m + 2)->max_mcs_set;
				}

				if ((vmacSta_p->he_cap.phy_cap.channel_width_set & HE_SUPPORT_80P80MHZ_BW_5G) &&
				    (pPeerInfo->he_cap.phy_cap.channel_width_set & HE_SUPPORT_80P80MHZ_BW_5G)) {

					if ((&vmacSta_p->he_cap.rx_he_mcs_80m + 4)->max_mcs_set > (&pPeerInfo->he_cap.tx_he_mcs_80m + 4)->max_mcs_set) {
						(&pPeerInfo->he_cap.tx_he_mcs_80m + 4)->max_mcs_set = (prx_he_mcs_80m + 4)->max_mcs_set;
					}

					if (pPeerInfo->vht_RxChannelWidth == 3) {
						StaInfo_p->operating_mode.rxnss = get_he_peer_nss(&pPeerInfo->he_cap.rx_he_mcs_80m + 4);
						StaInfo_p->operating_mode.tx_nsts = get_he_peer_nss(&pPeerInfo->he_cap.tx_he_mcs_80m + 4);
					} else {
						StaInfo_p->operating_mode.rxnss = get_he_peer_nss(&pPeerInfo->he_cap.rx_he_mcs_80m);
						StaInfo_p->operating_mode.tx_nsts = get_he_peer_nss(&pPeerInfo->he_cap.tx_he_mcs_80m);
					}
				} else {
					if (pPeerInfo->vht_RxChannelWidth == 3) {
						StaInfo_p->operating_mode.rxnss = get_he_peer_nss(&pPeerInfo->he_cap.rx_he_mcs_80m + 2);
						StaInfo_p->operating_mode.tx_nsts = get_he_peer_nss(&pPeerInfo->he_cap.tx_he_mcs_80m + 2);
					} else {
						StaInfo_p->operating_mode.rxnss = get_he_peer_nss(&pPeerInfo->he_cap.rx_he_mcs_80m);
						StaInfo_p->operating_mode.tx_nsts = get_he_peer_nss(&pPeerInfo->he_cap.tx_he_mcs_80m);
					}
				...
				if (mib->PhyDSSSTable->Chanflag.FreqBand == FREQ_BAND_5GHZ) {
					if (!(pPeerInfo->he_cap.phy_cap.channel_width_set & HE_SUPPORT_40_80MHZ_BW_5G))
						pPeerInfo->vht_RxChannelWidth = 0;
				}
			}

			StaInfo_p->operating_mode.chbw = pPeerInfo->vht_RxChannelWidth;
			StaInfo_p->operating_mode.ulmu_disable = 0;
			break;
		case HE_OPERATION_IE:
			memcpy((void *)&pPeerInfo->he_op, (void *)pIeExt, pIeExt->Len + 2);
			break;
			...
			if (WME_param_elem->OUI.Type == 2) {
				...
				if (WME_param_elem->OUI.Subtype == 1) {
					...
					for (i = 0; i < 4; i++) {
						...
						if (*(pMasterMIB->mib_optlevel) && i == 0) {
							wlFwSetEdcaParam(dev,
									 ...
									 WME_param_elem->AC_VI.ACI_AIFSN.AIFSN, WME_param_elem->AC_VI.TXOP_lim);
						} else {
							wlFwSetEdcaParam(dev,
									 i,
									 ((0x01 << ac_Param_p->ECW_min_max.ECW_min) - 1),
									 ((0x01 << ac_Param_p->ECW_min_max.ECW_max) - 1),
									 ac_Param_p->ACI_AIFSN.AIFSN, ac_Param_p->TXOP_lim);
						}
#ifndef SOC_W906X
						if (ac_Param_p->ACI_AIFSN.ACM == 1 && *QosInfo_p == 0) {	/* This function will be called twice. Check QosInfo_p to avoid running the code segment below twice */
							switch (ac_Param_p->ACI_AIFSN.ACI) {
							case AC_BE_Q:
								QosTsData[0].TidStatus = QOS_TS_PENDING;
								QosTsData[3].TidStatus = QOS_TS_PENDING;
								break;
							case AC_VI_Q:
								QosTsData[4].TidStatus = QOS_TS_PENDING;
								QosTsData[5].TidStatus = QOS_TS_PENDING;
								break;
							case AC_VO_Q:
								QosTsData[6].TidStatus = QOS_TS_PENDING;
								QosTsData[7].TidStatus = QOS_TS_PENDING;
								break;
							case AC_BK_Q:
							default:
								break;
					...
					for (i = 0; i < 8 && *QosInfo_p == 0; i++) {	/* This function will be called twice. Check QosInfo_p to avoid running the code segment below twice *//* Downgrade AC and TID */
						if (QosTsData[i].TidStatus != QOS_TS_ADMITTED) {
							switch (i) {
								/* BE */
							case 0:
								QosTsData[i].TidDowngrade = 1;
								QosTsData[i].AccCategoryQDowngrade = AC_BK_Q;
								break;
							case 3:
								QosTsData[i].TidDowngrade = 2;
								QosTsData[i].AccCategoryQDowngrade = AC_BK_Q;
								break;
							case 4:
								QosTsData[i].TidDowngrade = QosTsData[0].TidDowngrade;
								QosTsData[i].AccCategoryQDowngrade = QosTsData[0].AccCategoryQDowngrade;
								break;
							case 5:
								QosTsData[i].TidDowngrade = QosTsData[3].TidDowngrade;
								QosTsData[i].AccCategoryQDowngrade = QosTsData[3].AccCategoryQDowngrade;
								break;
							case 6:
								QosTsData[i].TidDowngrade = QosTsData[4].TidDowngrade;
								QosTsData[i].AccCategoryQDowngrade = QosTsData[4].AccCategoryQDowngrade;
								break;
							case 7:
								QosTsData[i].TidDowngrade = QosTsData[5].TidDowngrade;
								QosTsData[i].AccCategoryQDowngrade = QosTsData[5].AccCategoryQDowngrade;
								break;
							default:
								break;
							}
...
void DeleteClientInfo(UINT8 * macAddr_p, vmacEntry_t * clientVMacEntry_p)
{
	...
	if ((StaInfo_p = extStaDb_GetStaInfo(wlpptrSta->vmacSta_p, (IEEEtypes_MacAddr_t *) macAddr_p, STADB_UPDATE_AGINGTIME)) != NULL) {
		FreeAid(wlpptrSta->vmacSta_p, StaInfo_p->Aid);
		ResetAid(wlpptrSta->vmacSta_p, StaInfo_p->StnId, StaInfo_p->Aid);
		FreeStnId(wlpptrSta->vmacSta_p, StaInfo_p->StnId);
		extStaDb_DelSta(wlpptrSta->vmacSta_p, (IEEEtypes_MacAddr_t *) macAddr_p, STADB_DONT_UPDATE_AGINGTIME);
		wlFwSetNewStn(wlpptrSta->master, (u_int8_t *) macAddr_p, StaInfo_p->Aid, StaInfo_p->StnId, StaInfoDbActionRemoveEntry, NULL, 0, 0, 0);
#ifndef SOC_W906X
		//only remove local address, when there is a BSSID entry.
		extStaDb_DelSta(wlpptrSta->vmacSta_p, (IEEEtypes_MacAddr_t *) clientVMacEntry_p->vmacAddr, STADB_DONT_UPDATE_AGINGTIME);
#endif
	}
}

int wlset_rateSupport(MIB_802DOT11 * mib)
{
	...
	} else if ((*(mib->mib_ApMode) & (AP_MODE_G_ONLY | AP_MODE_B_ONLY)) == (AP_MODE_G_ONLY | AP_MODE_B_ONLY)) {
...
void RemoveClientFw(UINT8 * macAddr_p, vmacEntry_t * clientVMacEntry_p)
{
	...
	if ((StaInfo_p = extStaDb_GetStaInfo(wlpptrSta->vmacSta_p, (IEEEtypes_MacAddr_t *) macAddr_p, STADB_UPDATE_AGINGTIME)) != NULL) {
		...
		wlFwSetNewStn(wlpptrSta->master, (u_int8_t *) macAddr_p, 0, 0, StaInfoDbActionRemoveEntry, NULL, 0, 0, 0);
	}
}
...
int isSupport160MhzByCenterFreq(struct wlprivate *wlpptr, UINT8 vhtExtNssBwCap, UINT8 freq0, UINT8 freq1, UINT8 freq2)
{
