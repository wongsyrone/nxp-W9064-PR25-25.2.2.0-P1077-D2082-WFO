 ...
 *=============================================================================
 */
void _map_hex_dump(const char *title, UINT8 * buf, UINT32 len)
{
	int i;
	printk("%s - %s - hexdump(len=%lu):\n", "[1905]", title, (unsigned long)len);
	if (buf == NULL) {
		printk(" [NULL]");
	} else {
		for (i = 0; i < len; i++) {
...
static void _map_add_token(UINT8 token)
{
	UINT8 index;

	for (index = 0; index < 16 && MAP_BcnReport_Token[index] != 0; index++) ;
...
static UINT8 _map_del_token(UINT8 token)
{
	UINT8 i, j;

	for (i = 0; i < 16 && MAP_BcnReport_Token[i] != token; i++) ;
	if (i < 16) {		// full token
		for (j = i; j < 16; j++) {
			if (j == 15) {
				MAP_BcnReport_Token[j] = 0;
			} else {
				MAP_BcnReport_Token[j] = MAP_BcnReport_Token[j + 1];
			}
...
void MAP_tlv_AP_metrics_process(IEEEtypes_MacAddr_t * addr, vmacApInfo_t * vmac_p, unsigned char *buf, UINT16 * buf_len)
{
	...
	if (IW_CUSTOM_MAX - *buf_len < sizeof(struct MultiAP_AP_LM_Resp_Element_t) + (sizeof(struct IEEEtypes_ESP_info_field_t) * 4)) {
	...
	len = sizeof(struct MultiAP_AP_LM_Resp_Element_t) + (sizeof(struct IEEEtypes_ESP_info_field_t) * 4) - sizeof(struct MultiAP_TLV_Element_t);
...
void MAP_tlv_STA_traffic_stats_process(vmacApInfo_t * vmac_p, unsigned char *buf, UINT16 * buf_len)
{
	...
	if (IW_CUSTOM_MAX - *buf_len < sizeof(struct MultiAP_STA_TS_Resp_Element_t) + sizeof(struct MultiAP_STA_TS_Value_t)) {
		printk("MAP_TLV_AP_LM_RESP not enough buffer size!!!\n");
		return;
	}
	...
	while (Curr_p != NULL) {
		...
		if ((StaInfo_p = extStaDb_GetStaInfo(vmac_p, &(Item_p->StaInfo.Addr), STADB_NO_BLOCK))) {
			if (StaInfo_p->State == ASSOCIATED) {
				...
				memcpy(value->mac_addr, StaInfo_p->Addr, sizeof(IEEEtypes_MacAddr_t));
				value->BytesSent = 0;
...
void MAP_tlv_STA_metrics_process(IEEEtypes_MacAddr_t * addr, vmacApInfo_t * vmac_p, unsigned char *buf, UINT16 * buf_len)
{
	...
	while (Curr_p != NULL) {
		...
		if ((StaInfo_p = extStaDb_GetStaInfo(vmac_p, &(Item_p->StaInfo.Addr), STADB_NO_BLOCK))) {
			if (StaInfo_p->State == ASSOCIATED) {
				...
				memcpy(report->bssid, addr, sizeof(IEEEtypes_MacAddr_t));
				report->delta_ms = 100;
				report->EST_downlink = 1;
				report->EST_uplink = 1;
				report->rssi_uplink = 10;
				len += (sizeof(struct MultiAP_STA_LM_Value_t) + sizeof(struct MultiAP_STA_LM_Report_t));
 ...
 *
 */
void MAP_tlv_Query_process(vmacApInfo_t * vmac_p, MultiAP_TLV_Element_t * map_tlv)
{
	...
	case MAP_TLV_AP_LM_QUERY:
		{
			struct MultiAP_AP_LM_Query_Element_t *tlv_query = (struct MultiAP_AP_LM_Query_Element_t *)map_tlv;
			UINT8 index = 0;

			msg_len = strlen(tag);
			for (index = 0; index < tlv_query->BSSID_num; index++) {
				/* 17.2.22 AP Metrics TLV format */
				MAP_tlv_AP_metrics_process(&tlv_query->bssid[index], vmac_p, buf, &msg_len);

				/* 17.2.35 Associated STA Traffic Stats TLV */
				MAP_tlv_STA_traffic_stats_process(vmac_p, buf, &msg_len);

				/* 17.2.24 Associated STA Link Metrics TLV format */
				MAP_tlv_STA_metrics_process(&tlv_query->bssid[index], vmac_p, buf, &msg_len);
			}
		}
		break;
#endif				/* AP_STEERING_SUPPORT */
	case MAP_TLV_STA_MAC_ADDR:
		{
			struct MultiAP_STA_LM_Query_Element_t *tlv_query = (struct MultiAP_STA_LM_Query_Element_t *)map_tlv;
			struct MultiAP_STA_LM_Resp_Element_t *tlv_resp = NULL;
			struct MultiAP_STA_LM_Report_t *report = NULL;
			...
			nb_num = MSAN_get_neighbor_bySSID(vmac_p->dev, &vmac_p->macSsId);
			nlistbyssid = &(wlpptr->wlpd_p->nb_info.nb_elem[0]);
			tlv_resp = (struct MultiAP_STA_LM_Resp_Element_t *)pos;
			tlv_resp->tlv.tlvType = MAP_TLV_STA_LM_RESP;
			memcpy(tlv_resp->value[0].mac_addr, tlv_query->mac_addr, sizeof(IEEEtypes_MacAddr_t));
			tlv_resp->value[0].num_bssid = nb_num;
			pos += sizeof(struct MultiAP_STA_LM_Resp_Element_t) + sizeof(struct MultiAP_STA_LM_Value_t);
			for (index = 0; index < nb_num; index++) {
				if ((pos - buf) > (IW_CUSTOM_MAX - (sizeof(struct MultiAP_STA_LM_Report_t)))) {
					printk("MAP_TLV_STA_LM_RESP not enough buffer size!!!\n");
					break;
				}
				report = (struct MultiAP_STA_LM_Report_t *)pos;
				memcpy(report->bssid, nlistbyssid[index].Bssid, sizeof(IEEEtypes_MacAddr_t));
				report->delta_ms = 100;
				report->EST_downlink = 1;
				report->EST_uplink = 1;
				report->rssi_uplink = 10;
...
#endif				//IEEE80211K
			msg_len = (pos - buf);
			tlv_resp->tlv.tlvLen = msg_len - strlen(tag) - sizeof(struct MultiAP_TLV_Element_t);
		}
		break;
	case MAP_TLV_UNASSOC_STA_LM_QUERY:
		{
			struct MultiAP_Unassociated_STA_LM_Query_Element_t *tlv_query = (struct MultiAP_Unassociated_STA_LM_Query_Element_t *)map_tlv;
			struct MultiAP_Unassociated_STA_LM_Resp_Element_t *tlv_resp = NULL;
			struct MultiAP_Unassociated_STA_LM_Report_t *report = NULL;
			UINT8 index = 0, ch_list_num;
			UINT8 *pos = (buf + strlen(tag));
			UINT8 *ch_list, *sta_mac_pos;

			tlv_resp = (struct MultiAP_Unassociated_STA_LM_Resp_Element_t *)pos;
			...
			if (ch_list_num > (tlv_resp->tlv.tlvLen - sizeof(MultiAP_Unassociated_STA_LM_Resp_Element_t))) {
			...
			pos += sizeof(MultiAP_Unassociated_STA_LM_Resp_Element_t);

			for (index = 0; index < tlv_resp->STA_num && *sta_mac_pos != 0; index++) {
				if ((pos - buf) > (IW_CUSTOM_MAX - (sizeof(struct MultiAP_Unassociated_STA_LM_Report_t)))) {
					printk("MAP_TLV_STA_LM_RESP not enough buffer size!!!\n");
					break;
				}
				report = (struct MultiAP_Unassociated_STA_LM_Report_t *)pos;
				memcpy(report->mac_addr, sta_mac_pos, sizeof(IEEEtypes_MacAddr_t));
				sta_mac_pos += sizeof(IEEEtypes_MacAddr_t);
				report->channel = 36;
				report->delta_ms = 100;
				report->rssi_uplink = 10;
				pos += sizeof(MultiAP_Unassociated_STA_LM_Report_t);
			}
			msg_len = (pos - buf);
			tlv_resp->tlv.tlvLen = msg_len - strlen(tag) - sizeof(struct MultiAP_TLV_Element_t);
		}
	...
	case MAP_TLV_BEACON_LM_QUERY:
		{
			struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, vmac_p->dev);
			struct MultiAP_Beacon_LM_Query_Element_t *tlv_query = (struct MultiAP_Beacon_LM_Query_Element_t *)map_tlv;
			...
			if ((StaInfo_p = extStaDb_GetStaInfo(vmac_p, &(tlv_query->mac_addr), 1)) == NULL || (StaInfo_p->State != ASSOCIATED)) {
				printk("MAP_TLV_BEACON_LM_QUERY cannot found STA info:%s\n", mac_display((UINT8 *) & (tlv_query->mac_addr)));
				/* The STA is unassociated */
				return;
			}

			if (!StaInfo_p->RRM_Cap_IE.BcnActMeasCap || !StaInfo_p->RRM_Cap_IE.BcnActMeasCap) {
				...
				printk("MAP_TLV_BEACON_LM_QUERY STA does not support beacon report (%d,%d)\n", StaInfo_p->RRM_Cap_IE.BcnActMeasCap,
				       StaInfo_p->RRM_Cap_IE.BcnActMeasCap);
				MAP_tlv_Resp_process(vmac_p, NULL, (IEEEtypes_MacAddr_t *) & (tlv_query->mac_addr), 2);
				return;
			}
			skb = mlmeApiPrepMgtMsg2(IEEE_MSG_QOS_ACTION,
						 (IEEEtypes_MacAddr_t *) tlv_query->mac_addr, (IEEEtypes_MacAddr_t *) vmac_p->dev->dev_addr, 512);
			if (skb == NULL) {
			...
			*(UINT8 *) & RequestElement->Mode = 0;
			...
			memcpy(bcn_req->Bssid, tlv_query->bssid, sizeof(IEEEtypes_MacAddr_t));
			opt = (UINT8 *) & bcn_req->OptSubElem;
			var_pos = tlv_query->variable;
			/***** SSID Start  *******/
			if (var_pos[0] != 0) {
				struct IEEEtypes_SsIdElement_t *element = (struct IEEEtypes_SsIdElement_t *)opt;
			...
			if (var_pos[0] != 0) {
				...
				for (index_h = 0; index_h < ch_num; index_h++) {
					struct IEEEtypes_ChannelReportEL_t *element = (struct IEEEtypes_ChannelReportEL_t *)opt;

					var_pos++;	// Length of an AP Channel Report.
					if (var_pos[0]) {
						element->ElementId = CHAN_REPORT;
						element->Len = var_pos[0];	// include 1 octet of op class.
						element->RegClass = var_pos[1];
						memcpy(element->ChanList, &var_pos[2], (var_pos[0] - 1));
						opt += element->Len + 2;
						var_pos += element->Len;
					}
				}
			...
			if (tlv_query->report_detail) {
				struct IEEEtypes_ReportingDetail_t *element = (struct IEEEtypes_ReportingDetail_t *)opt;
				element->ElementId = 2;
				element->Len = var_pos[0] + 1;
				element->reporting_detail_value = tlv_query->report_detail;
				if (var_pos[0] != 0) {
					memcpy(element->variable, &var_pos[1], var_pos[0]);
				}
				opt += element->Len + 2;
				var_pos += element->Len;
			}
			...
			RequestElement->Len = 3 + sizeof(IEEEtypes_BeaconRequest_t) + len - 1;
			skb_trim(skb, sizeof(struct IEEEtypes_MgmtHdr2_t) + 23 + len);
...
void MAP_tlv_Resp_process(vmacApInfo_t * vmacSta_p, void *msg_data, IEEEtypes_MacAddr_t * sta_mac_addr_p, UINT8 status)
{
	static const char *tag = "1905LM";
	unsigned char *buf = NULL;
	int buf_len = 1024;
	union iwreq_data wreq;
	struct macmgmtQ_MgmtMsg3_t *MgmtMsg_p = (struct macmgmtQ_MgmtMsg3_t *)msg_data;
...
extern int DecideDFSOperation(struct net_device *netdev, BOOLEAN bChannelChanged, BOOLEAN bBandWidthChanged,
...
extern void set_dfs_ctl_status(DfsApDesc * dfsDesc_p, UINT8 path, DFS_STATE status);
extern void FireEMCACTimer(DfsAp * me);
extern void EM_update_cac_status(vmacApInfo_t * vmacSta_p, UINT8 ch, UINT8 status, UINT32 indication);
...
void EMCACTimeoutHandler(void *data_p)
{
	DfsAp *me;
	...
	if (DfsFindInNOL(&me->dfsApDesc.NOCList, me->dfsApDesc.CtlChanInfo.channel) == DFS_FAILURE) {
		EM_update_cac_status(wlpptr->vmacSta_p, me->dfsApDesc.CtlChanInfo.channel, 0 /*available channel */ , 1 /*indecation */ );
	}
...
void FireEMCACTimer(DfsAp * me)
{
	...
	EM_update_cac_status(wlpptr->vmacSta_p, me->dfsApDesc.CtlChanInfo.channel, 6 /*on-going channel */ , 0 /*indecation */ );
	TimerFireIn(&dfsDesc_p->EMCACTimer, 1, &EMCACTimeoutHandler, (unsigned char *)me, dfsDesc_p->CtlCACTimeOut);
	me->scnr_ctl_evt(dev, ScnrCtl_Channel_switch_start_cac, DFS_STATE_SCAN, 1);
	return;
}

void DisarmEMCACTimer(DfsAp * me)
{
	DfsApDesc *dfsDesc_p = NULL;
	struct net_device *dev;
	struct wlprivate *wlpptr = NULL;
...
void EM_remove_ch_from_available_report(UINT8 ch)
{
	UINT8 i;

	/* remove ch from available_channel_report */
	for (i = 0; i < CAC_STATUS.available_channel_report.num_of_available_channel; i++) {
		if (ch == CAC_STATUS.available_channel_report.available_channel_info[i].channel) {
			break;
		}
	}
	if (i < CAC_STATUS.available_channel_report.num_of_available_channel) {
		for (; i < CAC_STATUS.available_channel_report.num_of_available_channel; i++) {
			memcpy(&CAC_STATUS.available_channel_report.available_channel_info[i],
			       &CAC_STATUS.available_channel_report.available_channel_info[i + 1], sizeof(cac_available_channel_t));
		}
		memset(&CAC_STATUS.available_channel_report.available_channel_info[i], 0, sizeof(cac_available_channel_t));
		CAC_STATUS.available_channel_report.num_of_available_channel--;
	}
}

void EM_remove_ch_from_radar_report(UINT8 ch)
{
	UINT8 i;

	/* remove ch from radar_channel_report */
	for (i = 0; i < CAC_STATUS.radar_channel_report.num_of_radar_channel; i++) {
		if (ch == CAC_STATUS.radar_channel_report.radar_channel_info[i].channel) {
			break;
		}
	}
	if (i < CAC_STATUS.radar_channel_report.num_of_radar_channel) {
		for (; i < CAC_STATUS.radar_channel_report.num_of_radar_channel; i++) {
			memcpy(&CAC_STATUS.radar_channel_report.radar_channel_info[i],
			       &CAC_STATUS.radar_channel_report.radar_channel_info[i + 1], sizeof(cac_radar_channel_t));
		}
		memset(&CAC_STATUS.radar_channel_report.radar_channel_info[i], 0, sizeof(cac_radar_channel_t));
		CAC_STATUS.radar_channel_report.num_of_radar_channel--;
	}
}

void EM_update_cac_status(vmacApInfo_t * vmacSta_p, UINT8 ch, UINT8 status, UINT32 indication)
{
	...
	memset(&CAC_STATUS.ongoing_channel_report.ongoing_channel_info[0], 0, sizeof(cac_ongoing_channel_t));
	...
	for (i = 0; i < CAC_max_ch_num; i++) {
		if (CSC_chList[i].channel == dfsDesc_p->CtlChanInfo.channel) {
			CSC_chList[i].timestamp = ktime_to_timespec(ktime_get_real()).tv_sec;
	...
	case 0:
		{
			...
			for (i = 0; i < 30; i++) {
				cac_channel = &CAC_STATUS.available_channel_report.available_channel_info[i];
	...
	case 1:
		{
			...
			for (i = 0; i < 30; i++) {
				cac_channel = &CAC_STATUS.radar_channel_report.radar_channel_info[i];
				if (cac_channel->channel == 0) {
					CAC_STATUS.radar_channel_report.num_of_radar_channel++;
					cac_channel->channel = ch;
					cac_channel->op_class = op_class;
	...
	case 6:
		{
			...
			cac_channel = &CAC_STATUS.ongoing_channel_report.ongoing_channel_info[0];
			if (cac_channel->channel == 0) {
				CAC_STATUS.ongoing_channel_report.num_of_ongoing_channel = 1;
				cac_channel->channel = ch;
				cac_channel->op_class = op_class;
				cac_channel->seconds = 0;
				break;
	...
	if (indication && CAC_Scan_Flag) {
		...
		/* both ioctl and softirq(timer) will go here, so we use wl_kmalloc_autogfp to alloc buf */
		buf = wl_kmalloc_autogfp(buf_len);
		if (!buf)
			return;
		...
		memset(&wreq, 0, sizeof(wreq));
		wreq.data.length = sizeof(cac_complete_indication_t) + strlen(tag);
		wireless_send_event(vmacSta_p->dev, IWEVCUSTOM, &wreq, buf);
		wl_kfree(buf);
	}
}

SINT32 EM_get_cac_status(UINT8 * buf, UINT8 log)
{
	SINT32 cur_time;
	...
	for (ch_index = 0; ch_index < CAC_max_ch_num; ch_index++) {
		found = 0;
		for (i = 0; i < CAC_STATUS.available_channel_report.num_of_available_channel; i++) {
			if (CSC_chList[ch_index].channel == CAC_STATUS.available_channel_report.available_channel_info[i].channel) {
				found = 1;
				CAC_STATUS.available_channel_report.available_channel_info[i].minutes =
				    (cur_time - CSC_chList[ch_index].timestamp) / 60;
				break;
			}
		}
		if (found) {
		...
		for (i = 0; i < CAC_STATUS.radar_channel_report.num_of_radar_channel; i++) {
			if (CSC_chList[ch_index].channel == CAC_STATUS.radar_channel_report.radar_channel_info[i].channel) {
				found = 1;
				CAC_STATUS.radar_channel_report.radar_channel_info[i].seconds = (cur_time - CSC_chList[ch_index].timestamp);
				break;
			}
		}
		if (found) {
		...
		for (i = 0; i < CAC_STATUS.ongoing_channel_report.num_of_ongoing_channel; i++) {
			if (CSC_chList[ch_index].channel == CAC_STATUS.ongoing_channel_report.ongoing_channel_info[i].channel) {
				CAC_STATUS.ongoing_channel_report.ongoing_channel_info[i].seconds = (cur_time - CSC_chList[ch_index].timestamp);
				break;
			}
		}
	}
	...
	if (log > 0) {
		...
		for (i = 0; i < CAC_STATUS.available_channel_report.num_of_available_channel; i++) {
			printk("ch:%d,%d,%d\n", CAC_STATUS.available_channel_report.available_channel_info[i].op_class,
			       CAC_STATUS.available_channel_report.available_channel_info[i].channel,
			       CAC_STATUS.available_channel_report.available_channel_info[i].minutes);
		}
		printk("\nradar_channel_report:\n");
		for (i = 0; i < CAC_STATUS.radar_channel_report.num_of_radar_channel; i++) {
			printk("ch:%d,%d,%d\n", CAC_STATUS.radar_channel_report.radar_channel_info[i].op_class,
			       CAC_STATUS.radar_channel_report.radar_channel_info[i].channel,
			       CAC_STATUS.radar_channel_report.radar_channel_info[i].seconds);
		}
		printk("\nongoing_channel_report:\n");
		for (i = 0; i < CAC_STATUS.ongoing_channel_report.num_of_ongoing_channel; i++) {
			printk("ch:%d,%d,%d\n", CAC_STATUS.ongoing_channel_report.ongoing_channel_info[i].op_class,
			       CAC_STATUS.ongoing_channel_report.ongoing_channel_info[i].channel,
			       CAC_STATUS.ongoing_channel_report.ongoing_channel_info[i].seconds);
		}
		printk("\n");
	}
...
void EM_get_dfs_ch_list(vmacApInfo_t * vmacSta_p, UINT8 op_class, UINT8 ch)
{
	...
	for (i = 0; i < DFS_MAX_CHANNELS; i++) {
		...
		CSC_chList[count].channel = dfsEnabledChannels[domainInd].dfschannelEntry[i];
		count++;
	}
	CAC_max_ch_num = count;
}

void EM_CAC_Scan(vmacApInfo_t * vmacSta_p, UINT8 op_class, UINT8 ch, UINT8 enable)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, vmacSta_p->dev);
	DfsApDesc *dfsDesc_p;
	MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
	...
	if (enable == 2) {
		...
		memset(&CSC_chList[0], 0, sizeof(cac_channel_scan_list_t) * DFS_MAX_CHANNELS);
		/* get range to scan */
		...
		for (i = 0; i < CAC_max_ch_num && i < DFS_MAX_CHANNELS; i++) {
			if (CSC_chList[i].channel >= 36 && CSC_chList[i].channel <= 48) {
				CSC_chList[i].op_class = 115;
			} else if (CSC_chList[i].channel >= 52 && CSC_chList[i].channel <= 64) {
				CSC_chList[i].op_class = 118;
			} else if (CSC_chList[i].channel >= 100 && CSC_chList[i].channel <= 144) {
				CSC_chList[i].op_class = 121;
			} else if (CSC_chList[i].channel >= 149 && CSC_chList[i].channel <= 161) {
	...
	if (mib_SpectrumMagament_p->spectrumManagement == 0 && enable) {
		...
		if (DecideDFSOperation(vmacSta_p->dev, TRUE, FALSE, DFS_DISABLED, DFS_ENABLED, mib) == FAIL) {
	...
	if (wlpptr->wlpd_p->pdfsApMain == NULL) {
		WLDBG_ERROR(DBG_LEVEL_0, "pdfsApMain is Null, not enable 11hspecmgt\n");
		return;
	}
	...
	if (enable == 0) {
		...
		if (DfsFindInNOL(&dfsDesc_p->NOCList, dfsDesc_p->CtlChanInfo.channel) == DFS_FAILURE) {
			EM_update_cac_status(wlpptr->vmacSta_p, dfsDesc_p->CtlChanInfo.channel, 0 /*available channel */ , 1 /*indecation */ );
		}
		return;
	}
