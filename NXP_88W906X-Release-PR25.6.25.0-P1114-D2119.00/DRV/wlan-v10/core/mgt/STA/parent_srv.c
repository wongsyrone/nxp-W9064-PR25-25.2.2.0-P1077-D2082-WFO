...
vmacEntry_t vmacEntry_parent[NUM_OF_WLMACS];
vmacStaInfo_t mlmeStaInfo_parent[NUM_OF_WLMACS];
Station_t Station_parent[NUM_OF_WLMACS];

/* Global to hold the assigned vMac Id of the Parent Session */
#ifdef SOC_W906X
vmacId_t idAssigned_parent[NUM_OF_WLMACS] = { -1, -1, -1 };
#else
vmacId_t idAssigned_parent[NUM_OF_WLMACS] = { -1, -1, -1, -1 };
#endif
...
extern vmacEntry_t *mlmeStaInit_Parent(UINT8 macIndex, UINT8 * macAddr, void *callBackFunc_p)
{
	...
	memcpy(&vmacEntry_parent[macIndex].vmacAddr[0], macAddr, sizeof(IEEEtypes_MacAddr_t));
