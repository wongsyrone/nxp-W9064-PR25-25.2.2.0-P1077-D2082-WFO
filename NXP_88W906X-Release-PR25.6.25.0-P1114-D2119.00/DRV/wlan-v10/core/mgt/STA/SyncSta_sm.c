...
MhsmEvent_t const *No_Bss_Handle_Sta(SyncSrvSta * me, MhsmEvent_t * msg)
{
	...
	case Tbcn:
		...
		return 0;
	case ProbeRsp:
		syncSrv_PrbeRspNoBssHandler((vmacStaInfo_t *) msg->info, (dot11MgtFrame_t *) syncMsg_p->mgtFrame_p, syncMsg_p->rfHdr_p);
		return 0;
	case MlmeJoin_Cnfm:
...
MhsmEvent_t const *Bss_Handle_Sta(SyncSrvSta * me, MhsmEvent_t * msg)
{
	...
	case Beacon:
		syncSrv_BncRecvAssociatedHandler((vmacStaInfo_t *) msg->info, (dot11MgtFrame_t *) syncMsg_p->mgtFrame_p, syncMsg_p->rfHdr_p);
		return 0;
	...
	case MlmeReset_Req:
		...
		return 0;
	case ProbeRsp:
		syncSrv_ProbeRspRcvd((vmacStaInfo_t *) msg->info, (dot11MgtFrame_t *) syncMsg_p->mgtFrame_p, syncMsg_p->rfHdr_p);
		return 0;
...
MhsmEvent_t const *Act_Listen_Handle_Sta(SyncSrvSta * me, MhsmEvent_t * msg)
{
	...
	case Beacon:
		...
		printk("%s[%d]\n", __FUNCTION__, __LINE__);
#endif				/* ETH_DEBUG */
		syncSrv_ScanFilter((vmacStaInfo_t *) msg->info, (dot11MgtFrame_t *) syncMsg_p->mgtFrame_p, syncMsg_p->rfHdr_p);
		return 0;
	case ProbeRsp:
		syncSrv_ScanFilter((vmacStaInfo_t *) msg->info, (dot11MgtFrame_t *) syncMsg_p->mgtFrame_p, syncMsg_p->rfHdr_p);
		return 0;
...
void SyncSrvCtorSta(SyncSrvSta * me)
{
	...
	mhsm_add(&me->Sync_Srv_Sta, &me->sTop, (MhsmFcnPtr) Synchronization_Sta_Handle);
	mhsm_add(&me->Act_Listen, &me->Sync_Srv_Sta, (MhsmFcnPtr) Act_Listen_Handle_Sta);
	mhsm_add(&me->Act_Recv, &me->Sync_Srv_Sta, (MhsmFcnPtr) Act_Recv_Handle_Sta);
	mhsm_add(&me->AP_Active, &me->Sync_Srv_Sta, (MhsmFcnPtr) STA_Active_Handle);
	mhsm_add(&me->Bss, &me->Sync_Srv_Sta, (MhsmFcnPtr) Bss_Handle_Sta);
	mhsm_add(&me->IBss_Active, &me->Sync_Srv_Sta, (MhsmFcnPtr) IBss_Active_Handle_Sta);
	mhsm_add(&me->IBss_Idle, &me->Sync_Srv_Sta, (MhsmFcnPtr) IBss_Idle_Handle_Sta);
	mhsm_add(&me->Init_Wait_Probe_Delay, &me->Sync_Srv_Sta, (MhsmFcnPtr) Init_Wait_Probe_Delay_Handle_Sta);
	mhsm_add(&me->Join_Wait_Beacon, &me->Sync_Srv_Sta, (MhsmFcnPtr) Join_Wait_Beacon_Handle_Sta);
	mhsm_add(&me->No_Bss, &me->Sync_Srv_Sta, (MhsmFcnPtr) No_Bss_Handle_Sta);
	mhsm_add(&me->Pas_Listen, &me->Sync_Srv_Sta, (MhsmFcnPtr) Pas_Listen_Handle_Sta);
	mhsm_add(&me->Wait_Csw_done, &me->Sync_Srv_Sta, (MhsmFcnPtr) Wait_Csw_done_Handle_Sta);
	mhsm_add(&me->Wait_Hop_Bss, &me->Sync_Srv_Sta, (MhsmFcnPtr) Wait_Hop_Bss_Handle_Sta);
	mhsm_add(&me->Wait_Probe_Delay, &me->Sync_Srv_Sta, (MhsmFcnPtr) Wait_Probe_Delay_Handle_Sta);
}
