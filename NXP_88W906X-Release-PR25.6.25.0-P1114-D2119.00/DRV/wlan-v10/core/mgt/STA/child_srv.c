...
UINT8 childBlockAddr_mask[NUM_OF_WLMACS] = { 0, 0, 0 };
halMacId_t childBlockAddrMacId[NUM_OF_WLMACS] = { -1, -1, -1 };
...
static vmacEntry_t *childSrv_GetEntry(UINT8 * vMacAddr)
{
	...
	for (i = 0; i < MAX_MLME_CHILD_SESSIONS; i++) {
		if (!childVMacEntry[i].active) {
			...
			memcpy(&childVMacEntry[i].vmacAddr[0], vMacAddr, sizeof(IEEEtypes_MacAddr_t));
...
static BOOLEAN childSrv_PutEntry(UINT8 * vMacAddr)
{
	...
	for (i = 0; i < MAX_MLME_CHILD_SESSIONS; i++) {
		if (memcmp(&childVMacEntry[i].vmacAddr[0], vMacAddr, sizeof(IEEEtypes_MacAddr_t)) == 0) {
...
*
**************************************************************************/
static SINT32 childSrv_CopyParentSessionParam(vmacStaInfo_t * childStaInfo_p, UINT8 parentMacIndx)
{
...
#endif				/* WPA2 */
#endif				/* WPA_STA */
	memcpy(&childStaInfo_p->ScanParams, &parentStaInfo_p->ScanParams, sizeof(IEEEtypes_ScanCmd_t));
	memcpy(&childStaInfo_p->LastJoinMsg, &parentStaInfo_p->LastJoinMsg, sizeof(macmgmtQ_CmdReq_t));
	return MLME_SUCCESS;
}
...
*
**************************************************************************/
extern vmacEntry_t *childSrv_StartSession(UINT8 macIndex, UINT8 * macAddr, void *callBack_fp, UINT32 controlParam)
{
	vmacId_t idAssigned;
	halMacId_t macId;
	...
	if (childSrv_CopyParentSessionParam(childStaInfo_p, macIndex) == MLME_FAILURE) {
		if (childStaInfo_p->childControlParam & MLME_CHILD_SET_ADDR_TO_HW) {
...
#endif				/* ETH_DEBUG */
		childStaInfo_p = (vmacStaInfo_t *) childVMacEntry_p->info_p;
		if (!childStaInfo_p->isParentSession && (childStaInfo_p->childControlParam & MLME_CHILD_SET_ADDR_TO_HW)) {
...
extern SINT32 childSrv_TerminateAllLinks(void)
{
	...
	for (i = 0; i < MAX_MLME_CHILD_SESSIONS; i++) {
		if (childVMacEntry[i].active) {
			mlmeApiUpdateLinkStatus((vmacStaInfo_t *) childVMacEntry[i].info_p, LINK_DOWN);
		}
	}
	return MLME_SUCCESS;
}
...
*
**************************************************************************/
extern halMacId_t childSrv_RegisterBlockAddress(vmacStaInfo_t * vStaInfo_p, UINT8 macIndex, UINT8 * macAddr, UINT8 addrMask)
{
	halMacId_t macId;
