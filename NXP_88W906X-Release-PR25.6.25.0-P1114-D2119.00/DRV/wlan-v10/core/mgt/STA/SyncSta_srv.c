...
*
**************************************************************************/
static void syncSrv_SndScanCnfm(vmacStaInfo_t * vStaInfo_p, UINT16 scanResult, UINT8 numSet, UINT16 bufSize, IEEEtypes_BssDesc_t * BssDescSet_p)
{
	mlmeApiSndScanNotificationOnly(vStaInfo_p, scanResult, numSet, bufSize, (UINT8 *) BssDescSet_p);
}
...
*
**************************************************************************/
static void syncSrv_SndJoinCnfm(vmacStaInfo_t * vStaInfo_p, UINT16 joinResult)
{
...
*
**************************************************************************/
static void syncSrv_SndStartCnfm(vmacStaInfo_t * vStaInfo_p, UINT16 startResult)
{
...
*
**************************************************************************/
static void syncSrv_SndResetCnfm(vmacStaInfo_t * vStaInfo_p, UINT16 resetResult)
{
...
extern SINT32 syncSrvSta_ScanDwellTimeOut(UINT8 * data)
{
	...
	if (vStaInfo_p->ScanParams.ScanType == SCAN_ACTIVE) {
		...
		if (!(((currentChnl >= 52) && (currentChnl <= 64)) || ((currentChnl >= 100) && (currentChnl <= 140))))
#endif
...
#else
	mlmeApiStartTimer(vStaInfo_p, (UINT8 *) & vStaInfo_p->scaningTimer, &syncSrvSta_ScanActTimeOut, vStaInfo_p->scanTime_tick);
	...
	if (vStaInfo_p->AssociatedFlag) {
		...
		if (vStaInfo_p->mib_WB_p->opMode) {
			...
			if (ClientModeDataCount[macIndex] == 0) {
...
#endif
						mlmeApiEventNotification(vStaInfo_p, Tbcn, &vStaInfo_p->macMgmtMlme_ThisStaData.BssId[0], 0);
				...
				if ((*(vmacSta_p->Mib802dot11->mib_wpaWpa2Mode) > 3) && (!vStaInfo_p->rxBcnCnt)) {
					mlmeApiUpdateLinkStatus(vStaInfo_p, LINK_DOWN);
					vStaInfo_p->AssociatedFlag = 0;
				}
#endif
		...
		if (vStaInfo_p->isParentSession)
			if (mlmeApiIsTrunkIdActive(vStaInfo_p, vmacEntry_p->trunkId))
				...
				if (!vStaInfo_p->mib_WB_p->opMode) {
					for (ii = 0; ii < 3; ii++) {
						if (SUCCESS ==
						    mlmeApiSendNullDataPktUr(vStaInfo_p, &vStaInfo_p->macMgmtMlme_ThisStaData.BssId,
...
*
**************************************************************************/
extern BOOLEAN syncSrv_ValidatePeerSetting(vmacStaInfo_t * vStaInfo_p, dot11MgtFrame_t * MgmtMsg_p)
{
	void *attrib_p;
	...
	vmacEntry_p = (vmacEntry_t *) vStaInfo_p->vMacEntry_p;
	if (memcmp(&(MgmtMsg_p->Hdr.BssId), &(vStaInfo_p->macMgmtMlme_ThisStaData.BssId), sizeof(IEEEtypes_MacAddr_t))) {
		return FALSE;
	}
	/* Validate the CapInfo */
	memcpy((void *)&profileCapInfo, (const void *)&vStaInfo_p->bssDescProfile_p->Cap, sizeof(UINT16));
	if ((*(UINT16 *) & MgmtMsg_p->Body.ProbeRsp.CapInfo & MLME_CAPINFO_VALIDATION_MASK) != (profileCapInfo & MLME_CAPINFO_VALIDATION_MASK)) {
		return FALSE;
	}
	/* Validate SSID */
	if ((attrib_p = syncSrvSta_ParseAttrib(MgmtMsg_p, SSID)) != NULL) {
		...
		if ((ssidLen != ((IEEEtypes_SsIdElement_t *) attrib_p)->Len)
		    || memcmp(&((IEEEtypes_SsIdElement_t *) attrib_p)->SsId[0], &vStaInfo_p->bssDescProfile_p->SsId[0], ssidLen))
		{
			return FALSE;
		}
	}
	...
	if ((attrib_p = syncSrvSta_ParseAttrib(MgmtMsg_p, DS_PARAM_SET)) != NULL) {
		if (((IEEEtypes_DsParamSet_t *) attrib_p)->CurrentChan != vStaInfo_p->bssDescProfile_p->PhyParamSet.DsParamSet.CurrentChan) {
			return FALSE;
		}
	}
...
*
**************************************************************************/
extern void syncSrv_ProbeRspRcvd(vmacStaInfo_t * vStaInfo_p, dot11MgtFrame_t * MgmtMsg_p, UINT8 * rfHdr_p)
{
	UINT8 ssidLen;
...
static SINT32 syncSrv_SndProbeReq(vmacStaInfo_t * vStaInfo_p, BOOLEAN isScan)
{
	...
	struct net_device *clientDev_p, *staDev = (struct net_device *)vmacEntry_p->privInfo_p;
	...
	if (!isScan && !is_zero_ether_addr(&vStaInfo_p->macMgmtMlme_ThisStaData.BssId[0])) {
		memcpy(&destAddr, &vStaInfo_p->macMgmtMlme_ThisStaData.BssId[0], sizeof(IEEEtypes_MacAddr_t));
		memcpy(&bssIdAddr, &vStaInfo_p->macMgmtMlme_ThisStaData.BssId[0], sizeof(IEEEtypes_MacAddr_t));
	} else {
...
#endif
	mlmePrepDefaultMgtMsg_Sta(vStaInfo_p, probeMsg_p, &destAddr, IEEE_MSG_PROBE_RQST, &bssIdAddr);
	probeMsg_p->Hdr.FrmBodyLen = 0;
	/* Add SSID Attrib */
	if (!isScan) {
		lenSsId = strlen(vStaInfo_p->staSystemMibs.mib_StaCfg_p->DesiredSsId);
		syncSrv_AddAttrib(probeMsg_p, SSID, vStaInfo_p->staSystemMibs.mib_StaCfg_p->DesiredSsId, lenSsId);
	} else {
		lenSsId = strlen(vStaInfo_p->ScanParams.SsId);
		syncSrv_AddAttrib(probeMsg_p, SSID, vStaInfo_p->ScanParams.SsId, lenSsId);
	}

#ifdef SOC_W906X
	if (vStaInfo_p->ChanIdx >= (IEEEtypes_MAX_CHANNELS + IEEEtypes_MAX_CHANNELS_A))
		return MLME_FAILURE;
	...
	for (currRate = 0, rateid = 0; currRate < 14; currRate++) {
		if (rateMask & 0x01) {
			if (mib->StationConfig->OpRateSet[currRate] != 0) {
				rateSupport[rateid] = mib->StationConfig->OpRateSet[currRate] & 0x7F;
				supportedRateLen++;
	...
	for (rateid = 0; currRate < 14; currRate++) {
		if (rateMask & 0x01) {
			if (mib->StationConfig->OpRateSet[currRate] != 0) {
				rateExt[rateid] = mib->StationConfig->OpRateSet[currRate] & 0x7F;
				extendedRateLen++;
				rateid++;
			}
		}
	...
	if (supportedRateLen) {
		syncSrv_AddAttrib(probeMsg_p, SUPPORTED_RATES, &rateSupport[0], supportedRateLen);
	}
#ifdef MRVL_WPS_CLIENT
	if (stapriv->wpsProbeRequestIeLen > 0) {
		UINT8 *pos = (UINT8 *) & probeMsg_p->Body + probeMsg_p->Hdr.FrmBodyLen;
		memcpy(pos, stapriv->wpsProbeRequestIe, stapriv->wpsProbeRequestIeLen);
	...
	if (stapriv->mboProbeRequestIeLen > 0) {
		UINT8 *pos = (UINT8 *) & probeMsg_p->Body + probeMsg_p->Hdr.FrmBodyLen;
		memcpy(pos, stapriv->mboProbeRequestIe, stapriv->mboProbeRequestIeLen);
	...
	if (extendedCap_needAdd) {
		...
		if (pExtCap) {
			...
			syncSrv_AddAttrib(probeMsg_p, EXT_CAP_IE, (UINT8 *) & pExtCap->ExtCap, sizeof(IEEEtypes_Extended_Cap_Element_t) - 2);
			wl_kfree(pExtCap);
		}
	}
	...
	if (!supportedRateLen && extendedRateLen) {
		syncSrv_AddAttrib(probeMsg_p, SUPPORTED_RATES, &rateExt[0], extendedRateLen);
	} else if (extendedRateLen) {
		syncSrv_AddAttrib(probeMsg_p, EXT_SUPPORTED_RATES, &rateExt[0], extendedRateLen);
	}
#ifdef SOC_W906X
	if ((*(mib->mib_ApMode) & AP_MODE_N_ONLY)
		...
		probeMsg_p->Hdr.FrmBodyLen +=
		    AddHT_IE(priv_p->vmacSta_p, (IEEEtypes_HT_Element_t *) ((UINT8 *) & probeMsg_p->Body + probeMsg_p->Hdr.FrmBodyLen));
	}

	if (*(mib->mib_ApMode) & AP_MODE_11AC) {
		probeMsg_p->Hdr.FrmBodyLen += Build_IE_191(priv_p->vmacSta_p, (UINT8 *) & probeMsg_p->Body + probeMsg_p->Hdr.FrmBodyLen, FALSE, 0);
	}

	if (*(mib->mib_ApMode) & AP_MODE_11AX) {
		probeMsg_p->Hdr.FrmBodyLen += Build_IE_HE_CAP(priv_p->vmacSta_p, (UINT8 *) & probeMsg_p->Body + probeMsg_p->Hdr.FrmBodyLen);
	}
#endif
	/* Adding MRVL Rptr IE */
	if (*(vmacSta_p->Mib802dot11->mib_STAMacCloneEnable) == 2) {
		...
		if (probeMsg_p->Hdr.FrmBodyLen + ieHdr_p->Len < sizeof(*p))
			memcpy((void *)&(p[probeMsg_p->Hdr.FrmBodyLen]), mrvl_id, ieHdr_p->Len);
		else
			return MLME_FAILURE;

		probeMsg_p->Hdr.FrmBodyLen += ieHdr_p->Len;
		memcpy((void *)&(p[probeMsg_p->Hdr.FrmBodyLen]), vmacSta_p->Mib802dot11->mib_RptrDeviceType,
...
*
**************************************************************************/
extern void syncSrv_ProbeReqRcvd(vmacStaInfo_t * vStaInfo_p, dot11MgtFrame_t * MgmtMsg_p, UINT8 * rfHdr_p)
{
	...
	if (ssidLen && (*(UINT8 *) & MgmtMsg_p->Body.ProbeRqst.SsId.SsId[0] != 0x0)) {
		if ((ssidLen != vStaInfo_p->macMgmtMlme_ThisStaData.BssSsIdLen)
		    || memcmp((void *)MgmtMsg_p->Body.ProbeRqst.SsId.SsId,
			      vStaInfo_p->macMgmtMlme_ThisStaData.BssSsId, vStaInfo_p->macMgmtMlme_ThisStaData.BssSsIdLen)) {
			return;
		}
	}
...
static void syncSrv_UpdateAdhocTxInfo(vmacStaInfo_t * vStaInfo_p,
				      ...
				      IEEEtypes_MacAddr_t * BssId_p, UINT8 packType, macmgmtQ_MgmtMsg_t * bcnBuf_p, txBcnInfo_t * MgtTxInfo)
{
	UINT8 *nextElement_p;
   ...
   /*----------------------------------*/
	mlmePrepDefaultMgtMsg_Sta(vStaInfo_p, (dot11MgtFrame_t *) & (bcnBuf_p->Hdr), &bcastAddr, packType, BssId_p);
   ...
   /*-------------------------------------------------------------*/
	...
	ssId_p->Len = util_CopyList(ssId_p->SsId, StartCmd_p->SsId, IEEEtypes_SSID_SIZE);

	nextElement_p += sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + ssId_p->Len;
   ...
   /*----------------------------------------------------------*/
	...
	suppRates_p->Len = util_CopyList(suppRates_p->Rates, StartCmd_p->OpRateSet, IEEEtypes_MAX_DATA_RATES_G);
	...
	nextElement_p += sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + suppRates_p->Len;
   ...
   /*-----------------------------------------------------------*/
	if (vStaInfo_p->macMgt_StaMode == CLIENT_MODE_G) {
		...
		ExtsuppRates_p->Len = util_CopyList(ExtsuppRates_p->Rates,
						    &StartCmd_p->OpRateSet[MAX_B_DATA_RATES], IEEEtypes_MAX_DATA_RATES_G - MAX_B_DATA_RATES);

		nextElement_p += sizeof(IEEEtypes_ElementId_t) + sizeof(IEEEtypes_Len_t) + ExtsuppRates_p->Len;
	}
   ...
   /*----------------------------------------------------------------*/
	bcnBuf_p->Hdr.FrmBodyLen = (UINT32) tim_p - (UINT32) bcnBuf_p - sizeof(IEEEtypes_MgmtHdr_t);
   ...
   /*--------------------------------------------------------------*/
	...
	/* paramters.                                                   */
   /*--------------------------------------------------------------*/
	MgtTxInfo->CfOffset = (UINT32) ssParamSet_p - (UINT32) bcnBuf_p - sizeof(bcnBuf_p->Hdr.FrmBodyLen) - sizeof(bcnBuf_p->Hdr.Rsrvd);
   ...
   /*-----------------------------------------------------------*/
	MgtTxInfo->TimOffset = (UINT32) tim_p - (UINT32) bcnBuf_p - sizeof(bcnBuf_p->Hdr.FrmBodyLen) - sizeof(bcnBuf_p->Hdr.Rsrvd);
   ...
   /*--------------------------------------------------------------*/
	MgtTxInfo->ProbeRespLen = bcnBuf_p->Hdr.FrmBodyLen +
	    sizeof(IEEEtypes_MgmtHdr_t) - sizeof(bcnBuf_p->Hdr.FrmBodyLen) - sizeof(bcnBuf_p->Hdr.Rsrvd) + sizeof(UINT32);
...
static void syncSrv_ScanPrep(vmacStaInfo_t * vStaInfo_p)
{
	...
	memset(&vStaInfo_p->scanTableResult_p->ScanResults, 0, MAX_SCAN_BUF_SIZE);

	memset(&vStaInfo_p->scanTableResult_p->BssSourceAddr, 0, sizeof(IEEEtypes_MacAddr_t) * IEEEtypes_MAX_BSS_DESCRIPTS);
...
extern void syncSrvSta_ScanCmd(vmacStaInfo_t * vStaInfo_p, macMgmtQ_ScanCmd_t * ScanCmd_p)
{
	UINT32 i = 0;
	UINT32 ssIdLen;
   ...
   /*-----------------------------------------*/
	if (vStaInfo_p->ScanParams.ChanList[0] == 0) {
		...
		syncSrv_SndScanCnfm(vStaInfo_p, SCAN_RESULT_INVALID_PARAMETERS, 0, 0, NULL);
		return;
	}
...
#else
	mlmeApiStartTimer(vStaInfo_p, (UINT8 *) & vStaInfo_p->scaningTimer, &syncSrvSta_ScanDwellTimeOut, time_scan_start);
#endif				/* MLME_SEPERATE_SCAN_TIMER */
...
*
**************************************************************************/
extern void syncSrv_JoinCmd(vmacStaInfo_t * vStaInfo_p, IEEEtypes_JoinCmd_t * JoinCmd_p)
{
	vmacEntry_t *vmacEntry_p;
	UINT32 length;
	...
	/********************************************
		...
		*********************************************/
		mlmeApiCreateSupportedRateElement((IEEEtypes_SuppRatesElement_t *) mlmeApiGetClientModeSupportedRates(vmacEntry_p->phyHwMacIndx),
						  (IEEEtypes_ExtSuppRatesElement_t *) mlmeApiGetClientModeExtSupportedRates(vmacEntry_p->
															    phyHwMacIndx),
						  &(vStaInfo_p->bOpRateSet[0]), &(vStaInfo_p->gOpRateSet[0]), vmacEntry_p->phyHwMacIndx);
	...
	if (vStaInfo_p->isParentSession) {
		/* Set SS Id to HW Mac */
		mlmeApiSetSsIdToMac(vStaInfo_p, &JoinCmd_p->BssDesc.SsId[0], length);
	...
	if (JoinCmd_p->BssDesc.Cap.Ibss) {
		...
		rateLen = util_CopyList(&StartCmd.OpRateSet[0], &JoinCmd_p->BssDesc.DataRates[0], IEEEtypes_MAX_DATA_RATES_G);

		if (JoinCmd_p->BssDesc.PhyParamSet.DsParamSet.ElementId == DS_PARAM_SET) {
			memcpy(&(StartCmd.PhyParamSet.DsParamSet), &(JoinCmd_p->BssDesc.PhyParamSet.DsParamSet), sizeof(IEEEtypes_DsParamSet_t));
		} else {
			memcpy(&(StartCmd.PhyParamSet.FhParamSet), &(JoinCmd_p->BssDesc.PhyParamSet.FhParamSet), sizeof(IEEEtypes_FhParamSet_t));
		}

		memcpy(&(StartCmd.SsId), JoinCmd_p->BssDesc.SsId, sizeof(IEEEtypes_SsId_t));

		if (JoinCmd_p->BssDesc.SsParamSet.CfParamSet.ElementId == CF_PARAM_SET) {
			memcpy(&(StartCmd.SsParamSet.CfParamSet), &(JoinCmd_p->BssDesc.SsParamSet.CfParamSet), sizeof(IEEEtypes_CfParamSet_t));
		} else {
			memcpy(&(StartCmd.SsParamSet.IbssParamSet), &(JoinCmd_p->BssDesc.SsParamSet.IbssParamSet), sizeof(IEEEtypes_IbssParamSet_t));
		}

#ifdef STA_ADHOC_SUPPORTED
		syncSrv_UpdateAdhocTxInfo(vStaInfo_p,
					  &StartCmd, &(JoinCmd_p->BssDesc.BssId), IEEE_MSG_BEACON, vStaInfo_p->BcnBuffer_p, vStaInfo_p->BcnTxInfo_p);

		syncSrv_UpdateAdhocTxInfo(vStaInfo_p,
					  &StartCmd,
					  &(JoinCmd_p->BssDesc.BssId), IEEE_MSG_PROBE_RSP, vStaInfo_p->PrbRspBuf_p, vStaInfo_p->PrbRspTxInfo_p);
#endif				/* STA_ADHOC_SUPPORTED */
		...
		mlmeApiEventNotification(vStaInfo_p, MlmeStart_Cnfm, (UINT8 *) & vStaInfo_p->macMgmtMlme_ThisStaData.BssId, 0);
	...
	if (smeStateMgr_SndAuthCmd((UINT8 *) vStaInfo_p, &JoinCmd_p->BssDesc.BssId) == MLME_FAILURE) {
...
*
**************************************************************************/
extern void syncSrv_StartCmd(vmacStaInfo_t * vStaInfo_p, IEEEtypes_StartCmd_t * StartCmd_p)
{
	vmacEntry_t *vmacEntry_p;
	UINT8 newSsIdLen;
   ...
   *********************************************/
   ...
   /*------------------------------------------------------------------*/
	if (vStaInfo_p->macMgmtMain_State == STATE_SCANNING) {
		...
		return;
		    ...
		    vStaInfo_p->macMgmtMain_State == STATE_JOINED) && vStaInfo_p->macMgmtMlme_ThisStaData.CapInfo.Ibss) {
		/* Check that a network with the same SS ID has not already been started. */
		newSsIdLen = util_ListLen((UINT8 *) & (StartCmd_p->SsId), IEEEtypes_SSID_SIZE);

		if (newSsIdLen == vStaInfo_p->macMgmtMlme_ThisStaData.BssSsIdLen &&
		    !memcmp(vStaInfo_p->macMgmtMlme_ThisStaData.BssSsId, StartCmd_p->SsId, vStaInfo_p->macMgmtMlme_ThisStaData.BssSsIdLen)) {
   ...
   /*-------------------------------------------------*/
	syncSrv_UpdateAdhocTxInfo(vStaInfo_p,
				  StartCmd_p,
				  &(vStaInfo_p->macMgmtMlme_ThisStaData.BssId), IEEE_MSG_BEACON, vStaInfo_p->BcnBuffer_p, vStaInfo_p->BcnTxInfo_p);

	syncSrv_UpdateAdhocTxInfo(vStaInfo_p,
				  StartCmd_p,
				  &(vStaInfo_p->macMgmtMlme_ThisStaData.BssId),
				  IEEE_MSG_PROBE_RSP, vStaInfo_p->PrbRspBuf_p, vStaInfo_p->PrbRspTxInfo_p);
#endif				/* STA_ADHOC_SUPPORTED */
   ...
   /*-------------------------------------------*/
	if (StartCmd_p->PhyParamSet.DsParamSet.ElementId == DS_PARAM_SET) {
		mlmeApiSetRfChannel(vStaInfo_p, StartCmd_p->PhyParamSet.DsParamSet.CurrentChan, 1, TRUE);
	}
...
*
**************************************************************************/
extern void syncSrv_ResetCmd(vmacStaInfo_t * vStaInfo_p, IEEEtypes_ResetCmd_t * ResetCmd_p)
{
	vmacEntry_t *vmacEntry_p;
	...
	if (!ResetCmd_p->mode) {
		...
		if (vStaInfo_p->mib_WB_p->opMode == MLME_CMD_CLIENT_IBSS_START) {
			smeStateMgr_SndStartCmd((UINT8 *) vStaInfo_p, vStaInfo_p->bssDescProfile_p);
		} else {
			smeStateMgr_SndJoinCmd((UINT8 *) vStaInfo_p, vStaInfo_p->bssDescProfile_p);
		}
	}
	/* Notify SME of Reset */
...
{
	vmacEntry_t *vmacEntry_p;
	struct net_device *clientDev_p = NULL;
	struct wlprivate *priv_p = NULL;
	...
	/* Restore mib_ApMode when scan done if changed. */
	clientDev_p = mainNetdev_p[vmacEntry_p->phyHwMacIndx];
	priv_p = NETDEV_PRIV_P(struct wlprivate, clientDev_p);
	mib = priv_p->vmacSta_p->Mib802dot11;
	if ((*mib->mib_ApMode) != priv_p->vmacSta_p->lst_mib_ApMode)
		(*mib->mib_ApMode) = priv_p->vmacSta_p->lst_mib_ApMode;
	if ((*mib->mib_shortSlotTime) != priv_p->vmacSta_p->lst_mib_shortSlotTime)
		(*mib->mib_shortSlotTime) = priv_p->vmacSta_p->lst_mib_shortSlotTime;
#ifdef BRS_SUPPORT
	if ((*mib->BssBasicRateMask) != priv_p->vmacSta_p->lst_BssBasicRateMask)
		(*mib->BssBasicRateMask) = priv_p->vmacSta_p->lst_BssBasicRateMask;
	if ((*mib->NotBssBasicRateMask) != priv_p->vmacSta_p->lst_NotBssBasicRateMask)
		(*mib->NotBssBasicRateMask) = priv_p->vmacSta_p->lst_NotBssBasicRateMask;
#endif
}
...
extern void syncSrvSta_SetNextChannel(vmacStaInfo_t * vStaInfo_p)
{
...
#endif
	{
		...
		syncSrv_SndScanCnfm(vStaInfo_p,
				    ...
				    vStaInfo_p->ScanResultsLen, (IEEEtypes_BssDesc_t *) & vStaInfo_p->scanTableResult_p->ScanResults);
		return;
	}
#ifdef SC_PALLADIUM
	vStaInfo_p->ChanIdx++;
...
#ifndef SC_PALLADIUM
	mlmeApiSetRfChannel(vStaInfo_p, vStaInfo_p->ScanParams.ChanList[vStaInfo_p->ChanIdx], 1, TRUE);
#endif
...
void syncSrv_SetInitValues(vmacStaInfo_t * vStaInfo_p)
{
	...
	memset(&(vStaInfo_p->macMgmtMlme_ThisStaData.UsedRateSet), 0, sizeof(IEEEtypes_SuppRatesElement_t));
	memset(&(vStaInfo_p->macMgmtMlme_ThisStaData.OpRateSet), 0, sizeof(IEEEtypes_SuppRatesElement_t));
	memset(&(vStaInfo_p->macMgmtMlme_ThisStaData.CapInfo), 0, sizeof(IEEEtypes_CapInfo_t));
	...
	vStaInfo_p->macMgmtMlme_ThisStaData.OpRateSet.Len =
	    util_CopyList(vStaInfo_p->macMgmtMlme_ThisStaData.OpRateSet.Rates,
			  vStaInfo_p->staSystemMibs.mib_StaCfg_p->OpRateSet, IEEEtypes_MAX_DATA_RATES);
...
*
**************************************************************************/
static void syncSrv_adhocCheckDoRescan(vmacStaInfo_t * vStaInfo_p, dot11MgtFrame_t * MgmtMsg_p)
{
	UINT32 seed;
...
*
**************************************************************************/
static void syncSrv_SetBeaconTime(vmacStaInfo_t * vStaInfo_p, IEEEtypes_Bcn_t * Bcn_p)
{
	UINT64 intervals;
	UINT64 *bcnTime_p;
	UINT64 nextBcnTime;
   ...
   /*-------------------------------------------------------------*/
	nextBcnTime = (intervals * Bcn_p->BcnInterval * IEEEtypes_TIME_UNIT) + (INTERVAL_LOOK_AHEAD * Bcn_p->BcnInterval * IEEEtypes_TIME_UNIT);
...
extern void *syncSrv_ParseAttribWithinFrame(dot11MgtFrame_t * mgtFrame_p, UINT8 * data_p, UINT8 attrib)
{
	...
	lenOffset += (unsigned long)data_p - (unsigned long)(&(mgtFrame_p->Body))
...
*
**************************************************************************/
extern UINT32 syncSrv_AddAttrib(dot11MgtFrame_t * mgtMsg_p, UINT8 attribType, UINT8 * attribData, UINT8 attribLen)
{
	UINT32 curMsgLen;
	UINT8 *curData_p;
	UINT8 lenAdded = 0;
...
extern void AccumulateScanResults(vmacStaInfo_t * vStaInfo_p, void *BssData_p, UINT8 * rfHdr_p)
{
	...
	while (notFound && i < IEEEtypes_MAX_BSS_DESCRIPTS) {
		if (memcmp(((scanDescptHdr_t *) vStaInfo_p->scanTableResult_p->ScanResultsMap_p[i])->bssId,
			   emptyAddr, sizeof(IEEEtypes_MacAddr_t)) ||
		    ((scanDescptHdr_t *) vStaInfo_p->scanTableResult_p->ScanResultsMap_p[i])->length > 0) {
			/* Not Empty so let's see if Bssid Matched */
			if (!memcmp((((scanDescptHdr_t *) vStaInfo_p->scanTableResult_p->ScanResultsMap_p[i])->bssId),
				    ((dot11MgtFrame_t *) BssData_p)->Hdr.BssId, sizeof(IEEEtypes_MacAddr_t))) {
		...
		/********************************************************** 
				...
				while ((i >= IEEEtypes_MAX_BSS_DESCRIPTS ||
					(length_plus > MAX_SCAN_BUF_SIZE && vStaInfo_p->NumDescripts > 0)) && notFound) {
					weakest = 0;
					...
					if (local_RSSI < vStaInfo_p->scanTableResult_p->macMgmtMlme_SigQltyResults[weakest].RSSI) {
						...
						if (stepShift) {
							memcpy(vStaInfo_p->scanTableResult_p->ScanResultsMap_p[weakest],
							       ...
							       vStaInfo_p->scanTableResult_p->ScanResultsMap_p[weakest + 1]);
							memcpy((void *)&vStaInfo_p->scanTableResult_p->macMgmtMlme_SigQltyResults[weakest],
							...
							for (j = weakest + 1; j < vStaInfo_p->NumDescripts - 1; j++) {
								vStaInfo_p->scanTableResult_p->ScanResultsMap_p[j] =
								    vStaInfo_p->scanTableResult_p->ScanResultsMap_p[j + 1] - bufShift;
							}
						}
						memset(vStaInfo_p->ScanResults_p - bufShift, 0, bufShift);
						memset(&vStaInfo_p->scanTableResult_p->macMgmtMlme_SigQltyResults[vStaInfo_p->NumDescripts - 1],
						       0, sizeof(macMgmtMlme_SigQltySet_t));
						vStaInfo_p->ScanResultsLen -= bufShift;
						vStaInfo_p->ScanResults_p -= bufShift;
						vStaInfo_p->scanTableResult_p->ScanResultsMap_p[vStaInfo_p->NumDescripts - 1] =
						    vStaInfo_p->ScanResults_p;
						i--;
						vStaInfo_p->NumDescripts--;
						length_plus = scanDesctLen + vStaInfo_p->ScanResultsLen;
		...
		memcpy(&vStaInfo_p->scanTableResult_p->BssSourceAddr[i],
		       &(((dot11MgtFrame_t *) BssData_p)->Hdr.SrcAddr), sizeof(IEEEtypes_MacAddr_t));
		vStaInfo_p->NumDescripts++;
		...
		/*Pack in the BssId */
		memcpy((void *)vStaInfo_p->ScanResults_p, (void *)((dot11MgtFrame_t *) BssData_p)->Hdr.BssId, sizeof(IEEEtypes_MacAddr_t));
...
*
**************************************************************************/
extern void syncSrv_BncRecvAssociatedHandler(vmacStaInfo_t * vStaInfo_p, dot11MgtFrame_t * MgmtMsg_p, UINT8 * rfHdr_p)
{
	void *attrib_p;
#ifdef STA_QOS
...
//  UINT32                 capInfo;
	...
	/* Measurements in -dBm. */
	if ((vStaInfo_p->linkInfo.wStats.qual.level < vStaInfo_p->linkInfo.max_qual.level) || (vStaInfo_p->linkInfo.max_qual.level == 0)) {
		vStaInfo_p->linkInfo.max_qual.level = vStaInfo_p->linkInfo.wStats.qual.level;
	}
	if ((vStaInfo_p->linkInfo.wStats.qual.noise < vStaInfo_p->linkInfo.max_qual.noise) || (vStaInfo_p->linkInfo.max_qual.noise == 0)) {
	...
	if (vStaInfo_p->macMgmtMlme_ThisStaData.BssType == BSS_INDEPENDENT) {
		...
		if (!vStaInfo_p->macMgmtMlme_ThisStaData.AP_gRateLen && vStaInfo_p->macMgmtMlme_ThisStaData.AP_RateLen <= MAX_B_DATA_RATES) {
		...
		if ((attrib_p = syncSrvSta_ParseAttrib(MgmtMsg_p, ERP_INFO)) != NULL) {
			...
			if (!(*mib->mib_forceProtectiondisable)) {
				if (vStaInfo_p->Station_p->ChipCtrl.Protection4g)
					...
					protection4g = 0;
				if (stapriv->master)
					wlFwSetGProt(stapriv->master, protection4g);
			}
		}
		...
		if (vStaInfo_p->staSystemMibs.mib_StaCfg_p->sta11hMode) {
			...
			if ((attrib_p = syncSrvSta_ParseAttrib(MgmtMsg_p, CSA)) != NULL) {
				UINT8 channel = ((IEEEtypes_ChannelSwitchAnnouncementElement_t *) attrib_p)->Channel;
				UINT8 count = ((IEEEtypes_ChannelSwitchAnnouncementElement_t *) attrib_p)->Count;
				UINT8 mode = ((IEEEtypes_ChannelSwitchAnnouncementElement_t *) attrib_p)->Mode;
				...
				if (vStaInfo_p->station11hTimerFired == 0) {
					...
					TimerFireIn(&vStaInfo_p->station11hTimer, 1, station11hTimerCB, (unsigned char *)vStaInfo_p, tickCount);
				} else {
...
extern SINT32 syncSrv_LinkLostHandler(vmacStaInfo_t * vStaInfo_p)
{
	...
	if (vStaInfo_p->AssociatedFlag) {
		authSrv_SndDeAuthMsg(vStaInfo_p,
				     &vStaInfo_p->macMgmtMlme_ThisStaData.BssId,
				     &vStaInfo_p->macMgmtMlme_ThisStaData.BssId, IEEEtypes_REASON_DEAUTH_LEAVING);
		vStaInfo_p->AssociatedFlag = 0;
#ifdef SOC_W8964
		vStaInfo_p->in_obss_scan = FALSE;
#endif
...
*
**************************************************************************/
extern void syncSrv_ScanFilter(vmacStaInfo_t * vStaInfo_p, dot11MgtFrame_t * MgmtMsg_p, UINT8 * rfHdr_p)
{
	UINT8 ssidLen;
	UINT8 tmpBufSSID[IEEEtypes_SSID_SIZE];
	UINT16 parseLen = 0;
	...
	if (vStaInfo_p->scanFilterMap) {
		...
		if (vStaInfo_p->scanFilterMap & MLME_BSSID_FILTER) {
			if (memcmp(&MgmtMsg_p->Hdr.BssId[0], &vStaInfo_p->ScanParams.BssId[0], sizeof(IEEEtypes_MacAddr_t))) {
				return;
			}
		}
		/* SSID Filter */
		if (vStaInfo_p->scanFilterMap & MLME_SSID_FILTER) {
			...
			if ((ssidLen != MgmtMsg_p->Body.ProbeRsp.SsId.Len)
			    || memcmp(MgmtMsg_p->Body.ProbeRsp.SsId.SsId, vStaInfo_p->ScanParams.SsId, ssidLen)) {
				return;
			}
		}
		/* BSS Type Filter */
		if (vStaInfo_p->scanFilterMap & MLME_BSS_TYPE_FILTER) {
			if ((vStaInfo_p->ScanParams.BssType == BSS_INFRASTRUCTURE) && !MgmtMsg_p->Body.ProbeRsp.CapInfo.Ess) {
				return;
			} else if ((vStaInfo_p->ScanParams.BssType == BSS_INDEPENDENT) && !MgmtMsg_p->Body.ProbeRsp.CapInfo.Ibss) {
				return;
			}
		}
			...
			if (wpa2IE_p || wpaIE_p) {
				if (!vStaInfo_p->staSecurityMibs.mib_PrivacyTable_p->RSNEnabled) {
					if (vStaInfo_p->staSystemMibs.mib_StaCfg_p->wpawpa2Mode != 16)
						...
						return;
				} else if (vStaInfo_p->staSecurityMibs.mib_RSNConfigWPA2_p->WPA2OnlyEnabled) {
					if (!wpa2IE_p) {
...
extern SINT32 syncSrv_SetStatusTimer(vmacStaInfo_t * vStaInfo_p, UINT8 status)
{
	...
	macIndex = mlmeApiGetMacIndex(vStaInfo_p);
	if (status == 1) {
		mlmeApiStartTimer(vStaInfo_p, (UINT8 *) & vStaInfo_p->statusTimer, &syncSrv_StatusCheckTimeOut, g_CheckStatusTime[macIndex]);
	} else if (status == 0) {
		vStaInfo_p->urProbeRspMissedCnt = 0;
		ClientModeDataCount[macIndex] = 0;
		mlmeApiStopTimer(vStaInfo_p, (UINT8 *) & vStaInfo_p->statusTimer);
	}
	return MLME_SUCCESS;
}
...
*
**************************************************************************/
extern SINT32 syncSrv_SetKeepAliveTimer(vmacStaInfo_t * vStaInfo_p, UINT8 status)
{
	if (!g_KeepAliveEnable)
		return MLME_SUCCESS;

	if (status == 1) {
		mlmeApiStartTimer(vStaInfo_p, (UINT8 *) & vStaInfo_p->keepaliveTimer, &syncSrv_KeepAliveTimeOut, g_KeepAliveTime);
	} else if (status == 0) {
		mlmeApiStopTimer(vStaInfo_p, (UINT8 *) & vStaInfo_p->keepaliveTimer);
	}
	return MLME_SUCCESS;
}
...
extern void SyncSrvSta_AccumulateSiteSurveyResults(vmacStaInfo_t * vStaInfo_p, void *BssData_p, UINT8 * rfHdr_p)
{
	UINT32 i;
	BOOLEAN notFound = TRUE;
	...
	for (i = 0; (i < SITE_SURVEY_ENTRY_MAX) && notFound; i++) {
		if (!vStaInfo_p->sSurveyTable_p->siteSurveyInfo[i].dirty) {
			...
			notFound = FALSE;
		} else if (!memcmp(vStaInfo_p->sSurveyTable_p->siteSurveyInfo[i].BssId, ((dot11MgtFrame_t *) BssData_p)->Hdr.BssId, 6)) {
			/* Record RSSI */
...
extern SINT32 syncSrv_CmdExceptionHandler(vmacStaInfo_t * vStaInfo_p)
{
	if (vStaInfo_p->cmdHistory & cmdScan) {
		syncSrv_SndScanCnfm(vStaInfo_p, SCAN_RESULT_UNEXPECTED_ERROR, 0, 0, NULL);
	}
...
*
**************************************************************************/
extern void syncSrv_PrbeRspNoBssHandler(vmacStaInfo_t * vStaInfo_p, dot11MgtFrame_t * MgmtMsg_p, UINT8 * rfHdr_p)
{
	/* Check BSSID */
	if (memcmp(&MgmtMsg_p->Hdr.BssId[0], &vStaInfo_p->bssDescProfile_p->BssId[0], sizeof(IEEEtypes_MacAddr_t))) {
		return;
	}
	if (syncSrv_ValidatePeerSetting(vStaInfo_p, MgmtMsg_p) == FALSE) {
		mlmeApiEventNotification(vStaInfo_p, PeerDetectInCompat, &vStaInfo_p->bssDescProfile_p->BssId[0], 0);
		return;
	}
	smeStateMgr_SndJoinCmd((UINT8 *) vStaInfo_p, vStaInfo_p->bssDescProfile_p);
}
