...
extern SINT32 mlmeApiUpdateBasicRatesSetting(vmacStaInfo_t * vStaInfo_p, UINT8 * rateBuf_p, UINT32 rateBufLen);
...
*
**************************************************************************/
static UINT32 smeStateMgr_SetCmd(UINT8 * info_p, UINT32 curCmd)
{
...
*
**************************************************************************/
static UINT32 smeStateMgr_UnSetCmd(UINT8 * info_p, UINT32 curCmd)
{
	vmacStaInfo_t *vStaInfo_p = (vmacStaInfo_t *) info_p;
	vStaInfo_p->cmdHistory &= ~curCmd;
	return vStaInfo_p->cmdHistory;
...
static SINT32 smeNotifyApSrv(UINT8 * info_p)
{
	...
	if (!vStaInfo_p->isParentSession || (vStaInfo_p->macMgmtMlme_ThisStaData.CapInfo.Ibss)) {
		return -1;
	}
	...
	if (curLen) {
		curLen = (curLen < MAX_B_DATA_RATES) ? curLen : MAX_B_DATA_RATES;
		memcpy((void *)&rateBuf[rateBufLen], (void const *)&vStaInfo_p->bOpRateSet[0], curLen);
		rateBufLen += curLen;
	}
	/* Copy over 11G rates */
	curLen = util_ListLen(vStaInfo_p->gOpRateSet, MAX_G_DATA_RATES);
	if (curLen) {
		curLen = (curLen < MAX_G_DATA_RATES) ? curLen : MAX_G_DATA_RATES;
		memcpy((void *)&rateBuf[rateBufLen], (void const *)&vStaInfo_p->gOpRateSet[0], curLen);
		rateBufLen += curLen;
	}
...
extern void smeStateMgr_ScanCfrm(UINT8 * info_p, macmgmtQ_CmdRsp_t * MgmtMsg_p)
{
	...
	UINT32 size;
	UINT8 *sorted_p;
	static UINT32 SigQltyMark[IEEEtypes_MAX_BSS_DESCRIPTS];
	static macMgmtMlme_SigQltySet_t SigQltyTmp[IEEEtypes_MAX_BSS_DESCRIPTS];
	...
	for (i = 0; (i < IEEEtypes_MAX_BSS_DESCRIPTS) && (i < vStaInfo_p->NumDescripts); i++) {
		SigQltyMark[i] = 0;
		memcpy((void *)&SigQltyTmp[i],
		       (void *)&vStaInfo_p->scanTableResult_p->macMgmtMlme_SigQltyResults[i], sizeof(macMgmtMlme_SigQltySet_t));
	}
	...
	for (i = 0; (i < IEEEtypes_MAX_BSS_DESCRIPTS) && (i < vStaInfo_p->NumDescripts); i++) {
		...
		for (j = k + 1; (j < IEEEtypes_MAX_BSS_DESCRIPTS) && (j < vStaInfo_p->NumDescripts); j++) {
			if (SigQltyMark[j]) {
				continue;
			}
		...
		if (strongest == vStaInfo_p->NumDescripts - 1) {
			...
			size =
			    vStaInfo_p->scanTableResult_p->ScanResultsMap_p[strongest + 1] -
			    vStaInfo_p->scanTableResult_p->ScanResultsMap_p[strongest];
		}
		memcpy(sorted_p,
		       //vStaInfo_p->scanTableResult_p->ScanResultsMap_p[strongest] -2, 
		       vStaInfo_p->scanTableResult_p->ScanResultsMap_p[strongest], size);
		sorted_p += size;
		memcpy((void *)&vStaInfo_p->scanTableResult_p->macMgmtMlme_SigQltyResults[i],
		       (void *)(&SigQltyTmp[strongest]), sizeof(macMgmtMlme_SigQltySet_t));

	}
	memcpy((void *)&vStaInfo_p->scanTableResult_p->ScanResults[0],
	       (void *)&ScanResults_tmp[0], (UINT8 *) vStaInfo_p->ScanResults_p - (UINT8 *) & vStaInfo_p->scanTableResult_p->ScanResults[0]);

	smeStateMgr_UnSetCmd(info_p, cmdScan);

	memcpy((void *)vStaInfo_p->BssDescSet_p, &(MgmtMsg_p->Msg.ScanCfrm.Rsp.BssDescSet), MgmtMsg_p->Msg.ScanCfrm.BufSize);
   ...
   /*-----------------------------------------------------------------*/
	...
	fixedSize = sizeof(IEEEtypes_MacAddr_t) +
	    ...
	    sizeof(IEEEtypes_Bss_t) + sizeof(IEEEtypes_BcnInterval_t) + sizeof(IEEEtypes_DtimPeriod_t) + 2 * sizeof(IEEEtypes_TimeStamp_t);
	...
	mlmeApiEventNotification(vStaInfo_p, MlmeScan_Cnfm, (UINT8 *) & vStaInfo_p->macMgmtMlme_ThisStaData.BssId[0], 0);
	return;
 ...
 *
 *****************************************************************************/
extern void smeStateMgr_StartCfrm(UINT8 * info_p, macmgmtQ_CmdRsp_t * MgmtMsg_p)
{
	vmacStaInfo_t *vStaInfo_p = (vmacStaInfo_t *) info_p;
	...
	if (MgmtMsg_p->Msg.StartCfrm.Result == START_RESULT_SUCCESS) {
		...
		memcpy(&(vStaInfo_p->JoinAddr), vStaInfo_p->macMgmtMlme_ThisStaData.BssId, sizeof(IEEEtypes_MacAddr_t));
 ...
 *
 *****************************************************************************/
extern SINT32 smeStateMgr_JoinCfrm(UINT8 * info_p, macmgmtQ_CmdRsp_t * MgmtMsg_p)
{
	vmacStaInfo_t *vStaInfo_p = (vmacStaInfo_t *) info_p;
	    ...
	    /*===============================================================*/
	       ...
	       /*-----------------------------------------------------*/
				if ((macMgmtMsg_p = (macmgmtQ_CmdReq_t *) mlmeApiAllocSmeMsg()) != NULL) {
					...
					memcpy(macMgmtMsg_p, &vStaInfo_p->LastJoinMsg, sizeof(macmgmtQ_CmdReq_t));

					memcpy((void *)&macMgmtMsg_p->targetAddr[0],
					       (const void *)&vmacEntry_p->vmacAddr[0], sizeof(IEEEtypes_MacAddr_t));
 ...
 *
 *****************************************************************************/
extern void smeStateMgr_AuthenticateCfrm(UINT8 * info_p, macmgmtQ_CmdRsp_t * MgmtMsg_p)
{
	vmacStaInfo_t *vStaInfo_p = (vmacStaInfo_t *) info_p;
	...
	if (!memcmp(MgmtMsg_p->Msg.AuthCfrm.PeerStaAddr, vStaInfo_p->macMgmtMlme_ThisStaData.BssId, sizeof(IEEEtypes_MacAddr_t))) {
	    /*------------------------------------------------------------*/
		...
		if (MgmtMsg_p->Msg.AuthCfrm.Result == AUTH_RESULT_SUCCESS) {
			...
			if ((macMgmtMsg_p = (macmgmtQ_CmdReq_t *) mlmeApiAllocSmeMsg()) != NULL) {
				...
				memcpy((void *)&macMgmtMsg_p->targetAddr[0], (const void *)&vmacEntry_p->vmacAddr[0], sizeof(IEEEtypes_MacAddr_t));
				...
				memcpy(&(macMgmtMsg_p->Body.AssocCmd.PeerStaAddr), MgmtMsg_p->Msg.AuthCfrm.PeerStaAddr, sizeof(IEEEtypes_MacAddr_t));
		  ...
		  /*------------------------------------------------*/
				if ((macMgmtMsg_p = (macmgmtQ_CmdReq_t *) mlmeApiAllocSmeMsg()) != NULL) {
					memcpy(macMgmtMsg_p, &vStaInfo_p->LastAuthMsg, sizeof(macmgmtQ_CmdReq_t));
 ...
 *
 *****************************************************************************/
extern void smeStateMgr_AuthenticateInd(UINT8 * info_p, macmgmtQ_CmdRsp_t * MgmtMsg_p)
{
   ...
   /*---------------------------------------------------------------------*/
	...
	/* state.                                                              */
   /*---------------------------------------------------------------------*/
	if (!memcmp(MgmtMsg_p->Msg.AuthInd.PeerStaAddr, vStaInfo_p->macMgmtMlme_ThisStaData.BssId, sizeof(IEEEtypes_MacAddr_t))) {
		vStaInfo_p->smeMain_State = STATE_AUTHENTICATED_WITH_AP;
	}
 ...
 *
 *****************************************************************************/
extern void smeStateMgr_AssociateCfrm(UINT8 * info_p, macmgmtQ_CmdRsp_t * MgmtMsg_p)
{
	vmacStaInfo_t *vStaInfo_p = (vmacStaInfo_t *) info_p;
	UINT8 macIndx;
		...
		if (vStaInfo_p->AssocRetryCount < MAX_ASSOC_RETRIES) {
			if ((macMgmtMsg_p = (macmgmtQ_CmdReq_t *) mlmeApiAllocSmeMsg()) != NULL) {
				...
				memcpy(&(macMgmtMsg_p->Body.AssocCmd.PeerStaAddr),
				       &vStaInfo_p->macMgmtMlme_ThisStaData.BssId, sizeof(IEEEtypes_MacAddr_t));
 ...
 *
 *****************************************************************************/
extern void smeStateMgr_AssociateInd(UINT8 * info_p, macmgmtQ_CmdRsp_t * MgmtMsg_p)
{

	vmacStaInfo_t *vStaInfo_p = (vmacStaInfo_t *) info_p;

	if (!memcmp(MgmtMsg_p->Msg.AssocInd.PeerStaAddr, vStaInfo_p->macMgmtMlme_ThisStaData.BssId, sizeof(IEEEtypes_MacAddr_t))) {
		vStaInfo_p->smeMain_State = STATE_ASSOCIATED;
		mlmeApiUpdateLinkStatus(vStaInfo_p, LINK_UP);
	}
 ...
 *
 *****************************************************************************/
extern void smeStateMgr_DeauthenticateCfrm(UINT8 * info_p, macmgmtQ_CmdRsp_t * MgmtMsg_p)
{
	vmacStaInfo_t *vStaInfo_p = (vmacStaInfo_t *) info_p;
	vmacEntry_t *vmacEntry_p;
   ...
   /*----------------------------------------------------------------*/
	if (!memcmp(MgmtMsg_p->Msg.DeauthCfrm.PeerStaAddr, vStaInfo_p->macMgmtMlme_ThisStaData.BssId, sizeof(IEEEtypes_MacAddr_t))) {
		if (MgmtMsg_p->Msg.DeauthCfrm.Result == DEAUTH_RESULT_SUCCESS) {
			...
			memset(&(vStaInfo_p->macMgmtMlme_ThisStaData.BssSourceAddr), 0, sizeof(IEEEtypes_MacAddr_t));
			...
			memset(&(vStaInfo_p->macMgmtMlme_ThisStaData.BssSsId), 0, sizeof(IEEEtypes_SsId_t));
 ...
 *
 *****************************************************************************/
extern void smeStateMgr_DeauthenticateInd(UINT8 * info_p, macmgmtQ_CmdRsp_t * MgmtMsg_p)
{
	vmacStaInfo_t *vStaInfo_p = (vmacStaInfo_t *) info_p;
	vmacEntry_t *vmacEntry_p;
#ifdef AMPDU_SUPPORT_TX_CLIENT
   ...
   /*--------------------------------------------------------------*/
	...
	/* we know about, then change the state.                        */
   /*--------------------------------------------------------------*/
	if (!memcmp(MgmtMsg_p->Msg.DeauthInd.PeerStaAddr, vStaInfo_p->macMgmtMlme_ThisStaData.BssId, sizeof(IEEEtypes_MacAddr_t))) {
		...
		cleanupAmpduTx(vmacSta_p, (UINT8 *) & vStaInfo_p->macMgmtMlme_ThisStaData.BssId[0]);
#endif
		if ((vStaInfo_p->AssociatedFlag) && (vStaInfo_p->keyMgmtInfoSta_p->sta_MIC_Error.status == NO_MIC_FAILURE)) {
 ...
 *
 *****************************************************************************/
extern void smeStateMgr_DisassociateCfrm(UINT8 * info_p, macmgmtQ_CmdRsp_t * MgmtMsg_p)
{
	vmacStaInfo_t *vStaInfo_p = (vmacStaInfo_t *) info_p;
	vmacEntry_t *vmacEntry_p;
 ...
 *
 *****************************************************************************/
extern void smeStateMgr_DisassociateInd(UINT8 * info_p, macmgmtQ_CmdRsp_t * MgmtMsg_p)
{
	vmacStaInfo_t *vStaInfo_p = (vmacStaInfo_t *) info_p;
	vmacEntry_t *vmacEntry_p;
#ifdef AMPDU_SUPPORT_TX_CLIENT
   ...
   /*------------------------------------------------------------*/
	...
	/* we know about, then change the state.                      */
   /*------------------------------------------------------------*/
	if (!memcmp(MgmtMsg_p->Msg.DisassocInd.PeerStaAddr, vStaInfo_p->macMgmtMlme_ThisStaData.BssId, sizeof(IEEEtypes_MacAddr_t))) {
		if (smePendingCmd(info_p)) {
			syncSrv_CmdExceptionHandler(vStaInfo_p);
		}
		if (vStaInfo_p->smeMain_State == STATE_SCANNING || vStaInfo_p->smeMain_State == STATE_RESTORING_FROM_SCAN) {
			if (vStaInfo_p->PostScanState == STATE_ASSOCIATED || vStaInfo_p->PostScanState == STATE_REASSOCIATING) {
				vStaInfo_p->PostScanState = STATE_AUTHENTICATED_WITH_AP;
			}
		} else if (vStaInfo_p->smeMain_State == STATE_ASSOCIATED || vStaInfo_p->smeMain_State == STATE_REASSOCIATING) {
			vStaInfo_p->smeMain_State = STATE_AUTHENTICATED_WITH_AP;
		}
		if (vStaInfo_p->isParentSession) {
			// Set the trunkid as INACTIVE so that no data traffic is sent      
 ...
 *
 *****************************************************************************/
extern void smeStateMgr_ReassociateInd(UINT8 * info_p, macmgmtQ_CmdRsp_t * MgmtMsg_p)
{
	vmacStaInfo_t *vStaInfo_p = (vmacStaInfo_t *) info_p;
 ...
 *
 *****************************************************************************/
extern void smeStateMgr_ResetCfrm(UINT8 * info_p, macmgmtQ_CmdRsp_t * MgmtMsg_p)
{
	vmacStaInfo_t *vStaInfo_p = (vmacStaInfo_t *) info_p;

	mlmeApiEventNotification(vStaInfo_p, MlmeReset_Cnfm, &vStaInfo_p->macMgmtMlme_ThisStaData.BssId[0], 0);
	return;
...
*
**************************************************************************/
extern SINT32 smeStateMgr_SndAuthCmd(UINT8 * info_p, IEEEtypes_MacAddr_t * peerAddr)
{
	vmacStaInfo_t *vStaInfo_p = (vmacStaInfo_t *) info_p;
	macmgmtQ_CmdReq_t *macMgmtMsg_p;
	vmacEntry_t *vmacEntry_p;
	...
	memcpy(&(macMgmtMsg_p->Body.AuthCmd.PeerStaAddr), peerAddr, sizeof(IEEEtypes_MacAddr_t));

	vStaInfo_p->AuthRetryCount = 0;
	memcpy(&(vStaInfo_p->LastAuthMsg), macMgmtMsg_p, sizeof(macmgmtQ_CmdReq_t));
	if (!mlmeApiSendSmeMsg(macMgmtMsg_p)) {
		mlmeApiFreeSmeMsg((macmgmtQ_CmdBuf_t *) macMgmtMsg_p);
		return MLME_FAILURE;
	}
...
//        clientType = (vmacEntry_p->phyHwMacIndx == MAC_1)?LiberoClient:DaemonClient;
	...
	memcpy((void *)&vmacEntry_p->vmacAddr[0], (const void *)&parentMacAddr[0], IEEEtypes_ADDRESS_SIZE);
	...
	clientSrvId = vmacGetClientSrvId(vmacEntry_p->phyHwMacIndx);
...
SINT32 smeStopBss(UINT8 phyMacIndx)
{
	...
	if (vStaInfo_p->AssociatedFlag && !vStaInfo_p->mib_WB_p->opMode) {
		...
		if (authSrv_SndDeAuthMsg(vStaInfo_p,
					 ...
					 &vStaInfo_p->macMgmtMlme_ThisStaData.BssId, IEEEtypes_REASON_DEAUTH_LEAVING) == MLME_SUCCESS) {
...
SINT32 smeApiValidateChildModeChange(UINT8 phyMacIndx)
{
	...
	clientSrvId = vmacGetClientSrvId(phyMacIndx);
...
SINT32 smeSendResetCmd_Sta(UINT8 macIndex, int quiet)
{
	...
	if ((resetMsg_p = (macmgmtQ_CmdReq_t *) mlmeApiAllocSmeMsg()) != NULL) {
		memcpy((void *)&resetMsg_p->targetAddr[0], (const void *)&vmacEntry_p->vmacAddr[0], sizeof(IEEEtypes_MacAddr_t));
...
extern BOOLEAN smeGetStaLinkInfo(vmacId_t mlme_vMacId, UINT8 * AssociatedFlag_p, UINT8 * bssId_p)
{
	...
	*AssociatedFlag_p = vStaInfo_p->AssociatedFlag;
	memcpy(bssId_p, &vStaInfo_p->macMgmtMlme_ThisStaData.BssId[0], sizeof(IEEEtypes_MacAddr_t));
...
*
**************************************************************************/
extern SINT32 smeStateMgr_SndJoinCmd(UINT8 * info_p, IEEEtypes_BssDesc_t * bssDesc_p)
{
	vmacStaInfo_t *vStaInfo_p = (vmacStaInfo_t *) info_p;
	macmgmtQ_CmdReq_t *macMgmtMsg_p;
	vmacEntry_t *vmacEntry_p;
...
#ifdef WPA_STA
	if (vStaInfo_p->staSecurityMibs.mib_PrivacyTable_p->RSNEnabled
	    && vStaInfo_p->keyMgmtInfoSta_p->sta_MIC_Error.MICCounterMeasureEnabled && vStaInfo_p->keyMgmtInfoSta_p->sta_MIC_Error.disableStaAsso) {
		return MLME_FAILURE;
	}
#endif				/* WPA_STA */
	...
	memcpy((void *)&macMgmtMsg_p->Body.JoinCmd.BssDesc, (const void *)bssDesc_p, sizeof(IEEEtypes_BssDesc_t));
	...
	memcpy(&(vStaInfo_p->LastJoinMsg), macMgmtMsg_p, sizeof(macmgmtQ_CmdReq_t));
...
*
**************************************************************************/
extern IEEEtypes_InfoElementHdr_t *smeParseExtIeType(UINT8 extType, UINT8 * ieBuf_p, UINT16 ieBufLen)
{
	UINT8 *ieBufEnd_p;
	UINT8 *ieCurrent_p;
	IEEEtypes_InfoElementHdr_t *IE_p;
	IEEEtypes_InfoElementExtHdr_t *IEext_p;
	...
	while (ieCurrent_p < ieBufEnd_p) {
		IE_p = (IEEEtypes_InfoElementHdr_t *) ieCurrent_p;
		if (IE_p->ElementId == EXT_IE) {
			IEext_p = (IEEEtypes_InfoElementExtHdr_t *) IE_p;
			if (IEext_p->ext == extType)
				return IE_p;
		}
		ieCurrent_p += (IE_p->Len + sizeof(IEEEtypes_InfoElementHdr_t));
	}
...
extern SINT32 smeSetBssProfile(UINT8 macIndex, UINT8 * bssid, IEEEtypes_CapInfo_t * capInfo, UINT8 * ieBuf_p, UINT16 ieBufLen, BOOLEAN isApMrvl)
{
	...
	struct wlprivate *stapriv = NULL;
	vmacApInfo_t *vmacSta_p = NULL;
	struct wlprivate *parent_wlpptr = NULL;
	int opmode_changed = 0;
#ifdef SOC_W8964
	UINT8 wpa2_enable = 0;
#endif
	...
	if (vStaInfo_p) {
		...
		stapriv = NETDEV_PRIV_P(struct wlprivate, staDev);
#ifdef SOC_W906X
		parent_wlpptr = GET_PARENT_PRIV(stapriv);
#endif				/* SOC_W906X */
		vmacSta_p = (vmacApInfo_t *) stapriv->vmacSta_p;
		...
		rateOffset = 0;
		/* Parse here */
		if ((IE_p = smeParseIeType(SSID, ieBuf_p, ieBufLen)) == NULL) {
			return MLME_FAILURE;
		}
		...
		util_CopyList(vStaInfo_p->staSystemMibs.mib_StaCfg_p->DesiredSsId, &ssidBuf[0], IEEEtypes_SSID_SIZE);

		if ((IE_p = smeParseIeType(DS_PARAM_SET, ieBuf_p, ieBufLen)) == NULL) {
			return MLME_FAILURE;
		}
		if (IE_p->Len > 1) {
			return MLME_FAILURE;
		...
		if ((IE_p = smeParseIeType(SUPPORTED_RATES, ieBuf_p, ieBufLen)) == NULL) {
			return MLME_FAILURE;
		}
		if (IE_p->Len > 14) {
			return MLME_FAILURE;
		...
		if ((rateOffset < 14) && (IE_p = smeParseIeType(EXT_SUPPORTED_RATES, ieBuf_p, ieBufLen)) != NULL) {
			if ((IE_p->Len + rateOffset) > 14) {
				memcpy(&rateBuf[rateOffset], (UINT8 *) IE_p + sizeof(IEEEtypes_InfoElementHdr_t),
				       (rateOffset > 14) ? 0 : 14 - rateOffset);
			} else {
				memcpy(&rateBuf[rateOffset], (UINT8 *) IE_p + sizeof(IEEEtypes_InfoElementHdr_t), IE_p->Len);
			}
		}
		...
		/* HT Element */
		if ((IE_p = smeParseIeType(HT, ieBuf_p, ieBufLen)) != NULL) {
		...
		if ((IE_p = smeParseIeType(ADD_HT, ieBuf_p, ieBufLen)) != NULL) {
		...
		/*VHT Element */
		if ((IE_p = smeParseIeType(VHT_CAP, ieBuf_p, ieBufLen)) != NULL) {
		...
		if ((IE_p = smeParseIeType(VHT_OPERATION, ieBuf_p, ieBufLen)) != NULL) {
...
#ifdef SOC_W906X
		if ((IE_p = smeParseExtIeType(HE_CAPABILITIES_IE, ieBuf_p, ieBufLen)) != NULL) {
			if (!((IE_p->Len + sizeof(IEEEtypes_InfoElementHdr_t)) > sizeof(HE_Capabilities_IE_t)))
				memcpy(&vStaInfo_p->bssDescProfile_p->hecap, (UINT8 *) IE_p, IE_p->Len + sizeof(IEEEtypes_InfoElementHdr_t));
		}

		if ((IE_p = smeParseExtIeType(HE_OPERATION_IE, ieBuf_p, ieBufLen)) != NULL) {
			if (!((IE_p->Len + sizeof(IEEEtypes_InfoElementHdr_t)) > sizeof(HE_Operation_IE_t)))
				memcpy(&vStaInfo_p->bssDescProfile_p->heop, (UINT8 *) IE_p, IE_p->Len + sizeof(IEEEtypes_InfoElementHdr_t));
		}
#endif				/* SOC_W906X */
		/* RSN Element (WPA2) */
		if ((IE_p = smeParseIeType(RSN_IEWPA2, ieBuf_p, ieBufLen)) != NULL) {
#ifdef SOC_W8964
			wpa2_enable = 1;
#endif
...
#ifdef SOC_W8964
					IEEEtypes_RSN_IE_t *pWPA_IE = NULL;
					if ((0 == wpa2_enable) &&
					    (pWPA_IE = linkMgtParseWpaIe((UINT8 *) IE_p, IE_p->Len)) &&
					    (1 == pWPA_IE->PwsKeyCnt[0]) &&
					    (RSN_TKIP_ID == pWPA_IE->PwsKeyCipherList[3]) && (vStaInfo_p->bssDescProfile_p->HTElement.Len > 0)) {
						/* If AP is HT but RSN is WPA-TKIP only, STA considers it as a legacy AP by empty HT element */
						printk("AP is HT and WPA-TKIP only \n");
						vStaInfo_p->bssDescProfile_p->HTElement.Len = 0;	//add this then STA asso_req won't include HT IE
					}
#endif
					memcpy(&vStaInfo_p->bssDescProfile_p->vendorBuf[addedVendorLen], (UINT8 *) IE_p,
					       IE_p->Len + sizeof(IEEEtypes_InfoElementHdr_t));
...
#ifdef SOC_W906X
		if (channel <= 14) {
			if (*(parent_wlpptr->vmacSta_p->Mib802dot11->mib_ApMode) & AP_MODE_A_ONLY) {
				*(parent_wlpptr->vmacSta_p->Mib802dot11->mib_ApMode) &= ~AP_MODE_A_ONLY;
				*(parent_wlpptr->vmacSta_p->Mib802dot11->mib_ApMode) |= AP_MODE_B_ONLY | AP_MODE_G_ONLY;
				*(parent_wlpptr->vmacSta_p->Mib802dot11->BssBasicRateMask) = MRVL_BSSBASICRATEMASK_BGN;
				*(parent_wlpptr->vmacSta_p->Mib802dot11->NotBssBasicRateMask) = MRVL_NOTBSSBASICRATEMASK_BGN;
				*(parent_wlpptr->vmacSta_p->Mib802dot11->mib_shortSlotTime) = TRUE;
				*(parent_wlpptr->vmacSta_p->Mib802dot11->mib_ApMode) &= ~AP_MODE_A_ONLY;
				*(parent_wlpptr->vmacSta_p->Mib802dot11->mib_ApMode) |= AP_MODE_B_ONLY | AP_MODE_G_ONLY;
				*(stapriv->vmacSta_p->Mib802dot11->mib_ApMode) &= ~AP_MODE_A_ONLY;
				*(stapriv->vmacSta_p->Mib802dot11->mib_ApMode) |= AP_MODE_B_ONLY | AP_MODE_G_ONLY;
				*(stapriv->vmacSta_p->Mib802dot11->BssBasicRateMask) = MRVL_BSSBASICRATEMASK_BGN;
				*(stapriv->vmacSta_p->Mib802dot11->NotBssBasicRateMask) = MRVL_NOTBSSBASICRATEMASK_BGN;
				*(stapriv->vmacSta_p->Mib802dot11->mib_shortSlotTime) = TRUE;
				opmode_changed = 1;
			}
		} else if (!(*(parent_wlpptr->vmacSta_p->Mib802dot11->mib_ApMode) & AP_MODE_A_ONLY)) {
			*(parent_wlpptr->vmacSta_p->Mib802dot11->mib_ApMode) &= ~(AP_MODE_B_ONLY | AP_MODE_G_ONLY);
			*(parent_wlpptr->vmacSta_p->Mib802dot11->mib_ApMode) |= AP_MODE_A_ONLY;
			*(parent_wlpptr->vmacSta_p->Mib802dot11->BssBasicRateMask) = MRVL_BSSBASICRATEMASK_A;
			*(parent_wlpptr->vmacSta_p->Mib802dot11->NotBssBasicRateMask) = MRVL_NOTBSSBASICRATEMASK_A;
			*(parent_wlpptr->vmacSta_p->Mib802dot11->mib_shortSlotTime) = TRUE;
			*(stapriv->vmacSta_p->Mib802dot11->mib_ApMode) &= ~(AP_MODE_B_ONLY | AP_MODE_G_ONLY);
			*(stapriv->vmacSta_p->Mib802dot11->mib_ApMode) |= AP_MODE_A_ONLY;
			*(stapriv->vmacSta_p->Mib802dot11->BssBasicRateMask) = MRVL_BSSBASICRATEMASK_A;
			*(stapriv->vmacSta_p->Mib802dot11->NotBssBasicRateMask) = MRVL_NOTBSSBASICRATEMASK_A;
			*(stapriv->vmacSta_p->Mib802dot11->mib_shortSlotTime) = TRUE;
			opmode_changed = 1;
		}
		...
		if (opmode_changed) {
			if (staDev->flags & IFF_RUNNING) {
				...
				netif_stop_queue(staDev);
				staDev->flags &= ~IFF_RUNNING;
			}

			wlFwApplyClientSettings(staDev);

			if (wlFwSetBssForClientMode(staDev, WL_DISABLE)) {
			...
			if (wlFwSetBssForClientMode(staDev, WL_ENABLE)) {
				WLDBG_ERROR(DBG_LEVEL_0, "Falied to start the %d" "th BSS for client mode\n", vmacSta_p->VMacEntry.macId);
				return -EIO;
			}

			netif_wake_queue(staDev);	/* Start/Restart Q if stopped. */
...
*
**************************************************************************/
extern SINT32 smeGetBssProfile(UINT8 macIndex, UINT8 * bssid, IEEEtypes_CapInfo_t * capInfo, UINT8 * ieBuf_p, UINT16 * ieBufLen)
{
	vmacStaInfo_t *vStaInfo_p;
	IEEEtypes_InfoElementHdr_t *IE_p;
...
*
**************************************************************************/
extern SINT32 smeGetScanResults(UINT8 macIndex, UINT8 * numDescpt_p, UINT16 * scanResultLen_p, UINT8 ** inBuf_p)
{
	vmacStaInfo_t *vStaInfo_p;
	vStaInfo_p = (vmacStaInfo_t *) vmacGetVMacStaInfo(parentGetVMacId(macIndex));
...
#else
		memcpy((void *)inBuf_p, (const void *)&vStaInfo_p->scanTableResult_p->ScanResults[0], vStaInfo_p->ScanResultsLen);
#endif				/* PORT_TO_LINUX_OS */
		memset(vStaInfo_p->scanTableResult_p->ScanResultsMap_p, 0, sizeof(void *) * IEEEtypes_MAX_BSS_DESCRIPTS);
		SPIN_UNLOCK_IRQRESTORE(&vStaInfo_p->ScanResultsLock, vStaInfo_p->ScanResultsFlags);
		return MLME_SUCCESS;
	}
	return MLME_FAILURE;
...
extern SINT32 smeSendScanRequest(UINT8 macIndex, UINT8 scanType, UINT8 bssType, UINT16 scanTime, UINT8 * bssid, UINT8 * ieBuf_p, UINT16 ieBufLen)
{
	...
	memcpy((void *)&scanMsg_p->targetAddr[0], (const void *)&vmacEntry_p->vmacAddr[0], sizeof(IEEEtypes_MacAddr_t));
	...
	memset(&scanMsg_p->Body.ScanCmd_sta.Cmd.BssId[0], 0xff, sizeof(IEEEtypes_MacAddr_t));
	if (memcmp(bssid, bcAddr1, sizeof(IEEEtypes_MacAddr_t)) && memcmp(bssid, bcAddr2, sizeof(IEEEtypes_MacAddr_t))) {
		vStaInfo_p->scanFilterMap |= MLME_BSSID_FILTER;
		memcpy(&scanMsg_p->Body.ScanCmd_sta.Cmd.BssId, bssid, sizeof(IEEEtypes_MacAddr_t));
	}
	...
	if ((IE_p = smeParseIeType(DS_PARAM_SET, ieBuf_p, ieBufLen)) != NULL) {
		if (IE_p->Len > (IEEEtypes_MAX_CHANNELS + IEEEtypes_MAX_CHANNELS_A)) {
			...
			memcpy(&scanMsg_p->Body.ScanCmd_sta.Cmd.ChanList[0], (UINT8 *) IE_p + sizeof(IEEEtypes_InfoElementHdr_t),
	...
	memcpy(&vStaInfo_p->smeMain_LastScanMsg, scanMsg_p, sizeof(macmgmtQ_CmdReq_t));

	/* Save master dev mib_ApMode, it might be changed during scan. Restore it after scan done if changed. */
	...
	priv_p->vmacSta_p->lst_mib_ApMode = (*mib->mib_ApMode);
	priv_p->vmacSta_p->lst_mib_shortSlotTime = (*mib->mib_shortSlotTime);
#ifdef BRS_SUPPORT
	priv_p->vmacSta_p->lst_BssBasicRateMask = (*mib->BssBasicRateMask);
	priv_p->vmacSta_p->lst_NotBssBasicRateMask = (*mib->NotBssBasicRateMask);
#endif
...
*
**************************************************************************/
extern void smeStateMgr_SndStartCmd(UINT8 * info_p, IEEEtypes_BssDesc_t * bssDesc_p)
{
	...
	if ((startMsg_p = (macmgmtQ_CmdReq_t *) mlmeApiAllocSmeMsg()) != NULL) {
		...
		memcpy((void *)&startMsg_p->targetAddr[0], (const void *)&vmacEntry_p->vmacAddr[0], sizeof(IEEEtypes_MacAddr_t));
		...
		util_CopyList(startMsg_p->Body.StartCmd.SsId, bssDesc_p->SsId, IEEEtypes_SSID_SIZE);
...
*
**************************************************************************/
vmacEntry_t *smeInitParentSession(UINT8 phyMacIndx, UINT8 * macAddr, trunkId_t trunkId, void *callBack_fp, void *privInfo_p)
{
	vmacEntry_t *vmacEntry_p;

	/* Initialize the Parent MLME session */
	if ((vmacEntry_p = mlmeStaInit_Parent(phyMacIndx, macAddr, callBack_fp)) != NULL) {
...
*
**************************************************************************/
vmacEntry_t *smeStartChildSession(UINT8 phyMacIndx, UINT8 * macAddr, trunkId_t trunkId, void *callBack_fp, UINT32 controlParam, void *privInfo_p)
{
	vmacEntry_t *vmacEntry_p;
	vmacStaInfo_t *vStaInfo_p;
	macmgmtQ_CmdReq_t *macMgmtMsg_p;
	...
	if ((vmacEntry_p = childSrv_StartSession(phyMacIndx, macAddr, callBack_fp, controlParam)) == NULL) {
		return NULL;
	}
...
*
**************************************************************************/
BOOLEAN smeSetChildBlockAddr(UINT8 phyMacIndx, UINT8 * macAddr, UINT8 maskAddr)
{
	vmacStaInfo_t *parentStaInfo_p;
	vmacId_t parentVMacId;
	...
	if (childSrv_RegisterBlockAddress(parentStaInfo_p, phyMacIndx, macAddr, maskAddr) == -1) {
		return FALSE;
	}
	return TRUE;
}
...
void smeChildSendDeAuth(vmacId_t childSessionId)
{
	...
	if ((vmacEntry_p = vmacGetVMacEntryById(childSessionId)) != NULL) {
		vStaInfo_p = (vmacStaInfo_t *) vmacEntry_p->info_p;
		if (!vStaInfo_p->isParentSession && vStaInfo_p->AssociatedFlag && !vStaInfo_p->mib_WB_p->opMode) {
			vStaInfo_p->staSecurityMibs.mib_PrivacyTable_p->RSNLinkStatus = 0;
			vStaInfo_p->AssociatedFlag = 0;
#ifdef SOC_W8964
			vStaInfo_p->in_obss_scan = FALSE;
...
extern void smeSndLinkLostInd(vmacEntry_t * vmacEntry_p, UINT16 reason)
{
	...
	if (vStaInfo_p->AssociatedFlag && !vStaInfo_p->mib_WB_p->opMode) {
		...
		vStaInfo_p->in_obss_scan = FALSE;
#endif
		authSrv_SndDeAuthMsg(vStaInfo_p, &vStaInfo_p->macMgmtMlme_ThisStaData.BssId, &vStaInfo_p->macMgmtMlme_ThisStaData.BssId, reason);
	}
