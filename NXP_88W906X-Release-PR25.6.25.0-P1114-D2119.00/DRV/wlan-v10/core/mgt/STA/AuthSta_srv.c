...
*
**************************************************************************/
static void authSrv_SndAuthCnfm(vmacStaInfo_t * vStaInfo_p, UINT8 authType, UINT16 authResult, UINT8 * authPeerAddr)
{
...
*
**************************************************************************/
static void authSrv_SndDeAuthCnfm(vmacStaInfo_t * vStaInfo_p, UINT16 deAuthResult, UINT8 * deAuthPeerAddr)
{
...
/*static*/ extern void authSrv_SndDeAuthInd(vmacStaInfo_t * vStaInfo_p,
					    UINT16 deAuthReason, UINT8 * deAuthPeerAddr)
{
...
extern SINT32 authSrv_AuthActTimeOut(UINT8 * data)
{
	...
	mlmeApiEventNotification(vStaInfo_p, MlmeAuth_Cnfm, (UINT8 *) & vStaInfo_p->macMgmtMlme_ThisStaData.BssId, ETH_EVT_JOIN_TIMEOUT);
	return 0;
}
...
extern SINT32 authSrv_SndAuthError(vmacStaInfo_t * vStaInfo_p,
				   IEEEtypes_StatusCode_t statusCode, IEEEtypes_AuthTransSeq_t seqNum, UINT16 arAlg_in, IEEEtypes_MacAddr_t * destMac)
{
	...
	mlmePrepDefaultMgtMsg_Sta(vStaInfo_p, mgtAuthRsp_p, destMac, IEEE_MSG_AUTHENTICATE, &(vStaInfo_p->macMgmtMlme_ThisStaData.BssId));
	mgtAuthRsp_p->Hdr.FrmBodyLen = 0;
	/* Add Auth Alg */
	mgtAuthRsp_p->Body.Auth.AuthAlg = ENDIAN_SWAP16(arAlg_in);
	mgtAuthRsp_p->Hdr.FrmBodyLen += sizeof(IEEEtypes_AuthAlg_t);
	/* Add Auth Seq Number */
	mgtAuthRsp_p->Body.Auth.AuthTransSeq = ENDIAN_SWAP16(seqNum + 1);
...
*
**************************************************************************/
extern SINT32 authSrv_SndDeAuthMsg(vmacStaInfo_t * vStaInfo_p, IEEEtypes_MacAddr_t * destMac_p, IEEEtypes_MacAddr_t * bssId_p, UINT16 reasonCode)
{
	...
	/* Build mgt frame */
	mlmePrepDefaultMgtMsg_Sta(vStaInfo_p, mgtDeAuth_p, destMac_p, IEEE_MSG_DEAUTHENTICATE, bssId_p);
	...
	/* L2 Event Notification */
	mlmeApiEventNotification(vStaInfo_p, MlmeDeAuth_Req, (UINT8 *) destMac_p, reasonCode);
	return MLME_SUCCESS;
}
...
static SINT32 authSrv_DoOpenAuth(vmacStaInfo_t * vStaInfo_p, dot11MgtFrame_t * MgmtMsg_p)
{
	...
	if (MgmtMsg_p->Body.Auth.AuthTransSeq != ENDIAN_SWAP16(2)) {
		return MLME_FAILURE;
	}
...
#endif				/* ETH_DEBUG */
		authSrv_SndAuthCnfm(vStaInfo_p, AUTH_OPEN_SYSTEM, AUTH_RESULT_SUCCESS, &MgmtMsg_p->Hdr.SrcAddr[0]);
	} else {
#ifdef ETH_DEBUG
		eprintf("authSrv_DoOpenAuth: case default\n");
#endif				/* ETH_DEBUG */
		authSrv_SndAuthCnfm(vStaInfo_p, AUTH_OPEN_SYSTEM, AUTH_RESULT_REFUSED, &MgmtMsg_p->Hdr.SrcAddr[0]);
		/* L2 Event Notification */
		mlmeApiEventNotification(vStaInfo_p, MlmeAuth_Cnfm, (UINT8 *) & MgmtMsg_p->Hdr.SrcAddr, MgmtMsg_p->Body.Auth.StatusCode);
	}
	return MLME_SUCCESS;
}
...
*
**************************************************************************/
extern UINT32 authSrv_AuthCmd(vmacStaInfo_t * vStaInfo_p, IEEEtypes_AuthCmd_t * AuthCmd_p)
{
...
#endif
#ifdef SOC_W906X
	if ((StaInfoRmAp_p = extStaDb_GetStaInfo(wlpptrSta->vmacSta_p, (IEEEtypes_MacAddr_t *) & AuthCmd_p->PeerStaAddr,
		...
		memcpy(&StaInfo->Addr, (void *)&AuthCmd_p->PeerStaAddr, sizeof(IEEEtypes_MacAddr_t));
	...
	StaInfo = StaInfoRmAp_p;

	memset(&PeerInfo, 0x00, sizeof(PeerInfo_t));
	PeerInfo.LegacyRateBitMap = 0xFFFF;
	/* Add AP entry into FW STADB */
	wlFwSetNewStn(pStaDev, (u_int8_t *) & AuthCmd_p->PeerStaAddr, StaInfo->Aid, StaInfo->StnId, StaInfoDbActionAddEntry, &PeerInfo, 0, 0, 0);
#endif
	...
	if ((mgtFrame_p = mlmeApiAllocMgtMsg(vmacEntry_p->phyHwMacIndx)) == NULL) {
		...
		authSrv_SndAuthCnfm(vStaInfo_p, AuthCmd_p->AuthType, AUTH_RESULT_RESOURCE_ERROR, &AuthCmd_p->PeerStaAddr[0]);
		return MLME_FAILURE;
	}
	mlmePrepDefaultMgtMsg_Sta(vStaInfo_p, mgtFrame_p, &srcAddr, IEEE_MSG_AUTHENTICATE, &(vStaInfo_p->macMgmtMlme_ThisStaData.BssId));
	mgtFrame_p->Hdr.FrmBodyLen = 0;
	/* Set Auth Alg Type */
	mgtFrame_p->Body.Auth.AuthAlg = ENDIAN_SWAP16(AuthCmd_p->AuthType);
	mgtFrame_p->Hdr.FrmBodyLen += sizeof(IEEEtypes_AuthAlg_t);
	/* Set Auth Seq Number */
	mgtFrame_p->Body.Auth.AuthTransSeq = ENDIAN_SWAP16(1);
	...
	mlmeApiStartTimer(vStaInfo_p, (UINT8 *) & vStaInfo_p->authTimer, &authSrv_AuthActTimeOut, AUTH_TIME);
	return MLME_SUCCESS;
}
...
*
**************************************************************************/
extern SINT32 authSrv_DeAuthCmd(vmacStaInfo_t * vStaInfo_p, IEEEtypes_DeauthCmd_t * DeauthCmd_p)
{
	dot11MgtFrame_t *mgtFrame_p;
	...
	if (memcmp(vStaInfo_p->macMgmtMlme_ThisStaData.BssId, DeauthCmd_p->PeerStaAddr, sizeof(IEEEtypes_MacAddr_t))) {
		authSrv_SndDeAuthCnfm(vStaInfo_p, DEAUTH_RESULT_INVALID_PARAMETERS, DeauthCmd_p->PeerStaAddr);
		return MLME_FAILURE;
	}
	/* Build mgt frame */
	if ((mgtFrame_p = mlmeApiAllocMgtMsg(vmacEntry_p->phyHwMacIndx)) == NULL) {
	...
	mlmePrepDefaultMgtMsg_Sta(vStaInfo_p,
				  mgtFrame_p, &DeauthCmd_p->PeerStaAddr, IEEE_MSG_DEAUTHENTICATE, &(vStaInfo_p->macMgmtMlme_ThisStaData.BssId));
	...
	if (authSrv_SndDeAuthMsg(vStaInfo_p,
				 &DeauthCmd_p->PeerStaAddr, &(vStaInfo_p->macMgmtMlme_ThisStaData.BssId), DeauthCmd_p->Reason) == MLME_FAILURE) {
		/* Notify SME of failure */
		authSrv_SndDeAuthCnfm(vStaInfo_p, DEAUTH_RESULT_INVALID_PARAMETERS, DeauthCmd_p->PeerStaAddr);
		return MLME_FAILURE;
	}
	...
	authSrv_SndDeAuthCnfm(vStaInfo_p, DEAUTH_RESULT_SUCCESS, DeauthCmd_p->PeerStaAddr);
	/*Milind. 09/29/05 */
...
*
**************************************************************************/
extern SINT32 authSrv_RecvMsgRsp(vmacStaInfo_t * vStaInfo_p, dot11MgtFrame_t * MgmtMsg_p)
{
	SINT32 authResult;
	...
	switch (ENDIAN_SWAP16(MgmtMsg_p->Body.Auth.AuthAlg)) {
	...
	default:
		...
		authSrv_SndAuthError(vStaInfo_p,
				     ...
				     MgmtMsg_p->Body.Auth.AuthTransSeq, MgmtMsg_p->Body.Auth.AuthAlg, &MgmtMsg_p->Hdr.SrcAddr);
		authSrv_SndAuthCnfm(vStaInfo_p, AUTH_NOT_SUPPORTED, AUTH_RESULT_REFUSED, &MgmtMsg_p->Hdr.SrcAddr[0]);
		/* L2 Event Notification */
		mlmeApiEventNotification(vStaInfo_p, MlmeAuth_Cnfm, (UINT8 *) & MgmtMsg_p->Hdr.SrcAddr, MgmtMsg_p->Body.Auth.StatusCode);
...
*
**************************************************************************/
extern SINT32 authSrv_RecvMsgDeAuth(vmacStaInfo_t * vStaInfo_p, dot11MgtFrame_t * MgmtMsg_p)
{
	vmacEntry_t *vmacEntry_p;
	...
	if (vStaInfo_p->macMgmtMain_State == STATE_SCANNING || vStaInfo_p->macMgmtMain_State == STATE_RESTORING_FROM_SCAN) {
		vStaInfo_p->macMgmtMain_PostScanState = STATE_JOINED;
	} else {
		vStaInfo_p->macMgmtMain_State = STATE_IDLE;
	}
	/* L2 Event Notification */
	mlmeApiEventNotification(vStaInfo_p, MlmeDeAuth_Ind, &MgmtMsg_p->Hdr.SrcAddr[0], MgmtMsg_p->Body.Deauth.ReasonCode);
	/* Notify SME of DeAuth */
	authSrv_SndDeAuthInd(vStaInfo_p, MgmtMsg_p->Body.Deauth.ReasonCode, MgmtMsg_p->Hdr.SrcAddr);
...
*
**************************************************************************/
static SINT32 authSrv_SKeyProcessSeq2(vmacStaInfo_t * vStaInfo_p, dot11MgtFrame_t * MgmtMsg_p)
{
	dot11MgtFrame_t *mgtFrame_p;
	vmacEntry_t *vmacEntry_p = (vmacEntry_t *) vStaInfo_p->vMacEntry_p;
	...
	if (MgmtMsg_p->Body.Auth.StatusCode != IEEEtypes_STATUS_SUCCESS) {
		...
		authSrv_SndAuthCnfm(vStaInfo_p, AUTH_SHARED_KEY, AUTH_RESULT_REFUSED, &MgmtMsg_p->Hdr.SrcAddr[0]);
		/* L2 Event Notification */
		mlmeApiEventNotification(vStaInfo_p, MlmeAuth_Cnfm, (UINT8 *) & MgmtMsg_p->Hdr.SrcAddr, MgmtMsg_p->Body.Auth.StatusCode);
		return MLME_FAILURE;
	}
	...
	mlmePrepDefaultMgtMsg_Sta(vStaInfo_p,
				  mgtFrame_p, &MgmtMsg_p->Hdr.SrcAddr, IEEE_MSG_AUTHENTICATE, &(vStaInfo_p->macMgmtMlme_ThisStaData.BssId));
	mgtFrame_p->Hdr.FrmBodyLen = 0;
	mgtFrame_p->Hdr.FrmCtl.Wep = 1;
	/* Set Auth Auth Alg Type */
	mgtFrame_p->Body.Auth.AuthAlg = ENDIAN_SWAP16(AUTH_SHARED_KEY);
	mgtFrame_p->Hdr.FrmBodyLen += sizeof(IEEEtypes_AuthAlg_t);
	/* Set Aut Seq Number */
	mgtFrame_p->Body.Auth.AuthTransSeq = ENDIAN_SWAP16(3);
	mgtFrame_p->Hdr.FrmBodyLen += sizeof(IEEEtypes_AuthTransSeq_t);
	/* Set Status Code */
	mgtFrame_p->Body.Auth.StatusCode = IEEEtypes_STATUS_SUCCESS;
	mgtFrame_p->Hdr.FrmBodyLen += sizeof(IEEEtypes_StatusCode_t);
	...
	mlmeApiWepEncrypt(vStaInfo_p, (UINT8 *) & mgtFrame_p->Body.Auth,
			  MgmtMsg_p->Body.Auth.ChallengeText.Text, MgmtMsg_p->Body.Auth.ChallengeText.Len);
	mgtFrame_p->Hdr.FrmBodyLen += MgmtMsg_p->Body.Auth.ChallengeText.Len + WEP_ENCRYPT_OVER_HDR_LEN;
	/* Send mgt frame */
	if (mlmeApiSendMgtMsg_Sta(vStaInfo_p, mgtFrame_p, NULL) == MLME_FAILURE) {
		return MLME_FAILURE;
	}
...
*
**************************************************************************/
static SINT32 authSrv_SKeyProcessSeq4(vmacStaInfo_t * vStaInfo_p, dot11MgtFrame_t * MgmtMsg_p)
{
	if (MgmtMsg_p->Body.Auth.StatusCode != IEEEtypes_STATUS_SUCCESS) {
		/* Notify SME of Auth Failure */
		authSrv_SndAuthCnfm(vStaInfo_p, AUTH_SHARED_KEY, AUTH_RESULT_REFUSED, &MgmtMsg_p->Hdr.SrcAddr[0]);
	...
	if (!memcmp(MgmtMsg_p->Hdr.SrcAddr, MgmtMsg_p->Hdr.BssId, sizeof(IEEEtypes_MacAddr_t))) {
		vStaInfo_p->macMgmtMain_State = STATE_AUTHENTICATED_WITH_AP;
	}
	/* Notify SME of Auth Success */
	authSrv_SndAuthCnfm(vStaInfo_p, AUTH_SHARED_KEY, AUTH_RESULT_SUCCESS, MgmtMsg_p->Hdr.SrcAddr);
	return MLME_SUCCESS;
}
...
static SINT32 authSrv_DoSharedKeyAuth(vmacStaInfo_t * vStaInfo_p, dot11MgtFrame_t * MgmtMsg_p)
{
	switch (ENDIAN_SWAP16(MgmtMsg_p->Body.Auth.AuthTransSeq)) {
	case 2:
