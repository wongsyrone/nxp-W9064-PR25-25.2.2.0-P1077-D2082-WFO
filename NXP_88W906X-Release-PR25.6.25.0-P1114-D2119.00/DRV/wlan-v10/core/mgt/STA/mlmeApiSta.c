...
macmgmtQ_CmdBuf_t *smeCmdBuf_q = NULL;
...
extern void InitClientInfo(UINT8 * macAddr_p, dot11MgtFrame_t * MgmtMsg_p, vmacEntry_t * clientVMacEntry_p, BOOLEAN isApMrvl, BOOLEAN isConnectbBSS);
#else
...
extern BOOLEAN macMgmtMlme_UpdateRateInfoForPeerStationWithRateSetInfo(UINT32 Aid,
								       IEEEtypes_DataRate_t * bOpRateSet_p, IEEEtypes_DataRate_t * gOpRateSet_p);
...
extern BOOLEAN macMgmtMlme_UpdateRateInfoForPeerStation(UINT32 Aid,
							IEEEtypes_SuppRatesElement_t * Rates_p, IEEEtypes_ExtSuppRatesElement_t * ExtRates_p);
...
static BOOLEAN mlmeApiIsThisRateSupported(IEEEtypes_DataRate_t Rate,
					  IEEEtypes_SuppRatesElement_t * peer_stn_Rates_p, IEEEtypes_SuppRatesElement_t * peer_stn_ExtRates_p);
...
extern SINT32 mlmeApi_NotificationActTimeOut(UINT8 * data)
{
	...
	if (data && linuxdrvMsg_p->dirty) {
		vStaInfo_p->mlmeCallBack_fp(linuxdrvMsg_p->eventId, (UINT8 *) vmacEntry_p, linuxdrvMsg_p->data1);
...
*
**************************************************************************/
extern void mlmeApiGetTimerTxTSF(vmacStaInfo_t * vStaInfo_p, UINT32 * valueHi_p, UINT32 * valueLo_p)
{
	UINT16 macId;
	UINT8 macIndx;
...
extern void mlmeApiPrepHwStartIBss(vmacStaInfo_t * vStaInfo_p, IEEEtypes_StartCmd_t * StartCmd_p)
{
	...
	if (!memcmp(&vStaInfo_p->macMgmtMlme_ThisStaData.BssId, nullMac, sizeof(IEEEtypes_MacAddr_t))) {
		...
		memcpy(&(vStaInfo_p->macMgmtMlme_ThisStaData.BssId), &valueLo, sizeof(UINT32));
		valueHi = (~valueLo + 0x111) & 0xFFFF;
		memcpy(&(vStaInfo_p->macMgmtMlme_ThisStaData.BssId[4]), &valueHi, sizeof(IEEEtypes_MacAddr_t) - sizeof(UINT32));
		msi_wl_SetBSSID2(macId, valueHi, valueLo);
	}
	mlmeApiSetBSSIDFilter(vStaInfo_p, 0);
	mlmeApiSetSsIdToMac(vStaInfo_p, &StartCmd_p->SsId[0], vStaInfo_p->macMgmtMlme_ThisStaData.BssSsIdLen);
...
*
**************************************************************************/
extern SINT32 mlmeApiSetIbssDefaultFilter(vmacStaInfo_t * vStaInfo_p, BOOLEAN enable)
{
	UINT32 rxModeValue;
	UINT16 macId;
	UINT8 macIndx;
...
extern void mlmeApiSetRfChannel(vmacStaInfo_t * vStaInfo_p, UINT8 channel, UINT8 initRateTable, BOOLEAN scanning)
{
	...
	/* DOMAIN_CODE_FCC:Channel 165 currently only supports 20 MHz BW. */
	if (*(mib->mib_regionCode) == DOMAIN_CODE_ALL) {
		if (scanning || (lchan == 181)) {
			chanflag.ChnlWidth = CH_20_MHz_WIDTH;
			chanflag.ExtChnlOffset = NO_EXT_CHANNEL;
		}
	} else if (scanning || (lchan == 165)) {
		chanflag.ChnlWidth = CH_20_MHz_WIDTH;
		chanflag.ExtChnlOffset = NO_EXT_CHANNEL;
		...
		if (memcmp(vStaInfo_p->bssDescProfile_p->BssId, nullAddr, 6) != 0) {
			...
			if (IsHTmode(*(priv_p->vmacSta_p->Mib802dot11->mib_ApMode))) {
				if (vStaInfo_p->bssDescProfile_p->ADDHTElement.Len) {
					...
					if (vStaInfo_p->bssDescProfile_p->ADDHTElement.AddChan.STAChannelWidth) {
			...
			if ((*(priv_p->vmacSta_p->Mib802dot11->mib_ApMode) >= AP_MODE_5GHZ_11AC_ONLY)
#ifdef SOC_W906X
			    && (*(priv_p->vmacSta_p->Mib802dot11->mib_ApMode) != AP_MODE_11AX)
			    && (*(priv_p->vmacSta_p->Mib802dot11->mib_ApMode) != AP_MODE_2_4GHZ_Nand11AX)
			    && (*(priv_p->vmacSta_p->Mib802dot11->mib_ApMode) != AP_MODE_2_4GHZ_11AX_MIXED)
#endif
			    ) {
#ifdef SUPPORTED_EXT_NSS_BW
				SINT32 ret __maybe_unused = 0;
#endif
				if ((vStaInfo_p->bssDescProfile_p->VHTCap.cap.SupportedChannelWidthSet == 1)
				    ...
				    || (vStaInfo_p->bssDescProfile_p->VHTOp.ch_width == 2)) {
					chanflag.ChnlWidth = CH_160_MHz_WIDTH;

					/*Check against own bw setting to make sure don't exceed intended bw */
					if ((PhyDSSSTable->Chanflag.ChnlWidth > CH_AUTO_WIDTH) &&
					    (PhyDSSSTable->Chanflag.ChnlWidth < CH_160_MHz_WIDTH))
						chanflag.ChnlWidth = PhyDSSSTable->Chanflag.ChnlWidth;
					...
					if (1 == (ret = isSupport160MhzByCenterFreq(priv_p, VHT_EXTENDED_NSS_BW_CAPABLE,
										    ...
										    vStaInfo_p->bssDescProfile_p->ADDHTElement.OpMode.
										    center_freq2))) {
...
#ifdef SOC_W906X
	// TODO: need to set the second channel?
	result = wlchannelSet(priv_p->vmacSta_p->dev, lchan, 0, &chanflag, initRateTable);
#else
...
extern void mlmePrepDefaultMgtMsg_Sta(vmacStaInfo_t * vStaInfo_p,
				      dot11MgtFrame_t * mgtFrame_p, IEEEtypes_MacAddr_t * DestAddr, UINT32 Subtype, IEEEtypes_MacAddr_t * BssId_p)
{
	vmacEntry_t *vmacEntry_p = (vmacEntry_t *) vStaInfo_p->vMacEntry_p;
	...
	memcpy(&mgtFrame_p->Hdr.DestAddr, DestAddr, sizeof(IEEEtypes_MacAddr_t));
	memcpy(&mgtFrame_p->Hdr.SrcAddr, &vmacEntry_p->vmacAddr[0], sizeof(IEEEtypes_MacAddr_t));
	memcpy(&mgtFrame_p->Hdr.BssId, BssId_p, sizeof(IEEEtypes_MacAddr_t));
}
...
extern BOOLEAN mlmeApiSendSmeMsg(macmgmtQ_CmdReq_t * smeCmd_p)
{
	...
	targetVMacEntry_p->mlmeMsgEvt((UINT8 *) smeCmd_p, NULL, targetVMacEntry_p->info_p);
#else				/* PORT_TO_LINUX_OS */
...
{
	memset((void *)&cmdMsg->Cmd.cmdReq.targetAddr[0], 0, sizeof(IEEEtypes_MacAddr_t));
#ifdef PORT_TO_LINUX_OS
...
*
**************************************************************************/
extern SINT32 mlmeApiSendMgtMsg_Sta(vmacStaInfo_t * vStaInfo_p, dot11MgtFrame_t * mgtFrame_p, UINT8 * pRxSign)
{
	UINT8 macIndx;
	UINT8 rate;
	UINT8 preamble;
...
#ifdef SOC_W906X
		if (txMgmtMsg((struct net_device *)vMacEntry_p->privInfo_p, skb_p) != OS_SUCCESS)
#else
...
*
**************************************************************************/
extern SINT32 mlmeApiSndScanNotificationOnly(vmacStaInfo_t * vStaInfo_p, UINT16 scanResult, UINT8 numSet, UINT16 bufSize, UINT8 * BssDescSet_p)
{
	...
	memcpy((void *)&smeMsg_p->targetAddr[0], (const void *)&vmacEntry_p->vmacAddr[0], sizeof(IEEEtypes_MacAddr_t));
	...
	if (bufSize) {
		memcpy(&(smeMsg_p->Msg.ScanCfrm.Rsp.BssDescSet.BssDesc), BssDescSet_p, bufSize);

		scanCfrm_p = (unsigned char *)
		    &(smeMsg_p->Msg.ScanCfrm.Rsp.BssDescSet.BssDesc) + bufSize;
...
extern SINT32 mlmeApiSndNotification(vmacStaInfo_t * vStaInfo_p, UINT8 * evtMsg_p, UINT8 mlmeEvt)
{
	macmgmtQ_CmdRsp_t *smeMsg_p;
	vmacEntry_t *vmacEntry_p;
	...
	case MlmeAuth_Ind:
		{
			...
			memcpy(&smeMsg_p->Msg.AuthInd, evtMsg_p, sizeof(IEEEtypes_AuthInd_t));
		}
		break;
	...
	case MlmeDeAuth_Ind:
		{
			...
			memcpy(&smeMsg_p->Msg.DeauthInd, evtMsg_p, sizeof(IEEEtypes_DeauthInd_t));
		}
		break;
	...
	case MlmeDeAuth_Cnfm:
		{
			...
			memcpy(&smeMsg_p->Msg.DeauthCfrm, evtMsg_p, sizeof(IEEEtypes_DeauthCfrm_t));
	...
	case MlmeAssoc_Cnfm:
		{
			...
			memcpy(&smeMsg_p->Msg.AssocCfrm, evtMsg_p, sizeof(IEEEtypes_AssocCfrm_t));
		}
		break;
	...
	case MlmeAssoc_Ind:
		{
			...
			memcpy(&smeMsg_p->Msg.AssocInd, evtMsg_p, sizeof(IEEEtypes_AssocInd_t));
		}
		break;
	...
	case MlmeReAssoc_Cnfm:
		{
			...
			memcpy(&smeMsg_p->Msg.ReassocCfrm, evtMsg_p, sizeof(IEEEtypes_ReassocCfrm_t));
		}
		break;
	...
	case MlmeReAssoc_Ind:
		{
			...
			memcpy(&smeMsg_p->Msg.ReassocInd, evtMsg_p, sizeof(IEEEtypes_ReassocInd_t));
		}
		break;
	...
	case MlmeDisAssoc_Cnfm:
		{
			...
			memcpy(&smeMsg_p->Msg.DisassocCfrm, evtMsg_p, sizeof(IEEEtypes_DisassocCfrm_t));
		}
		break;
	...
	case MlmeDisAssoc_Ind:
		{
			...
			memcpy(&smeMsg_p->Msg.DisassocInd, evtMsg_p, sizeof(IEEEtypes_DisassocInd_t));
		}
		break;
	...
	case MlmeScan_Cnfm:
		{
			...
			memcpy(&smeMsg_p->Msg.ScanCfrm, evtMsg_p, sizeof(smeQ_ScanRsp_t));
		}
		break;
	...
	case MlmeJoin_Cnfm:
		{
			...
			memcpy(&smeMsg_p->Msg.JoinCfrm, evtMsg_p, sizeof(IEEEtypes_JoinCfrm_t));
		}
		break;
	...
	case MlmeStart_Cnfm:
		{
			...
			memcpy(&smeMsg_p->Msg.StartCfrm, evtMsg_p, sizeof(IEEEtypes_StartCfrm_t));
		}
		break;
	...
	case MlmeReset_Cnfm:
		{
			...
			memcpy(&smeMsg_p->Msg.ResetCfrm, evtMsg_p, sizeof(IEEEtypes_ResetCfrm_t));
		}
		break;
...
*
**************************************************************************/
extern void mlmeApiEventNotification(vmacStaInfo_t * vStaInfo_p, UINT32 eventInvoked, UINT8 * peerMacAddr, UINT32 reasonCode)
{
	...
	case MlmeScan_Cnfm:
		{
			...
			mlmeApiStartTimer((vmacStaInfo_t *) linuxdrvMsg_p,
					  (UINT8 *) & linuxdrvMsg_p->notificationTimer, &mlmeApi_NotificationActTimeOut, 1);
		}
		break;
	case MlmeReset_Cnfm:
		{
			vStaInfo_p->mlmeCallBack_fp(MlmeReset_Cnfm, (UINT8 *) vmacEntry_p, 0);
		}
		break;
	default:
		break;
	...
	case MlmeDisAssoc_Req:
		{
			...
			memcpy((void *)&evtBufAssoc.apMacAddr[0], (const void *)peerMacAddr, sizeof(IEEEtypes_MacAddr_t));
			memcpy((void *)&evtBufAssoc.staMacAddr[0], vmacEntry_p->vmacAddr, sizeof(IEEEtypes_MacAddr_t));
			eventGenerate(STAEVT_ASSOC_WITH_AP,
				      STAEVT_ASSOC_SUBTYPE_DISASSOC_SEND, sizeof(EVTBUF_STAEVT_ASSOC_SUBTYPE_DISASSOC_SEND), (UINT8 *) & evtBufAssoc);
		}
		break;
	...
	case MlmeDisAssoc_Ind:
		{
			...
			memcpy((void *)&evtBufAssoc.staMacAddr[0], vmacEntry_p->vmacAddr, sizeof(IEEEtypes_MacAddr_t));
			eventGenerate(STAEVT_ASSOC_WITH_AP,
				      STAEVT_ASSOC_SUBTYPE_DISASSOC_RECV, sizeof(EVTBUF_STAEVT_ASSOC_SUBTYPE_DISASSOC_RECV), (UINT8 *) & evtBufAssoc);
		}
		break;
	...
	case MlmeDeAuth_Req:
		{
			...
			memcpy((void *)&evtBufAuth.apMacAddr[0], (const void *)peerMacAddr, sizeof(IEEEtypes_MacAddr_t));
			memcpy((void *)&evtBufAuth.staMacAddr[0], vmacEntry_p->vmacAddr, sizeof(IEEEtypes_MacAddr_t));
			eventGenerate(STAEVT_AUTH_WITH_AP,
				      STAEVT_AUTH_SUBTYPE_DEAUTH_SEND, sizeof(EVTBUF_STAEVT_AUTH_SUBTYPE_DEAUTH_SEND), (UINT8 *) & evtBufAuth);
		}
		break;
	...
	case MlmeDeAuth_Ind:
		{
			...
			memcpy((void *)&evtBufAuth.staMacAddr[0], vmacEntry_p->vmacAddr, sizeof(IEEEtypes_MacAddr_t));
			eventGenerate(STAEVT_AUTH_WITH_AP,
				      STAEVT_AUTH_SUBTYPE_DEAUTH_RECV, sizeof(EVTBUF_STAEVT_AUTH_SUBTYPE_DEAUTH_RECV), (UINT8 *) & evtBufAuth);
	...
	case Tbcn:
		{
			...
			eventGenerate(EVT_CLIENT,
				      EVT_CLIENT_SUBTYPE_DISCONNECT, sizeof(EVTBUF_EVT_CLIENT_SUBTYPE_DISCONNECT), (UINT8 *) & evtBufLink);
		}
		break;
	...
	case MlmeScan_Cnfm:
		{
			...
			eventGenerate(EVT_CLIENT,
				      ...
				      sizeof(EVTBUF_EVT_CLIENT_SUBTYPE_SCAN_COMPLETE), (UINT8 *) & evtScanCompleted);
		}
		break;
	case PeerDetectInCompat:
		{
			...
			memcpy((void *)&evtIncompatiblePeerDetected.bssId[0], (const void *)peerMacAddr, sizeof(IEEEtypes_MacAddr_t));
			eventGenerate(EVT_CLIENT,
				      EVT_CLIENT_SUBTYPE_INCOMPATIBLE_PEER_DETECTED,
				      sizeof(EVTBUF_EVT_CLIENT_SUBTYPE_INCOMPATIBLE_PEER_DETECTED), (UINT8 *) & evtIncompatiblePeerDetected);
	...
	case MlmeStart_Cnfm:
		{
			...
			eventGenerate(EVT_CLIENT,
				      EVT_CLIENT_SUBTYPE_IBSS_STARTED, sizeof(EVTBUF_EVT_CLIENT_SUBTYPE_IBSS_STARTED), (UINT8 *) & evtIbssStarted);
		}
		break;
...
extern BOOLEAN WlanEnabled[NUM_OF_WLMACS];
...
*
**************************************************************************/
extern void mlmeApiStaMacReset(UINT8 phyMacIndx, vmacStaInfo_t * vStaInfo_p)
{
	vmacEntry_t *vmacEntry_p;
	UINT32 hwMacId;
	...
	if (!mlmeApiHalAccessAllow(vStaInfo_p)) {
		if (vStaInfo_p->isParentSession) {
			msi_wl_SetMacAddrByMacId(vmacEntry_p->phyHwMacIndx, vmacEntry_p->macId, &vmacEntry_p->vmacAddr[0]);
		}
		return;
	}
...
extern UINT32 mlmeApiSetControlRates(vmacStaInfo_t * vStaInfo_p)
{
	...
	if ((curLen <= MAX_B_DATA_RATES) && (curLen + rateBuf_len) < MAX_ALL_RATE_LEN) {
		UINT8 *pdst = (UINT8 *) & rateBuf[rateBuf_len];
		UINT8 *psrc = (UINT8 *) & vStaInfo_p->bOpRateSet[0];

		//for klocwork checking
		memcpy((void *)pdst, (void const *)psrc, curLen);
		rateBuf_len += curLen;
	}
	/* Copy over 11G rates */
	curLen = util_ListLen(vStaInfo_p->gOpRateSet, MAX_G_DATA_RATES);
	if ((curLen <= MAX_G_DATA_RATES) && (curLen + rateBuf_len) < MAX_ALL_RATE_LEN) {
		UINT8 *pdst = (UINT8 *) & rateBuf[rateBuf_len];
		UINT8 *psrc = (UINT8 *) & vStaInfo_p->gOpRateSet[0];
			   ...
			   IEEEtypes_SuppRatesElement_t * peer_stn_Rates_p, IEEEtypes_SuppRatesElement_t * peer_stn_ExtRates_p)
{
	...
	if (peer_stn_Rates_p != NULL) {
		for (i = 0; i < peer_stn_Rates_p->Len; i++) {
			if ((peer_stn_Rates_p->Rates[i] & IEEEtypes_BASIC_RATE_MASK) == (Rate & IEEEtypes_BASIC_RATE_MASK)) {
	...
	if (peer_stn_ExtRates_p != NULL) {
		for (i = 0; i < peer_stn_ExtRates_p->Len; i++) {
			if ((peer_stn_ExtRates_p->Rates[i] & IEEEtypes_BASIC_RATE_MASK) == (Rate & IEEEtypes_BASIC_RATE_MASK)) {
				/*the rate has been found. */
				return TRUE;
			}
		}
...
mlmeApiGetCombinedSupportedRates(IEEEtypes_SuppRatesElement_t * my_stn_Rates_p,
				 ...
				 IEEEtypes_ExtSuppRatesElement_t * combined_stn_Rates_p, IEEEtypes_ExtSuppRatesElement_t * combined_stn_ExtRates_p)
{
	UINT32 i, IE_len;
	...
	for (i = 0; i < my_stn_Rates_p->Len; i++) {
		if (mlmeApiIsThisRateSupported(my_stn_Rates_p->Rates[i],
					       ...
					       (IEEEtypes_SuppRatesElement_t *) peer_stn_ExtRates_p) == TRUE) {
...
mlmeApiCreateSupportedRateElement(IEEEtypes_SuppRatesElement_t * Rates_p,
				  ...
				  IEEEtypes_DataRate_t * bOpRateSet_p, IEEEtypes_DataRate_t * gOpRateSet_p, phyMacId_t phyHwMacIndx)
{
	UINT32 i, curLen;
...
mlmeApiCreatePeerStationInfoForWBMode(IEEEtypes_MacAddr_t * staMacAddr,
				      IEEEtypes_SuppRatesElement_t * Rates_p, IEEEtypes_ExtSuppRatesElement_t * ExtRates_p, phyMacId_t phyHwMacIndx)
{
	 ...
	 */
	if ((StaInfo = wl_kmalloc(sizeof(extStaDb_StaInfo_t), GFP_ATOMIC)) == NULL) {
	...
	StaInfo_p = extStaDb_GetStaInfo(staMacAddr, STADB_UPDATE_AGINGTIME);
	if (pStaInfo->Aid == 0)
	...
	mlmeApiGetCombinedSupportedRates(&(mlmeApiClientModeSupportedRates[phyHwMacIndx]),
					 ...
					 ExtRates_p, (IEEEtypes_ExtSuppRatesElement_t *) & combined_stn_Rates, &combined_stn_ExtRates);
	...
	macMgmtMlme_UpdateRateInfoForPeerStation(StaInfo_p->Aid, &combined_stn_Rates, &combined_stn_ExtRates);
#endif				/* def PORT_TO_LINUX_OS */
	wl_kfree(StaInfo);
	return TRUE;
}
...
*
**************************************************************************/
extern BOOLEAN mlmeApiGetPeerStationStaInfoAndAid(IEEEtypes_MacAddr_t * staMacAddr_p, extStaDb_StaInfo_t ** StaInfo_pp, UINT32 * Aid_p)
{

#ifdef PORT_TO_LINUX_OS
	return FALSE;
#else				/* PORT_TO_LINUX_OS */
	*StaInfo_pp = extStaDb_GetStaInfo(staMacAddr_p, STADB_DONT_UPDATE_AGINGTIME);
	if (*StaInfo_pp != NULL) {
...
extern BOOLEAN mlmeApiFreePeerStationStaInfoAndAid(IEEEtypes_MacAddr_t * staMacAddr_p, vmacEntry_t * clientVMacEntry_p)
{
	extStaDb_StaInfo_t *StaInfo_p;
...
extern BOOLEAN mlmeApiSetPeerStationStateForWB(extStaDb_StaInfo_t * StaInfo_p, extStaDb_State_e state)
{
...
void mlmeApiUpdateLinkStatus(vmacStaInfo_t * vStaInfo_p, UINT8 linkId)
{
	...
	if (vStaInfo_p->mlmeCallBack_fp != NULL) {
		{
			vStaInfo_p->mlmeCallBack_fp(MmgtIndicationSignals, (UINT8 *) vmacEntry_p, linkId);
		}
	}
#else				/* PORT_TO_LINUX_OS */
	if (vStaInfo_p->mlmeCallBack_fp != NULL) {
		vStaInfo_p->mlmeCallBack_fp(vmacEntry_p->phyHwMacIndx, &vMacAddr[0], linkId);
	}
#endif				/* def PORT_TO_LINUX_OS */
}
...
*
**************************************************************************/
extern SINT32 mlmeApiStartScanTimer(vmacStaInfo_t * vStaInfo_p, UINT32 numTick, void *callBackFunct_p)
{
	ENTER_CRITICAL;
	...
	cyg_alarm_create(hCounter_scan, (cyg_alarm_t *) & TimerTick_scan, (cyg_addrword_t) alarmData_scan, &hAlarm_scan, &timerAlarm_scan);
...
*
**************************************************************************/
extern SINT32 mlmeApiDelMacAddr(vmacStaInfo_t * vStaInfo_p, UINT32 mac_id)
{
#ifdef PORT_TO_LINUX_OS
	return -1;
#else				/* def PORT_TO_LINUX_OS */
...
*
**************************************************************************/
extern SINT32 mlmeApiAddMacAddr(vmacStaInfo_t * vStaInfo_p, UINT8 macIndex, UINT8 * addr, UINT8 addr_mask)
{
	UINT16 macId;
	UINT8 macIndx;
...
*
**************************************************************************/
extern UINT8 mlmeApiMamGetMACAddress(vmacStaInfo_t * vStaInfo_p, int clientType, UINT8 * pMacAddress)
{
	vmacEntry_t *vmacEntry_p = (vmacEntry_t *) vStaInfo_p->vMacEntry_p;
#ifdef PORT_TO_LINUX_OS
...
*
**************************************************************************/
extern UINT8 mlmeApiMamGetHostMACAddress(vmacStaInfo_t * vStaInfo_p, UINT8 * pAddr)
{
	UINT8 macIndx;
	vmacEntry_t *vmacEntry_p = (vmacEntry_t *) vStaInfo_p->vMacEntry_p;
...
extern SINT32 mlmeApiUpdateBasicRatesSetting(vmacStaInfo_t * vStaInfo_p, UINT8 * rateBuf_p, UINT32 rateBufLen)
{
	...
	return -1;
#else				/* PORT_TO_LINUX_OS */
	return smeUpdateBasicRatesSetting(macIndx, rateBuf_p, rateBufLen);
#endif				/* PORT_TO_LINUX_OS */
}
...
*
**************************************************************************/
extern void mlmeApiRateUpdateRxStats(vmacStaInfo_t * vStaInfo_p, UINT8 * rfHdr_p, UINT32 length)
{
#ifdef PORT_TO_LINUX_OS
	return;
#else				/* PORT_TO_LINUX_OS */
...
*
**************************************************************************/
extern void mlmeApiUpdateSTAVendorIEs(vmacStaInfo_t * vStaInfo_p, UINT8 fForceUpdate)
{
	UINT16 macId;
	UINT8 macIndx;
...
*
**************************************************************************/
extern BOOLEAN mlmeApiMacMgtMlmeGetPeerStationAid(vmacStaInfo_t * vStaInfo_p, extStaDb_StaInfo_t * StaInfo_p, UINT32 * Aid_p)
{
#ifdef PORT_TO_LINUX_OS
	return FALSE;
#else				/* PORT_TO_LINUX_OS */
...
*
**************************************************************************/
extern WL_STATUS mlmeApiSendNullDataPktUr(vmacStaInfo_t * vStaInfo_p, IEEEtypes_MacAddr_t * DestAddr, IEEEtypes_MacAddr_t * SrcAddr)
{
...
*
**************************************************************************/
extern UINT8 mlmeApiHalRssiDbmGet(vmacStaInfo_t * vStaInfo_p, UINT8 rawRssi, UINT8 rawSq1)
{
#ifdef PORT_TO_LINUX_OS
	return rawRssi;
#else				/* PORT_TO_LINUX_OS */
...
*
**************************************************************************/
extern SINT32 mlmeApiSetTrunkIdActive(vmacStaInfo_t * vStaInfo_p, SINT8 trunk_id, BOOLEAN active, UINT8 trunk_mode)
{
	UINT8 macIndx;
...
*
**************************************************************************/
extern UINT32 mlmeApiIsTrunkIdActive(vmacStaInfo_t * vStaInfo_p, SINT8 trunk_id)
{
#ifdef PORT_TO_LINUX_OS
	return -1;
#else				/* PORT_TO_LINUX_OS */
...
*
**************************************************************************/
extern void mlmeApiStartTimer(vmacStaInfo_t * vStaInfo_p, UINT8 * timer_p, void *callback, UINT32 ticks)
{
	TimerRemove((Timer *) timer_p);
	TimerInit((Timer *) timer_p);
	TimerFireIn((Timer *) timer_p, 1, callback, (UINT8 *) vStaInfo_p, ticks);
}
...
*
**************************************************************************/
extern void mlmeApiAddStaDbEntry(vmacStaInfo_t * vStaInfo_p, dot11MgtFrame_t * MgmtMsg_p)
{
#ifdef PORT_TO_LINUX_OS
#ifdef MULTI_AP_SUPPORT
	InitClientInfo(MgmtMsg_p->Hdr.BssId, MgmtMsg_p, (vmacEntry_t *) vStaInfo_p->vMacEntry_p, vStaInfo_p->isApMrvl, vStaInfo_p->isConnectbBSS);
#else
	...
	if (mlmeApiGetPeerStationStaInfoAndAid(&(MgmtMsg_p->Hdr.SrcAddr), &wbStaInfo_p, &local_urAid) == FALSE) {
		...
		if (mlmeApiCreatePeerStationInfoForWBMode(&(MgmtMsg_p->Hdr.SrcAddr),
			...
			memcpy(&(wbStaInfo_p->Bssid[0]), &(vStaInfo_p->macMgmtMlme_ThisStaData.BssId[0]), sizeof(IEEEtypes_MacAddr_t));
		} else {
...
struct sk_buff *mlmeApiPrepMgtMsg2_sta(vmacApInfo_t * vmacSta_p, UINT32 Subtype, IEEEtypes_MacAddr_t * DestAddr, UINT16 size)
{
	...
	mlmePrepDefaultMgtMsg_Sta(vStaInfo_p, MgmtRsp_p, DestAddr, Subtype, &(vStaInfo_p->macMgmtMlme_ThisStaData.BssId));

	return (struct sk_buff *)MgmtRsp_p;
}
