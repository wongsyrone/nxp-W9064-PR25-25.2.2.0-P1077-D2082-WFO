...
#define NF_REGS_NUM 16
typedef struct nf_reg_t {
	UINT32 reg[NF_REGS_NUM];
	UINT32 val[NF_REGS_NUM];
	UINT8 len;
} nf_reg_t;
#endif				/* AUTOCHANNEL */
...
#define AOASupMacNum 5
typedef enum {
	aoa_stop_mode = 0,
	aoa_macfilter_mode,
	aoa_brd_mode,
} t_aoamode;
typedef struct _aoacmddata {
	UINT8 tlvData[MAX_TLV_LEN];
	struct net_device *netdev;
	unsigned int ticks;
	signed int setcsiCounter;
	signed int CurrentcsiCounter;
	UINT8 csiType;
	UINT8 AoAMode;
	Timer AOATimer;
	unsigned char StaAddr[AOASupMacNum][6];
	signed int StaCounter[AOASupMacNum];
} aoacmddata;
...
typedef struct vmacApInfo_t {
	...
	UINT8 lst_mib_ApMode;
	UINT8 lst_mib_shortSlotTime;
#ifdef BRS_SUPPORT
	UINT32 lst_BssBasicRateMask;
	UINT32 lst_NotBssBasicRateMask;
#endif
	...
	unsigned char acs_2g_sorted_score_idx[IEEEtypes_MAX_CHANNELS];
	unsigned char acs_2g_sorted_nf_idx[IEEEtypes_MAX_CHANNELS];
	...
	u16 acs_adjacent_bss_weight;
	u16 acs_adjacent_bss_weight_plus;
	nf_reg_t nf_regs;
	UINT32 reg_glb_timeout;
#endif
#ifdef WDS_FEATURE
	...
	UINT8 MDIE[5];
#endif
	UINT8 RSNXIE[MAX_SIZE_RSNX_IE_BUF];
...
#define MAX_OFDMADL_STA	4
	...
	unsigned long tcp_tx_bytes_last;
