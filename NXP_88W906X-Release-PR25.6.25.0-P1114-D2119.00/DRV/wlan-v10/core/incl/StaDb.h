...
typedef struct _rxppdu_airtime_t {
	...
	U32 dbg_pktcnt, dbg_nss, dbg_mcs, dbg_bw, dbg_gi_ltf, dbg_sum_pktlen, dbg_sum_pktcnt;
	U32 dbg_Ndbps10x;
...
struct vht_cbf_report_stat {
	u8 pre_snr;
	u8 pre_snr1;
	int snr_unchange_cnt;
};

#define NUM_NFPATH_BUF_MAX 100
typedef struct extStaDb_StaInfo_t {
	...
	UINT16 QueueToUse;
	UINT8 ClientMode;
	UINT8 ClientMode_mix_ext;	/* This is valid if ClientMode == MIXED_MODE */
	...
	UINT8 EXT_RsnIE[64];
	UINT8 RsnxIE_Buf[MAX_SIZE_RSNX_IE_BUF];
...
//      UINT8    owe_skb_rssi;
	...
	UINT32 rx_err;
	UINT32 tx_retries;
	UINT32 tx_packets_sampled;
	UINT32 tx_retried_sampled;
	UINT32 tx_retried_last_100;
	...
	cbinfo_sta_t cbinfo_sta;
	int nxp_sta;
	int mtk_sta;
	struct vht_cbf_report_stat cbf_stat;
	UINT64 tx_tcp_bytes;
	bool addba_rxamsdu;

	u16 nf_path_id;
	NfPathInfo_t nf_path[NUM_NFPATH_BUF_MAX];
} extStaDb_StaInfo_t;
...
*
*****************************************************************************/
extern extStaDb_Status_e extStaDb_GetState2(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * Addr_p, extStaDb_State_e * State_p);

extern extStaDb_StaInfo_t *extStaDb_GetStaInfo(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * Addr_p, int option);
...
*
*****************************************************************************/
extern extStaDb_Status_e extStaDb_SetPwrMode(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * Addr_p, IEEEtypes_PwrMgmtMode_e NewMode);
...
*
*****************************************************************************/
extern extStaDb_Status_e extStaDb_GetPwrMode(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * Addr_p, IEEEtypes_PwrMgmtMode_e * Mode_p);

extern extStaDb_Status_e extStaDb_SetAid(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * Addr_p, UINT16 Aid);

extern extStaDb_Status_e extStaDb_GetStnId(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * Addr_p, UINT16 * StnId_p);
...
#ifdef WPA
extern extStaDb_Status_e extStaDb_SetRSNDataTrafficEnabled(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * Addr_p, UINT8 value);
...
//                                        timer_Data_t *tData);
extStaDb_Status_e extStaDb_GetRSN_IE(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * Addr_p, UINT8 * RsnIE_p);
#ifdef MRVL_80211R
extStaDb_Status_e extStaDb_Get_11r_IEs(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * Addr_p, UINT8 * MDIE_p, UINT16 * ret_len, UINT8 * reassoc);
#endif
extStaDb_Status_e extStaDb_SetRSN_IE(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * Addr_p, IEEEtypes_RSN_IE_t * RsnIE_p);
#ifdef MRVL_WSC
extStaDb_Status_e extStaDb_GetWSC_IE(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * Addr_p, UINT8 * WscIE_p);
#endif
extern extStaDb_Status_e extStaDb_GetPairwiseTSC(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * Addr_p, UINT32 * pTxIV32, UINT16 * pTxIV16);
extern extStaDb_Status_e extStaDb_SetPairwiseTSC(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * Addr_p, UINT32 TxIV32, UINT16 TxIV16);
extStaDb_Status_e extStaDb_GetStaInfoAndKeys(vmacApInfo_t * vmacSta_p,
					     IEEEtypes_MacAddr_t * Addr_p, extStaDb_StaInfo_t * StaInfo_p, int AgingTimeMode, PacketType_e mode);
...
//extStaDb_Status_e extStaDb_SetTimerData(IEEEtypes_MacAddr_t *Addr_p,
//                                    timer_Data_t *tData);
extStaDb_Status_e extStaDb_SetPhase1Key(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * Addr_p, UINT16 * Phase1Key, PacketType_e mode, UINT32 RxIV32);
...
extStaDb_StaInfo_t *extStaDb_GetStaInfoWPA(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * Addr_p, UINT32 AgingTimeMode);
extern void extStaDb_ProcessAgeEvt(vmacApInfo_t * vmacSta_p);
extern extStaDb_Status_e extStaDb_SetQoSOptn(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t *, UINT8);
...
extern extStaDb_Status_e extStaDb_GetWMM_DeliveryEnableInfo(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t *, UINT8, UINT8 *, UINT8 *);

extern UINT8 extStaDb_Check_Uapsd_Capability(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t *);
extern UINT8 extStaDb_Check_ALL_AC_DeliveryEnableInfo(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t *);
extern extStaDb_Status_e extStaDb_GetMeasurementInfo(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t *, extStaDb_measurement_info_t *);
extern extStaDb_Status_e extStaDb_SetMeasurementInfo(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t *, extStaDb_measurement_info_t *);
...
extern int FourAddr_SearchHashEntry(IEEEtypes_MacAddr_t * Addr_p, MultiAP_4Addr_Entry_t ** entry, UINT8 type);
extern int FourAddr_AddHashEntry(MultiAP_4Addr_Entry_t ** currentEntry, IEEEtypes_MacAddr_t * Addr2_p, IEEEtypes_MacAddr_t * Addr4_p);
extern int FourAddr_IfDelHashEntry(IEEEtypes_MacAddr_t * Addr_p);
extern int FourAddr_ClearHashEntry(void);
extern int FourAddr_ClearHashEntrySTA(void);
#endif

static UINT8 ismemzero(UINT8 * mem, UINT32 size)
{
	UINT8 *ptr = mem;
...
#ifdef SOC_W906X
static inline u8 is_he_capable_sta(extStaDb_StaInfo_t * pStaInfo)
{
	if (pStaInfo->he_cap_ie != HE_CAPABILITIES_IE ||
	    (ismemzero((u8 *) & pStaInfo->he_mac_cap, sizeof(HE_Mac_Capabilities_Info_t)) &&
	     ismemzero((u8 *) & pStaInfo->he_phy_cap, sizeof(HE_Phy_Capabilities_Info_t))))
		return 0;
	else
		return 1;
}
...
#define STADB_IS_HE(STAP)	(STAP->heCap.hdr.ext!=0)
#define STADB_IS_HT(STAP)	(STAP->HtElem.ElementId!=0)
#define STADB_IS_VHT(STAP)	(STAP->vhtCap.id!=0)
#endif				/* _STADB_H */
