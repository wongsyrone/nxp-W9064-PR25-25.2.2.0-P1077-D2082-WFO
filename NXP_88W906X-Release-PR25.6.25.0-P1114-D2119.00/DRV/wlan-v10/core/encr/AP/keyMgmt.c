...
void StartGrpKeyTimer(vmacApInfo_t * vmacSta_p)
{
	...
	/* Group rekey in seconds, base timer is 100ms. */
	TimerFireIn(&vmacSta_p->GrpKeytimer, 1, GrpKey_TimeoutHdlr, (unsigned char *)vmacSta_p,
...
void GenerateGrpTransKey(vmacApInfo_t * vmacSta_p)
{
	...
	Mrvl_PRF(mib->mib_MrvlRSN_GrpKey->GrpMasterKey, TK_SIZE_MAX, prefix, strlen(prefix), inp_data, sizeof(inp_data), GTK, 32);
...
void KeyMgmtInit(vmacApInfo_t * vmacSta_p)
{
	...
	if (*(mib->mib_wpaWpa2Mode) < 4) {	/* MRV_8021X For PSK modes use internal WPA state machine */
		...
		if ((mib_RSNConfigWPA2_p->WPA2Enabled || mib_RSNConfigWPA2_p->WPA2OnlyEnabled)
		    && mib_RSNConfigWPA2AuthSuites_p->Enabled && memcmp(mib_RSNConfigWPA2AuthSuites_p->AuthSuites, AuthPSKWPA2, 4) == 0) {
		...
		if (mib_RSNConfigAuthSuites_p->Enabled && memcmp(mib_RSNConfigAuthSuites_p->AuthSuites, AuthPSK, 4) == 0) {
			if (!*(mib->mib_WPAPSKValueEnabled)) {
				PKCS5_PBKDF2(vmacSta_p->Mib802dot11->RSNConfig->PSKPassPhrase,
					     vmacSta_p->Mib802dot11->StationConfig->DesiredSsId,
					     strlen(vmacSta_p->Mib802dot11->StationConfig->DesiredSsId), vmacSta_p->Mib802dot11->RSNConfig->PSKValue);
			}
		}
	...
	if (mib_PrivacyTable_p->RSNEnabled && !mib_RSNConfigWPA2_p->WPA2Enabled && !mib_RSNConfigWPA2_p->WPA2OnlyEnabled) {
		if (mib_RSNConfig_p->MulticastCipher[3] == 2) {
			...
			keyInfo = ENCR_KEY_FLAG_GTK_RX_KEY | ENCR_KEY_FLAG_MICKEY_VALID | ENCR_KEY_FLAG_TSC_VALID;

			memcpy(tkipParam.KeyMaterial, mib->mib_MrvlRSN_GrpKey->EncryptKey, MAX_ENCR_KEY_LENGTH);
			memcpy(tkipParam.TxMicKey, mib->mib_MrvlRSN_GrpKey->TxMICKey, MIC_KEY_LENGTH);
			memcpy(tkipParam.RxMicKey, mib->mib_MrvlRSN_GrpKey->RxMICKey, MIC_KEY_LENGTH);
			tkipParam.Rsc.low = 0;
			...
			keymgmt_aesInfoGet(mib_RSNConfig_p->MulticastCipher[3], &keyType, &keyLen);
			memcpy(aesParam.KeyMaterial, mib->mib_MrvlRSN_GrpKey->EncryptKey, keyLen);
#else
			wlFwSetWpaAesGroupK(vmacSta_p->dev, mib->mib_MrvlRSN_GrpKey->g_KeyIndex, mib_RSNConfig_p->MulticastCipher[3]);
#endif
		...
		if (mib_RSNConfigUnicastCiphers_p->UnicastCipher[3] == 2) {
			...
			wlFwSetWpaAesMode(vmacSta_p->dev, vmacSta_p->macStaAddr, mib_RSNConfigUnicastCiphers_p->UnicastCipher[3]);
		}
#endif
...
#ifdef SOC_W906X
			keyInfo = ENCR_KEY_FLAG_GTK_TX_KEY;
			keymgmt_aesInfoGet(mib_RSNConfigWPA2_p->MulticastCipher[3], &keyType, &keyLen);
			memcpy(aesParam.KeyMaterial, mib->mib_MrvlRSN_GrpKey->EncryptKey, keyLen);
#else
			wlFwSetWpaAesGroupK(vmacSta_p->dev, mib->mib_MrvlRSN_GrpKey->g_KeyIndex, mib_RSNConfigWPA2_p->MulticastCipher[3]);
#endif
		...
		if (mib_RSNConfigWPA2UnicastCiphers_p->UnicastCipher[3] == 2) {
			...
			wlFwSetWpaAesMode(vmacSta_p->dev, vmacSta_p->macStaAddr, mib_RSNConfigWPA2UnicastCiphers_p->UnicastCipher[3]);
			WLDBG_INFO(DBG_LEVEL_5, "************** Set WPA2 AES mode \n");
		}
#endif
	}
#ifdef SOC_W906X
	wlFwSetSecurityKey(vmacSta_p->dev, ACT_SET, keyType, vmacSta_p->macStaAddr, mib->mib_MrvlRSN_GrpKey->g_KeyIndex, keyLen, keyInfo, pParam);
#endif
...
Status_e GeneratePWKMsg1(vmacApInfo_t * vmacSta_p, keyMgmtInfo_t * pKMgmtStnInfo, struct sk_buff **skb_pp, IEEEtypes_MacAddr_t * StaAddr)
{
	...
	*skb_pp = skb;
	...
	if ((pKMgmtStnInfo->RsnIEBuf[0] == 221 && pKMgmtStnInfo->RsnIEBuf[17] == 2)
		...
		keyType = 2;
		   ...
		   || (pKMgmtStnInfo->RsnIEBuf[0] == 48 && isAes4RsnValid(pKMgmtStnInfo->RsnIEBuf[13]))
	    ) {			//CCMP
		tx_eapol_ptr->k.key_info.desc_ver = 2;
		if (pKMgmtStnInfo->RsnIEBuf[0] == 221)
			keymgmt_aesInfoGet(pKMgmtStnInfo->RsnIEBuf[17], &keyType, &keyLen);
		else
			keymgmt_aesInfoGet(pKMgmtStnInfo->RsnIEBuf[13], &keyType, &keyLen);
	} else
		return FAIL;
	...
	tx_eapol_ptr->k.key_info16 = ((tx_eapol_ptr->k.key_info16 << 8) | (tx_eapol_ptr->k.key_info16 >> 8));
...
Status_e GeneratePWKMsg3(vmacApInfo_t * vmacSta_p, keyMgmtInfo_t * pKMgmtStnInfo, struct sk_buff ** skb_pp, IEEEtypes_MacAddr_t * StaAddr)
{
	MIB_802DOT11 *mib = vmacSta_p->Mib802dot11;
	MIB_RSNCONFIGWPA2 *mib_RSNConfigWPA2_p = vmacSta_p->Mib802dot11->RSNConfigWPA2;
	EAPOL_KeyMsg_t *tx_eapol_ptr;
	...
	*skb_pp = skb;
	...
	if (*(pKMgmtStnInfo->RsnIEBuf) == 221) {	//TKIP
		...
		if (mib_RSNConfigWPA2_p->WPA2OnlyEnabled)
			...
			tx_eapol_ptr->key_material_len =
			    AddRSN_IEWPA2MixedMode(vmacSta_p, (IEEEtypes_RSN_IE_WPA2MixedMode_t *) (&tx_eapol_ptr->key_data));
	} else
		return FAIL;
	...
	if (*(pKMgmtStnInfo->RsnIEBuf) == 48) {
		...
		if (mib_RSNConfigWPA2_p->MulticastCipher[3] != 2) {
			...
			keyDataWPA2.length = 6 + keyLen;
			tx_eapol_ptr->key_material_len = RsnIE_len + keyDataWPA2.length + 2;
			memcpy(p_key_data + RsnIE_len, pkeyDataWPA2, 6);
			memcpy(p_key_data + RsnIE_len + 6, (UINT8 *) & GTK_IE, 2 + keyLen);
		} else {
			UINT8 *p_key_data = tx_eapol_ptr->key_data;
			keyDataWPA2.length = 22 + 16;
			tx_eapol_ptr->key_material_len = RsnIE_len + 24 + 16;
			memcpy(p_key_data + RsnIE_len, pkeyDataWPA2, 6);
			memcpy(p_key_data + RsnIE_len + 6, (UINT8 *) & GTK_IE, 18);
		...
		if (PadLen) {
			...
			*(tx_eapol_ptr->key_data + tx_eapol_ptr->key_material_len) = 0xdd;
			memset(p_key_data + tx_eapol_ptr->key_material_len + 1, 0, PadLen - 1);
			tx_eapol_ptr->key_material_len += PadLen;
		}

		memcpy(key, pKMgmtStnInfo->EAPOL_Encr_Key, 16);
		memcpy(plnText, tx_eapol_ptr->key_data,
		       tx_eapol_ptr->key_material_len > sizeof(plnText) ? sizeof(plnText) : tx_eapol_ptr->key_material_len);
		...
		memcpy(tx_eapol_ptr->key_data, cipherText,
		       tx_eapol_ptr->key_material_len > sizeof(cipherText) ? sizeof(cipherText) : tx_eapol_ptr->key_material_len);
	}
	...
	ComputeEAPOL_MIC(vmacSta_p, hdr_8021x,
			 frameLen + sizeof(Hdr_8021x_t), pKMgmtStnInfo->EAPOL_MIC_Key, EAPOL_MIC_KEY_SIZE, MIC, pKMgmtStnInfo->RsnIEBuf);
...
Status_e GenerateGrpMsg1(vmacApInfo_t * vmacSta_p, keyMgmtInfo_t * pKMgmtStnInfo, struct sk_buff ** skb_pp, IEEEtypes_MacAddr_t * StaAddr)
{
	MIB_802DOT11 *mib = vmacSta_p->Mib802dot11;
	MIB_RSNCONFIGWPA2 *mib_RSNConfigWPA2_p = vmacSta_p->Mib802dot11->RSNConfigWPA2;
	...
	*skb_pp = skb;
	...
	if ((pKMgmtStnInfo->RsnIEBuf[0] == 221 && pKMgmtStnInfo->RsnIEBuf[17] == 2)
		...
		memcpy(p_key_data + 16 + 8, mib->mib_MrvlRSN_GrpKey->RxMICKey, 8);
	...
	if ((pKMgmtStnInfo->RsnIEBuf[0] == 221 && pKMgmtStnInfo->RsnIEBuf[17] == 2)
		...
		EncryptGrpKey((UINT8 *) & (pKMgmtStnInfo->EAPOL_Encr_Key),
			      (UINT8 *) & (pTx_eapol->EAPOL_key_IV), (UINT8 *) & (pTx_eapol->key_data), pTx_eapol->key_material_len);
	} else if ((pKMgmtStnInfo->RsnIEBuf[0] == 221 && pKMgmtStnInfo->RsnIEBuf[17] == 4)
		   || (pKMgmtStnInfo->RsnIEBuf[0] == 48 && isAes4RsnValid(pKMgmtStnInfo->RsnIEBuf[13]))
	    ) {
		...
		AES_Wrap(plnText, (WRAPUINT64 *) cipherText, (WRAPUINT64 *) key, pTx_eapol->key_material_len);
...
void KeyMgmtCleanUp(vmacApInfo_t * vmacSta_p, IEEEtypes_MacAddr_t * pStaAddr)
{
	...
	if ((pStaInfo = extStaDb_GetStaInfo(vmacSta_p, pStaAddr, STADB_DONT_UPDATE_AGINGTIME)) == NULL) {
...
MhsmEvent_t const *KeyMgmtHsk_Wait_4_Msg2(keyMgmthsk_hsm_t * me, MhsmEvent_t * msg_p)
{
	...
	case (Event_t) MSGRECVD_EVT:
		...
		if (pStaInfo->keyMgmtStateInfo.RsnIEBuf[0] == 221) {
			...
			} else if (mib_RSNConfigAuthSuites_p->Enabled && memcmp(mib_RSNConfigAuthSuites_p->AuthSuites, Auth8021x, 4) == 0)
		...
		genetate_PTK(me->vmacSta_p, PMK, &rx_eapol_ptr->Ether_Hdr.sa,
			     &rx_eapol_ptr->Ether_Hdr.da, pStaInfo->keyMgmtStateInfo.ANonce, pStaInfo->keyMgmtStateInfo.SNonce, PTK);

		memcpy(pStaInfo->keyMgmtStateInfo.EAPOL_MIC_Key, PTK, 16);
		memcpy(pStaInfo->keyMgmtStateInfo.EAPOL_Encr_Key, (PTK + 16), 16);
		memcpy(pStaInfo->keyMgmtStateInfo.PairwiseTempKey1_tmp, (PTK + 16 + 16), TK_SIZE_MAX);
		...
		ComputeEAPOL_MIC(me->vmacSta_p, hdr_8021x,
				 packetLen, pStaInfo->keyMgmtStateInfo.EAPOL_MIC_Key, EAPOL_MIC_KEY_SIZE, MIC, pStaInfo->keyMgmtStateInfo.RsnIEBuf);

		if (checkEAPOL_MIC(MIC, rx_MIC, EAPOL_MIC_SIZE) != SUCCESS)	//verify MIC
		{
			struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, me->vmacSta_p->dev);
			// free buffer,
			...
			if ((wlpptr->cb_enable == FALSE) || (wlpptr->cbinfo_bss.enable_tx == true)) {
...
#ifdef SOC_W906X
			macMgmtMlme_SendDeauthenticateMsg(me->vmacSta_p, &pStaInfo->Addr, pStaInfo->StnId, IEEEtypes_REASON_IE_4WAY_DIFF, TRUE);
#else
			macMgmtMlme_SendDeauthenticateMsg(me->vmacSta_p, &pStaInfo->Addr, pStaInfo->StnId, IEEEtypes_REASON_IE_4WAY_DIFF);
#endif
...
MhsmEvent_t const *KeyMgmtHsk_Wait_4_Msg4(keyMgmthsk_hsm_t * me, MhsmEvent_t * msg_p)
{
	...
	case (Event_t) MSGRECVD_EVT:
		...
		memcpy(pStaInfo->keyMgmtStateInfo.PairwiseTempKey1, pStaInfo->keyMgmtStateInfo.PairwiseTempKey1_tmp, TK_SIZE_MAX);
		...
		if ((pStaInfo->keyMgmtStateInfo.RsnIEBuf[0] == 221 && pStaInfo->keyMgmtStateInfo.RsnIEBuf[17] == 2)
			...
			if (pStaInfo->keyMgmtStateInfo.RsnIEBuf[0] == 221)
				wlFwSetWpaAesMode(me->vmacSta_p->dev, pStaInfo->Addr, pStaInfo->keyMgmtStateInfo.RsnIEBuf[17]);
			else
				wlFwSetWpaAesMode(me->vmacSta_p->dev, pStaInfo->Addr, pStaInfo->keyMgmtStateInfo.RsnIEBuf[13]);
#endif
		}
...
void KeyMgmtHskCtor(vmacApInfo_t * vmacSta_p, extStaDb_StaInfo_t * pStaInfo)
{
	...
	mhsm_add(&me->waiting_4_msg_2, &me->sTop, (MhsmFcnPtr) KeyMgmtHsk_Wait_4_Msg2);
	mhsm_add(&me->waiting_4_msg_4, &me->sTop, (MhsmFcnPtr) KeyMgmtHsk_Wait_4_Msg4);
	mhsm_add(&me->waiting_4_grpmsg_2, &me->sTop, (MhsmFcnPtr) KeyMgmtHsk_Wait_4_GrpMsg2);
...
void ProcessKeyMgmtData(vmacApInfo_t * vmacSta_p, void *pBuffDesc, IEEEtypes_MacAddr_t * SourceAddr, MhsmEvent_t * msg)
{
	MIB_RSNSTATS *mib_RSNStats_p = vmacSta_p->Mib802dot11->RSNStats;
	extStaDb_StaInfo_t *pStaInfo;

	pStaInfo = extStaDb_GetStaInfo(vmacSta_p, SourceAddr, STADB_DONT_UPDATE_AGINGTIME);
	if (pStaInfo && pStaInfo->State == ASSOCIATED) {
		...
		if (msg->event == MSGRECVD_EVT) {
			...
			if (rx_eapol_ptr->k.key_info16 & KEY_INFO_ERROR && pStaInfo->keyMgmtStateInfo.RSNDataTrafficEnabled == TRUE) {
			...
			if (WORD_SWAP(rx_eapol_ptr->replay_cnt[1]) != pStaInfo->keyMgmtStateInfo.counter && rx_eapol_ptr->k.key_info.key_type == 1) {
				return;
			}
		}
...
void keyMgmt_Timeout(vmacApInfo_t * vmacSta_p, distQ_TimerMsg_t * TimerMsg_p)
{
	...
	if (TimerMsg_p->PendingData_p.type == KEYMGMTTIMEOUTEVENT) {
		ProcessKeyMgmtData(vmacSta_p, NULL, &TimerMsg_p->PendingData_p.StnAddr, &msg);
...
#ifdef SOC_W906X
		wlFwSetSecurityKey(vmacSta_p->dev, ACT_SET, keyType, vmacSta_p->macStaAddr,
				   mib->mib_MrvlRSN_GrpKey->g_KeyIndex, keyLen, keyInfo, pParam);
#endif
...
#ifdef SOC_W906X
			macMgmtMlme_SendDeauthenticateMsg(vmacSta_p, &bcast, MCBC_STN_ID, IEEEtypes_REASON_MIC_FAILURE, FALSE);
#else
			macMgmtMlme_SendDeauthenticateMsg(vmacSta_p, &bcast, MCBC_STN_ID, IEEEtypes_REASON_MIC_FAILURE);
#endif
			ENTER_CRITICAL;
	...
	if (mib_PrivacyTable_p->RSNEnabled) {
		if (pDistMsg->MsgType == STA_ASSOMSGRECVD) {
			...
			WLDBG_INFO(DBG_LEVEL_5, "keyMgmt_msg: STA_ASSOMSGRECVD for = %x:%x:%x:%x:%x:%x \n",
				   ...
				   staAssocMsg_p->staMACAddr[3], staAssocMsg_p->staMACAddr[4], staAssocMsg_p->staMACAddr[5]);
