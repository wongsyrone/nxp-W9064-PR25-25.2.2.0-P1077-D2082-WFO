 ...
 *   */
// static void
void hmac_sha1(const u_int8_t * text, size_t text_len, const u_int8_t * key, size_t key_len, u_int8_t digest[SHA1_DIGEST_LENGTH])
{
	SHA1_CTX ctx;
 ...
 *    */
int pkcs5_pbkdf2(const char *pass, size_t pass_len, const char *salt, size_t salt_len, u_int8_t * key, size_t key_len, u_int rounds)
{
	...
	if (salt_len == 0 || salt_len > SIZE_MAX - 1)
		return -1;
	if ((asalt = wl_kmalloc_autogfp(salt_len + 4)) == NULL)
		return -1;
...
static void printhex(const char *s, const u_int8_t * buf, size_t len)
{
	size_t i;
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < NVECS; i++) {
		...
		for (j = 1; j < sizeof(result); j += 3) {
			if (pkcs5_pbkdf2(vec->pass, strlen(vec->pass), vec->salt, strlen(vec->salt), result, j, vec->rounds) != 0)
