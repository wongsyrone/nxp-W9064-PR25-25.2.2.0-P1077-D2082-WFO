...
void Mrvl_PRF(unsigned char *key, int key_len,
	      unsigned char *prefix, int prefix_len, unsigned char *data, int data_len, unsigned char *output, int len)
{
	int i;
	...
	for (i = 0; i < (len + 19) / 20; i++) {
		Mrvl_hmac_sha1(input, total_len, key, key_len, &output[currentindex]);
