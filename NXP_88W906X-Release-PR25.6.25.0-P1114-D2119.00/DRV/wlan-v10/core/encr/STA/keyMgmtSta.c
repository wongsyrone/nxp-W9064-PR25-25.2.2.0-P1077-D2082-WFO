...
int keyMgmtTxData(struct sk_buff *skb, struct net_device *netdev, extStaDb_StaInfo_t * pStaInfo, BOOLEAN unencrypt)
{
	...
	if (wlxmit(netdev, skb, IEEE_TYPE_DATA, pStaInfo, bcast, unencrypt, 0))
#else
...
void keyMgmtUpdateRsnIE(UINT8 phymacIndex, UINT8 rsn_modeId, UINT32 mCipherId, UINT32 uCipherId, UINT32 aCipherId)
{
	...
	if (rsn_modeId == RSN_WPA_ID) {
		...
		memcpy(pStaSecurityMibs->mib_RSNConfig_p->MulticastCipher,
		       &pStaSecurityMibs->thisStaRsnIE_p->GrpKeyCipher[0], RSN_CIPHER_VALUE_LEN_MAX);

		memcpy(pStaSecurityMibs->mib_RSNConfigUnicastCiphers_p->UnicastCipher,
		       &pStaSecurityMibs->thisStaRsnIE_p->PwsKeyCipherList[0], RSN_CIPHER_VALUE_LEN_MAX);

		/* Reset WPA2 ciphers */
		memset(pStaSecurityMibs->mib_RSNConfigWPA2_p->MulticastCipher, 0, RSN_CIPHER_VALUE_LEN_MAX);
		memset(pStaSecurityMibs->mib_RSNConfigWPA2UnicastCiphers_p->UnicastCipher, 0, RSN_CIPHER_VALUE_LEN_MAX);
		memset(pStaSecurityMibs->thisStaRsnIEWPA2_p, 0, sizeof(IEEEtypes_RSN_IE_WPA2_t));
		...
		memcpy(pStaSecurityMibs->mib_RSNConfigWPA2_p->MulticastCipher,
		       &pStaSecurityMibs->thisStaRsnIEWPA2_p->GrpKeyCipher[0], RSN_CIPHER_VALUE_LEN_MAX);

		memcpy(pStaSecurityMibs->mib_RSNConfigWPA2UnicastCiphers_p->UnicastCipher,
		       &pStaSecurityMibs->thisStaRsnIEWPA2_p->PwsKeyCipherList[0], RSN_CIPHER_VALUE_LEN_MAX);

		/*Reset WPA ciphers */
		memset(pStaSecurityMibs->mib_RSNConfig_p->MulticastCipher, 0, RSN_CIPHER_VALUE_LEN_MAX);
		memset(pStaSecurityMibs->mib_RSNConfigUnicastCiphers_p->UnicastCipher, 0, RSN_CIPHER_VALUE_LEN_MAX);
		memset(pStaSecurityMibs->thisStaRsnIE_p, 0, sizeof(IEEEtypes_RSN_IE_t));
...
*
**************************************************************************/
extern void keyMgmtStaStartTimer(UINT8 * data_p, UINT8 * timer_p, void *callback, UINT32 ticks)
{
...
extern void ComputeEAPOL_MIC_DualMacSta(UINT8 * data, UINT16 data_length,
					UINT8 * MIC_Key, UINT8 MIC_Key_length, UINT8 * computed_MIC, UINT8 * RsnIEBuf, vmacEntry_t * vmacEntry_p)
{
	STA_SECURITY_MIBS *pStaSecurityMibs;
	...
	if (RsnIEBuf) {
		...
		if ((!pStaSecurityMibs->mib_RSNConfigWPA2_p->WPA2OnlyEnabled
		    ...
		    || ((pStaSecurityMibs->mib_RSNConfigWPA2_p->WPA2OnlyEnabled || pStaSecurityMibs->mib_RSNConfigWPA2_p->WPA2Enabled)
		...
		if (((pStaSecurityMibs->thisStaRsnIE_p->ElemId == 221)
		    ...
		    || ((pStaSecurityMibs->thisStaRsnIEWPA2_p->ElemId == 48)
			&& (pStaSecurityMibs->thisStaRsnIEWPA2_p->PwsKeyCipherList[3] == 2)))
#endif
		{
			Mrvl_hmac_md5(data, (int)data_length, MIC_Key, (int)MIC_Key_length, (void *)computed_MIC);
		} else {
...
Status_e GeneratePWKMsg2(struct sk_buff **skb_pp, EAPOL_KeyMsg_t * pRxEapol, keyMgmtInfoSta_t * pKeyMgmtInfoSta)
{
	...
	*skb_pp = skb_p;
	...
	if (pStaSecurityMibs->mib_RSNConfigWPA2_p->WPA2OnlyEnabled || pStaSecurityMibs->mib_RSNConfigWPA2_p->WPA2Enabled)
#else
	if (isAes4RsnValid(pStaSecurityMibs->thisStaRsnIEWPA2_p->PwsKeyCipherList[3]))
#endif
	{
		// WPA2
#ifdef SOC_W906X
	...
	pTx_eapol->k.key_info16 = ((pTx_eapol->k.key_info16 << 8) | (pTx_eapol->k.key_info16 >> 8));

#ifdef PORT_TO_LINUX_OS
	//pTx_eapol->key_length = SHORT_SWAP((TK_SIZE + TK_SIZE));
	pTx_eapol->key_length = 0;
#else
	memcpy(&pTx_eapol->k.key_info16, &tmpKeyInfo.key_info16, sizeof(tmpKeyInfo));
	pTx_eapol->key_length = 0;
#endif				/* PORT_TO_LINUX_OS */
...
#endif
	{
		...
		pTx_eapol->key_material_len =
		    AddRSN_IEWPA2_TO(pStaSecurityMibs->thisStaRsnIEWPA2_p, (IEEEtypes_RSN_IE_WPA2_t *) (&pTx_eapol->key_data));
#endif
	}
#ifdef SOC_W906X
	else
...
#else
	ComputeEAPOL_MIC_DualMacSta((UINT8 *) pTx_eapol + sizeof(ether_hdr_t), frameLen + sizeof(Hdr_8021x_t),
...
Status_e GeneratePWKMsg4(struct sk_buff ** skb_pp, EAPOL_KeyMsg_t * pRxEapol, keyMgmtInfoSta_t * pKeyMgmtInfoSta)
{
	...
	*skb_pp = skb;
...
#endif
	{
		...
		memcpy(&tmpRx_KeyInfo.key_info16, &pRxEapol->k.key_info16, sizeof(key_info_t));
#endif				//port_end
		pRxEapol->k.key_info16 = ((pRxEapol->k.key_info16 << 8) | (pRxEapol->k.key_info16 >> 8));
...
#ifndef AP_MULTI_BSS_WEP
	ComputeEAPOL_MIC(hdr_8021x, frameLen + sizeof(Hdr_8021x_t), gkeyMgmtInfoSta[phymac].EAPOL_MIC_Key, EAPOL_MIC_KEY_SIZE, MIC, 0);
#else
...
MhsmEvent_t const *KeyMgmtStaHsk_Recvd_PWKMsg1(keyMgmtStahsk_hsm_t * me, MhsmEvent_t * msg_p)
{
	...
	struct net_device *dev = ((vmacEntry_t *) pKeyMgmtInfoSta->vmacEntry_p)->privInfo_p;
	...
	genetate_PTK(NULL, PSKValueUr[phymac], &rx_eapol_ptr->Ether_Hdr.sa,
		     &rx_eapol_ptr->Ether_Hdr.da, pKeyMgmtInfoSta->ANonce, pKeyMgmtInfoSta->SNonce, PTK);
#else
	genetate_PTK(0,
		     &rx_eapol_ptr->Ether_Hdr.sa,
		     &rx_eapol_ptr->Ether_Hdr.da, pKeyMgmtInfoSta->ANonce, pKeyMgmtInfoSta->SNonce, PTK, PSKValueUr[phymac]);
#endif				/* PORT_TO_LINUX_OS */

	memcpy(pKeyMgmtInfoSta->EAPOL_MIC_Key, PTK, 16);
	memcpy(pKeyMgmtInfoSta->EAPOL_Encr_Key, (PTK + 16), 16);
	memcpy(pKeyMgmtInfoSta->PairwiseTempKey_tmp, (PTK + 16 + 16), TK_SIZE_MAX);
	...
	if ((StaInfo_p = extStaDb_GetStaInfo(wlpptrSta->vmacSta_p, (IEEEtypes_MacAddr_t *) GetParentStaBSSID(phymac),
...
EAPOL_KeyDataWPA2_t *parseKeyKDE_DataType(UINT8 * pData, SINT32 dataLen, IEEEtypes_KDEDataType_e KDEDataType)
{
	...
	while (dataLen > (SINT32) sizeof(IEEEtypes_InfoElementHdr_t)) {
		...
		if (pIe->ElementId == PROPRIETARY_IE) {
			...
			if (pKde->length > sizeof(EAPOL_KeyDataWPA2_t) &&
			    !memcmp((void *)pKde->OUI, kde_oui, sizeof(kde_oui)) && (pKde->dataType == KDEDataType)) {
				return pKde;
			} else if (pIe->Len == 0) {
...
MhsmEvent_t const *KeyMgmtStaHsk_Recvd_PWKMsg3(keyMgmtStahsk_hsm_t * me, MhsmEvent_t * msg_p)
{
	...
	if (memcmp(&rx_eapol_ptr->key_nonce, pKeyMgmtInfoSta->ANonce, NONCE_SIZE) != 0) {
		...
		memcpy(evtBufAnonceDiff.authenticatorMACAddr, GetParentStaBSSID(vmacEntry_p->phyHwMacIndx), 6);
		memcpy(evtBufAnonceDiff.supplicantMACAddr, vmacEntry_p->vmacAddr, 6);
		evtBufAnonceDiff.messageType = RSN_PWK_MSG_3;
		eventGenerate(EVT_CLIENT, EVT_CLIENT_SUBTYPE_RSN_FAIL_ANONCE_DIFF, sizeof(evtBufAnonceDiff), &evtBufAnonceDiff);
#endif				/* PORT_TO_LINUX_OS */
		return msg_p;
	}
	...
	ComputeEAPOL_MIC((UINT8 *) & rx_eapol_ptr->hdr_8021x,
			 ...
			 pKeyMgmtInfoSta->EAPOL_MIC_Key, EAPOL_MIC_KEY_SIZE, MIC, 0);
#else
	...
	if (checkEAPOL_MIC(MIC, rx_MIC, EAPOL_MIC_SIZE) != SUCCESS) {
		...
		memcpy(evtBufMicDiff.authenticatorMACAddr, GetParentStaBSSID(vmacEntry_p->phyHwMacIndx), 6);
		memcpy(evtBufMicDiff.supplicantMACAddr, vmacEntry_p->vmacAddr, 6);
		evtBufMicDiff.messageType = RSN_PWK_MSG_3;
		eventGenerate(EVT_CLIENT, EVT_CLIENT_SUBTYPE_RSN_FAIL_MIC_DIFF, sizeof(evtBufMicDiff), &evtBufMicDiff);
#endif				/* PORT_TO_LINUX_OS */
...
#ifndef PORT_TO_LINUX_OS
	memcpy(&tmpKeyInfo.key_info16, &rx_eapol_ptr->k.key_info16, sizeof(tmpKeyInfo));
#endif				/* PORT_TO_LINUX_OS */

	if (((rx_eapol_ptr->k.key_info16 << 8) | (rx_eapol_ptr->k.key_info16 >> 8)) & ENCRYPTEDKEYDATA) {
#ifdef SOC_W906X
		pKeyMgmtInfoSta->pKeyData->RSNSecured = 1;
		mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].g_IV16 = rx_eapol_ptr->key_RSC[1] << 8;
		mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].g_IV16 |= (rx_eapol_ptr->key_RSC[0] & 0x00FF);
		...
		if (pStaSecurityMibs->thisStaRsnIE_p->ElemId == 221) {
...
#endif
			{
				...
				EncryptGrpKey(pKeyMgmtInfoSta->EAPOL_Encr_Key,
					      rx_eapol_ptr->EAPOL_key_IV, rx_eapol_ptr->key_data, SHORT_SWAP(rx_eapol_ptr->key_length));
			}
		}
#ifdef SOC_W906X
		else {
			...
			if (isAes4RsnValid(pStaSecurityMibs->mib_RSNConfigWPA2UnicastCiphers_p->UnicastCipher[3])) {
				...
				if (keyLen >= 256) {
					...
					WLDBG_ERROR(DBG_LEVEL_0, "Incorrect key len: %d\n ", keyLen);
					keyLen = 24;
				}
#else
			...
			if (isAes4RsnValid(pStaSecurityMibs->thisStaRsnIEWPA2_p->PwsKeyCipherList[3])) {
				...
				AES_UnWrap((WRAPUINT64 *) & plnText[0], (WRAPUINT64 *) & cipherText[0],
					   (WRAPUINT64 *) & pKeyMgmtInfoSta->EAPOL_Encr_Key[0], keyLen);
				memcpy(rx_eapol_ptr->key_data, plnText + 8, keyLen - 8);
			} else {
				// TKIP
				EncryptGrpKey(pKeyMgmtInfoSta->EAPOL_Encr_Key, rx_eapol_ptr->EAPOL_key_IV, rx_eapol_ptr->key_data, keyLen);
			}
			...
			if (pKde) {
				...
				// handle Mixed case 
				if (isAes4RsnValid(pStaSecurityMibs->mib_RSNConfigWPA2_p->MulticastCipher[3])) {
					// AES
					memcpy(mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].EncryptKey, pGtk, TK_SIZE_MAX);

					keymgmt_aesInfoGet(pStaSecurityMibs->mib_RSNConfigWPA2_p->MulticastCipher[3], &keyType, &keyLen);

					memcpy(param.KeyMaterial, mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].EncryptKey, keyLen);

					keyInfo = ENCR_KEY_FLAG_GTK_RX_KEY | ENCR_KEY_FLAG_STA_MODE;

#ifdef PORT_TO_LINUX_OS

					wlFwSetSecurityKey(dev, ACT_SET, keyType,
							   vmacEntry_p->vmacAddr, keyIndex, keyLen, keyInfo, (UINT8 *) & param);
					...
					memcpy(mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].RxMICKey, pGtk + TK_SIZE, MIC_KEY_LENGTH);
					memcpy(mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].TxMICKey, pGtk + TK_SIZE + MIC_KEY_LENGTH,
					       MIC_KEY_LENGTH);

					memcpy(param.KeyMaterial, pGtk, TK_SIZE);
					memcpy(param.RxMicKey, pGtk + TK_SIZE, MIC_KEY_LENGTH);
					memcpy(param.TxMicKey, pGtk + TK_SIZE + MIC_KEY_LENGTH, MIC_KEY_LENGTH);
#ifdef PORT_TO_LINUX_OS

					param.Tsc.low = mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].g_IV16;
					param.Tsc.high = mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].g_IV32;

					wlFwSetSecurityKey(dev, ACT_SET, KEY_TYPE_ID_TKIP,
							   (UINT8 *) vmacEntry_p->vmacAddr, keyIndex, TK_SIZE, keyInfo, (UINT8 *) & param);
#endif				/* PORT_TO_LINUX_OS */
#else
				pGtk = &pKde->data[2];
				keyIndex = pKde->data[0] & 0x03;
				//Pete, avoid reinstall GTK if it's the same 
				if (!(key_installed && (memcmp(mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].EncryptKey, pGtk, TK_SIZE) == 0))) {
					pKeyMgmtInfoSta->pKeyData->RSNSecured = 1;
					mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].g_IV16 = rx_eapol_ptr->key_RSC[1] << 8;
					mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].g_IV16 |= rx_eapol_ptr->key_RSC[0];
					mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].g_IV32 = 0xFFFFFFFF;
					// handle Mixed case 
					if (isAes4RsnValid(pStaSecurityMibs->thisStaRsnIEWPA2_p->GrpKeyCipher[3])) {
						// AES
						memcpy(mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].EncryptKey, pGtk, TK_SIZE_MAX);
#ifdef PORT_TO_LINUX_OS
						wlFwSetWpaAesMode_STA(dev, GetParentStaBSSID(vmacEntry_p->phyHwMacIndx),
						...
						wlFwSetWpaAesGroupK_STA(dev,
									...
									keyIndex, pStaSecurityMibs->thisStaRsnIEWPA2_p->GrpKeyCipher[3]);
#ifdef V6FW
						wlFwSetWpaAesMode_STA(dev, (UINT8 *) vmacEntry_p->vmacAddr,
								      pStaSecurityMibs->thisStaRsnIEWPA2_p->GrpKeyCipher[3]);
						...
						// Tkip
						memcpy(mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].EncryptKey, pGtk, TK_SIZE);
						memcpy(mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].RxMICKey, pGtk + 16, 8);
						memcpy(mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].TxMICKey, pGtk + 16 + 8, 8);
#ifdef PORT_TO_LINUX_OS
						{
							...
							wlFwSetWpaTkipMode_STA(dev, GetParentStaBSSID(vmacEntry_p->phyHwMacIndx));
							...
							wlFwSetWpaTkipGroupK_STA(dev,
										 ...
										 MIC_KEY_LENGTH, TkipTsc, keyIndex);
#ifdef V6FW
							wlFwSetWpaTkipMode_STA(dev, (UINT8 *) vmacEntry_p->vmacAddr);
	...
	updateApReplayCounter(pKeyMgmtInfoSta, (UINT8 *) & rx_eapol_ptr->replay_cnt[0]);
	memcpy(pKeyMgmtInfoSta->pKeyData->PairwiseTempKey, pKeyMgmtInfoSta->PairwiseTempKey_tmp, TK_SIZE_MAX);
	memcpy(pKeyMgmtInfoSta->pKeyData->RSNPwkTxMICKey, pKeyMgmtInfoSta->RSNPwkTxMICKey_tmp, 8);
	memcpy(pKeyMgmtInfoSta->pKeyData->RSNPwkRxMICKey, pKeyMgmtInfoSta->RSNPwkRxMICKey_tmp, 8);
...
#ifdef SOC_W906X
	{
		...
		if ((StaInfo_p =
		     extStaDb_GetStaInfo(wlpptrSta->vmacSta_p,
					 (IEEEtypes_MacAddr_t *) GetParentStaBSSID(((vmacEntry_t *) pKeyMgmtInfoSta->vmacEntry_p)->phyHwMacIndx),
					 STADB_DONT_UPDATE_AGINGTIME)) != NULL) {
			if (keyMgmtTxData(skb, dev, StaInfo_p, TRUE)) {
				wl_free_skb(skb);
				return msg_p;
...
//#endif /* WFA_11AX_PF */
			...
			AddRSN_IEWPA2_TO(pStaSecurityMibs->thisStaRsnIEWPA2_p,
					 (IEEEtypes_RSN_IE_WPA2_t *) (&StaInfo_p->keyMgmtStateInfo.RsnIEBuf[0]));
			memcpy(&StaInfo_p->keyMgmtStateInfo.PairwiseTempKey1[0], &pKeyMgmtInfoSta->pKeyData->PairwiseTempKey[0], TK_SIZE_MAX);
			memcpy(&StaInfo_p->keyMgmtStateInfo.RSNPwkTxMICKey[0], &pKeyMgmtInfoSta->pKeyData->RSNPwkTxMICKey[0], 8);
			memcpy(&StaInfo_p->keyMgmtStateInfo.RSNPwkRxMICKey[0], &pKeyMgmtInfoSta->pKeyData->RSNPwkRxMICKey[0], 8);
			...
			if (!pStaSecurityMibs->mib_RSNConfigWPA2_p->WPA2OnlyEnabled && !pStaSecurityMibs->mib_RSNConfigWPA2_p->WPA2Enabled) {
				...
				if (pStaSecurityMibs->mib_RSNConfigUnicastCiphers_p->UnicastCipher[3] == 2) {
					...
					keyLen = TK_SIZE;
					getTkipStaKeyMaterial(StaInfo_p, &tkipParam);
				...
				if (isAes4RsnValid(pStaSecurityMibs->mib_RSNConfigWPA2UnicastCiphers_p->UnicastCipher[3])) {
					...
					keymgmt_aesInfoGet(pStaSecurityMibs->mib_RSNConfigWPA2UnicastCiphers_p->UnicastCipher[3], &keyType, &keyLen);
			...
			wlFwSetSecurityKey(dev, ACT_SET, keyType, StaInfo_p->Addr, 0, keyLen, keyInfo, pParam);
		} else {
			wl_free_skb(skb);
			return msg_p;
		}
	...
	if ((StaInfo_p =
		...
		if (!key_installed) {
			...
			if (pStaSecurityMibs->thisStaRsnIE_p->ElemId == 221) {
				...
				if (pStaSecurityMibs->thisStaRsnIE_p->PwsKeyCipherList[3] == 2) {
					// TKIP
					wlFwSetWpaTkipMode_STA(dev, (UINT8 *) & StaInfo_p->Addr);
				} else if (pStaSecurityMibs->thisStaRsnIE_p->PwsKeyCipherList[3] == 4) {
					// AES
					wlFwSetWpaAesMode_STA(dev, (UINT8 *) & StaInfo_p->Addr,
	...
	if (pKeyMgmtInfoSta->pKeyData->RSNSecured != 0) {
		...
		memcpy(evtBufRsnSecured.authenticatorMACAddr, GetParentStaBSSID(vmacEntry_p->phyHwMacIndx), 6);
		memcpy(evtBufRsnSecured.supplicantMACAddr, vmacEntry_p->vmacAddr, 6);
		eventGenerate(EVT_CLIENT, EVT_CLIENT_SUBTYPE_RSN_SECURED, sizeof(evtBufRsnSecured), &evtBufRsnSecured);
#endif				/* PORT_TO_LINUX_OS */

		vStaInfo_p->staSecurityMibs.mib_PrivacyTable_p->RSNLinkStatus = 1;
		WLSYSLOG(dev, WLSYSLOG_CLASS_ALL, WLSYSLOG_MSG_PSK_SUCCESS "%02x%02x%02x%02x%02x%02x\n",
			 rx_eapol_ptr->Ether_Hdr.sa[0], rx_eapol_ptr->Ether_Hdr.sa[1],
			 rx_eapol_ptr->Ether_Hdr.sa[2], rx_eapol_ptr->Ether_Hdr.sa[3], rx_eapol_ptr->Ether_Hdr.sa[4], rx_eapol_ptr->Ether_Hdr.sa[5]);
...
MhsmEvent_t const *KeyMgmtStaHsk_Recvd_GrpMsg1(keyMgmtStahsk_hsm_t * me, MhsmEvent_t * msg_p)
{
	...
	UINT8 *enKey = mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].EncryptKey,
	    *txMicKey = mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].TxMICKey,
	    *rxMicKey = mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].RxMICKey;
#endif
	...
	if (pStaSecurityMibs->thisStaRsnIE_p->ElemId == 221) {
...
#endif				/* SOC_W906X */
		{
			...
			EncryptGrpKey(pKeyMgmtInfoSta->EAPOL_Encr_Key,
				      rx_eapol_ptr->EAPOL_key_IV, rx_eapol_ptr->key_data, SHORT_SWAP(rx_eapol_ptr->key_length));
#ifdef SOC_W906X
			memcpy(mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].EncryptKey, p_key_data, TK_SIZE);
			memcpy(mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].RxMICKey, p_key_data + TK_SIZE, 8);
			memcpy(mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].TxMICKey, p_key_data + TK_SIZE + 8, 8);
#else
			if (memcmp(enKey, rx_eapol_ptr->key_data, 16) == 0 &&
			    memcmp(txMicKey, rx_eapol_ptr->key_data + 16, 8) == 0 && memcmp(rxMicKey, rx_eapol_ptr->key_data + 24, 8) == 0)
				replayed_gtk = 1;
			...
			if (keyLen >= (256 - 8)) {
				// Abnormal situation => hardcode keyLen = 24
				WLDBG_ERROR(DBG_LEVEL_0, "Incorrect key len: %d, CCMP unicast, \n ", keyLen);
				keyLen = 24;
			}
#endif
			memcpy(cipherText, rx_eapol_ptr->key_data, keyLen);
			AES_UnWrap((WRAPUINT64 *) & plnText[0], (WRAPUINT64 *) & cipherText[0], (WRAPUINT64 *) & pKeyMgmtInfoSta->EAPOL_Encr_Key[0],
				   keyLen);
			...
			if (pStaSecurityMibs->mib_RSNConfig_p->MulticastCipher[3] == 4) {
				// CCMP group
				memcpy(mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].EncryptKey, p_key_data, TK_SIZE_MAX);
			} else {
#ifdef SOC_W906X
				memcpy(mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].EncryptKey, p_key_data, TK_SIZE);
				memcpy(mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].RxMICKey, p_key_data + TK_SIZE, 8);
...
#else
		if (pStaSecurityMibs->thisStaRsnIEWPA2_p->PwsKeyCipherList[3] == 2)
#endif
		{
			//TKIP
			EncryptGrpKey(pKeyMgmtInfoSta->EAPOL_Encr_Key, rx_eapol_ptr->EAPOL_key_IV, rx_eapol_ptr->key_data, keyLen);
			...
			if (keyLen >= 256) {
				// Abnormal situation => hardcode keyLen = 24
				WLDBG_ERROR(DBG_LEVEL_0, "Incorrect key len: %d, CCMP unicast_1\n ", keyLen);
				keyLen = 24;
			}
#endif
			memcpy(cipherText, rx_eapol_ptr->key_data, keyLen);
...
#ifdef SOC_W906X
		ouiType = pStaSecurityMibs->mib_RSNConfigWPA2_p->MulticastCipher[3];
#else
	...
	if ((isAes4RsnValid(pStaSecurityMibs->mib_RSNConfigWPA2_p->MulticastCipher[3])) ||
		...
		if (keyLen > TK_SIZE_MAX) {
			WLDBG_ERROR(DBG_LEVEL_0, "Incorrect keyLen=%d, exp(0~%d)\n", keyLen, TK_SIZE_MAX);
			keyLen = TK_SIZE_MAX;
		}
		memcpy(param.KeyMaterial, mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].EncryptKey, keyLen);
		...
		wlFwSetSecurityKey(dev, ACT_SET, keyType, vmacEntry_p->vmacAddr, keyIndex, keyLen, keyInfo, (UINT8 *) & param);
	} else {
		TKIP_TYPE_KEY param;
		UINT32 keyInfo = ENCR_KEY_FLAG_GTK_RX_KEY | ENCR_KEY_FLAG_MICKEY_VALID | ENCR_KEY_FLAG_TSC_VALID | ENCR_KEY_FLAG_STA_MODE;

		memcpy(param.KeyMaterial, &mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].EncryptKey[0], TK_SIZE);
		memcpy(param.RxMicKey, &mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].RxMICKey, MIC_KEY_LENGTH);
		memcpy(param.TxMicKey, &mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].TxMICKey, MIC_KEY_LENGTH);

		param.Tsc.low = mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].g_IV16;
		param.Tsc.high = mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].g_IV32;

		wlFwSetSecurityKey(dev, ACT_SET, KEY_TYPE_ID_TKIP, vmacEntry_p->vmacAddr, keyIndex, TK_SIZE, keyInfo, (UINT8 *) & param);

	}
#else
	if (!replayed_gtk) {
		if ((isAes4RsnValid(pStaSecurityMibs->thisStaRsnIEWPA2_p->GrpKeyCipher[3])) ||
		    (pStaSecurityMibs->thisStaRsnIEWPA2_p->GrpKeyCipher[3] == 4)) {
			wlFwSetWpaAesGroupK_STA(dev,
						GetParentStaBSSID(vmacEntry_p->phyHwMacIndx),
						&mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].EncryptKey[0], keyIndex, ouiType);
#ifdef V6FW
			wlFwSetWpaAesGroupK_STA(dev,
						(UINT8 *) vmacEntry_p->vmacAddr,
						&mib_MrvlRSN_GrpKeyUr[vmacEntry_p->phyHwMacIndx].EncryptKey[0], keyIndex, ouiType);
...
void KeyMgmtStaHskCtor(keyMgmtStahsk_hsm_t * me)
{
	...
	mhsm_add(&me->sta_hsk_start, &me->sTop, (MhsmFcnPtr) KeyMgmtStaHsk_Start);
	mhsm_add(&me->recvd_pwk_msg_1, &me->sTop, (MhsmFcnPtr) KeyMgmtStaHsk_Recvd_PWKMsg1);
	mhsm_add(&me->recvd_pwk_msg_3, &me->sTop, (MhsmFcnPtr) KeyMgmtStaHsk_Recvd_PWKMsg3);
	mhsm_add(&me->recvd_grp_msg_1, &me->sTop, (MhsmFcnPtr) KeyMgmtStaHsk_Recvd_GrpMsg1);
...
void KeyMgmtInitSta(UINT8 phymac)
{
	...
	if (pStaSecurityMibs->mib_RSNConfigWPA2_p->WPA2OnlyEnabled || pStaSecurityMibs->mib_RSNConfigWPA2_p->WPA2Enabled) {
		if (!staMib_WPA2_PSKValueEnabled[phymac]) {
			...
			memcpy(PSKValueUr[phymac], pStaSecurityMibs->mib_RSNConfigWPA2_p->PSKValue, RSN_PSK_VALUE_LEN_MAX);
		}
		...
		if (!staMib_WPA_PSKValueEnabled[phymac]) {
			...
			memcpy(PSKValueUr[phymac], pStaSecurityMibs->mib_RSNConfig_p->PSKValue, RSN_PSK_VALUE_LEN_MAX);
		}
	}
...
#ifndef PORT_TO_LINUX_OS
	if (!pStaSecurityMibs->mib_RSNConfigWPA2_p->WPA2OnlyEnabled
	    && !pStaSecurityMibs->mib_RSNConfigWPA2_p->WPA2Enabled && ((pStaSecurityMibs->mib_RSNConfigUnicastCiphers_p->UnicastCipher[3] == 2)
...
#ifndef PORT_TO_LINUX_OS
	memcpy(&tmpKeyInfo.key_info16, &pEAPoL->k.key_info16, sizeof(key_info_t));
#endif				/* PORT_TO_LINUX_OS */
...
#endif				/* PORT_TO_LINUX_OS */
	if (keyMgmtInfoSta_p->staCounterHi == 0xffffffff && keyMgmtInfoSta_p->staCounterLo == 0xffffffff) {
...
#ifdef PORT_TO_LINUX_OS
	if ((StaInfo_p =
	     extStaDb_GetStaInfo(vmac_p, (IEEEtypes_MacAddr_t *) GetParentStaBSSID(((vmacEntry_t *) keyMgmtInfoSta_p->vmacEntry_p)->phyHwMacIndx),
				 STADB_DONT_UPDATE_AGINGTIME)) != NULL) {
		if (keyMgmtTxData(skb, dev, StaInfo_p, TRUE)) {
			wl_free_skb(skb);
			return FAIL;
...
#ifndef SOC_W906X
void MicErrDisconnect_Sta(UINT8 * data)
{
	...
	if (keyMgmtInfo_p->sta_MIC_Error.MICCounterMeasureEnabled) {
		...
		case NO_MIC_FAILURE:
			...
			keyMgmtStaStartTimer((UINT8 *) & keyMgmtInfo_p->sta_MIC_Error,
					     (UINT8 *) & keyMgmtInfo_p->sta_MIC_Error.timer, &MicErrTimerExp_Sta, MIC_ERROR_QUIET_TIME_INTERVAL);
#else
			if (!keyMgmtInfo_p->sta_MIC_Error.timer.active) {
				...
				TimerFireIn(&keyMgmtInfo_p->sta_MIC_Error.timer, 1,
					    &MicErrTimerExp_Sta, (UINT8 *) & keyMgmtInfo_p->sta_MIC_Error, MIC_ERROR_QUIET_TIME_INTERVAL);
			} else {
		...
		case FIRST_MIC_FAIL_IN_60_SEC:
			...
			smeSndLinkLostInd(vmacEntry_p, IEEEtypes_REASON_MIC_FAILURE);
#else
			keyMgmtStaStartTimer((UINT8 *) vmacEntry_p, (UINT8 *) & keyMgmtInfo_p->sta_MIC_Error.discon_timer, &MicErrDisconnect_Sta, 10);
#endif
			ENTER_CRITICAL;
			...
			keyMgmtStaStartTimer((UINT8 *) & keyMgmtInfo_p->sta_MIC_Error,
					     ...
					     &MicErrTimerExp_Sta, MIC_ERROR_QUIET_TIME_INTERVAL + MIC_ERROR_QUIET_TIME_INTERVAL_CAL);
#else
			if (!keyMgmtInfo_p->sta_MIC_Error.timer.active) {
				TimerInit(&keyMgmtInfo_p->sta_MIC_Error.timer);
				TimerFireIn(&keyMgmtInfo_p->sta_MIC_Error.timer, 1,
...
static void keyMgmtStaRsnSecuredTimeoutHandler(UINT8 * ctx)
{
	...
	if (pKeyMgmtInfoSta->pKeyData->RSNSecured == 0) {
		...
		eventGenerate(EVT_CLIENT, EVT_CLIENT_SUBTYPE_DISCONNECT, sizeof(evtBufLink), &evtBufLink);
#endif				/* PORT_TO_LINUX_OS */
...
extern void keyMgmtSta_StartSession(vmacEntry_t * vmacEntry_p)
{
	...
	keyMgmtStaStartTimer((UINT8 *) pKeyMgmtInfoSta, (UINT8 *) pTimer, &keyMgmtStaRsnSecuredTimeoutHandler, gStaRsnSecuredTimeout);
#else
	if (pTimer->active) {
		TimerRearm(pTimer, gStaRsnSecuredTimeout);
	} else {
		TimerInit(pTimer);
		TimerFireIn(pTimer, 1, keyMgmtStaRsnSecuredTimeoutHandler, (UINT8 *) pKeyMgmtInfoSta, gStaRsnSecuredTimeout);
	}
#endif				/* PORT_TO_LINUX_OS */
...
UINT8 tmpClientSSID[NUM_OF_WLMACS][33];
...
void defaultKeyMgmtInit(UINT8 phymacIndex)
{
	...
	if (mib->Privacy->RSNEnabled) {
		if (staMib_RSNConfigWPA2_p[phymacIndex]->WPA2Enabled || staMib_RSNConfigWPA2_p[phymacIndex]->WPA2OnlyEnabled) {
			...
			memcpy(staMib_RSNConfigWPA2_p[phymacIndex]->MulticastCipher, rsnMultiCastCipher, RSN_CIPHER_VALUE_LEN_MAX);
			memcpy(staMib_RSNConfigWPA2UnicastCiphers_p[phymacIndex]->UnicastCipher, rsnUniCastCipher, RSN_CIPHER_VALUE_LEN_MAX);
			staMib_RSNConfigWPA2UnicastCiphers_p[phymacIndex]->Enabled = TRUE;
			memcpy(staMib_RSNConfigWPA2UnicastCiphers2_p[phymacIndex]->UnicastCipher, rsnUniCastCipher, RSN_CIPHER_VALUE_LEN_MAX);
			staMib_RSNConfigWPA2UnicastCiphers2_p[phymacIndex]->Enabled = TRUE;
			/* Set Auth Suite */
			memcpy(staMib_RSNConfigWPA2AuthSuites_p[phymacIndex]->AuthSuites, rsnAuthSuite, RSN_SUITE_VALUE_LEN_MAX);
			staMib_RSNConfigWPA2AuthSuites_p[phymacIndex]->Enabled = TRUE;

			memcpy(staMib_RSNConfigWPA2_p[phymacIndex]->PSKPassPhrase, mib->RSNConfigWPA2->PSKPassPhrase, RSN_PSK_PASS_PHRASE_LEN_MAX);

			memcpy(staMib_RSNConfigWPA2_p[phymacIndex]->PSKValue, mib->RSNConfigWPA2->PSKValue, RSN_PSK_VALUE_LEN_MAX);
		} else {
			rsn_mode = RSN_WPA_ID;
			rsnMultiCastCipher[0] = 0x00;
			...
			memcpy(staMib_RSNConfig_p[phymacIndex]->MulticastCipher, rsnMultiCastCipher, RSN_CIPHER_VALUE_LEN_MAX);
			memcpy(staMib_RSNConfigUnicastCiphers_p[phymacIndex]->UnicastCipher, rsnUniCastCipher, RSN_CIPHER_VALUE_LEN_MAX);
			staMib_RSNConfigUnicastCiphers_p[phymacIndex]->Enabled = TRUE;

			memcpy(staMib_RSNConfig_p[phymacIndex]->PSKPassPhrase, mib->RSNConfig->PSKPassPhrase, RSN_PSK_PASS_PHRASE_LEN_MAX);

			memcpy(staMib_RSNConfig_p[phymacIndex]->PSKValue, mib->RSNConfig->PSKValue, RSN_PSK_VALUE_LEN_MAX);
		}
		KeyMgmtInitSta(phymacIndex);
		keyMgmtUpdateRsnIE(phymacIndex, rsn_mode, mCipherId, uCipherId, aCipherId);
	}
