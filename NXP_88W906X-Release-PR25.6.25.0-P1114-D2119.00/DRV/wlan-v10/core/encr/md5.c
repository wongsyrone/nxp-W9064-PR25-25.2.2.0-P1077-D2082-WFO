...
{
	md5_word_t a = pms->state[0], b = pms->state[1], c = pms->state[2], d = pms->state[3];
	md5_word_t t;
#if BYTE_ORDER > 0
...
#define ROTATE_LEFT(x, n) (((x) << (n)) | ((x) >> (32 - (n))))
	...
	/* Round 1. */
	/* Let [state k s i] denote the operation
...
#undef SET
	...
	/* Round 3. */
	/* Let [state k s t] denote the operation
...
void md5_append(md5_state_t * pms, const md5_byte_t * data, int nbytes)
{
	...
	if (offset) {
		...
		{
			md5_byte_t *pdst = (md5_byte_t *) (pms->buffer + offset);
...
void md5_finish(md5_state_t * pms, md5_byte_t digest[16])
{
	...
	for (i = 0; i < 16; ++i)
		digest[i] = (md5_byte_t) (pms->state[i >> 2] >> ((i & 3) << 3));
}
