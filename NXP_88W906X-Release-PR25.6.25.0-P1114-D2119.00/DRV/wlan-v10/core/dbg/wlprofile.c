...
u64 xxGetTimeStamp(void)
{
	...
	return (UINT64) ((UINT64) (tv.tv_sec) * 1000000LL + (UINT64) (tv.tv_usec));
#else
	//ns resolution
	...
	return (UINT64) ((UINT64) (tv.tv_sec) * 1000000000LL + (UINT64) (tv.tv_nsec));
#endif
}
...
use do_div() for both 
*/
void convert_tscale(UINT64 tick, UINT64 * sec, UINT64 * ms, UINT64 * us)
{
	UINT64 rem;
...
void logTPStats(TP_STATS * ptpstats, u32 cnt, u32 size)
{
...
void logTPCounter(TP_STATS * ptpstats, u32 idv0, u32 idv1, u32 idv2)
{
	u16 idx = ptpstats->idx;
...
void calculateTpStats(TP_STATS * ptpstats, int level, char *sysfs_buff)
{
	...
	for (i = 0; i < TP_TRACE_POINTS_MAX - 1; i++) {
		...
		if (duration) {
			...
			if (level == 0) {	//full log
				Sysfs_Printk("%u frames in ivl %u usec, PPS:%u, Rate:%llu Kbps\n", pktcnt, duration,
					     ((tsec) ? (pktcnt / tsec) : pktcnt), temp64);
			} else {
				Sysfs_Printk("PPS:%u, Rate:%llu Kbps\n", ((tsec) ? (pktcnt / tsec) : pktcnt), temp64);
			}

			if (pktcnt)
				temp = ptpstats->tp_records[idx].idvcnt[0][2] / pktcnt;

			if (level == 0)	//full log
			{
				Sysfs_Printk("  Pkt pending counts, Max:%u, Min:%u, Avg:%u\n",
					     ptpstats->tp_records[idx].idvcnt[0][0], ptpstats->tp_records[idx].idvcnt[0][1], temp);
				Sysfs_Printk("  Last [txq_drv_sent_cnt] value:%u \n", ptpstats->tp_records[idx].idvcnt[1][0]);
				Sysfs_Printk("  Last [txq_full_cnt] value:%u \n", ptpstats->tp_records[idx].idvcnt[2][0]);
			}
		}
	}
}
...
#ifdef TP_PROFILE
int wl_tp_profile_test(int tp_point, struct sk_buff *skb, struct net_device *netdev)
{
