...
#define MAX_WDS_PORT 6		// Also in wds.c
	...
	mib_conf_capability mib_conf_capab[MAX_CARDS_SUPPORT][MAX_VMAC_MIB_INSTANCE];
	MIB_ATFINFO mib_atf_info;
#ifdef WLS_FTM_SUPPORT
	UINT8 wls_ftm_enable[MAX_CARDS_SUPPORT];
#endif
	IEEEtypes_BSS_TM_Request_t mib_BSSTMRequest[MAX_CARDS_SUPPORT][MAX_VMAC_MIB_INSTANCE];
} MIB_s;
...
extern BOOLEAN mib_InitAp(MIB_802DOT11 * mib, char *addr, int phyMacId, int vMacId, int shadow)
{
	...
	for (i = 0; i < 2; i++) {
		if (!wlanmib[i]) {
			wlanmib[i] = (struct MIB_s *)wl_vzalloc(sizeof(struct MIB_s));
			if (!wlanmib[i]) {
...
#ifdef BRS_SUPPORT
	mib->BssBasicRateMask = &wlanmib[shadow]->BssBasicRateMask[phyMacId][vMacId];
	mib->NotBssBasicRateMask = &wlanmib[shadow]->NotBssBasicRateMask[phyMacId][vMacId];
#endif
	...
	mib->mib_shortSlotTime = &wlanmib[shadow]->mib_shortSlotTime[phyMacId][vMacId];
...
#ifdef CONFIG_MC_BC_RATE
	mib->mib_mcDataRateInfo = &wlanmib[shadow]->mib_mcDataRateInfo[phyMacId][vMacId];
	mib->mib_bcDataRateInfo = &wlanmib[shadow]->mib_bcDataRateInfo[phyMacId][vMacId];
#endif
	...
	mib->mib_txDataRateInfo = &wlanmib[shadow]->mib_txDataRateInfo[phyMacId];
	...
	mib->mib_BAReorder_holdtime = &wlanmib[shadow]->mib_BAReorder_holdtime[phyMacId][vMacId];
#ifdef AP_TWT
...
#endif
	mib->mib_mumimo_mgmt = &wlanmib[shadow]->mib_mumimo_mgmt[phyMacId][vMacId];

	/* AIRTIME_FAIRNESS */
	mib->mib_atf_info = &wlanmib[shadow]->mib_atf_info;
	...
	*(mib->mib_Channel_Transition_Delay_Factor) = 5;
	...
	mib_QAPEDCATable[2].QAPEDCATblCWmin = (BE_CWMIN >= 2) ? ((BE_CWMIN + 1) >> 1) - 1 : 0;
	...
	mib_QAPEDCATable[3].QAPEDCATblCWmin = (BE_CWMIN >= 4) ? ((BE_CWMIN + 1) >> 2) - 1 : 0;
	mib_QAPEDCATable[3].QAPEDCATblCWmax = (BE_CWMIN >= 2) ? ((BE_CWMIN + 1) >> 1) - 1 : 1;
	...
	mib_QStaEDCATable[2].QStaEDCATblCWmin = (BE_CWMIN >= 2) ? ((BE_CWMIN + 1) >> 1) - 1 : 0;
	...
	mib_QStaEDCATable[3].QStaEDCATblCWmin = (BE_CWMIN >= 4) ? ((BE_CWMIN + 1) >> 2) - 1 : 0;
	mib_QStaEDCATable[3].QStaEDCATblCWmax = (BE_CWMIN >= 2) ? ((BE_CWMIN + 1) >> 1) - 1 : 1;
	...
	*(mib->mib_CACTimeOut) = 60;	// 60 seconds
	mib->mib_ETSICACTimeOut = &wlanmib[shadow]->mib_ETSICACTimeOut[phyMacId];
...
#ifdef AP_STEERING_SUPPORT
	mib->mib_btm_enabled = &wlanmib[shadow]->mib_btm_enabled[phyMacId][vMacId];
	*(mib->mib_btm_enabled) = 1;	//enable
#endif				/* AP_STEERING_SUPPORT */
#ifdef BAND_STEERING
	mib->mib_bandsteer = &wlanmib[shadow]->mib_bandsteer[phyMacId];
	mib->mib_bandsteer_handler = &wlanmib[shadow]->mib_bandsteer_handler[phyMacId];
	mib->mib_bandsteer_mode = &wlanmib[shadow]->mib_bandsteer_mode[phyMacId];
	mib->mib_bandsteer_timer_interval = &wlanmib[shadow]->mib_bandsteer_timer_interval[phyMacId];
	mib->mib_bandsteer_rssi_threshold = &wlanmib[shadow]->mib_bandsteer_rssi_threshold[phyMacId];
	mib->mib_bandsteer_sta_track_max_num = &wlanmib[shadow]->mib_bandsteer_sta_track_max_num[phyMacId];
	mib->mib_bandsteer_sta_track_max_age = &wlanmib[shadow]->mib_bandsteer_sta_track_max_age[phyMacId];
	mib->mib_bandsteer_sta_auth_retry_cnt = &wlanmib[shadow]->mib_bandsteer_sta_auth_retry_cnt[phyMacId];
...
#endif				/* BAND_STEERING */
#ifdef MULTI_AP_SUPPORT
	mib->mib_unassocsta_track_enabled = &wlanmib[shadow]->mib_unassocsta_track_enabled[phyMacId];
	mib->mib_unassocsta_track_max_num = &wlanmib[shadow]->mib_unassocsta_track_max_num[phyMacId];
	mib->mib_unassocsta_track_max_age = &wlanmib[shadow]->mib_unassocsta_track_max_age[phyMacId];
	mib->mib_unassocsta_track_time = &wlanmib[shadow]->mib_unassocsta_track_max_age[phyMacId];
...
#endif
	mib->mib_reset_rate_mode = &wlanmib[shadow]->mib_reset_rate_mode[phyMacId][vMacId];
	*(mib->mib_reset_rate_mode) = 0;	//default: RSSI

	mib->mib_eap_rate_fixed = &wlanmib[shadow]->mib_eap_rate_fixed[phyMacId][vMacId];
	*(mib->mib_eap_rate_fixed) = 0;

	mib->mib_beamChange_disable = &wlanmib[shadow]->mib_beamChange_disable[phyMacId];
	*(mib->mib_beamChange_disable) = 1;

	mib->HEConfig = &wlanmib[shadow]->HEConfig[phyMacId];
	mib->HEConfig->pe_type = MIB_PE_TYPE_AGGRESSIVE;

	mib->mib_conf_capab = &wlanmib[shadow]->mib_conf_capab[phyMacId][vMacId];
	memset(mib->mib_conf_capab, 0, sizeof(mib_conf_capability));	// initial value

	/* AIRTIME_FAIRNESS */
	(mib->mib_atf_info)->enable = 0;	/* default: disable ATF */
	/* end of AIRTIME_FAIRNESS */
	...
	mib->UL_mimo_enable = FALSE;

	mib->mib_BSSTMRequest = &wlanmib[shadow]->mib_BSSTMRequest[phyMacId][vMacId];
	mib->mib_BSSTMRequest->Abridged = 1;
	mib->mib_BSSTMRequest->DisassocImm = 1;
	mib->mib_BSSTMRequest->BSSTermiInc = 0;
	mib->mib_BSSTMRequest->ESSDisassocImm = 0;
	mib->mib_BSSTMRequest->disassoc_timer = 0;
	mib->mib_BSSTMRequest->validity_interval = 100;
	return (TRUE);
}
