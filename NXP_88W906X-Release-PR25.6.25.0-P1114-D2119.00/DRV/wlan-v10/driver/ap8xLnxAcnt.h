...
#define ACNT_MAX_STR_LEN (sizeof(acnt_tx_t) * 3)
...
typedef struct {		// Accounting Record For Tx (after Tx)
	...
	u_int8_t pad;		// alignment for generic, but specific can reuse
	u_int32_t TSF;		// Timestamp for Entry (when len>1)
	//TODO: AC_NUM gets changed from 5 to 6 when SCFG_MCQ_SUPPORT flag is enabled in firmware.
	u_int32_t stat[6 + 3];	// Direct mapping to internal_stat[]. CAUTION: AC_NUM=6. We need to update the array size when AC_NUM is changed!!
} acnt_tx3_t;
#endif				/* SOC_W906X */
