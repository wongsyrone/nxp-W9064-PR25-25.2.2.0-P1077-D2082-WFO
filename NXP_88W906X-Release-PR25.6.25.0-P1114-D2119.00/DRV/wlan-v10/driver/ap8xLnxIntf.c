...
MODULE_PARM_DESC(dbg_max_tx_pend_cnt_per_sta, "Max num of Tx pending pkt per STA");

unsigned int dbg_max_tx_pend_cnt_per_q = 4096;
module_param(dbg_max_tx_pend_cnt_per_q, uint, 0644);
MODULE_PARM_DESC(dbg_max_tx_pend_cnt_per_q, "Max num of Tx pending pkt per Queue");

unsigned int dbg_max_tx_pend_cnt_per_mgmt_q = 128;
module_param(dbg_max_tx_pend_cnt_per_mgmt_q, uint, 0644);
MODULE_PARM_DESC(dbg_max_tx_pend_cnt_per_mgmt_q, "Max num of Tx pending pkt per Mgmt Queue");

unsigned int dbg_max_tx_pend_cnt_per_bcast_q = 128;
module_param(dbg_max_tx_pend_cnt_per_bcast_q, uint, 0644);
MODULE_PARM_DESC(dbg_max_tx_pend_cnt_per_bcast_q, "Max num of Tx pending pkt per BCAST Queue");
...
MODULE_PARM_DESC(dbg_max_tx_pending_lo, "Max num of Tx pending pkt when free mem is low");

unsigned int dbg_max_tx_pend_cnt_per_sta_lo = 1074;
module_param(dbg_max_tx_pend_cnt_per_sta_lo, uint, 0644);
MODULE_PARM_DESC(dbg_max_tx_pend_cnt_per_sta_lo, "Max num of Tx pending pkt per STA when free mem is low");

unsigned int dbg_max_tx_pend_cnt_per_q_lo = 1024;
module_param(dbg_max_tx_pend_cnt_per_q_lo, uint, 0644);
MODULE_PARM_DESC(dbg_max_tx_pend_cnt_per_q_lo, "Max num of Tx pending pkt per Queue when free mem is low");
...
unsigned int mcpkt_show = 0;
module_param(mcpkt_show, uint, 0644);
MODULE_PARM_DESC(mcpkt_show, "Show multicast packets");
...
MODULE_PARM_DESC(use_localadmin_addr, "Use locally administered MAC address for the virtual APs");
...
MODULE_PARM_DESC(protect_tx_rate_thres, "Tx rate threshold for protection check");

int protect_rx_rate_thres = 200000;	/* 200kbps per connected sta */
module_param(protect_rx_rate_thres, int, 0644);
MODULE_PARM_DESC(protect_rx_rate_thres, "Rx rate threshold per connected sta for protection check");

int protect_rx_rate_thres_max = 1600000;	/* 1600kbps */
module_param(protect_rx_rate_thres_max, int, 0644);
MODULE_PARM_DESC(protect_rx_rate_thres_max, "Max Rx rate threshold for protection check");

int txq_per_sta_timeout = 0;	//0: disable >0: timeout (us)
module_param(txq_per_sta_timeout, int, 0644);
MODULE_PARM_DESC(txq_per_sta_timeout, "WFA 11AX Plugfest: for tx per sta fairly. 0: disable xmit fairly. >0: timeout");
...
MODULE_PARM_DESC(chld_nf_delta, "channel load and noise floor delta for acs_mode is 0");

unsigned int nf_score_delta = 20;	// channel load and nf delta
module_param(nf_score_delta, uint, 0644);
MODULE_PARM_DESC(nf_score_delta, "channel score delta for acs_mode is 1 (NF reading mode)");
...
int nf_weight = 205;		// nf floor weight
module_param(nf_weight, int, 0644);
MODULE_PARM_DESC(nf_weight, "noise floor weight");
...
int rdnf_loop = 50;
module_param(rdnf_loop, int, 0644);
MODULE_PARM_DESC(rdnf_loop, "read nf loop");
...
extern void dbg_replay_attack_release(void);

extern void wlOffChannelReset(struct net_device *netdev);
...
#else
static int __devinit wlprobe_pci(struct pci_dev *, const struct pci_device_id *);
...
#ifdef WL_DEBUG
static const char *wlgetAdapterDescription(struct wlprivate *wlpptr, u_int32_t, u_int32_t);
#endif
...
static struct device *get_host_bridge_device(struct pci_dev *dev)
{
...
static int wl_pcie_reset(struct pci_dev *pdev)
{
...
static int wdev_mmap_fault(struct vm_fault *vmf)
{
	return 0;
}
#else
 ...
 */
static int wdev_mmap_fault(struct vm_area_struct *vma, struct vm_fault *vmf)
{
	struct page *page;
	mmap_info *info;
...
static void AllocSharedMem(struct wlprivate *wlpptr)
{
...
#ifdef SOC_W906X
		    wl_dma_alloc_coherent(wlpptr->wlpd_p->dev, FW_IO_MB_SIZE, &wlpptr->wlpd_p->AllocSharedMeminfo.dataPhysicalLoc,
					  wlpptr->wlpd_p->dma_alloc_flags);
#else
		    pci_alloc_consistent(wlpptr->pPciDev, FW_IO_MB_SIZE, &wlpptr->wlpd_p->AllocSharedMeminfo.dataPhysicalLoc);
#endif				/* SOC_W906X */

		if (wlpptr->wlpd_p->AllocSharedMeminfo.data) {
			memset(wlpptr->wlpd_p->AllocSharedMeminfo.data, 0, FW_IO_MB_SIZE);
...
#ifdef SOC_W906X
		    wl_dma_alloc_coherent(wlpptr->wlpd_p->dev, sizeof(drv_fw_shared_t), &wlpptr->wlpd_p->MrvlPriSharedMem.dataPhysicalLoc,
					  wlpptr->wlpd_p->dma_alloc_flags);
#else
		    pci_alloc_consistent(wlpptr->pPciDev, sizeof(drv_fw_shared_t), &wlpptr->wlpd_p->MrvlPriSharedMem.dataPhysicalLoc);
#endif
	}
//      printk("mrvl pri mailbox Physical loc =0x%pad \n", &wlpptr->wlpd_p->MrvlPriSharedMem.dataPhysicalLoc);  //0316
}

static int wdev_open(int cardindex, struct inode *inode, struct file *filp)
{
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(4,10,0)
static int ACNT_mmap_fault(struct vm_fault *vmf)
{
	return 0;
}
#else
static int ACNT_mmap_fault(struct vm_area_struct *vma, struct vm_fault *vmf)
{
	struct page *page;
	mmap_info *info;
...
static int __smdata_mmap_fault(struct vm_area_struct *vma, struct vm_fault *vmf)
{
	struct page *page;
	mmap_info *info;
...
static int smdata_mmap_fault(struct vm_area_struct *vma, struct vm_fault *vmf)
{
	return __smdata_mmap_fault(vma, vmf);
}
#else
static int smdata_mmap_fault(struct vm_fault *vmf)
{
...
static int smdata_open0(struct inode *inode, struct file *filp)
{
	return smdata_open(0, inode, filp);
}

static int smdata_open1(struct inode *inode, struct file *filp)
{
	return smdata_open(1, inode, filp);
}

static int smdata_open2(struct inode *inode, struct file *filp)
{
...
static void create_smdata_mmap(struct wlprivate *priv)
{
	char buf[10];
	int cardindex;
	U32 pa;
	...
	priv->wlpd_p->smdata_mmap_info.file = debugfs_create_file(buf, 0644, NULL, NULL, &smdata_fops[cardindex]);
	...
	/* mmap base on page size (4KB), calculate offset */
	priv->wlpd_p->smdata_offset = pa & 0xFFF;
...
static ssize_t write_pid(int slot, struct file *file, const char __user * buf, size_t count, loff_t * ppos)
{
	char mybuf[10];
	int pid = 0;
	struct wlprivate *wlpptr;
...
static ssize_t write_pid0(struct file *file, const char __user * buf, size_t count, loff_t * ppos)
{
	return write_pid(0, file, buf, count, ppos);
}

static ssize_t write_pid1(struct file *file, const char __user * buf, size_t count, loff_t * ppos)
{
...
static int __init wlmodule_init(void)
{
	...
	int j = 0, k = 0, l = 0;
#ifdef SOC_W906X
	unsigned short Supported_VID[MRVL_PCI_VENDOR_ID_Count] = { MRVL_PCI_VENDOR_ID2 };
#else
	unsigned short Supported_VID[MRVL_PCI_VENDOR_ID_Count] = { MRVL_PCI_VENDOR_ID1, MRVL_PCI_VENDOR_ID2 };
#endif				/* SOC_W906X */
	...
	for (l = 0; l < MRVL_PCI_VENDOR_ID_Count; l++) {
		dev = NULL;
		while ((dev = pci_get_device(Supported_VID[l], PCI_ANY_ID, dev)) != NULL) {
			if (((dev->class >> 16) & 0xff) != PCI_BASE_CLASS_NETWORK) {
				continue;
			}
			...
			while (dev1) {
				...
				k++;
				dev1 = pci_get_device(Supported_VID[l], device_id, dev);
...
#ifdef SOC_W906X
static void wl_init_txpend_cnt(void)
{
	u32 len = 0;
...
static void wlprobe_set_reg_value(struct wlprivate *wlpptr)
{
	if (IS_BUS_TYPE_MCI(wlpptr)) {
		wlpptr->wlpd_p->reg.h2a_int_events = MACREG_REG_H2A_INTERRUPT_EVENTS_MCI;
		wlpptr->wlpd_p->reg.h2a_int_cause = MACREG_REG_H2A_INTERRUPT_CAUSE_MCI;
		wlpptr->wlpd_p->reg.h2a_int_mask = MACREG_REG_H2A_INTERRUPT_MASK_MCI;
		wlpptr->wlpd_p->reg.h2a_int_clear_sel = MACREG_REG_H2A_INTERRUPT_CLEAR_SEL_MCI;
		wlpptr->wlpd_p->reg.h2a_int_status_mask = MACREG_REG_H2A_INTERRUPT_STATUS_MASK_MCI;

		wlpptr->wlpd_p->reg.a2h_int_events = MACREG_REG_A2H_INTERRUPT_EVENTS_MCI;
		wlpptr->wlpd_p->reg.a2h_int_cause = MACREG_REG_A2H_INTERRUPT_CAUSE_MCI;
		wlpptr->wlpd_p->reg.a2h_int_mask = MACREG_REG_A2H_INTERRUPT_MASK_MCI;
		wlpptr->wlpd_p->reg.a2h_int_clear_sel = MACREG_REG_A2H_INTERRUPT_CLEAR_SEL_MCI;
		wlpptr->wlpd_p->reg.a2h_int_status_mask = MACREG_REG_A2H_INTERRUPT_STATUS_MASK_MCI;
		...
		wlpptr->wlpd_p->reg.FwDbgStateAddr = MACREG_REG_FwDbgStateAddr_MCI;

		wlpptr->wlpd_p->reg.acnt_head = MACREG_REG_AcntHead_MCI;
		wlpptr->wlpd_p->reg.acnt_tail = MACREG_REG_AcntTail_MCI;

		wlpptr->wlpd_p->reg.offch_req_head = MACREG_REG_OffchReqHead_MCI;
		wlpptr->wlpd_p->reg.offch_req_tail = MACREG_REG_OffchReqTail_MCI;

		wlpptr->wlpd_p->reg.smac_buf_hi_addr = SMAC_BUF_HI_ADDR_MCI;
		wlpptr->wlpd_p->reg.smac_ctrlbase_nss_hi_val_intr = SMAC_CTRLBASE_NSS_MCI_HI_VAL_INTR;

		wlpptr->wlpd_p->reg.fw_int_event_offeset = MACREG_REG_A2H_INTERRUPT_EVENTS_MCI;
		wlpptr->wlpd_p->reg.fw_len_offset = MACREG_REG_INT_CODE_MCI;
		wlpptr->wlpd_p->reg.fw_int_cause_offset = MACREG_REG_A2H_INTERRUPT_CAUSE_MCI;
		wlpptr->wlpd_p->reg.fw_setup_int_trigger = MACREG_H2ARIC_BIT_DOOR_BELL;

	} else {
		wlpptr->wlpd_p->reg.h2a_int_events = MACREG_REG_H2A_INTERRUPT_EVENTS;
		wlpptr->wlpd_p->reg.h2a_int_cause = MACREG_REG_H2A_INTERRUPT_CAUSE;
		wlpptr->wlpd_p->reg.h2a_int_mask = MACREG_REG_H2A_INTERRUPT_MASK;
		wlpptr->wlpd_p->reg.h2a_int_clear_sel = MACREG_REG_H2A_INTERRUPT_CLEAR_SEL;
		wlpptr->wlpd_p->reg.h2a_int_status_mask = MACREG_REG_H2A_INTERRUPT_STATUS_MASK;

		wlpptr->wlpd_p->reg.a2h_int_events = MACREG_REG_A2H_INTERRUPT_EVENTS;
		wlpptr->wlpd_p->reg.a2h_int_cause = MACREG_REG_A2H_INTERRUPT_CAUSE;
		wlpptr->wlpd_p->reg.a2h_int_mask = MACREG_REG_A2H_INTERRUPT_MASK;
		wlpptr->wlpd_p->reg.a2h_int_clear_sel = MACREG_REG_A2H_INTERRUPT_CLEAR_SEL;
		wlpptr->wlpd_p->reg.a2h_int_status_mask = MACREG_REG_A2H_INTERRUPT_STATUS_MASK;
		...
		wlpptr->wlpd_p->reg.smac_ctrlbase_nss_hi_val_intr = SMAC_CTRLBASE_NSS_PCIE_HI_VAL_INTR;

		wlpptr->wlpd_p->reg.fw_int_event_offeset = MACREG_REG_H2A_INTERRUPT_EVENTS;
		wlpptr->wlpd_p->reg.fw_len_offset = PCI_REG_SCRATCH2_REG;
		wlpptr->wlpd_p->reg.fw_int_cause_offset = MACREG_REG_H2A_INTERRUPT_CAUSE;
		wlpptr->wlpd_p->reg.fw_setup_int_trigger = MACREG_A2HRIC_BIT_MASK;
...
static void wlprobe_set_intr_info(struct wlprivate *wlpptr)
{
	if (IS_BUS_TYPE_MCI(wlpptr)) {
		...
		if (IS_PLATFORM(MAX))	// For generic platform
			wlpptr->wlpd_p->msix_num = intr_info_tbl_generic.msix_num;
		else
			wlpptr->wlpd_p->msix_num = intr_info_tbl[platform_id].msix_num;
	}
...
const static struct intr_frame sysintr_frames[PLATFORM_ID_MAX][INTR_FRAME_MAX_NUM] = {
	{			/* PCI: A3900_A7K */
 ...
 */
static void wl_init_intrfrm(U32 intr_base, struct intr_frame *pintr_frm)
{
	...
	if (i == INTR_FRAME_MAX_NUM) {
		WLDBG_ERROR(DBG_LEVEL_0, "Failed to find Interrupt Frame (%x)\n", intr_base);
		memcpy(pintr_frm, &sysintr_frames[idx_sf][0], sizeof(struct intr_frame));
	}
	return;
}

static void wl_get_intr_base(struct wlprivate *wlpptr)
{
	...
	if (IS_BUS_TYPE_MCI(wlpptr)) {
		if (of_property_read_u32(pdev->dev.of_node, "device-identifier", &id)) {
			WLDBG_ERROR(DBG_LEVEL_0, "Failed to get \"device-identifier\"\n");
		}

		if (of_property_read_u32(pdev->dev.of_node, "msi-base", &intr_base)) {
			WLDBG_ERROR(DBG_LEVEL_0, "Failed to get \"msi-base\"\n");
...
static void wl_init_intr(struct wlprivate *wlpptr)
{
	U32 regval;
	...
	if (!IS_BUS_TYPE_MCI(wlpptr)) {
		if (wlpptr->intr_type == INTR_TYPE_MSIX) {
			...
			//0x38
			printk("%s(%d)\n", __func__, __LINE__);
			writel(SC5_PCIE_MODE_GIC, wlpptr->ioBase1 + SC5_REG_PCIE_INTR_MODE_SEL);
			//0x30
			writel(SMAC_CTRLBASE_NSS_PCIE_HI_VAL_NOINTR, wlpptr->ioBase1 + SC5_REG_SMAC_CTRLBASE_NSS_PCIE_HI);
			writel(PCI_REG_DOORBELL_ADDR, wlpptr->ioBase1 + SC5_REG_FRAME_0);
			writel(PCI_REG_DOORBELL_ADDR, wlpptr->ioBase1 + SC5_REG_FRAME_1);
			writel(PCI_REG_DOORBELL_ADDR, wlpptr->ioBase1 + SC5_REG_FRAME_2);
			writel(PCI_REG_DOORBELL_ADDR, wlpptr->ioBase1 + SC5_REG_FRAME_3);
			...
			//0x30
			writel(SMAC_CTRLBASE_NSS_PCIE_HI_VAL_NOINTR, wlpptr->ioBase1 + SC5_REG_SMAC_CTRLBASE_NSS_PCIE_HI);
			writel(PCI_REG_HOST_ITR_ADDR, wlpptr->ioBase1 + SC5_REG_FRAME_0);
			writel(PCI_REG_HOST_ITR_ADDR, wlpptr->ioBase1 + SC5_REG_FRAME_1);
			writel(PCI_REG_HOST_ITR_ADDR, wlpptr->ioBase1 + SC5_REG_FRAME_2);
			writel(PCI_REG_HOST_ITR_ADDR, wlpptr->ioBase1 + SC5_REG_FRAME_3);
		}
		return;
	} else {
		...
		printk("(base, spi)=(%xh, %xh)\n", wlpptr->wlpd_p->sysintr_frm.frm_base, wlpptr->wlpd_p->sysintr_frm.spi_num);
		// ++++++++
		// Fill base address of frame_x
		writel(wlpptr->wlpd_p->sysintr_frm.frm_base, wlpptr->ioBase1 + SC5_REG_FRAME_0);
		WLDBG_INFO(DBG_LEVEL_0, "=> %s(), W906x_REG_FRAME_0, w_reg(ioBase1+%xh) = %xh\n", __func__,
			   SC5_REG_FRAME_0, wlpptr->wlpd_p->sysintr_frm.frm_base);
		writel(wlpptr->wlpd_p->sysintr_frm.frm_base, wlpptr->ioBase1 + SC5_REG_FRAME_1);
		WLDBG_INFO(DBG_LEVEL_0, "=> %s(), W906x_REG_FRAME_1, w_reg(ioBase1+%xh) = %xh\n", __func__,
			   SC5_REG_FRAME_1, wlpptr->wlpd_p->sysintr_frm.frm_base);
		writel(wlpptr->wlpd_p->sysintr_frm.frm_base, wlpptr->ioBase1 + SC5_REG_FRAME_2);
		WLDBG_INFO(DBG_LEVEL_0, "=> %s(), W906x_REG_FRAME_2, w_reg(ioBase1+%xh) = %xh\n", __func__,
			   SC5_REG_FRAME_2, wlpptr->wlpd_p->sysintr_frm.frm_base);
		writel(wlpptr->wlpd_p->sysintr_frm.frm_base, wlpptr->ioBase1 + SC5_REG_FRAME_3);
		WLDBG_INFO(DBG_LEVEL_0, "=> %s(), W906x_REG_FRAME_3, w_reg(ioBase1+%xh) = %xh\n", __func__,
			   SC5_REG_FRAME_3, wlpptr->wlpd_p->sysintr_frm.frm_base);
		// --------

		// HW PCIE-MSIX setting
		// => #1, set intr mode selected 
		writel(SC5_PCIE_MODE_GIC, wlpptr->ioBase1 + SC5_REG_PCIE_INTR_MODE_SEL);
		printk("=> %s(),w_reg(ioBase1+%xh) = %xh\n", __func__, SC5_REG_PCIE_INTR_MODE_SEL, SC5_PCIE_MODE_GIC);
		// set hframe register base
		writel(wlpptr->hframe_phy_addr, wlpptr->ioBase1 + SC5_REG_HFRAME_BASE);
		printk("=> %s(),w_reg(ioBase1+%xh) = %llxh\n", __func__, SC5_REG_HFRAME_BASE, (long long unsigned int)wlpptr->hframe_phy_addr);
		// => #2, set ncc pcie val (Using interrupt mode or not)
		regval = wlpptr->wlpd_p->reg.smac_ctrlbase_nss_hi_val_intr;

		writel(regval, wlpptr->ioBase1 + SC5_REG_SMAC_CTRLBASE_NSS_PCIE_HI);
		printk("=> %s(),w_reg(ioBase1+%xh) = %xh\n", __func__, SC5_REG_SMAC_CTRLBASE_NSS_PCIE_HI, regval);

		// => #3, set pcie msi address 
		writel(wlpptr->wlpd_p->sysintr_frm.frm_base, wlpptr->ioBase1 + SC5_REG_PCIE_MSI_ADDR);
		printk("=> %s(),w_reg(ioBase1+%xh) = %xh\n", __func__, SC5_REG_PCIE_MSI_ADDR, wlpptr->wlpd_p->sysintr_frm.frm_base);

		// => #4, Set SC5_REG_PCIE_MSIX_DATA 
		writel(wlpptr->wlpd_p->sysintr_frm.spi_num, wlpptr->ioBase1 + SC5_REG_PCIE_MSIX_DATA);
		printk("=> %s(),w_reg(ioBase1+%xh) = %xh\n", __func__, SC5_REG_PCIE_MSIX_DATA, wlpptr->wlpd_p->sysintr_frm.spi_num);

		writel(wlpptr->hframe_phy_addr, wlpptr->ioBase1 + SC5_REG_BASE_ADDR_HOST_128B);
		printk("===> w_reg(ioBase1+%xh) = %llxh\n", SC5_REG_BASE_ADDR_HOST_128B, (long long unsigned int)wlpptr->hframe_phy_addr);
		return;
	}
}

static void wl_hook_intr(struct wlprivate *wlpptr)
{
	...
	if (wlpptr->intr_type == INTR_TYPE_MSIX) {
		...
		for (idx = 0; idx < wlpptr->wlpd_p->msix_num; idx++) {
			...
			writel(BIT(idx), wlpptr->ioBase1 + SC5_REG_EFF_ID(qid, ((int)(issq) & 0x1)));
		}

		if (request_irq(wlpptr->netDev->irq, wlISR, IRQF_SHARED, wlpptr->netDev->name, (wlpptr->netDev))) {
			WLDBG_ERROR(DBG_LEVEL_2, "%s: request_irq failed failed\n", wlpptr->netDev->name);
		}

	} else {
		WLDBG_ERROR(DBG_LEVEL_2, "%s: INT not remap and request_irq. intr_type %d \n", wlpptr->netDev->name, wlpptr->intr_type);
	}
...
*/
static void wl_get_chipinfo(struct wlprivate *wlpptr)
{
	UINT32 val = cpu_to_le32(*(volatile unsigned int *)(wlpptr->ioBase1 + SC5_REG_SMAC_CTRLBASE));
	UINT8 chipid, rev;
...
static int wlwlan_platform_data_get(struct platform_device *pdev, struct wlprivate *wlpptr)
{
	...
	wlpptr->ioBaseExt = devm_ioremap(&pdev->dev, mciportbase[wlpptr->cardindex], 0x10);
	printk("%s(), carindex:%u: ioBaseExt: (v,p)=(%p, %x)\n", __func__, wlpptr->cardindex, wlpptr->ioBaseExt, mciportbase[wlpptr->cardindex]);
...
/* just back port it from v4.5 and should be remove it after upgrading
   major verion
 */
static int platform_irq_count(struct platform_device *dev)
{
	int ret, nr = 0;
...
static int wlprobe_mci(struct platform_device *pdev)
{
...
#ifdef CONFIG_MARVELL_MOCHI_DRIVER
	{
		...
		do {
			...
			if (mci_ret == MCI_UNSUPPORTED_SPEED)
				mci_speed--;
		} while ((rcnt++ < MAX_BUS_RESET) && (mci_speed >= MCI_LINK_SPEED_1G));
	...
	for (i = 0; i < MAX_CARDS_SUPPORT; i++) {
		if (global_private_data[i] == NULL) {
			...
			if (!(global_private_data[i] = wl_vzalloc(sizeof(struct wlprivate_data)))) {
				dev_err(&pdev->dev, "Unable to allocate buffer for global_private_data(size %zu)\n", sizeof(struct wlprivate_data));
				return -ENOMEM;
			}
			memcpy(global_private_data[i]->RateGrpDefault, RateGrpDefault, sizeof(RateGrp_t) * MAX_GROUP_PER_CHANNEL);
			global_private_data[i]->AmpduPckReorder = (Ampdu_Pck_Reorder_t *) wl_vzalloc(sizeof(Ampdu_Pck_Reorder_t) * (sta_num + 1));
			...
			global_private_data[i]->Ampdu_tx = (Ampdu_tx_t *) wl_vzalloc(sizeof(Ampdu_tx_t) * (MAX_SUPPORT_AMPDU_TX_STREAM_RUNNING));
	...
	dev = alloc_netdev(sizeof(struct wlprivate), DRV_NAME, NET_NAME_UNKNOWN, ether_setup);
#else
	...
	cardindex++;
	if (wlpdptr == NULL) {
		dev_err(&pdev->dev, "%s: no mem for private driver data context\n", DRV_NAME);
		goto err;
	}
	...
	if ((wlpptr->hwData.chipRevision == REV_Z1) || (wlpptr->hwData.chipRevision == REV_Z2)) {
	...
	wlpptr->smacStatusAddr = &((SMAC_CTRL_BLK_st *) wlpptr->ioBase0)->status;
...
#define WIFIARB_POST_REQUEST_INTR_DEV_1 "wifiarb_post_request_dev1"
	...
	case 0:
		{
			wlpptr->wlpd_p->AllocSharedMeminfo.file = debugfs_create_file(wlpptr->netDev->name, 0644, NULL, NULL, &wdev0_fops);
			wlpptr->wlpd_p->PostReqSiginfo.file = debugfs_create_file(WIFIARB_POST_REQUEST_INTR_DEV_0, 0200, NULL, NULL, &postreq_fops0);
			ACNT_f0 = debugfs_create_file("AcntChunk0", 0644, NULL, NULL, &ACNT_fops);
			break;
		}
	case 1:
		{
			wlpptr->wlpd_p->AllocSharedMeminfo.file = debugfs_create_file(wlpptr->netDev->name, 0644, NULL, NULL, &wdev1_fops);
			wlpptr->wlpd_p->PostReqSiginfo.file = debugfs_create_file(WIFIARB_POST_REQUEST_INTR_DEV_1, 0200, NULL, NULL, &postreq_fops1);
			ACNT_f1 = debugfs_create_file("AcntChunk1", 0644, NULL, NULL, &ACNT1_fops);
			break;
		}
	default:
		printk(" wlinitcnt = %d \n", wlinitcnt);
	...
	wlpptr->hframe_virt_addr = wl_dma_alloc_coherent(&pdev->dev, SC5_HFRAME_MEM_SIZE, &wlpptr->hframe_phy_addr, GFP_KERNEL);

	if (wlpptr->hframe_virt_addr == NULL) {
		printk(KERN_ERR "%s: Can not allocate memory for hframe register", wlpptr->netDev->name);
		goto err;
	}
...
*/
static void intr_sel_frm_effid(struct wlprivate *wlpptr, UINT32 intr_id, SINT32 qid, BOOLEAN issq)
{
	if (qid < 0) {
		return;
	}
	...
	// Set the interrupt
	writel(intr_id + wlpptr->wlpd_p->sysintr_frm.spi_num, wlpptr->ioBase1 + SC5_REG_EFF_ID(qid, ((int)(issq) & 0x1)));
	WLDBG_INFO(DBG_LEVEL_0, "[EFF_ID] writel(ioBase1+%xh) = %xh\n",
		   SC5_REG_EFF_ID(qid, ((int)(issq) & 0x1)), intr_id + wlpptr->wlpd_p->sysintr_frm.spi_num);
	return;
}

static void wldisable_intr(struct net_device *netdev)
{
...
static void wlfree_intr(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
...
static BOOLEAN hook_intr_2_queue(struct wlprivate *wlpptr, UINT32 isr_id, SINT32 qid, BOOLEAN issq)
{
	...
	if (IS_BUS_TYPE_MCI(wlpptr))
		irq_vec = wlpptr->msix_ctx[isr_id].irq_vec = platform_get_irq(wlpptr->wlpd_p->pDev, isr_id);
	else
		irq_vec = wlpptr->msix_ctx[isr_id].irq_vec = wlpptr->msix_entries[isr_id].vector;

	card_idx = wlpptr->cardindex;
	sprintf(irq_name[card_idx][isr_id], "%s_%sQ[%d]", wlpptr->netDev->name, (issq ? "S" : "R"), qid);

	// ======== TXQ ========
	if ((((pbqm_args->txq_start_index <= qid) && (qid < (pbqm_args->txq_start_index + pbqm_args->txq_num))) ||
	     ((SC5_BMQ_START_INDEX <= qid) && (qid < (SC5_BMQ_START_INDEX + SC5_BMQ_NUM)))) && (issq == FALSE)) {
		if (request_irq(irq_vec, wlSC5MSIX_tx, IRQF_SHARED, irq_name[card_idx][isr_id], &(wlpptr->msix_ctx[isr_id]))) {	//=> Give any to test....
			WLDBG_ERROR(DBG_LEVEL_2, "%s: request_irq for Q(%d) failed\n", wlpptr->netDev->name, qid);
			wlpptr->num_vectors = isr_id;
			return FALSE;
		} else {
			WLDBG_INFO(DBG_LEVEL_2, "Intr[%d], %sQ[%d] => wlSC5MSIX_tx()\n", isr_id, (issq ? "S" : "R"), qid);
		}
	} else
		// ======== RXQ ========
	if (((SC5_RXQ_START_INDEX == qid) || (SC5_RXQ_PROMISCUOUS_INDEX == qid) || (SC5_RXQ_MGMT_INDEX == qid)) && (issq == TRUE)) {
		if (request_irq(irq_vec, wlSC5MSIX_rx, IRQF_SHARED, irq_name[card_idx][isr_id], &(wlpptr->msix_ctx[isr_id]))) {	//=> Give any to test....
			WLDBG_ERROR(DBG_LEVEL_2, "%s: request_irq for Q(%d) failed\n", wlpptr->netDev->name, qid);
			wlpptr->num_vectors = isr_id;
			return FALSE;
		} else {
			WLDBG_INFO(DBG_LEVEL_2, "Intr[%d], %sQ[%d] => wlSC5MSIX_rx()\n", isr_id, (issq ? "S" : "R"), qid);
		}
	} else
		// ======== RelQ ========
	if (((pbqm_args->bmq_release_index <= qid) && (qid < (pbqm_args->bmq_release_index + pbqm_args->bmq_release_num))) && (issq == TRUE)) {
		if (request_irq(irq_vec, wlSC5MSIX_rel, IRQF_SHARED, irq_name[card_idx][isr_id], &(wlpptr->msix_ctx[isr_id]))) {	//=> Give any to test....
			WLDBG_ERROR(DBG_LEVEL_2, "%s: request_irq for Q(%d) failed\n", wlpptr->netDev->name, qid);
			wlpptr->num_vectors = isr_id;
			return FALSE;
		} else {
			WLDBG_INFO(DBG_LEVEL_2, "Intr[%d], %sQ[%d] => wlSC5MSIX_rel()\n", isr_id, (issq ? "S" : "R"), qid);
		}
	} else
#if defined(ACNT_REC)
		// ======== SMAC Accounting Record ========
	if (((pbqm_args->racntq_index <= qid) && (qid < (pbqm_args->racntq_index + pbqm_args->racntq_num))) && (issq == TRUE)) {
#if 0
		if (request_irq(irq_vec, wlSC5MSIX_RxInfo, IRQF_SHARED, irq_name[card_idx][isr_id], &(wlpptr->msix_ctx[isr_id]))) {	//=> Give any to test....
			...
			WLDBG_INFO(DBG_LEVEL_2, "Intr[%d], %sQ[%d] => wlSC5MSIX_RxInfo()\n", isr_id, (issq ? "S" : "R"), qid);
		}
#else
		/*
	...
	if (pbqm_args->rxacnt_intrid == (isr_id - wlpptr->wlpd_p->intr_shift)) {
		if (request_irq(irq_vec, wlSC5MSIX_RAcntRec, IRQF_SHARED, irq_name[card_idx][isr_id], &(wlpptr->msix_ctx[isr_id]))) {	//=> Give any to test....
			...
			WLDBG_INFO(DBG_LEVEL_2, "Intr[%d], %sQ[%d] => wlSC5MSIX_RAcntRec()\n", isr_id, (issq ? "S" : "R"), qid);
		}
	} else
#endif				//RXACNT_REC
		...
		// ======== R7 ========
	{
		if (request_irq(irq_vec, wlSC5MSIX_r7, IRQF_SHARED, irq_name[card_idx][isr_id], &(wlpptr->msix_ctx[isr_id]))) {	//=> Give any to test....
			...
			WLDBG_INFO(DBG_LEVEL_2, "Intr[%d], %sQ[%d] => wlSC5MSIX_r7()\n", isr_id, (issq ? "S" : "R"), qid);
		}
	}
	return TRUE;
}
...
*/
static void remap_intr(struct wlprivate *wlpptr, UINT32 isr_id, SINT32 * pqid, BOOLEAN * pissq)
{
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct bqm_args *pbqm_args = &wlpd_p->bmq_args;
...
static BOOLEAN hook_intr(struct wlprivate *wlpptr, UINT32 isr_id)
{
	...
	intr_sel_frm_effid(wlpptr, isr_id - wlpptr->wlpd_p->intr_shift, qid, issq);
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 8, 0)
static int wlprobe_pci(struct pci_dev *pdev, const struct pci_device_id *id)
#else
static int __devinit wlprobe_pci(struct pci_dev *pdev, const struct pci_device_id *id)
#endif
{
	struct wlprivate *wlpptr = NULL;
	struct wlprivate_data *wlpdptr = NULL;
	...
	wlpptr->wlpd_p->dma_alloc_flags = GFP_ATOMIC;
	{
		...
		pci_read_config_dword(wlpptr->wlpd_p->pPciDev, PCI_BASE_ADDRESS_0, (u32 *) & wlpptr->wlpd_p->baseaddress0);
		pci_read_config_dword(wlpptr->wlpd_p->pPciDev, PCI_BASE_ADDRESS_2, (u32 *) & wlpptr->wlpd_p->baseaddress2);
#ifdef SOC_W906X
		pci_read_config_dword(wlpptr->wlpd_p->pPciDev, PCI_BASE_ADDRESS_4, (u32 *) & wlpptr->wlpd_p->baseaddress4);
#endif				/* SOC_W906X */
...
#define WIFIARB_POST_REQUEST_INTR_DEV_1 "wifiarb_post_request_dev1"
	...
	case 0:
		{
			...
			ACNT_f0 = debugfs_create_file("AcntChunk0", 0644, NULL, NULL, &ACNT_fops);
#else
			wlpptr->wlpd_p->ACNTmemInfo.file = debugfs_create_file("ACNT0mem", 0644, NULL, NULL, &ACNT_fops);
#endif
	...
	case 1:
		{
			...
			ACNT_f1 = debugfs_create_file("AcntChunk1", 0644, NULL, NULL, &ACNT1_fops);
#else
			wlpptr->wlpd_p->ACNTmemInfo.file = debugfs_create_file("ACNT1mem", 0644, NULL, NULL, &ACNT_fops);
#endif
			break;
		}
	default:
...
#ifdef SOC_W906X
	wlpptr->hframe_virt_addr = pci_alloc_consistent(pdev, SC5_HFRAME_MEM_SIZE, &wlpptr->hframe_phy_addr);
	if (wlpptr->hframe_virt_addr == NULL) {
		printk(KERN_ERR "%s: Can not allocate memory for hframe register", wlpptr->netDev->name);
	...
	if (pci_find_capability(pdev, PCI_CAP_ID_MSIX)) {
		...
		if (wlpptr->num_vectors)
			wlpptr->msix_entries = kcalloc(wlpptr->num_vectors, sizeof(struct msix_entry), GFP_KERNEL);
		...
		if (wlpptr->msix_entries) {
			...
			if ((msi_err = ap8x_enable_msix(pdev, wlpptr->msix_entries, wlpptr->num_vectors)) < 0) {
				dev_info(&pdev->dev, "MSI-X Allocation Failed with error = %d. Fall back to try MSI\n", msi_err);
			...
			dev_info(&pdev->dev, "MSI-X enabled with %d vectors(starts from %d) allocated\n",
				 wlpptr->num_vectors, wlpptr->msix_entries[0].vector);
		}
...
#ifdef SOC_W906X
	if (wlpptr->hframe_virt_addr != NULL)
		pci_free_consistent(pdev, SC5_HFRAME_MEM_SIZE, wlpptr->hframe_virt_addr, wlpptr->hframe_phy_addr);
	if (wlpptr->msix_entries) {
		int i = 0;
		for (i = 0; i < wlpptr->num_vectors; i++) {
			free_irq(wlpptr->msix_entries[i].vector, &(wlpptr->msix_ctx[i]));
...
static int wlremove_mci(struct platform_device *pdev)
{
	...
	if (wlpptr->hframe_virt_addr) {
		wl_dma_free_coherent(&(pdev->dev), SC5_HFRAME_MEM_SIZE, wlpptr->hframe_virt_addr, wlpptr->hframe_phy_addr);
	}
	...
	if (wlpptr->wlpd_p->AllocSharedMeminfo.data) {
		wl_dma_free_coherent(wlpptr->wlpd_p->dev, FW_IO_MB_SIZE, wlpptr->wlpd_p->AllocSharedMeminfo.data,
				     wlpptr->wlpd_p->AllocSharedMeminfo.dataPhysicalLoc);
	}
	if (wlpptr->wlpd_p->MrvlPriSharedMem.data) {
		wl_dma_free_coherent(wlpptr->wlpd_p->dev, sizeof(drv_fw_shared_t), wlpptr->wlpd_p->MrvlPriSharedMem.data,
...
#endif				/* SOC_W906X */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 8, 0)
static void wlremove_pci(struct pci_dev *pdev)
#else
static void __devexit wlremove_pci(struct pci_dev *pdev)
#endif
{
...
static int wlsuspend_pci(struct pci_dev *pdev, pm_message_t state)
{
	WLDBG_INFO(DBG_LEVEL_2, "%s: suspended device\n", DRV_NAME);
	return 0;
}
...
static int wlresume_pci(struct pci_dev *pdev)
{
	WLDBG_INFO(DBG_LEVEL_2, "%s: resumed device\n", DRV_NAME);
	return 0;
}
...
#ifdef WL_DEBUG
static const char *wlgetAdapterDescription(struct wlprivate *wlpptr, u_int32_t vendorid, u_int32_t devid)
{
#ifdef SOC_W906X
	if (IS_BUS_TYPE_MCI(wlpptr)) {
...
#endif				/* SOC_W906X */
	{
		...
		while (numEntry) {
			numEntry--;
			if ((wlid_tbl[numEntry].vendor == vendorid) && (wlid_tbl[numEntry].device == devid)) {
...
static void timer_routine(unsigned long arg)
{
...
#ifdef SOC_W906X
		if ((wlpptr->vdev[wlpptr->wlpd_p->MUcurVapidx] != NULL) && ((wlpptr->vdev[wlpptr->wlpd_p->MUcurVapidx]->flags & IFF_RUNNING))) {
#else
		if ((wlpptr->vdev[wlpptr->wlpd_p->MUcurVapidx]->flags & IFF_RUNNING)) {
#endif

#ifdef USE_TASKLET
			wlpptr_vap_p = NETDEV_PRIV_P(struct wlprivate, wlpptr->vdev[wlpptr->wlpd_p->MUcurVapidx]);
	...
	if (wlpd_p->idx_test_arg.pkt_cnt > 0) {
		long pktcnt = (wlpd_p->idx_test_arg.pkt_cnt < 128) ? wlpd_p->idx_test_arg.pkt_cnt : 128;
		wlTxSkbTest_1(netdev, pktcnt, wlpd_p->idx_test_arg.pkt_size, wlpd_p->idx_test_arg.qid, wlpd_p->idx_test_arg.frameType);
...
#endif				/* SOC_W906X */
	{
		...
		if (passcnt < 100) {
			...
			uint8 i;
			extern const u_int32_t buf_pool_max_entries[SC5_BMQ_NUM];
			...
			for (i = SC5_BMQ_START_INDEX; i < SC5_BMQ_START_INDEX + SC5_BMQ_NUM; i++) {
				...
				rpkt_reuse_free_resource(&wlqm->rq.skbTrace,
							 &last_qlen[i - SC5_BMQ_START_INDEX], buf_pool_max_entries[i - SC5_BMQ_START_INDEX]);
			}
			passcnt = 0;
		}
	}
...
#ifdef IEEE80211K
	offchan_ms = ktime_ms_delta(ktime_get_real(), wlpd_p->offchan_ts);
	if (offchan_ms < 0) {
		wlpd_p->offchan_ts = ktime_get_real();
	} else if (offchan_ms >= 1000) {
		wlpd_p->offchan_ts = ktime_get_real();
		MSAN_update_avg_ptkcnt(netdev);
	}
#endif				/* IEEE80211K */
...
extern void dfs_proc_aux(struct net_device *dev, SCANNER_CTL_EVENT event, DFS_STATE dfs_status, UINT8 IsFromAux);
static void _wlAuxChRadarDetection(struct work_struct *work)
{
	struct wlprivate_data *wlpd_p = container_of(work, struct wlprivate_data, dfstaskAux);
	struct wlprivate *wlpptr = wlpd_p->masterwlp;
...
static void _wlAcntRecordReady(struct work_struct *work)
{
	struct wlprivate_data *wlpd_p = container_of(work, struct wlprivate_data, acnttask);
	struct wlprivate *wlpptr = wlpd_p->masterwlp;
...
#ifdef SOC_W906X
static void _wlOffChanTask(struct work_struct *work)
{
	struct wlprivate_data *wlpd_p = container_of(work, struct wlprivate_data, offchantask);
	struct wlprivate *wlpptr = wlpd_p->masterwlp;

	wlOffChanTask(wlpptr->netDev);
}
#else				//906X off-channel
static void _wlOffChanDone(struct work_struct *work)
{
	struct wlprivate_data *wlpd_p = container_of(work, struct wlprivate_data, offchandonetask);
	struct wlprivate *wlpptr = wlpd_p->masterwlp;
...
#ifdef SYSFS_STADB_INFO
static void _wlSysfsSTAHdlTask(struct work_struct *work)
{
	struct wlprivate_data *wlpd_p = container_of(work, struct wlprivate_data, sysfstask);
	struct wlprivate *wlpptr = wlpd_p->masterwlp;
...
BOOLEAN wlInitChkSmacRdy(struct net_device *netdev)
{
...
static void mfg_send_cmd(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
#ifdef KF_MFG_MODE
...
#endif
#if 0				//For MOCHI
	writel(wlpptr->wlpd_p->pPhysCmdBuf, wlpptr->ioBase1 + MACREG_REG_GEN_PTR_MCI);
	writel(MACREG_H2ARIC_BIT_DOOR_BELL, wlpptr->ioBase1 + MACREG_REG_H2A_INTERRUPT_EVENTS_MCI);
#else				//For Auto check (PCIe or Mochi) reference from function wlprobe_set_reg_value(struct wlprivate *wlpptr)
	writel(wlpptr->wlpd_p->pPhysCmdBuf, wlpptr->ioBase1 + wlpptr->wlpd_p->reg.gen_ptr);
	writel(MACREG_H2ARIC_BIT_DOOR_BELL, wlpptr->ioBase1 + wlpptr->wlpd_p->reg.h2a_int_events);
#endif
}

static int mfg_wait_complete(struct net_device *netdev, unsigned short cmd)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
...
static int mfg_exec_cmd(struct net_device *netdev, unsigned short cmd)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	bool busy = false;
...
static void process_mfgbridge_cmd(struct sk_buff *skb)
{
	int device_no;
	unsigned short len;
	unsigned char *cmd;
	...
	ret = netlink_unicast(skb->sk, resp_skb, NETLINK_CB(skb).portid, MSG_DONTWAIT);
	if (ret < 0)
...
int wlInit(struct net_device *netdev, u_int16_t devid)
{
	...
	if (smeCmdBuf_q == NULL)
		smeCmdBuf_q = (macmgmtQ_CmdBuf_t *) wl_vzalloc(sizeof(macmgmtQ_CmdBuf_t) * SME_CMD_BUF_Q_LIMIT);
	if (!smeCmdBuf_q) {
...
#endif
#if defined(ACNT_REC) && defined (SOC_W906X)
	wlpptr->wlpd_p->rxinfotask.handle_func = (void (*)(unsigned long))wlrxinfo_qproc;
	...
	wlpptr->wlpd_p->racnttask.handle_func = (void (*)(unsigned long))wlRxPPDUAcntHndl;
...
//A390/A385 platform supports only 16 interrupts => Use polling function instead
	if (IS_PLATFORM(A390) || IS_PLATFORM(A380)) {
#ifdef USE_TASKLET
		tasklet_init(&wlpptr->wlpd_p->intrtask, (void *)wlIntrPoll, (unsigned long)netdev);
#else
		INIT_WORK(&wlpptr->wlpd_p->intrtask, (void (*)(void *))wlIntrPoll);
#endif
	}

	tasklet_init(&wlpptr->wlpd_p->rx_refill_task, (void *)wlRxBufFill, (unsigned long)netdev);
#ifdef USE_TASKLET
	...
	tasklet_init(&wlpptr->wlpd_p->buf_rel_task, (void *)wlTxDone, (unsigned long)netdev);
#else
...
#ifdef CONCURRENT_DFS_SUPPORT
	INIT_WORK(&wlpptr->wlpd_p->dfstaskAux, (void *)(void *)_wlAuxChRadarDetection);
#endif				/* CONCURRENT_DFS_SUPPORT */
#endif
...
#else				//906X off-channel
	INIT_WORK(&wlpptr->wlpd_p->offchandonetask, (void *)(void *)_wlOffChanDone);
#endif				//906X off-channel
#ifdef NEW_DP
	if (!wfa_11ax_pf)
		INIT_WORK(&wlpptr->wlpd_p->acnttask, (void *)(void *)_wlAcntRecordReady);
#endif
#ifdef SYSFS_STADB_INFO
	INIT_WORK(&wlpptr->wlpd_p->sysfstask, (void *)(void *)_wlSysfsSTAHdlTask);
#endif				/* SYSFS_STADB_INFO */

	INIT_WORK(&wlpptr->wlpd_p->offchan_scan_mgt, (void *)(void *)offchan_scan_mgt_handler);
	...
	SPIN_LOCK_INIT(&wlpptr->wlpd_p->locks.probeRspIEsLock);
	SPIN_LOCK_INIT(&wlpptr->wlpd_p->locks.offChanScanLock);
	...
	wlpptr->pCmdBuf = (unsigned short *)
	    wl_dma_alloc_coherent(wlpptr->wlpd_p->dev, CMD_BUF_SIZE, &wlpptr->wlpd_p->pPhysCmdBuf, wlpptr->wlpd_p->dma_alloc_flags);
#ifdef SOC_W906X
	...
	wlpptr->pFwDlBuf = (unsigned short *)
	    wl_dma_alloc_coherent(wlpptr->wlpd_p->dev, CMD_BUF_SIZE, &wlpptr->wlpd_p->pPhysFwDlBuf, wlpptr->wlpd_p->dma_alloc_flags);
	if (IS_BUS_TYPE_MCI(wlpptr))
	...
	wlpptr->pSsuBuf = (unsigned short *)
	    wl_dma_alloc_coherent(wlpptr->wlpd_p->dev, wlpptr->ssuSize, &wlpptr->wlpd_p->pPhysSsuBuf, wlpptr->wlpd_p->dma_alloc_flags);
	...
	wlpptr->pDspBuf = (int *)
	    wl_dma_alloc_coherent(wlpptr->wlpd_p->dev, wlpptr->dspSize, &wlpptr->wlpd_p->pPhysDspBuf, wlpptr->wlpd_p->dma_alloc_flags);
	...
	if (wlPrepareFwFile(netdev) == FAIL) {
		...
		dev_info(wlpptr->wlpd_p->dev, "Setup device to %d BSSs and %d STAs\n", bss_num, sta_num);
		wlpptr->smacconfig.num_running_bss = bss_num;
		wlpptr->smacconfig.num_running_sta = sta_num;

		memcpy(wlpptr->ioBase0, (void *)&wlpptr->smacconfig, sizeof(SMAC_CONFIG_st));
		...
		if ((major >= 25) || ((major == 24) && ((minor == 4) || (minor == 5)) && (rel >= 11))) {
			/* After SMAC PR24.4.11/PR24.5.11(STA-Only) and PR25, we can check the cap bit to know the MMDU support */
			wlpptr->wlpd_p->mmdu_mgmt_enable = wlpptr->wlpd_p->mmdu_data_enable =
			    !(wlpptr->smacStatusAddr->smacCap & SMAC_CAP_MMDU_NOT_SUPPORT);
		} else {
			wlpptr->wlpd_p->mmdu_mgmt_enable = wlpptr->wlpd_p->mmdu_data_enable = FALSE;
		}
		dev_info(wlpptr->wlpd_p->dev, "%s MMDU support\n", wlpptr->wlpd_p->mmdu_mgmt_enable ? "Enable" : "Disable");

		// Save the base address of rxSBinfoBaseAddr_v
		wlpptr->rxSBinfoBaseAddr_v = wlpptr->smac_base_vp + (wlpptr->smacStatusAddr->rxSBinfoBaseAddr - wlpptr->smacconfig.smacBmBaseAddr);

		printk("(v,p)=(%p, %x)\n", wlpptr->smac_base_vp, wlpptr->smacconfig.smacBmBaseAddr);
		printk("rxSBinfoBaseAddr_v=%p, %x, %u, %lu\n", wlpptr->rxSBinfoBaseAddr_v,
		       wlpptr->smacStatusAddr->rxSBinfoBaseAddr, wlpptr->smacStatusAddr->rxSBinfoUnitSize, (unsigned long)sizeof(RxSidebandInfo_t));
		...
		do {
			...
			if (wlFwGetCoreSniff(netdev, core_dump, buff) == FAIL) {
				...
				memcpy(&wlpptr->wlpd_p->coredump, buff, sizeof(coredump_t));
#if 0
/* FW version foramt is changed, below version need remap, temporary disbaled */
				...
				for (regionIdx = 0; regionIdx < wlpptr->wlpd_p->coredump.num_regions; regionIdx++) {
					printk("region[%2d].address = 0x%10x, region[%2d].length = 0x%10x\n", regionIdx,
					       wlpptr->wlpd_p->coredump.region[regionIdx].address, regionIdx,
					       wlpptr->wlpd_p->coredump.region[regionIdx].length);
				}
			}
		} while (0);
	...
	wlpptr->vmacSta_p->ampducfg = 0xFF;
	wlpptr->vmacSta_p->amsducfg.priority_aggr = 0xFF;
#ifdef FS_CAL_FILE_SUPPORT
	...
	init_completion(&wlpptr->wlpd_p->wlreset_complete);
	complete(&wlpptr->wlpd_p->wlreset_complete);
	init_completion(&wlpptr->wlpd_p->offchan_scan.offchan_complete);
	...
	for (index = 0; index < wlpptr->wlpd_p->NumOfAPs; index++) {
		...
		if (!use_localadmin_addr) {
			macaddr[5] = wlpptr->hwData.macAddr[5] + ((index + 1) & 0xff);
		} else {
...
#endif				/* BAND_STEERING */
	memset(CH_radio_status, 0, sizeof(mvl_status_t) * (IEEEtypes_MAX_CHANNELS + IEEEtypes_MAX_CHANNELS_A));
...
#endif
#ifdef PRD_CSI_DMA
	INIT_WORK(&wlpptr->wlpd_p->prd_csi_dma_done_wq, (void *)(void *)wl_WiFi_AoA_Decode);
#endif
	...
	wlpptr->cck_des.cck_des_conf.threshold_ceiling = CCK_DESENSE_THRESHOLD_CEILING;
	...
	wlpptr->cck_des.rx_abort_conf.threshold_ceiling = RX_ABORT_THRESHOLD_CEILING;
 ...
 err_init_cmd_buf:
	...
	wl_dma_free_coherent(wlpptr->wlpd_p->dev, CMD_BUF_SIZE, wlpptr->pCmdBuf, wlpptr->wlpd_p->pPhysCmdBuf);
 failed:
...
//#endif
...
#if defined(MRVL_MUG_ENABLE)
	INIT_WORK(&wlpptr->wlpd_p->mug.irq_task, (void *)(void *)mug_irq_task_handler);
#endif
	...
	wlpptr->ssuSize = SSU_BUF_SIZE;
	wlpptr->pSsuBuf = (unsigned short *)
	    pci_alloc_consistent(wlpptr->pPciDev, wlpptr->ssuSize, &wlpptr->wlpd_p->pPhysSsuBuf);
	printk("wlInit SSU pSsuBuf = %p  pPhysSsuBuf = %pad size=0x%08x\n", wlpptr->pSsuBuf, &wlpptr->wlpd_p->pPhysSsuBuf, wlpptr->ssuSize);
#endif
...
#ifndef NEW_DP
	writel((wlpptr->wlpd_p->descData[0].pPhysTxRing), wlpptr->ioBase0 + wlpptr->wlpd_p->descData[0].wcbBase);
#if NUM_OF_DESCRIPTOR_DATA >3
	int i;
	for (i = 1; i < TOTAL_TX_QUEUES; i++)
		writel((wlpptr->wlpd_p->descData[i].pPhysTxRing), wlpptr->ioBase0 + wlpptr->wlpd_p->descData[i].wcbBase);
#endif
	writel((wlpptr->wlpd_p->descData[0].pPhysRxRing), wlpptr->ioBase0 + wlpptr->wlpd_p->descData[0].rxDescRead);
	writel((wlpptr->wlpd_p->descData[0].pPhysRxRing), wlpptr->ioBase0 + wlpptr->wlpd_p->descData[0].rxDescWrite);
#endif
	if (wlFwSetHwSpecs(netdev)) {
		WLDBG_ERROR(DBG_LEVEL_2, "failed to set HW specs");
	}
 ...
 err_init_cmd_buf:
	...
	pci_free_consistent(wlpptr->pPciDev, CMD_BUF_SIZE, wlpptr->pCmdBuf, wlpptr->wlpd_p->pPhysCmdBuf);
	WLDBG_EXIT_INFO(DBG_LEVEL_2, NULL);
	return FAIL;
}
#endif				/* SOC_W906X */
...
int wlDeinit(struct net_device *netdev)
{
...
#endif
	{
		if (!vmacSta_p->preautochannelfinished || timer_pending(&vmacSta_p->scaningTimer))
			wait_ret = wait_for_completion_timeout(&wlpd_p->wlreset_complete, 30 * HZ);
	}

	wlFwSetAcntStop(netdev);
	TimerRemoveSync(&wfa_test_timer);
	...
	del_timer_sync(&wlpptr->wlpd_p->Timer);
	{
		int stream;
		for (stream = 0; stream < MAX_SUPPORT_AMPDU_TX_STREAM_RUNNING; stream++)
			TimerRemoveSync(&wlpptr->wlpd_p->Ampdu_tx[stream].timer);
	}
#ifdef MRVL_DFS
...
#ifdef TP_PROFILE
	del_timer_sync(&wlpptr->wlpd_p->tp_profile_timer);
#endif
#ifdef USE_TASKLET
#ifndef NAPI
	tasklet_kill(&wlpptr->wlpd_p->rxtask);
...
#ifdef AUTOCHANNEL
		TimerRemoveSync(&vmacSta_p->acs_cload.timer);
#endif				/* AUTOCHANNEL */
#ifdef COEXIST_20_40_SUPPORT
	...
	wlinitcnt--;
	wl_dma_free_coherent(wlpptr->wlpd_p->dev, CMD_BUF_SIZE, (caddr_t) wlpptr->pCmdBuf, wlpptr->wlpd_p->pPhysCmdBuf);
#ifdef SOC_W906X
	wl_dma_free_coherent(wlpptr->wlpd_p->dev, CMD_BUF_SIZE, (caddr_t) wlpptr->pFwDlBuf, wlpptr->wlpd_p->pPhysFwDlBuf);
#endif				/*SOC_W906X */
#ifdef SSU_SUPPORT
	if (wlpptr->pSsuBuf)
		wl_dma_free_coherent(wlpptr->wlpd_p->dev, wlpptr->ssuSize, wlpptr->pSsuBuf, wlpptr->wlpd_p->pPhysSsuBuf);
#endif
#ifdef DSP_COMMAND
	if (wlpptr->pDspBuf)
		wl_dma_free_coherent(wlpptr->wlpd_p->dev, wlpptr->dspSize, wlpptr->pDspBuf, wlpptr->wlpd_p->pPhysDspBuf);
#endif
#ifdef WLS_FTM_SUPPORT
	{
...
int wlDeinit(struct net_device *netdev)
{
	...
	if (wlinitcnt == 1)	//last one
		ap8x_remove_folder();
	wlinitcnt--;
	pci_free_consistent(wlpptr->pPciDev, CMD_BUF_SIZE, (caddr_t) wlpptr->pCmdBuf, wlpptr->wlpd_p->pPhysCmdBuf);
	dbg_replay_attack_release();
	//      free(wlpptr->wlpd_p);
	WLDBG_EXIT(DBG_LEVEL_2);
	return SUCCESS;
}
...
#if defined(RXACNT_REC) && defined (SOC_W906X)
static void wlRAcntHdlr(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
...
int wlRadarDetection(struct net_device *netdev, UINT8 from)
{
...
#ifdef CONCURRENT_DFS_SUPPORT
	if (from == DFS_AUX) {
		WLSYSLOG(netdev, WLSYSLOG_CLASS_ALL, WLSYSLOG_MSG_GEN_RADARDETECTION_AUX);
		WLSNDEVT(netdev, IWEVCUSTOM, (IEEEtypes_MacAddr_t *) & wlpptr->hwData.macAddr[0], WLSYSLOG_MSG_GEN_RADARDETECTION_AUX);
	} else
#endif
	{
	...
	toSmeMsg->Msg.RadarDetectionInd.chInfo.channel = PhyDSSSTable->CurrChan;
#endif
	memcpy(&toSmeMsg->Msg.RadarDetectionInd.chInfo.chanflag, &PhyDSSSTable->Chanflag, sizeof(CHNL_FLAGS));
...
static void AcntRdyIsrHdlr(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
...
#if defined(MRVL_MUG_ENABLE)
static void MugDataRdyIsrHdrl(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
...
void offChanDoneHdlr(struct net_device *netdev, offchan_status next_state)
{
	...
	if ((wlpptr->offchan_state == OFFCHAN_STARTED) && (next_state == OFFCHAN_CH_CHANGE)) {
		wlpptr->offchan_state = OFFCHAN_CH_CHANGE;
		schedule_work(&wlpptr->wlpd_p->offchantask);
	} else if ((wlpptr->offchan_state == OFFCHAN_CH_CHANGE) && (next_state == OFFCHAN_DONE)) {
...
#ifdef SOC_W8964
static void OffChanDoneHdlr(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
...
#if defined(ACNT_REC) && defined (SOC_W906X)
irqreturn_t wlSC5MSIX_RxInfo(int irq, void *dev_id)
{
	...
	qid = msg_id >> 1;
	intStatus = 1 << qid;
	WLDBG_INFO(DBG_LEVEL_3, "ISR: %s: qid %d %s, msg_id=%d, INTR_SHIFT=%d\n", __func__,
		   qid, (msg_id & 0x01) ? "SQ" : "RQ", msg_id, wlpptr->wlpd_p->intr_shift);
...
irqreturn_t wlSC5MSIX_r7(int irq, void *dev_id)
{
	struct msix_context *ctx = (struct msix_context *)dev_id;
	struct net_device *netdev = ctx->netDev;
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	...
	if ((msg_id & 0x01) == 0) {	//RQ = 0
		...
		if (intStatus & SC5_EVENT_RADAR_DETECTED) {
			...
			WLSYSLOG(netdev, WLSYSLOG_CLASS_ALL, WLSYSLOG_MSG_GEN_RADARDETECTION);
			if (wlpptr->wlpd_p->MrvlPriSharedMem.data) {
				dfs_freq = ((drv_fw_shared_t *) wlpptr->wlpd_p->MrvlPriSharedMem.data)->dfs_freg;
				if (dfs_freq != 0) {
					sprintf(evBuf, "DFS radar detection Freq = %d\n", ENDIAN_SWAP32(dfs_freq));
		...
		if ((intStatus & SC5_EVENT_FW) && (wlpptr->event_bufq_vaddr)) {
			...
			while (rdptr != wrptr) {
				 ...
				 */
				wlEventHandler(vmacSta_p, wlpptr->event_bufq_vaddr + rdptr * EVENT_BUFFQ_SIZE);
...
irqreturn_t wlSC5MSIX_rx(int irq, void *dev_id)
{
	struct msix_context *ctx = (struct msix_context *)dev_id;
	struct net_device *netdev = ctx->netDev;
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	...
	intStatus = 1 << qid;
	WLDBG_INFO(DBG_LEVEL_3, "ISR: %s: qid %d %s, msg_id=%d, INTR_SHIFT=%d\n", __func__,
		   qid, (msg_id & 0x01) ? "SQ" : "RQ", msg_id, wlpd_p->intr_shift);
...
irqreturn_t wlSC5MSIX_rel(int irq, void *dev_id)
{
	struct msix_context *ctx = (struct msix_context *)dev_id;
	struct net_device *netdev = ctx->netDev;
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
...
irqreturn_t wlSC5MSIX_tx(int irq, void *dev_id)
{
	struct msix_context *ctx = (struct msix_context *)dev_id;
	struct net_device *netdev = ctx->netDev;
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
...
irqreturn_t wlISR(int irq, void *dev_id)
{
	...
	do {
		intStatus = (readl(wlpptr->ioBase1 + SC5_MACREG_REG_INTERRUPT_CAUSE));
		if (intStatus != 0x00000000) {
			...
			if (!wlpptr->wlpd_p->pPciDev->msi_enabled) {
				writel((MACREG_A2HRIC_BIT_MASK_MSI & ~intStatus), wlpptr->ioBase1 + SC5_MACREG_REG_INTERRUPT_CAUSE);
			}
		}

		if (intStatus & SC5_RX_INTR_MASK) {
			spin_lock(&wlpptr->wlpd_p->locks.intLock);
			wlpptr->RxQId |= (intStatus & SC5_RX_INTR_MASK) >> SC5_RX_INTR_START;;
			spin_unlock(&wlpptr->wlpd_p->locks.intLock);
...
irqreturn_t wlISR(int irq, void *dev_id)
{
	...
	do {
		...
		if (intStatus != 0x00000000) {
			...
			intStatus &= mask;
#endif
			writel((MACREG_A2HRIC_BIT_MASK & ~intStatus), wlpptr->ioBase1 + MACREG_REG_A2H_INTERRUPT_CAUSE);
		}
		if ((intStatus & ISR_SRC_BITS) || (currIteration < MAX_ISR_ITERATION)) {
...
#ifdef NAPI
				if (netdev->flags & IFF_RUNNING) {
					wlInterruptMask(netdev, MACREG_A2HRIC_RX_DONE_HEAD_RDY);
					napi_schedule(&wlpptr->napi);
				}
#else
				wlRecvHdlr(netdev);
			...
			if (intStatus & MACREG_A2HRIC_NEWDP_CHANNEL_SWITCH) {
				intStatus &= ~MACREG_A2HRIC_NEWDP_CHANNEL_SWITCH;
				dfsChanSwitchHdlr(netdev);
				retVal = IRQ_HANDLED;
			}
#if defined(MRVL_MUG_ENABLE)
...
#ifdef SOC_W906X
		UINT32 hfctrl = IS_BUS_TYPE_MCI(wlpptr) ? MACREG_HFCTRL_MASK : MACREG_HFCTRL_MASK_MSI;
		printk("%s(), w_reg(%p)=%xh\n", __func__, (wlpptr->ioBase1 + reg_a2h_intr_mask), 0);
		writel(0x00, wlpptr->ioBase1 + reg_a2h_intr_mask);
		//writel((MACREG_A2HRIC_BIT_MASK),
		printk("%s(), w_reg(%p)=%xh\n", __func__, (wlpptr->ioBase1 + reg_a2h_intr_mask), hfctrl);
		writel(hfctrl, wlpptr->ioBase1 + reg_a2h_intr_mask);
#else
		writel(0x00, wlpptr->ioBase1 + MACREG_REG_A2H_INTERRUPT_MASK);

		writel((MACREG_A2HRIC_BIT_MASK), wlpptr->ioBase1 + MACREG_REG_A2H_INTERRUPT_MASK);
#endif				/* SOC_W906X */
	}
}
...
void wlInterruptUnMask(struct net_device *netdev, int mask)
{
	...
	if (wlChkAdapter(netdev)) {
#ifdef SOC_W906X
		writel((readl(wlpptr->ioBase1 + reg_a2h_intr_mask) | (mask)), wlpptr->ioBase1 + reg_a2h_intr_mask);
#else
		writel((readl(wlpptr->ioBase1 + MACREG_REG_A2H_INTERRUPT_MASK) | (mask)), wlpptr->ioBase1 + MACREG_REG_A2H_INTERRUPT_MASK);
#endif				/* SOC_W906X */
	}
}
...
int wlValidateSettings(struct net_device *netdev)
{
	...
	if (!allow_ht_tkip && (((*(mib->mib_wpaWpa2Mode) & 0x0F) == 1) ||
			       ((*(mib->mib_wpaWpa2Mode) & 0x0F) == 3)) && (IsHTmode(*(mib->mib_ApMode)) || IsVHTmode(*(mib->mib_ApMode)))) {
	...
	if ((mib->Privacy->PrivInvoked == 1) && (IsHTmode(*(mib->mib_ApMode)) || IsVHTmode(*(mib->mib_ApMode)))) {
...
void sta_track_expire(struct wlprivate *wlpptr, int force)
{
	...
	while ((info = list_first_entry(&wlpptr->wlpd_p->bandSteer.sta_track_list, struct sta_track_info, list)) &&
	       (wlpptr->wlpd_p->bandSteer.sta_track_num)) {
		if (!force &&
		    !((jiffies - info->last_seen) >= *(mib->mib_bandsteer_sta_track_max_age)) &&
		    !(wlpptr->wlpd_p->bandSteer.sta_track_num > *(mib->mib_bandsteer_sta_track_max_num)))
			break;
		force = 0;
...
struct sta_track_info *sta_track_get(struct wlprivate *wlpptr, const u8 * addr)
{
	struct sta_track_info *info;
	...
	list_for_each_entry(info, &wlpptr->wlpd_p->bandSteer.sta_track_list, list)
...
void sta_track_add(struct wlprivate *wlpptr, const u8 * addr)
{
	...
	if (wlpptr->wlpd_p->bandSteer.sta_track_num >= *(mib->mib_bandsteer_sta_track_max_num)) {
...
struct sta_auth_info *sta_auth_get(struct wlprivate *wlpptr, const u8 * addr)
{
	...
	list_for_each_entry(info, &wlpptr->wlpd_p->bandSteer.sta_auth_list, list)
	    if (memcmp(addr, info->addr, ETH_ALEN) == 0)
		return info;
...
void sta_auth_deinit(struct wlprivate *wlpptr)
{
	...
	while ((info = list_first_entry(&wlpptr->wlpd_p->bandSteer.sta_auth_list, struct sta_auth_info, list)) &&
	       (wlpptr->wlpd_p->bandSteer.sta_auth_num)) {
		list_del(&info->list);
		wlpptr->wlpd_p->bandSteer.sta_auth_num--;
		wl_kfree(info);
...
void unassocsta_track_expire(struct wlprivate *wlpptr, int force)
{
	...
	while ((info = list_first_entry(&wlpptr->wlpd_p->unassocSTA.sta_track_list, struct unassocsta_track_info, list)) &&
	       (wlpptr->wlpd_p->unassocSTA.sta_track_num)) {
		if (!force &&
		    !((jiffies - info->last_seen) >= *(mib->mib_unassocsta_track_max_age)) &&
		    !(wlpptr->wlpd_p->unassocSTA.sta_track_num > *(mib->mib_unassocsta_track_max_num)))
			break;
		force = 0;
...
struct unassocsta_track_info *unassocsta_track_get(struct wlprivate *wlpptr, const u8 * addr, u8 channel)
{
	struct unassocsta_track_info *info;
	...
	list_for_each_entry(info, &wlpptr->wlpd_p->unassocSTA.sta_track_list, list)
	    if ((memcmp(addr, info->addr, ETH_ALEN) == 0) && (info->channel == channel))
		return info;

	return NULL;
}

void unassocsta_track_add(struct wlprivate *wlpptr, const u8 * addr, u8 channel, u32 rssi)
{
	vmacApInfo_t *vmacSta_p = (vmacApInfo_t *) wlpptr->vmacSta_p;
	MIB_802DOT11 *mib = vmacSta_p->Mib802dot11;
	struct unassocsta_track_info *info;
	...
	if (wlpptr->wlpd_p->unassocSTA.sta_track_num >= *(mib->mib_unassocsta_track_max_num)) {
		/* Expire oldest entry to make room for a new one */
...
static int wlopen(struct net_device *netdev)
{
	...
	if (wfa_11ax_pf) {
		...
		memset(vmacSta_p->ofdma_mu_sta_addr, 0x0, IEEEtypes_ADDRESS_SIZE * MAX_OFDMADL_STA);
	}
	...
	wl_send_dev_up_event(netdev);
	if (!wlpptr->wlpd_p->privNdevStats.ch_start_time_sec)
...
static int wlstop(struct net_device *netdev)
{
	...
	for (j = 0; j < numVif; j++) {
		if ((wlpptr->vdev[j] != NULL) && (wlpptr->vdev[j]->flags & IFF_RUNNING)) {
			if (!warnmsg) {
				printk(KERN_INFO "disable %s will also disable all its virtual net devices.\n", netdev->name);
				warnmsg = 1;
			}
	...
	if (netdev->flags & IFF_RUNNING) {
		/* send ifdown event to customer */
		wl_send_dev_down_event(netdev);
		netif_stop_queue(netdev);
		netdev->flags &= ~IFF_RUNNING;
	}
	...
	TimerRemoveSync(&wlpptr->wlpd_p->bandSteer.queued_timer);
	for (i = 0; i < skb_queue_len(&wlpptr->wlpd_p->bandSteer.skb_queue); i++) {
...
#endif				/* BAND_STEERING */
#ifdef MULTI_AP_SUPPORT
	TimerRemoveSync(&wlpptr->wlpd_p->unassocSTA.waitTimer);
	TimerRemoveSync(&wlpptr->wlpd_p->unassocSTA.scanTimer);
	...
	if (entries) {
		...
		if (sta_buf != NULL) {
			...
			for (i = 0; i < entries; i++) {
				...
				pStaInfo->rx_err = StaStatsTbl.dot11FCSErrorCount;
				if (pStaInfo->rx_err > rx_err)
					wlpptr->netDevStats.rx_errors += pStaInfo->rx_err - rx_err;

				tx_err = pStaInfo->tx_err;
				if (StaStatsTbl.dot11MPDUCount > (StaStatsTbl.dot11SuccessCount + StaStatsTbl.dot11RetryCount))
					pStaInfo->tx_err = StaStatsTbl.dot11MPDUCount - StaStatsTbl.dot11SuccessCount - StaStatsTbl.dot11RetryCount;
				if (pStaInfo->tx_err > tx_err)
					wlpptr->netDevStats.tx_errors += pStaInfo->tx_err - tx_err;

				/* per radio retry count */
				tx_retries = pStaInfo->tx_retries;
				pStaInfo->tx_retries = StaStatsTbl.dot11RetryCount;
				if (pStaInfo->tx_retries > tx_retries)
					wlpptr->wlpd_p->privNdevStats.tx_retries += pStaInfo->tx_retries - tx_retries;
...
void get_phyif_device_stats(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	u32 i = 0;
	u32 numVif = wlpptr->wlpd_p->NumOfAPs + 1;	/* VAP + STA */

	if (wlpptr->vmacSta_p->master)
		return;

	wlpptr->phyif_netDevStats = wlpptr->netDevStats;
	for (i = 0; i < numVif; i++) {
		if (wlpptr->vdev[i] != NULL) {
			struct wlprivate *vdev_wlpptr = NETDEV_PRIV_P(struct wlprivate, wlpptr->vdev[i]);

			calculate_err_count(wlpptr->vdev[i]);

			/* Dropped packets by core network */
			wlpptr->phyif_netDevStats.tx_dropped += wlpptr->vdev[i]->tx_dropped.counter;
			wlpptr->phyif_netDevStats.rx_dropped += wlpptr->vdev[i]->rx_dropped.counter;

			wlpptr->phyif_netDevStats.rx_packets += vdev_wlpptr->netDevStats.rx_packets;
			wlpptr->phyif_netDevStats.tx_packets += vdev_wlpptr->netDevStats.tx_packets;
			wlpptr->phyif_netDevStats.rx_bytes += vdev_wlpptr->netDevStats.rx_bytes;
			wlpptr->phyif_netDevStats.tx_bytes += vdev_wlpptr->netDevStats.tx_bytes;
			wlpptr->phyif_netDevStats.rx_errors += vdev_wlpptr->netDevStats.rx_errors;
			wlpptr->phyif_netDevStats.tx_errors += vdev_wlpptr->netDevStats.tx_errors;
			wlpptr->phyif_netDevStats.rx_dropped += vdev_wlpptr->netDevStats.rx_dropped;
			wlpptr->phyif_netDevStats.tx_dropped += vdev_wlpptr->netDevStats.tx_dropped;
			wlpptr->phyif_netDevStats.multicast += vdev_wlpptr->netDevStats.multicast;
			wlpptr->phyif_netDevStats.collisions += vdev_wlpptr->netDevStats.collisions;
			wlpptr->phyif_netDevStats.rx_length_errors += vdev_wlpptr->netDevStats.rx_length_errors;
			wlpptr->phyif_netDevStats.rx_over_errors += vdev_wlpptr->netDevStats.rx_over_errors;
			wlpptr->phyif_netDevStats.rx_crc_errors += vdev_wlpptr->netDevStats.rx_crc_errors;
			wlpptr->phyif_netDevStats.rx_frame_errors += vdev_wlpptr->netDevStats.rx_frame_errors;
			wlpptr->phyif_netDevStats.rx_fifo_errors += vdev_wlpptr->netDevStats.rx_fifo_errors;
			wlpptr->phyif_netDevStats.rx_missed_errors += vdev_wlpptr->netDevStats.rx_missed_errors;
			wlpptr->phyif_netDevStats.tx_aborted_errors += vdev_wlpptr->netDevStats.tx_aborted_errors;
			wlpptr->phyif_netDevStats.tx_carrier_errors += vdev_wlpptr->netDevStats.tx_carrier_errors;
			wlpptr->phyif_netDevStats.tx_fifo_errors += vdev_wlpptr->netDevStats.tx_fifo_errors;
			wlpptr->phyif_netDevStats.tx_heartbeat_errors += vdev_wlpptr->netDevStats.tx_heartbeat_errors;
			wlpptr->phyif_netDevStats.tx_window_errors += vdev_wlpptr->netDevStats.tx_window_errors;
			wlpptr->phyif_netDevStats.rx_compressed += vdev_wlpptr->netDevStats.rx_compressed;
			wlpptr->phyif_netDevStats.tx_compressed += vdev_wlpptr->netDevStats.tx_compressed;
		}
	}
}
...
static struct net_device_stats *wlgetStats(struct net_device *netdev)
{
	...
	WLDBG_EXIT(DBG_LEVEL_2);
	if (wlpptr->vmacSta_p && !wlpptr->vmacSta_p->master) {
		get_phyif_device_stats(netdev);
		return &(wlpptr->phyif_netDevStats);
	}
...
#ifdef SOC_W906X
static void wlreset_allInf(struct net_device *netdev)
{
#if defined(MBSS)
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	int i = 0, gid;
...
#endif				//W906X
static void wlstop_allInf(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	int i = 0;
...
void wlVirtualInfUp(struct net_device *netdev)
{
	...
	if (wfa_11ax_pf) {
		memset(&vmacSta_p->dl_ofdma_para, 0x0, sizeof(struct dl_ofdma_parameter_s));
		memset(vmacSta_p->ofdma_mu_sta_addr, 0x0, IEEEtypes_ADDRESS_SIZE * MAX_OFDMADL_STA);
		if (vmacSta_p->master) {
			memset(&vmacSta_p->master->dl_ofdma_para, 0x0, sizeof(struct dl_ofdma_parameter_s));
			memset(vmacSta_p->master->ofdma_mu_sta_addr, 0x0, IEEEtypes_ADDRESS_SIZE * MAX_OFDMADL_STA);
		}
	}
...
static void wltxwait_txdone(struct net_device *netdev)
{
#ifdef SOC_W906X
	/*{
	...
	do {
		...
		txDoneTail = priv->wlpd_p->descData[0].TxDoneTail & (MAX_TX_RING_DONE_SIZE - 1);
		txDoneHead = txDoneHeadnew & (MAX_TX_RING_DONE_SIZE - 1);
		printk("\n\ntxDoneHead=%d, txDoneTail=%d\n\n", txDoneHead, txDoneTail);
...
#define GPIO_32_59_DATAOUT_CONTROL_REG 0x18144
	...
	printk("%s: power reset by GPIO %d\n", netdev->name, priv->wlpd_p->gpioresetpin);
...
#ifdef SOC_W906X
void wlOffChannelItemFree(offChanListItem * offchan_current_req_p)
{
	if (offchan_current_req_p != NULL) {
		if (offchan_current_req_p->txSkb_p != NULL) {
			wl_free_skb(offchan_current_req_p->txSkb_p);
		}
		wl_kfree(offchan_current_req_p);
	}
}

static void wlOffChannelReqListFree(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	...
	while ((offChanListItem_p = (offChanListItem *) ListGetItem(&wlpptr->wlpd_p->offChanList)) != NULL) {
		wlOffChannelItemFree(offChanListItem_p);
#if 0
...
static void wlOffChannelStop(struct net_device *netdev)
{
	...
	if (wlpptr->master) {
		...
		wlpptr_offchan = NETDEV_PRIV_P(struct wlprivate, netdev_offchan);
	} else {
...
#if 0
	/* Need this part if wlOffChannelStop caller have race condition with wlreset.
	 * Like wlreset_mbss, it may clear some ACS offch scan req triggered by wlreset.
	 * Protect scan request during ACS period.
	 */
	if ((*(wlpptr_offchan->vmacSta_p->ShadowMib802dot11->mib_autochannel) || !IsChanListScanFinished(netdev_offchan))
	    && (wlpptr_offchan->vmacSta_p->preautochannelfinished == 0)) {
		wait_ret = wait_for_completion_timeout(&wlpptr_offchan->vmacSta_p->scan_complete,
						       wlpptr_offchan->vmacSta_p->NumScanChannels * SCAN_TIME * TIMER_100MS * 2);
		printk("%s, %s Wait scan(%d) finish timeout,  %d\n", netdev_offchan->name, __func__, wlpptr_offchan->vmacSta_p->NumScanChannels,
		       wait_ret);
	}
#endif

	/* Clear flag for next SetupScan. */
	wlpptr_offchan->vmacSta_p->busyScanning = 0;
	DisableBlockTrafficMode(wlpptr_offchan->vmacSta_p);
	Disable_ScanTimerProcess(wlpptr_offchan->vmacSta_p);

	/* Stop offchan timer and reset init_flag before clean offchan list queue.
	 * Avoid another workqueue "offchan_scan_mgt" to start the next offchan-scan.
	 */
	OffchannelScanDisable(netdev_offchan);

	//Purge offchan queue
	wlOffChannelReqListFree(netdev_offchan);
	/* Wait 2s(MAX dwell time is 1000ms) the finial offchan scan DONE */
	retry_cnt = wait_for_completion_timeout(&wlpptr->wlpd_p->offchan_scan.offchan_complete, 2 * HZ);
	if (wlpptr_offchan->offchan_state != OFFCHAN_IDLE)
		wlOffChannelReset(netdev_offchan);

	/* When Free the offchan req list, also reset all offchan feature status and send it to customer */
	for (offch_feature = OFFCHAN_BY_LIST; offch_feature < OFFCHAN_FEATURE_END; offch_feature++) {
		if ((wlpptr->wlpd_p->offchan_scan.offchan_feature_active[offch_feature] == TRUE)) {
			wlpptr->wlpd_p->offchan_scan.next_offch.ch_idx = 0;
			wl_send_offchan_active_event(netdev_offchan, WLMGR_ID_DCS, WLMGR_CMD_DEV_OFFCH_COMPLETED,
						     OFFCHAN_GET_ID_FROM_FEATURE(offch_feature, 0));
		}
		wlpptr->wlpd_p->offchan_scan.offchan_feature_active[offch_feature] = FALSE;
	}

	return;
}

void wlOffChannelReset(struct net_device *netdev)
{
	struct wlprivate *priv = NETDEV_PRIV_P(struct wlprivate, netdev);
	...
	/* Reset offchan_state to OFFCHAN_DONE -> OFFCHAN_IDLE */
	if (priv->offchan_state == OFFCHAN_STARTED) {
		offChanDoneHdlr(priv->netDev, OFFCHAN_CH_CHANGE);
	}
	if (priv->offchan_state == OFFCHAN_CH_CHANGE) {
		offChanDoneHdlr(priv->netDev, OFFCHAN_DONE);
	}
}
...
static void wlFwHardreset(struct net_device *netdev, int firsttime)
{
	struct wlprivate *priv = NETDEV_PRIV_P(struct wlprivate, netdev);
...
#ifdef SOC_W906X
		txSendTail = readl(priv->ioBase1 + priv->wlpd_p->reg.tx_send_tail);
#else
...
#else
		WLDBG_WARNING(DBG_LEVEL_1, "No mci_do_reset() in system, firmware redownload may fail\n");
#endif				//CONFIG_MARVELL_MOCHI_DRIVER
	}
#endif				//SOC_W906X
	//cleanup tx queue
	if (!firsttime) {
		...
		printk("TxSentHead=%d TxSentTail=%d txQueRecord=%d\n", txSendHeadNew, txSendTail, skb_queue_len(&priv->wlpd_p->txQueRecord));
	...
	if (!IS_BUS_TYPE_MCI(priv)) {
		pci_free_consistent(priv->pPciDev, SC5_HFRAME_MEM_SIZE, priv->hframe_virt_addr, priv->hframe_phy_addr);
#else
	{
		pci_free_consistent(priv->pPciDev, 0x4000, priv->pCmdBuf, priv->wlpd_p->pPhysCmdBuf);
		...
		if (pci_save_state(priv->pPciDev)) {
			dev_err(&priv->pPciDev->dev, "Failed to save pci state\n");
			return;
		}
#else
		printk("%s: baseaddress0 0x%X \n", netdev->name, (unsigned int)priv->wlpd_p->baseaddress0);
		printk("%s: baseaddress2 0x%X \n", netdev->name, (unsigned int)priv->wlpd_p->baseaddress2);

		pci_write_config_dword(priv->wlpd_p->pPciDev, PCI_BASE_ADDRESS_0, priv->wlpd_p->baseaddress0);
		pci_write_config_dword(priv->wlpd_p->pPciDev, PCI_BASE_ADDRESS_2, priv->wlpd_p->baseaddress2);
#endif
		...
		release_mem_region(pci_resource_start(priv->pPciDev, 4), pci_resource_len(priv->pPciDev, 4));
#endif
		iounmap(priv->ioBase1);
		iounmap(priv->ioBase0);

		release_mem_region(pci_resource_start(priv->pPciDev, 2), pci_resource_len(priv->pPciDev, 2));
		release_mem_region(pci_resource_start(priv->pPciDev, 0), pci_resource_len(priv->pPciDev, 0));
...

		{
			...
			if (!request_mem_region(physAddr, pci_resource_len(priv->pPciDev, 0), DRV_NAME)) {
				printk(KERN_ERR "%s: cannot reserve PCI memory region 0\n", DRV_NAME);
			}

			physAddr1[0] = ioremap(physAddr, pci_resource_len(priv->pPciDev, 0));
			physAddr1[1] = 0;
...
#ifdef SOC_W906X
			priv->smacCfgAddr = &((SMAC_CTRL_BLK_st *) priv->ioBase0)->config;
			priv->smacStatusAddr = &((SMAC_CTRL_BLK_st *) priv->ioBase0)->status;
#endif

			physAddr = pci_resource_start(priv->pPciDev, priv->nextBarNum);
			if (!request_mem_region(physAddr, pci_resource_len(priv->pPciDev, priv->nextBarNum), DRV_NAME)) {
				printk(KERN_ERR "%s: cannot reserve PCI memory region 1\n", DRV_NAME);
			}
			physAddr2[0] = ioremap(physAddr, pci_resource_len(priv->pPciDev, priv->nextBarNum));
			physAddr2[1] = 0;
			...
			if (!priv->ioBase2) {
				printk(KERN_ERR "%s: cannot remap PCI memory region 2\n", DRV_NAME);
			}
			priv->hframe_virt_addr = (unsigned short *)
			    pci_alloc_consistent(priv->pPciDev, SC5_HFRAME_MEM_SIZE, &priv->hframe_phy_addr);
#else
			priv->pCmdBuf = (unsigned short *)
			    pci_alloc_consistent(priv->pPciDev, 0x4000, &priv->wlpd_p->pPhysCmdBuf);
#endif


			priv->netDev->mem_start = pci_resource_start(priv->pPciDev, 0);
			priv->netDev->mem_end = physAddr + pci_resource_len(priv->pPciDev, 1);
			...
			if (!firsttime) {
				...
				if ((retCode = wlRxRingAlloc(netdev)) == 0) {
					if ((retCode = wlRxRingReInit(netdev)) != 0) {
						printk(KERN_ERR "%s: initializing RX ring failed\n", netdev->name);
					}
				} else {
					printk(KERN_ERR "%s: allocating RX ring failed\n", netdev->name);
				...
				writel(1023, priv->ioBase1 + MACREG_REG_RxDescHead);
			}
			AllocSharedMem(priv);
...
void wlReinit(struct net_device *netdev)
{
	...
	memcpy(parent_wlpptr->ioBase0, (void *)&parent_wlpptr->smacconfig, sizeof(SMAC_CONFIG_st));
	...
	priv->wlpd_p->smon.exceptionCmdTOEvt_rcvd = 0;
...
#ifdef AUTOCHANNEL
	if (priv->vmacSta_p->preautochannelfinished == 0) {
		/* If the ACS is not completed, execute it again. */
		scanControl(priv->vmacSta_p);
	}
#endif
	return;
}
...
void wlFwHardResetAndReInit(struct net_device *netdev, U8 halt)
{
	...
	/* If there is offchan scan still work, wait it complete before disable interrupts */
	wlOffChannelStop(netdev);
...
#endif
			if (priv->vdev[index]) {
				wlpptr = NETDEV_PRIV_P(struct wlprivate, priv->vdev[index]);
				wlpptr->ioBase0 = priv->ioBase0;
				wlpptr->ioBase1 = priv->ioBase1;
				wlpptr->netDev->mem_start = priv->netDev->mem_start;
...
int wlInit_wds(struct wlprivate *wlpptr)
{
	...
	for (i = 0; i < MAX_WDS_PORT; i++) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 17, 0)
		dev = alloc_netdev(0, DRV_NAME_WDS, NET_NAME_UNKNOWN, wlwlan_setup);
#else
		...
		wlpptr->vmacSta_p->wdsPort[i].netDevWds->needed_headroom = SKB_INFO_SIZE;
		...
		if (name_len <= IFNAMSIZ)
			memcpy(wlpptr->vmacSta_p->wdsPort[i].netDevWds->name, temp_name, name_len);
		...
		wlpptr->vmacSta_p->wdsPort[i].wdev.netdev = wlpptr->vmacSta_p->wdsPort[i].netDevWds;
		wlpptr->vmacSta_p->wdsPort[i].netDevWds->ieee80211_ptr = &wlpptr->vmacSta_p->wdsPort[i].wdev;
		SET_NETDEV_DEV(wlpptr->vmacSta_p->wdsPort[i].netDevWds, wiphy_dev(wlpptr->vmacSta_p->wdsPort[i].wdev.wiphy));
#endif
...
static int wlopen_mbss(struct net_device *netdev)
{
	...
	WLDBG_ENTER(DBG_LEVEL_2);
	if ((vmacSta_p->VMacEntry.macId == (bss_num - 1)) && (wlpd_p->SharedBssState == SHARE_STA)) {
	...
	if (wlpptr->cb_enable == TRUE) {
		...
		TimerFireIn(&wlpptr->bnc_timer, 1, &bcn_timer_routine, (void *)wlpptr->netDev, 1);
	...
	if (vmacSta_p->master) {
		...
		MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
		if (Is5GBand(*(rootmib->mib_ApMode)) != (Is5GBand(*(mib->mib_ApMode)))) {
			...
			*(mib->NotBssBasicRateMask) = *(rootmib->NotBssBasicRateMask);
			*(mib->mib_shortSlotTime) = *(rootmib->mib_shortSlotTime);
#endif
		}
	}
	...
	/* If there is one vdev running, enable it */
	OffchannelScanEnable(netdev);

	/* send ifup event to customer */
	wl_send_dev_up_event(netdev);

	if ((wlpptr->devid == SC5 || wlpptr->devid == SCBT) && (vmacSta_p->VMacEntry.macId == (bss_num - 1)))
...
#endif				/* CCK_DESENSE */
	if (!IsACSOnoing(netdev) && wlpptr->wlpd_p->pdfsApMain && DfsEnabledChannel(netdev, &wlpptr->wlpd_p->pdfsApMain->dfsApDesc.currChanInfo)) {
		MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
		MIB_PHY_DSSS_TABLE *PhyDSSSTable = mib->PhyDSSSTable;
...
static int wlstop_mbss(struct net_device *netdev)
{
	...
	/* reset HM bcnstucktimestamp when MBSS stop */
	wlpptr->wlpd_p->smon.bcnstucktimestamp[vmacSta_p->VMacEntry.macId] = 0;
...
#ifdef CB_SUPPORT
	if (wlpptr->cb_enable == TRUE) {
		TimerRemoveSync(&wlpptr->bnc_timer);
		// Close the timer if it's not cb_enabled
		TimerRemoveSync(&wlpptr->cb_mctmer);
	}
#endif				//CB_SUPPORT
	if (wlpptr->vmacSta_p != NULL)
	...
	if (wl_util_all_vapif_is_down(netdev) == TRUE)
		/* If only the radio interface is turned up and all vdevs are turned down, FW cannot handle offchan scanning.
		 * It must check all vdev is down and disable offchannel timer and reset the init_flag.
		 */
		OffchannelScanDisable(netdev);

	if (wlpptr->vmacSta_p != NULL) {
		DisableMacMgmtTimers(wlpptr->vmacSta_p);
		if ((wlpptr->devid == SC5 || wlpptr->devid == SCBT) &&
		    vmacSta_p->VMacEntry.macId == (bss_num - 1) && wlpd_p->SharedBssState == SHARE_VAP)
	...
	TimerRemoveSync(&vmacSta_p->deauth_block_timer);
...
int wlreset_mbss(struct net_device *netdev)
{
	...
	/* reset HM bcnstucktimestamp before MBSS reset */
	wlpptr->wlpd_p->smon.bcnstucktimestamp[vmacSta_p->VMacEntry.macId] = 0;
...
#ifdef WDS_FEATURE
	{
		...
		for (i = 0; i < MAX_WDS_PORT; i++) {
			if (wdsPortActive(netdev, i) && netif_running(wlpptr->vmacSta_p->wdsPort[i].netDevWds)) {
...
static int wlopen_monif(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	PROM_CNF_t PromCnf;
...
static int wlstop_monif(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	PROM_CNF_t PromCnf;
...
int wlInit_monif(struct wlprivate *wlp, unsigned char *macAddr)
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 17, 0)
	dev = alloc_netdev(sizeof(struct wlprivate), DEV_NAME_MON_INTF, NET_NAME_UNKNOWN, wlwlan_setup);
#else
	dev = alloc_netdev(sizeof(struct wlprivate), DEV_NAME_MON_INTF, wlwlan_setup);
#endif
	if (dev) {
		wlpptr = NETDEV_PRIV(struct wlprivate, dev);
		NETDEV_PRIV_S(dev) = wlpptr;
	...
	sprintf(wlpptr->netDev->name, "%s%1d%s%1d", DRV_NAME, wlinitcnt, DEV_NAME_MON_INTF, 0);
	...
	wlpptr->vmacSta_p = Mac_Init((void *)wlp, wlpptr->netDev, &macAddr[0], WL_OP_MODE_MONIF, wlinitcnt);
	if (wlpptr->vmacSta_p == NULL) {
		printk(KERN_ERR "%s: failed to init driver mac\n", wlpptr->netDev->name);
		goto err_out;
	}
	...
	printk("[%s] wlinitcnt=%d, vmacIndex=%d\n", __func__, wlinitcnt, wlp->wlpd_p->vmacIndex);
...
int wlInit_mbss(struct wlprivate *wlp, unsigned char *macAddr)
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 17, 0)
	dev = alloc_netdev(sizeof(struct wlprivate), DRV_NAME_VMAC, NET_NAME_UNKNOWN, wlwlan_setup);
#else
	...
	sprintf(temp_name, "%s%1d%s%1d", DRV_NAME, wlinitcnt, DRV_NAME_VMAC, wlp->wlpd_p->vmacIndex);
	name_len = strlen(temp_name);
...
void wlLinkMgt(struct net_device *netdev, UINT8 phyIndex)
{
	...
	if (*(primary_mib->mib_autochannel)) {
		...
		} else if ((*(vmacSta_p->Mib802dot11->mib_STAMode) < CLIENT_MODE_A) || (*(vmacSta_p->Mib802dot11->mib_STAMode) == CLIENT_MODE_N_24)) {
			for (i = 0; i < IEEEtypes_MAX_CHANNELS; i++) {
...
#ifdef RSN_RESOLVE
	strncpy((char *)&tmpClientSSID[phyIndex][0], (const char *)&(mib->StationConfig->DesiredSsId[0]), 33);
#endif				/* RSN_RESOLVE */
	ssidLen = strlen((const char *)&(mib->StationConfig->DesiredSsId[0]));
	IE_p->ElementId = SSID;
	IE_p->Len = ssidLen;
	ieBufLen += sizeof(IEEEtypes_InfoElementHdr_t);
	strncpy((char *)&ieBuf[ieBufLen], (const char *)&(mib->StationConfig->DesiredSsId[0]), sizeof(ieBuf) - ieBufLen);
	ieBufLen += IE_p->Len;
	IE_p = (IEEEtypes_InfoElementHdr_t *) & ieBuf[ieBufLen];
	...
	if (IsVHTmode(*(mib->mib_ApMode))) {
		...
		if (vStaInfo_p->bssDescProfile_p->VHTOp.len) {
			if (vStaInfo_p->bssDescProfile_p->VHTOp.ch_width == 2) {
				...
				if (1 == (ret = isSupport160MhzByCenterFreq(wlpptr, VHT_EXTENDED_NSS_BW_CAPABLE,
		...
		if (ch_bw_changed_to) {
			if (mib->PhyDSSSTable->Chanflag.ChnlWidth == CH_AUTO_WIDTH) {
				wlpptr->wlpd_p->repeaterUpdateChannelWidth = ch_bw_changed_to;
			} else if (mib->PhyDSSSTable->Chanflag.ChnlWidth > ch_bw_changed_to) {
				wlpptr->wlpd_p->repeaterUpdateChannelWidth = ch_bw_changed_to;
			}
		}
...
void wlStatusUpdate_clientParent(UINT32 data1, UINT8 * info_p, UINT32 data2)
{
	...
	case MmgtIndicationSignals:
		...
		if (linkUp && mlmeAssociatedFlag) {
			...
			mwl_cfg80211_connect_result_event(dev_p, (uint8_t *) & mlmeBssid, WLAN_STATUS_SUCCESS);
#else
#ifdef SOC_W906X
			mwl_cfg80211_connect_result_event(dev_p, (uint8_t *) & mlmeBssid, WLAN_STATUS_SUCCESS);
#else
			mwl_send_vendor_assoc_event(dev_p, (uint8_t *) & mlmeBssid);
#endif				//SOC_W906X
			...
			wlUpdateMibsWithBssProfile(priv, (vmacStaInfo_t *) vmacEntry_p->info_p);
		} else {
...
#ifdef MRVL_WPS_CLIENT
			/* Send event to user space */
			if ((mlmeBssid[0] && mlmeBssid[1] && mlmeBssid[2] && mlmeBssid[3] && mlmeBssid[4] && mlmeBssid[5])) {
				WLSNDEVT(dev_p, IWEVEXPIRED, (IEEEtypes_MacAddr_t *) & mlmeBssid[0], NULL);
#ifdef CFG80211
#ifdef CFG80211_COMPATIABLE
				mwl_cfg80211_connect_result_event(dev_p, (uint8_t *) & mlmeBssid, WLAN_STATUS_UNSPECIFIED_FAILURE);
#else
#ifdef SOC_W906X
				mwl_cfg80211_connect_result_event(dev_p, (uint8_t *) & mlmeBssid, WLAN_STATUS_UNSPECIFIED_FAILURE);
#else
				mwl_send_vendor_disassoc_event(dev_p, (uint8_t *) & mlmeBssid[0]);
#endif				//SOC_W906X
#endif				/* CFG80211_COMPATIABLE */
#endif				/* CFG80211 */
			}
...
int wlopen_client(struct net_device *netdev)
{
	...
	MIB_802DOT11 *mib;
#ifdef CFG80211
	struct wlprivate *wiphy_priv = mwl_cfg80211_get_priv(wlpptr->wdev.wiphy);
#endif
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	...
	if ((wlpptr->devid == SC5 || wlpptr->devid == SCBT) && vmacSta_p->VMacEntry.macId == (bss_num - 1) && wlpd_p->SharedBssState == SHARE_VAP) {
	...
	if (!(*(mib->mib_STAMacCloneEnable) == 1)) {
		if (wlFwSetBssForClientMode(netdev, WL_ENABLE)) {
			WLDBG_ERROR(DBG_LEVEL_0, "Falied to start the %d" "th BSS for client mode\n", vmacSta_p->VMacEntry.macId);
			return -EIO;
		}
	}
#endif				/* SOC_W906X */
...
static int wlstop_client(struct net_device *netdev)
{
...
#ifdef SOC_W906X
	if ((wlpd_p->downloadSuccessful == TRUE) && wlFwSetBssForClientMode(netdev, WL_DISABLE)) {
		WLDBG_ERROR(DBG_LEVEL_0, "Falied to stop the %d" "th BSS for client mode\n", vmacSta_p->VMacEntry.macId);
		return -EIO;
	}
#endif				/* SOC_W906X */
	...
	if ((wlpptr->devid == SC5 || wlpptr->devid == SCBT) && vmacSta_p->VMacEntry.macId == (bss_num - 1) && wlpd_p->SharedBssState == SHARE_STA)
		wlpd_p->SharedBssState = SHARE_NONE;
...
#ifdef SOC_W906X
	if ((!(*(vmacSta_p->Mib802dot11->mib_STAMacCloneEnable) == 1)) && (wlpptr->wlpd_p->downloadSuccessful == TRUE) &&
...
int wlInit_client(struct wlprivate *wlp, unsigned char *macAddr_p, unsigned char *ApRootmacAddr_p)
{
	...
	{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 17, 0)
		dev = alloc_netdev(sizeof(struct wlprivate), DRV_NAME_CLIENT, NET_NAME_UNKNOWN, wlwlan_setup);
#else
		...
		sprintf(wlpptr->netDev->name, "%s%1d%s%1d", DRV_NAME, wlinitcnt, DRV_NAME_CLIENT, 0);
		wlpptr->netDev->irq = wlp->netDev->irq;
		wlpptr->netDev->mem_start = wlp->netDev->mem_start;
		wlpptr->netDev->mem_end = wlp->netDev->mem_end;
		...
		{
			{
				if ((clientVMacEntry_p = smeInitParentSession(wlinitcnt,
									      macAddr_p,
									      0, &wlStatusUpdate_clientParent, (void *)wlpptr->netDev)) == NULL) {
					goto err_init;
				}
...
void WlSendDeauth(struct net_device *netdev)
{
	...
	show_buf = sta_buf;
	for (i = 0; i < entries; i++) {
		if ((pStaInfo = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) show_buf, STADB_DONT_UPDATE_AGINGTIME)) == NULL) {
			printk("error: NO station info found \n");
			break;
		}
#ifdef SOC_W906X
		macMgmtMlme_SendDeauthenticateMsg(vmacSta_p, &pStaInfo->Addr, pStaInfo->StnId, IEEEtypes_REASON_DEAUTH_LEAVING, TRUE);
#else
		macMgmtMlme_SendDeauthenticateMsg(vmacSta_p, (IEEEtypes_MacAddr_t *) pStaInfo->Addr, pStaInfo->StnId,
...
void wlReadyStart160MhzBcn(DfsApDesc * dfsDesc_p)
{
	...
	if (priv->wlpd_p->bStopBcnProbeResp && macMgmtMlme_DfsEnabled(dev) &&
	    ((PhyDSSSTable->Chanflag.ChnlWidth == CH_160_MHz_WIDTH) || (PhyDSSSTable->Chanflag.ChnlWidth == CH_AUTO_WIDTH))) {
...
#ifdef CONCURRENT_DFS_SUPPORT
		channel = DfsDecideNewTargetChannel(dev, dfsDesc_p, TRUE, FALSE);
#else
		...
		memcpy(&me->dfsApDesc.currChanInfo.chanflag, &PhyDSSSTable->Chanflag, sizeof(CHNL_FLAGS));
		FireCACTimer(me);
...
{
	struct wlprivate_data *wlpd_p = (wlpptr != NULL) ? wlpptr->wlpd_p : NULL;
	...
	if (status) {
		...
		wlpd_p->bss_active &= (~(1 << vmacSta_p->VMacEntry.macId));
		wlpd_p->smon.ActVapBitmap &= (~(1 << vmacSta_p->VMacEntry.macId));
	}

}
#endif
#ifdef MV_NSS_SUPPORT
struct sk_buff *wlAllocSkb(unsigned int length)
{
	struct sk_buff *skb;
	...
	if (skb) {
		mdat_ptr = (mv_nss_metadata_t *) wlNssOps->get_metadata_skb(skb);
