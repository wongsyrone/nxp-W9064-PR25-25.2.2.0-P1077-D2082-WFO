...
static int ap8x_dump_seq_show(struct seq_file *s, void *v)
{
	...
	j = 0;
	seq_printf(s, "%s [%2d %2d %2d %2d]", netdev->name, *mib_guardInterval_p, PhyDSSSTable->Chanflag.ChnlWidth,
...
int ap8x_dump_proc_register(struct net_device *dev)
{
	...
	for (i = 0; i < TOTAL_SETS; i++) {
		...
		case 4:
			...
			break;
		case 5:
			sprintf(devdump[cardid][i].filename, DUMP_RFE_FILE_NAME, dev->name);
			break;
		case 6:
			sprintf(devdump[cardid][i].filename, DUMP_RFF_FILE_NAME, dev->name);
			break;
		case 7:
			sprintf(devdump[cardid][i].filename, DUMP_RFG_FILE_NAME, dev->name);
			break;
		default:
			sprintf(devdump[cardid][i].filename, DUMP_RFH_FILE_NAME, dev->name);
			break;
#else
		default:
			sprintf(devdump[cardid][i].filename, DUMP_RFD_FILE_NAME, dev->name);
		...
		devdump[cardid][i].ap8x_dump_proc =
		    proc_create_data(devdump[cardid][i].filename, 0666, ap8x, &ap8x_dump_file_ops, &devdump[cardid][i]);
