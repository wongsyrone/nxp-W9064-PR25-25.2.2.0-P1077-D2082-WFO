...
extern void get_phyif_device_stats(struct net_device *netdev);
...
#endif				/* BAND_STEERING */
	DECLARE_LOCK(probeRspIEsLock);	/* used to protect off band steering SKB list */
	DECLARE_LOCK(offChanScanLock);	/* used to protect off chan req list */
};
...
struct wlpriv_net_device_stats {
	...
	time64_t ch_start_time_sec;
	u_int32_t ch_change_count;
};
...
typedef struct {
	...
	UINT16 BufSizeInReq;	/* Record AddBaReq.BufSize */
} Ampdu_tx_t;
...
typedef struct _DRV_RATE_HIST {
	...
	u32 VHtRates[QS_NUM_SUPPORTED_11AC_NSS_BIG][QS_NUM_SUPPORTED_11AC_BW][QS_NUM_SUPPORTED_GI][QS_NUM_SUPPORTED_11AC_MCS];
	u32 HERates[QS_NUM_SUPPORTED_11AX_NSS][QS_NUM_SUPPORTED_11AX_BW][QS_NUM_SUPPORTED_11AX_GILTF_EXT][QS_NUM_SUPPORTED_11AX_MCS];
} DRV_RATE_HIST;
#endif				//defined(ACNT_REC) && defined (SOC_W906X)
...
/* debug for invalid skb */
	...
	ktime_t offchan_ts;
	UINT32 offchan_sum_pktcnt;
	UINT32 offchan_avg_pktcnt;
	offChanListItem *offchan_current_req_p;
	struct sk_buff_head RxDropMgmtSkb;
	U32 RxDropMgmtCnt;
	dcs_data_t dcs_db[IEEE_80211_MAX_NUMBER_OF_CHANNELS];
	BOOLEAN wlmgr_dcs;
	U32 smdata_offset;
	struct completion wlreset_complete;
	struct work_struct offchan_scan_mgt;
	struct offchan_scan_t offchan_scan;
	struct nb_info_t nb_info;
	UINT8 dfs_ctl_chlist[IEEE_80211_MAX_NUMBER_OF_CHANNELS];
	UINT16 dfs_ctl_ch_index;
};
...
struct wlprivate {
	...
	struct net_device_stats phyif_netDevStats;	/* phyIf net_device statistics */
	struct platform_device *pDev;
	...
	UINT32 txqBasePtr;
	UINT32 staBasePtr;
	cbinfo_bss_t cbinfo_bss;
	cbcallbk_intf cb_callbk_func;
	struct timer_list bnc_timer;	/* timer tick for Timer.c     */
	UINT8 cust_ie[64];
	UINT8 custie_len;
	struct timer_list cb_mctmer;	// timer to mcast checking
	cb_mcinfo_t cbmc_info;
	struct sk_buff_head cb_ncpkt_q;
	bool cb_mc_wait2sync;
	spinlock_t cbmc_lock;
	struct timer_list cb_mctesttmer;	// timer to mcast testing checking
	uint16_t mctest_cnt;
	uint16_t mctest_sngap;
	UINT32 bcn_drop_cnt;
	bool bcn_is_on;
	uint16_t txbcn_sn;
#endif				//CB_SUPPORT
...
extern void wl_util_free_skb(struct sk_buff *skb, const char *func, const int line);
extern void wl_util_receive_skb(struct sk_buff *skb, const char *func, const int line);
...
extern void wlOffChannelItemFree(offChanListItem * offchan_current_req_p);
#endif
...
extern void offChanDoneHdlr(struct net_device *netdev, offchan_status next_state);
...
extern int nf_weight;
extern unsigned int chld_nf_delta;
extern unsigned int nf_score_delta;
