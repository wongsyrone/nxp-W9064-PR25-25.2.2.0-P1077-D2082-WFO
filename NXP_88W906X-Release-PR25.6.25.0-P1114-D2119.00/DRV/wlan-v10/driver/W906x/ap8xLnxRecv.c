...
#else
extern int monif_handle_recv(struct wlprivate *wlpptr, wlrxdesc_t * pCurrent, struct sk_buff *skb);
#endif
#endif
extern void wlResetCfhUl(wlrxdesc_t * cfh_ul);
extern void reset_deauth_block_timer(UINT8 * data);

static void trace_func_delta_begin(vmacApInfo_t * vmacSta_p)
{
...
static void trace_func_delta_end(vmacApInfo_t * vmacSta_p)
{
	RxTimeCntStat *RxInfo = &(vmacSta_p->Rx_StatInfo);
	ktime_t delta_t1;
	SINT32 delta_us;
...
int wl_bad_mic_handler(struct wlprivate *wlpptr, UINT8 isUcast)
{
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;

	if ((vmacSta_p->OpMode == WL_OP_MODE_VSTA) || (vmacSta_p->OpMode == WL_OP_MODE_STA)) {
		vmacEntry_t *vmacEntry_p = vmacGetVMacEntryById(parentGetVMacId(vmacSta_p->VMacEntry.phyHwMacIndx));
...
extern U8 *get_cfhul_template(struct sk_buff *skb, wlrxdesc_t * pCfhul);

UINT32 wl_proc_mic_defrag(struct net_device *netdev, struct except_cnt *wlexcept_p, wlrxdesc_t * pCfhul, struct sk_buff **pRxSkBuff)
{
		...
		if (pCfhul->icv_err) {
			...
			if (pCfhul->euMode == SHAL_EU_MODE_TKIP) {
				...
				if ((*pRxSkBuff)->data[0] == (*pRxSkBuff)->len) {
					UINT8 isUcast = !IS_GROUP((UINT8 *) & (*pRxSkBuff)->data[0]);
					/* mwl_hex_dump((*pRxSkBuff)->data, (*pRxSkBuff)->len); */

					pStaInfo =
					    extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) & (*pRxSkBuff)->data[14], STADB_SKIP_MATCH_VAP);
					if (pStaInfo) {
						vmacApInfo_t *vmactem_p = NULL;
						vmactem_p = vmacGetMBssByAddr(vmacSta_p, pStaInfo->Bssid);
						if (vmactem_p) {
							wlpptr = NETDEV_PRIV_P(struct wlprivate, vmactem_p->dev);
						}
					}
	...
	if (frame_ctlp->MoreFrag || (pCfhul->hdr.seqNum & 0x000F)) {
		...
		if (*(mib->mib_STAMode))	//STA mode
		{
			pStaInfo = extStaDb_GetStaInfo(vmacSta_p,
						       (IEEEtypes_MacAddr_t *) GetParentStaBSSID(vmacSta_p->VMacEntry.phyHwMacIndx),
...
#endif
		{
			if (frame_ctlp->Type == IEEE_TYPE_DATA)
				pStaInfo = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) & (*pRxSkBuff)->data[6], STADB_SKIP_MATCH_VAP);
			else
				pStaInfo = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) & (*pRxSkBuff)->data[10], STADB_SKIP_MATCH_VAP);
		}
		if (pStaInfo) {
			if (((pCfhul->hdr.seqNum >> 4) != (pStaInfo->seqNum >> 4)) && pStaInfo->pDefragSkBuff) {
			...
			cfhul_template = get_cfhul_template(*pRxSkBuff, pCfhul);
			if (frame_ctlp->Wep) {
				if ((pCfhul->fpkt == 1) && cfhul_template)
					pn = (UINT8 *) cfhul_template + SMAC_CFHUL_MAC_HDR_OFFSET + pCfhul->macHdrLen;
			}
			...
			if ((pCfhul->hdr.seqNum & 0x000F) == 0)	//First frag packet
			{
				...
				*pRxSkBuff = NULL;
				if (pn != NULL)
					pStaInfo->DeFragBufInfo.pn0_no = pn[0];
				return 1;	//continue;                                    
			} else {
				UINT32 leftLen;
				...
				if ((pStaInfo->pDefragSkBuff == NULL) || ((pn != NULL) && (pn[0] != (pStaInfo->DeFragBufInfo.pn0_no + 1)))
				    ) {
					wlexcept_p->cnt_defrag_drop++;
					...
					*pRxSkBuff = NULL;
					...
					pStaInfo->DeFragBufInfo.pn0_no = 0;
					return 1;	//continue;                                                           
				}

				leftLen = skb_tailroom(pStaInfo->pDefragSkBuff);

				if (leftLen > (pCfhul->cfh_offset + 16 + sizeof(wlrxdesc_t))) {
					//increase space 
					UINT8 *ptr = skb_tail_pointer(pStaInfo->pDefragSkBuff);
					...
					if (ptr != NULL) {
						...
						pktLen = ((*pRxSkBuff)->len - offset);

						skb_put(pStaInfo->pDefragSkBuff, pktLen);
						memcpy(ptr, &(*pRxSkBuff)->data[offset], pktLen);

						if (frame_ctlp->Type == IEEE_TYPE_DATA) {
							len = SHORT_SWAP(*((UINT16 *) & pStaInfo->pDefragSkBuff->data[12])) + pktLen;
							*((UINT16 *) & pStaInfo->pDefragSkBuff->data[12]) = SHORT_SWAP(len);
						}
					} else {
						wlexcept_p->cnt_defrag_drop++;
						wlexcept_p->cnt_defrag_drop_x[4]++;
						wl_free_skb(*pRxSkBuff);
						*pRxSkBuff = NULL;
				...
				if (frame_ctlp->MoreFrag == 0)	//Last frag packet
				{
					...
					new_cfh_offset = (pStaInfo->pDefragSkBuff->len + 16 + 0xF) & ~0xF;

					if (frame_ctlp->Type == IEEE_TYPE_MANAGEMENT)
						tempOffset = SMAC_MGMT_EXTRA_BYTE;

					cur_cfh_offset = pCfhul->cfh_offset - tempOffset;

					pTmpCfhul = (wlrxdesc_t *) & (*pRxSkBuff)->data[cur_cfh_offset];
					pTmpCfhul->cfh_offset = new_cfh_offset;
					pCfhul->cfh_offset = pTmpCfhul->cfh_offset;

					memcpy(&pStaInfo->pDefragSkBuff->data[new_cfh_offset - tempOffset],
					       &(*pRxSkBuff)->data[cur_cfh_offset], sizeof(wlrxdesc_t));
...
void iTWT_monitor_Rx_start(struct wlprivate *wlpptr, u32 pktlen)
{
	...
	pfwFlag = (u32 *) (wlpptr->ioBase0 + (wfa_twt_rxing_flag_addr - DMEM_BASE));
#endif
		...
		case 2:
			if (diff < wfa_twt_rx_mon_time) {	//500ms
				...
				*pfwFlag = 1;
				...
				cfg[0] = (TWT_WFA_RXMODE | TWT_WFA_CODEWORD << 16 | ((++itwtswcnt) << 24));
				wlFwGetAddrValue(((wlpptr->master) ? wlpptr->master : wlpptr->netDev), wfa_twt_rxing_flag_addr, 4, cfg, 1);
...
#if 1
					memset(cfg, 0, sizeof(cfg));
					cfg[0] = (TWT_WFA_NONRXMODE | TWT_WFA_CODEWORD << 16 | ((++itwtswcnt) << 24));
					if (!wlFwGetAddrValue
					    (((wlpptr->master) ? wlpptr->master : wlpptr->netDev), wfa_twt_rxing_flag_addr, 4, cfg, 1)) {
...
//#endif
	...
	if (wlpptr->wlpd_p->bfwreset && !wlpptr->wlpd_p->bpreresetdone) {
		WLDBG_DATA(DBG_LEVEL_3, "%s(), (bfwreset, bpreresetdone)=(%d, %d)\n", __func__,
			   wlpptr->wlpd_p->bfwreset, wlpptr->wlpd_p->bpreresetdone);
		return;
	}
	...
	for_each_set_bit(qid, (unsigned long *)&RxQs, SC5_RXQ_NUM) {
		...
		while ((wrinx != wlqm->sq.rdinx)
			...
			msduNo = 0;
			cfh_ul_amsdu = wlProcessMsdu(netdev, cfh_ul, &msduNo, qid);
			...
			do {
				...
				if (cfh_ul_amsdu) {
					...
					pRxSkBuff = wlCfhUlToSkb(netdev, &cfh_ul_amsdu[idx++], qid);
#ifdef MULTI_AP_SUPPORT
					if (pCurCfhul && (pCurCfhul->fpkt == 1)) {
						if (pRxSkBuff && (pRxSkBuff->protocol & WL_WLAN_TYPE_RX_FAST_DATA)) {
							//cfhul_template = pRxSkBuff->data + pCurCfhul->cfh_offset;

							mac_hdr =
							    (IEEEtypes_fullHdr_t *) (pRxSkBuff->data + pCurCfhul->cfh_offset + SMAC_MAC_HDR_OFFSET);
							memcpy(&wlpd_p->mac_addr_sta_ta, &mac_hdr->Addr2, IEEEtypes_ADDRESS_SIZE);
						}
					}
#endif
				} else {
				...
				if (pCurCfhul)
					frame_ctlp = (IEEEtypes_FrameCtl_t *) & pCurCfhul->frame_ctrl;
				else {
...
#endif
				if ((frame_ctlp->Type != IEEE_TYPE_CONTROL) && wl_proc_mic_defrag(netdev, wlexcept_p, pCurCfhul, &pRxSkBuff)) {
					continue;
				}

				vmacSta_p->Rx_StatInfo.RxMSDUCnt++;
				LMFbit = (pCurCfhul->lpkt << 2) | ((~(pCurCfhul->lpkt | pCurCfhul->fpkt) & 0x1) << 1) | pCurCfhul->fpkt;
				if (pRxSkBuff == NULL) {
					wlCfhUlDump(pCurCfhul);
					WLDBG_WARNING(DBG_LEVEL_0, "ERROR: RxQ SQ(%d) rdinx %x wrinx %x, skb is NULL\n",
				...
				// rx_info processing
				wlrxinfo_notify_new_msdu(netdev, pCurCfhul, qid, pRxSkBuff);
				...
				WLDBG_DATA(DBG_LEVEL_3, "dump [%u]th amsdu-subframe: len=%d\n", ((idx) ? (idx - 1) : idx), pRxSkBuff->len);
				WLDBG_HEXDUMP(DBG_LEVEL_3, pRxSkBuff->data, pRxSkBuff->len);
				...
				//add stnid
				if (qid == 0) {
					if ((frame_ctlp->FromDs == 0) && (frame_ctlp->ToDs == 0)) {
						wl_free_skb(pRxSkBuff);
						pRxSkBuff = NULL;
						wlexcept_p->msdu_err++;
						continue;
					} else if ((frame_ctlp->FromDs == 0) && (frame_ctlp->ToDs == 1)) {
						...
						if ((StaInfo_p = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) & wlpd_p->mac_addr_sta_ta,
					...
					} else if ((frame_ctlp->FromDs == 1) && (frame_ctlp->ToDs == 0)) {
						stnid = rxring_Ctrl_STAfromDS;
						pRxSkBuff->protocol |= WL_WLAN_TYPE_STA;
					} else if ((frame_ctlp->FromDs == 1) && (frame_ctlp->ToDs == 1)) {
						stnid = rxring_Ctrl_STAfromDS;
				...
				if (qid == SC5_RXQ_PROMISCUOUS_INDEX) {
					...
					if (((wlpptr->vdev[wlpptr->wlpd_p->MonIfIndex] != NULL) &&
					     (wlpptr->vdev[wlpptr->wlpd_p->MonIfIndex]->flags & IFF_RUNNING))) {
						//memset((void *)&rx_info, 0 ,sizeof(rx_info));
						if (pRxSkBuff != NULL) {
							//*(generic_buf**)pRxSkBuff->cb = pgbuf;
							//monif_handle_recv(wlpptr, &rx_info, pRxSkBuff, 1);
							monif_handle_recv(wlpptr, &wlpd_p->rxinfo_aux_poll[pCurCfhul->hdr.rxInfoIndex], pRxSkBuff, 1);
						}
					} else
#endif				/* ENABLE_MONIF */
					{
						struct ieee80211_frame *wh = (struct ieee80211_frame *)pRxSkBuff->data;
						struct IEEEtypes_Frame_t *wlanMsg_p = (IEEEtypes_Frame_t *) ((UINT8 *) pRxSkBuff->data - 2);
						...
						if (wh->FrmCtl.Type == IEEE_TYPE_DATA) {
							if (wlpptr->offchan_state == OFFCHAN_IDLE) {
								...
								if (vmactem_p && (vmactem_p->dev->flags & IFF_RUNNING)) {
									pStaInfo =
									    extStaDb_GetStaInfo(vmactem_p, (IEEEtypes_MacAddr_t *) (wh->addr2),
								...
								if (*(vmacSta_p->Mib802dot11->mib_unassocsta_track_enabled))
									MSAN_unassocsta_recv_proc(vmacSta_p, wlanMsg_p, pCurCfhul->hdr.rssi);
							}
							...
							if (wlanMsg_p->Hdr.FrmCtl.Subtype == IEEE_MSG_BEACON)
								MSAN_neighbor_bcnproc(netdev, wlanMsg_p, pRxSkBuff->len, &wlpd_p->rssi_path_info,
										      SCAN_BY_OFFCHAN);
#ifdef MULTI_AP_SUPPORT
							else if (wlanMsg_p->Hdr.FrmCtl.Subtype == IEEE_MSG_PROBE_RQST) {
								if (*(vmacSta_p->Mib802dot11->mib_unassocsta_track_enabled))
									MSAN_unassocsta_recv_proc(vmacSta_p, wlanMsg_p, pCurCfhul->hdr.rssi);
							}
#endif				/* MULTI_AP_SUPPORT */
					...
					work_done +=
					    ieee80211_input(netdev, pRxSkBuff, pCurCfhul->hdr.rssi, &wlpd_p->rssi_path_info, pCurCfhul,
							    (idx == 0 ? TRUE : FALSE), stnid, LMFbit);
				}
...
void wl_show_recv_info(struct net_device *netdev, char *sysfs_buff)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	...
	Sysfs_Printk(" | 0 ~ 5 ms | 5 ~ 10ms | 10 ~ 100ms | 100 ~ 500ms | 500 ~ 1s | 1s ~     |\n");
	Sysfs_Printk(" | %8d | %8d |   %8d |    %8d | %8d | %8d |\n",
		     RxInfo->RxTaskCnt_t[0], RxInfo->RxTaskCnt_t[1], RxInfo->RxTaskCnt_t[2], RxInfo->RxTaskCnt_t[3], RxInfo->RxTaskCnt_t[4],
		     RxInfo->RxTaskCnt_t[5]);

	Sysfs_Printk("\nProcess Time of Rx Tasklet:\n");
	Sysfs_Printk(" | 0 ~ 50us | 50 ~ 100us | 100 ~ 300us | 300 ~ 500us | 500us ~  | max time    |\n");
	Sysfs_Printk(" | %8d |   %8d |    %8d |    %8d | %8d | %8d us |\n",
		     RxInfo->RxProcCnt_t[0], RxInfo->RxProcCnt_t[1], RxInfo->RxProcCnt_t[2], RxInfo->RxProcCnt_t[3], RxInfo->RxProcCnt_t[4],
		     RxInfo->RxProcCnt_t[5]);

	Sysfs_Printk("\nReveive Queue Count:\n");
	for (i = 0; i < 10; i++) {
		Sysfs_Printk("~ %3d pkts: %8d\n", (i + 1) * 10, RxInfo->RxQuStatCnt[i]);
	}
	Sysfs_Printk("> %3d pkts: %8d\n", i * 10, RxInfo->RxQuStatCnt[i]);

	Sysfs_Printk("\nReveive MSDU pkts Count:\n");
	for (i = 0; i < 10; i++) {
		Sysfs_Printk("~ %3d pkts: %8d\n", (i + 1) * 10, RxInfo->RxMSDUStatCnt[i]);
	}
