...
#define SMAC_STATUS_NUM 12	// SMAC status length:  volatile U32  smacSts[12];  ///<  0x500: SMAC status
int wlmon_kthread(void *arg);

int wlmon_bmq_refill_Info_dump(struct net_device *netdev, u8 * plog, U32 maxlen);
static int sprintSmacStatus(u8 * pdest, SMAC_STATUS_st * pSMACStatus, UINT32 maxlen, UINT32 format);
int wlmon_rx_stuck_detct(struct net_device *netdev, SMAC_STATUS_st * pCurSmacStatus, void **buf);
int wlmon_tx_stuck_detct(struct net_device *netdev, SMAC_STATUS_st * pCurSmacStatus, void **buf);
...
int wlmon_beacon_stuck_detct(struct net_device *netdev, SMAC_STATUS_st * pCurSmacStatus, void **buf);
void wlmon_log_bmq_buff_refill(struct net_device *netdev, U32 qid, U32 refill_cnt);
...
u32 wlmon_read_hw_registers(struct wlprivate *wlpptr, u32 start, u32 len, u32 * pbuf);
...
extern u64 dump_file(UINT8 * valbuf, UINT32 length, UINT8 * fname, UINT32 append);
...
static int hm_reboot_handler(struct notifier_block *nb, unsigned long state, void *cmd)
{
...
static void hm_reboot_nofifier_register(int action, struct notifier_block *nb)
{
...
static int ap8x_mci_bus_handler(struct notifier_block *self, unsigned long val, void *port)
{
	switch (val) {
	case MCI_CRC_ERR:
	case MCI_SEQ_ERR:
...
static void wlmon_ext_trigger_init(struct wlprivate *wlpptr)
{
	u32 val;
	unsigned char tlvData[MAX_TLV_LEN];
	char buff[120];

	dev_info(wlpptr->wlpd_p->dev, "Set gpio trigger mode to %ul", hm_gpio_trigger);
	...
	case 3:
		...
		break;
	default:
		dev_err(wlpptr->wlpd_p->dev, "unsupported gpio trigger mode %ul", hm_gpio_trigger);
	}
	return;
}

void wlmon_ext_trigger_assert(struct net_device *netdev)
{
	u32 val;
...
void wlmon_ext_trigger_deassert(struct wlprivate *wlpptr)
{
	u32 val;
...
static void wlmon_ext_trigger_release(struct wlprivate *wlpptr)
{
	u32 val;
...
int register_wlmon_notifier(void *wlpd)
{
	struct wlprivate_data *wlpd_p = (struct wlprivate_data *)wlpd;
	struct notifier_block *nb = 0;

	if (wlpd_p->smon.nb == NULL) {
		nb = (struct notifier_block *)wl_kmalloc(sizeof(struct notifier_block), GFP_ATOMIC);
		if (!nb) {
	...
	return blocking_notifier_chain_register(&wlpd_p->smon.wlmon_notifier_list, nb);
}

int unregister_wlmon_notifier(void *wlpd)
{
	struct wlprivate_data *wlpd_p = (struct wlprivate_data *)wlpd;
	...
	if (nb) {
		rc = blocking_notifier_chain_unregister(&wlpd_p->smon.wlmon_notifier_list, nb);
		if (rc == 0) {
			wlpd_p->smon.nb = NULL;
			wl_kfree(nb);
			return 0;
...
static int wlmon_smac_detect(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	u32 *smacL;
	u8 *stuckCnt;
	u32 smac_cur[SMAC_STATUS_NUM];
	int stuck_detected = 0;
	u32 i;

	if (wlpd_p->smon.active == 0 || !(wlpd_p->smon.ActiveBitmap & MON_SMAC_STUCK))
		return 0;
	...
	memcpy((void *)&smac_cur[0], (void *)&wlpptr->smacStatusAddr->smacSts[0], sizeof(smac_cur));
	...
	if (stuck_detected) {
		printk(" macsts:%X %X %X %X %X %X %X %X %X %X %X %X\n", smacL[0], smacL[1], smacL[2], smacL[3], smacL[4], smacL[5], smacL[6],
		       smac_cur[7], smac_cur[8], smac_cur[9], smac_cur[10], smac_cur[11]);
		printk("%s MAC CM3_x stuck cnt:%u %u %u %u %u %u %u\n", wlpd_p->rootdev->name, stuckCnt[0], stuckCnt[1], stuckCnt[2], stuckCnt[3],
		       stuckCnt[4], stuckCnt[5], stuckCnt[6]);
		return 1;
	}
...
static int wlmon_bmq_resouce_detct(struct net_device *netdev, void **buf)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	...
	if (wlpd_p->smon.active == 0 || !(wlpd_p->smon.ActiveBitmap & MON_DRV_BMQ_RESOURCE))
		return 0;

	if ((result = wlDbg_chk_bm_enq(netdev, hm_max_bmq_diff))) {

		if ((plog = (void *)wl_kmalloc(WLMON_PRNT_BUF_SIZE, GFP_ATOMIC)) == NULL) {
		...
		initidx = sprintf(&plog[idx], "[%llu.%llu] Buffer resources warning:\n", ts_sec, ts_ms);
		idx = initidx;
		...
		for (i = SC5_BMQ_START_INDEX; i < SC5_BMQ_START_INDEX + SC5_BMQ_NUM; i++) {
			...
			idx += sprintf(&plog[idx], "\t Q[%d] = %d, (eq=%d, drop=%d, ret=%d)\n", i,
				       (wldrvstat_p->enq_bmqbuf_cnt[offset] - wldrvstat_p->xx_buf_free_SQ14[offset] -
					wldrvstat_p->bmqbuf_ret_cnt[offset]), wldrvstat_p->enq_bmqbuf_cnt[offset],
				       wldrvstat_p->xx_buf_free_SQ14[offset], wldrvstat_p->bmqbuf_ret_cnt[offset]);
		}

		wlmon_bmq_refill_Info_dump(netdev, &plog[idx], WLMON_PRNT_BUF_SIZE - idx);
...
static int wlmon_error_cnt_detect(struct net_device *netdev, void **buf)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	...
	if (wlpd_p->smon.active == 0 || !(wlpd_p->smon.ActiveBitmap & MON_DRV_ERR_CNTS))
		return 0;
	...
	if (pnew->cnt_cfhul_invalid_signature != pold->cnt_cfhul_invalid_signature) {
		idx += sprintf(&plog[idx], "cnt_cfhul_invalid_signature:%u\n", pnew->cnt_cfhul_invalid_signature);
		pold->cnt_cfhul_invalid_signature = pnew->cnt_cfhul_invalid_signature;
	}
	...
	if (pnew->cnt_invalid_amsdu_subframe_len != pold->cnt_invalid_amsdu_subframe_len) {
		idx += sprintf(&plog[idx], "cnt_invalid_amsdu_subframe_len:%u\n", pnew->cnt_invalid_amsdu_subframe_len);
		pold->cnt_invalid_amsdu_subframe_len = pnew->cnt_invalid_amsdu_subframe_len;
	...
	for (i = 0; i < SC5_BMQ_NUM; i++) {
		if (pnew->rx_invalid_sig_cnt[i] != pold->rx_invalid_sig_cnt[i]) {
			idx += sprintf(&plog[idx], "rx_invalid_sig_cnt[%u]:%u\n", i, pnew->rx_invalid_sig_cnt[i]);
			pold->rx_invalid_sig_cnt[i] = pnew->rx_invalid_sig_cnt[i];
	...
	if (pnew->cfhul_flpkt_lost[0] != pold->cfhul_flpkt_lost[0] ||
	    pnew->cfhul_flpkt_lost[1] != pold->cfhul_flpkt_lost[1] || pnew->cfhul_flpkt_lost[2] != pold->cfhul_flpkt_lost[2]) {
		idx +=
		    sprintf(&plog[idx], "Lost AMSDU subframes (fpkt,midle,lpkt)=%u,%u,%u\n", pnew->cfhul_flpkt_lost[0], pnew->cfhul_flpkt_lost[1],
	...
	if (pnew->cfhul_flpkt_lost[3] != pold->cfhul_flpkt_lost[3]) {
		idx += sprintf(&plog[idx], "AMSDU subframe number over limit:%u\n", pnew->cfhul_flpkt_lost[3]);
	...
	if (pnew->badAcntStnid != pold->badAcntStnid) {
		idx += sprintf(&plog[idx], "Invalid STN ID in Acnt Record cnt:%u\n", pnew->badAcntStnid);
...
void wlmon_log_buffer(struct net_device *netdev, UINT8 * buf, UINT32 len)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	...
	if (wlpd_p->smon.active == 0 || !(wlpd_p->smon.ActiveBitmap & MON_DRV_CMD))
		return;
...
static int wlmon_log_cmd(struct net_device *netdev, void **buf)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	char *pcmdbuf = (char *)wlpd_p->smon.piocmdlog;
...
static int wlmon_temperature_check(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	u64 ts;
	...
	if (wlpd_p->smon.ActiveBitmap & MON_TEMPERATURE) {
		...
		if (wlFwGetRadioStatus(netdev, &tmp_status) == SUCCESS) {
			...
			temperature_radio = (tmp_status.temperature * 4935 - 2271500) / 10000;
			if (temperature_radio > wlpd_p->smon.temperature_threshold) {
		...
		if (wlmon_host_temperature_get(&temp) == SUCCESS) {
			...
			wlpd_p->smon.thm_chanload_idx = (idx + 1) % MAX_SMACSTATUS_LOG_ENTRY;
			wlpd_p->smon.thm_chanload[idx].timestamp = ts;
			wlpd_p->smon.thm_chanload[idx].chan_load = tmp_status.total_load;
			wlpd_p->smon.thm_chanload[idx].host_temp = temp;
			wlpd_p->smon.thm_chanload[idx].radio_temp = temperature_radio;
		} else {
...
static void wlmon_collect_active_interfaces(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	UINT32 idx;
	...
	for (idx = 0; idx <= wlpd_p->NumOfAPs; idx++) {
		...
		if (wlpptr->vdev[idx] != NULL && (wlpptr->vdev[idx]->flags & IFF_RUNNING) && (wlpptr->vdev[idx]->flags & IFF_UP)) {
			struct net_device *vdev = wlpptr->vdev[idx];
			struct wlprivate *wlp = NETDEV_PRIV(struct wlprivate, vdev);
...
static int sprintSmacStatus(u8 * pdest, SMAC_STATUS_st * pSMACStatus, UINT32 maxlen, UINT32 format)
{
	...
	if (format & SMAC_STATUS_FORMAT_TXT) {
		...
		for (i = 0, k = 0; i < 1; i++) {
			idx += sprintf(&pdest[idx], "%08x: %02x%02x%02x%02x %02x%02x%02x%02x \n", (i * 16),
				       p->smacRdy[k], p->smacRdy[k + 1], p->smacRdy[k + 2], p->smacRdy[k + 3],
				       p->smacRdy[k + 4], p->smacRdy[k + 5], p->smacRdy[k + 6], p->smacRdy[k + 7]);
		}
		...
		for (i = 0, k = 0; i < 1; i++, k += 4) {
			idx +=
			    sprintf(&pdest[idx], "%08x: %08x %08x %08x %08x\n", (i * 16), p->verCtrl[k], p->verCtrl[k + 1], p->verCtrl[k + 2],
				    p->verCtrl[k + 3]);
		}

		idx += sprintf(&pdest[idx], " 0x420: txInputCnt %u\ttxSchedCnt %u\ttxProcCnt %u\ttxBufRetCnt %u\n",
			       p->txInputCnt, p->txSchedCnt, p->txProcCnt, p->txBufRetCnt);

		idx += sprintf(&pdest[idx], " 0x430: txAcRingCnt %u\ttxCtlFrmCnt %u\ttxEuDoneCnt %u\ttxRdyDeassert %u\n",
			       p->txAcRingCnt, p->txCtlFrmCnt, p->txEuDoneCnt, p->txRdyDeassert);

		idx += sprintf(&pdest[idx], " 0x440: sop_EvtMacHdr %u\teop_EvtEuDone %u\tfcs_EvtFcs %u\teop2_Q2RxAmsdu %u\n",
			       p->sop_EvtMacHdr, p->eop_EvtEuDone, p->fcs_EvtFcs, p->eop2_Q2RxAmsdu);

		idx += sprintf(&pdest[idx], " 0x450: sop_EuPrgm %u\teopDrp_EuErr %u\tfcsDrp_FcsErr %u\teop2Drp_Cnt %u\n",
			       p->sop_EuPrgm, p->eopDrp_EuErr, p->fcsDrp_FcsErr, p->eop2Drp_Cnt);

		idx += sprintf(&pdest[idx], " 0x460: uniPktCnt %u\tmultiPktCnt %u\tbman_GetBuf %u\tbman_RetBuf %u\n",
			       p->fcs_UniMCast[0], p->fcs_UniMCast[1], p->bman_GetBuf, p->bman_RetBuf);

		idx += sprintf(&pdest[idx], " 0x470: slotTickCnt %u\tfirstSlotTickCnt %u\ttxMgmPktCnt %u\ttxBcnCnt %u\n",
			       p->slotTickCnt, p->firstSlotTickCnt, p->txMgmPktCnt, p->txBcnCnt);

		idx += sprintf(&pdest[idx], " 0x480: debug log1: %08x %08x\n", p->sysRsvd1[0], p->sysRsvd1[1]);
		idx += sprintf(&pdest[idx], " 0x488: navBlockCnt %u\n", p->navBlockCnt);
		idx += sprintf(&pdest[idx], " 0x48C: txBcnAbort %u\n", p->txBcnAbort);

		idx += sprintf(&pdest[idx], " 0x490: TXD1 counts:\n");
		for (i = 0, k = 0; i < 1; i++, k += 4) {
			idx +=
			    sprintf(&pdest[idx], "%08x: %08x %08x %08x %08x\n", (i * 16), p->sysRsvd2[k], p->sysRsvd2[k + 1], p->sysRsvd2[k + 2],
				    p->sysRsvd2[k + 3]);
		}

		idx += sprintf(&pdest[idx], " 0x4a0: PGM %u, DONE %u, MSDU %u, MPDU %u\n",
			       p->sysRsvd3[0], p->sysRsvd3[1], p->sysRsvd3[2], p->sysRsvd3[3]);

		idx += sprintf(&pdest[idx], " 0x4b0: TXD5 counts:\n");
		for (i = 0, k = 0; i < 1; i++, k += 4) {
			idx +=
			    sprintf(&pdest[idx], "%08x: %08x %08x %08x %08x\n", (i * 16), p->sysRsvd4[k], p->sysRsvd4[k + 1], p->sysRsvd4[k + 2],
				    p->sysRsvd4[k + 3]);
		}
		...
		for (i = 0, k = 0; i < 1; i++, k += 4) {
			idx +=
			    sprintf(&pdest[idx], "%08x: %08x %08x %08x %08x\n", (i * 16), p->sysRsvd5[k], p->sysRsvd5[k + 1], p->sysRsvd5[k + 2],
				    p->sysRsvd5[k + 3]);
		}
		...
		for (i = 0, k = 0; i < 3; i++, k += 4) {
			idx +=
			    sprintf(&pdest[idx], "%08x: %08x %08x %08x %08x\n", (i * 16), p->sysRsvd6[k], p->sysRsvd6[k + 1], p->sysRsvd6[k + 2],
				    p->sysRsvd6[k + 3]);
		}
		...
		for (i = 0, k = 0; i < 3; i++, k += 4) {
			idx +=
			    sprintf(&pdest[idx], "%08x: %08x %08x %08x %08x\n", (i * 16), p->smacSts[k], p->smacSts[k + 1], p->smacSts[k + 2],
				    p->smacSts[k + 3]);
		}
		idx +=
		    sprintf(&pdest[idx], "%08x: %08x %08x %08x %08x\n", (i * 16), p->sysRsvd7[0], p->sysRsvd7[1], p->txAcntNoADMA, p->rxAcntNoADMA);

		idx += sprintf(&pdest[idx], " 0x540: sopDrp_GiantPkt %u, txStopAck %u, lastTxInfoErr %u, bmanErr_GetBuf %u,%u,%u,%u\n",
			       p->sopDrp_GiantPkt, p->txStopAck, p->lastTxInfoErr, p->bmanErr_GetBuf[0], p->bmanErr_GetBuf[1], p->bmanErr_GetBuf[2],
			       p->bmanErr_GetBuf[3]);

		idx += sprintf(&pdest[idx], " 0x550: eopDrp_EmptyBuf %u\n", p->eopDrp_EmptyBuf);

		idx += sprintf(&pdest[idx], " 0x560: txDataTxMsduCnt %u, txDataBufRetMsduCnt %u, txMgtTxMsduCnt %u, txMgtBufRetMsduCnt %u\n",
			       p->txDataTxMsduCnt, p->txDataBufRetMsduCnt, p->txMgtTxMsduCnt, p->txMgtBufRetMsduCnt);

		idx += sprintf(&pdest[idx], " 0x570: bman_StsReqBp %u, maxSizeBcnbuf %u, rxSBinfoBaseAddr %xh, rxSBinfoUnitSize %u\n",
			       p->bman_StsReqBp, p->maxSizeBcnbuf, p->rxSBinfoBaseAddr, p->rxSBinfoUnitSize);
		idx += sprintf(&pdest[idx], " 0x580: sysRsvd9:\n");
		for (i = 0, k = 0; i < 4; i++, k += 4) {
			idx +=
			    sprintf(&pdest[idx], "%08x: %08x %08x %08x %08x\n", (i * 16), p->sysRsvd9[k], p->sysRsvd9[k + 1], p->sysRsvd9[k + 2],
				    p->sysRsvd9[k + 3]);
		}
		...
		for (i = 0, k = 0; i < 2; i++, k += 4) {
			idx +=
			    sprintf(&pdest[idx], "%08x: %08x %08x %08x %08x\n", (i * 16), p->sysRsvd10[k], p->sysRsvd10[k + 1], p->sysRsvd10[k + 2],
				    p->sysRsvd10[k + 3]);
		}
		idx += sprintf(&pdest[idx], "0x5E0: TStxPeRise %x, TStxRdyFall %x, TStxSum_H %x, TSsys_H %x, TStxSum %x\n",
			       p->TStxPeRise, p->TStxRdyFall, p->TStxSum_H, p->TSsys_H, p->TStxSum);

		idx += sprintf(&pdest[idx], "0x5F0: %08x %08x %08x %08x\n", p->sysRsvdBB[0], p->sysRsvdBB[1], p->sysRsvdBB[2], p->sysRsvdBB[3]);
		idx += sprintf(&pdest[idx], "0x600: %08x %08x %08x %08x\n", p->sysRsvdMU0[0], p->sysRsvdMU0[1], p->sysRsvdMU0[2], p->sysRsvdMU0[3]);

		idx += sprintf(&pdest[idx], " 0x610: sysRsvdMU:\n");
		for (i = 0, k = 0; i < 3; i++, k += 4) {
			idx +=
			    sprintf(&pdest[idx], "%08x: %08x %08x %08x %08x\n", (i * 16), p->sysRsvdMU[k], p->sysRsvdMU[k + 1], p->sysRsvdMU[k + 2],
				    p->sysRsvdMU[k + 3]);
		}
		...
		for (i = 0, k = 0; i < 4; i++, k += 4) {
			idx +=
			    sprintf(&pdest[idx], "%08x: %08x %08x %08x %08x\n", (i * 16), p->sysRsvd11[k], p->sysRsvd11[k + 1], p->sysRsvd11[k + 2],
				    p->sysRsvd11[k + 3]);
		}
		...
		for (i = 0, k = 0; i < 1; i++, k += 4) {
			idx +=
			    sprintf(&pdest[idx], "%08x: %08x %08x %08x %08x\n", (i * 16), p->sysRsvd12[k], p->sysRsvd12[k + 1], p->sysRsvd12[k + 2],
				    p->sysRsvd12[k + 3]);
		}

		idx += sprintf(&pdest[idx], " 0x690: CSI Information:\n");
		idx +=
		    sprintf(&pdest[idx], "CSI_Pkt_RAW_RSSI - AB: %d CD: %d EF: %d GH: %d\n", pSMACStatus->CSI_RSSI_AB, pSMACStatus->CSI_RSSI_CD,
			    pSMACStatus->CSI_RSSI_EF, pSMACStatus->CSI_RSSI_GH);
		idx +=
		    sprintf(&pdest[idx], "CSI_Pkt_MAC_Addr - %02x:%02x:%02x:%02x:%02x:%02x\n", pSMACStatus->CSI_Pkt_MAC_Addr[0],
			    pSMACStatus->CSI_Pkt_MAC_Addr[1], pSMACStatus->CSI_Pkt_MAC_Addr[2], pSMACStatus->CSI_Pkt_MAC_Addr[3],
			    pSMACStatus->CSI_Pkt_MAC_Addr[4], pSMACStatus->CSI_Pkt_MAC_Addr[5]);
		...
		idx += sprintf(&pdest[idx], "%08x %08x\n", p->cm3StartFlag, p->cm3StopFlag);

		idx += sprintf(&pdest[idx], " 0x6c0: txBcnCntBss:\n");
		for (i = 0, k = 0; i < 4; i++, k += 8) {
			idx += sprintf(&pdest[idx], "%08x: %04x %04x %04x %04x : %04x %04x %04x %04x\n", (i * 16),
				       p->txBcnCntBss[k], p->txBcnCntBss[k + 1], p->txBcnCntBss[k + 2],
				       p->txBcnCntBss[k + 3], p->txBcnCntBss[k + 4], p->txBcnCntBss[k + 5],
				       p->txBcnCntBss[k + 6], p->txBcnCntBss[k + 7]);
		}
		...
		for (i = 0, k = 0; i < (sizeof(p->rsvd) / 16); i++, k += 16) {
			idx += sprintf(&pdest[idx], "%08x: %02x%02x%02x%02x %02x%02x%02x%02x %02x%02x%02x%02x %02x%02x%02x%02x\n", (i * 16),
				       p->rsvd[k], p->rsvd[k + 1], p->rsvd[k + 2], p->rsvd[k + 3],
				       p->rsvd[k + 4], p->rsvd[k + 5], p->rsvd[k + 6], p->rsvd[k + 7],
				       p->rsvd[k + 8], p->rsvd[k + 9], p->rsvd[k + 10], p->rsvd[k + 11],
				       p->rsvd[k + 12], p->rsvd[k + 13], p->rsvd[k + 14], p->rsvd[k + 15]);
		}
	}
...
int IsBssBcnStuck(struct net_device *netdev, UINT32 * bssbitmap, U64 * maxstuckivl)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	...
	*maxstuckivl = 0;	//output max time interval of stuck BSSs
	...
	while (map) {
		...
		if (map & 0x1) {
			...
			if (pLast->txBcnCntBss[idx] && (pCur->txBcnCntBss[idx] == pLast->txBcnCntBss[idx]) && !wlpd_p->bStopBcnProbeResp && !wlpd_p->bCACTimerFired && ((tmp_status.rxload <= BCN_STUCK_MAX_RXLOAD) || (tmp_status.rxload > 100)) &&	// By AE report, sometimes bcn stuck and "rxload" is in abnormal value "rxload: 123672139 (percentages)".
			    (tmp_status.cca_block <= BCN_STUCK_CCA_BLOCK) && (tmp_status.nav_block <= BCN_STUCK_NAV_BLOCK)) {
				*bssbitmap |= (1 << idx);
				...
				if (wlpd_p->smon.bcnstucktimestamp[idx] == 0)	//handle corner case in case vap is just started.
					wlpd_p->smon.bcnstucktimestamp[idx] = tms;
				else {
					UINT64 diff = tms - wlpd_p->smon.bcnstucktimestamp[idx];
...
int wlmon_beacon_stuck_detct(struct net_device *netdev, SMAC_STATUS_st * pCurSmacStatus, void **buf)
{
	...
	if (wlpd_p->smon.active == 0 || !(wlpd_p->smon.ActiveBitmap & MON_SMAC_BCN_STUCK))
		return 0;

	if ((wlpptr->wlpd_p->inResetQ == WL_TRUE) ||
	    ((vmacSta_p->preautochannelfinished == 0) && (*(mib->mib_autochannel) != 0)) ||
	    vmacSta_p->busyScanning || (wlpptr->wlpd_p->bBssStartEnable == 0)) {
		/* healthmonitor should ignore beacon stuck detect during ACS scan or wlreset workqueue. */
		memset(wlpd_p->smon.bcnstucktimestamp, 0x0, sizeof(wlpd_p->smon.bcnstucktimestamp));
		return 0;
	}
	...
	if (wlpd_p->smon.ActiveIf & WL_OP_MODE_VAP) {
		...
		tdif = wlpd_p->smon.smacStsLogtime[1] - wlpd_p->smon.smacStsLogtime[0];
		if (((bssstuck = IsBssBcnStuck(netdev, &bssbitmap, &tdif)) &&	//same tx bss beacon cnt
		    ...
		    ((wlpd_p->offchan_scan.init_flag == TRUE) && (wlpd_p->offchan_scan.status_abnormal == TRUE))) {	// offchannel status abnormal from FW.

			if (wlpd_p->offchan_scan.status_abnormal == TRUE)
			...
			if (bssstuck) {
				idx += sprintf(&plog[idx], "[%llu.%llu] BSS Beacon stuck alarm: Stuck VAPs Bitmap:%08x \n", ts_sec, ts_ms, bssbitmap);
			} else {
				idx += sprintf(&plog[idx], "[%llu.%llu] Beacon stuck alarm: \n", ts_sec, ts_ms);
			}
...
int wlmon_tx_stuck_detct(struct net_device *netdev, SMAC_STATUS_st * pCurSmacStatus, void **buf)
{
	...
	if (wlpd_p->smon.active == 0 || !(wlpd_p->smon.ActiveBitmap & MON_SMAC_TX_STUCK))
		return 0;
	...
	if (wlpd_p->smon.ActiveIf) {
		...
		txdiff[TxPktIdx][0] =
		    (pCurSmacStatus->txInputCnt >=
		     pLast->txInputCnt) ? (pCurSmacStatus->txInputCnt - pLast->txInputCnt) : (pCurSmacStatus->txInputCnt + ~pLast->txInputCnt + 1);
		txdiff[TxPktIdx][1] =
		    (pCurSmacStatus->txAcRingCnt >=
		     pLast->txAcRingCnt) ? (pCurSmacStatus->txAcRingCnt - pLast->txAcRingCnt) : (pCurSmacStatus->txAcRingCnt + ~pLast->txAcRingCnt +
												 1);
		txdiff[TxPktIdx][2] =
		    (pCurSmacStatus->txSchedCnt >=
		     pLast->txSchedCnt) ? (pCurSmacStatus->txSchedCnt - pLast->txSchedCnt) : (pCurSmacStatus->txSchedCnt + ~pLast->txSchedCnt + 1);
		txdiff[TxPktIdx][3] =
		    (pCurSmacStatus->txProcCnt >=
		     pLast->txProcCnt) ? (pCurSmacStatus->txProcCnt - pLast->txProcCnt) : (pCurSmacStatus->txProcCnt + ~pLast->txProcCnt + 1);
		txdiff[TxPktIdx][4] =
		    (pCurSmacStatus->txEuDoneCnt >=
		     pLast->txEuDoneCnt) ? (pCurSmacStatus->txEuDoneCnt - pLast->txEuDoneCnt) : (pCurSmacStatus->txEuDoneCnt + ~pLast->txEuDoneCnt +
												 1);
		txdiff[TxPktIdx][5] =
		    (pCurSmacStatus->txBufRetCnt >=
		     pLast->txBufRetCnt) ? (pCurSmacStatus->txBufRetCnt - pLast->txBufRetCnt) : (pCurSmacStatus->txBufRetCnt + ~pLast->txBufRetCnt +
												 1);
		...
		if (txdiff[TxPktIdx][0]) {
			...
			for (i = 1; i < NUM_SFWTX_CHK_POINTS; i++) {
				...
				if (sum[i] == 0 && sum[0] > txdiff[TxPktIdx][0]) {
					...
					idx += sprintf(&plog[idx], "[%llu.%llu] Tx stuck alarm: \n", ts_sec, ts_ms);
			...
			if (wlpd_p->smon.txstuck_st) {
				...
				for (i = 0; i < NUM_SFWTX_CHK_POINTS && sum[i]; i++) ;
...
int wlmon_rx_stuck_detct(struct net_device *netdev, SMAC_STATUS_st * pCurSmacStatus, void **buf)
{
	...
	if (wlpd_p->smon.active == 0 || !(wlpd_p->smon.ActiveBitmap & MON_SMAC_RX_STUCK))
		return 0;
	...
	if (wlpd_p->smon.ActiveIf) {
		...
		rxdiff[RxPktIdx][0] =
		    (pCurSmacStatus->sop_EvtMacHdr >=
		     pLast->sop_EvtMacHdr) ? (pCurSmacStatus->sop_EvtMacHdr - pLast->sop_EvtMacHdr) : (pCurSmacStatus->sop_EvtMacHdr +
												       ~pLast->sop_EvtMacHdr + 1);
		rxdiff[RxPktIdx][1] =
		    (pCurSmacStatus->sop_EuPrgm >=
		     pLast->sop_EuPrgm) ? (pCurSmacStatus->sop_EuPrgm - pLast->sop_EuPrgm) : (pCurSmacStatus->sop_EuPrgm + ~pLast->sop_EuPrgm + 1);
		rxdiff[RxPktIdx][2] =
		    (pCurSmacStatus->eop_EvtEuDone >=
		     pLast->eop_EvtEuDone) ? (pCurSmacStatus->eop_EvtEuDone - pLast->eop_EvtEuDone) : (pCurSmacStatus->eop_EvtEuDone +
												       ~pLast->eop_EvtEuDone + 1);
		rxdiff[RxPktIdx][3] =
		    (pCurSmacStatus->eop2_Q2RxAmsdu >=
		     pLast->eop2_Q2RxAmsdu) ? (pCurSmacStatus->eop2_Q2RxAmsdu - pLast->eop2_Q2RxAmsdu) : (pCurSmacStatus->eop2_Q2RxAmsdu +
													  ~pLast->eop2_Q2RxAmsdu + 1);
		rxdiff[RxPktIdx][4] =
		    (pCurSmacStatus->eop2Drp_Cnt >=
		     pLast->eop2Drp_Cnt) ? (pCurSmacStatus->eop2Drp_Cnt - pLast->eop2Drp_Cnt) : (pCurSmacStatus->eop2Drp_Cnt + ~pLast->eop2Drp_Cnt +
												 1);
		...
		if (rxdiff[RxOldIdx][0]) {
			...
			for (i = 1; i < NUM_SFWRX_CHK_POINTS - 1; i++) {
				 ...
				 */
				if (sum[i] == 0 && sum[0] > rxdiff[RxOldIdx][0] && (i != 3 || sum[4] == 0)) {
					...
					ts = xxGetTimeStamp();	//us
					convert_tscale(ts, &ts_sec, &ts_ms, NULL);
					idx += sprintf(&plog[idx], "[%llu.%llu] Rx stuck warning: \n", ts_sec, ts_ms);
			...
			if (wlpd_p->smon.rxstuck_st) {
				...
				for (i = 0; i < NUM_SFWRX_CHK_POINTS - 1 && sum[i]; i++) ;
...
int wlmon_rxamsdu_qfull_detect(struct net_device *netdev, SMAC_STATUS_st * pCurSmacStatus)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	MIB_802DOT11 *mib = vmacSta_p->Mib802dot11;
	SMAC_STATUS_st *pLastSmacStatus = (SMAC_STATUS_st *) wlpd_p->smon.psmacStatus;

	if (wlpd_p->smon.active == 0 || !(wlpd_p->smon.ActiveBitmap & MON_SMAC_RXAMSDU_QFULL))
		return 0;

	if ((wlpptr->wlpd_p->inResetQ == WL_TRUE) ||
	    ((vmacSta_p->preautochannelfinished == 0) && (*(mib->mib_autochannel) != 0)) ||
	    vmacSta_p->busyScanning || (wlpptr->wlpd_p->bBssStartEnable == 0)) {
		/* healthmonitor should ignore rxamsduq full detect during ACS scan or wlreset workqueue. */
		wlpd_p->smon.smaceop2_Q2RxAmsduStuck = 0;
		wlpd_p->smon.smacRxAMSDU_QFullCnt = 0;
		return 0;
	}

	if (pCurSmacStatus->eop2Drp_RxAmsduQFull && (pLastSmacStatus->eop2Drp_RxAmsduQFull != pCurSmacStatus->eop2Drp_RxAmsduQFull))
		wlpd_p->smon.smaceop2_Q2RxAmsduStuck += 1;
	else
		wlpd_p->smon.smaceop2_Q2RxAmsduStuck = 0;
	if (pCurSmacStatus->eop2_Q2RxAmsdu && (pLastSmacStatus->eop2_Q2RxAmsdu == pCurSmacStatus->eop2_Q2RxAmsdu))
		wlpd_p->smon.smacRxAMSDU_QFullCnt += 1;
	else
		wlpd_p->smon.smacRxAMSDU_QFullCnt = 0;
	if (wlpd_p->smon.smaceop2_Q2RxAmsduStuck >= 5 && wlpd_p->smon.smacRxAMSDU_QFullCnt >= 5) {
		WLDBG_ERROR(DBG_LEVEL_0, "Rx AMSDUQ full detected, "
			    "eop2_Q2RxAmsdu: %x, %x, %x, eop2Drp_RxAmsduQFull: %x, %x, %x\n",
			    wlpd_p->smon.smaceop2_Q2RxAmsduStuck,
			    pLastSmacStatus->eop2_Q2RxAmsdu, pCurSmacStatus->eop2_Q2RxAmsdu,
			    wlpd_p->smon.smacRxAMSDU_QFullCnt, pLastSmacStatus->eop2Drp_RxAmsduQFull, pCurSmacStatus->eop2Drp_RxAmsduQFull);
		return 1;
	}
	return 0;
}
...
void wlmon_dbg_show_last_cmd_fw_processed(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	void *addr[1];
...
void wlmon_dbg_show_alivecnt(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	...
	for (i = 0; i < MAX_SMACSTATUS_LOG_ENTRY; i++) {
		printk("cmd_thread: %u, sche_thread: %u, idle_thread: %u\n", wlpd_p->smon.pfwaliveCnt[0].alivecnt[index],
		       wlpd_p->smon.pfwaliveCnt[1].alivecnt[index], wlpd_p->smon.pfwaliveCnt[2].alivecnt[index]);

		index = (index + 1) % MAX_SMACSTATUS_LOG_ENTRY;
	}
}

int wlmon_dbg_mem_usage_check(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct sysinfo val;
	...
	if (wlpd_p->smon.active == 0 || !(wlpd_p->smon.ActiveBitmap & MON_MEM_USAGE))
		return 0;
...
void wlmon_dbg_show_adma_cmd_status(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	void *addr[4];
...
#define EU_SM_STATUS         0x1C2F4
	...
	if (wlpd_p->bus_type == BUS_TYPE_MCI) {
		...
		if ((pbuf = (u32 *) wl_kmalloc(1024, GFP_KERNEL))) {
			...
			if (!wlFwGetAddrValue(netdev, (SMAC_DMEM_START + ADMA_STUCK_CHK_ADDR), 4, pbuf, 0)) {
...
int wlmon_pfw_alive_counters_detct(struct net_device *netdev, void **buf)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	char *plog = NULL;
	...
	if (wlpd_p->smon.active == 0 || !(wlpd_p->smon.ActiveBitmap & MON_PFW_ALIVE_CNTS))
		return 0;

	if (wlpd_p->smon.ActiveIf) {
		//monitor pfw alive counters stuck
		pfw_alive_cnter *pLast = (pfw_alive_cnter *) wlpd_p->smon.pfwaliveCnt;
		int idx = 0;
		int i;
		UINT32 newcnt[3];
		void *pfwdbgstate;
		UINT32 dmemaddr;
		U32 index = (wlpd_p->smon.smacStatusLogIdx + 4) % MAX_SMACSTATUS_LOG_ENTRY;
		U32 index_last = (wlpd_p->smon.smacStatusLogIdx + 3) % MAX_SMACSTATUS_LOG_ENTRY;
...
#define DMEM_DEBUG_START_OFFSET 0x123800
		...
		if (pLast[0].stuckcnt == PFW_ALIVE_THRESHOLD || pLast[1].stuckcnt == PFW_ALIVE_THRESHOLD || pLast[2].stuckcnt == PFW_ALIVE_THRESHOLD) {
			...
			if (pLast[0].stuckcnt == PFW_ALIVE_THRESHOLD) {
				idx +=
				    sprintf(&plog[idx], "[%llu.%llu] PFW cmd thread stuck alarm: cmd_thread_alive:%u\n", ts_sec, ts_ms,
					    pLast[0].alivecnt[index]);
			}

			if (pLast[1].stuckcnt == PFW_ALIVE_THRESHOLD) {
				idx +=
				    sprintf(&plog[idx], "[%llu.%llu] PFW sche thread stuck alarm: sche_thread_alive:%u\n", ts_sec, ts_ms,
					    pLast[1].alivecnt[index]);
			}

			if (pLast[2].stuckcnt == PFW_ALIVE_THRESHOLD) {
				idx +=
				    sprintf(&plog[idx], "[%llu.%llu] PFW idle thread stuck alarm: idle_thread_alive:%u\n", ts_sec, ts_ms,
...
void wlmon_log_bmq_buff_refill(struct net_device *netdev, U32 qid, U32 refill_cnt)
{
	...
	//qid = 10 ~ 12 
	if (qid < SC5_BMQ_START_INDEX || qid > SC5_BMQ_START_INDEX + SC5_BMQ_NUM - 2) {
...
int wlmon_bmq_refill_Info_dump(struct net_device *netdev, u8 * plog, U32 maxlen)
{
	...
	for (i = 0; i < 3; i++) {
		...
		idx +=
		    sprintf(&plog[idx], "Q[%u]:\nThe last refill timestamp:[%llu.%llu], %u buff refilled\n", (i + SC5_BMQ_START_INDEX), ts_sec, ts_ms,
			    bqInfo[i].lastbuffcnt);
		idx +=
		    sprintf(&plog[idx], "measuring interval:[%llu ms], refill cnt:%u, %u buff refilled\n", ts_ims, bqInfo[i].refillcnt,
			    bqInfo[i].buffcnt);
...
int mochi_error_detect(struct net_device *netdev, void **buf)
{
	...
	if (wlpd_p->smon.active == 0 || !(wlpd_p->smon.ActiveBitmap & MON_MOCHI_ERROR) || wlpd_p->bus_type == BUS_TYPE_PCI)
		return 0;
...
void wlmon_log_pfw_schInfo(struct net_device *netdev, QS_TX_SCHEDULER_INFO_t * pbuf)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
...
int pfw_scheduler_info_detect(struct net_device *netdev, void **buf)
{
	...
	if (wlpd_p->smon.active == 0 || !(wlpd_p->smon.ActiveBitmap & MON_PFW_SCHE_INFO))
		return 0;
	...
	if (wlFwGetQueueStats(netdev, QS_GET_TX_SCHEDULER_INFO, 1, NULL) == 0) {
		...
		for (i = 0; i < 10; i++) {
			...
			if (pQS_Sched->debug_scheduler2[i][2] > PFW_SCHEDULE_DELAY) {	//> 10ms
				...
				if (pQS_Sched->debug_scheduler2[i][2] > maxval) {
					maxval = pQS_Sched->debug_scheduler2[i][2];
					didx = i;
				}
			}
		}
		...
		if (flag && maxval == wlpd_p->smon.maxschdelay && didx == wlpd_p->smon.pfwidx)
			flag = 0;
		...
		if (flag) {
			...
			idx += sprintf(&plog[idx], "\tNumMpdu   \tNumBytes  \tTimeDelay\n");
			for (i = 0; i < 10; i++) {
				idx += sprintf(&plog[idx], "\t%10u\t%10u\t%10u\n", ENDIAN_SWAP32((int)pQS_Sched->debug_scheduler2[i][0]),
...
u8 hm_dump_file(void *wlpd, UINT8 * valbuf, UINT32 length, UINT8 * fname, UINT32 append)
{
	struct wlprivate_data *wlpd_p = (struct wlprivate_data *)wlpd;
	u64 flen = 0;
	u32 fnameid;
	...
	if (flen > MAX_DUMP_FILESIZE) {
		...
		fnameid++;
		swNewFile = 1;
		if (memcmp(wlpd_p->smon.dumpstsname, fname, (u32) strlen(wlpd_p->smon.dumpstsname)) == 0) {
			sprintf(&fname[idx], "-%u", fnameid);	// ++sts_index);
			sprintf(&wlpd_p->smon.dumpstsname[idx], "-%u", fnameid);	//sts_index);
			//printk("dump to next file:%s\n",fname);
		} else if (memcmp(wlpd_p->smon.dumpcmdname, fname, (u32) strlen(wlpd_p->smon.dumpcmdname)) == 0) {
			...
			//printk("dump to next file:%s\n",fname);
			size = (u32) sprintf((char *)&tempbuf[0], "-- logFile_idx:%u --\n", cmdlogidx);
...
void hm_copy_file(UINT8 * srcfname, UINT8 * dstfname)
{
	...
	lenS = kernel_read(filp_coreS, plog, MAX_DUMP_FILESIZE, &filp_coreS->f_pos);
...
void wlmon_kumper_callback(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	//char  stsfname[48];
	...
	if (wlmon_log_cmd(netdev, (void **)&pbuf)) {
		hm_dump_file((void *)wlpd_p, pbuf, strlen(pbuf), (UINT8 *) wlpd_p->smon.dumpcmdname, 1);
		wl_kfree(pbuf);
	}
...
static void wlmon_fw_recovery(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
...
static void wlmon_config_fw_exception(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	...
	blocking_notifier_call_chain(&wlpd_p->smon.wlmon_notifier_list, 1, (void *)netdev);
...
extern int wlIoctlGet(struct net_device *netdev, int cmd, char *param_str, int param_len, char *ret_str, UINT16 * ret_len);

static void wlmon_smacstatus_log_buffer(struct net_device *netdev, SMAC_STATUS_st * pCurSmacStatus, U64 tms)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	...
	memcpy((void *)&pbuf[wlpd_p->smon.smacStatusLogIdx], (void *)pCurSmacStatus, sizeof(SMAC_STATUS_st));
	memset(wlpd_p->smon.psmacWarningLog[wlpd_p->smon.smacStatusLogIdx], 0, SMACSTATUS_LOG_MAX_LENGTH);
	wl_show_except_cnt(netdev, wlpd_p->smon.psmacWarningLog[wlpd_p->smon.smacStatusLogIdx]);
	memset(wlpd_p->smon.psmacScheduleInfo[wlpd_p->smon.smacStatusLogIdx], 0, SMACSTATUS_LOG_MAX_LENGTH);
	wl_show_stat(netdev, drvstatsopt_scheduleinfo, 0, wlpd_p->smon.psmacScheduleInfo[wlpd_p->smon.smacStatusLogIdx]);
	memset(wlpd_p->smon.psmacPktcnt[wlpd_p->smon.smacStatusLogIdx], 0, SMACSTATUS_LOG_MAX_LENGTH);
	wl_show_pktcnt_stat(netdev, wlpd_p->smon.psmacPktcnt[wlpd_p->smon.smacStatusLogIdx]);
	memset(wlpd_p->smon.psmacGenInfo[wlpd_p->smon.smacStatusLogIdx], 0, SMACSTATUS_LOG_MAX_LENGTH);
	wl_show_stat(netdev, drvstatsopt_geninfo, 0, wlpd_p->smon.psmacGenInfo[wlpd_p->smon.smacStatusLogIdx]);

	wlpd_p->smon.smacStsTimestamp[wlpd_p->smon.smacStatusLogIdx] = tms;
	wlpd_p->smon.smacStatusLogIdx = (wlpd_p->smon.smacStatusLogIdx + 1) % MAX_SMACSTATUS_LOG_ENTRY;

}

static u8 is_Invalid_Addr(u32 offset, invalid_addr_hole * pholes)
{
	u8 invalid = FALSE;
	u32 i;
	...
	for (i = 0; i < pholes->num; i++) {
		...
		if (offset >= pholes->addr[i][0] && offset <= pholes->addr[i][1]) {
			invalid = TRUE;
			break;
		}
	}
...
static int hw_registers_print(char *plog, u32 * pbuf, u32 reg_addr_offset, u32 len)
{
	int i;
	int str_len = 0;
	...
	for (i = 0; i < len; i++) {
		...
		if ((reg_addr_offset & 0xF) == 0)
			str_len += sprintf(&plog[str_len], "%08x:", (u32) (reg_addr_offset + IOBASE1_R7_MEMORY_BASE));
...
int wlmon_dump_adma_registers(struct wlprivate *wlpptr, char *plog)
{
	u32 *pbuf;
	...
	for (section = 0; section < 3; section++) {
		...
		for (block = 0; block < 5; block++) {
			...
			reg_addr_offset = adma_base + (section * 0x800) + (block * 0x80);

			//print "-----" for non-accessable registers. for compatible with mochi interfaces
			if (prev_end) {
				u32 offset = reg_addr_offset - prev_end;
				str_len += hw_registers_print(&plog[str_len], NULL, prev_end, (offset / sizeof(u32)));
			}

			if (!wlmon_read_hw_registers(wlpptr, (IOBASE1_R7_MEMORY_BASE + reg_addr_offset), len, pbuf))
				str_len += hw_registers_print(&plog[str_len], pbuf, reg_addr_offset, len);
			else
				str_len += hw_registers_print(&plog[str_len], NULL, reg_addr_offset, len);
...
u32 wlmon_read_hw_registers(struct wlprivate * wlpptr, u32 start, u32 len, u32 * pbuf)
{
	...
	while (len > 0) {
		...
		if (!(ret = wlFwGetCoreDumpAddrValue(netdev, start, entry, buffer, 0)))
			buffer += entry;
		else
			break;
...
static int wlmon_dump_hw_regsiters(struct wlprivate *wlpptr, char *plog, unsigned int start_addr, unsigned end_addr, invalid_addr_hole * pholes)
{
	...
	for (reg_addr_offset = start_addr; reg_addr_offset <= end_addr;) {
		...
		if (is_Invalid_Addr((reg_addr_offset - start_addr), pholes)) {
			...
			if (!IS_BUS_TYPE_MCI(wlpptr) && reg_addr_offset > SZ_256K) {	//PCIE only provide 256K remap size in iobase1
				...
				str_len += sprintf(&plog[str_len], "%08x", readl(wlpptr->ioBase1 + reg_addr_offset));
			}
		}
...
static int wlmon_dump_txq_cnt(struct wlprivate *wlpptr, char *plog, unsigned int startid, unsigned int length)
{
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	...
	str_len += sprintf(&plog[str_len], "TXQ pkt cnt: txqid form %4u to %4u:\n", startid, endid);
	str_len += sprintf(&plog[str_len], "txqid:  pend  drop  send  rel txd1_drop\n");
	for (i = startid; i <= endid; i++) {
		...
		str_len += sprintf(&plog[str_len], "%4u:  %u  %u  %u  %u %u\n", i, txq_pend, txq_drop, txq_send, txq_rel, txq_txd1_drop);
	}
	str_len += sprintf(&plog[str_len], "\n");
...
int wlmon_smacstatus_log_dump(struct net_device *netdev, void **buf, u8 * stsfname, u8 skipSmac)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	SMAC_STATUS_st *pbuf = (SMAC_STATUS_st *) wlpd_p->smon.psmacStatusLog;
	...
	if ((plog = (void *)wl_kzalloc(WLMON_PRNT_BUF_SIZE, GFP_ATOMIC)) == NULL) {
		printk("Error[%s:%d]: Allocating wlmon buffer failure \n", __func__, __LINE__);
		goto exit;
	}

	if (wlpd_p->smon.exceptionDelayCoreDumpCnt == DELAY_COREDUMP_TIME) {
		idx += sprintf(&plog[idx], "---- SMAC_STATUS_st dump before entering coredump mode ---\n");
	}
	...
	if (skipSmac == 0) {
		...
		for (i = 0; i < wlpptr->wlpd_p->NumOfAPs; i++) {
			idx += sprintf(&plog[idx], "\n----%sap%d ", netdev->name, i);
			wlIoctlGet(wlpptr->vdev[i], WL_IOCTL_GET_STALISTEXT, NULL, 0, &plog[idx], &ret_len);
			idx += ret_len;
		}
		idx += sprintf(&plog[idx], "\n\n");
	}
	...
	for (i = 0; i < MAX_SMACSTATUS_LOG_ENTRY && skipSmac == 0; i++) {
		...
		convert_tscale(wlpd_p->smon.smacStsTimestamp[index], &ts_sec, &ts_ms, NULL);
		idx += sprintf(&plog[idx], "[%llu.%llu]:\n", ts_sec, ts_ms);
		idx += sprintSmacStatus(&plog[idx], &pbuf[index], WLMON_PRNT_BUF_SIZE - idx, format);
		idx += sprintf(&plog[idx], "\n\n");

		idx += sprintf(&plog[idx], "\nSTAT 1 -----------\n");
		memcpy(&plog[idx], wlpd_p->smon.psmacGenInfo[index], strlen(wlpd_p->smon.psmacGenInfo[index]));
		idx += strlen(wlpd_p->smon.psmacGenInfo[index]);

		idx += sprintf(&plog[idx], "\nSTAT 2 -----------\n");
		memcpy(&plog[idx], wlpd_p->smon.psmacWarningLog[index], strlen(wlpd_p->smon.psmacWarningLog[index]));
		idx += strlen(wlpd_p->smon.psmacWarningLog[index]);

		idx += sprintf(&plog[idx], "\nSTAT 4 -----------\n");
		memcpy(&plog[idx], wlpd_p->smon.psmacScheduleInfo[index], strlen(wlpd_p->smon.psmacScheduleInfo[index]));
		idx += strlen(wlpd_p->smon.psmacScheduleInfo[index]);

		idx += sprintf(&plog[idx], "\nSTAT 8 -----------\n");
		memcpy(&plog[idx], wlpd_p->smon.psmacPktcnt[index], strlen(wlpd_p->smon.psmacPktcnt[index]));
	...
	if (skipSmac == 0) {
		u64 ts = ((sta_db_snap *) (wlpd_p->smon.pStaDbTable))->StaDbTimestamp;
		u8 *lbuf = ((sta_db_snap *) (wlpd_p->smon.pStaDbTable))->StaDbBuf;

		idx += sprintf(&plog[idx], "the last snapshot of avl sta db:\n");
		convert_tscale(ts, &ts_sec, &ts_ms, NULL);
		idx += sprintf(&plog[idx], "[%llu.%llu]:\n", ts_sec, ts_ms);
	...
	idx += sprintf(&plog[idx], "\nHFrame1 0x20000 ~ 0x21000\n");
	idx += wlmon_dump_hw_regsiters(wlpptr, &plog[idx], 0x20000, 0x21000, NULL);
	/* HFrame2 0x28000 ~ 0x29000 */
	idx += sprintf(&plog[idx], "\nHFrame2 0x28000 ~ 0x29000\n");
	idx += wlmon_dump_hw_regsiters(wlpptr, &plog[idx], 0x28000, 0x29000, NULL);
	/* HFrame3 0x30000 ~ 0x31000 */
	idx += sprintf(&plog[idx], "\nHFrame3 0x30000 ~ 0x31000\n");
	idx += wlmon_dump_hw_regsiters(wlpptr, &plog[idx], 0x30000, 0x31000, NULL);
	/* Rx 0x10000 ~ 0x10ffc */
	idx += sprintf(&plog[idx], "\nRx 0x10000 ~ 0x10FFC\n");
	idx += wlmon_dump_hw_regsiters(wlpptr, &plog[idx], 0x10000, 0x10FFC, NULL);

	/* BF 0x11000 ~ 0x111FC */
	idx += sprintf(&plog[idx], "\nBF 0x11000 ~ 0x111FC\n");
	idx += wlmon_dump_hw_regsiters(wlpptr, &plog[idx], 0x11000, 0x111FC, NULL);

	/* DFS 0x12000 ~ 0x120FC */
	idx += sprintf(&plog[idx], "\nDFS 0x12000 ~ 0x120FC\n");
	idx += wlmon_dump_hw_regsiters(wlpptr, &plog[idx], 0x12000, 0x120FC, NULL);

	/* BMAN 0x13800 ~ 0x13A68 holes:13920~139FC, efined but unaccess: 930~93C */
	idx += sprintf(&plog[idx], "\nBMAN 0x13800 ~ 0x13A68\n");
	idx += wlmon_dump_hw_regsiters(wlpptr, &plog[idx], 0x13800, 0x13A68, &inv_bman);
	...
	/* EU 0x1C000 ~ 0x20000 */
	idx += sprintf(&plog[idx], "\nEU 0x1C000 ~ 0x1CA00\n");
	idx += wlmon_dump_hw_regsiters(wlpptr, &plog[idx], 0x1C000, 0x1CA00, NULL);

	/* TxDMA 0x14000 ~ 0x15000 */
	idx += sprintf(&plog[idx], "\nTxDMA 0x14000 ~ 0x15000\n");
	idx += wlmon_dump_hw_regsiters(wlpptr, &plog[idx], 0x14000, 0x15000, NULL);

	/* AVL 0x1A000 ~ 0x1A5FC */
	idx += sprintf(&plog[idx], "\nAVL 0x1A000 ~ 0x1A5FC\n");
	idx += wlmon_dump_hw_regsiters(wlpptr, &plog[idx], 0x1A000, 0x1AC00, &inv_avl);

	/* SMAC_CTRL 0x1A800 ~ 0x1ABAC, no memory hole */
	idx += sprintf(&plog[idx], "\nSMAC_CTRL 0x1A800 ~ 0x1ABAC\n");
	idx += wlmon_dump_hw_regsiters(wlpptr, &plog[idx], 0x1A800, 0x1ABAC, NULL);

	/* ADMA 0xA0000 ~ 0xA125C */
	idx += sprintf(&plog[idx], "\nADMA 0xA0000 ~ 0xA125C\n");
	if (IS_BUS_TYPE_MCI(wlpptr))
		idx += wlmon_dump_hw_regsiters(wlpptr, &plog[idx], 0xA0000, 0xA125C, &inv_adma);
	else
...
void wlmon_show_version(struct net_device *netdev, UINT8 * stsfname)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	int idx;
	...
	if ((pbuf = (void *)wl_kmalloc(WLMON_PRNT_BUF_SIZE, GFP_ATOMIC)) == NULL) {
		printk("Error[%s:%d]: Allocating wlmon buffer failure \n", __func__, __LINE__);
		goto exit;
	}
...
static inline UINT32 prev_idx(UINT32 i)
{
	return (((i - 1) < i) ? (i - 1) : 3);
}

static inline UINT32 get_delta(UINT32(*arr)[1], UINT32 i)
{
...
static UINT32 wlmon_txstuck_detect_persta(struct wlprivate_data *wlpd_p, UINT16 stnId)
{
	UINT32(*txdonediff)[1];
	...
	if (wlpd_p == NULL)
		return 0;
	if (wlpd_p->except_cnt.tx_sta_pend_cnt[stnId] >= max_tx_pend_cnt_per_sta) {
		/* record tx done */
		txdidx = wlpd_p->except_cnt.txdidx[stnId][0];
		txdonediff = wlpd_p->except_cnt.txdonediff[stnId];
		txdonediff[txdidx][0] = wlpd_p->except_cnt.tx_sta_rel_cnt[stnId];
		txdidx = (txdidx + 1) % 4;
		if (txdonediff[txdidx][0]) {
			delta_txdone = get_delta(txdonediff, wlpd_p->except_cnt.txdidx[stnId][0]);
			if (delta_txdone == 0) {
				memset(txdonediff, 0, sizeof(wlpd_p->except_cnt.txdonediff[stnId]));
...
static UINT32 wlmon_txstuck_recovery_detect(struct net_device *netdev)
{
	UINT8 macAddr[6];
	...
	if (vmacSta_p->VMacEntry.modeOfService != VMAC_MODE_CLNT_INFRA) {
		...
		if (sta_buf == NULL) {
			printk("Failed to monitor connected STAs (%u)," "no memory\n", entries);
			return 0;
		}
		p = sta_buf;
		for (i = 0; i < entries; i++) {
			memcpy(macAddr, p, sizeof(macAddr));
			if ((pStaInfo = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) macAddr, STADB_DONT_UPDATE_AGINGTIME)) == NULL) {
				p += sizeof(STA_INFO);
				continue;
			}
...
extern SMAC_STATUS_st smacStatus;

static UINT32 wlmon_rxstuck_recovery_detect(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	UINT32 rxidx = 0, delta_cfhuld = 0, delta_stat2 = 0, delta_stat8 = 0, delta_stat10 = 0;

	/* rx amsduq full detect */
	memset(&smacStatus, 0, sizeof(SMAC_STATUS_st));
	memcpy(&smacStatus, wlpptr->smacStatusAddr, sizeof(SMAC_STATUS_st));
	if (smacStatus.eop2Drp_RxAmsduQFull) {
	...
	wlpd_p->except_cnt.rx_exception[rxidx][0] = wlpd_p->except_cnt.cnt_cfhul_invalid_signature +
	    wlpd_p->except_cnt.cnt_cfhul_error + wlpd_p->except_cnt.cnt_cfhul_snap_error + wlpd_p->except_cnt.cnt_cfhul_oversize;
	wlpd_p->except_cnt.stat8_data_diff[rxidx][0] = wlpd_p->rpkt_type_cnt.data_cnt;
	wlpd_p->except_cnt.stat10_data_diff[rxidx][0] = wlpptr->vmacSta_p->BA_Rodr2Host;

	rxidx = (rxidx + 1) % 4;
	if (wlpd_p->except_cnt.cfhuld[rxidx][0] && wlpd_p->except_cnt.rx_exception[rxidx][0]) {
		delta_cfhuld = get_delta(wlpd_p->except_cnt.cfhuld, wlpd_p->except_cnt.rxidx);
		delta_stat2 = get_delta(wlpd_p->except_cnt.rx_exception, wlpd_p->except_cnt.rxidx);
		if ((delta_stat2 * 10) > (delta_cfhuld * 8)) {
			memset(wlpd_p->except_cnt.cfhuld, 0, sizeof(wlpd_p->except_cnt.cfhuld));
			memset(wlpd_p->except_cnt.rx_exception, 0, sizeof(wlpd_p->except_cnt.rx_exception));
	...
	if (!wlpd_p->fastdata_reordering_disable) {
		if (wlpd_p->except_cnt.stat8_data_diff[rxidx][0] && wlpd_p->except_cnt.stat10_data_diff[rxidx][0]) {
			delta_stat8 = get_delta(wlpd_p->except_cnt.stat8_data_diff, wlpd_p->except_cnt.rxidx);
			delta_stat10 = get_delta(wlpd_p->except_cnt.stat10_data_diff, wlpd_p->except_cnt.rxidx);
			if (delta_stat10 * 10 < delta_stat8) {
				memset(wlpd_p->except_cnt.stat8_data_diff, 0, sizeof(wlpd_p->except_cnt.stat8_data_diff));
				memset(wlpd_p->except_cnt.stat10_data_diff, 0, sizeof(wlpd_p->except_cnt.stat10_data_diff));
...
static UINT32 wlmon_memleak_detect(struct net_device *netdev)
{
#define MEM_LEAK_THRESH 25600
	...
	/* memory leak */
	diff = (wlpd_p->except_cnt.cnt_cfhul_invalid_signature > wlpd_p->except_cnt.free_err_pkts[0]) ?
	    (wlpd_p->except_cnt.cnt_cfhul_invalid_signature - wlpd_p->except_cnt.free_err_pkts[0]) : wlpd_p->except_cnt.cnt_cfhul_invalid_signature;
	if (diff >= MEM_LEAK_THRESH) {
		wlpd_p->except_cnt.mon_fw_recovery |= BIT(4);
		mem_leak = 1;
	}

	diff = (wlpd_p->except_cnt.cnt_cfhul_error > wlpd_p->except_cnt.free_err_pkts[0]) ?
	    (wlpd_p->except_cnt.cnt_cfhul_error - wlpd_p->except_cnt.free_err_pkts[0]) : wlpd_p->except_cnt.cnt_cfhul_error;
	if (diff >= MEM_LEAK_THRESH) {
		wlpd_p->except_cnt.mon_fw_recovery |= BIT(5);
		mem_leak = 1;
	}

	diff = (wlpd_p->except_cnt.cnt_cfhul_snap_error > wlpd_p->except_cnt.free_err_pkts[0]) ?
	    (wlpd_p->except_cnt.cnt_cfhul_snap_error - wlpd_p->except_cnt.free_err_pkts[0]) : wlpd_p->except_cnt.cnt_cfhul_snap_error;
	if (diff >= MEM_LEAK_THRESH) {
		wlpd_p->except_cnt.mon_fw_recovery |= BIT(6);
		mem_leak = 1;
	}

	diff = (wlpd_p->except_cnt.cnt_cfhul_oversize > wlpd_p->except_cnt.free_err_pkts[0]) ?
	    (wlpd_p->except_cnt.cnt_cfhul_oversize - wlpd_p->except_cnt.free_err_pkts[0]) : wlpd_p->except_cnt.cnt_cfhul_oversize;
	if (diff >= MEM_LEAK_THRESH) {
		wlpd_p->except_cnt.mon_fw_recovery |= BIT(7);
		mem_leak = 1;
	}

	if (wlpd_p->except_cnt.cfhul_flpkt_lost[0] - recovered_errcfhul >= MEM_LEAK_THRESH) {
		wlpd_p->except_cnt.mon_fw_recovery |= BIT(8);
		mem_leak = 1;
	}

	if (wlpd_p->except_cnt.cfhul_flpkt_lost[2] - recovered_errcfhul >= MEM_LEAK_THRESH) {
	...
	if (wlpd_p->except_cnt.rx_invalid_sig_cnt[0] >= MEM_LEAK_THRESH ||
	    ...
	    wlpd_p->except_cnt.rx_invalid_sig_cnt[2] > MEM_LEAK_THRESH || wlpd_p->except_cnt.rx_invalid_sig_cnt[3] > MEM_LEAK_THRESH) {
		mem_leak = 1;
...
static UINT32 wlmon_recovery_detect(struct net_device *netdev)
{
...
int wlmon_kthread(void *arg)
{
	...
	len = sprintf(stsfname, "/tmp/dump%s_sts_%04x-1", wlpd_p->rootdev->name, time);
	...
	if (!wlpd_p->smon.pexcept_cnt) {
		if (!(wlpd_p->smon.pexcept_cnt = (void *)wl_kmalloc(sizeof(struct except_cnt), GFP_ATOMIC))) {
	...
	if (!wlpd_p->smon.piocmdlog) {
		if (!(wlpd_p->smon.piocmdlog = (void *)wl_kmalloc(WLMON_PRNT_BUF_SIZE, GFP_ATOMIC))) {
			printk("Error[%s:%d]: Allocating IOCTL fwcmd log Memory \n", __func__, __LINE__);
			return 0;
		}
		memset(wlpd_p->smon.piocmdlog, 0, WLMON_PRNT_BUF_SIZE);
		wlpd_p->smon.cmdlogidx = (UINT32) sprintf((char *)wlpd_p->smon.piocmdlog, "\n--- Starting IOCTL FwCmd log---\n");
	}
	...
	if (!wlpd_p->smon.psmacStatus) {
		...
		if (!(wlpd_p->smon.psmacStatus = (void *)wl_kmalloc(2 * sizeof(SMAC_STATUS_st), GFP_ATOMIC))) {
	...
	if (!wlpd_p->smon.psmacStatusLog) {
		...
		//Allocate 2*sizeof(SMAC_STATUS_st) for store old, @[0], and current, @[1], SMAC_STATUS_st counters. 
		if (!(wlpd_p->smon.psmacStatusLog = (void *)wl_kmalloc(MAX_SMACSTATUS_LOG_ENTRY * sizeof(SMAC_STATUS_st), GFP_ATOMIC))) {
			printk("Error[%s:%d]: Allocating MAC_STATUS_st log buffer \n", __func__, __LINE__);
			return 0;
		}
		memset(wlpd_p->smon.psmacStatusLog, 0, MAX_SMACSTATUS_LOG_ENTRY * sizeof(SMAC_STATUS_st));
		wlpd_p->smon.smacStatusLogIdx = 0;
	}

	if (!wlpd_p->smon.psmacWarningLog[0]) {
		for (i = 0; i < MAX_SMACSTATUS_LOG_ENTRY; i++) {
			if (!(wlpd_p->smon.psmacWarningLog[i] = (void *)wl_kmalloc(SMACSTATUS_LOG_MAX_LENGTH, GFP_ATOMIC))) {
				printk("Error[%s:%d]: Allocating MAC_STATUS_st log buffer \n", __func__, __LINE__);
				return 0;
			}
			memset(wlpd_p->smon.psmacWarningLog[i], 0, SMACSTATUS_LOG_MAX_LENGTH);
		}
	}
	if (!wlpd_p->smon.psmacScheduleInfo[0]) {
		for (i = 0; i < MAX_SMACSTATUS_LOG_ENTRY; i++) {
			if (!(wlpd_p->smon.psmacScheduleInfo[i] = (void *)wl_kmalloc(SMACSTATUS_LOG_MAX_LENGTH, GFP_ATOMIC))) {
				printk("Error[%s:%d]: Allocating MAC_STATUS_st log buffer \n", __func__, __LINE__);
				return 0;
			}
			memset(wlpd_p->smon.psmacScheduleInfo[i], 0, SMACSTATUS_LOG_MAX_LENGTH);
		}
	}
	if (!wlpd_p->smon.psmacPktcnt[0]) {
		for (i = 0; i < MAX_SMACSTATUS_LOG_ENTRY; i++) {
			if (!(wlpd_p->smon.psmacPktcnt[i] = (void *)wl_kmalloc(SMACSTATUS_LOG_MAX_LENGTH, GFP_ATOMIC))) {
				printk("Error[%s:%d]: Allocating MAC_STATUS_st log buffer \n", __func__, __LINE__);
				return 0;
			}
			memset(wlpd_p->smon.psmacPktcnt[i], 0, SMACSTATUS_LOG_MAX_LENGTH);
		}
	}
	//per SMAC request, add "stat 1" info to log 
	if (!wlpd_p->smon.psmacGenInfo[0]) {
		for (i = 0; i < MAX_SMACSTATUS_LOG_ENTRY; i++) {
			if (!(wlpd_p->smon.psmacGenInfo[i] = (void *)wl_kmalloc(SMACSTATUS_LOG_MAX_LENGTH, GFP_ATOMIC))) {
				printk("Error[%s:%d]: Allocating MAC_STATUS_st log buffer (psmacGenInfo) \n", __func__, __LINE__);
				return 0;
			}
			memset(wlpd_p->smon.psmacGenInfo[i], 0, SMACSTATUS_LOG_MAX_LENGTH);
		}
	}

	if (!wlpd_p->smon.pPFWSchInfo) {
		if (!(wlpd_p->smon.pPFWSchInfo = (void *)wl_kmalloc(sizeof(QS_TX_SCHEDULER_INFO_t), GFP_ATOMIC))) {
			printk("Error[%s:%d]: Allocating PFW scheduler Info log Memory \n", __func__, __LINE__);
			return 0;
		}
		memset(wlpd_p->smon.pPFWSchInfo, 0, sizeof(QS_TX_SCHEDULER_INFO_t));
	}

	if (!wlpd_p->smon.pStaDbTable) {

		if (!(wlpd_p->smon.pStaDbTable = (void *)wl_kmalloc(sizeof(sta_db_snap), GFP_ATOMIC))) {
	...
	if (!wlpd_p->smon.pLastCmdBuf) {
		...
		if (!(wlpd_p->smon.pLastCmdBuf = (void *)wl_kmalloc(HM_CMDBUF_SIZE, GFP_ATOMIC))) {
	...
	if (!wlpd_p->smon.active) {
		//default enable HM
		wldbgCoreMonitor(netdev, WLMON_DEFAULT_DISABLE, WLMON_DEFAULT_HMMASK, SMAC_STATUS_FORMAT_RAW);
	}

	wlpd_p->smon.temperature_threshold = WLMON_DEFAULT_TEMPERATURE_THRESHOLD;
	wlpd_p->smon.temperature_threshold_host = WLMON_DEFAULT_TEMPERATURE_THRESHOLD_HOST;
	...
	for (;;) {
		...
		if (wlpd_p->smon.exceptionCmdTOEvt_rcvd) {
			...
			size = (UINT32) sprintf(&logbuf[0], "[HM]: Cmd Timeout detected. Auto active fw exception event\n");
			hm_dump_file(wlpd_p, logbuf, strlen(logbuf), (UINT8 *) stsfname, 1);

			wlmon_dbg_show_adma_cmd_status(netdev);

		}

		if (wlpd_p->smon.exceptionEvt_rcvd || wlpd_p->smon.exceptionDelayCoreDumpCnt) {
			...
			if (wlpd_p->smon.exceptionDelayCoreDumpCnt == 0)
				wlpd_p->smon.exceptionDelayCoreDumpCnt = DELAY_COREDUMP_TIME;
			...
			if (wlpd_p->smon.exceptionEvt_rcvd && wlpd_p->smon.exceptionDelayCoreDumpCnt == DELAY_COREDUMP_TIME) {
				...
				if (wlmon_smacstatus_log_dump(netdev, (void **)&pbuf, stsfname, 0)) {
					hm_dump_file(wlpd_p, pbuf, strlen(pbuf), (UINT8 *) stsfname, 1);
					wl_kfree(pbuf);
				}
			}
			...
			if (--(wlpd_p->smon.exceptionDelayCoreDumpCnt) > 0) {
				...
				if (wlmon_smacstatus_log_dump(netdev, (void **)&pbuf, stsfname, 1)) {

					sprintf(enddump, "%s%s", stsfname, DUMPEND_SIGNATURE);
					//printk("filename:%s\n",enddump);
					hm_dump_file(wlpd_p, pbuf, strlen(pbuf), (UINT8 *) stsfname, 1);
					wl_kfree(pbuf);
					//create a empty file with the last sts filename as prefix and postfix by __End to denote end of coredump log
					hm_dump_file(wlpd_p, CoreDumpEndSig, strlen(CoreDumpEndSig), (UINT8 *) enddump, 1);
				}
			}

			//PFW alreay in coredumpmode, action=0
			if (wlpd_p->smon.exceptionAbortCmdExec == 0) {
				blocking_notifier_call_chain(&wlpd_p->smon.wlmon_notifier_list, 0, (void *)netdev);
			} else {
		...
		if (wlpd_p->smon.parityErrEvt_rcvd) {
			...
			printk("[HM]:%s ECC/parity error 0x%X\n", wlpd_p->rootdev->name, wlpd_p->smon.cpu_parity_check_status);
		...
		if (wlpd_p->smon.ActiveIf && pCurSmacStatus) {
			...
			memcpy(pCurSmacStatus, wlpptr->smacStatusAddr, sizeof(SMAC_STATUS_st));

			convert_tscale(xxGetTimeStamp(), NULL, &tms, NULL);
			wlpd_p->smon.smacStsLogtime[1] = tms;	//record ms timetick of pCurSmacStatus  

			//copy to smac log buffer
			wlmon_smacstatus_log_buffer(netdev, pCurSmacStatus, (tms * 1000LL));
			...
			if (wlmon_beacon_stuck_detct(netdev, pCurSmacStatus, (void **)&pbuf)) {
				if (!bcnflag) {
					bcnflag = 1;
			...
			//Tx Stuck detecting
			if (wlmon_tx_stuck_detct(netdev, pCurSmacStatus, (void **)&pbuf)) {
				if (!txstuckflag) {
					txstuckflag = 1;
			...
			//Rx Stuck detecting
			if (wlmon_rx_stuck_detct(netdev, pCurSmacStatus, (void **)&pbuf)) {
				if (!rxstuckflag) {
					rxstuckflag = 1;
			...
			// Rx AMSDUQ Full detecting
			if (wlmon_rxamsdu_qfull_detect(netdev, pCurSmacStatus)) {
			...
			memcpy(wlpd_p->smon.psmacStatus, pCurSmacStatus, sizeof(SMAC_STATUS_st));
			wlpd_p->smon.smacStsLogtime[0] = wlpd_p->smon.smacStsLogtime[1];

			if (wlmon_pfw_alive_counters_detct(netdev, (void **)&pbuf)) {
				if (!pfwaliveflag) {
					pfwaliveflag = 1;
		...
		if (wlmon_log_cmd(netdev, (void **)&pbuf)) {
			...
			swNextFile = hm_dump_file(wlpd_p, pbuf, strlen(pbuf), (UINT8 *) cmdfname, 1);
			wl_kfree(pbuf);
			...
			if (swNextFile) {
				...
				size = (UINT32) sprintf(&logbuf[0], "[CmdTimeStamp]:   DevName  FWCmd   CPUID   PID   ProcName  CmdCmpTime(usec)\n");
...
int start_wlmon(void *wlp)
{
	...
	wlpdptr->wlmon_task = kthread_create(wlmon_kthread, (void *)wlpptr, name);
