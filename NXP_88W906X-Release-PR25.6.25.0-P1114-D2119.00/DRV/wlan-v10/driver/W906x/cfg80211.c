...
static const struct ieee80211_txrx_stypes mwl_mgmt_stypes[NUM_NL80211_IFTYPES] = {
	[NL80211_IFTYPE_STATION] = {
				    .tx = BIT(IEEE80211_STYPE_ACTION >> 4) | BIT(IEEE80211_STYPE_PROBE_RESP >> 4),
				    .rx = BIT(IEEE80211_STYPE_ACTION >> 4) | BIT(IEEE80211_STYPE_PROBE_REQ >> 4) | BIT(IEEE80211_STYPE_AUTH >> 4),
				    },
	[NL80211_IFTYPE_AP] = {
			       .tx = BIT(IEEE80211_STYPE_ACTION >> 4) |
#if defined(OWE_SUPPORT) || defined(MBO_SUPPORT)
			       BIT(IEEE80211_STYPE_ASSOC_RESP >> 4) | BIT(IEEE80211_STYPE_REASSOC_RESP >> 4) |
#endif				/* OWE_SUPPORT || MBO_SUPPORT */
			       BIT(IEEE80211_STYPE_AUTH >> 4) | BIT(IEEE80211_STYPE_PROBE_RESP >> 4),
			       .rx = BIT(IEEE80211_STYPE_ACTION >> 4) | BIT(IEEE80211_STYPE_PROBE_REQ >> 4) |
#if defined(OWE_SUPPORT) || defined(MBO_SUPPORT)
			       BIT(IEEE80211_STYPE_ASSOC_REQ >> 4) | BIT(IEEE80211_STYPE_REASSOC_REQ >> 4) |
#endif				/* OWE_SUPPORT || MBO_SUPPORT */
...
static int mwl_cfg80211_mgmt_tx(struct wiphy *wiphy, struct wireless_dev *wdev, struct cfg80211_mgmt_tx_params *params, u64 * cookie)
{
	...
	*cookie = prandom_u32() | 1;
	...
	if (wlanMsg_p->Hdr.FrmCtl.Subtype == IEEE_MSG_AUTHENTICATE) {
		...
		if ((pStaInfo = extStaDb_GetStaInfo(vmacSta_p, &wlanMsg_p->Hdr.Addr1, STADB_DONT_UPDATE_AGINGTIME)) == NULL) {
			...
			if ((pStaInfo = macMgtStaDbInit(vmacSta_p, &wlanMsg_p->Hdr.Addr1, (IEEEtypes_MacAddr_t *) vmacSta_p->macBssId)) == NULL) {
				wl_free_skb(skb);
				WLDBG_ENTER_INFO(DBG_LEVEL_11, "init data base fail\n");
				return -1;
			}
		}
		if (MgmtMsg_p->Body.Auth.AuthAlg == 0x03 && MgmtMsg_p->Body.Auth.AuthTransSeq == 0x02 && MgmtMsg_p->Body.Auth.StatusCode == 0x00) {
			if (pStaInfo->State != ASSOCIATED)
				pStaInfo->State = AUTHENTICATED;
		}
	}

	if ((wlanMsg_p->Hdr.FrmCtl.Subtype == IEEE_MSG_ASSOCIATE_RSP) || (wlanMsg_p->Hdr.FrmCtl.Subtype == IEEE_MSG_REASSOCIATE_RSP)) {
		...
		temp_p = FindIEWithinIEs(&wlanMsg_p->Body[0] + 6, len - 6 - sizeof(IEEEtypes_GenHdr_t) + sizeof(UINT16), PROPRIETARY_IE, MBO_OUI);
		if (temp_p)
			memcpy(&pStaInfo->AP_MBOIEBuf[0], temp_p, *(temp_p + 1) + 2);
#endif				/* MBO_SUPPORT */

#ifdef OWE_SUPPORT
		temp_p = FindIEWithinIEs(&wlanMsg_p->Body[0] + 6, len - 6 - sizeof(IEEEtypes_GenHdr_t) + sizeof(UINT16), EXTENSION, NULL);
		if (temp_p) {
			memcpy(&pStaInfo->AP_DHIEBuf[0], temp_p, *(temp_p + 1) + 2);
		}

		memset(pStaInfo->EXT_RsnIE, 0, 64);
		temp_p = FindIEWithinIEs(&wlanMsg_p->Body[0] + 6, len - 6 - sizeof(IEEEtypes_GenHdr_t) + sizeof(UINT16), RSN_IEWPA2, NULL);
		if (temp_p) {
		...
		if (MgmtMsg_p->Body.AssocRsp.StatusCode == IEEEtypes_STATUS_SUCCESS) {
			if (pStaInfo->assocReq_skb == NULL) {
				printk("ASSOC_RSP expired assoc timer\n");
				wl_free_skb(skb);
				return -EINVAL;
			}
...
#if defined(OWE_SUPPORT) || defined(MBO_SUPPORT)
	if (wlanMsg_p->Hdr.FrmCtl.Subtype == IEEE_MSG_ACTION && !memcmp(wlanMsg_p->Hdr.Addr3, wildcard_bssid, IEEEtypes_ADDRESS_SIZE)) {
		wlFwSendFrame(vmacSta_p->dev, 0xFFFF, 0, 0, 0, 24, len - 24, (UINT8 *) buf, (UINT8 *) & buf[24]);
		wl_free_skb(skb);
	} else
#endif				/* OWE_SUPPORT || MBO_SUPPORT */
	{
...
static void mwl_cfg80211_mgmt_frame_register(struct wiphy *wiphy, struct wireless_dev *wdev, u16 frame_type, bool reg)
{
	return;
}

static int mwl_cfg80211_set_antenna(struct wiphy *wiphy, u32 tx_ant, u32 rx_ant)
{
...
static int mwl_cfg80211_get_antenna(struct wiphy *wiphy, u32 * tx_ant, u32 * rx_ant)
{
	struct wlprivate *priv = mwl_cfg80211_get_priv(wiphy);
	vmacApInfo_t *vmacSta_p = priv->vmacSta_p;
	MIB_802DOT11 *mib;
...
static int mwl_cfg80211_sta_trigger_scan(struct wlprivate *priv)
{
...
void mwl_cfg80211_scan_done(UINT8 * data)
{
	...
	for (i = 0; i < tmpNumScanDesc[vmacSta_p->VMacEntry.phyHwMacIndx]; i++) {
		...
		if ((dsPSetIE_p = (IEEEtypes_DsParamSet_t *) smeParseIeType(DS_PARAM_SET,
			...
			if (dsPSetIE_p->CurrentChan > 0 && dsPSetIE_p->CurrentChan <= 14)
		...
		if ((ssidIE_p = (IEEEtypes_SsIdElement_t *) smeParseIeType(SSID,
			...
			memcpy(&ie_buf[ie_len + 2], &ssidIE_p->SsId[0], ssidIE_p->Len);
		...
		if ((wpa2IE_p = (IEEEtypes_RSN_IE_WPA2_t *) smeParseIeType(RSN_IEWPA2,
			...
			memcpy(&ie_buf[ie_len + 2], &wpa2IE_p->Ver[0], wpa2IE_p->Len);
		...
		if ((RSN_p = linkMgtParseWpaIe((((UINT8 *) curDescpt_p) + sizeof(scanDescptHdr_t)),
			...
			memcpy(&ie_buf[ie_len + 2], &RSN_p->OuiType[0], RSN_p->Len);
			ie_len = ie_len + RSN_p->Len + 2;
		}
#ifdef MRVL_WPS_CLIENT
		/* Set WPS IE */
		if ((RSN_p = linkMgtParseWpsIe((((UINT8 *) curDescpt_p) + sizeof(scanDescptHdr_t)),
					       curDescpt_p->length + sizeof(curDescpt_p->length) - sizeof(scanDescptHdr_t)))) {
			ie_buf[ie_len] = RSN_p->ElemId;
			ie_buf[ie_len + 1] = RSN_p->Len;
			memcpy(&ie_buf[ie_len + 2], &RSN_p->OuiType[0], RSN_p->Len);
		...
		bss = cfg80211_inform_bss(priv->wiphy,
					  ...
					  bssid, timestamp, cap_info_bitmap, beacon_period, ie_buf, ie_len, rssi, GFP_KERNEL);
		if (bss)
...
static int mwl_cfg80211_scan(struct wiphy *wiphy, struct cfg80211_scan_request *request)
{
	struct wlprivate *priv = mwl_cfg80211_get_priv(wiphy);
	int ret;
	...
	if (!ret) {
		TimerFireInByJiffies(&scanTimer, 1, &mwl_cfg80211_scan_done, (UINT8 *) priv, MAX_SCAN_TIME);
		scanTimerFlag = true;
	}
	return ret;
}

static int mwl_cfg80211_fill_chandef(CHNL_FLAGS Chanflag, struct cfg80211_chan_def *chandef, UINT8 extSubCh)
{
...
static int mwl_cfg80211_get_channel(struct wiphy *wiphy, struct wireless_dev *wdev, struct cfg80211_chan_def *chandef)
{
	...
	for (j = 0; j < mwl_band_50.n_channels; j++) {
		if (wlpptr->channel.hw_value == mwl_channels_50[j].hw_value) {
			...
			wlpptr->channel.center_freq = mwl_channels_50[j].center_freq;
			break;
		}
	}
	...
	for (j = 0; j < mwl_band_24.n_channels; j++) {
		if (wlpptr->channel.hw_value == mwl_channels_24[j].hw_value) {
			...
			wlpptr->channel.center_freq = mwl_channels_24[j].center_freq;
			break;
		}
	}
	chandef->chan = &(wlpptr->channel);
	mwl_cfg80211_fill_chandef(mib->PhyDSSSTable->Chanflag, chandef, *(mib->mib_extSubCh));
	return 0;
}

static int mwl_cfg80211_dump_station(struct wiphy *wiphy, struct net_device *dev, int idx, u8 * mac, struct station_info *sinfo)
{
	...
	if (entries) {
		sta_buf = wl_kzalloc(entries * sizeof(STA_INFO), GFP_KERNEL);
		if (sta_buf == NULL)
			return -ENOMEM;
		...
		sinfo->filled = BIT(NL80211_STA_INFO_TX_BITRATE) | BIT(NL80211_STA_INFO_SIGNAL) | BIT(NL80211_STA_INFO_SIGNAL_AVG);
		//sinfo->txrate.flags = BIT(0) | BIT(1) | BIT(2) | BIT(3) | BIT(5) | BIT(6);
		//sinfo->txrate.mcs = pStaInfo->RateInfo.RateIDMCS;
		sinfo->txrate.legacy = getPhyRate((dbRateInfo_t *) & (pStaInfo->RateInfo)) * 10;
...
static int mwl_cfg80211_set_ap_chanwidth(struct wiphy *wiphy, struct net_device *wdev, struct cfg80211_chan_def *chandef)
{
	struct wlprivate *priv = mwl_cfg80211_get_priv(wiphy);
...
static int marvell_set_ciphersuite(struct net_device *netdev, uint8_t wpamode, uint32_t cipher_pairwise, uint32_t cipher_group)
{
	...
	if (wpamode == NL80211_WPA_VERSION_1) {	/* wpa */
		...
		*(mib->mib_cipherSuite) = cipher_pairwise | 0xff;
		...
		mib->WPA2UnicastCiphers->UnicastCipher[3] = cipher_pairwise & 0xff;
		...
		if (cipher_group)
			...
			mib->RSNConfigWPA2->MulticastCipher[3] = cipher_pairwise & 0xff;
...
static int mwl_beacon_get_rsn(struct cfg80211_beacon_data beacon, u8 * beacon_akm)
{
	const u8 *rsn_ie;
	size_t rsn_ie_len;
	u16 cnt;
...
static int mwl_beacon_get_wpa(struct cfg80211_beacon_data beacon, u8 * beacon_akm)
{
	...
	if (!beacon.tail)
		return -EINVAL;
	do {
		thisStaRsnIE_p = (IEEEtypes_RSN_IE_t *) cfg80211_find_ie(WLAN_EID_VENDOR_SPECIFIC, beacon.tail, beacon.tail_len);
		if (thisStaRsnIE_p == NULL) {
			break;
		}
		/* Find WPA Version */
		if (thisStaRsnIE_p->Ver[0] == 0x01 && thisStaRsnIE_p->Ver[1] == 0x00) {
			break;
		}
...
static int mwl_parse_conn_params(struct net_device *dev, struct cfg80211_connect_params *sme)
{
	...
	rsn_ie = FindIEWithinIEs((UINT8 *) sme->ie, (UINT32) sme->ie_len, RSN_IEWPA2, NULL);
	if (rsn_ie != NULL) {
...
static int mwl_configura_wpa(struct net_device *dev, struct cfg80211_crypto_settings crypto, u32 beacon_akm)
{
	...
	if (0 == crypto.wpa_versions) {
			...
			*(mib->mib_wpaWpa2Mode) = 4;
			if ((vmacEntry_p = sme_GetParentVMacEntry(vmacSta_p->VMacEntry.phyHwMacIndx)) != NULL) {
				...
				if (pStaSystemMibs != NULL) {
					pStaSystemMibs->mib_StaCfg_p->wpawpa2Mode = 16;
				}
			}
		}
#endif
	} else {
		if (crypto.wpa_versions != (NL80211_WPA_VERSION_1 | NL80211_WPA_VERSION_2)
		...
		*(mib->mib_WPAPSKValueEnabled) = 0;
		...
		if ((crypto.wpa_versions & NL80211_WPA_VERSION_1) && !(crypto.wpa_versions & NL80211_WPA_VERSION_2)) {
			mib->RSNConfigWPA2->WPA2Enabled = FALSE;
			mib->RSNConfigWPA2->WPA2OnlyEnabled = FALSE;

			memcpy(mib->RSNConfigAuthSuites->AuthSuites, &wpa_key_mgmt, 4);
			mib->RSNConfigAuthSuites->Enabled = TRUE;
		} else if (!(crypto.wpa_versions & NL80211_WPA_VERSION_1) && (crypto.wpa_versions & NL80211_WPA_VERSION_2)) {
			mib->RSNConfigWPA2->WPA2Enabled = TRUE;
			mib->RSNConfigWPA2->WPA2OnlyEnabled = TRUE;

			memcpy(mib->WPA2AuthSuites->AuthSuites, &wpa_key_mgmt, 4);
		...
		return marvell_set_ciphersuite(dev, crypto.wpa_versions, crypto.ciphers_pairwise[0], crypto.cipher_group);
	}

	return 0;
}

static int mwl_cfg80211_start_ap(struct wiphy *wiphy, struct net_device *dev, struct cfg80211_ap_settings *params)
{
	int ret = 0;
	struct wlprivate *priv = mwl_netdev_get_priv(dev);
	u32 beacon_akm = 0;
	...
	if (params->beacon_interval)
		mwl_config_set_bcninterval(priv->master, params->beacon_interval);

	if (params->ssid && params->ssid_len && params->ssid_len <= 32)
		mwl_config_set_essid(dev, params->ssid, params->ssid_len);

	if ((params->crypto.wpa_versions & NL80211_WPA_VERSION_1) && !(params->crypto.wpa_versions & NL80211_WPA_VERSION_2)) {
	...
	if (params->beacon.beacon_ies) {
		...
		if (appie.len)
			memcpy(appie.buf, params->beacon.beacon_ies, params->beacon.beacon_ies_len);
		else {
			appie.len = IE_BUF_LEN;
			memset(appie.buf, 0, IE_BUF_LEN);
		}
	...
	if (params->beacon.proberesp_ies) {
		...
		if (appie.len)
			memcpy(appie.buf, params->beacon.proberesp_ies, params->beacon.proberesp_ies_len);
		else {
			appie.len = IE_BUF_LEN;
			memset(appie.buf, 0, IE_BUF_LEN);
		}
...
static int mwl_cfg80211_stop_ap(struct wiphy *wiphy, struct net_device *dev)
{
	struct mwl_appie appie;
	struct wlprivate *priv = mwl_netdev_get_priv(dev);
	vmacApInfo_t *vmacSta_p = priv->vmacSta_p;
...
static int mwl_cfg80211_connect(struct wiphy *wiphy, struct net_device *dev, struct cfg80211_connect_params *sme)
{
	int ret = 0;
	struct wlprivate *priv = mwl_netdev_get_priv(dev);
	...
	if (sme->channel && sme->channel->center_freq) {
		u8 channel = ieee80211_frequency_to_channel(sme->channel->center_freq);
...
static int mwl_cfg80211_disconnect(struct wiphy *wiphy, struct net_device *dev, u16 reason_code)
{
	...
	if (priv->wdev.current_bss && priv->wdev.iftype == NL80211_IFTYPE_STATION)
		cfg80211_disconnected(dev, 0, NULL, 0, TRUE, GFP_KERNEL);

	if (vmacEntry_p != NULL) {
		vmacStaInfo_t *vStaInfo_p = (vmacStaInfo_t *) vmacEntry_p->info_p;
		memcpy(mlme.im_macaddr, vStaInfo_p->macMgmtMlme_ThisStaData.BssId, ETH_ALEN);
		mlme.im_op = MWL_MLME_DEAUTH;
		mlme.im_reason = reason_code;

		return mwl_config_send_mlme(dev, &mlme);
	} else
		return 0;
}

static int mwl_cfg80211_add_key(struct wiphy *wiphy, struct net_device *netdev,
				u8 key_index, bool pairwise, const u8 * mac_addr, struct key_params *params)
{
	u8 cipher;
	int ret = 0;
	struct wlreq_key *wk = (struct wlreq_key *)wl_kmalloc(sizeof(struct wlreq_key),
							      GFP_KERNEL);
	if (!wk)
		return -ENOMEM;
...
static int mwl_cfg80211_del_key(struct wiphy *wiphy, struct net_device *netdev, u8 key_index, bool pairwise, const u8 * mac_addr)
{
...
static int mwl_cfg80211_set_default_key(struct wiphy *wiphy, struct net_device *netdev, u8 key_index, bool unicast, bool multicast)
{
	return 0;
}

static int mwl_cfg80211_set_default_mgmt_key(struct wiphy *wiphy, struct net_device *netdev, u8 key_index)
{
	return 0;
}

static int mwl_cfg80211_add_station(struct wiphy *wiphy, struct net_device *dev, const u8 * mac, struct station_parameters *params)
{
	return 0;
}

static int mwl_cfg80211_del_station(struct wiphy *wiphy, struct net_device *dev, struct station_del_parameters *params)
{
	struct wlreq_mlme mlme;
	...
	if (params->mac == NULL || is_broadcast_ether_addr(params->mac))
		memset(mlme.im_macaddr, 0xff, ETH_ALEN);
	else {
		memcpy(mlme.im_macaddr, params->mac, ETH_ALEN);
#ifdef MULTI_AP_SUPPORT
		send_event_to_map = 1;
#endif
	}
	...
	if (params->subtype == IEEE80211_STYPE_DISASSOC >> 4)
		mlme.im_op = MWL_MLME_DISASSOC;
	else {
		mlme.im_op = MWL_MLME_DEAUTH;
#ifdef MULTI_AP_SUPPORT
		if (send_event_to_map) {
			snprintf(buf, sizeof(buf), "%s", tag);
			failed_connection_p = (struct mwl_failed_connection *)(buf + strlen(tag));
			memcpy(failed_connection_p->sta_mac_addr, params->mac, ETH_ALEN);
			failed_connection_p->reason = params->reason_code;
			memset(&wreq, 0, sizeof(wreq));
			wreq.data.length = strlen(tag) + sizeof(struct mwl_failed_connection);
			if (dev->flags & IFF_RUNNING)
				wireless_send_event(dev, IWEVCUSTOM, &wreq, buf);
		}
#endif
	}
	mlme.im_reason = params->reason_code;

	return mwl_config_send_mlme(dev, &mlme);
}

static int mwl_cfg80211_change_station(struct wiphy *wiphy, struct net_device *dev, const u8 * mac, struct station_parameters *params)
{
	return 0;
}

static struct mwl_sta_capability sta_capa;
static int mwl_cfg80211_get_station(struct wiphy *wiphy, struct net_device *dev, const u8 * mac, struct station_info *sinfo)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	...
	sinfo->filled |= BIT(NL80211_STA_INFO_TX_BITRATE) | BIT(NL80211_STA_INFO_SIGNAL) | BIT(NL80211_STA_INFO_SIGNAL_AVG);
	sinfo->txrate.legacy = getPhyRate((dbRateInfo_t *) & (pStaInfo->RateInfo));
	sinfo->txrate.nss = getNss((dbRateInfo_t *) & (pStaInfo->RateInfo));
	sinfo->signal = -(pStaInfo->RSSI);
	sinfo->signal_avg = -(pStaInfo->RSSI);

	sinfo->filled |= BIT(NL80211_STA_INFO_RX_BYTES) |
	    BIT(NL80211_STA_INFO_TX_BYTES) | BIT(NL80211_STA_INFO_RX_PACKETS) | BIT(NL80211_STA_INFO_TX_PACKETS);
	...
	sinfo->filled |= BIT(NL80211_STA_INFO_INACTIVE_TIME) | BIT(NL80211_STA_INFO_CONNECTED_TIME);
#ifdef OPENWRT
	...
	memcpy(&sta_capa.htcap.cap, &pStaInfo->HtElem.HTCapabilitiesInfo, sizeof(u16));
	memcpy(&sta_capa.htcap.mcs, pStaInfo->HtElem.SupportedMCSset, 16);
	memcpy(&sta_capa.vhtcap.cap, &pStaInfo->vhtCap.cap, sizeof(u32));
	sta_capa.vhtcap.vht_mcs.rx_mcs_map = pStaInfo->vhtCap.SupportedRxMcsSet & 0xffff;
	sta_capa.vhtcap.vht_mcs.rx_highest = (pStaInfo->vhtCap.SupportedRxMcsSet >> 16) & 0xffff;
	sta_capa.vhtcap.vht_mcs.tx_mcs_map = pStaInfo->vhtCap.SupportedTxMcsSet & 0xffff;
	sta_capa.vhtcap.vht_mcs.tx_highest = (pStaInfo->vhtCap.SupportedTxMcsSet >> 16) & 0xffff;
...
static int mwl_cfg80211_get_key(struct wiphy *wiphy, struct net_device *dev,
				u8 key_index, bool pairwise, const u8 * mac_addr, void *cookie, void (*callback) (void *cookie, struct key_params *))
{
	int ret = 0;
	uint8_t seqnum[6];
...
static int mwl_cfg80211_change_beacon(struct wiphy *wiphy, struct net_device *dev, struct cfg80211_beacon_data *data)
{
	if (data->beacon_ies) {
		struct mwl_appie appie;
		appie.type = MWL_APPIE_FRAMETYPE_BEACON;
		...
		if (appie.len)
			memcpy(appie.buf, data->beacon_ies, data->beacon_ies_len);
		else {
			appie.len = IE_BUF_LEN;
			memset(appie.buf, 0, IE_BUF_LEN);
		}
	...
	if (data->proberesp_ies) {
		...
		if (appie.len)
			memcpy(appie.buf, data->proberesp_ies, data->proberesp_ies_len);
		else {
			appie.len = IE_BUF_LEN;
			memset(appie.buf, 0, IE_BUF_LEN);
		}
...
static int mwl_cfg80211_dump_survey(struct wiphy *wiphy, struct net_device *dev, int idx, struct survey_info *survey)
{
	...
	survey->filled = SURVEY_INFO_TIME | SURVEY_INFO_TIME_BUSY | SURVEY_INFO_TIME_RX | SURVEY_INFO_TIME_TX | SURVEY_INFO_NOISE_DBM;
...
int mwl_cfg80211_vendor_config_wpa(struct net_device *netdev,
				   struct cfg80211_beacon_data beacon, struct cfg80211_crypto_settings crypto, u8 * ssid, size_t ssid_len)
{
	struct wlprivate *priv = mwl_netdev_get_priv(netdev);
	vmacApInfo_t *vmacSta_p = priv->vmacSta_p;
	MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
...
static int mwl_cfg80211_set_ht_caps(struct ieee80211_supported_band *band)
{
...
static int mwl_cfg80211_set_vht_caps(struct ieee80211_supported_band *band)
{
...
int mwl_cfg80211_create(struct wlprivate *wlpptr, struct device *dev)
{
	int ret;
	void *wdev_priv;
	struct wiphy *wiphy;
...
int mwl_cfg80211_rx_mgmt(struct net_device *netdev, void *buf, size_t len, uint8_t rssi)
{
	struct wlprivate *priv = mwl_netdev_get_priv(netdev);
	...
	cfg80211_rx_mgmt(&priv->wdev, priv->channel.center_freq, rssi, buf, len, 0);
...
{
	int i;
	printk("%s - %s - hexdump(len=%lu):\n", "[cfg80211]", title, (unsigned long)len);
	if (buf == NULL) {
		printk(" [NULL]");
	} else {
		for (i = 0; i < len; i++) {
...
int mwl_cfg80211_connect_result_event(struct net_device *netdev, uint8_t * macaddr, uint8_t status)
{
	...
	if (cfg80211_get_bss(wdev->wiphy, NULL, macaddr, wdev->ssid, wdev->ssid_len, IEEE80211_BSS_TYPE_ESS, IEEE80211_PRIVACY_ANY))
		cfg80211_connect_result(netdev, macaddr, NULL, 0, NULL, 0, status, GFP_KERNEL);
