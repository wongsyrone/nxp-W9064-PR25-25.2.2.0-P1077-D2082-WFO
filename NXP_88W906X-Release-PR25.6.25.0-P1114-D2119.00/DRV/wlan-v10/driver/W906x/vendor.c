...
struct nla_policy mwl_vendor_attr_policy[NUM_MWL_VENDOR_ATTR] = {
	...
	[MWL_VENDOR_ATTR_MLME] = {.type = NLA_BINARY,.len = sizeof(struct mwl_mlme)},
#ifdef BAND_STEERING
	...
	[MWL_VENDOR_ATTR_ASSOC_REQ_FRAME] = {.type = NLA_BINARY,.len = sizeof(struct assoc_req_msg_t)},
	[MWL_VENDOR_ATTR_STATION_INFO] = {.type = NLA_BINARY,.len = sizeof(struct station_info)},
	[MWL_VENDOR_ATTR_HT_CAPA] = {.type = NLA_BINARY,.len = sizeof(IEEEtypes_HT_Element_t)}
	,
	[MWL_VENDOR_ATTR_VHT_CAPA] = {.type = NLA_BINARY,.len = sizeof(IEEEtypes_VhtCap_t)}
	,
	[MWL_VENDOR_ATTR_HE_CAPA] = {.type = NLA_BINARY,.len = sizeof(HE_Capabilities_IE_t)}
	,
	...
	,
	[MWL_VENDOR_ATTR_ESPI] = {.type = NLA_BINARY,.len = sizeof(struct mwl_espi_info)},
	[MWL_VENDOR_ATTR_UNASSOCIATED_STA_LINK_METRICS_QUERY] = {.type = NLA_BINARY,.len = sizeof(struct unassociated_sta_link_metrics_query)},
	[MWL_VENDOR_ATTR_UNASSOCIATED_STA_LINK_METRICS_RESP] = {.type = NLA_BINARY,.len = UNASSOC_RESP_SIZE},
	[MWL_VENDOR_ATTR_AP_RADIO_BASIC_CAPABILITIES] = {.type = NLA_BINARY,.len = AP_RADIO_BASIC_CAPA_SIZE},
};

static int mwl_vendor_cmd_commit(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	return mwl_config_commit(wdev->netdev);
}

static int mwl_vendor_cmd_send_mlme(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	int ret;
	...
	ret = nla_parse(tb, MAX_MWL_VENDOR_ATTR, data, data_len, mwl_vendor_attr_policy);
#else
	ret = nla_parse(tb, MAX_MWL_VENDOR_ATTR, data, data_len, mwl_vendor_attr_policy, NULL);
#endif
	if (ret)
		return ret;
...
static int mwl_vendor_cmd_set_appie(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	int ret;
	struct nlattr *tb[NUM_MWL_VENDOR_ATTR];
...
int mwl_drv_send_mgmt(struct net_device *netdev, struct mwl_mgmt *mgmt)
{
	...
	if (memcmp(wlanMsg_p->Hdr.Addr1, vmacSta_p->macBssId, 6) == 0 || memcmp(wlanMsg_p->Hdr.Addr1, bcastMacAddr, 6) == 0) {
		struct sk_buff *skb = NULL;

		if ((mgmt->len <= 24) || (mgmt->len > 1024)) {
			return -EINVAL;
		}
...
static int mwl_vendor_cmd_set_bandsteer(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	...
	if (value == 1) {
		...
		*(mib->mib_bandsteer_handler) = BAND_STEERING_HDL_BY_HOST;
		...
		if ((*(mib->mib_bandsteer) == 0) || ((*(mib->mib_bandsteer) == 1) && (*(mib->mib_bandsteer_handler) == BAND_STEERING_HDL_BY_HOST))) {
			*(mib->mib_bandsteer) = value;
...
static int mwl_vendor_cmd_send_mgmt(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	int ret;
	struct nlattr *tb[NUM_MWL_VENDOR_ATTR];
...
#ifdef IEEE80211K
static int mwl_vendor_cmd_set_rrm(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	struct wlprivate *priv = mwl_netdev_get_priv(wdev->netdev);
	vmacApInfo_t *vmacSta_p = priv->vmacSta_p;
	MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
...
static int mwl_vendor_cmd_get_rrm(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
...
#ifdef AUTOCHANNEL
static int mwl_vendor_cmd_do_acs(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
...
static int mwl_vendor_cmd_get_multiap(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	struct sk_buff *skb;
	uint8_t multiap;
...
static int mwl_vendor_cmd_set_wpawpa2mode(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	struct wlprivate *priv = mwl_netdev_get_priv(wdev->netdev);
	vmacApInfo_t *vmacSta_p = priv->vmacSta_p;
	MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
...
static int mwl_vendor_cmd_set_ssid(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	struct net_device *netdev = wdev->netdev;
	struct nlattr *tb[NUM_MWL_VENDOR_ATTR];
	uint8_t *ssid;
...
static int mwl_vendor_cmd_get_ssid(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	struct sk_buff *skb;
	struct wlprivate *priv = mwl_netdev_get_priv(wdev->netdev);
	vmacApInfo_t *vmacSta_p = priv->vmacSta_p;
...
static int mwl_vendor_cmd_config_wpa(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	struct net_device *netdev = wdev->netdev;
	struct nlattr *tb[NUM_MWL_VENDOR_ATTR];
	...
	ap_settings = (struct mwl_ap_settings *)nla_data(tb[MWL_VENDOR_ATTR_WPA]);
	...
	memcpy(&crypto, &ap_settings->crypto, sizeof(struct mwl_crypto_settings));

	mwl_cfg80211_vendor_config_wpa(netdev, beacon, crypto, ap_settings->ssid, ap_settings->ssid_len);
	mib_Update();
	return 0;
}

extern void fillApCapInfo(struct net_device *netdev, int mode, u8 * pApCap, u32 * pLen);
static int mwl_vendor_cmd_get_band_capa(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	struct wlprivate *priv = mwl_netdev_get_priv(wdev->netdev);
	vmacApInfo_t *vmacSta_p = priv->vmacSta_p;
	MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
	...
	skb = cfg80211_vendor_cmd_alloc_reply_skb(wiphy,
						  10 + sizeof(IEEEtypes_HT_Element_t) + sizeof(IEEEtypes_VhtCap_t) + sizeof(HE_Capabilities_IE_t));
	if (skb == NULL)
		return -ENOMEM;
	...
	if (*(mib->mib_ApMode) & AP_MODE_11AC) {
		fillApCapInfo(wdev->netdev, 1, (u8 *) & vht_cap, (u32 *) & vht_cap_len);
		if (nla_put(skb, MWL_VENDOR_ATTR_VHT_CAPA, sizeof(IEEEtypes_VhtCap_t), &vht_cap)) {
			kfree_skb(skb);
			return -EMSGSIZE;
		}
	}
	...
	if (*(mib->mib_ApMode) & AP_MODE_11AX) {
		fillApCapInfo(wdev->netdev, 2, (u8 *) & he_cap, (u32 *) & he_cap_len);
		if (nla_put(skb, MWL_VENDOR_ATTR_HE_CAPA, sizeof(HE_Capabilities_IE_t), &he_cap)) {
			kfree_skb(skb);
			return -EMSGSIZE;
		}
	}
...
static int mwl_vendor_cmd_get_channel_util(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	//struct wlprivate *priv = mwl_netdev_get_priv(wdev->netdev);
	//vmacApInfo_t *vmacSta_p = priv->vmacSta_p;
	struct sk_buff *skb;
	...
	if (nla_put_u8(skb, MWL_VENDOR_ATTR_UTILIZATION, util) ||
	    ...
	    nla_put_u8(skb, MWL_VENDOR_ATTR_UTIL_RX_OTHER, util_rx_other) || nla_put_u8(skb, MWL_VENDOR_ATTR_UTIL_TX, util_tx)) {
		kfree_skb(skb);
		return -EMSGSIZE;
	}
...
extern void fillStaInfo(struct net_device *netdev, struct station_info *pSinfo, struct extStaDb_StaInfo_t *pStaInfo);
static int mwl_vendor_cmd_get_station_info(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	struct wlprivate *priv = mwl_netdev_get_priv(wdev->netdev);
	vmacApInfo_t *vmacSta_p = priv->vmacSta_p;
	struct sk_buff *skb;
	...
	memcpy((void *)macaddr, (const void *)nla_data(tb[MWL_VENDOR_ATTR_MAC_ADDRESS]), ETH_ALEN);
	...
	skb = cfg80211_vendor_cmd_alloc_reply_skb(wiphy, 10 + ETH_ALEN + sizeof(struct station_info) + 8);
	if (skb == NULL)
		return -ENOMEM;

	if (nla_put(skb, MWL_VENDOR_ATTR_MAC_ADDRESS, ETH_ALEN, macaddr) ||
	    nla_put(skb, MWL_VENDOR_ATTR_STATION_INFO, sizeof(struct station_info), &sinfo) ||
	    nla_put_u8(skb, MWL_VENDOR_ATTR_UTIL_TX, util_tx) || nla_put_u8(skb, MWL_VENDOR_ATTR_UTIL_RX, util_rx)) {
		kfree_skb(skb);
		return -EMSGSIZE;
	}
...
static int mwl_vendor_cmd_get_espi(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	//struct wlprivate *priv = mwl_netdev_get_priv(wdev->netdev);
	//vmacApInfo_t *vmacSta_p = priv->vmacSta_p;
	struct sk_buff *skb;
	...
	skb = cfg80211_vendor_cmd_alloc_reply_skb(wiphy, 10 + sizeof(struct mwl_espi_info));
	if (skb == NULL)
		return -ENOMEM;
	...
	if (nla_put(skb, MWL_VENDOR_ATTR_ESPI, sizeof(struct mwl_espi_info), &espi_info)) {
		nlmsg_free(skb);
		return -EMSGSIZE;
	}
...
void mwl_vendor_cmd_get_unassociated_sta_timeout(struct net_device *netdev)
{
	struct wlprivate *priv = mwl_netdev_get_priv(netdev);
	...
	TimerFireInByJiffies(&priv->wlpd_p->unassocSTA.waitTimer, 1, &mwl_vendor_cmd_get_unassociated_sta_timeout, (UINT8 *) netdev, 200 * TIMER_1MS);
	return;
...
static int mwl_vendor_cmd_get_unassociated_sta_link_metrics(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	struct wlprivate *priv = mwl_netdev_get_priv(wdev->netdev);
	...
	query = wl_kzalloc(sizeof(struct unassociated_sta_link_metrics_query), GFP_ATOMIC);
	if (!query)
		return -ENOMEM;

	memcpy((void *)query,
	       (const void *)nla_data(tb[MWL_VENDOR_ATTR_UNASSOCIATED_STA_LINK_METRICS_QUERY]), sizeof(struct unassociated_sta_link_metrics_query));

	/* Pre-check query data */
	if ((query->num_of_channel == 0) || (query->num_of_channel > UNASSOC_METRICS_CHANNEL_MAX)) {
		wl_kfree(query);
		return -EINVAL;
	}

	for (i = 0; i < query->num_of_channel; i++) {
		if (query->unassociated_sta_info[i].num_of_sta > UNASSOC_METRICS_STA_MAX) {
			wl_kfree(query);
			return -EINVAL;
		}
	}
	...
	Jiffies = ((*(mib->mib_unassocsta_track_time) + 300) * query->num_of_channel) * TIMER_1MS;
	...
	*(mib->mib_unassocsta_track_enabled) = 0;
	...
	TimerFireInByJiffies(&priv_master->wlpd_p->unassocSTA.waitTimer, 1, &mwl_vendor_cmd_get_unassociated_sta_timeout, (UINT8 *) netdev, Jiffies);

	return 0;
}

extern void fillApRadioInfo(struct net_device *netdev, struct ap_radio_basic_capa_rpt_t *pRpt);
static int mwl_vendor_cmd_get_ap_radio_basic_capa(struct wiphy *wiphy, struct wireless_dev *wdev, const void *data, int data_len)
{
	//struct wlprivate *priv = mwl_netdev_get_priv(wdev->netdev);
	//vmacApInfo_t *vmacSta_p = priv->vmacSta_p;
	struct sk_buff *skb;
	...
	skb = cfg80211_vendor_cmd_alloc_reply_skb(wiphy, 10 + AP_RADIO_BASIC_CAPA_SIZE);
	if (skb == NULL)
		return -ENOMEM;
	...
	if (nla_put(skb, MWL_VENDOR_ATTR_AP_RADIO_BASIC_CAPABILITIES, AP_RADIO_BASIC_CAPA_SIZE, ap_radio_basic_capa)) {
...
static const struct nl80211_vendor_cmd_info mwl_vendor_events[] = {
	...
	{
	 .vendor_id = MRVL_OUI,
	 .subcmd = MWL_VENDOR_EVENT_MULTIAP_IE,
	 },
