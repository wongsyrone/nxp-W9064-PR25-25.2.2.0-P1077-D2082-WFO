 ...
 */
static inline void *mwl_cfg80211_get_priv(struct wiphy *wiphy)
{
 ...
 */
static inline struct wlprivate *mwl_netdev_get_priv(struct net_device *netdev)
{
	return (struct wlprivate *)netdev_priv(netdev);
}

static inline struct wlprivate *mwl_netdev_get_master_priv(struct net_device *netdev)
{
	struct wlprivate *priv = netdev_priv(netdev);

	if (priv->master)
		return (struct wlprivate *)netdev_priv(priv->master);
	else
		return (struct wlprivate *)netdev_priv(netdev);
}
...
int mwl_send_vendor_neighbor_event(struct net_device *netdev, void *mgmt, size_t len, uint8_t rssi);
