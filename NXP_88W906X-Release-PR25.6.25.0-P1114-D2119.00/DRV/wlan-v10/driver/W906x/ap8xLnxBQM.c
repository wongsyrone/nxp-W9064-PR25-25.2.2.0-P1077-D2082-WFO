...
#define SYSFS_RX_DROP_MGMT_SKB_NUM    10
...
static void wlget_extmembuf(struct wlprivate *wlpptr, struct _ext_membuf *pext_membuf);
static void wlclean_extmembuf(struct wlprivate *wlpptr, struct _ext_membuf *pext_membuf);
...
void wl_txdatpkt_stat_per_sta(struct net_device *netdev, extStaDb_StaInfo_t * pStaInfo, struct sk_buff *skb, U16 llc_type);
void wl_txdatpkt_bcast_mcast(struct net_device *netdev, ether_hdr_t * eh, struct sk_buff *skb);

extern void wlmon_log_bmq_buff_refill(struct net_device *netdev, UINT32 qid, UINT32 refill_cnt);
...
//u32 qidcnt[3] = { 0, 0, 0 };
/** private data **/
static const u_int32_t rx_q_size[SC5_RXQ_NUM] = { SC5_RXQ_SIZE, SC5_RXQ_SIZE, SC5_RXQ_SIZE, SC5_RXQ_SIZE, SC5_RXQ_SIZE,
	SC5_RXQ_SIZE, SC5_RXQ_SIZE, SC5_RXQ_SIZE, SC5_PROMQ_SIZE, SC5_RMGTQ_SIZE
};

static const u_int32_t scbt_tx_q_size[SCBT_TXQ_NUM] = { SC5_TXQ_SIZE, SC5_TXQ_SIZE, SC5_TXQ_SIZE, SC5_TXQ_SIZE };
static const u_int32_t sc5_tx_q_size[SC5_TXQ_NUM] = { SC5_TXQ_SIZE, SC5_TXQ_SIZE };
static const u_int32_t bm_q_size[SC5_BMQ_NUM] = { SC5_BMQ_SIZE, SC5_BMQ_SIZE, SC5_BMQ_SIZE, SC5_BMQ13_SIZE };
static const u_int32_t scbt_relbuf_q_size[SCBT_BMQ_RELEASE_NUM] = { SC5_RELQ_SIZE, SC5_RELQ_SIZE, SC5_RELQ_SIZE, SC5_RELQ_SIZE };
const u_int32_t buf_pool_size[SC5_BMQ_NUM] = { 0x1000, 0x3000, 0x6400, BUF_8K };	// 4k, 12k, 25k 8k,
const u_int32_t buf_pool_max_entries[SC5_BMQ_NUM] = { 0x6000, 0x3000, 0x1000, 0 };
static const u_int32_t sc5_relbuf_q_size[SC5_BMQ_RELEASE_INDEX] = { SC5_RELQ_SIZE };
...
BOOLEAN wlSQIndexGet(wl_qpair_sq_t * sq)
{
...
BOOLEAN wlSQIndexPut(wl_qpair_sq_t * sq)
{
...
BOOLEAN wlRQIndexGet(wl_qpair_rq_t * rq)
{
...
BOOLEAN wlRQIndexPut(u16 qid, wl_qpair_rq_t * rq)
{
	...
	WLDBG_WARNING(DBG_LEVEL_0, "QINDEX: Put RQ(%d) is full (rdinx, wrinx) = (%d, %d)\n", qid, rq->rdinx, rq->wrinx);
	return FALSE;
}

static inline u32 wlGetDescSize(struct bqm_args *pbqm_args, u16 qid, int qoff)
{
	u32 qelm_size;

	// Input parameter checking
	if ((NUM_OF_HW_DESCRIPTOR_DATA <= qid) || (max_qpair <= (QPAIR) qoff) || (pbqm_args == NULL)) {
...
u32 wlQueryRdPtr(struct net_device * netdev, int qid, int qoff)
{
	...
	WLDBG_INFO(DBG_LEVEL_5, "QINDEX(%d), desc_size from wlGetDescSize() = %d\n", qid, desc_size);
	...
	if (qoff == SC5_RQ) {
		...
		qsize = wlqm->rq.qsize;
		WLDBG_INFO(DBG_LEVEL_5, "QINDEX: RQ(%d),r_reg(ioBase1+%xh) = %xh\n", qid, SC5_RQ_RDPTR_REG(qid), regval);
	} else {
		orig_rdinx = wlqm->sq.rdinx;
		regval = readl(wlpptr->ioBase1 + SC5_SQ_RDPTR_REG(qid));
		qsize = wlqm->sq.qsize;
		WLDBG_INFO(DBG_LEVEL_5, "QINDEX: SQ(%d),r_reg(ioBase1+%xh) = %xh\n", qid, SC5_SQ_RDPTR_REG(qid), regval);
	}
	...
	WLDBG_INFO(DBG_LEVEL_5, "QINDEX:  Queue(%d), rdinx=%d, from regval(%xh)\n", qid, ((regval & 0xffff0000) >> 12) / desc_size, regval);

	// Make sure index is valid
	idx_val = ((regval & 0xffff0000) >> 12) / desc_size;
	if (idx_val >= qsize) {
		WLDBG_ERROR(DBG_LEVEL_0, "Incorrect %s(%d) rd_idx = %u", ((qoff == SC5_RQ) ? "RQ" : "SQ"), qid, idx_val);
	}
...
BOOLEAN CheckIndexReady(struct net_device * netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	u32 loopcnt = 0;
	u32 regval;
	...
	while (loopcnt++ < 1000) {
		regval = *(u32 *) (&((SMAC_CTRL_BLK_st *) wlpptr->ioBase0)->sfw.ready);
	...
	if (regval != 1) {
		WLDBG_ERROR(DBG_LEVEL_0, "Failed to get H/W Index(SMAC_CTRL_BLK_st + 0x800) ready, regval=%xh\n", regval);
		return TRUE;
	}
...
void wlUpdateRdPtr(struct net_device *netdev, int qid, int qoff, u32 rdinx, bool is_init)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct bqm_args *pbqm_args = &wlpd_p->bmq_args;
	...
	if (qoff == SC5_RQ) {
		...
		WLDBG_INFO(DBG_LEVEL_5, "QINDEX(%d) SQ: w_reg(%p)=%xh\n", qid, (wlpptr->ioBase1 + SC5_SQ_RDPTR_REG(qid)), ptval);
	}

	return;
}

u32 wlQueryWrPtr(struct net_device * netdev, int qid, int qoff)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct bqm_args *pbqm_args = &wlpd_p->bmq_args;
...
void wlUpdateWrPtr(struct net_device *netdev, int qid, int qoff, u32 wrinx, bool is_init)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct bqm_args *pbqm_args = &wlpd_p->bmq_args;
...
BOOLEAN wlSQEmpty(struct net_device * netdev, u8 qid)
{
...
BOOLEAN wlSQFull(struct net_device * netdev, u8 qid)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wldesc_data *wlqm;
...
BOOLEAN wlRQFull(struct net_device * netdev, u8 qid)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wldesc_data *wlqm;

	wlqm = &wlpptr->wlpd_p->descData[qid];
	WLDBG_INFO(DBG_LEVEL_5, "QINDEX(%d), RQ(rdinx, wrinx)=(%d, %d)\n", qid, wlqm->rq.rdinx, wlqm->rq.wrinx);
...
void wlRxQueueCleanUp(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wldesc_data *wlqm;
	int qid;

	for (qid = SC5_RXQ_START_INDEX; qid < (SC5_RXQ_START_INDEX + SC5_RXQ_NUM); qid++) {
		...
		if (wlqm->sq.virt_addr != NULL) {
			wl_dma_free_coherent(wlpptr->wlpd_p->dev, (wlqm->sq.qsize * sizeof(wlrxdesc_t)), wlqm->sq.virt_addr, wlqm->sq.phys_addr);

			wlqm->sq.virt_addr = NULL;
		}
		// reset descruptor queue size.
		wlqm->sq.qsize = 0;
			writel(wlqm->sq.qsize, (wlpptr->ioBase1 + SC5_Q_SIZE_REG(qid, SC5_SQ)));
...
int wlRxQueueInit(struct net_device *netdev, int qid)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wldesc_data *wlqm;
	int i;
	...
	wlqm->sq.virt_addr = wl_dma_alloc_coherent(wlpptr->wlpd_p->dev,
						   (wlqm->sq.qsize * sizeof(wlrxdesc_t)), &wlqm->sq.phys_addr, wlpptr->wlpd_p->dma_alloc_flags);
	WLDBG_INFO(DBG_LEVEL_0, "RXQ(%d), sq (v, p)=(%p, %llxh) from dma_alloc_consistent\n", qid, wlqm->sq.virt_addr, wlqm->sq.phys_addr);
	...
	if (qid == 0 || qid == 8 || qid == 9) {
		for (cfh_ul = (wlrxdesc_t *) wlqm->sq.virt_addr; i < wlqm->sq.qsize; cfh_ul++, i++) {
	...
	WLDBG_INFO(DBG_LEVEL_0, "RXQ(%d) qid = %d, CFH base = 0x%X \n", qid, qid, wlqm->sq.phys_addr);
	// set CFH-UL descruptor queue size and start address.
	writel((u32) wlqm->sq.phys_addr, (wlpptr->ioBase1 + SC5_Q_BASE_ADDR_REG(qid, SC5_SQ)));
	WLDBG_INFO(DBG_LEVEL_0, "RXQ(%d), SC5_Q_BASE_ADDR_REG w_reg(ioBase1+%xh) = %xh\n", qid,
		   SC5_Q_BASE_ADDR_REG(qid, SC5_SQ), (u32) wlqm->sq.phys_addr);
	// high address
	writel(smac_buf_hi_addr, (wlpptr->ioBase1 + SC5_Q_BASE_ADDR_REG(qid, SC5_SQ) + 4));
	WLDBG_INFO(DBG_LEVEL_0, "RXQ(%d),w_reg(ioBase1+%xh) = %xh\n", qid, (SC5_Q_BASE_ADDR_REG(qid, SC5_SQ) + 4), smac_buf_hi_addr);

	writel(((wlqm->sq.qsize * sizeof(wlrxdesc_t)) / 128) << 3, (wlpptr->ioBase1 + SC5_Q_SIZE_REG(qid, SC5_SQ)));
	WLDBG_INFO(DBG_LEVEL_0, "RXQ(%d) SQ, SC5_Q_SIZE_REG w_reg(ioBase1+%xh) = %xh, qsize=%d, sizeof(wlrxdesc_t)=%d\n",
		   qid, SC5_Q_SIZE_REG(qid, SC5_SQ), ((wlqm->sq.qsize * sizeof(wlrxdesc_t)) / 128) << 3, wlqm->sq.qsize, sizeof(wlrxdesc_t));

	return SUCCESS;
}

int wlTxQueueInit(struct net_device *netdev, int qid)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct bqm_args *pbqm_args = &wlpd_p->bmq_args;
	...
	wlqm->rq.virt_addr = wl_dma_alloc_coherent(wlpd_p->dev, (wlqm->rq.qsize * sizeof(wltxdesc_t)), &wlqm->rq.phys_addr, wlpd_p->dma_alloc_flags);
	...
	WLDBG_INFO(DBG_LEVEL_0, "TXQ(%d), RQ (v, p)=(%p, %llxh) from dma_alloc_consistent\n", qid, wlqm->rq.virt_addr, wlqm->rq.phys_addr);
	...
	WLDBG_INFO(DBG_LEVEL_0, "TXQ(%d) qid = %d, CFH base = 0x%X \n", qid, qid, wlqm->rq.phys_addr);

	// set CFH-DL descruptor queue size and start address.
	writel((u32) wlqm->rq.phys_addr, (wlpptr->ioBase1 + SC5_Q_BASE_ADDR_REG(qid, SC5_RQ)));
	WLDBG_INFO(DBG_LEVEL_0, "TXQ(%d),RQ, SC5_Q_BASE_ADDR_REG w_reg(ioBase1+%xh) = %xh\n", qid,
		   SC5_Q_BASE_ADDR_REG(qid, SC5_RQ), (u32) wlqm->rq.phys_addr);
	// high address
	writel(smac_buf_hi_addr, (wlpptr->ioBase1 + SC5_Q_BASE_ADDR_REG(qid, SC5_RQ)) + 4);
	WLDBG_INFO(DBG_LEVEL_0, "TXQ(%d),w_reg(ioBase1+%xh) = %xh\n", qid, (SC5_Q_BASE_ADDR_REG(qid, SC5_RQ) + 4), smac_buf_hi_addr);
	writel(((wlqm->rq.qsize * sizeof(wltxdesc_t)) / 128) << 3, (wlpptr->ioBase1 + SC5_Q_SIZE_REG(qid, SC5_RQ)));
	WLDBG_INFO(DBG_LEVEL_0, "TXQ(%d),RQ, SC5_Q_SIZE_REG w_reg(ioBase1+%xh) = %xh, qsize=%d, sizeof(wltxdesc_t)=%d\n",
		   qid, SC5_Q_SIZE_REG(qid, SC5_RQ), ((wlqm->rq.qsize * sizeof(wltxdesc_t)) / 128) << 3, wlqm->rq.qsize, sizeof(wltxdesc_t));

	return SUCCESS;
}

void wlTxQueueCleanUp(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct bqm_args *pbqm_args = &wlpd_p->bmq_args;
	...
	for (qid = pbqm_args->txq_start_index; qid < (pbqm_args->txq_start_index + pbqm_args->txq_num); qid++) {
		wlqm = &wlpptr->wlpd_p->descData[qid];

		if (wlqm->rq.virt_addr != NULL) {
			wl_dma_free_coherent(wlpd_p->dev, (wlqm->rq.qsize * sizeof(wltxdesc_t)), wlqm->rq.virt_addr, wlqm->rq.phys_addr);

			wlqm->rq.virt_addr = NULL;
		}
		// reset descruptor queue size.
		wlqm->rq.qsize = 0;
			writel(wlqm->rq.qsize, (wlpptr->ioBase1 + SC5_Q_SIZE_REG(qid, SC5_RQ)));
...
int wlBmQueueInit(struct net_device *netdev, int qid)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct wldesc_data *wlqm;
	...
	if (qid == (SC5_BMQ_START_INDEX + SC5_BMQ_NUM - 1)) {	// Last queue => buffer for L0/L1
		wlqm->rq.qsize = wlpd_p->ext_membuf[1].buf_pool_size / buf_pool_size[qid - SC5_BMQ_START_INDEX];
	} else {
		wlqm->rq.qsize = bm_q_size[((U8) (qid - SC5_BMQ_START_INDEX) % SC5_BMQ_NUM)];
	}
	WLDBG_INFO(DBG_LEVEL_0, "BMQ[%d] size = %d\n", qid, wlqm->rq.qsize);

	wlqm->rq.virt_addr = wl_dma_alloc_coherent(wlpptr->wlpd_p->dev,
						   (wlqm->rq.qsize * sizeof(bm_pe_hw_t)), &wlqm->rq.phys_addr, wlpptr->wlpd_p->dma_alloc_flags);
	WLDBG_INFO(DBG_LEVEL_0, "BMQ(%d), RQ (v, p)=(%p, %llxh) from dma_alloc_consistent\n", qid, wlqm->rq.virt_addr, wlqm->rq.phys_addr);
	...
	for (i = 0, ppehw = (bm_pe_hw_t *) wlqm->rq.virt_addr; i < wlqm->rq.qsize; i++, ppehw++) {
	...
	writel((u32) wlqm->rq.phys_addr, (wlpptr->ioBase1 + SC5_Q_BASE_ADDR_REG(qid, SC5_RQ)));
	WLDBG_INFO(DBG_LEVEL_0, "BMQ(%d),RQ, SC5_Q_BASE_ADDR_REG w_reg(ioBase1+%llxh) = %xh\n", qid,
		   SC5_Q_BASE_ADDR_REG(qid, SC5_RQ), (u64) wlqm->rq.phys_addr);
	...
	//
	writel(((wlqm->rq.qsize * sizeof(bm_pe_hw_t)) / 128) << 3, (wlpptr->ioBase1 + SC5_Q_SIZE_REG(qid, SC5_RQ)));
	WLDBG_INFO(DBG_LEVEL_0, "BMQ(%d),RQ, SC5_Q_SIZE_REG w_reg(ioBase1+%xh) = %xh, qsize=%d, sizeof(bm_pe_hw_t)=%d\n",
		   qid, SC5_Q_SIZE_REG(qid, SC5_RQ), ((wlqm->rq.qsize * sizeof(bm_pe_hw_t)) / 128) << 3, wlqm->rq.qsize, sizeof(bm_pe_hw_t));

	// update rdinx and wrinx in wlRxBufInit
	wlRxBufInit(netdev, qid);
	wlpptr->smacconfig.bpReqInfo[qid - SC5_BMQ_START_INDEX].size = wlqm->rq.bm.buf_size / 16;
...
void wlBmBufDump(struct net_device *netdev, int qid)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wldesc_data *wlqm;
	struct sk_buff *skb;
	...
	while (rdinx != wlqm->rq.wrinx) {
		WLDBG_INFO(DBG_LEVEL_4, "rdinx %d  virt_addr %x phys_addr %x skb %p skb->data %p\n", rdinx,
			   ENDIAN_SWAP32(pe_hw->pe1_lo_dword_addr), ENDIAN_SWAP32(pe_hw->pe0_lo_dword_addr), skb, skb->data);
...
void wlReleaseBufDump(struct net_device *netdev, int qid)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wldesc_data *wlqm;
	...
	while (rdinx != wlqm->sq.wrinx) {
		WLDBG_INFO(DBG_LEVEL_4, "rdinx %d  virt_addr %x phys_addr %x\n", rdinx,
			   ENDIAN_SWAP32(pe_hw->pe1_lo_dword_addr), ENDIAN_SWAP32(pe_hw->pe0_lo_dword_addr));
		rdinx = (rdinx + 1) % wlqm->sq.qsize;
		pe_hw = (bm_pe_hw_t *) (wlqm->sq.virt_addr + rdinx * sizeof(bm_pe_hw_t));
	}			// end of while
}

void wlRxBufCleanUp(struct net_device *netdev, int qid)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct wldesc_data *wlqm;
...
int wlRxBufInit(struct net_device *netdev, int qid)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	...
	pe = (bm_pe_t *) wl_kmalloc((wlqm->rq.qsize * sizeof(bm_pe_t)), GFP_KERNEL);
	if (!pe) {
		WLDBG_ERROR(DBG_LEVEL_0, "PE memory allocation for qid(%d) fail \n");
		return -ENOMEM;
	}
	wlqm->rq.bm.pe = pe;
	...
	for (entry = 0; entry < wlqm->rq.qsize; entry++) {
		...
		if (qid != (SC5_BMQ_START_INDEX + SC5_BMQ_NUM - 1)) {
			...
			if (!skb) {
				wlpd_p->drv_stats_val.bmqbuf_alloc_fail_cnt[qid - SC5_BMQ_START_INDEX]++;
				return -ENOMEM;
			}
			skb_queue_tail(&wlpd_p->pend_skb_trace[PENDSKB_RX], skb);
			skb->dev = netdev;
			...
			*((u32 *) (skb->data + size + SKB_TAIL_SIGNATURE_OFFSET)) = SKB_TAIL_SIGNATURE;

			buff_phy_addr = dma_map_single(wlpptr->wlpd_p->dev, skb->data, size, DMA_FROM_DEVICE);
		...
		if (qid != (SC5_BMQ_START_INDEX + SC5_BMQ_NUM - 1)) {
			...
			buf_virt_addr += buf_pool_size[qid - SC5_BMQ_START_INDEX];
			buff_phy_addr += buf_pool_size[qid - SC5_BMQ_START_INDEX];
		}
...
void wlBmQueueCleanUp(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wldesc_data *wlqm;
	int qid;

	for (qid = SC5_BMQ_START_INDEX; qid < (SC5_BMQ_START_INDEX + SC5_BMQ_NUM); qid++) {
		wlqm = &wlpptr->wlpd_p->descData[qid];
		wlRxBufCleanUp(netdev, qid);

		if ((void *)wlqm->rq.virt_addr != NULL) {
			wl_dma_free_coherent(wlpptr->wlpd_p->dev,
					     (wlqm->rq.qsize * sizeof(bm_pe_hw_t)), (void *)wlqm->rq.virt_addr, wlqm->rq.phys_addr);
...
int wlBufReleaseQueueInit(struct net_device *netdev, int qid)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct bqm_args *pbqm_args = &wlpd_p->bmq_args;
	...
	wlqm = &wlpptr->wlpd_p->descData[qid];
	wlqm->id = qid;
	wlqm->sq.qsize = pbqm_args->relbuf_q_size[qid - pbqm_args->bmq_release_index];

	wlqm->sq.virt_addr = wl_dma_alloc_coherent(wlpd_p->dev, (wlqm->sq.qsize * sizeof(bm_pe_hw_t)), &wlqm->sq.phys_addr, wlpd_p->dma_alloc_flags);
	WLDBG_INFO(DBG_LEVEL_0, "BMQ(%d), sq(%d) (v, p)=(%p, %llxh) from dma_alloc_consistent\n", qid, wlqm->sq.virt_addr, wlqm->sq.phys_addr);

	if ((void *)wlqm->sq.virt_addr == NULL) {
		WLDBG_ERROR(DBG_LEVEL_0, "Qid %d no valid Buffer Release Pool Mem \n", qid);
		return FAIL;
	}
	// Init the signature to the buffer desc
	for (i = 0, ppehw = (bm_pe_hw_t *) wlqm->sq.virt_addr; i < wlqm->sq.qsize; i++, ppehw++) {
		ppehw->bgn_signature = ppehw->end_signature = BMBUF_SIGNATURE;
	}
	...
	writel(wlqm->sq.phys_addr, (wlpptr->ioBase1 + SC5_Q_BASE_ADDR_REG(qid, SC5_SQ)));
	WLDBG_INFO(DBG_LEVEL_0, "BMQ(%d), SQ, SC5_Q_BASE_ADDR_REG(%d): %p, phyaddr=%llxh\n", qid,
		   (wlpptr->ioBase1 + SC5_Q_BASE_ADDR_REG(qid, SC5_SQ)), (u64) wlqm->sq.phys_addr);

	// high address
	writel(smac_buf_hi_addr, (wlpptr->ioBase1 + SC5_Q_BASE_ADDR_REG(qid, SC5_SQ) + 4));
	WLDBG_INFO(DBG_LEVEL_0, "BMQ(%d),w_reg(ioBase1+%xh) = %xh\n", qid, (SC5_Q_BASE_ADDR_REG(qid, SC5_SQ) + 4), smac_buf_hi_addr);

	writel(((wlqm->sq.qsize * sizeof(bm_pe_hw_t)) / 128) << 3, (wlpptr->ioBase1 + SC5_Q_SIZE_REG(qid, SC5_SQ)));
	WLDBG_INFO(DBG_LEVEL_0, "BMQ(%d),SQ, SC5_Q_SIZE_REG w_reg(ioBase1+%xh) = %xh, qsize=%d, sizeof(bm_pe_hw_t)=%d\n", qid,
		   SC5_Q_SIZE_REG(qid, SC5_SQ), ((wlqm->sq.qsize * sizeof(wlrxdesc_t)) / 128) << 3, wlqm->sq.qsize, sizeof(bm_pe_hw_t));

	/* set TX DONE interrupt threshold and timeout */
	writel(SC5_TXDONE_INT_THRESHOLD, (wlpptr->ioBase1 + SC5_Q_THRES_REG(qid, SC5_SQ)));
	WLDBG_INFO(DBG_LEVEL_0, "BMQ(%d),w_reg(ioBase1+%xh) = %xh\n", qid, SC5_Q_THRES_REG(qid, SC5_SQ), 0x1);

	writel(SQ5_MAC_CTRL_TIMEOUT(SC5_TXDONE_INT_TIMEOUT), (wlpptr->ioBase1 + SC5_Q_MAC_CTRL_REG(qid, SC5_SQ)));
	WLDBG_INFO(DBG_LEVEL_0, "BMQ(%d),w_reg(ioBase1+%xh) = %xh\n", qid, SC5_Q_MAC_CTRL_REG(qid, SC5_SQ), SQ5_MAC_CTRL_TIMEOUT(0x10));

	return SUCCESS;

}

void wlBufReleaseQueueCleanUp(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct bqm_args *pbqm_args = &wlpd_p->bmq_args;
	...
	for (qid = pbqm_args->bmq_release_index; qid < (pbqm_args->bmq_release_index + pbqm_args->bmq_release_num); qid++) {
		wlqm = &wlpptr->wlpd_p->descData[qid];

		if ((void *)wlqm->sq.virt_addr != NULL) {
			wl_dma_free_coherent(wlpd_p->dev, (wlqm->sq.qsize * sizeof(bm_pe_hw_t)), (void *)wlqm->sq.virt_addr, wlqm->sq.phys_addr);
...
void wlRAcntBufInit(struct net_device *netdev, int qid)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct bqm_args *pbqm_args = &wlpd_p->bmq_args;
	...
	p_smac_cfg->acntRxInfoQueSize = pbqm_args->racnt_q_size[qid - pbqm_args->racntq_index];
	WLDBG_INFO(DBG_LEVEL_0, "%s(), acntRxInfoQueSize = %u\n", __func__, p_smac_cfg->acntRxInfoQueSize);
	wlpd_p->acntRxInfoQueBaseAddr_v = (rx_info_ppdu_t *) wl_dma_alloc_coherent(wlpd_p->dev,
										   (p_smac_cfg->acntRxInfoQueSize * sizeof(rx_info_ppdu_t)),
										   &phys_addr, wlpd_p->dma_alloc_flags);
	p_smac_cfg->acntRxInfoQueBaseAddr = (U32) phys_addr;
	// Initial acntRxRdPtr/acntRxWrPtr
	WLDBG_INFO(DBG_LEVEL_0, "%s(), acntRxInfoQueBaseAddr (v, p)=(%p, %x), %x, (phy=%llx)\n", __func__,
		   wlpd_p->acntRxInfoQueBaseAddr_v, p_smac_cfg->acntRxInfoQueBaseAddr, (U32) phys_addr, virt_to_phys(wlpd_p->acntRxInfoQueBaseAddr_v)
	    );
	wlpd_p->rxinfo_aux_poll = (rx_info_aux_t *) wl_kmalloc(sizeof(rx_info_aux_t) * p_smac_cfg->acntRxInfoQueSize, GFP_KERNEL);
	return;
}

int wlRAcntQueueInit(struct net_device *netdev, int qid)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct bqm_args *pbqm_args = &wlpd_p->bmq_args;
	...
	wlqm->sq.virt_addr = wl_dma_alloc_coherent(wlpd_p->dev, (wlqm->sq.qsize * sizeof(rxacnt_rec)), &wlqm->sq.phys_addr, wlpd_p->dma_alloc_flags);
	WLDBG_INFO(DBG_LEVEL_0, "RxAcntQ(%d), sq(%d) (v, p)=(%p, %llxh) from dma_alloc_consistent\n", qid, wlqm->sq.virt_addr, wlqm->sq.phys_addr);
	...
	// Init the signature to the buffer desc
	for (i = 0, pq_entity = (rxacnt_rec *) wlqm->sq.virt_addr; i < wlqm->sq.qsize; i++, pq_entity++) {
		pq_entity->bgn_signature = pq_entity->end_signature = BMBUF_SIGNATURE;
	}
	...
	// set Release buffer descruptor queue size and start address.
	writel(wlqm->sq.phys_addr, (wlpptr->ioBase1 + SC5_Q_BASE_ADDR_REG(qid, SC5_SQ)));
	WLDBG_INFO(DBG_LEVEL_0, "RxAcntQ(%d), SQ, SC5_Q_BASE_ADDR_REG(%d): %p, phyaddr=%llxh\n", qid,
		   (wlpptr->ioBase1 + SC5_Q_BASE_ADDR_REG(qid, SC5_SQ)), (u64) wlqm->sq.phys_addr);
	...
	writel(((wlqm->sq.qsize * sizeof(rxacnt_rec)) / 128) << 3, (wlpptr->ioBase1 + SC5_Q_SIZE_REG(qid, SC5_SQ)));
	WLDBG_INFO(DBG_LEVEL_0, "RxAcntQ(%d),SQ, SC5_Q_SIZE_REG w_reg(ioBase1+%xh) = %xh, qsize=%d, sizeof(rxacnt_rec)=%d\n", qid,
		   SC5_Q_SIZE_REG(qid, SC5_SQ), ((wlqm->sq.qsize * sizeof(wlrxdesc_t)) / 128) << 3, wlqm->sq.qsize, sizeof(rxacnt_rec));
...
void wlRAcntBufCleanUp(struct net_device *netdev, int qid)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct wlprivate *parent_wlpptr = GET_PARENT_PRIV(wlpptr);
	...
	if (wlpd_p->acntRxInfoQueBaseAddr_v != NULL) {
		wl_dma_free_coherent(wlpd_p->dev,
				     (p_smac_cfg->acntRxInfoQueSize * sizeof(rx_info_ppdu_t)),
				     (void *)wlpd_p->acntRxInfoQueBaseAddr_v, p_smac_cfg->acntRxInfoQueBaseAddr);
...
void wlRAcntQueueCleanUp(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct bqm_args *pbqm_args = &wlpd_p->bmq_args;
	...
	for (qid = pbqm_args->racntq_index; qid < (pbqm_args->racntq_index + pbqm_args->racntq_num); qid++) {
		wlqm = &wlpptr->wlpd_p->descData[qid];
		if ((void *)wlqm->sq.virt_addr != NULL) {
			wl_dma_free_coherent(wlpd_p->dev, (wlqm->sq.qsize * sizeof(rxacnt_rec)), (void *)wlqm->sq.virt_addr, wlqm->sq.phys_addr);
...
#endif				//#if defined(ACNT_REC)
bm_pe_hw_t *wlGetRelBufPe(struct net_device *netdev, int qid)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate *parent_wlpptr = GET_PARENT_PRIV(wlpptr);
	...
	wlqm = &wlpptr->wlpd_p->descData[qid];
	pe_hw_src = (bm_pe_hw_t *) (wlqm->sq.virt_addr + wlqm->sq.rdinx * sizeof(bm_pe_hw_t));
	 ...
	 */
	if ((pe_hw->bgn_signature == BMBUF_SIGNATURE) || (pe_hw->end_signature == BMBUF_SIGNATURE)) {
		wlpd_p->except_cnt.buf_desc_not_updated++;
		invalid_pe = 1;
#ifdef ASSERT_MALBUF
		WLDBG_ERROR(DBG_LEVEL_0, "Desc hasn't been updated: (%xh, %xh)\n", pe_hw->bgn_signature, pe_hw->end_signature);
#endif
	}
	if (!pe_addr_valid(pe_hw->pe0_lo_dword_addr)) {
		...
		WLDBG_ERROR(DBG_LEVEL_0, "pe0_lo_dword_addr == %xh", pe_hw->pe0_lo_dword_addr);
		WL_ASSERT(FALSE, ("pe0_lo_dword_addr == %xh", pe_hw->pe0_lo_dword_addr));
#endif
	}

	if (!(((pbqm_args->txq_start_index <= bpid) && (bpid < (pbqm_args->txq_start_index + pbqm_args->txq_num))) ||
	      ((SC5_BMQ_START_INDEX <= bpid) && (bpid < (SC5_BMQ_START_INDEX + SC5_BMQ_NUM))))) {
	...
	if (invalid_pe) {
		...
		WLDBG_ERROR(DBG_LEVEL_0, "Got unused/wrong desc at Q(%d), (rd, wr)=(%d, %d)\n", qid, wlqm->sq.rdinx, wlqm->sq.wrinx);
...
/*
	...
	returned:
		...
		-1: down too much
*/
CNT_RANGE wlCheckCnterRange(SINT32 cntval, SINT32 * plastval, SINT32 diffval)
{
 ...
 */
wltxdesc_t *wlSkbToCfhDl(struct net_device * netdev, struct sk_buff * skb, wltxdesc_t * txcfg, extStaDb_StaInfo_t * pStaInfo, int qid, int type)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	...
	if ((wlpptr->cb_enable == TRUE) && (cb_tx_allow(netdev, skb, (void *)pStaInfo, type) == false)) {
		// this packet should not be sent => dropped it
		wl_free_skb(skb);
		return 0;
	}
	...
	if (type == IEEE_TYPE_DATA) {
		...
		if (txcfg->mpdu_flag) {
			U16 ethertype = get_unaligned_le16(data_p + 12);
			put_unaligned_le16(ethertype, (U8 *) (&(txcfg->mpdu_ht_a_ctrl)) + 6);
			skb_push(skb, 8);
	...
	if (wlRQFull(netdev, qid) == FALSE) {	// check rd/wr index to check queue is full or not
		txdesc = (wltxdesc_t *) (wlqm->rq.virt_addr + wlqm->rq.wrinx * sizeof(wltxdesc_t));
		...
		if (skb_cloned(skb)) {
			...
			struct sk_buff *skb_align = wl_alloc_skb(skb->len + TXBUF_ALIGN + SKB_INFO_SIZE);
			if (!skb_align) {
				wl_free_skb(skb);
				return NULL;
			}
			...
			skb_align->data = PTR_ALIGN(skb_align->data, TXBUF_ALIGN);
		...
		*(struct sk_buff **)(skb->data - SKB_POINTER_OFFSET) = skb;
		...
		txdesc->hdr.lo_dword_addr = dma_map_single(wlpptr->wlpd_p->dev, skb->data, skb->len, DMA_TO_DEVICE);

		// Record the counter of pkt types
		{
			if (type != IEEE_TYPE_DATA) {
				// Mgmt / Ctrl pkt
				wl_pkttype_stat(wlpkt_typecnt_p, &wlpkt_typecnt_p->pkt_fc);
				memset(&wlpkt_typecnt_p->pkt_fc, 0, sizeof(IEEEtypes_FrameCtl_t));
			} else {
				IEEEtypes_FrameCtl_t fc;
				fc.Type = type;
				wl_pkttype_stat(wlpkt_typecnt_p, &fc);
				wl_get_datpkt_prot(wlpkt_typecnt_p, txcfg->snap1 >> 16, (struct iphdr *)skb->data, skb->len);
			}
		}
#if defined(TXACNT_REC)
		...
		*((struct sk_buff_head **)skb->cb) = &wlpd_p->pend_skb_trace[PENDSKB_TX];
...

		{
			...
			wlpd_p->except_cnt.txq_pend_cnt[txcfg->qid] =
			    wlpd_p->except_cnt.txq_send_cnt[txcfg->qid] - wlpd_p->except_cnt.txq_rel_cnt[txcfg->qid];

			if (txcfg->qid < QUEUE_STAOFFSET) {
				if ((txcfg->qid % MAX_TID) >= 6) {
					max_tx_pend_cnt = dbg_max_tx_pend_cnt_per_mgmt_q;
				} else {
					max_tx_pend_cnt = dbg_max_tx_pend_cnt_per_bcast_q;
				}

				if (wlpd_p->except_cnt.txq_pend_cnt[txcfg->qid] >= max_tx_pend_cnt) {
					wlpd_p->except_cnt.txq_drop_cnt[txcfg->qid]++;
					goto error_to_send;
				}
				...
				stnId = (txcfg->qid - QUEUE_STAOFFSET) / MAX_TID;
				wlpd_p->except_cnt.tx_sta_pend_cnt[stnId] =
				    wlpd_p->except_cnt.tx_sta_send_cnt[stnId] - wlpd_p->except_cnt.tx_sta_rel_cnt[stnId];
				if ((wlpd_p->except_cnt.txq_pend_cnt[txcfg->qid] >= max_tx_pend_cnt) ||
				    (wlpd_p->except_cnt.tx_sta_pend_cnt[stnId] >= max_tx_pend_cnt_per_sta)) {

					struct ethhdr *pEth = (struct ethhdr *)(skb->data - sizeof(struct ethhdr));
					if ((type == IEEE_TYPE_DATA) && (pEth->h_proto == IEEE_ETHERTYPE_ARP)) {
						UINT32 qid = txcfg->qid % MAX_TID;
						/* For ARP pkt, if it is in BK Queue 0, try another BK Queue 3 if it could be sent */
						if (qid == 0) {
							txcfg->qid += 3;
							txdesc->qid += 3;
							skb->priority += 3;
							if ((wlpd_p->except_cnt.txq_pend_cnt[txcfg->qid] < max_tx_pend_cnt) &&
							    (wlpd_p->except_cnt.tx_sta_pend_cnt[stnId] < dbg_max_tx_pend_cnt_per_sta))
								goto ok_to_send;
						}
					}

					wlpd_p->except_cnt.txq_drop_cnt[txcfg->qid]++;
					wlpd_p->except_cnt.tx_sta_drop_cnt[stnId]++;
					goto error_to_send;
				}
			}
 ...
 ok_to_send:
		WLDBG_DATA(DBG_LEVEL_4, " skb before wlRQIndexPut(): (%p, %p, %p, %p)\n", __func__, skb->head, skb->data, skb->end, skb->tail);
		wl_chk_drop_pkt(wlpd_p);
		if (wlRQIndexPut(qid, &wlqm->rq) == TRUE) {
			UINT32 tx_pend_cnt;
			spin_lock_bh(&wlpd_p->pend_skb_trace[PENDSKB_TX].lock);
			wlpd_p->tx_pend_skb_msg[tst_send][wlpd_p->tx_pend_skb_msg_id[tst_send]] = skb;
			wlpd_p->tx_pend_skb_msg_id[tst_send] = (wlpd_p->tx_pend_skb_msg_id[tst_send] + 1) % MAX_PENDSKBMSG;
			__skb_queue_tail(&wlpd_p->pend_skb_trace[PENDSKB_TX], skb);
			spin_unlock_bh(&wlpd_p->pend_skb_trace[PENDSKB_TX].lock);

			/* debug for invalid skb */
			if (unlikely((dbg_invalid_skb & dbg_ivalskb_tx) && wlpd_p->dbgskb.skb_send && !wlpd_p->dbgskb.skb_stop)) {
				dbg_skb *p = &wlpd_p->dbgskb;
				(p->skb_send + p->skb_send_idx)->pa = txdesc->hdr.lo_dword_addr;
				(p->skb_send + p->skb_send_idx)->va_data = skb->data;
				(p->skb_send + p->skb_send_idx)->va_skb = (u_int8_t *) skb;
				(p->skb_send + p->skb_send_idx)->wr = wlqm->rq.wrinx;
				ktime_get_ts(&(p->skb_send + p->skb_send_idx)->ts);

				if (virt_to_pfn(skb->data) == 0) {
					WLDBG_ERROR(DBG_LEVEL_0, "dbgskb: send skb %p skb->data %p pfn 0, ts %lu:%lu\n",
						    (p->skb_send + p->skb_send_idx)->va_skb,
						    (p->skb_send + p->skb_send_idx)->va_data,
						    (p->skb_send + p->skb_send_idx)->ts.tv_sec, (p->skb_send + p->skb_send_idx)->ts.tv_nsec);
				}

				p->skb_send_idx = (p->skb_send_idx + 1) % DBG_SKB_MAX_NUM;

			}

			if (type == IEEE_TYPE_DATA) {
				wl_txdatpkt_stat_per_sta(netdev, pStaInfo, skb, txcfg->snap1 >> 16);
				wl_txdatpkt_bcast_mcast(netdev, (ether_hdr_t *) data_p, skb);
			}

			wlUpdateWrPtr(netdev, qid, SC5_RQ, wlqm->rq.wrinx, false);
#ifdef TP_PROFILE
			logTPStats(&wlpd_p->drv_stats_val.cfhdltx_stat, 1, skb->len);
			...
			tx_pend_cnt = (wlpd_p->drv_stats_val.txq_drv_sent_cnt - wlpd_p->drv_stats_val.txbuf_rel_cnt);

#ifdef TP_PROFILE
			logTPCounter(&wlpd_p->drv_stats_val.cfhdltx_stat,
				     tx_pend_cnt, wlpd_p->drv_stats_val.txq_drv_sent_cnt, wlpd_p->drv_stats_val.txq_full_cnt);
#endif
			...
			{	//If the counter is over the level => print the warning message automatically
				SINT32 res = wlCheckCnterRange((SINT32) tx_pend_cnt,
							       &(wlpd_p->drv_stats_val.txpend_lastcnt),
							       BMQ_DIFFMSG_COUNT);
				if (res == CNT_RANGE_UP) {
			...
			WLDBG_WARNING(DBG_LEVEL_0, "Queue is full, failed to send\n");
			goto error_to_send;
		}
	} else {
		...
		if (type == IEEE_TYPE_DATA)
			wlpptr->wlpd_p->drv_stats_val.txq_ac_stats[AccCategoryQ[txcfg->qid % MAX_TID]].drop_cfh++;
	...
	return txdesc;
 error_to_send:
	dma_unmap_single(wlpptr->wlpd_p->dev, virt_to_phys(skb->data), wlqm->rq.bm.buf_size, DMA_FROM_DEVICE);
 ...
 */
void wl_dump_dbgrelq_info(struct net_device *netdev, u32 offset_base, u32 offset_cnt)
{
	struct wlprivate *priv = NETDEV_PRIV_P(struct wlprivate, netdev);
	unsigned int i, offset, j = 0;
	unsigned char *valbuf = NULL;
	volatile unsigned int baseptr = le32_to_cpu(*(volatile unsigned int *)(priv->ioBase0 + offset_base));
	volatile unsigned int entrycnt = le32_to_cpu(*(volatile unsigned int *)(priv->ioBase0 + offset_cnt));
	SMAC_QMINFO *pinfo;
	...
	for (i = 0, pinfo = (SMAC_QMINFO *) valbuf; i < DUMP_SIZE; i++, pinfo++) {
		pinfo->bpid = le32_to_cpu(*(volatile unsigned int *)(priv->ioBase0 + offset + j * sizeof(SMAC_QMINFO) + 0));
		pinfo->buf_addr = le32_to_cpu(*(volatile unsigned int *)(priv->ioBase0 + offset + j * sizeof(SMAC_QMINFO) + 4));
		j++;
		j &= (LOG_SIZE - 1);
	}
	/*
...
struct sk_buff *wlPeToSkb(struct net_device *netdev, bm_pe_hw_t * pe_hw)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct sk_buff *skb = NULL;
	...
	WLDBG_DATA(DBG_LEVEL_2, "bpid: %d, SKB: pe_hw->pe0_lo_dword_addr = %x \n", pe_hw_ptr->bpid, pe_hw_ptr->pe0_lo_dword_addr);
	...
	if ((SC5_BMQ_START_INDEX <= bpid) && (bpid < (SC5_BMQ_START_INDEX + SC5_BMQ_NUM - 1)))
		bmq_flag = 1;

	dma_unmap_single(wlpptr->wlpd_p->dev, pe_hw_ptr->pe0_lo_dword_addr, wlqm->rq.bm.buf_size, ((bmq_flag) ? DMA_FROM_DEVICE : DMA_TO_DEVICE));
	...
	if (unlikely(!virt_addr_valid(skb_virt - SKB_INFO_SIZE))) {
		...
		if ((dbg_invalid_skb & (dbg_ivalskb_tx | dbg_ivalskb_rx)) && (dbg_invalid_skb & dbg_ivalskb_class_1)) {
			WLDBG_ERROR(DBG_LEVEL_0, "dbgskb: invalid skb signature va %p\n", skb_virt - SKB_INFO_SIZE);
			dbg_coredump(netdev);
		}
		return NULL;
	}
	...
	/* debug for invalid skb */
	if (unlikely((dbg_invalid_skb & dbg_ivalskb_tx) && !bmq_flag && wlpd_p->dbgskb.skb_back && !wlpd_p->dbgskb.skb_stop)) {
		dbg_skb *p = &wlpd_p->dbgskb;

		(p->skb_back + p->skb_back_idx)->pa = pe_hw_ptr->pe0_lo_dword_addr;
		(p->skb_back + p->skb_back_idx)->va_data = skb_virt;
		(p->skb_back + p->skb_back_idx)->va_skb = *(u_int8_t **) (skb_virt - SKB_POINTER_OFFSET);
	...
	if (signature == SKB_SIGNATURE) {
		...
		memset((skb_virt - SKB_INFO_SIZE), 0, SKB_SIGNATURE_SIZE + sizeof(void *));
	} else {
		if (signature == USED_SIGNATURE) {
			...
			if (unlikely(dbg_invalid_skb != dbg_ivalskb_disable)) {
				...
				WLDBG_ERROR(DBG_LEVEL_0, "invalid skb: signature %x skb_data %p\n", signature, skb_virt);
			}
		}
	...
	if (bmq_flag) {
		// RX drop packets
		WLDBG_INFO(DBG_LEVEL_0, "Rx Drop Pkt, bpid=%d, pe0_lo_dword_addr:%p", REL_RX_BPID(pe_hw->bpid), pe_hw->pe0_lo_dword_addr);
		...
		wlpd_p->drv_stats_val.xx_buf_free_SQ14[bpid - SC5_BMQ_START_INDEX]++;

		skb->data = skb_virt;
		if (skb->next && skb->prev)
			reset_signature(skb_virt);

		WLDBG_DATA(DBG_LEVEL_1, "Txdone drop bpid:%u, cnt:%u %u %u %u\n",
			   REL_RX_BPID(pe_hw->bpid),
			   wlpd_p->drv_stats_val.xx_buf_free_SQ14[0],
			   wlpd_p->drv_stats_val.xx_buf_free_SQ14[1],
			   wlpd_p->drv_stats_val.xx_buf_free_SQ14[2], wlpd_p->drv_stats_val.xx_buf_free_SQ14[3]);

		// Remove the skb from the pend_skb_trace[PENDSKB_RX],
		if ((skb->next != (struct sk_buff *)&wlpd_p->pend_skb_trace[PENDSKB_RX] && !virt_addr_valid(skb->next)) ||
		    (skb->prev != (struct sk_buff *)&wlpd_p->pend_skb_trace[PENDSKB_RX] && !virt_addr_valid(skb->prev))) {
			wlpd_p->except_cnt.skb_notlinked_cnt++;
			if (unlikely((dbg_invalid_skb & dbg_ivalskb_rx) && (dbg_invalid_skb & dbg_ivalskb_class_2))) {
				if (!virt_addr_valid(skb->data) || !virt_addr_valid(skb->data - SKB_INFO_SIZE)) {
					WLDBG_ERROR(DBG_LEVEL_0,
						    "dbgskb: wlPeToSkb Rx bpid %d: skb %p link error. data %p next %p prev %p\n",
						    bpid, skb, skb->data, skb->next, skb->prev);
				} else {
					WLDBG_ERROR(DBG_LEVEL_0,
						    "dbgskb: wlPeToSkb Rx bpid %d: skb %p link error. data %p next %p prev %p signature 0x%8x dump skb_data:\n",
						    bpid, skb, skb->data, skb->next, skb->prev, *((u32 *) (skb->data - SKB_INFO_SIZE)));
					if (skb->data && skb->len)
...
int wlQMCleanUp(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	...
	{
		struct sk_buff *skb;
		while ((skb = skb_dequeue(&(wlpd_p->pend_skb_trace[PENDSKB_TX]))) != NULL) {
			wl_free_skb(skb);
		}
		while ((skb = skb_dequeue(&(wlpd_p->pend_skb_trace[PENDSKB_RX]))) != NULL) {
			wl_free_skb(skb);
		}
	}

	if (wlpptr->event_bufq_vaddr != NULL) {
		wl_dma_free_coherent(wlpptr->wlpd_p->dev, EVENT_BUFFQ_NUM * EVENT_BUFFQ_SIZE, wlpptr->event_bufq_vaddr, wlpptr->event_bufq_paddr);
	...
	/* delete all rx drop mgmt skb */
	skb_queue_purge(&wlpd_p->RxDropMgmtSkb);
...
void post_init_bq_idx(struct net_device *netdev, bool is_init)
{
	...
	for (qid = SC5_BMQ_START_INDEX; qid < (SC5_BMQ_START_INDEX + SC5_BMQ_NUM); qid++) {
		...
		WLDBG_INFO(DBG_LEVEL_0, "Q(%d), rqsize=%d, (rinx, winx)=(%d, %d)\n", qid, wlqm->rq.qsize, wlqm->rq.rdinx, wlqm->rq.wrinx);
	}
	...
	for (qid = SC5_RXQ_START_INDEX; qid < (SC5_RXQ_START_INDEX + SC5_RXQ_NUM); qid++) {
		...
		wlqm->sq.wrinx = wlQueryWrPtr(netdev, qid, SC5_SQ);
		WLDBG_INFO(DBG_LEVEL_0, "Q(%d), sqsize=%d, (rinx, winx)=(%d, %d)\n", qid, wlqm->sq.qsize, wlqm->sq.rdinx, wlqm->sq.wrinx);
	}
	WLDBG_INFO(DBG_LEVEL_0, "================= TxQ =============\n");
	// Init the rd/wr idx of TxQ
	for (qid = pbqm_args->txq_start_index; qid < (pbqm_args->txq_start_index + pbqm_args->txq_num); qid++) {
		WLDBG_INFO(DBG_LEVEL_0, "Q(%d), qsize=%d, (rinx, winx)=(%d, %d)\n", qid, wlqm->rq.qsize, wlqm->rq.rdinx, wlqm->rq.wrinx);
		wlqm = &wlpptr->wlpd_p->descData[qid];
		wlqm->rq.rdinx = wlQueryRdPtr(netdev, qid, SC5_RQ);
		wlUpdateWrPtr(netdev, qid, SC5_RQ, wlqm->rq.wrinx, is_init);
		WLDBG_INFO(DBG_LEVEL_0, "Q(%d), rqsize=%d, (rinx, winx)=(%d, %d)\n", qid, wlqm->rq.qsize, wlqm->rq.rdinx, wlqm->rq.wrinx);
	...
	{
		int i;
		for (i = SC5_BMQ_START_INDEX; i < SC5_BMQ_START_INDEX + SC5_BMQ_NUM; i++) {
...
//      }
	WLDBG_INFO(DBG_LEVEL_0, "STA=%02x:%02x:%02x:%02x:%02x:%02x\n",
		   qm_sta_mac[0], qm_sta_mac[1], qm_sta_mac[2], qm_sta_mac[3], qm_sta_mac[4], qm_sta_mac[5]);
	return;
}

void wl_set_qm_sa(IEEEtypes_MacAddr_t StaMacAddr)
{
	int i;
	...
	WLDBG_INFO(DBG_LEVEL_0, "OWN=%02x:%02x:%02x:%02x:%02x:%02x\n",
		   qm_own_mac[0], qm_own_mac[1], qm_own_mac[2], qm_own_mac[3], qm_own_mac[4], qm_own_mac[5]);
...
void InitCFHDLMgmt(struct net_device *netdev, struct bqm_args *pbqm_args, wltxdesc_t * cfg, struct sk_buff *skb)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	u16 offset;
 ...
 */
void InitCFHDL(struct net_device *netdev, struct bqm_args *pbqm_args, wltxdesc_t * cfg, struct sk_buff *skb, void *StaInfo, BOOLEAN eap,
	       UINT8 nullpkt)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	...
	if (wlpptr->wlpd_p->mmdu_data_enable == 0) {
		cfg->mpdu_flag = 0;
	} else {
	...
	if (cfg->hdr.length < TXDESC_IPHDR_SIZE) {
		memcpy(&cfg->ip_hdr, &skb->data[ETHER_HDR_LEN], cfg->hdr.length);
		cfg->hdr.length = TXDESC_IPHDR_SIZE;
	} else
		memcpy(&cfg->ip_hdr, &skb->data[ETHER_HDR_LEN], TXDESC_IPHDR_SIZE);
	return;
}
...
static void wlget_extmembuf(struct wlprivate *wlpptr, struct _ext_membuf *pext_membuf)
{
	...
	if (varpt != NULL) {
		...
		printk("=> %s(), using %s\n", __func__, pext_membuf->extsym_name);
		pext_membuf->vbuf_pool = *varpt;
		if (PTR_ALIGN(pext_membuf->vbuf_pool, BUF_16K) != pext_membuf->vbuf_pool) {
			WLDBG_ERROR(DBG_LEVEL_0, "%s(%p) is not 16K aligned...\n", pext_membuf->extsym_name, pext_membuf->vbuf_pool);
			pext_membuf->vbuf_pool = PTR_ALIGN(pext_membuf->vbuf_pool, BUF_16K);
		...
		if (!strcmp("bootmem_0", pext_membuf->extsym_name) || !strcmp("bootmem_1", pext_membuf->extsym_name))
			strcpy(memreg_name, "bootmem0");
		else if (!strcmp("bootmem_2", pext_membuf->extsym_name) || !strcmp("bootmem_3", pext_membuf->extsym_name))
		...
		if (!nd) {
			...
			if (rc) {
				...
				printk("=> %s(), using %s\nrsvd mem: pbuf=%llx vbuf=%p size=%llx\n",
				       __func__, pext_membuf->extsym_name, pext_membuf->pbuf_pool,
				       pext_membuf->vbuf_pool, (u64) pext_membuf->buf_pool_size);
			}
		}
		...
		if (!rsvd_buf_avail) {
			...
			pext_membuf->vbuf_pool = wl_dma_alloc_coherent(wlpptr->wlpd_p->dev,
								       pext_membuf->buf_pool_size,
								       &pext_membuf->pbuf_pool, wlpptr->wlpd_p->dma_alloc_flags);
		}

	}

	WLDBG_INFO(DBG_LEVEL_0, "Buffer Pool, buf_virt_addr=%p buff_phy_addr=%llxh\n", pext_membuf->vbuf_pool, (u64) pext_membuf->pbuf_pool);
...
static void wlclean_extmembuf(struct wlprivate *wlpptr, struct _ext_membuf *pext_membuf)
{
	if ((pext_membuf->buf_pool_from_sys == FALSE) && (pext_membuf->vbuf_pool != NULL)) {
		// The buffer pool of the last queue is from allocation => need to free it
		wl_dma_free_coherent(wlpptr->wlpd_p->dev, pext_membuf->buf_pool_size, pext_membuf->vbuf_pool, pext_membuf->pbuf_pool);
...
static void init_qelm_size_tbl(struct qelmsize_tbl *tbl, U32 size, U8 bgnid, U8 num_of_q, QPAIR qpairs)
{
	u8 qid;
...
*/
void wl_init_const(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct bqm_args *pbqm_args = &wlpd_p->bmq_args;
	...
	// Initial the q_size
	memset(&pbqm_args->q_elmsize_tbl, INVALID_QSIZE, sizeof(struct qelmsize_tbl));
	// RXQ
	init_qelm_size_tbl(&pbqm_args->q_elmsize_tbl, sizeof(wlrxdesc_t), SC5_RXQ_START_INDEX, SC5_RXQ_NUM, is_sq);
	// TXQ
	init_qelm_size_tbl(&pbqm_args->q_elmsize_tbl, sizeof(wltxdesc_t), pbqm_args->txq_start_index, pbqm_args->txq_num, is_rq);
	// BMQ
	init_qelm_size_tbl(&pbqm_args->q_elmsize_tbl, sizeof(bm_pe_hw_t), SC5_BMQ_START_INDEX, SC5_BMQ_NUM, is_rq);
	// ReleaseQ
	init_qelm_size_tbl(&pbqm_args->q_elmsize_tbl, sizeof(bm_pe_hw_t), pbqm_args->bmq_release_index, pbqm_args->bmq_release_num, is_sq);
#if defined(ACNT_REC)
	// AcntQ
	init_qelm_size_tbl(&pbqm_args->q_elmsize_tbl, sizeof(rxacnt_rec), pbqm_args->racntq_index, pbqm_args->racntq_num, is_sq);
#endif				//#if defined(ACNT_REC)
...
static void oops_do_dump(struct kmsg_dumper *dumper, enum kmsg_dump_reason reason)
{
	int i;
	printk("===> %s(), reason = %d\n", __func__, reason);
...
void wl_register_dump_func(struct wlprivate_data *wlpd_p)
{
	int res;
...
void wl_unregister_dump_func(struct wlprivate_data *wlpd_p)
{
...
void wl_init_droppkt_info(struct net_device *netdev)
{
	...
	pdropbuf->TxD1DropBuf_v = (void *)wl_dma_alloc_coherent(wlpd_p->dev, sizeof(wltxdesc_t) * MAX_HWDROP_BUF,
								&phys_addr, wlpd_p->dma_alloc_flags);
#else
	// Using cacheable buffer
	pdropbuf->TxD1DropBuf_v = (void *)wl_kmalloc(sizeof(wltxdesc_t) * MAX_HWDROP_BUF, GFP_KERNEL);
	phys_addr = dma_map_single(wlpd_p->dev, pdropbuf->TxD1DropBuf_v, sizeof(wltxdesc_t) * MAX_HWDROP_BUF, DMA_FROM_DEVICE);
#endif				//

	pdropbuf->TxD1DropBuf_p = phys_addr;
	// Initialize the txd1 drop registers
	*(volatile unsigned int *)(wlpptr->ioBase1 + TXD1_DDR_DROPBUF_CFG) = (pdropbuf->TxD1DropBuf_p >> 4) | 0xf0000000;
	*(volatile unsigned int *)(wlpptr->ioBase1 + TXD1_DDR_DROPBUF_RDPTR) = pdropbuf->TxD1DropBuf_p;
	*(volatile unsigned int *)(wlpptr->ioBase1 + TXD1_PUNT_CTRL) = PUNT_ALL;

	// Initialize the internal variables
	pdropbuf->TxD1DropBuf_WrCnt_reg = (U32 *) (wlpptr->ioBase1 + TXD1_DDR_DROP_WRITE_CNT);
...
#ifdef USE_NCBUF
		wl_dma_free_coherent(wlpd_p->dev, sizeof(wltxdesc_t) * MAX_HWDROP_BUF, (void *)pdropbuf->TxD1DropBuf_v, pdropbuf->TxD1DropBuf_p);
#else
		dma_unmap_single(wlpptr->wlpd_p->dev, pdropbuf->TxD1DropBuf_p, sizeof(wltxdesc_t) * MAX_HWDROP_BUF, DMA_FROM_DEVICE);
...
static void wl_push_droppkt_rec(DROPPKT_INFO * pdropbuf, wltxdesc_t * pcfhdl)
{
	u8 i;
	u8 rbit = 1;
	...
	memcpy(&pdropbuf->dropbuf[pdropbuf->dropbuf_wid], pcfhdl, sizeof(wltxdesc_t));
...
void wl_chk_drop_pkt(struct wlprivate_data *wlpd_p)
{
	...
	if (dropcnt < MAX_HWDROP_BUF) {
		for (i = 0; i < dropcnt; i++) {
			wltxdesc_t *pcfhul = &txd1_bufpt[pdropbuf->TxD1DropBuf_wid];
			wl_push_droppkt_rec(pdropbuf, pcfhul);
			pdropbuf->TxD1DropBuf_wid = (pdropbuf->TxD1DropBuf_wid + 1) % MAX_HWDROP_BUF;
			wlpd_p->except_cnt.txq_txd1_drop_cnt[pcfhul->qid]++;
		}
	} else {
		// Overflow. Just save the latest records
		pdropbuf->TxD1DropBuf_wid = (pdropbuf->TxD1DropBuf_wid + dropcnt) % MAX_HWDROP_BUF;
...
static void dbgskb_init(struct net_device *netdev)
{
	...
	dbgskb_p->skb_send = (dbg_skb_send *) wl_kmalloc(DBG_SKB_MAX_NUM * sizeof(dbg_skb_send), GFP_KERNEL);
	if (!dbgskb_p->skb_send) {
		WLDBG_ERROR(DBG_LEVEL_0, "dbgskb: skb_send mem alloc fail!\n");
	} else
		memset(dbgskb_p->skb_send, 0x0, DBG_SKB_MAX_NUM * sizeof(dbg_skb_send));

	dbgskb_p->skb_back = (dbg_skb_back *) wl_kmalloc(DBG_SKB_MAX_NUM * sizeof(dbg_skb_back), GFP_KERNEL);
	if (!dbgskb_p->skb_back) {
		WLDBG_ERROR(DBG_LEVEL_0, "sbgskb: skb_back mem alloc fail!\n");
	} else
		memset(dbgskb_p->skb_back, 0x0, DBG_SKB_MAX_NUM * sizeof(dbg_skb_back));
}
...
int wlQMInit(struct net_device *netdev)
{
	int qid, err;
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	...
	if (mem_dbg) {
		...
		L0L1_Buf_Size = ((bss_num + sta_num) * 256 * 1024) + 4 * 1024 * 1024;
	}
	...
	printk("=> %s(), set qIntOffset = %xh\n", __func__, wlpptr->smacconfig.qIntOffset);
	...
	wlpptr->event_bufq_vaddr = (void *)wl_dma_alloc_coherent(wlpd_p->dev, EVENT_BUFFQ_NUM * EVENT_BUFFQ_SIZE,
								 &wlpptr->event_bufq_paddr, wlpd_p->dma_alloc_flags);
	...
	wlpd_p->descData[0].pInfoPwrTbl = wl_kmalloc(sizeof(Info_rate_power_table_t), GFP_KERNEL);
	wlpd_p->descData[0].pPhyInfoPwrTbl = virt_to_phys(wlpd_p->descData[0].pInfoPwrTbl);
	memset((void *)wlpd_p->descData[0].pInfoPwrTbl, 0, sizeof(Info_rate_power_table_t));
	...
	skb_queue_head_init(&wlpd_p->RxDropMgmtSkb);
	wlpd_p->RxDropMgmtCnt = 0;

	return SUCCESS;
}

void wlCfhUlDump(wlrxdesc_t * cfh_ul)
{
...
void wlRxQDump(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wldesc_data *wlqm;
	int qid, i;
	...
	for (qid = SC5_RXQ_START_INDEX; qid < (SC5_RXQ_START_INDEX + SC5_RXQ_NUM); qid++) {
		...
		WLDBG_INFO(DBG_LEVEL_3, "addr: base %p qsize %p rdptr %p wrptr %p \n", addr[0], addr[1], addr[2], addr[3]);
		for (i = 0; i < 4; i++)
			value[i] = readl(addr[i]);
		WLDBG_INFO(DBG_LEVEL_3, "value: base %x qsize %x rdptr %x wrptr %x \n", value[0], value[1], value[2], value[3]);
		wlqm = &wlpptr->wlpd_p->descData[qid];
		rdinx = wlqm->sq.rdinx;

		while (rdinx != wlqm->sq.wrinx) {
			cfh_ul = (wlrxdesc_t *) (wlqm->sq.virt_addr + rdinx * sizeof(wlrxdesc_t));
			wlCfhUlDump(cfh_ul);
			rdinx = (rdinx + 1) % wlqm->sq.qsize;
		}		// end of while
...
void wlTxQDump(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct bqm_args *pbqm_args = &wlpd_p->bmq_args;
	...
	for (qid = pbqm_args->txq_start_index; qid < (pbqm_args->txq_start_index + pbqm_args->txq_num); qid++) {
		...
		while (rdinx != wlqm->rq.wrinx) {
			cfh_dl = (wltxdesc_t *) (wlqm->rq.virt_addr + rdinx * sizeof(wltxdesc_t));
			wlCfhDlDump(cfh_dl);
			rdinx = (rdinx + 1) % wlqm->rq.qsize;
		}		// end of while
...
void wlBmQDump(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	int qid, i;
	void *addr[4];
...
void wlReleaseQDump(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct bqm_args *pbqm_args = &wlpd_p->bmq_args;
...
*/
BOOLEAN wlDbg_chk_bm_enq(struct net_device * netdev, UINT32 max_diff)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	CNT_RANGE res;
	...
	for (buf_qid = SC5_BMQ_START_INDEX; buf_qid < (SC5_BMQ_START_INDEX + SC5_BMQ_NUM - 1); buf_qid++) {
		int offset = buf_qid - SC5_BMQ_START_INDEX;
		struct drv_stats *wldrvstat_p = &wlpd_p->drv_stats_val;

		pendcnt = wldrvstat_p->enq_bmqbuf_cnt[offset] - wldrvstat_p->xx_buf_free_SQ14[offset] - wldrvstat_p->bmqbuf_ret_cnt[offset];
		res = wlCheckCnterRange(pendcnt, &(wlpd_p->drv_stats_val.enq_bmq_lastcnt[offset]), (SINT32) max_diff);
	...
	if (is_showmsg == TRUE) {
		...
		printk("Level cnt: %d, %d, %d\n",
		       wlpd_p->drv_stats_val.enq_bmq_lastcnt[0], wlpd_p->drv_stats_val.enq_bmq_lastcnt[1], wlpd_p->drv_stats_val.enq_bmq_lastcnt[2]);
	}
#endif
	return is_showmsg;
}

BOOLEAN wlDbg_chk_bm_enq_qid(struct net_device * netdev, U16 buf_qid, UINT32 max_diff)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	SINT32 pendcnt;
...
int wlRxBufFill(struct net_device *netdev)
{
	...
	for (buf_qid = SC5_BMQ_START_INDEX; buf_qid < (SC5_BMQ_START_INDEX + SC5_BMQ_NUM - 1); buf_qid++) {
		int qid = buf_qid;
		int offset __attribute__ ((unused)) = buf_qid - SC5_BMQ_START_INDEX;
		struct drv_stats * __attribute__ ((unused)) wldrvstat_p = &wlpd_p->drv_stats_val;
		CNT_RANGE res;

		/* if the enqueue to this qid is too large, then skip
		 */
		if (wlDbg_chk_bm_enq_qid(netdev, qid, buf_pool_max_entries[offset]) == TRUE) {
			continue;
		}
		...
		wlmon_log_bmq_buff_refill(netdev, (UINT32) buf_qid, (UINT32) refill_cnt);

		if (refill_cnt > 0) {
			wlUpdateWrPtr(netdev, qid, SC5_RQ, wlqm->rq.wrinx, false);
			WLDBG_DATA(DBG_LEVEL_3, "Fill in %d buffers to RQ(%d)\n", refill_cnt, qid);
		}

		res = wlCheckCnterRange((SINT32) skb_queue_len(&wlqm->rq.skbTrace),
					&(wlpd_p->drv_stats_val.trcq_lastcnt[buf_qid - SC5_BMQ_START_INDEX]), BMQ_DIFFMSG_COUNT);
		if (res == CNT_RANGE_UP) {
			WLDBG_WARNING(DBG_LEVEL_0, "Traced packets count to Q[%d] reachs %d\n", buf_qid,
				      (wldrvstat_p->enq_bmqbuf_cnt[offset] - wldrvstat_p->xx_buf_free_SQ14[offset] -
				       wldrvstat_p->bmqbuf_ret_cnt[offset]));
		} else if (res == CNT_RANGE_DOWN) {
			WLDBG_WARNING(DBG_LEVEL_0, "Traced packets count to Q[%d] drops down to %d\n", buf_qid,
				      (wldrvstat_p->enq_bmqbuf_cnt[offset] - wldrvstat_p->xx_buf_free_SQ14[offset] -
				       wldrvstat_p->bmqbuf_ret_cnt[offset]));
		}
...
#if 1
int wlRxBufFillBMEM_Q13(struct net_device *netdev, bm_pe_hw_t * pehw)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct wldesc_data *wlqm;
	...
	pe_hw = (bm_pe_hw_t *) (wlqm->rq.virt_addr + wlqm->rq.wrinx * sizeof(bm_pe_hw_t));
...
static int _wlRxBufFill(struct net_device *netdev, int qid)
{
	...
	if (found == FALSE) {
		...
		if (!skb) {
			wlpd_p->drv_stats_val.bmqbuf_alloc_fail_cnt[qid - SC5_BMQ_START_INDEX]++;
			WLDBG_ERROR(DBG_LEVEL_0, "Failed to allocate skb, size=%d\n", skb_size);
			WL_ASSERT(FALSE, ("Failed to allocate skb, size=%d\n", skb_size));
			return FAIL;
		} else {
			WLDBG_DATA(DBG_LEVEL_3, "Q(%d), rx_buf_fill alloc_size=%d\n", qid, skb_size);
		}
		skb->dev = netdev;
		//skb_reserve(skb, SKB_INFO_SIZE);
		if (((U32) (skb->data - skb->head)) < SKB_INFO_SIZE) {
	...
	*skb_addr = skb;
	...
	//Add tail signature check
	*((u32 *) (skb->data + size + SKB_TAIL_SIGNATURE_OFFSET)) = SKB_TAIL_SIGNATURE;
	phy_addr = dma_map_single(wlpptr->wlpd_p->dev, skb->data, size, DMA_FROM_DEVICE);
	...
	WLDBG_DATA(DBG_LEVEL_3, "CFHUL: [RxBufFill] RQ( %d ) wrinx %d virt_addr %p skb %p phy_addr %xh\n",
		   //qid, wlqm->rq.wrinx, pe->virt_addr, skb, pe->phy_addr);
		   qid, wlqm->rq.wrinx, skb->data, skb, pe_hw->pe0_lo_dword_addr);
	spin_lock(&wlpd_p->pend_skb_trace[PENDSKB_RX].lock);
...
extern UINT32 wlFwSetSchedMode(struct net_device *netdev, UINT16 action, UINT32 mode_selected, void *pCfg, UINT16 len, UINT16 * pStatus);
void wl_nullpkt_hndl(struct net_device *netdev, wlrxdesc_t * pcfh_ul)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	...
	if (mac_hdr->FrmCtl.ToDs && mac_hdr->FrmCtl.FromDs) {
		if ((frame_ctlp->Type == IEEE_TYPE_DATA) && (mac_hdr->FrmCtl.Subtype & BIT(3)))	// QoS Packet
			memcpy(&htc, &mac_hdr->wds_qos_htc.htc, sizeof(IEEEtypes_htcField_t));
		else
			memcpy(&htc, &mac_hdr->wds_htc.htc, sizeof(IEEEtypes_htcField_t));
	} else if ((frame_ctlp->Type == IEEE_TYPE_DATA) && (mac_hdr->FrmCtl.Subtype & BIT(3)))	// QoS Packet
		memcpy(&htc, &mac_hdr->qos_htc.htc, sizeof(IEEEtypes_htcField_t));
	else
	...
	//printk("\t shift_bits: %d\n", shift_bits);
	if (shift_bits) {
		acontrol_om.om_control = (htc.he_variant.a_control >> shift_bits) & 0xFFF;
		if (wfa_11ax_pf)	/* Only print for WFA tests */
			printk("acontrol_om.chbw %d acontrol_om.rxnss %d, acontrol_om.tx_nsts %d, acontrol_om.ulmu_disable %d\n", acontrol_om.chbw,
			       acontrol_om.rxnss, acontrol_om.tx_nsts, acontrol_om.ulmu_disable);
		if ((pStaInfo =
		     extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) & mac_hdr->Addr2, STADB_SKIP_MATCH_VAP | STADB_NO_BLOCK)) != NULL) {

			vap_wlpptr = NETDEV_PRIV_P(struct wlprivate, pStaInfo->dev);

			if (wfa_11ax_pf && (vap_wlpptr->vmacSta_p->ul_ofdma.period_tmr != 0)) {
				if (pStaInfo->operating_mode.tx_nsts != acontrol_om.tx_nsts) {
					ul_usr_num = 1;
					mib_val = acontrol_om.tx_nsts;
					wlFwSetMib(pStaInfo->dev, HostCmd_ACT_GEN_SET, MIB_TF_NSS, &mib_val, &ul_usr_num);
					wlFwSetMib(pStaInfo->dev, HostCmd_ACT_GEN_GET, MIB_TF_DATA_LEN, data_len, &ul_usr_num);
					data_len[0] = data_len[0] * (acontrol_om.tx_nsts + 1) / (pStaInfo->operating_mode.tx_nsts + 1);
					ul_usr_num = 1;
					wlFwSetMib(pStaInfo->dev, HostCmd_ACT_GEN_SET, MIB_TF_DATA_LEN, data_len, &ul_usr_num);
				}

				if (pStaInfo->operating_mode.ulmu_disable != acontrol_om.ulmu_disable) {

					if (acontrol_om.ulmu_disable == 1) {
						memcpy((void *)&tf_cfg, (void *)&vap_wlpptr->vmacSta_p->ul_ofdma, sizeof(sched_cfg_ul_ofdma_t));
						tf_cfg.period_tmr = 0;
						wlFwSetSchedMode(pStaInfo->dev, HostCmd_ACT_GEN_SET, MODE_SELECT_UL_OFDMA, &tf_cfg,
								 sizeof(sched_cfg_ul_ofdma_t), &status);
					} else {
						wlFwSetSchedMode(pStaInfo->dev, HostCmd_ACT_GEN_SET, MODE_SELECT_UL_OFDMA,
								 (void *)&vap_wlpptr->vmacSta_p->ul_ofdma, sizeof(sched_cfg_ul_ofdma_t), &status);
					}
				}
			} else if (pStaInfo->operating_mode.ulmu_disable != acontrol_om.ulmu_disable) {
				msg_buf = wl_kmalloc(IW_CUSTOM_MAX, GFP_ATOMIC);
				if (msg_buf == NULL) {
				...
				if (acontrol_om.ulmu_disable == 1) {
					...
					sprintf(msg_buf, "wlmgr: mumode ul_ofdma_disable stnid:%d", pStaInfo->StnId);
					memset(&wreq, 0, sizeof(wreq));
					wreq.data.length = strlen(msg_buf);
					wireless_send_event(pStaInfo->dev, IWEVCUSTOM, &wreq, msg_buf);
				} else {
					sprintf(msg_buf, "wlmgr: mumode ul_ofdma_enable stnid:%d", pStaInfo->StnId);
					memset(&wreq, 0, sizeof(wreq));
					wreq.data.length = strlen(msg_buf);
					wireless_send_event(pStaInfo->dev, IWEVCUSTOM, &wreq, msg_buf);
				}
			...
			if ((pStaInfo->operating_mode.rxnss != acontrol_om.rxnss) || (pStaInfo->operating_mode.chbw != acontrol_om.chbw)) {
				wlFwSetVHTOpMode(pStaInfo->dev, pStaInfo->StnId, acontrol_om.chbw, acontrol_om.rxnss + 1);
			}

			if (wfa_11ax_pf && (vap_wlpptr->vmacSta_p->ul_ofdma.period_tmr != 0)) {
				if (pStaInfo->operating_mode.chbw != acontrol_om.chbw) {
					switch (acontrol_om.chbw) {
					case 3:
						ul_usr_num = 1;
						mib_val = 68;
						wlFwSetMib(pStaInfo->dev, HostCmd_ACT_GEN_SET, MIB_TF_RU_ALLOC, &mib_val, &ul_usr_num);
						break;
					case 2:
						ul_usr_num = 1;
						mib_val = 67;
					...
					wlFwSetMib(pStaInfo->dev, HostCmd_ACT_GEN_GET, MIB_TF_DATA_LEN, data_len, &ul_usr_num);
					data_len[0] = data_len[0] * (2 << acontrol_om.chbw) / (2 << pStaInfo->operating_mode.chbw);
					ul_usr_num = 1;
					wlFwSetMib(pStaInfo->dev, HostCmd_ACT_GEN_SET, MIB_TF_DATA_LEN, data_len, &ul_usr_num);
				}
			}

			if (pStaInfo->operating_mode.om_control != acontrol_om.om_control)
				pStaInfo->operating_mode.om_control = acontrol_om.om_control;
		}
	}
...
void wl_reset_ba(struct net_device *netdev, extStaDb_StaInfo_t * pStaInfo, UINT8 tsid)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	...
	for (i = 0; i < MAX_SUPPORT_AMPDU_TX_STREAM_RUNNING; i++) {
		if (!MACADDR_CMP(wlpd_p->Ampdu_tx[i].MacAddr, &(pStaInfo->Addr))) {
			/** they are equal **/
			if (wlpd_p->Ampdu_tx[i].AccessCat == tsid && wlpd_p->Ampdu_tx[i].InUse == 1) {
				WLDBG_INFO(DBG_LEVEL_0, "del ba !!!! They match!!!!\n");
				/* Reset the flags so that stream can be started once traffic is back on */
				pStaInfo->aggr11n.onbytid[wlpd_p->Ampdu_tx[i].AccessCat] = 0;
				pStaInfo->aggr11n.startbytid[wlpd_p->Ampdu_tx[i].AccessCat] = 0;
				pStaInfo->aggr11n.type &= ~WL_WLAN_TYPE_AMPDU;
				wlFwUpdateDestroyBAStream(vmacSta_p->dev, 0, 0, i, wlpd_p->Ampdu_tx[i].AccessCat,
							  wlpd_p->Ampdu_tx[i].MacAddr, pStaInfo->StnId);
...
void wl_ack_policy_mismatch(struct net_device *netdev, wlrxdesc_t * pcfh_ul)
{
	U8 *ppayload = (u8 *) phys_to_virt(pcfh_ul->hdr.lo_dword_addr);
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	...
	StaInfo_p = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) & sta_macaddr,
 ...
 */
wlrxdesc_t *wlGetCfhUl(struct net_device * netdev, int qid, wlrxdesc_t * cfh_ul)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct wldesc_data *wlqm;
	...
	wlqm = &wlpptr->wlpd_p->descData[qid];
	cfh_ul_hw = (wlrxdesc_t *) (wlqm->sq.virt_addr + wlqm->sq.rdinx * sizeof(wlrxdesc_t));

	memcpy(cfh_ul, cfh_ul_hw, sizeof(wlrxdesc_t));
	// Start processing
	if (cfhul_skb_recov == TRUE)
		wl_update_cfhul_buf_rec(&wlpd_p->icfhul_buf_pool, netdev, qid, wlqm->sq.rdinx, cfh_ul);
	wlpd_p->irxdbg_intr.rxdbg_push(&wlpd_p->vrxdbg_db, cfh_ul, qid, wlqm->sq.rdinx, wlqm->sq.wrinx);
	...
	if (cfh_ul->hdrFormat == 1) {
		...
		if ((cfh_ul->mic_err == 0) && (cfh_ul->icv_err == 0)) {
			...
			pktlen = (cfh_ul->hdr.length < 1024) ? cfh_ul->hdr.length : 1024;
			ppayload = (u8 *) phys_to_virt(cfh_ul->hdr.lo_dword_addr);
			memcpy(&wlpd_p->nullpkt_cfhul[id], cfh_ul, sizeof(wlrxdesc_t));
			...
			{
				wl_free_cfhul_lo(cfh_ul->hdr.lo_dword_addr, netdev, cfh_ul->hdr.bpid);
				wl_clr_cfhul_buf_rec(&wlpd_p->icfhul_buf_pool, qid, wlqm->sq.rdinx, cfh_ul);
			}
			return NULL;
		}
	}
	...
	if (cfh_ul->nss_hdr[2] == HF_OWN_SIGNATURE || cfh_ul->hdr.length == USED_BUFLEN || cfh_ul->hdr.used_signature == BMBUF_SIGNATURE) {
	...
	if ((cfh_ul->hdr.bpid < SC5_BMQ_START_INDEX) || ((SC5_BMQ_START_INDEX + SC5_BMQ_NUM - 1) <= cfh_ul->hdr.bpid)) {
		wlpd_p->except_cnt.cnt_cfhul_error++;
#ifdef ASSERT_MALBUF
		WLDBG_WARNING(DBG_LEVEL_0, "ERROR: Invalid bpid:%u from SQ(%u)\n", cfh_ul->hdr.bpid, qid);
		cfhul_err = 1;
#else
		return NULL;
#endif
	...
	if ((frame_ctlp->Type == IEEE_TYPE_DATA) && (!frame_ctlp->MoreFrag && !(cfh_ul->hdr.seqNum & 0xF) && (cfh_ul->hdrFormat != 1))) {
		/* Check the 0xAAAA of SNAP in data MSDU header
		 */
		snap_aaaa = (u16 *) cfh_ul->nss_hdr;
		if ((qid == SC5_RXQ_START_INDEX) && (snap_aaaa[7] != 0xAAAA) && (snap_aaaa[7] != 0x4242) && (snap_aaaa[7] != 0x0100)) {
...
#ifdef ASSERT_MALBUF
			WLDBG_ERROR(DBG_LEVEL_0, "ERROR: data size is over-size %d  bpid:%d from SQ(%u)\n",
	...
	if (cfhul_err == 1) {
		...
		printk("qidcnt SQ0:%u SQ8:%u SQ9:%u\n",
		       wlpd_p->drv_stats_val.rxq_rcv_cnt[0], wlpd_p->drv_stats_val.rxq_rcv_cnt[8], wlpd_p->drv_stats_val.rxq_rcv_cnt[9]);
		writel(0xdeadbeef, wlpptr->ioBase1 + PCI_REG_SCRATCH14_REG);
		wl_dump_dbgrelq_info(netdev, 0x528, 0x52c);
		wl_dump_dbgrelq_info(netdev, 0x520, 0x524);
		wl_dump_dbgrelq_info(netdev, 0x518, 0x51c);
...
wlrxdesc_t *wlSpiltAMSDU(struct net_device * netdev, wlrxdesc_t * cfh_ul)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	...
	u8 *msdu_addr = NULL;
	u16 length;
	struct wldesc_data *wlqm = &wlpptr->wlpd_p->descData[(cfh_ul->hdr.bpid) & 0xf];

	//wlrxdesc_t *pcfh;

	if (msdu_no == 0) {
		WLDBG_ERROR(DBG_LEVEL_0, "Total %u msdu subframes in this AMSDU frame\n", cfh_ul->hdrFormat);
		return NULL;
	}
	...
	dma_unmap_single(wlpptr->wlpd_p->dev, virt_to_phys(amsdu_start), wlqm->rq.bm.buf_size, DMA_FROM_DEVICE);

	cfh_ul_amsdu = (wlrxdesc_t *) wl_kmalloc((msdu_no * sizeof(wlrxdesc_t)), GFP_ATOMIC);

	for (i = 0; i < msdu_no; i++) {
		memcpy((u8 *) & cfh_ul_amsdu[i], (u8 *) cfh_ul, sizeof(wlrxdesc_t));
		WLDBG_DATA(DBG_LEVEL_3, "CFHUL_AMSDU: [%d]th msdu start at %p\n", i, msdu_addr);
		if (i == 0) {	// first
			cfh_ul_amsdu[i].fpkt = 1;
			cfh_ul_amsdu[i].lpkt = 0;
			cfh_ul_amsdu[i].hdr.lo_dword_addr = cfh_ul->hdr.lo_dword_addr;
		} else {
			cfh_ul_amsdu[i].fpkt = 0;
			cfh_ul_amsdu[i].lpkt = 0;
			cfh_ul_amsdu[i].hdr.lo_dword_addr = (u32) virt_to_phys(msdu_addr);
		}

		if (i == (msdu_no - 1)) {	// last msdu
			cfh_ul_amsdu[i].lpkt = 1;
		}
		WLDBG_DATA(DBG_LEVEL_3, "CFHUL_AMSDU: [%d] fpkt %d lpkt %d  phy addr %x\n",
			   i, cfh_ul_amsdu[i].fpkt, cfh_ul_amsdu[i].lpkt, cfh_ul_amsdu[i].hdr.lo_dword_addr);
		...
		if (length > 1508 || length < (LLC_HDR_LEN + IP_HDR_LEN) || *((u16 *) & msdu_addr[14]) != 0xaaaa) {
			wlpd_p->except_cnt.cnt_invalid_amsdu_subframe_len++;
			WLDBG_WARNING(DBG_LEVEL_0, "ERROR: Invalid AMSDU subframe length:%u ,cfh_ul->hdr.length:%u\n", length, cfh_ul->hdr.length);
			//darlee
			WLDBG_HEXDUMP(DBG_LEVEL_0, amsdu_start, cfh_ul->hdr.length);
...
wlrxdesc_t *wlProcessErrCfhul(struct net_device * netdev, u32 * msduNo)
{
	wl_cfhul_amsdu *pcfhul_amsdu = NULL;
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;

	pcfhul_amsdu = (wl_cfhul_amsdu *) wlpd_p->irxdbg_intr.rxdbg_pull_errcfhul(&wlpd_p->vrxdbg_db);
	if (pcfhul_amsdu) {
...
wlrxdesc_t *wlProcessMsdu(struct net_device * netdev, wlrxdesc_t * cfh_ul, u32 * msduNo, u32 qid)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	//dralee
	...
	case 0:		//middle subframe
		{
			if (*pidx != 0) {	//middle frames arrives. put in acc pool
				memcpy((void *)&cfh_ul_amsdu[(*pidx)++], (void *)cfh_ul, sizeof(wlrxdesc_t));
			} else {
				//error handling.  individual middle come along.
				wlpd_p->except_cnt.cfhul_flpkt_lost[0]++;
				//printk("missing fpkt...\n");
				wlpd_p->irxdbg_intr.rxdbg_push_errcfhul(&wlpd_p->vrxdbg_db, NULL, cfh_ul);
			}
	...
	case 2:		//first subframe
		{
			if (*pidx == 0) {	//10(first) received, put in accu pool
				...
				wlpd_p->irxdbg_intr.rxdbg_push_errcfhul(&wlpd_p->vrxdbg_db, &wlpptr->cfhul_amsdu[qid], NULL);
				//printk("missing lpkt...\n");
...
static void check_queue_index(struct net_device *netdev, u16 qid, int qoff)
{
	...
	if (qoff == SC5_SQ) {
		...
		WLDBG_DATA(DBG_LEVEL_0, "Index of SQ[%d] (wrinx, rdinx) = (%d, %d)\n", qid, wlqm->sq.wrinx, wlqm->sq.rdinx);
	} else {
		wlqm->rq.rdinx = wlQueryRdPtr(netdev, qid, qoff);
		WLDBG_DATA(DBG_LEVEL_0, "Index of RQ[%d] (wrinx, rdinx) = (%d, %d)\n", qid, wlqm->rq.wrinx, wlqm->rq.rdinx);
	}
	return;
}

void wlResetCfhUl(wlrxdesc_t * cfh_ul)
{
...
void wldbg_aggr_stat(struct drv_stats *drvstat_p, wlrxdesc_t * cfh_ul)
{
	...
	if ((cfh_ul->lpkt == 1) && (drvstat_p->rxq_midaggr[qid - SC5_BMQ_START_INDEX] > 1)) {
		// last fragment => record the result
		if (drvstat_p->rxq_midaggr[qid - SC5_BMQ_START_INDEX] < AGGRCNT_ARYSIZE - 2) {
			drvstat_p->rxq_aggrcnt[qid - SC5_BMQ_START_INDEX][drvstat_p->rxq_midaggr[qid - SC5_BMQ_START_INDEX] - 2]++;
		} else {
			drvstat_p->rxq_aggrcnt[qid - SC5_BMQ_START_INDEX][AGGRCNT_ARYSIZE - 1]++;
		}
	}
	return;
}
...
/*
	...
	Correct seq: 
		...
		- (1, 0), (0, 0), (0, 1)
*/
static void wldbg_chkrxpkt_seq(struct except_cnt *p_except, wlrxdesc_t * cfh_ul)
{
	...
	if (((cfh_ul->fpkt == 1) && (p_except->in_pkt[idx] == TRUE)) ||	// fpkt=1 & in the middle of pkt
		...
		p_except->cfhul_flpkt_log[idx][0] = p_except->lastpkt_status[idx];
		p_except->cfhul_flpkt_log[idx][1] = cfh_ul->fpkt << 4 | cfh_ul->lpkt;
		return;
	}
	if (cfh_ul->fpkt == 1) {
		// pkt start
...
void wlUnmapBuffer(struct net_device *netdev, u8 * vaddr, u32 size)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);

	dma_unmap_single(wlpptr->wlpd_p->dev, virt_to_phys(vaddr), size, DMA_FROM_DEVICE);
}
...
void wl_txdatpkt_stat_per_sta(struct net_device *netdev, extStaDb_StaInfo_t * pStaInfo, struct sk_buff *skb, U16 llc_type)
{
	SMAC_STA_STATISTICS_st StaStatsTbl = { 0 };
	...
	if (!(pStaInfo->tx_packets % 100)) {
		if (wlFwGetStaStats(netdev, pStaInfo->StnId, &StaStatsTbl) == SUCCESS) {
			u32 retried = StaStatsTbl.dot11FailedRertransCount + StaStatsTbl.dot11RetryCount_1;
			pStaInfo->tx_packets_sampled = pStaInfo->tx_packets;
			pStaInfo->tx_retried_last_100 = retried - pStaInfo->tx_retried_sampled;
			pStaInfo->tx_retried_sampled = retried;
		}
	}
	...
	if (skb) {
		...
		if ((llc_type == 0x0008) && (iph->protocol == IPPROTO_TCP)) {
			pStaInfo->tx_tcp_bytes += skb->len;
		}
	}
...
void wl_txdatpkt_bcast_mcast(struct net_device *netdev, ether_hdr_t * eh, struct sk_buff *skb)
{
	...
	if (is_broadcast_ether_addr((UINT8 *) eh->da))
		wlpptr->wlpd_p->privNdevStats.tx_bcast_bytes += skb->len;
	else if (is_multicast_ether_addr((UINT8 *) eh->da))
...
*/
void wl_mpkt_subtype(struct pkttype_info *wlpkt_type, IEEEtypes_FrameCtl_t * fc)
{
	switch (fc->Subtype) {
...
*/
void wl_cpkt_subtype(struct pkttype_info *wlpkt_type, IEEEtypes_FrameCtl_t * fc)
{
	switch (fc->Subtype) {
	case BLK_ACK_REQ:
...
*/
void wl_get_datpkt_prot(struct pkttype_info *wlpkt_type, U16 llc_type, struct iphdr *iph, u32 len)
{
...
void wl_pkttype_stat(struct pkttype_info *wlpkt_type, IEEEtypes_FrameCtl_t * fc)
{
	switch (fc->Type) {
	case IEEE_TYPE_DATA:
		wlpkt_type->data_cnt++;
...
void pkt_statistics(struct net_device *netdev, int rxQid, struct sk_buff *skb_c)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct pkttype_info *wlpkt_typecnt_p = &wlpd_p->rpkt_type_cnt;

	if ((rxQid == SC5_RXQ_PROMISCUOUS_INDEX) || (rxQid == SC5_RXQ_MGMT_INDEX)) {
		...
		if (pmrvl->FrmCtl.Type == IEEE_TYPE_DATA) {
			...
			wl_get_datpkt_prot(wlpkt_typecnt_p,
					   ((llc_snap_hdr_t *) (&skb_c->data[sizeof(mrvl_hdr_t) + ETHER_HDR_LEN]))->Type,
					   (struct iphdr *)&skb_c->data[sizeof(mrvl_hdr_t) + ETHER_HDR_LEN + sizeof(llc_snap_hdr_t)], skb_c->len);
		...
		wl_get_datpkt_prot(wlpkt_typecnt_p,
				   ((llc_snap_hdr_t *) (&skb_c->data[ETHER_HDR_LEN]))->Type,
				   (struct iphdr *)&skb_c->data[ETHER_HDR_LEN + sizeof(llc_snap_hdr_t)], skb_c->len);
	}

	return;
}

u32 mrvl_hdr_2_80211_hdr_promisc(wlrxdesc_t * cfh_ul, struct sk_buff * skb_c)
{
	u32 offset = 0;
	...
	if (pmrvl->FrmCtl.Type == IEEE_TYPE_DATA) {
		...
		if ((pmrvl->FrmCtl.Subtype == QoS_DATA) || (pmrvl->FrmCtl.Subtype == QoS_NULL_DATA)) {
			if (pmrvl->FrmCtl.FromDs && pmrvl->FrmCtl.ToDs) {
				//remove HTC
				IEEEtypes_Promiscuous_WDS_QoSHdr_t *pHdr;
				skb_c->data += sizeof(mrvl_hdr_t) - sizeof(IEEEtypes_Promiscuous_WDS_QoSHdr_t);
				pHdr = (IEEEtypes_Promiscuous_WDS_QoSHdr_t *) skb_c->data;

				psrc = (u8 *) pmrvl + (size_t) (&((mrvl_hdr_t *) 0)->FrmCtl);
				pdst = (u8 *) pHdr + (size_t) (&((IEEEtypes_Promiscuous_WDS_QoSHdr_t *) 0)->FrmCtl);
				memmove(pdst, psrc,
					sizeof(IEEEtypes_Promiscuous_WDS_QoSHdr_t) - sizeof(((IEEEtypes_Promiscuous_WDS_QoSHdr_t *) 0)->FrmBodyLen));
				offset += sizeof(mrvl_hdr_t) - sizeof(IEEEtypes_Promiscuous_WDS_QoSHdr_t);
			} else {
				//remove addr4, HTC
				IEEEtypes_Promiscuous_QoSHdr_t *pHdr;
				skb_c->data += sizeof(mrvl_hdr_t) - sizeof(IEEEtypes_Promiscuous_QoSHdr_t);
				pHdr = (IEEEtypes_Promiscuous_QoSHdr_t *) skb_c->data;
				...
				offset += sizeof(mrvl_hdr_t) - sizeof(IEEEtypes_Promiscuous_QoSHdr_t);
			}
			...
			memmove(pdst, psrc, sizeof(IEEEtypes_GenHdr_t) - sizeof(((IEEEtypes_GenHdr_t *) 0)->FrmBodyLen));
			offset += sizeof(mrvl_hdr_t) - sizeof(IEEEtypes_GenHdr_t);
		} else {
			//remove addr4, QoS, HTC
			IEEEtypes_Promiscuous_None_QoSHdr_t *pHdr;
			skb_c->data += sizeof(mrvl_hdr_t) - sizeof(IEEEtypes_Promiscuous_None_QoSHdr_t);
			pHdr = (IEEEtypes_Promiscuous_None_QoSHdr_t *) skb_c->data;

			psrc = (u8 *) pmrvl + (size_t) (&((mrvl_hdr_t *) 0)->FrmCtl);
			pdst = (u8 *) pHdr + (size_t) (&((IEEEtypes_Promiscuous_None_QoSHdr_t *) 0)->FrmCtl);
			memmove(pdst, psrc,
				sizeof(IEEEtypes_Promiscuous_None_QoSHdr_t) - sizeof(((IEEEtypes_Promiscuous_None_QoSHdr_t *) 0)->FrmBodyLen));
			offset += sizeof(mrvl_hdr_t) - sizeof(IEEEtypes_Promiscuous_None_QoSHdr_t);
		}
		...
		WLDBG_DATA(DBG_LEVEL_2, "CFHUL: Mgmt/Ctrl Frame Type:%xh, Subtype:%xh\n", pmrvl->FrmCtl.Type, pmrvl->FrmCtl.Subtype);
	} else if (pmrvl->FrmCtl.Type == IEEE_TYPE_CONTROL) {
		...
		psrc = (u8 *) pmrvl + (size_t) (&((mrvl_hdr_t *) 0)->FrmCtl);
		pdst = (u8 *) pHdr + (size_t) (&((IEEEtypes_CtlHdr_t *) 0)->FrmCtl);
		memmove(pdst, psrc, sizeof(IEEEtypes_CtlHdr_t) - sizeof(((IEEEtypes_CtlHdr_t *) 0)->FrmBodyLen));
		offset += sizeof(mrvl_hdr_t) - sizeof(IEEEtypes_CtlHdr_t);
	}

	*(UINT16 *) skb_c->data = cfh_ul->hdr.length - offset - sizeof(((IEEEtypes_GenHdr_t *) 0)->FrmBodyLen);
...
u32 mrvl_hdr_2_80211_hdr_mgmt(wlrxdesc_t * cfh_ul, struct sk_buff * skb_c)
{
	u32 offset = 0;
	mrvl_hdr_t *pmrvl = (mrvl_hdr_t *) skb_c->data;
	u8 *psrc;
 ...
 */
struct sk_buff *wlCfhUlToSkb(struct net_device *netdev, wlrxdesc_t * cfh_ul, int rxQid)
{
	...
	if ((cfh_ul->hdr.bpid < SC5_BMQ_START_INDEX) || ((SC5_BMQ_START_INDEX + SC5_BMQ_NUM) <= cfh_ul->hdr.bpid)) {
	...
	if (cfh_ul->hdr.lo_dword_addr == 0xffffffff || cfh_ul->hdr.lo_dword_addr == 0) {
#ifdef ASSERT_MALBUF
	...
	skb_addr = (u8 *) phys_to_virt(cfh_ul->hdr.lo_dword_addr);
		...
		if (cfh_ul->fpkt == 1) {	// first packet
			...
			if ((*((u32 *) (skb_addr - SKB_INFO_SIZE)) != SKB_SIGNATURE) ||
			    (*((u32 *) (skb_addr + wlqm->rq.bm.buf_size + SKB_TAIL_SIGNATURE_OFFSET)) != SKB_TAIL_SIGNATURE)) {
				skb = NULL;
				// consuming 1 buffer
				wlpd_p->except_cnt.rx_invalid_sig_cnt[wlqm_id - SC5_BMQ_START_INDEX]++;

				if (unlikely((dbg_invalid_skb & dbg_ivalskb_rx))) {
					WLDBG_ERROR(DBG_LEVEL_0, "Invalid signature: qid %u pa %x va %p skb %p sig: head %x tail %x dump cfh_ul:\n",
						    wlqm_id, cfh_ul->hdr.lo_dword_addr, skb_addr,
						    (*(struct sk_buff **)(skb_addr - SKB_POINTER_OFFSET)), (*((u32 *) (skb_addr - SKB_INFO_SIZE))),
						    (*((u32 *) (skb_addr + wlqm->rq.bm.buf_size + SKB_TAIL_SIGNATURE_OFFSET))));
					mwl_hex_dump((u8 *) cfh_ul, sizeof(wlrxdesc_t));
				...
				if (unlikely(!virt_addr_valid(skb))) {
					...
					if ((dbg_invalid_skb & dbg_ivalskb_rx) && (dbg_invalid_skb & dbg_ivalskb_class_1)) {
						WLDBG_ERROR(DBG_LEVEL_0, "Invalid skb addr %p dump chf_ul\n", skb);
						mwl_hex_dump((u8 *) cfh_ul, sizeof(wlrxdesc_t));
				...
				wlpd_p->drv_stats_val.bmqbuf_ret_cnt[wlqm_id - SC5_BMQ_START_INDEX]++;
				...
				if (skb != NULL) {
					if ((skb->next != (struct sk_buff *)&wlpd_p->pend_skb_trace[PENDSKB_RX] && !virt_addr_valid(skb->next)) ||
						...
						if (unlikely((dbg_invalid_skb & dbg_ivalskb_rx) && (dbg_invalid_skb & dbg_ivalskb_class_2))) {
							if (!virt_addr_valid(skb->data) || !virt_addr_valid(skb->data - SKB_INFO_SIZE)) {
								WLDBG_ERROR(DBG_LEVEL_0,
									    "dbgskb: wlCfhUlToSkb qid %d: skb %p link error. data %p next %p prev %p\n",
									    rxQid, skb, skb->data, skb->next, skb->prev);
							} else {
								WLDBG_ERROR(DBG_LEVEL_0,
									    "dbgskb: wlCfhUlToSkb qid %d: skb %p link error. data %p next %p prev %p signature 0x%8x dump skb_data:\n",
									    rxQid, skb, skb->data, skb->next, skb->prev,
		...
		if (skb == NULL) {
#ifdef ASSERT_MALBUF
			WLDBG_ERROR(DBG_LEVEL_1, "CFHUL: without skb, skb_addr:%p  hdr.lo_dword_addr:%x, cfh_ul->hdr.length:%u\n",
				    skb_addr, cfh_ul->hdr.lo_dword_addr, cfh_ul->hdr.length);
#endif
			// Destroy the lo_dword_addr after used
			cfh_ul->hdr.used_signature = BMBUF_SIGNATURE;
			cfh_ul->hdr.length = USED_BUFLEN;
...
#ifdef ASSERT_MALBUF
			WLDBG_ERROR(DBG_LEVEL_0, "data size is over-size %d  bpid %d \n", cfh_ul->hdr.length, cfh_ul->hdr.bpid);
			mwl_hex_dump((u8 *) cfh_ul, sizeof(wlrxdesc_t));
#endif
			// Destroy the lo_dword_addr after used
			cfh_ul->hdr.used_signature = BMBUF_SIGNATURE;
		...
		if (unlikely(!virt_addr_valid(skb_shinfo(skb)) || !virt_addr_valid(skb->head))) {
			wlpd_p->except_cnt.skb_invalid_addr_cnt++;
			if ((dbg_invalid_skb & dbg_ivalskb_rx) && (dbg_invalid_skb & dbg_ivalskb_class_1)) {
				WLDBG_ERROR(DBG_LEVEL_0, "Invalid skb addr %p skb->head %p skb_shinfo %p dump chf_ul\n", skb, skb->head,
					    skb_shinfo(skb));
				mwl_hex_dump((u8 *) cfh_ul, sizeof(wlrxdesc_t));
	...
	skb_c = skb_clone(skb, GFP_ATOMIC);
		if (cfh_ul->fpkt == 1) {	// first packet
			...
			WLDBG_DATA(DBG_LEVEL_3, "CFHUL: [RxBufFetch] first skb_c->data %p , address_31_0 = %xh \n", skb_c->data,
				   cfh_ul->hdr.lo_dword_addr);
			if (skb_c->data != skb->data) {
				WLDBG_ERROR(DBG_LEVEL_0, "1st pkt, (skb->data, skb_c->data)=(%p, %p)\n", skb->data, skb_c->data);
			}
		} else {
	...
	// Save the fc for debugging purpose
	if (wlpptr->cb_enable == TRUE) {
		memcpy(skb_c->cb, &cfh_ul->frame_ctrl, sizeof(IEEEtypes_FrameCtl_t));
	}
...

	{
		...
		if ((skb_c->data - skb->data) > buf_pool_size[cfh_ul->hdr.bpid - SC5_BMQ_START_INDEX]) {
			/*
	...
	if (skb_c->head > skb_c->data) {	//Invalid skb structure => 
		...
		if (skb_c->tail + (cfh_ul->hdr.length - offset) > skb_c->end) {
			...
			if (unlikely(skb_c->data_len)) {
				if ((dbg_invalid_skb & dbg_ivalskb_rx) && (dbg_invalid_skb & dbg_ivalskb_class_3)) {
					WLDBG_ERROR(DBG_LEVEL_0, "skb is nonlinear %p skb->data_len %u \n", skb_c, skb_c->data_len);
					dbg_coredump(netdev);
				}
	...
	if ((rxQid == SC5_RXQ_START_INDEX) && (*(u16 *) & skb_c->data[14] != 0xaaaa) && (*(u16 *) & skb_c->data[14] == *(u16 *) & skb_c->data[6])) {
		wlpd_p->except_cnt.cnt_invalid_mpdu_frames++;
	}

	if (cfh_ul->lpkt == 1) {	// free bit is on, last packet
		WLDBG_DATA(DBG_LEVEL_3, "CFHUL: [RxDropBuf] last data  %p  len %d \n", skb->data, skb_c->len);
 ...
 */
static void wl_show_rx_stats(struct net_device *netdev, char *sysfs_buff)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	int qid, i;
	...
	for (qid = SC5_RXQ_START_INDEX; qid < (SC5_RXQ_START_INDEX + SC5_RXQ_NUM); qid++) {
		...
		Sysfs_Printk("Q[%d]= %u, ", qid, wlpd_p->drv_stats_val.rxq_intr_cnt[qid]);
	...
	for (qid = SC5_RXQ_START_INDEX; qid < (SC5_RXQ_START_INDEX + SC5_RXQ_NUM); qid++) {
		...
		Sysfs_Printk("Q[%d]= %u, ", qid, wlpd_p->drv_stats_val.rxq_rcv_cnt[qid]);
	...
	for (qid = SC5_BMQ_START_INDEX; qid < SC5_BMQ_START_INDEX + SC5_BMQ_NUM; qid++) {
		Sysfs_Printk("Q[%d] = %d, ", qid, wlpd_p->drv_stats_val.rx_drop_cnt[qid - SC5_BMQ_START_INDEX]);
	...
	for (qid = SC5_BMQ_START_INDEX; qid < SC5_BMQ_START_INDEX + SC5_BMQ_NUM - 1; qid++) {
		for (i = 0; i < AGGRCNT_ARYSIZE; i++) {
			if (wlpd_p->drv_stats_val.rxq_aggrcnt[qid - SC5_BMQ_START_INDEX][i] == 0) {
				continue;
			}
			Sysfs_Printk("Q[%d], %d pkts %d times ", qid, i + 2, wlpd_p->drv_stats_val.rxq_aggrcnt[qid - SC5_BMQ_START_INDEX][i]);
	...
	Sysfs_Printk("pn check data passed count: ucast=%d, mcast=%d\n",
		     wlpd_p->drv_stats_val.rx_data_ucast_pn_pass_cnt, wlpd_p->drv_stats_val.rx_data_mcast_pn_pass_cnt);
	Sysfs_Printk("pn check mgmt passed count: ucast=%d, mcast=%d\n\n",
		     wlpd_p->drv_stats_val.rx_mgmt_ucast_pn_pass_cnt, wlpd_p->drv_stats_val.rx_mgmt_mcast_pn_pass_cnt);
 ...
 */
static void wl_show_scheduleinfo(struct net_device *netdev, int level, char *sysfs_buff)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	...
	if (level == 0 && (addr_val != NULL)) {
		fwDbgStateAddr = readl(wlpptr->ioBase1 + wlpptr->wlpd_p->reg.FwDbgStateAddr);

		wlFwGetAddrValue(netdev, (fwDbgStateAddr + 0x134), 1, addr_val, 0);
		Sysfs_Printk("schedulercount  : 0x%X\n", addr_val[0]);
		wlFwGetAddrValue(netdev, (fwDbgStateAddr + 0x138), 1, addr_val, 0);
		Sysfs_Printk("schedulercount1 : 0x%X\n", addr_val[0]);
		wlFwGetAddrValue(netdev, (fwDbgStateAddr + 0x13C), 1, addr_val, 0);
		Sysfs_Printk("schedulercount2 : 0x%X\n", addr_val[0]);
		wlFwGetAddrValue(netdev, (fwDbgStateAddr + 0x140), 1, addr_val, 0);
		Sysfs_Printk("schedulercount3 : 0x%X\n", addr_val[0]);
		wlFwGetAddrValue(netdev, (fwDbgStateAddr + 0x144), 1, addr_val, 0);
		Sysfs_Printk("schedulercount4 : 0x%X\n", addr_val[0]);
		wlFwGetAddrValue(netdev, (fwDbgStateAddr + 0x148), 1, addr_val, 0);
		Sysfs_Printk("schedulercount5 : 0x%X\n", addr_val[0]);
		wlFwGetAddrValue(netdev, (fwDbgStateAddr + 0x14C), 1, addr_val, 0);
		Sysfs_Printk("schedulercount6 : 0x%X\n", addr_val[0]);
		wlFwGetAddrValue(netdev, (fwDbgStateAddr + 0x17C), 1, addr_val, 0);
...
#ifdef TP_PROFILE
static void wl_show_tx_profile(struct net_device *netdev, int level, char *sysfs_buff)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;

	calculateTpStats(&wlpd_p->drv_stats_val.cfhdltx_stat, level, sysfs_buff);
}
#endif
...
static void wl_show_generic_info(struct net_device *netdev, char *sysfs_buff)
{
	...
	if (wlpptr->vmacSta_p->master)
		...
		stat = &(wlpptr->phyif_netDevStats);
	}

	memcpy(pSMACStatus, wlpd_p->masterwlp->smacStatusAddr, sizeof(SMAC_STATUS_st));
	...
	for (i = SC5_BMQ_START_INDEX; i < (SC5_BMQ_START_INDEX + SC5_BMQ_NUM - 1); i++) {
		struct wldesc_data *wlqm = &wlpptr->wlpd_p->descData[i];
		Sysfs_Printk("\t BMQ[%d] recycle len: %u\n", i, skb_queue_len(&wlqm->rq.skbTrace));
	}
	...
	for (i = SC5_BMQ_START_INDEX; i < SC5_BMQ_START_INDEX + SC5_BMQ_NUM; i++) {
		...
		Sysfs_Printk("\t Q[%d] = %d, (eq=%d, drop=%d, ret=%d)\n", i,
			     (wldrvstat_p->enq_bmqbuf_cnt[offset] - wldrvstat_p->xx_buf_free_SQ14[offset] - wldrvstat_p->bmqbuf_ret_cnt[offset]),
			     wldrvstat_p->enq_bmqbuf_cnt[offset], wldrvstat_p->xx_buf_free_SQ14[offset], wldrvstat_p->bmqbuf_ret_cnt[offset]);
	}
	Sysfs_Printk("\n");
	Sysfs_Printk("EventQ\t\tRD ptr\t\tWR ptr\n");
	Sysfs_Printk(" \t\t%08x\t%08x\n", readl(wlpptr->ioBase1 + reg_evt_rdptr), readl(wlpptr->ioBase1 + reg_evt_wrptr));
	...
	Sysfs_Printk("txq_drv_release_cnt = %d - %d - %d - %d\n",
		     ...
		     wlpd_p->drv_stats_val.txq_drv_release_cnt[2], wlpd_p->drv_stats_val.txq_drv_release_cnt[3]);
#ifdef DUPLICATED_MGMT_DBG
	Sysfs_Printk("RX re-transmitted MGMT packet count = %d\n", wlpptr->rx_retry_mgmt_cnt);
#endif

	Sysfs_Printk("AMSDU subframe typecnt (fpkt,mpkt,lpkt,single-msdu):%u,%u,%u,%u\n",
		     wlpd_p->drv_stats_val.amsdu_frag[0], wlpd_p->drv_stats_val.amsdu_frag[1], wlpd_p->drv_stats_val.amsdu_frag[2],
		     wlpd_p->drv_stats_val.amsdu_frag[3]);
...
static void wl_show_hframe_info(struct net_device *netdev, char *sysfs_buff)
{
	...
	for (qid = SC5_BMQ_START_INDEX; qid < (SC5_BMQ_START_INDEX + SC5_BMQ_NUM); qid++) {
		Sysfs_Printk("%d\t\t%08x\t%08x\n", qid, wlQueryRdPtr(netdev, qid, SC5_RQ), wlQueryWrPtr(netdev, qid, SC5_RQ));
	}
	Sysfs_Printk("\n");
	...
	for (qid = SC5_RXQ_START_INDEX; qid < (SC5_RXQ_START_INDEX + SC5_RXQ_NUM); qid++) {
		...
		Sysfs_Printk("%d\t\t%08x\t%08x\n", qid, wlQueryRdPtr(netdev, qid, SC5_SQ), wlQueryWrPtr(netdev, qid, SC5_SQ));
	}
	Sysfs_Printk("\n");
...
void wl_show_pfw_alive_counts(struct net_device *netdev, char *sysfs_buff)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	//struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	UINT32 newcnt[3];
	...
	if (IS_BUS_TYPE_MCI(wlpptr)) {	//MOCHI 
		...
		dmemaddr = readl(wlpptr->ioBase1 + wlpptr->wlpd_p->reg.FwDbgStateAddr) - SMAC_DMEM_START;
		...
		memcpy((void *)newcnt, (void *)(pfwdbgstate + PFW_ALIVE_CNT_OFFSET), sizeof(newcnt));
	} else {		//PCIE interface
		u8 *pbuf = NULL;
...
#define DMEM_DEBUG_START_OFFSET 0x123800
		if ((pbuf = (u8 *) wl_kmalloc(1024, GFP_KERNEL))) {
			...
			if (!wlFwGetAddrValue(netdev, (SMAC_DMEM_START + DMEM_DEBUG_START_OFFSET), 64, (u32 *) pbuf, 0)) {
				memcpy((void *)newcnt, (void *)(pbuf + PFW_ALIVE_CNT_OFFSET), sizeof(newcnt));
			} else {
				wl_kfree(pbuf);
				goto err_exit;
			}
...
void wl_show_smac_stat(struct net_device *netdev, SMAC_STATUS_st * pSMACStatus, char *sysfs_buff)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	SMAC_STATUS_st smacStatus;
	...
	Sysfs_Printk(" 0x420: txInputCnt %u\ttxSchedCnt %u\ttxProcCnt %u\ttxBufRetCnt %u\n",
		     pSMACStatus->txInputCnt, pSMACStatus->txSchedCnt, pSMACStatus->txProcCnt, pSMACStatus->txBufRetCnt);

	Sysfs_Printk(" 0x430: txAcRingCnt %u\ttxCtlFrmCnt %u\ttxEuDoneCnt %u\ttxRdyDeassert %u\n",
		     pSMACStatus->txAcRingCnt, pSMACStatus->txCtlFrmCnt, pSMACStatus->txEuDoneCnt, pSMACStatus->txRdyDeassert);

	Sysfs_Printk(" 0x440: sop_EvtMacHdr %u\teop_EvtEuDone %u\tfcs_EvtFcs %u\teop2_Q2RxAmsdu %u\n",
		     pSMACStatus->sop_EvtMacHdr, pSMACStatus->eop_EvtEuDone, pSMACStatus->fcs_EvtFcs, pSMACStatus->eop2_Q2RxAmsdu);

	Sysfs_Printk(" 0x450: sop_EuPrgm %u\teopDrp_EuErr %u\tfcsDrp_FcsErr %u\teop2Drp_Cnt %u\n",
		     pSMACStatus->sop_EuPrgm, pSMACStatus->eopDrp_EuErr, pSMACStatus->fcsDrp_FcsErr, pSMACStatus->eop2Drp_Cnt);

	Sysfs_Printk(" 0x460: uniPktCnt %u\tmultiPktCnt %u\tbman_GetBuf %u\tbman_RetBuf %u\n",
		     pSMACStatus->fcs_UniMCast[0], pSMACStatus->fcs_UniMCast[1], pSMACStatus->bman_GetBuf, pSMACStatus->bman_RetBuf);

	Sysfs_Printk(" 0x470: slotTickCnt %u\tfirstSlotTickCnt %u\ttxMgmPktCnt %u\ttxBcnCnt %u\n",
		     pSMACStatus->slotTickCnt, pSMACStatus->firstSlotTickCnt, pSMACStatus->txMgmPktCnt, pSMACStatus->txBcnCnt);

	Sysfs_Printk(" 0x480: debug log1:\n");
	for (i = 0, k = 0; i < 1; i++, k += 4) {
		Sysfs_Printk("%08x: %08x %08x\n", (i * 16), pSMACStatus->sysRsvd1[k], pSMACStatus->sysRsvd1[k + 1]);
	}

	Sysfs_Printk(" 0x488: navBlockCnt txBcnAbort\n");
	Sysfs_Printk("%08x: %08x %08x\n", 0x0, pSMACStatus->navBlockCnt, pSMACStatus->navBlockCnt);

	Sysfs_Printk(" 0x490: TXD1 counts:\n");
	for (i = 0, k = 0; i < 1; i++, k += 4) {
		Sysfs_Printk("%08x: %08x %08x %08x %08x\n", (i * 16), pSMACStatus->sysRsvd2[k], pSMACStatus->sysRsvd2[k + 1],
			     pSMACStatus->sysRsvd2[k + 2], pSMACStatus->sysRsvd2[k + 3]);
	}

	Sysfs_Printk(" 0x4a0: PGM %u, DONE %u, MSDU %u, MPDU %u\n",
		     pSMACStatus->sysRsvd3[0], pSMACStatus->sysRsvd3[1], pSMACStatus->sysRsvd3[2], pSMACStatus->sysRsvd3[3]);

	Sysfs_Printk(" 0x4b0: TXD5 counts:\n");
	for (i = 0, k = 0; i < 1; i++, k += 4) {
		Sysfs_Printk("%08x: %08x %08x %08x %08x\n", (i * 16), pSMACStatus->sysRsvd4[k], pSMACStatus->sysRsvd4[k + 1],
			     pSMACStatus->sysRsvd4[k + 2], pSMACStatus->sysRsvd4[k + 3]);
	}

	Sysfs_Printk(" 0x4c0: debug log2:\n");
	for (i = 0, k = 0; i < 1; i++, k += 4) {
		Sysfs_Printk("%08x: %08x %08x %08x %08x\n", (i * 16), pSMACStatus->sysRsvd5[k], pSMACStatus->sysRsvd5[k + 1],
			     pSMACStatus->sysRsvd5[k + 2], pSMACStatus->sysRsvd5[k + 3]);
	}
	//mwl_hex_dump_to_sysfs(pSMACStatus->sysRsvd5, 16, sysfs_buff);

	Sysfs_Printk(" 0x4d0: sysRsvd6:\n");
	for (i = 0, k = 0; i < 3; i++, k += 4) {
		Sysfs_Printk("%08x: %08x %08x %08x %08x\n", (i * 16), pSMACStatus->sysRsvd6[k], pSMACStatus->sysRsvd6[k + 1],
			     pSMACStatus->sysRsvd6[k + 2], pSMACStatus->sysRsvd6[k + 3]);
	}

	Sysfs_Printk(" 0x500: MAC status:\n");
	for (i = 0, k = 0; i < 3; i++, k += 4) {
		Sysfs_Printk("%08x: %08x %08x %08x %08x\n", (i * 16), pSMACStatus->smacSts[k], pSMACStatus->smacSts[k + 1],
			     pSMACStatus->smacSts[k + 2], pSMACStatus->smacSts[k + 3]);
	}
	Sysfs_Printk("%08x: %08x %08x %08x %08x\n", (i * 16), pSMACStatus->sysRsvd7[0], pSMACStatus->sysRsvd7[1], pSMACStatus->txAcntNoADMA,
		     pSMACStatus->rxAcntNoADMA);

	Sysfs_Printk(" 0x540: sopDrp_GiantPkt %u, txStopAck %u, lastTxInfoErr %u, bmanErr_GetBuf %u,%u,%u,%u\n",
		     pSMACStatus->sopDrp_GiantPkt, pSMACStatus->txStopAck,
		     pSMACStatus->lastTxInfoErr,
		     pSMACStatus->bmanErr_GetBuf[0], pSMACStatus->bmanErr_GetBuf[1], pSMACStatus->bmanErr_GetBuf[2], pSMACStatus->bmanErr_GetBuf[3]);

	Sysfs_Printk(" 0x550: eopDrp_EmptyBuf %u\n", pSMACStatus->eopDrp_EmptyBuf);
	Sysfs_Printk(" 0x560: txDataTxMsduCnt %u, txDataBufRetMsduCnt %u, txMgtTxMsduCnt %u, txMgtBufRetMsduCnt %u\n",
		     pSMACStatus->txDataTxMsduCnt, pSMACStatus->txDataBufRetMsduCnt, pSMACStatus->txMgtTxMsduCnt, pSMACStatus->txMgtBufRetMsduCnt);

	Sysfs_Printk(" 0x570: bman_StsReqBp %u, maxSizeBcnbuf %u, rxSBinfoBaseAddr %xh, rxSBinfoUnitSize %u\n",
		     pSMACStatus->bman_StsReqBp, pSMACStatus->maxSizeBcnbuf, pSMACStatus->rxSBinfoBaseAddr, pSMACStatus->rxSBinfoUnitSize);

	Sysfs_Printk(" 0x580: sysRsvd9:\n");
	for (i = 0, k = 0; i < 4; i++, k += 4) {
		Sysfs_Printk("%08x: %08x %08x %08x %08x\n", (i * 16), pSMACStatus->sysRsvd9[k], pSMACStatus->sysRsvd9[k + 1],
			     pSMACStatus->sysRsvd9[k + 2], pSMACStatus->sysRsvd9[k + 3]);
	}

	Sysfs_Printk(" 0x5c0: sysRsvd10:\n");
	for (i = 0, k = 0; i < 5; i++, k += 4) {
		Sysfs_Printk("%08x: %08x %08x %08x %08x\n", (i * 16), pSMACStatus->sysRsvd10[k], pSMACStatus->sysRsvd10[k + 1],
			     pSMACStatus->sysRsvd10[k + 2], pSMACStatus->sysRsvd10[k + 3]);
	}

	Sysfs_Printk(" 0x610: sysRsvdMU:\n");
	for (i = 0, k = 0; i < 3; i++, k += 4) {
		Sysfs_Printk("%08x: %08x %08x %08x %08x\n", (i * 16), pSMACStatus->sysRsvdMU[k], pSMACStatus->sysRsvdMU[k + 1],
			     pSMACStatus->sysRsvdMU[k + 2], pSMACStatus->sysRsvdMU[k + 3]);
	}

	Sysfs_Printk(" 0x640: sysRsvd11:\n");
	for (i = 0, k = 0; i < 4; i++, k += 4) {
		Sysfs_Printk("%08x: %08x %08x %08x %08x\n", (i * 16), pSMACStatus->sysRsvd11[k], pSMACStatus->sysRsvd11[k + 1],
			     pSMACStatus->sysRsvd11[k + 2], pSMACStatus->sysRsvd11[k + 3]);
	}

	Sysfs_Printk(" 0x680: sysRsvd12:\n");
	for (i = 0, k = 0; i < 1; i++, k += 4) {
		Sysfs_Printk("%08x: %08x %08x %08x %08x\n", (i * 16), pSMACStatus->sysRsvd12[k], pSMACStatus->sysRsvd12[k + 1],
			     pSMACStatus->sysRsvd12[k + 2], pSMACStatus->sysRsvd12[k + 3]);
	}

	Sysfs_Printk(" 0x690: CSI Information:\n");
	Sysfs_Printk("CSI_Pkt_RAW_RSSI - AB: %d CD: %d EF: %d GH: %d\n", pSMACStatus->CSI_RSSI_AB, pSMACStatus->CSI_RSSI_CD, pSMACStatus->CSI_RSSI_EF,
		     pSMACStatus->CSI_RSSI_GH);
	Sysfs_Printk("CSI_Pkt_MAC_Addr - %02x:%02x:%02x:%02x:%02x:%02x\n", pSMACStatus->CSI_Pkt_MAC_Addr[0], pSMACStatus->CSI_Pkt_MAC_Addr[1],
		     pSMACStatus->CSI_Pkt_MAC_Addr[2], pSMACStatus->CSI_Pkt_MAC_Addr[3], pSMACStatus->CSI_Pkt_MAC_Addr[4],
	...
	Sysfs_Printk("%08x %08x\n", pSMACStatus->cm3StartFlag, pSMACStatus->cm3StopFlag);

	Sysfs_Printk(" 0x6c0: txBcnCntBss:\n");
	for (i = 0, k = 0; i < 4; i++, k += 8) {
		Sysfs_Printk("%08x: %04x %04x %04x %04x : %04x %04x %04x %04x\n", (i * 16),
			     pSMACStatus->txBcnCntBss[k], pSMACStatus->txBcnCntBss[k + 1], pSMACStatus->txBcnCntBss[k + 2],
			     pSMACStatus->txBcnCntBss[k + 3], pSMACStatus->txBcnCntBss[k + 4], pSMACStatus->txBcnCntBss[k + 5],
			     pSMACStatus->txBcnCntBss[k + 6], pSMACStatus->txBcnCntBss[k + 7]);
	}
...
void wl_show_except_cnt(struct net_device *netdev, char *sysfs_buff)
{
	...
	for (i = SC5_BMQ_START_INDEX; i < SC5_BMQ_START_INDEX + SC5_BMQ_NUM; i++) {
		Sysfs_Printk("\t Q[%d] = %u\n", i, wlpd_p->drv_stats_val.bmqbuf_alloc_fail_cnt[i - SC5_BMQ_START_INDEX]);
	}
	Sysfs_Printk("\nRx info:\n");
	...
	Sysfs_Printk("rcvd pkts SQ0:%u SQ8:%u, SQ9:%u\n", wlexcept_p->qidcnt[0], wlexcept_p->qidcnt[1], wlexcept_p->qidcnt[2]);
	Sysfs_Printk("buffer free count by SQ14.  SQ[10]=%u, SQ[11]:%u, SQ[12]:%u, SQ[X]=%u\n",
		     wldrvstat_p->xx_buf_free_SQ14[0],
		     wldrvstat_p->xx_buf_free_SQ14[1], wldrvstat_p->xx_buf_free_SQ14[2], wldrvstat_p->xx_buf_free_SQ14[3]);
	...
	for (i = SC5_BMQ_START_INDEX; i < SC5_BMQ_START_INDEX + SC5_BMQ_NUM; i++) {
		Sysfs_Printk("\t Q[%d] = %u\n", i, wlexcept_p->rx_invalid_sig_cnt[i - SC5_BMQ_START_INDEX]);
	}
	...
	for (i = SC5_BMQ_START_INDEX; i < SC5_BMQ_START_INDEX + SC5_BMQ_NUM; i++) {
		...
		Sysfs_Printk("q[%d](fpkt,lpkt)_error: %u\n", i, wlexcept_p->cfhul_flpkt_error[idx]);
		if (wlexcept_p->cfhul_flpkt_error[idx] > 0) {
			Sysfs_Printk("\t\t err_cond: [%u, %u], [%u, %u]\n",
				     (wlexcept_p->cfhul_flpkt_log[idx][0] >> 4), (wlexcept_p->cfhul_flpkt_log[idx][0] & 0xf),
				     (wlexcept_p->cfhul_flpkt_log[idx][1] >> 4), (wlexcept_p->cfhul_flpkt_log[idx][1] & 0xf));
		}
	}
	...
	Sysfs_Printk("Lost AMSDU subframes (fpkt,midle,lpkt)=%u,%u,%u\n",
		     wlexcept_p->cfhul_flpkt_lost[0], wlexcept_p->cfhul_flpkt_lost[1], wlexcept_p->cfhul_flpkt_lost[2]);
	...
	Sysfs_Printk("Incorrect PN count: ucast = %u, mcast= %u, mgmt= %u\n",
		     wlexcept_p->badPNcntUcast, wlexcept_p->badPNcntMcast, wlexcept_p->badPNcntMgmtcast);
	...
	Sysfs_Printk("Skip feeding the starving queue q(10, 11, 12, 13)=(%d, %d, %d, %d)\n",
		     wlexcept_p->skip_feed_starv[0], wlexcept_p->skip_feed_starv[1], wlexcept_p->skip_feed_starv[2], wlexcept_p->skip_feed_starv[3]);

	{
		...
		for (qid = SC5_RXQ_START_INDEX; qid < SC5_RXQ_START_INDEX + SC5_RXQ_NUM; qid++) {
			...
			Sysfs_Printk("SQ[%u]: %u\n", qid, wlexcept_p->qfull_empty[qid][SC5_SQ]);
		}

		Sysfs_Printk("[q_full_empty]: Tx:\n");
		for (qid = pbqm_args->txq_start_index; qid < pbqm_args->txq_start_index + pbqm_args->txq_num; qid++) {
			Sysfs_Printk("RQ[%u]: %u\n", qid, wlexcept_p->qfull_empty[qid][SC5_RQ]);
		}

		Sysfs_Printk("[q_full_empty]: BMQQ:\n");
		for (qid = SC5_BMQ_START_INDEX; qid < (SC5_BMQ_START_INDEX + SC5_BMQ_NUM); qid++) {
			Sysfs_Printk("RQ[%u]: %u\n", qid, wlexcept_p->qfull_empty[i][SC5_RQ]);
		}
	...
	Sysfs_Printk("Free pkts from err_cfhul: %u, %u, %u\n",
		     wlexcept_p->free_err_pkts[0], wlexcept_p->free_err_pkts[1], wlexcept_p->free_err_pkts[2]);
	Sysfs_Printk("lpkt missing: %u, %u, %u\n", wlexcept_p->lpkt_miss[0], wlexcept_p->lpkt_miss[1], wlexcept_p->lpkt_miss[2]);
...
static void wl_show_statusage(struct net_device *netdev, char *sysfs_buff)
{
...
static void wl_show_tx_mu_pkcnt_msg(struct net_device *netdev, char *sysfs_buff)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	UINT32 reg;
	...
	reg = wlpptr->wlpd_p->masterwlp->smacStatusAddr->sysRsvdMU[3];
	...
	Sysfs_Printk("[MU-MIMO]: %8u\n", wlpptr->wlpd_p->masterwlp->smacStatusAddr->sysRsvdMU[11]);

}

static void wl_show_pktcnt_msg(struct pkttype_info *wlpkttype_p, char *sysfs_buff)
{
	Sysfs_Printk("pkt_data: %u, pkt_mgmt: %u, pkt_ctrl: %u\n", wlpkttype_p->data_cnt, wlpkttype_p->mgmt_cnt, wlpkttype_p->ctrl_cnt);
	Sysfs_Printk("[mgmt]:\n");
	Sysfs_Printk("\t auth: %u, assoc_req: %u, assoc_resp: %u\n", wlpkttype_p->auth_cnt, wlpkttype_p->assoc_req_cnt, wlpkttype_p->assoc_resp_cnt);
	Sysfs_Printk("\t re-assoc_req: %u, re-assoc_resp: %u\n", wlpkttype_p->reassoc_req_cnt, wlpkttype_p->reassoc_resp_cnt);

	Sysfs_Printk("\t disassoc: %u, deauth: %u\n", wlpkttype_p->disassoc_cnt, wlpkttype_p->deauth_cnt);

	Sysfs_Printk("\t prob_req: %u, prob_resp: %u, beacon: %u\n", wlpkttype_p->prob_req_cnt, wlpkttype_p->prob_resp_cnt, wlpkttype_p->beacon_cnt);
	...
	Sysfs_Printk("\t tcp: %u, udp: %u, icmp: %u, arp: %u\n",
		     wlpkttype_p->tcp_cnt, wlpkttype_p->udp_cnt, wlpkttype_p->icmp_cnt, wlpkttype_p->arp_cnt);
...
static void wl_show_dra_stat(struct net_device *netdev, char *sysfs_buff)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	...
	if (IS_BUS_TYPE_MCI(wlpptr)) {	//MOCHI 
		dmemaddr = readl(wlpptr->ioBase1 + wlpptr->wlpd_p->reg.FwDbgStateAddr) - SMAC_DMEM_START;
		pfwdbgstate = (void *)(wlpptr->ioBase0 + dmemaddr);
		memcpy((void *)tmp_data, (void *)(pfwdbgstate + (PFW_DRA_STAT_CNT_OFFSET - 0x10)), sizeof(tmp_data));
	} else {		//PCIE interface
		u8 *pbuf = NULL;
...
#define DMEM_DEBUG_START_OFFSET 0x123800
		if ((pbuf = (u8 *) wl_kmalloc(1024, GFP_KERNEL))) {
			...
			if (!wlFwGetAddrValue
			    (netdev, (SMAC_DMEM_START + DMEM_DEBUG_START_OFFSET + PFW_DRA_STAT_CNT_OFFSET - 0x10), 64, (u32 *) pbuf, 0)) {
				memcpy((void *)tmp_data, (void *)pbuf, sizeof(tmp_data));
			} else {
				wl_kfree(pbuf);
				goto error_exit;
			}
	...
	memcpy(&dra_stat_cnt[0], &tmp_data[reg_pc + 4], (20 - reg_pc) * sizeof(UINT32));
	if (reg_pc != 0) {
		memcpy(&dra_stat_cnt[20 - reg_pc], &tmp_data[4], reg_pc * sizeof(UINT32));
	}

	Sysfs_Printk("\n======== [DRA statistics counters] ========\n");
	Sysfs_Printk("\n                         [1]      [2]      [3]      [4]      [5]\n");
	Sysfs_Printk(" [PFW] MPDUCount    = %8d %8d %8d %8d %8d\n", dra_stat_cnt[0], dra_stat_cnt[4], dra_stat_cnt[8], dra_stat_cnt[12],
		     dra_stat_cnt[16]);
	Sysfs_Printk(" [PFW] SuccessCount = %8d %8d %8d %8d %8d\n", dra_stat_cnt[1], dra_stat_cnt[5], dra_stat_cnt[9], dra_stat_cnt[13],
		     dra_stat_cnt[17]);
	Sysfs_Printk(" [PFW] RetryCount   = %8d %8d %8d %8d %8d\n", dra_stat_cnt[2], dra_stat_cnt[6], dra_stat_cnt[10], dra_stat_cnt[14],
		     dra_stat_cnt[18]);
	Sysfs_Printk(" [PFW] PER          = %8d %8d %8d %8d %8d\n", dra_stat_cnt[3], dra_stat_cnt[7], dra_stat_cnt[11], dra_stat_cnt[15],
		     dra_stat_cnt[19]);
...
static void wl_show_ba_reorder_stat(struct net_device *netdev, char *sysfs_buff)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	...
	if (entries) {
		...
		listBuf = staBuf;
		for (i = 0; i < entries; i++) {
			if ((StaInfo_p = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) listBuf, STADB_DONT_UPDATE_AGINGTIME)) != NULL) {

				if (StaInfo_p->State == ASSOCIATED) {
					int i;
					Ampdu_Pck_Reorder_t *baRxInfo = &wlpptr->wlpd_p->AmpduPckReorder[StaInfo_p->StnId];

					Sysfs_Printk(" StnId: %d MAC Addr: %s\n", StaInfo_p->StnId, mac_display(StaInfo_p->Addr));
					Sysfs_Printk(" Pkt sent to host count:\n");
					for (i = 0; i < MAX_TID; i++)
						Sysfs_Printk(" tid#%d=%d", i, StaInfo_p->rxBaStats[i].BA_Rodr2Host);
					Sysfs_Printk("\n");
					Sysfs_Printk(" Pkt drop cnt: Duplicate pkt:\n");
					for (i = 0; i < MAX_TID; i++)
						Sysfs_Printk(" tid#%d = %d", i, StaInfo_p->rxBaStats[i].BA_RodrDupDropCnt);
					Sysfs_Printk("\n");
					Sysfs_Printk(" Pkt drop cnt: Out of range seqno, SN>wExt:\n");
					for (i = 0; i < MAX_TID; i++)
						Sysfs_Printk(" tid#%d = %d", i, StaInfo_p->rxBaStats[i].BA_RodrOoRDropCnt);
					Sysfs_Printk("\n");
					Sysfs_Printk(" Pkt drop cnt: Retry:\n");
					for (i = 0; i < MAX_TID; i++)
						Sysfs_Printk(" tid#%d = %d", i, StaInfo_p->rxBaStats[i].BA_RodrRetryDropCnt);
					Sysfs_Printk("\n");
					Sysfs_Printk(" Pkt drop cnt: Enqueue to buf error:\n");
					for (i = 0; i < MAX_TID; i++)
						Sysfs_Printk(" tid#%d = %d", i, StaInfo_p->rxBaStats[i].BA_RodrAmsduEnQCnt);
					Sysfs_Printk("\n");
					Sysfs_Printk(" Pkt drop cnt: Timeout:\n");
					for (i = 0; i < MAX_TID; i++)
						Sysfs_Printk(" tid#%d = %d", i, StaInfo_p->rxBaStats[i].BA_RodrTMODropCnt);
					Sysfs_Printk("\n");
					Sysfs_Printk(" Pkt drop cnt: Missing pkt in flush any:\n");
					for (i = 0; i < MAX_TID; i++)
						Sysfs_Printk(" tid#%d = %d", i, StaInfo_p->rxBaStats[i].BA_RodrFlushDropCnt);
					Sysfs_Printk("\n");
					Sysfs_Printk(" WinEnd jump count, wEnd<SN<=wExt:\n");
					for (i = 0; i < MAX_TID; i++)
						Sysfs_Printk(" tid#%d = %d", i, StaInfo_p->rxBaStats[i].BA_RodrWinEndJumpCnt);
					Sysfs_Printk("\n");
					Sysfs_Printk(" The last drop reason:\n");
					for (i = 0; i < MAX_TID; i++)
						Sysfs_Printk(" tid#%d = %d", i, StaInfo_p->rxBaStats[i].BA_RodrLastDropReason);
					Sysfs_Printk("\n");
					Sysfs_Printk(" Window starting seqno:\n");
					for (i = 0; i < MAX_TID; i++)
						Sysfs_Printk(" tid#%d = %d", i, baRxInfo->ba[i].winStartB);
					Sysfs_Printk("\n");
					Sysfs_Printk(" Window ending seqno:\n");
					for (i = 0; i < MAX_TID; i++)
						Sysfs_Printk(" tid#%d = %d", i,
							     (baRxInfo->ba[i].winStartB + baRxInfo->ba[i].winSizeB - 1) & BA_MAX_SEQ_NUM);
					Sysfs_Printk("\n\n");
				}
...
void wl_show_txqos_stat(struct net_device *netdev, char *sysfs_buff)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	...
	Sysfs_Printk("\n======== [Tx Qos stats] ========\n");
	for (i = 0; i < 4; i++) {
		Sysfs_Printk("TxQ[%s]: Qlen %4d    Tx %8d    Drop: Qfull %8d    skb %8d    dfs %8d    if %8d    cfh %8d\n",
			     buf[i], wlpptr->wlpd_p->txQ[i].qlen, wlpd_p->drv_stats_val.txq_ac_stats[i].tx,
			     wlpd_p->drv_stats_val.txq_ac_stats[i].drop_qfl, wlpd_p->drv_stats_val.txq_ac_stats[i].drop_skb,
			     wlpd_p->drv_stats_val.txq_ac_stats[i].drop_dfs, wlpd_p->drv_stats_val.txq_ac_stats[i].drop_iff,
...
static void wl_show_ba(struct net_device *netdev, char *sysfs_buff)
{
	UINT32 i;

	for (i = 0; i < 256; i++) {
		if (ba_debug_buf[i].lo_dword_addr != 0) {
			Sysfs_Printk("\t mac address of STA who sent the msdu   = %s\n", mac_display(ba_debug_buf[i].Addr));
			Sysfs_Printk("\t station id                    = %u\n", ba_debug_buf[i].StnId);
			Sysfs_Printk("\t BA reoder window starting seq number = %u\n", ba_debug_buf[i].winStartB);
 ...
 */
void wl_show_stat(struct net_device *netdev, int option, int level, char *sysfs_buff)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	...
	if (option == (int)drvstatsopt_geninfo) {
		...
		SMAC_STATUS_st smacStatus;
		memcpy(&smacStatus, wlpptr->wlpd_p->masterwlp->smacStatusAddr, sizeof(SMAC_STATUS_st));
...
int wl_show_stat_cmd(struct net_device *netdev, char *info_item, char *info_level, char *sysfs_buff)
{
...
void wlTxSkbTest_1(struct net_device *netdev, int pktcnt, int pktsize, int txqid, int frameType)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
...
//      skb_queue_head_init(&txQTrace);
	...
	for (i = 0; i < pktcnt; i++) {
		if (genRandflag) {
			...
			send_size = randnum;
			WLDBG_INFO(DBG_LEVEL_0, "Generate random send size:%u\n", send_size);
		} else
			send_size = (pktsize <= LONG_PKTSIZE) ? (pktsize) : LONG_PKTSIZE;
		...
		if (frameType == IEEE_TYPE_MANAGEMENT) {
			extern extStaDb_StaInfo_t *extStaDb_GetVapStaInfoStn(vmacApInfo_t * vmac_p, UINT8 stnid);
			extern struct sk_buff *mlmeApiPrepMgtMsg2(UINT32 Subtype, IEEEtypes_MacAddr_t * DestAddr, IEEEtypes_MacAddr_t * SrcAddr,
								  UINT16 size);
			...
			if (txqid >= 256) {
				pStaInfo = extStaDb_GetVapStaInfoStn(vmacSta_p, (txqid - 256) / 8);
				if (pStaInfo) {
					pDestAddr = pStaInfo->Addr;
					pSrcAddr = pStaInfo->Bssid;
				}
			}
			txSkb_p = mlmeApiPrepMgtMsg2(IEEE_MSG_ASSOCIATE_RQST, (IEEEtypes_MacAddr_t *) pDestAddr, &vmacSta_p->macStaAddr, send_size);
			if (txSkb_p) {
		...
		if (send_size > (ETHER_HDR_LEN + IP_HDR_LEN + ICMP_HDR_LEN)) {
			u32 idx;
			u32 cnt = send_size - (ETHER_HDR_LEN + IP_HDR_LEN + ICMP_HDR_LEN);
			u8 *p = (u8 *) (skb->data + (ETHER_HDR_LEN + IP_HDR_LEN + ICMP_HDR_LEN));
...
//              pskb = skb_dequeue(&txQTrace);
		...
		cfh_dl = wlSkbToCfhDl(netdev, pskb, &cfg, pStaInfo, qid, IEEE_TYPE_DATA);
		...
		// Dump rq, wrptr, rdptr ++
		{
			...
			regval = readl((wlpptr->ioBase1 + SC5_RQ_WRPTR_REG(qid)));
			WLDBG_INFO(DBG_LEVEL_4, "RQ, RQ_WRPTR_REG(%d): %p = %08xh\n", qid, (wlpptr->ioBase1 + SC5_RQ_WRPTR_REG(qid)), regval);
			regval = readl((wlpptr->ioBase1 + SC5_RQ_RDPTR_REG(qid)));
			WLDBG_INFO(DBG_LEVEL_4, "RQ, RQ_RDPTR_REG(%d): %p = %08xh\n", qid, (wlpptr->ioBase1 + SC5_RQ_RDPTR_REG(qid)), regval);
		}
		// Dump rq, wrptr, rdptr --
		if (cfh_dl != NULL) {
			WLDBG_INFO(DBG_LEVEL_4, "wrinx %d rdinx %d skb = %p ptx = %p cfh_dl %p phy_addr %x \n",
				   wlqm->rq.wrinx, wlqm->rq.rdinx, (void *)pskb, (void *)cfh_dl, cfh_dl->hdr.lo_dword_addr);
...
void idx_test(struct net_device *netdev, long pktcnt, long pkt_size, long txqid, long frameType)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	...
	WLDBG_INFO(DBG_LEVEL_0, "\t txInputCnt = %d\n", wlpptr->wlpd_p->masterwlp->smacStatusAddr->txInputCnt);
	WLDBG_INFO(DBG_LEVEL_0, "\t buf_ret_cnt = %d\n", wlpptr->wlpd_p->masterwlp->smacStatusAddr->txBufRetCnt);
...
{
	if ((g_rpkt_reuse_type == rpkt_reuse_no) || (skb_queue_len(pqueue) > max_recycle_cnt)) {
		wl_free_skb(skb);
		return;
	}
...
struct sk_buff *rpkt_resue_get(struct sk_buff_head *pqueue)
{
	...
	skb_queue_walk(pqueue, skb_tmp) {
		if (atomic_read(&skb_shinfo(skb_tmp)->dataref) == 1) {
			...
			skb = skb_tmp;
			WLDBG_DATA(DBG_LEVEL_3, "CFHUL: find skb %p skb->data %p \n", skb, skb->data);
			break;
		}
	}
	spin_unlock(&pqueue->lock);
...
void wlBmRxDropMgmtKeep(struct net_device *netdev, struct sk_buff *skb)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	struct sk_buff *free_skb = NULL;

	wlpd_p->RxDropMgmtCnt++;
	if (skb_queue_len(&wlpd_p->RxDropMgmtSkb) >= SYSFS_RX_DROP_MGMT_SKB_NUM) {
		free_skb = skb_dequeue(&wlpd_p->RxDropMgmtSkb);
		if (free_skb) {
			wl_free_skb(free_skb);
		}
	}
	skb_queue_tail(&wlpd_p->RxDropMgmtSkb, skb);
}
