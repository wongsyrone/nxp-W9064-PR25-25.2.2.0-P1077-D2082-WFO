...
struct clients_attribute {
	struct attribute attr;
	 ssize_t(*show) (struct kobject * kobj, struct clients_attribute * attr, char *buf);
	 ssize_t(*store) (struct kobject * kobj, struct clients_attribute * attr, char *buf);
};
...
static UINT8 macAddr_query[IEEE80211_ADDR_LEN] = { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff };
...
static ssize_t ap8x_proc_write(struct file *file, const char __user * buffer, size_t count, loff_t * data)
#else
int ap8x_proc_write(struct file *file, const char *buffer, unsigned long count, void *data)
#endif
{
	if (buffer && !strncmp(buffer, "dbg_level=", strlen("dbg_level="))) {
		dbg_level = atohex2(&buffer[strlen("dbg_level=")]);
	} else if (buffer && !strncmp(buffer, "dbg_class=", strlen("dbg_class="))) {
...
static void ap8x_stat_hex_dump(const char *title, UINT8 * sysfs_buff, UINT8 * src_buf, UINT32 len, UINT32 limit_len)
{
	int i;
	Sysfs_Printk("%s - hexdump(len=%lu):\n", title, (unsigned long)len);
	if (src_buf == NULL) {
		Sysfs_Printk(" [NULL]");
	} else {
		len = len > limit_len ? limit_len : len;
		for (i = 0; i < len; i++) {
			Sysfs_Printk(" %02x", src_buf[i]);
			if ((i + 1) % 16 == 0)
				Sysfs_Printk("\n");
		}
	}
...
#else
static int ap8x_proc_read(struct net_device *netdev, char *page, char **start, off_t off, int count, int *eof, void *data)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct net_device_stats *stat = &(wlpptr->netDevStats);
	char *p = page;
	...
	{
		{
			...
			p += sprintf(p, "rx_multicast_bytes...............%10u\n", (int)wlpptr->wlpd_p->privNdevStats.rx_mcast_bytes);
			p += sprintf(p, "rx_broadcast_bytes...............%10u\n", (int)wlpptr->wlpd_p->privNdevStats.rx_bcast_bytes)
			...
			p += sprintf(p, "tx_tcp_ack_drop_count............%10u\n", (int)wlpptr->wlpd_p->privStats.tx_tcp_ack_drop_count);
			p += sprintf(p, "tx_multicast_bytes...............%10u\n", (int)wlpptr->wlpd_p->privNdevStats.tx_mcast_bytes);
			p += sprintf(p, "tx_broadcast_bytes...............%10u\n", (int)wlpptr->wlpd_p->privNdevStats.tx_bcast_bytes);
...
case x: \
	...
	devstat[devstat_index].ap8x_proc =
	    proc_create_data(devstat[devstat_index].filename, 0666, ap8x, &ap8x_proc_file_ops, &devstat[devstat_index]);
#else
...
static ssize_t ap8xLnxStat_tp_help(char *b)
{
	int o = 0;		/* buffer offset */
	int s = PAGE_SIZE;	/* buffer size */

	o += scnprintf(b + o, s - o, "\n");
	o += scnprintf(b + o, s - o, "cat                                  tx_packets          - Print tx packets processed\n");
	o += scnprintf(b + o, s - o, "cat                                  tx_bytes            - Print tx bytes processed\n");
	o += scnprintf(b + o, s - o, "cat                                  tx_packets_rate     - Print tx packets rate\n");
	o += scnprintf(b + o, s - o, "cat                                  tx_bytes_rate       - Print tx bytes rate\n");
	o += scnprintf(b + o, s - o, "cat                                  rx_packets          - Print rx packets processed\n");
	o += scnprintf(b + o, s - o, "cat                                  rx_bytes            - Print rx bytes  processed\n");
	o += scnprintf(b + o, s - o, "cat                                  rx_packets_rate     - Print rx packets rate\n");
	o += scnprintf(b + o, s - o, "cat                                  rx_bytes_rate       - Print rx bytes rate\n");
	o += scnprintf(b + o, s - o, "cat                                  drop_point  - Print drop point info\n");
	o += scnprintf(b + o, s - o, "echo [s] [d] [d]                           > tp_set      - Print Q registers\n");
	o += scnprintf(b + o, s - o, "\n");
	o += scnprintf(b + o, s - o, "parameters:[s]if_name [d]tx drop point [d]tp_profile_mode");
	o += scnprintf(b + o, s - o, "\n");
	o += scnprintf(b + o, s - o, "cat                                  tp_profile_mode      - Print tp profile mode\n");
	o += scnprintf(b + o, s - o, "\n");
	return o;
}

void tp_profile_timer_func(unsigned long data)
{
	...
	if (wlpptr != NULL) {
		...
		/* tx */
		wlpd_p->wl_tpprofile.tx.bytes_rate = wlpd_p->wl_tpprofile.tx.bytes - wlpd_p->wl_tpprofile.tx.bytes_last;
		wlpd_p->wl_tpprofile.tx.bytes_last = wlpd_p->wl_tpprofile.tx.bytes;

		wlpd_p->wl_tpprofile.tx.packets_rate = wlpd_p->wl_tpprofile.tx.packets - wlpd_p->wl_tpprofile.tx.packets_last;
		wlpd_p->wl_tpprofile.tx.packets_last = wlpd_p->wl_tpprofile.tx.packets;

		/* rx */
		wlpd_p->wl_tpprofile.rx.bytes_rate = wlpd_p->wl_tpprofile.rx.bytes - wlpd_p->wl_tpprofile.rx.bytes_last;
		wlpd_p->wl_tpprofile.rx.bytes_last = wlpd_p->wl_tpprofile.rx.bytes;

		wlpd_p->wl_tpprofile.rx.packets_rate = wlpd_p->wl_tpprofile.rx.packets - wlpd_p->wl_tpprofile.rx.packets_last;
		wlpd_p->wl_tpprofile.rx.packets_last = wlpd_p->wl_tpprofile.rx.packets;
...
static void ap8xLnxStat_sysfs_version_info(struct wlprivate *priv, char *sysfs_buff)
{
	extern void wlget_sw_version(struct wlprivate *priv, char *sysfs_buff, int more);
	UINT8 *ver_buf;
...
static ssize_t ap8xLnxStat_tp_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t ap8xLnxStat_tp_config(struct device *dev, struct device_attribute *attr, const char *buf, size_t len)
{
	int a, b, c, d, e, err;
	...
	if (sscanf(buf, "%" STRINGIFY(IFNAMSIZ) "s" "%d %d %d %d %d", if_name, &a, &b, &c, &d, &e) <= 0) {
		err = 1;
		goto exit;
	}
	...
	if (wlpd_p->wl_tpprofile.mode == 1) {
		if (!timer_pending(&wlpd_p->tp_profile_timer)) {
			...
			wlpd_p->tp_profile_timer.function = tp_profile_timer_func;
			add_timer(&wlpd_p->tp_profile_timer);
		}
	} else {
...
static ssize_t ap8xLnxStat_stat_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	if (!strcmp(name, "geninfo_stat")) {
		...
		off = wl_show_stat_cmd(netdev, "drvrxinfo", stat_info_level, buf);
		...
		off = wl_show_stat_cmd(netdev, "dra_stat", stat_info_level, buf);
	} else if (!strcmp(name, "bareorder_stat")) {
		off = wl_show_stat_cmd(netdev, "bareorder", stat_info_level, buf);
	} else {
		pr_err("%s: illegal operation <%s>\n", __func__, attr->attr.name);
		return -EINVAL;
	}
...
static ssize_t ap8xLnxStat_stat_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	/* Read input parameters */
...
static ssize_t ap8xLnxStat_rxdropmgmt_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct kobject *parent_kobj = kobj->parent;
	struct device *dev = NULL;
	...
	wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	Sysfs_Printk("rx drop big memt frame count:%d, keep count:%d\n", wlpptr->wlpd_p->RxDropMgmtCnt,
		     skb_queue_len(&wlpptr->wlpd_p->RxDropMgmtSkb));
	drop_skb = skb_dequeue(&wlpptr->wlpd_p->RxDropMgmtSkb);
	if (drop_skb != NULL) {
		ap8x_stat_hex_dump("rx drop big mgmt", sysfs_buff, drop_skb->data, drop_skb->len, 1024);
		wl_free_skb(drop_skb);
	} else {
		ap8x_stat_hex_dump("rx drop big mgmt", sysfs_buff, NULL, 0, 0);
	}

	return strlen(buf);
}

static ssize_t ap8xLnxStat_rxdropmgmt_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	return count;
}

static ssize_t ap8xLnxStat_radiostatus_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct kobject *parent_kobj = kobj->parent;
	struct device *dev = NULL;
	struct net_device *netdev = NULL;
	...
	Sysfs_Printk("current_channel: %d\n", wlpptr->vmacSta_p->Mib802dot11->PhyDSSSTable->CurrChan);
	Sysfs_Printk("noise: %d\n", tmp_status.noise);
	Sysfs_Printk("noiseavg: %d\n", tmp_status.noiseavg);
	Sysfs_Printk("noisemax: %d\n", tmp_status.noisemax);
	Sysfs_Printk("load: %d\n", tmp_status.load);
	Sysfs_Printk("rxload: %d\n", tmp_status.rxload);
	Sysfs_Printk("cca_block: %d\n", tmp_status.cca_block);
	Sysfs_Printk("nav_block: %d\n", tmp_status.nav_block);

	return strlen(buf);
}

static ssize_t ap8xLnxStat_nlist_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct kobject *parent_kobj = kobj->parent;
	struct device *dev = NULL;
	struct net_device *netdev = NULL;
	...
	if (wlpptr->wlpd_p->nb_info.sysfs_query_nlist_idx >= 0) {
		/* read the specific nlist index beacon data and convert to ascii string */
		bcn_len = wlpptr->wlpd_p->nb_info.nb_list[wlpptr->wlpd_p->nb_info.sysfs_query_nlist_idx].bcn_buf->len;
		bcn_p = wlpptr->wlpd_p->nb_info.nb_list[wlpptr->wlpd_p->nb_info.sysfs_query_nlist_idx].bcn_buf->buf;
		if (bcn_p)
			wl_util_digi_to_hex_string(buf, bcn_p, bcn_len);
	}

	return strlen(buf);
}

static ssize_t ap8xLnxStat_nlist_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	struct kobject *parent_kobj = kobj->parent;
	struct device *dev = NULL;
	struct net_device *netdev;
	...
	sscanf(buf, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &destaddr[0], &destaddr[1], &destaddr[2], &destaddr[3], &destaddr[4], &destaddr[5]);

	for (i = 0; (i < wlpptr->wlpd_p->nb_info.nb_number) && (i < NB_LIST_MAX_NUM); i++) {
		if (!memcmp(destaddr, wlpptr->wlpd_p->nb_info.nb_list[i].bssid, IEEEtypes_ADDRESS_SIZE))
			break;
	}
	if ((i < wlpptr->wlpd_p->nb_info.nb_number) && (i < NB_LIST_MAX_NUM)) {
		/* If find the neighbor mac addr, keep nlist index */
		wlpptr->wlpd_p->nb_info.sysfs_query_nlist_idx = i;
	} else {
		wlpptr->wlpd_p->nb_info.sysfs_query_nlist_idx = -1;
		printk("Can't find %pM in neighbor list\n", destaddr);
	}
...
static SYSFS_ATTR(geninfo_stat, S_IRUSR | S_IWUSR, ap8xLnxStat_stat_show, ap8xLnxStat_stat_store);
static SYSFS_ATTR(warn_stat, S_IRUSR | S_IWUSR, ap8xLnxStat_stat_show, ap8xLnxStat_stat_store);
static SYSFS_ATTR(drvrxinfo_stat, S_IRUSR | S_IWUSR, ap8xLnxStat_stat_show, ap8xLnxStat_stat_store);
static SYSFS_ATTR(schinfo_stat, S_IRUSR | S_IWUSR, ap8xLnxStat_stat_show, ap8xLnxStat_stat_store);
static SYSFS_ATTR(tp_stat, S_IRUSR | S_IWUSR, ap8xLnxStat_stat_show, ap8xLnxStat_stat_store);
static SYSFS_ATTR(mac_stat, S_IRUSR | S_IWUSR, ap8xLnxStat_stat_show, ap8xLnxStat_stat_store);
static SYSFS_ATTR(hframe_stat, S_IRUSR | S_IWUSR, ap8xLnxStat_stat_show, ap8xLnxStat_stat_store);
static SYSFS_ATTR(pktcnt_stat, S_IRUSR | S_IWUSR, ap8xLnxStat_stat_show, ap8xLnxStat_stat_store);
static SYSFS_ATTR(dra_stat, S_IRUSR | S_IWUSR, ap8xLnxStat_stat_show, ap8xLnxStat_stat_store);
static SYSFS_ATTR(rxdropmgmt, S_IRUSR | S_IWUSR, ap8xLnxStat_rxdropmgmt_show, ap8xLnxStat_rxdropmgmt_store);
static SYSFS_ATTR(radiostatus, S_IRUSR, ap8xLnxStat_radiostatus_show, NULL);
static SYSFS_ATTR(nlist, S_IRUSR | S_IWUSR, ap8xLnxStat_nlist_show, ap8xLnxStat_nlist_store);
...
static struct attribute *ap8xLnxStat_stat_attrs[] = {
	...
	&dev_attr_rxdropmgmt.attr,
	&dev_attr_radiostatus.attr,
	&dev_attr_nlist.attr,
	NULL
};
...
static SYSFS_ATTR(bareorder_stat, S_IRUSR | S_IWUSR, ap8xLnxStat_stat_show, ap8xLnxStat_stat_store);
...
static ssize_t ap8xLnxStat_hw_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct kobject *parent_kobj = kobj->parent;
	struct device *dev = NULL;
	struct net_device *netdev;
	...
	if (IS_BUS_TYPE_MCI(wlpptr)) {
		res[0] = platform_get_resource(wlpptr->wlpd_p->pDev, IORESOURCE_MEM, 0);
		res[1] = platform_get_resource(wlpptr->wlpd_p->pDev, IORESOURCE_MEM, 1);
		res[0]->end += 0x100000;
	...
	val = cpu_to_le32(*(volatile unsigned int *)(wlpptr->ioBase1 + SC5_REG_SMAC_CTRLBASE));
	Sysfs_Printk("smac ctrlBase(0x%x)=%xh\n", SC5_REG_SMAC_CTRLBASE, val);
	Sysfs_Printk("Device:%s \nRevision:%s\n", _DeviceID_to_string(wlpptr->devid), _ChipRevision_to_string(wlpptr->hwData.chipRevision));
	...
	for (val = 0; val < wlpptr->wlpd_p->coredump.num_regions; val++) {
		Sysfs_Printk("region[%2d].address = 0x%10x, region[%2d].length = 0x%10x\n", val,
			     wlpptr->wlpd_p->coredump.region[val].address, val, wlpptr->wlpd_p->coredump.region[val].length);
	}
	...
	Sysfs_Printk("%s rsvd mem: pbuf=%llx vbuf=%p size=%llx\n",
		     wlpptr->wlpd_p->ext_membuf[0].extsym_name, wlpptr->wlpd_p->ext_membuf[0].pbuf_pool,
		     wlpptr->wlpd_p->ext_membuf[0].vbuf_pool, (u64) wlpptr->wlpd_p->ext_membuf[0].buf_pool_size);
	Sysfs_Printk("%s rsvd mem: pbuf=%llx vbuf=%p size=%llx\n",
		     wlpptr->wlpd_p->ext_membuf[1].extsym_name, wlpptr->wlpd_p->ext_membuf[1].pbuf_pool,
		     wlpptr->wlpd_p->ext_membuf[1].vbuf_pool, (u64) wlpptr->wlpd_p->ext_membuf[1].buf_pool_size);

	/* convert channel bw format as htbw cmd */
	switch (wlpptr->vmacSta_p->ShadowMib802dot11->PhyDSSSTable->Chanflag.ChnlWidth) {
	case CH_AUTO_WIDTH:
		bw = 0;
		break;
	...
	Sysfs_Printk("BW: %u\n", bw);
	Sysfs_Printk("smdata_offset: %u\n", wlpptr->wlpd_p->smdata_offset);

	return strlen(buf);
}

static ssize_t ap8xLnxStat_hw_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	return count;
}

static SYSFS_ATTR(hw_info, S_IRUSR | S_IWUSR, ap8xLnxStat_hw_show, ap8xLnxStat_hw_store);
...
static ssize_t ap8xLnxStat_mem_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct kobject *parent_kobj = kobj->parent;
	struct device *dev = NULL;
	struct net_device *netdev;
	...
	if (mem_dbg_level == 0) {
		...
		for (i = 0; i < WL_MEM_TRACE_FUNC_NUM; i++) {
			if (MemTraceFunc[i].type == MEM_SKB || MemTraceFunc[i].type == MEM_KMALLOC) {
				if (!strcmp(MemTraceFunc[i].func, MEM_DBG_WLRXBUFINIT) ||
				    !strcmp(MemTraceFunc[i].func, MEM_DBG_WLRXBUFFILL) || !strcmp(MemTraceFunc[i].func, MEM_DBG_DBGSKB_INIT)) {
					/* rx buffer */
					rx_buf_size += MemTraceFunc[i].size;
				} else if (!strcmp(MemTraceFunc[i].func, MEM_DBG_EXTSTADB_INIT) ||
					   !strcmp(MemTraceFunc[i].func, MEM_DBG_ETHSTADB_INIT)) {
					/* data base */
					stadb_size += MemTraceFunc[i].size;
				} else if (!strcmp(MemTraceFunc[i].func, MEM_DBG_WLMON_KTHREAD)) {
					/* Health Monitor */
		...
		Sysfs_Printk("======== Total Size :%6d KB ========\n",
			     (t_vz_size + rx_buf_size + stadb_size + hmon_size + sys_size + t_dma_size) / 1024);
		free_size = wl_get_meminfo_stat();
		Sysfs_Printk("Actual usage:%6d KB (/proc/meminfo MemFree:%d KB - %d KB)\n", (WL_memfree - free_size), WL_memfree, free_size);
	} else {
		Sysfs_Printk("\n======== [Memory skb info] ========\n");
		t_skb_size = 0;
		for (i = 0; i < WL_MEM_TRACE_FUNC_NUM; i++) {
			if (MemTraceFunc[i].size != 0 && MemTraceFunc[i].type == MEM_SKB) {
		...
		t_vz_size = 0;
		for (i = 0; i < WL_MEM_TRACE_FUNC_NUM; i++) {
			if (MemTraceFunc[i].size != 0 && MemTraceFunc[i].type == MEM_VZALLOC) {
				Sysfs_Printk("%s \tline:%d \tsize:%d\n", MemTraceFunc[i].func, MemTraceFunc[i].line, MemTraceFunc[i].size);
				t_vz_size += MemTraceFunc[i].size;
			}
		}
		...
		t_km_size = 0;
		for (i = 0; i < WL_MEM_TRACE_FUNC_NUM; i++) {
			if (MemTraceFunc[i].size != 0 && MemTraceFunc[i].type == MEM_KMALLOC) {
				Sysfs_Printk("%s \tline:%d \tsize:%d\n", MemTraceFunc[i].func, MemTraceFunc[i].line, MemTraceFunc[i].size);
		...
		t_dma_size = 0;
		for (i = 0; i < WL_MEM_TRACE_FUNC_NUM; i++) {
			if (MemTraceFunc[i].size != 0 && MemTraceFunc[i].type == MEM_DMAALLOC) {
				Sysfs_Printk("%s \tline:%d \tsize:%d\n", MemTraceFunc[i].func, MemTraceFunc[i].line, MemTraceFunc[i].size);
				t_dma_size += MemTraceFunc[i].size;
			}
		}
		...
		Sysfs_Printk("\n======== [Total mem info] ========\n");
		Sysfs_Printk("total alloc size:%d KBytes\n", (t_skb_size + t_vz_size + t_km_size + t_dma_size) / 1024);
		free_size = wl_get_meminfo_stat();
		Sysfs_Printk("Actual usage:%6d KB (/proc/meminfo MemFree:%d KB - %d KB)\n", (WL_memfree - free_size), WL_memfree, free_size);
	}
	return strlen(buf);
...
static ssize_t ap8xLnxStat_mem_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	mem_dbg_level = simple_strtol(buf, NULL, 10);
	return count;
}

static SYSFS_ATTR(mem_info, S_IRUSR | S_IWUSR, ap8xLnxStat_mem_show, ap8xLnxStat_mem_store);
...
static ssize_t ap8xLnxStat_acs_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct kobject *parent_kobj = kobj->parent;
	struct device *dev = NULL;
	struct net_device *netdev;
	...
	if (!vmacSta_p->InfUpFlag) {
		...
		Sysfs_Printk("Please enable by \"iwpriv [phy_ap] autochannel 1\"\n");
	} else if ((!vmacSta_p->preautochannelfinished)) {
		/* Still scan */
		Sysfs_Printk("ACS(Auto Channel Select) still in scan, wait a few seconds\n");
	} else {
		Sysfs_Printk("\n======== [Auto Channel Select] ========\n");
		Sysfs_Printk("channel \tBSS \tminrss \tmaxrss \tNF \tCh load  Score \tCal Score \tUse 2nd-CH\n");
		for (i = 0; i < (IEEEtypes_MAX_CHANNELS + IEEEtypes_MAX_CHANNELS_A); i++) {
			if (vmacSta_p->acs_db[i].channel != 0) {
				Sysfs_Printk("%3d \t\t%3d \t%3d \t%3d \t%3d \t%3d \t%6d \t%6d \t\t%c\n",
					     ...
					     vmacSta_p->acs_db[i].score, vmacSta_p->autochannel[i], vmacSta_p->acs_db[i].is_2nd_ch ? 'Y' : 'N');
			}
		}
	}
...
static ssize_t ap8xLnxStat_acs_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	return count;
}

static ssize_t ap8xLnxStat_dcs_dfschlist(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct kobject *parent_kobj = kobj->parent;
	struct device *dev = NULL;
	struct net_device *netdev;
	...
	for (i = 0; i < DFS_MAX_CHANNELS; i++) {
		...
		Sysfs_Printk(" %d", dfsEnabledChannels[domainInd].dfschannelEntry[i]);
	}
	Sysfs_Printk("\n");
	return strlen(buf);
}

static ssize_t ap8xLnxStat_dcs_scanlist(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct kobject *parent_kobj = kobj->parent;
	struct device *dev = NULL;
	struct net_device *netdev;
	...
	if (vmacSta_p->ChannelList[0] == 0) {
		memset(scanChannel, 0, IEEEtypes_MAX_CHANNELS + IEEEtypes_MAX_CHANNELS_A);
		...
		for (i = 0; i < IEEEtypes_MAX_CHANNELS_A; i++) {
			if ((mib->PhyDSSSTable->Chanflag.ChnlWidth != CH_20_MHz_WIDTH)) {
				if (*(mib->mib_regionCode) == DOMAIN_CODE_ALL) {
					if (scanChannel[i + IEEEtypes_MAX_CHANNELS] == 181) {
						continue;
					}
				} else {
					if (scanChannel[i + IEEEtypes_MAX_CHANNELS] >= 165) {
						continue;
					}
				}
			}
			if (scanChannel[i + IEEEtypes_MAX_CHANNELS] != 0) {
				Sysfs_Printk(" %d", scanChannel[i + IEEEtypes_MAX_CHANNELS]);
			}
		}
	} else {		//2.4G
		for (i = 0; i < IEEEtypes_MAX_CHANNELS; i++) {
			if (scanChannel[i] != 0) {
				Sysfs_Printk(" %d", scanChannel[i]);
			}
		}
	}
	Sysfs_Printk("\n");
...
static ssize_t ap8xLnxStat_dcs_setcmd_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	struct kobject *parent_kobj = kobj->parent;
	struct device *dev = NULL;
	struct net_device *netdev;
	struct wlprivate *wlpptr;
	vmacApInfo_t *vmacSta_p = NULL;
	char (*param)[14] = wl_kzalloc(sizeof(*param) * MAX_GROUP_PER_CHANNEL, GFP_KERNEL);
	...
	sscanf(buf, "%64s %64s %64s %64s %64s %64s %64s %64s %64s %64s %64s %64s %64s %64s\n",
	       param[0], param[1], param[2], param[3], param[4], param[5], param[6],
	       param[7], param[8], param[9], param[10], param[11], param[12], param[13]);

	if (strcmp(param[0], "newdpoffch_nf") == 0) {
		DOT11_OFFCHAN_REQ_t offchan;
		memset((UINT8 *) & offchan, 0x0, sizeof(DOT11_OFFCHAN_REQ_t));
		offchan.channel = simple_strtol(param[1], NULL, 10);
		offchan.id = OFFCHAN_GET_ID_FROM_FEATURE(OFFCHAN_BY_DCS, simple_strtol(param[2], NULL, 10));
		offchan.dwell_time = simple_strtol(param[3], NULL, 10);
		offchan.req_type = OFFCHAN_TYPE_RX_NF;	/* NF-reading feedback */
		wlFwNewDP_queue_OffChan_req(netdev, &offchan);
	} else if (strcmp(param[0], "11hcsastart") == 0) {
		MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
		MIB_PHY_DSSS_TABLE *PhyDSSSTable = mib->PhyDSSSTable;
		UINT8 csacount, csachan;
		Dfs_ChanSwitchReq_t chanSwitch;

		csacount = simple_strtol(param[1], NULL, 10);
		csachan = simple_strtol(param[2], NULL, 10);
		if (csacount < 0 || csacount > 255) {
			goto setcmd_ret;
		}
		if (!domainChannelValid(csachan, PhyDSSSTable->Chanflag.FreqBand)) {
		...
		chanSwitch.ChannelSwitchCmd.Mode = 1;
		chanSwitch.ChannelSwitchCmd.ChannelNumber = csachan;
		chanSwitch.ChannelSwitchCmd.ChannelSwitchCount = csacount;
		chanSwitch.chInfo.channel = csachan;
		macMgmtMlme_SendChannelSwitchCmd(netdev, &chanSwitch);
	} else if (strcmp(param[0], "wlmgr_dcs") == 0) {
		wlpptr->wlpd_p->wlmgr_dcs = (simple_strtol(param[1], NULL, 10) == 0) ? FALSE : TRUE;
	} else {
		printk("Unknown parameter: %64s\n", param[0]);
	}
 setcmd_ret:
	wl_kfree(param);
	return count;
...
static void ap8xLnxStat_prepare_dcs_db(struct net_device *netdev, UINT8 ch_list_num)
{
	SINT32 i, j;
	SINT32 curr_time = 0, offset_time;
	s32 cal_nlist_rssi = 0;
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	dcs_data_t *dcs_db = wlpptr->wlpd_p->dcs_db;
	...
	for (j = 0; j < ch_list_num; j++) {
		...
		dcs_db[j].min_rssi = 0;
		dcs_db[j].max_rssi = 0;
		dcs_db[j].is_2nd_ch = 0;
		dcs_db[j].bw = 0;
		dcs_db[j].bw_2g_40_above = 0;
		dcs_db[j].rssi_ls = 0;
		dcs_db[j].raw_min_rssi = 0;
		dcs_db[j].raw_max_rssi = 0;
	}

	/* Update all DCS database */
	for (i = 0; i < wlpptr->wlpd_p->nb_info.nb_number; i++) {
		neighbor_list_entrie_t *nlist = &wlpptr->wlpd_p->nb_info.nb_list[i];
		cal_nlist_rssi = 0;
		...
		for (j = 0; j < ch_list_num; j++) {
			if (nlist->chan == dcs_db[j].channel) {
				dcs_db[j].bss_num++;
				cal_nlist_rssi = (nlist->rssi < -100) ? -(rssi_threshold) : nlist->rssi;
				if (dcs_db[j].min_rssi == 0) {
					dcs_db[j].min_rssi = nlist->rssi;
					dcs_db[j].raw_min_rssi = cal_nlist_rssi;
				}
				dcs_db[j].raw_min_rssi = dcs_db[j].raw_min_rssi < cal_nlist_rssi ? dcs_db[j].raw_min_rssi : cal_nlist_rssi;
				dcs_db[j].min_rssi = dcs_db[j].min_rssi < nlist->rssi ? dcs_db[j].min_rssi : nlist->rssi;

				cal_nlist_rssi = (nlist->rssi > 100) ? -35 : nlist->rssi;
				if (dcs_db[j].max_rssi == 0) {
					dcs_db[j].max_rssi = nlist->rssi;
					dcs_db[j].raw_max_rssi = cal_nlist_rssi;
				}
				dcs_db[j].raw_max_rssi = dcs_db[j].raw_max_rssi > cal_nlist_rssi ? dcs_db[j].raw_max_rssi : cal_nlist_rssi;
				dcs_db[j].max_rssi = dcs_db[j].max_rssi > nlist->rssi ? dcs_db[j].max_rssi : nlist->rssi;

				dcs_db[j].rssi_ls += (rssi_threshold - abs(cal_nlist_rssi));
				if (dcs_db[j].rssi_ls > rssi_threshold)
					dcs_db[j].rssi_ls = rssi_threshold;

				/* set bw */
				dcs_db[j].bw = dcs_db[j].bw > nlist->width ? dcs_db[j].bw : nlist->width;
				dcs_db[j].bw_2g_40_above |= nlist->bw_2g_40_above;
				printk("[%d], dcs_db.channel = %d, dcs_db.bss_num = %d, dcs_db.min_rssi = %d, dcs_db.max_rssi = %d, nlist.rssi=%d\n",
				       j, dcs_db[j].channel, dcs_db[j].bss_num, dcs_db[j].raw_min_rssi, dcs_db[j].raw_max_rssi, nlist->rssi);
				break;
			}
		}
	}
	...
	for (j = 0; j < ch_list_num; j++) {
		if (dcs_db[j].bss_num > 0) {
			dcs_db[j].min_rssi = dcs_db[j].min_rssi - dcs_db[j].noise_floor;
			dcs_db[j].min_rssi = dcs_db[j].min_rssi >= 100 ? 100 : dcs_db[j].min_rssi;
			dcs_db[j].min_rssi = dcs_db[j].min_rssi <= 0 ? 1 : dcs_db[j].min_rssi;
			dcs_db[j].max_rssi = dcs_db[j].max_rssi - dcs_db[j].noise_floor;
			dcs_db[j].max_rssi = dcs_db[j].max_rssi >= 100 ? 100 : dcs_db[j].max_rssi;
			dcs_db[j].max_rssi = dcs_db[j].max_rssi <= 0 ? 1 : dcs_db[j].max_rssi;
		}
	}
#endif
}

static ssize_t ap8xLnxStat_dcs_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct kobject *parent_kobj = kobj->parent;
	struct device *dev = NULL;
	struct net_device *netdev;
	...
	wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	dcs_db = wlpptr->wlpd_p->dcs_db;

	ap8xLnxStat_prepare_dcs_db(netdev, IEEE_80211_MAX_NUMBER_OF_CHANNELS);
	Sysfs_Printk("channel \tBSS \tminrss \tmaxrss \tNF \tCh load  Score\n");
	for (i = 0; i < IEEE_80211_MAX_NUMBER_OF_CHANNELS; i++) {
		if (dcs_db[i].channel != 0) {
			Sysfs_Printk("%3d \t\t%3d \t%3d \t%3d \t%3d \t%3d \t%6d\n",
				     dcs_db[i].channel,
				     dcs_db[i].bss_num,
				     dcs_db[i].min_rssi, dcs_db[i].max_rssi, dcs_db[i].noise_floor, dcs_db[i].ch_load, dcs_db[i].score);
		}
	}
	return strlen(buf);
}

static ssize_t ap8xLnxStat_dcs_command(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	return count;
}

static ssize_t ap8xLnxStat_dcs_nf_read(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct kobject *parent_kobj = kobj->parent;
	struct device *dev = NULL;
	struct net_device *netdev;
	...
	for (ch_idx = 0; ch_idx < IEEE_80211_MAX_NUMBER_OF_CHANNELS; ch_idx++) {
		if (wlpptr->wlpd_p->dcs_db[ch_idx].channel) {
			u32 *nf_dbm_p = wlpptr->wlpd_p->dcs_db[ch_idx].nf_bin;
			Sysfs_Printk("%3d: %4d %4d %4d"
				     " %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d : %4d %4d %4d\n",
				     wlpptr->wlpd_p->dcs_db[ch_idx].channel,
				     nf_dbm_p[0], nf_dbm_p[1], nf_dbm_p[2], nf_dbm_p[3], nf_dbm_p[4],
				     nf_dbm_p[5], nf_dbm_p[6], nf_dbm_p[7], nf_dbm_p[8], nf_dbm_p[9],
				     nf_dbm_p[10], nf_dbm_p[11], nf_dbm_p[12], nf_dbm_p[13], nf_dbm_p[14],
				     nf_dbm_p[15], nf_dbm_p[16], nf_dbm_p[17],
				     wlpptr->wlpd_p->dcs_db[ch_idx].score, wlpptr->wlpd_p->dcs_db[ch_idx].ch_load,
				     wlpptr->wlpd_p->dcs_db[ch_idx].noise_floor);
		}
	}
	return strlen(buf);
}

static ssize_t ap8xLnxStat_dcs_nf_write(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	struct kobject *parent_kobj = kobj->parent;
	struct device *dev = NULL;
	struct net_device *netdev;
	...
	memset(wlpptr->wlpd_p->dcs_db, 0, sizeof(wlpptr->wlpd_p->dcs_db));

	return count;
}

static SYSFS_ATTR(acs_info, S_IRUSR | S_IWUSR, ap8xLnxStat_acs_show, ap8xLnxStat_acs_store);
static SYSFS_ATTR(dcs_info, S_IRUSR | S_IWUSR, ap8xLnxStat_dcs_show, ap8xLnxStat_dcs_command);
static SYSFS_ATTR(dcs_nf, S_IRUSR | S_IWUSR, ap8xLnxStat_dcs_nf_read, ap8xLnxStat_dcs_nf_write);
static SYSFS_ATTR(scanlist, S_IRUSR, ap8xLnxStat_dcs_scanlist, NULL);
static SYSFS_ATTR(setcmd, S_IRUSR | S_IWUSR, NULL, ap8xLnxStat_dcs_setcmd_store);
static SYSFS_ATTR(dfs_chlist, S_IRUSR, ap8xLnxStat_dcs_dfschlist, NULL);

static struct attribute *ap8xLnxStat_acs_attrs[] = {
	&dev_attr_acs_info.attr,
	&dev_attr_dcs_info.attr,
	&dev_attr_dcs_nf.attr,
	&dev_attr_scanlist.attr,
	&dev_attr_setcmd.attr,
	&dev_attr_dfs_chlist.attr,
	NULL
};
...
static ssize_t ap8xLnxStat_mmdu_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct kobject *parent_kobj = kobj->parent;
	struct device *dev = NULL;
	struct net_device *netdev;
...
static UINT8 *_AmpduTx_to_string(UINT8 mode, UINT8 * str)
{
	switch (mode) {
	case 0:
		sprintf(str, "Disable");
...
static UINT8 *_MIMOPwSave_to_string(UINT8 mode)
{
	switch (mode) {
	case 0:
		return "POWER_STATIC";
...
static UINT8 *_CSMode_to_string(UINT8 mode)
{
	switch (mode) {
	case 0:
		return "CONSERV";
...
static UINT8 *_RateAdaptMode_to_string(UINT8 mode)
{
	switch (mode) {
	case 0:
		return "Indoor";
...
static UINT8 *_filtertype_to_string(UINT8 type)
{
	switch (type) {
	case DISABLE_MODE:
		return "DISABLE_MODE";
...
static UINT8 *_wpawpa2mode_to_string(UINT8 mode)
{
	switch (mode) {
	case 0:
...
static UINT8 *_opmode_to_string(UINT8 opmode)
{
	switch (opmode) {
	case AP_MODE_B_ONLY:
...
static ssize_t VAP_info_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct device *dev = NULL;
	struct net_device *netdev;
	...
	Sysfs_Printk("wpawpa2mode:     %s (%d)\n", _wpawpa2mode_to_string(*(mib->mib_wpaWpa2Mode)), *(mib->mib_wpaWpa2Mode));
	Sysfs_Printk("grouprekey time: %d\n", mib->RSNConfig->GroupRekeyTime);
	Sysfs_Printk("filter:          %s (%d)\n", _filtertype_to_string(*(mib->mib_wlanfiltertype)), *(mib->mib_wlanfiltertype));
	...
	Sysfs_Printk("ratemode:        %s (%d)\n", _RateAdaptMode_to_string(*(mib->mib_RateAdaptMode)), *(mib->mib_RateAdaptMode));
	Sysfs_Printk("csmode:          %s (%d)\n", _CSMode_to_string(*(mib->mib_CSMode)), *(mib->mib_CSMode));
	Sysfs_Printk("wdsmode:         %s (%d)\n", *(mib->mib_wdsEnable) == 1 ? "on" : "off", *(mib->mib_wdsEnable));
	Sysfs_Printk("strictshared:    %s (%d)\n", *(mib->mib_strictWepShareKey) == 1 ? "share" : "both", *(mib->mib_strictWepShareKey));
	Sysfs_Printk("disableassoc:    %s (%d)\n", *(mib->mib_disableAssoc) == 1 ? "TRUE" : "FALSE", *(mib->mib_disableAssoc));
	Sysfs_Printk("mimops mode:     %s (%d)\n", _MIMOPwSave_to_string(*(mib->mib_psHtManagementAct)), *(mib->mib_psHtManagementAct));
	Sysfs_Printk("ampdutx:         %s (%d)\n", _AmpduTx_to_string(*(mib->mib_AmpduTx), mode_str), *(mib->mib_AmpduTx));
	Sysfs_Printk("txqlimit:        %d bytes\n", vmacSta_p->txQLimit);
	Sysfs_Printk("rxintlimit:      %d bytes\n", vmacSta_p->work_to_do);
	Sysfs_Printk("mcast proxy:     %s (%d)\n", *(mib->mib_MCastPrxy) == 1 ? "on" : "off", *(mib->mib_MCastPrxy));
...
#ifdef SYSFS_STADB_INFO
static void ap8xLnxStat_clients_group_free(struct clients_attribute_group *free_group)
{
	int i = 0;
...
static struct clients_kobject *ap8xLnxStat_clients_kobj_found(const char *name, struct clients_kobject_list *list)
{
	...
	for (clients_kobj = list->head, i = 0; (clients_kobj != NULL) && (i < list->num); clients_kobj = clients_kobj->next, i++) {
		...
		if (!memcmp(clients_kobj->kobj->parent->name, name, MAX(strlen(clients_kobj->kobj->parent->name), strlen(name)))) {
			return clients_kobj;
		}
	}
...
static int ap8xLnxStat_clients_kobj_add(struct clients_kobject *kobj, struct clients_kobject_list *list)
{
...
static void ap8xLnxStat_clients_kobj_list_free(struct clients_kobject_list *list)
{
	...
	for (kobj = list->head, i = 0; (kobj != NULL) && (i < list->num); i++) {
		for (attr_group = kobj->group_head, j = 0; (attr_group != NULL) && (j < kobj->conn_cnt); j++) {
...
static struct clients_attribute_group *ap8xLnxStat_clients_group_found(const char *name, struct clients_kobject *kobj)
{
	struct clients_attribute_group *attr_group = NULL;
	int i;

	if ((kobj == NULL) || ((kobj->group_head == NULL) && (kobj->conn_cnt == 0)))
		return NULL;

	for (attr_group = kobj->group_head, i = 0; (attr_group != NULL) && (i < kobj->conn_cnt); attr_group = attr_group->next, i++) {
		if (!memcmp(attr_group->name, name, MAX(strlen(attr_group->name), strlen(name))))
			return attr_group;
	}
...
static int ap8xLnxStat_clients_group_add(struct clients_attribute_group *attr_group, struct clients_kobject *kobj)
{
...
static void ap8xLnxStat_clients_group_del(struct clients_attribute_group *attr_group, struct clients_kobject *kobj)
{
	...
	for (curr_group = kobj->group_head, i = 0; (curr_group != NULL) && (i < kobj->conn_cnt); curr_group = curr_group->next, i++) {
...
void *ap8xLnxStat_vap_init(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	...
	if (!vmacSta_p->vap_info_kobj) {
		vmacSta_p->vap_info_kobj = kobject_create_and_add("vap_info", dev_kobj);
		if (!vmacSta_p->vap_info_kobj) {
	...
	if (!vmacSta_p->vap_stat_kobj) {
		vmacSta_p->vap_stat_kobj = kobject_create_and_add("stat", dev_kobj);
		if (!vmacSta_p->vap_stat_kobj) {
			pr_err("%s: cannot create stat kobject\n", __func__);
			return NULL;
		}
	}
	err = sysfs_create_group(vmacSta_p->vap_stat_kobj, &ap8xLnxStat_vap_stat_group);
	if (err) {
...
int ap8xLnxStat_vap_exit(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	...
	if (vmacSta_p->vap_info_kobj) {
		...
		kobject_put(vmacSta_p->vap_info_kobj);
		vmacSta_p->vap_info_kobj = NULL;
	}
	if (vmacSta_p->vap_stat_kobj) {
		sysfs_remove_group(vmacSta_p->vap_stat_kobj, &ap8xLnxStat_vap_stat_group);
		kobject_del(vmacSta_p->vap_stat_kobj);
		kobject_put(vmacSta_p->vap_stat_kobj);
		vmacSta_p->vap_stat_kobj = NULL;
	}
	return 0;
}

#ifdef SYSFS_STADB_INFO
static ssize_t ap8xLnxStat_clients_ap_info(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	ssize_t size = 0;
	char *pos = buf;
...
static CLIENTS_ATTR(ap_info, S_IWUSR | S_IRUSR, ap8xLnxStat_clients_ap_info, NULL);
...
void *ap8xLnxStat_clients_init(struct net_device *netdev, UINT8 mode)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	struct kobject *dev_kobj = &netdev->dev.kobj;
	...
	if (mode == MODE_STA) {
		vmacEntry_t *vmacEntry_p = sme_GetParentVMacEntry(vmacSta_p->VMacEntry.phyHwMacIndx);
		...
		vStaInfo_p->sta_conn_info_kobj = kobject_create_and_add("connection_info", dev_kobj);
		if (!vStaInfo_p->sta_conn_info_kobj) {
			pr_err("%s: cannot create connection_info kobject\n", __func__);
			return NULL;
		}

		err = sysfs_create_group(vStaInfo_p->sta_conn_info_kobj, &ap8xLnxStat_ap_info_group);
		if (err) {
	...
	kobj_list = (mode == MODE_VAP) ? &ap8xLnxStat_clients_vap_kobjs : &ap8xLnxStat_clients_kobjs;
	if (kobj_list == NULL)
		return NULL;

	clients_kobj = ap8xLnxStat_clients_kobj_found((const char *)kobject_name(dev_kobj), kobj_list);
	if (clients_kobj)
		return NULL;
...
void ap8xLnxStat_clients_deinit(struct net_device *netdev, UINT8 mode)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	vmacEntry_t *vmacEntry_p = sme_GetParentVMacEntry(vmacSta_p->VMacEntry.phyHwMacIndx);
	...
	if (mode == MODE_STA) {
		...
		if (vStaInfo_p->sta_conn_info_kobj) {
			sysfs_remove_group(vStaInfo_p->sta_conn_info_kobj, &ap8xLnxStat_ap_info_group);
			kobject_del(vStaInfo_p->sta_conn_info_kobj);
			kobject_put(vStaInfo_p->sta_conn_info_kobj);
		}
	}
}
...
int ap8xLnxStat_sysfs_init(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	struct kobject *dev_kobj = &netdev->dev.kobj;
...
int ap8xLnxStat_sysfs_exit(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
...
char *ap8xLnxStat_get_client_mode_str(extStaDb_StaInfo_t * pStaInfo, MIB_802DOT11 * mib)
{
	...
	case NONLY_MODE:
	case MIXED_MODE:
		if (pStaInfo->ClientMode_mix_ext == 0) {
			if ((*(mib->mib_ApMode) | AP_MODE_A_ONLY) != 0) {
				return "a ";
			} else {
				return "bg ";
			}
		} else {
			switch (pStaInfo->ClientMode_mix_ext) {
			case NONLY_MODE:
				return "n ";
			case AC_1SS_MODE:
			case AC_2SS_MODE:
...
#ifdef SYSFS_STADB_INFO
static ssize_t ap8xLnxStat_clients_help(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	char *pos = buf;
	pos += sprintf(pos, "\nGet the informations of the client specified by MAC Address.\n");
	pos += sprintf(pos, "######################################################################\n");
	pos += sprintf(pos, "-Use \"query\" to get the client information by its MAC Address.\n");
	pos += sprintf(pos, "#echo \"<MAC Address> [<query information>]\" > query\n");
	pos += sprintf(pos, "  <MAC Address>\n");
	pos += sprintf(pos, "    the string of client MAC addrss, the format is \"xx:xx:xx:xx:xx:xx\"\n");
	pos += sprintf(pos, "  <query information>\n");
	pos += sprintf(pos, "    the string of what information of client wants to get\n");
	pos += sprintf(pos, "    \"summary\"    - get the summary of client with <MAC Address>\n");
	pos += sprintf(pos, "    \"ratetable\"  - get the ratetable of the client with <MAC Address>\n");
	pos += sprintf(pos, "    <query information> is optional, default is to get the summary\n");
	pos += sprintf(pos, "#cat query         - get the specific information of the selected client\n");
	pos += sprintf(pos, "######################################################################\n");
	pos += sprintf(pos, "-Enter the clinet folder to get client information by its MAC Address.\n");
	pos += sprintf(pos, "#cd <MAC Address>\n");
	pos += sprintf(pos, "#cat summary       - get the summary of client with <MAC Address>\n");
	pos += sprintf(pos, "#cat ratetable     - get the ratetable of the client with <MAC Address>\n");
	pos += sprintf(pos, "######################################################################\n\n");

	return (ssize_t) (pos - buf);
}

static ssize_t ap8xLnxStat_clients_summary(struct net_device *netdev, UINT8 * macAddr, char *buf)
{
	ssize_t size = 0;
	char *pos = buf;
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	...
	if ((StaInfo_p = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) macAddr, STADB_DONT_UPDATE_AGINGTIME)) == NULL) {
		return size;
	}
	strcpy(mode, ap8xLnxStat_get_client_mode_str(StaInfo_p, mib));
			...
			if ((mib->Privacy->RSNEnabled == 1) || (mib->RSNConfigWPA2->WPA2Enabled == 1)) {
				if (*(mib->mib_wpaWpa2Mode) < 4) {	/* For PSK modes use internal WPA state machine */
					if (StaInfo_p->keyMgmtHskHsm.super.pCurrent != NULL) {
						if (StaInfo_p->keyMgmtHskHsm.super.pCurrent == &StaInfo_p->keyMgmtHskHsm.hsk_end) {
							strcpy(state, "PSK PASSED");
							flagPsk = 1;
						}
					}
				} else if (StaInfo_p->keyMgmtStateInfo.RSNDataTrafficEnabled == TRUE) {
	...
	pos += sprintf(pos, "Rate    %d Mbps\n", (int)getPhyRate((dbRateInfo_t *) & (StaInfo_p->RateInfo)));
#ifdef SOC_W906X
	pos += sprintf(pos, "RSSI:   A %d  B %d  C %d  D %d E %d  F %d  G %d  H %d\n",
	...
	pos += sprintf(pos, "  CmprsdStrngNbOfBfmrAntSupd  0x%X\n", StaInfo_p->vhtCap.cap.CompressedSteeringNumberofBeamformerAntennaSupported);
	...
	pos += sprintf(pos, "VHT CBF report statistics:\n");
	pos += sprintf(pos, "  Consecutively unchanged SNR count 0x%d\n", StaInfo_p->cbf_stat.snr_unchange_cnt);
...
static ssize_t ap8xLnxStat_clients_ratetable_pack(UINT8 type, UINT8 * pTbl, char *buf)
{
	char *pos = buf;
...
#endif
	pos += sprintf(pos, "%3s %6s %5s %5s %5s %5s %5s %5s %4s %2s %5s %4s %5s %5s\n", "Num", "Fmt", "STBC",
#ifdef SOC_W906X
		       "DCM",
#endif				/* SOC_W906X */
	...
	while (*(UINT32 *) pRateTbl != 0) {
		...
		pos += sprintf(pos, "%3d %5d %5d %5d %5d %5d %5d %5d %5d %5d %5d %5d %5d %5d\n", (int)j, (int)pRateTbl->Format, (int)pRateTbl->Stbc,
#ifdef SOC_W906X
			       (int)pRateTbl->Dcm,
#endif				/* SOC_W906X */
...
static ssize_t ap8xLnxStat_clients_ratetable(struct net_device *netdev, UINT8 * macAddr, char *buf)
{
	ssize_t size = 0;
	char *pos = buf;
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	...
	wlFwGetRateTable(netdev, macAddr, pRateTable, size, 0, StaInfo_p->StnId);
#else
...
static ssize_t ap8xLnxStat_clients_mu_mode_info(struct net_device *netdev, UINT8 * macAddr, char *buf)
{
	ssize_t size = 0;
	UINT8 *pos = buf;
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	...
	0};
	acnt_tx_t *acnt_tx_record = (acnt_tx_t *) wlpptr->wlpd_p->acnt_tx_record;
	...
	UINT32 tx_mcs = 0;
	UINT32 tx_mcs_0 = 0, tx_mcs_1 = 0, tx_mcs_2 = 0, tx_mcs_3 = 0, tx_mcs_4 = 0;
	...
	if (*(mib->mib_ApMode) & AP_MODE_11AX) {
		...
		if (StaInfo_p->he_cap_ie == HE_CAPABILITIES_IE &&
		    !(ismemzero((u8 *) & StaInfo_p->he_mac_cap, sizeof(HE_Mac_Capabilities_Info_t)) &&
		      ismemzero((u8 *) & StaInfo_p->he_phy_cap, sizeof(HE_Phy_Capabilities_Info_t)))) {
			strcpy(mode, "ax");
	...
	/* Caculate tx_airtime */
	if ((!strcmp(mode, "ax") || !strcmp(mode, "ac")) && (acnt_tx_record != NULL)) {
		UINT32 ar_timestamp = (UINT32) (readl(wlpptr->ioBase1 + BBTX_TMR_TSF));
		UINT32 delta_time = 0;

		currIdx = (wlpptr->wlpd_p->acnt_tx_record_idx == 0) ? (ACNT_TX_RECORD_MAX - 1) : (wlpptr->wlpd_p->acnt_tx_record_idx - 1);
		acnt_tx_record_dump = acnt_tx_record;
		for (idx = 0; idx < ACNT_TX_RECORD_MAX; idx++) {
			if (StaInfo_p->StnId == acnt_tx_record_dump[currIdx].StnId && acnt_tx_record_dump[currIdx].NumBytes > 0) {
				delta_time = (ar_timestamp > acnt_tx_record_dump[currIdx].TimeStamp) ?
				    (ar_timestamp - acnt_tx_record_dump[currIdx].TimeStamp) : ((0xFFFFFFFF - acnt_tx_record_dump[currIdx].TimeStamp) +
											       ar_timestamp);
				if (delta_time < 5 * 1000000) {
					/* 5 seconds */
					tx_airtime += acnt_tx_record_dump[currIdx].AirTime;

					tx_mcs = (acnt_tx_record_dump[currIdx].rateInfo & 0x00000F00) >> 8;
					if (tx_mcs == 0)
						tx_mcs_0++;
					else if (tx_mcs == 1)
						tx_mcs_1++;
	...
	if (!strcmp(mode, "ax")) {
		...
		if (StaInfo_p->he_phy_cap.su_beamformee == 1 &&
		    ((StaInfo_p->he_phy_cap.beamformee_sts_le_80mhz >= 3) || (StaInfo_p->he_phy_cap.beamformee_sts_gt_80mhz >= 3))) {
			dl_mimo = 1;
		}
	...
	if ((StaInfo_p->addba_rxamsdu == FALSE) && (*(mib->mib_wpaWpa2Mode) != 0)) {
		dl_mimo = 0;
		if (*(mib->pMib_11nAggrMode) & WL_MODE_AMSDU_TX_MASK)	/* If AP enable AMSDU, denied dl ofdma */
			dl_ofdma = 0;
	}
	...
	pos += sprintf(pos, "dl_ofdma: %d, dl_mimo: %d, ul_ofdma: %d, ul_mimo: %d\n", dl_ofdma, dl_mimo, ul_ofdma, ul_mimo);
	pos += sprintf(pos, "TxAirTime: %d\n", tx_airtime);
	pos += sprintf(pos, "TxMCS0: %d, TxMCS1: %d, TxMCS2: %d, TxMCS3: %d, TxMCS4: %d\n", tx_mcs_0, tx_mcs_1, tx_mcs_2, tx_mcs_3, tx_mcs_4);
	{
		rxppdu_airtime_t l_rxppdu_airtime;
		memcpy(&l_rxppdu_airtime, &StaInfo_p->rxppdu_airtime, sizeof(rxppdu_airtime_t));
		pos += sprintf(pos, "RxAirTime: %u, pktlen: %u, l-sig: %u, rxTs: %x\n", StaInfo_p->rxppdu_airtime.rx_airtime,
			       l_rxppdu_airtime.dbg_sum_pktlen, l_rxppdu_airtime.rx_info_aux.ppdu_len, l_rxppdu_airtime.rx_info_aux.rxTs);
		pos += sprintf(pos, "pktcnt: %u, rx_su_pktcnt: %u, rx_mu_pktcnt: %u, nss: %u, mcs: %u, bw: %u, gi_ltf: %u, Ndbps10x: %u\n",
			       l_rxppdu_airtime.dbg_sum_pktcnt,
			       l_rxppdu_airtime.dbg_su_pktcnt,
			       l_rxppdu_airtime.dbg_mu_pktcnt,
			       l_rxppdu_airtime.dbg_nss,
			       l_rxppdu_airtime.dbg_mcs, l_rxppdu_airtime.dbg_bw, l_rxppdu_airtime.dbg_gi_ltf, l_rxppdu_airtime.dbg_Ndbps10x);
		pos += sprintf(pos, " sum_rx_airtime: %llu, sum_rx_pktcnt: %llu, sum_rx_pktlen: %llu\n",
			       l_rxppdu_airtime.sum_rx_airtime, l_rxppdu_airtime.sum_rx_pktcnt, l_rxppdu_airtime.sum_rx_pktlen);
	}
	...
	pos += sprintf(pos, "tx_tcp_bytes: %llu\n", StaInfo_p->tx_tcp_bytes);
...
char *pkt_type_strn(u8 rxinfo_pkttype, rx_info_aux_t * prx_info_aux, u8 op_chnl)
{
	switch (rxinfo_pkttype) {
	case 0:
		if (prx_info_aux->rx_mode == BBRX_RM_B) {
			return "11b";
		} else if (prx_info_aux->rx_mode == BBRX_RM_A_G) {
			if (op_chnl < 36) {
				return "11g";
			} else {
				return "11a";
...
ssize_t ap8xLnxStat_clients_rxrate(struct net_device * netdev, UINT8 * macAddr, char *buf)
{
	...
	U8 Nss = 0;
	s16 nf_value[MAX_RF_ANT_NUM] = { 0 };
	s32 num_value_nf_cnt = 0, avg_value_nf = 0, avg_nf = 0;
	u8 i = 0;
	...
	pos += sprintf(pos, "%6s %5s %5s %5s %5s %5s %5s %4s %2s %5s %4s %5s %6s\n",
		       "Fmt", "STBC", "DCM", "BW", "SGI", "Nss", "RateId", "GF/Pre", "PId", "LDPC", "BF", "TxAnt", "RxRate");
	if (StaInfo_p->rx_info_aux.ppdu_len > 0) {
		pos += sprintf(pos, "%6s %5d %5d %5d %5d %5d %5d %5d %5d %5d %5d %02x %d Mbps\n",
			       pkt_type_strn(prate_info->Format, &StaInfo_p->rx_info_aux, PhyDSSSTable->CurrChan),
			       (int)prate_info->Stbc,
			       (int)prate_info->Dcm,
			       (int)prate_info->Bandwidth,
			       (int)prate_info->ShortGI,
			       (int)Nss,
			       (int)prate_info->RateIDMCS & 0xf,
			       (int)prate_info->Preambletype,
			       (int)prate_info->PowerId,
			       (int)prate_info->AdvCoding, (int)prate_info->BF, prate_info->AntSelect, (int)getPhyRate(prate_info));
	} else {
		// Has no data packets received yet
		pos += sprintf(pos, "%6s %5s %5s %5s %5s %5s %5s %5s %5s %5s %5s %2s %s Mbps\n",
			       "***", "*", "*", "*", "*", "*", "**", "*", "*", "*", "*", "**", "**");
	}
	for (i = 0; i < NUM_NFPATH_BUF_MAX; i++) {
		avg_nf = wl_util_get_nf(netdev, &StaInfo_p->nf_path[i], nf_value);
		if (avg_nf == 0)
			break;
		avg_value_nf += avg_nf;
		num_value_nf_cnt++;
	}
	if (num_value_nf_cnt == 0)
		avg_value_nf = 0;
	else
		avg_value_nf /= num_value_nf_cnt;
	pos += snprintf(pos, 512, "nf_avg: %d\n ", avg_value_nf);
	size = pos - (UINT8 *) buf;
 exit:
...
static ssize_t ap8xLnxStat_clients_show(struct kobject *kobj, struct clients_attribute *attr, char *buf)
{
	ssize_t size = 0;
	UINT8 macAddr[IEEE80211_ADDR_LEN] = { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff };
	const char *ifname = NULL;
	const char *name = (attr) ? attr->attr.name : NULL;
	struct kobject *parent_kobj = (kobj) ? kobj->parent : NULL;
	struct net_device *netdev = NULL;
	struct clients_table_attribute *tab_attr = container_of(attr, struct clients_table_attribute, attr);
...
static UINT8 ap8xLnxStat_clients_find_query(char *query_str)
{
	int i;

	for (i = 0; i < OPT_NUM; i++) {
		if (!strncmp(ap8xLnxStat_clients_query[i].query_str, query_str, strlen(query_str)))
...
static ssize_t ap8xLnxStat_clients_query_get(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	ssize_t size = 0;
	UINT8 macAddr[IEEE80211_ADDR_LEN] = { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff };
	const char *ifname = NULL;
	...
	if (is_broadcast_ether_addr(macAddr_query)) {
		...
		pos += sprintf(pos, "For the detail usage, please refer \"clients_help\"\n");
		size = pos - buf;
		return size;
	} else
...
static ssize_t ap8xLnxStat_clients_query_set(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	UINT8 macAddr[IEEE80211_ADDR_LEN] = { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff };
	char option[128] = { 0 };
	...
	if (sscanf(buf, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx %10s",
		   &macAddr[0], &macAddr[1], &macAddr[2], &macAddr[3], &macAddr[4], &macAddr[5], option) <= 0) {
		pr_err("%s: cannot get STA mac address\n", __func__);
		return -EINVAL;
	}
...
static CLIENTS_ATTR(query, S_IWUSR | S_IRUSR, ap8xLnxStat_clients_query_get, ap8xLnxStat_clients_query_set);
...
static struct attribute **ap8xLnxStat_clients_attrs_alloc(ssize_t(*show) (struct kobject * kobj,
							  ...
							  IEEEtypes_MacAddr_t * addr_p, int len)
{
	...
	for (i = 0; i < len; i++) {
		element = wl_kzalloc(sizeof(struct clients_table_attribute), GFP_KERNEL);
		if (!element)
			goto exit;

		element->attr.attr.name = ap8xLnxStat_clients_query[i].query_str;
...
static void ap8xLnxStat_clients_add(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * addr_p)
{
	int err = 0;
	char name[128];
	char sta_file_name[MAX_MACFILENAME_LEN];
	...
	if (vmac_p->OpMode == WL_OP_MODE_STA || vmac_p->OpMode == WL_OP_MODE_VSTA)
		return;

#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 4, 52)
	memset(name, 0x00, sizeof(name));
	vmacSta_p = (!vmac_p->master) ? vmac_p : vmac_p->master;
	strncpy(name, vmacSta_p->dev->name, sizeof(name));

	clients_kobj = ap8xLnxStat_clients_kobj_found((const char *)name, &ap8xLnxStat_clients_kobjs);
#endif

	clients_vap_kobj = ap8xLnxStat_clients_kobj_found((const char *)kobject_name(dev_kobj), &ap8xLnxStat_clients_vap_kobjs);
	...
	memset(sta_file_name, 0x00, sizeof(sta_file_name));
	MAC2FILENAME((UINT8 *) addr_p, sta_file_name);

	if (clients_vap_kobj->conn_cnt > 0) {
		struct kernfs_node *kn = NULL;

		kn = kernfs_find_and_get(clients_vap_kobj->kobj->sd, sta_file_name);
		if (kn) {
			kernfs_put(kn);
			pr_err("%s: client \"%s\" existed!\n", kobject_name(dev_kobj), sta_file_name);
			return;
		}
	}

	clients_attr = wl_kzalloc(sizeof(struct clients_attribute_group), GFP_KERNEL);
	if (!clients_attr) {
		pr_err("%s: cannot alloc memory for clients kobject\n", __func__);
		return;
	}
	...
	strncpy(clients_attr->name, sta_file_name, MAX_MACFILENAME_LEN);
	clients_attr->group.name = clients_attr->name;
	clients_attr->group.attrs = ap8xLnxStat_clients_attrs_alloc(ap8xLnxStat_clients_show, addr_p, OPT_NUM);
	if (!clients_attr->group.attrs) {
		pr_err("%s: create attributes for %s failed\n", __func__, sta_file_name);
		goto exit;
	}
	...
	if (clients_vap_kobj->conn_cnt++ == 0) {
		err = sysfs_create_group(clients_vap_kobj->kobj, &ap8xLnxStat_clients_group);
		if (err)
	...
	if (clients_kobj) {
		if (clients_kobj->conn_cnt > 0) {
			...
			kn = kernfs_find_and_get(clients_kobj->kobj->sd, sta_file_name);
			if (kn) {
				kernfs_put(kn);
				pr_err("%s: client \"%s\" existed!\n", __func__, sta_file_name);
		...
		err = __compat_only_sysfs_link_entry_to_kobj(clients_kobj->kobj, clients_vap_kobj->kobj, (const char *)sta_file_name);
		if (err)
			pr_err("%s: create link for clients failed (%d)\n", __func__, err);
		else {
			if (clients_kobj->conn_cnt++ == 0) {
				err = sysfs_create_group(clients_kobj->kobj, &ap8xLnxStat_clients_group);
				if (err)
					pr_err("%s: create group for clients kobject failed (%d)\n", __func__, err);
			}
		}
...
static void ap8xLnxStat_clients_del(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * addr_p)
{
	char name[128];
	vmacApInfo_t *vmacSta_p = NULL;
	struct clients_kobject *clients_kobj = NULL;
	...
	clients_vap_kobj = ap8xLnxStat_clients_kobj_found((const char *)vmac_p->dev->dev.kobj.name, &ap8xLnxStat_clients_vap_kobjs);

	if (clients_vap_kobj) {
		memset(name, 0x00, sizeof(name));
		MAC2FILENAME((UINT8 *) addr_p, name);

		clients_attr = ap8xLnxStat_clients_group_found((const char *)name, clients_vap_kobj);
		if (!clients_attr) {
		...
		if (clients_kobj) {
			sysfs_remove_link(clients_kobj->kobj, (const char *)name);
			if (clients_kobj->conn_cnt > 0) {
				if (--clients_kobj->conn_cnt == 0) {
					sysfs_remove_group(clients_kobj->kobj, &ap8xLnxStat_clients_group);
				}
			}
		}

		sysfs_remove_group(clients_vap_kobj->kobj, &clients_attr->group);

		ap8xLnxStat_clients_group_del(clients_attr, clients_vap_kobj);

		if (clients_vap_kobj->conn_cnt > 0) {
			if (--clients_vap_kobj->conn_cnt == 0) {
				sysfs_remove_group(clients_vap_kobj->kobj, &ap8xLnxStat_clients_group);
			}
		}
	}
...
void ap8xLnxStat_clients_WQadd(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * addr_p, UINT8 hdl)
{
	...
	SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->locks.sysfsHdlListLock, listflags);

	schedule_work(&wlpptr->wlpd_p->sysfstask);
	return;
}

void ap8xLnxStat_clients_WQhdl(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct clients_WQ_item *wqitem;
	...
	SPIN_LOCK_IRQSAVE(&wlpptr->wlpd_p->locks.sysfsHdlListLock, listflags);
	memcpy(&sysfsSTAHdlList, &wlpptr->wlpd_p->sysfsSTAHdlList, sizeof(List));
	...
	for (i = 0; i < listcnt; i++) {
		wqitem = (struct clients_WQ_item *)ListGetItem(&sysfsSTAHdlList);
		if (!wqitem)
			continue;
...
#else				/* SYSFS_STADB_INFO */
void ap8xLnxStat_clients_WQadd(vmacApInfo_t * vmac_p, IEEEtypes_MacAddr_t * addr_p)
{
	return;
};
