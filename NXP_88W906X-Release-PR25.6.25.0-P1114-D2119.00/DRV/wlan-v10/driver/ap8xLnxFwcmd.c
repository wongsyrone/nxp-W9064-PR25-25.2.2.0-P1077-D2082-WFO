...
extern void keymgmt_aesInfoGet(UINT8 ouiType, UINT32 * pKeyTypeId, UINT32 * pKenLen);
#ifdef SOC_W906X
extern void wlmon_log_buffer(struct net_device *netdev, UINT8 * buf, UINT32 len);
extern void wlmon_log_pfw_schInfo(struct net_device *netdev, QS_TX_SCHEDULER_INFO_t * pbuf);
extern void update_nontxd_bssid_profile_ssid(vmacApInfo_t * vmacSta_p, void *pssidIE, UINT8 ssid_len);
extern void update_nontxd_bssid_profile_cap(vmacApInfo_t * vmacSta_p, UINT16 CapInfo);
extern void update_nontxd_bssid_profile_bssidIdx(vmacApInfo_t * vmacSta_p);
#endif
#ifdef CONCURRENT_DFS_SUPPORT
extern DFS_STATE get_dfs_ctl_status(DfsApDesc * dfsDesc_p, UINT8 path);
...
/** internal functions **/
#ifdef MRVL_DFS
int DecideDFSOperation(struct net_device *netdev, BOOLEAN bChannelChanged, BOOLEAN bBandWidthChanged,
		       UINT8 currDFSState, UINT8 newDFSState, MIB_802DOT11 * mib);
#endif
...
#ifdef SOC_W906X
static void SMAC_RX_ENABLE(struct wlprivate *wlpptr, MIB_802DOT11 * mib, UINT32 macId)
{
	if (macId != 0xffffffff)
...
static void SMAC_RX_DISABLE(struct wlprivate *wlpptr, MIB_802DOT11 * mib, UINT32 macId)
{
	//check 32 BSS and Promicous  
	if (macId != 0xffffffff)
		*(mib->mib_rx_enable) &= ~(1 << macId);

	if (mib && (*(mib->mib_rx_enable) == 0) && (*(u32 *) (&((SMAC_CTRL_BLK_st *) wlpptr->ioBase0)->config.opMode) == 0)) {
		*(u32 *) (&((SMAC_CTRL_BLK_st *) wlpptr->ioBase0)->config.rxEnable) = 0;
	}
}
#endif				/* #ifdef SOC_W906X */

INLINE u_int8_t GET_CMD_SEQ_NUM(struct wlprivate *wlpptr)
{
#ifdef SOC_W906X
	u_int8_t retNum;
...
UINT8 keymgmt_aesModeGet(UINT8 ouiType)
{
...
void wlFwCmdComplete(struct net_device *netdev)
{
	...
	if (result != HostCmd_RESULT_OK) {
		WLDBG_INFO(DBG_LEVEL_0, "%s: FW cmd 0x%04x=%s failed: 0x%04x=%s\n",
			   wlgetDrvName(netdev), cmd, wlgetCmdString(cmd), result, wlgetCmdResultString(result));
	}
}
...
int wlFwGetHwSpecs(struct net_device *netdev)
{
	...
	WLDBG_EXIT_INFO(DBG_LEVEL_0,
			...
			wlpptr->hwData.regionCode, wlpptr->hwData.regionCode, wlpptr->hwData.hwVersion, wlpptr->hwData.hwVersion);
...
#ifdef NEW_DP
#ifndef SOC_W906X
	pCmd->WcbBase[0] = ENDIAN_SWAP32(wlpptr->wlpd_p->descData[0].pPhysTxRing);
	pCmd->WcbBase[1] = ENDIAN_SWAP32(wlpptr->wlpd_p->descData[0].pPhysTxRingDone);
	pCmd->WcbBase[2] = ENDIAN_SWAP32(wlpptr->wlpd_p->descData[0].pPhysRxRing);
	pCmd->WcbBase[3] = ENDIAN_SWAP32(wlpptr->wlpd_p->descData[0].pPhysRxRingDone);
#endif				/* #ifndef SOC_W906X */

#if defined(SOC_W906X) || defined(NEWDP_ACNT_CHUNKS)
	for (m = 0; m < ACNT_NCHUNK; m++) {
		pCmd->AcntBaseAddr[m] = ENDIAN_SWAP32(wlpptr->wlpd_p->descData[0].pPhysAcntRing[m]);
	}
...
#else
	pCmd->AcntBaseAddr = ENDIAN_SWAP32(wlpptr->wlpd_p->descData[0].pPhysAcntRing);
#endif

	pCmd->acntBufSize = ENDIAN_SWAP32(wlpptr->wlpd_p->descData[0].AcntRingSize);
...
static const char *ntoa(const uint8_t * mac)
{
	static char addr[3 * 6 + 2];

	snprintf(addr, sizeof(addr), "%02x:%02x:%02x:%02x:%02x:%02x", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
	return addr;
}
...
int wlFwGetEncrInfo(struct net_device *netdev, unsigned char *addr)
{
	...
	static const char *encrnames[] = { "WEP", "-", "#2", "#3", "TKIP", "#5", "AES", "MIX" };
	static const char *ciphernames[] = { "WEP", "TKIP", "AES", "#3", "#4", "#5", "#6", "#7" };
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	int i = 0, j;
	HostCmd_DS_802_11_GET_STAT *pCmd = (HostCmd_DS_802_11_GET_STAT *) & wlpptr->pCmdBuf[0];
	...
	if (p->EnHwEncr == 1) {
		...
		printk("%4s	%s	%s	%s	%s	%s	%s	%s\n", encrnames[p->EnHwEncr & 7]
		       , p->key.gWepKeyData.LoopBackOn ? "ena" : "-", p->key.gWepKeyData.WepEn ? "ena" : "-", p->key.gWepKeyData.IntrEn ? "ena" : "-",
		       p->key.gWepKeyData.MultiCastEn ? "ena" : "-", p->key.gWepKeyData.BroadCastEn ? "ena" : "-",
		       p->key.gWepKeyData.PromiscuousEn ? "ena" : "-", p->key.gWepKeyData.AllMultiCastEn ? "ena" : "-");
		printk("KeyId   HWSpec  WepType	Protect\n");
		printk("%d	%s	%s	%s\n", p->key.gWepKeyData.KeyId, p->key.gWepKeyData.HWSpecCmdDone ? "yes" : "no",
		       p->key.gWepKeyData.WepType ? "104" : "40", p->key.gWepKeyData.EnforceProtection ? "yes" : "no");
		for (i = 0; i < 4; i++) {
		...
		printk("%4s	%s	%s		%s	%s		%d\n", encrnames[p->EnHwEncr & 7]
		       , p->key.gKeyData.UnicastKeyEnabled ? "ena" : "-", p->key.gKeyData.UnicastKeyEnabled ?
		       ciphernames[p->key.gKeyData.UnicastKeyType & 7] : "-", p->key.gKeyData.MulticastKeyEnabled ? "ena" : "-",
		       p->key.gKeyData.MulticastKeyEnabled ? ciphernames[p->key.gKeyData.MulticastKeyType & 7] : "-", p->key.gKeyData.groupKeyIndex);
		if (p->key.gKeyData.UnicastKeyEnabled) {
			printk("PTK:      ");
			for (i = 0; i < 16; i++)
				printk(" %02x", p->key.gKeyData.RSNPairwiseTempKey[i]);
			printk("\n");
			printk("PTK TxMIC: %08x %08x	TxIV: %08x %04x\n", p->key.gKeyData.RSNPwkTxMICKey[0], p->key.gKeyData.RSNPwkTxMICKey[1],
			       /*le32toh */ (p->key.gKeyData.TxIV32),
			       /*le16toh */ (p->key.gKeyData.TxIV16));
			printk("PTK RxMIC: %08x %08x	RxIV: %08x\n", p->key.gKeyData.RSNPwkRxMICKey[0], p->key.gKeyData.RSNPwkRxMICKey[1],
			       /*le32toh */ (p->key.gKeyData.RxIV32));
		}
		if (p->key.gKeyData.MulticastKeyEnabled) {
			printk("GTK:      ");
			for (i = 0; i < 16; i++)
				printk(" %02x", p->key.gKeyData.RSNTempKey_group[i]);
			printk("\n");
			printk("GTK TxMIC: %08x %08x	TxIV: %08x %04x\n",
			       p->key.gKeyData.RSNTxMICKey_group[0], p->key.gKeyData.RSNTxMICKey_group[1],
			       /*le32toh */ (p->key.gKeyData.groupTxIV32),
			       /*le16toh */ (p->key.gKeyData.groupTxIV16));
			printk("GTK RxMIC: %08x %08x	RxIV: %08x\n", p->key.gKeyData.RSNRxMICKey_group[0], p->key.gKeyData.RSNRxMICKey_group[1],
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_FW_HT_GUARD_INTERVAL *pCmd = (HostCmd_FW_HT_GUARD_INTERVAL *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_HT_GUARD_INTERVAL));
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_DS_802_11_RADIO_CONTROL *pCmd = (HostCmd_DS_802_11_RADIO_CONTROL *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;

	WLDBG_ENTER_INFO(DBG_LEVEL_0, "mode: %s,preamble: %i", (mode == WL_DISABLE) ? "disable" : "enable", preamble);
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_DS_802_11_RADIO_CONTROL));
	retval = wlexecuteCommand(netdev, HostCmd_CMD_802_11_RADIO_CONTROL);
	MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
	return retval;
}

void BFMRconfigRxAnt(struct wlprivate *wlpptr, UINT8 RxAntTmp)
{
...
void BFMRconfigTxAnt(struct wlprivate *wlpptr, UINT8 TxAntTmp)
{
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
...
int wlFwSetAntenna(struct net_device *netdev, wlantennatype_e dirSet)
{
	...
	HostCmd_DS_802_11_RF_ANTENNA *pCmd = (HostCmd_DS_802_11_RF_ANTENNA *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;

	WLDBG_ENTER_INFO(DBG_LEVEL_0, "will set %s antenna", (dirSet == WL_ANTENNATYPE_RX) ? "RX" : "TX");
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_DS_802_11_RF_ANTENNA));
...
int wlFwSetCoexConfig(struct net_device *netdev, u8 * enable, u8 * gpioLevelDetect, u8 * gpioLevelTrigger, u32 * gpioReqPin, u32 * gpioGrantPin,
		      u32 * gpioPriPin, u8 set)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_DS_802_11_COEX_CONF *pCmd = (HostCmd_DS_802_11_COEX_CONF *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
	...
	if (WL_GET == set) {
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_DS_802_11_COEX_CONF));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->reserved = set;
		WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_DS_802_11_COEX_CONF));
...
#ifndef SOC_W906X
int wlFwSetRTSRetry(struct net_device *netdev, int rts_retry)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_DS_802_11_RTS_RETRY *pCmd = (HostCmd_DS_802_11_RTS_RETRY *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;

	printk("%s(), RTS Retry: %i (0x%x)\n", __func__, rts_retry, rts_retry);
	WLDBG_ENTER_INFO(DBG_LEVEL_0, "RTS Retry: %i (0x%x)", rts_retry, rts_retry);

	MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);
	memset(pCmd, 0x00, sizeof(HostCmd_DS_802_11_RTS_RETRY));
	pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_SET_RTS_RETRY);
	pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_DS_802_11_RTS_RETRY));
	pCmd->Action = ENDIAN_SWAP16(WL_SET);
	pCmd->Retry = ENDIAN_SWAP16(rts_retry);
	pCmd->CmdHdr.macid = wlpptr->vmacSta_p->VMacEntry.macId;

	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_DS_802_11_RTS_THSD));
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_DS_802_11_RTS_THSD *pCmd = (HostCmd_DS_802_11_RTS_THSD *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;

	WLDBG_ENTER_INFO(DBG_LEVEL_0, "RTS threshold: %i (0x%x)", threshold, threshold);
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_FW_SET_INFRA_MODE *pCmd = (HostCmd_FW_SET_INFRA_MODE *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_SET_INFRA_MODE));
...
int wlFwSetBssForClientMode(struct net_device *netdev, wlfacilitate_e facility)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	...
	WLDBG_ENTER_INFO(DBG_LEVEL_0, "AP bss %s", (facility == WL_ENABLE) ? "enable" : "disable");
	...
	if (facility == WL_ENABLE) {
		...
		for (rate_id = 0; rate_id < 14; rate_id++) {
			if (rateMask & 0x01)
				pCmd->BasicRate[basic_rate_id] = mib->StationConfig->OpRateSet[rate_id] & 0x7F;
			basic_rate_id++;
		...
		memcpy((void *)pCmd->MacAddr, (void *)netdev->dev_addr, IEEEtypes_ADDRESS_SIZE);
	}
...
int wlFwSetRadarDetection(struct net_device *netdev, UINT32 action)
{
	...
	HostCmd_802_11h_Detect_Radar *pCmd = (HostCmd_802_11h_Detect_Radar *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
...
#ifndef SOC_W906X
	/* not used in FW */
	if (*(mib->mib_regionCode) == DOMAIN_CODE_MKK && domainChannelValid(chan, FREQ_BAND_5GHZ)) {
		if (chan >= 52 && chan <= 64) {
			radarTypeCode = 53;
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_802_11h_Detect_Radar));
...
#ifdef SOC_W906X
int wlFwSetChannelSwitchIE(struct net_device *netdev, UINT16 nextChannel, UINT16 secChan, UINT32 mode, UINT32 count, CHNL_FLAGS * Chanflag)
#else
int wlFwSetChannelSwitchIE(struct net_device *netdev, UINT32 nextChannel, UINT32 mode, UINT32 count, UINT32 FreqBand, UINT32 ChnlWidth)
#endif
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
#ifdef SOC_W906X
	...
	MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
#endif
	HostCmd_SET_SWITCH_CHANNEL *pCmd = (HostCmd_SET_SWITCH_CHANNEL *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;

	WLDBG_ENTER_INFO(DBG_LEVEL_0, "AP Channel To Switch to %d Mode :%d and Count :%d", nextChannel, mode, count);
	...
	if (pCmd->ChannelFlags.ChnlWidth == CH_160_MHz_WIDTH)
		pCmd->ChannelFlags.ActPrimary = macMgmtMlme_Get160MHzPrimaryChannelOffset(pCmd->Next11hChannel);
#ifdef MV_CPU_BE
	pCmd->ChannelFlags.u32_data = ENDIAN_SWAP32(pCmd->ChannelFlags.u32_data);
#endif

	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_SET_SWITCH_CHANNEL));
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_SET_SPECTRUM_MGMT *pCmd = (HostCmd_SET_SPECTRUM_MGMT *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;

	WLDBG_ENTER_INFO(DBG_LEVEL_0, "Set Spectrum Management to %d", spectrumMgmt);
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_SET_SPECTRUM_MGMT));
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_SET_POWER_CONSTRAINT *pCmd = (HostCmd_SET_POWER_CONSTRAINT *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
	IEEEtypes_PowerConstraintElement_t PowerConstraintIE;

	WLDBG_ENTER_INFO(DBG_LEVEL_0, "Set power constraint to %d dB", powerConstraint);

	memset((UINT8 *) & PowerConstraintIE, 0, sizeof(IEEEtypes_PowerConstraintElement_t));
	PowerConstraintIE.ElementId = PWR_CONSTRAINT;
	PowerConstraintIE.value = powerConstraint;
	PowerConstraintIE.Len = 1;

	macMgmtMlme_UpdateProbeRspBasicIes(wlpptr->vmacSta_p, (UINT8 *) & PowerConstraintIE, PowerConstraintIE.Len + 2);
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_SET_POWER_CONSTRAINT));
	...
	if (pCmd->VHTTransmitPowerEnvelopeElement.Len) {
		macMgmtMlme_UpdateProbeRspBasicIes(wlpptr->vmacSta_p,
						   (UINT8 *) & pCmd->VHTTransmitPowerEnvelopeElement, pCmd->VHTTransmitPowerEnvelopeElement.Len + 2);
	}
...
int wlFwSetCountryCode(struct net_device *netdev, UINT32 domainCode)
{
	...
	HostCmd_SET_COUNTRY_INFO *pCmd = (HostCmd_SET_COUNTRY_INFO *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
	...
	WLDBG_ENTER_INFO(DBG_LEVEL_0, "Set  the country infor for :%x\n", domainCode);
	...
	if (domainCode == 0) {
		...
		memset((UINT8 *) & CountryIE, 0, sizeof(IEEEtypes_COUNTRY_IE_t));
		...
		if (DomainInfo->AChannelLen) {
			...
			memcpy((UINT8 *) (CountryIE.DomainEntry), (UINT8 *) (DomainInfo->DomainEntryA), DomainInfo->AChannelLen);
		} else {
			CountryIE.Len = DomainInfo->GChannelLen + 3;  /** include 3 byte of country code here **/
			memcpy((UINT8 *) (CountryIE.DomainEntry), (UINT8 *) (DomainInfo->DomainEntryG), DomainInfo->GChannelLen);
		}
		macMgmtMlme_UpdateProbeRspBasicIes(wlpptr->vmacSta_p, (UINT8 *) & CountryIE, CountryIE.Len + 2);
	}

	pCmd->Action = ENDIAN_SWAP32(pCmd->Action);

	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_SET_COUNTRY_INFO));
...
#ifdef IEEE80211K
int wlFwSetRadioResourceMgmt(struct net_device *netdev, UINT8 rrm)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
...
int wlFwSetBcnChannelUtil(struct net_device *netdev, UINT8 ch_tril)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_SET_CH_UTIL *pCmd = (HostCmd_SET_CH_UTIL *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
...
int wlFwSetQuiet(struct net_device *netdev, UINT8 enable, UINT8 period, UINT16 duration, UINT16 offset, UINT16 offset1, UINT8 txStop_en)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	...
	WLDBG_ENTER_INFO(DBG_LEVEL_0, "Set Quiet to %d %d %d %d", enable, period, duration, offset);

#if 1				//dbg
	printk("wlFwSetQuiet(): %d %d %d %d %d %d\n", enable, period, duration, offset, offset1, txStop_en);
#endif
...
#ifdef WMM_AC_EDCA
int wlFwSetBssLoadAac(struct net_device *netdev, UINT16 aac)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_SET_BSS_LOAD_AAC *pCmd = (HostCmd_SET_BSS_LOAD_AAC *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_SET_BSS_LOAD_AAC));
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_SET_REGIONCODE_INFO *pCmd = (HostCmd_SET_REGIONCODE_INFO *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_FW_GET_NOISE_Level *pCmd = (HostCmd_FW_GET_NOISE_Level *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_FW_GET_NOISE_Level));
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_SET_WAPI_IE *pCmd = (HostCmd_SET_WAPI_IE *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
...
int wlFwSetRate(struct net_device *netdev, wlrate_e rate)
{
	...
	if (*(mib->mib_enableFixedRateTx) == 2) {
		...
		if (*(mib->mib_FixedRateTxType) == 0x4) {	//use rateinfo directly
			pCmd->FixedRateTable[0].FixRateTypeFlags.FixRateType = ENDIAN_SWAP32(0x4);
			pCmd->FixedRateTable[0].FixedRate = ENDIAN_SWAP32(*(mib->mib_txDataRateInfo));

		} else if (*(mib->mib_FixedRateTxType) == 0x2) {
			//fixed vht rate
			pCmd->FixedRateTable[0].FixRateTypeFlags.FixRateType = ENDIAN_SWAP32(0x2);
			pCmd->FixedRateTable[0].FixedRate = ENDIAN_SWAP32(*(mib->mib_txDataRateVHT));
		} else if (*(mib->mib_FixedRateTxType) == 0x1) {
			//fixed 11n rate
			pCmd->FixedRateTable[0].FixRateTypeFlags.FixRateType = ENDIAN_SWAP32(0x1);
			pCmd->FixedRateTable[0].FixedRate = ENDIAN_SWAP32(*(mib->mib_txDataRateN));
		} else {
			//legacy
			pCmd->FixedRateTable[0].FixRateTypeFlags.FixRateType = 0;

			mode = *(mib->mib_FixedRateTxType) >> 4;
			if (mode == 0x2)
				pCmd->FixedRateTable[0].FixedRate = ENDIAN_SWAP32(*(mib->mib_txDataRateA));
			else if (mode == 0x1)
				pCmd->FixedRateTable[0].FixedRate = ENDIAN_SWAP32(*(mib->mib_txDataRateG));
			else
				pCmd->FixedRateTable[0].FixedRate = ENDIAN_SWAP32(*(mib->mib_txDataRate));
		}

		pCmd->EntryCount = ENDIAN_SWAP32(1);
		WLDBG_INFO(DBG_LEVEL_0, "%s rate at %i w/o auto drop", *(mib->mib_FixedRateTxType) ? "HT" : "legacy",
			   *(mib->mib_FixedRateTxType) ? *(mib->mib_txDataRateN) : *(mib->mib_txDataRate));
		...
		WLDBG_INFO(DBG_LEVEL_0, "%s rate at %i w/ auto drop", *(mib->mib_FixedRateTxType) ? "HT" : "legacy",
			   *(mib->mib_FixedRateTxType) ? *(mib->mib_txDataRateN) : *(mib->mib_txDataRate));
	} else {
		pCmd->AllowRateDrop = ENDIAN_SWAP32(FIXED_RATE_WITH_AUTO_RATE_DROP);
...
int wlFwSetSlotTime(struct net_device *netdev, wlslot_e slot)
{
	...
	WLDBG_ENTER_INFO(DBG_LEVEL_0, "set slot: %s", (slot == WL_SHORTSLOT) ? "short" : "long");
...
static int wlFwSettxpowers(struct net_device *netdev, UINT16 txpow[], UINT8 action, UINT16 ch, UINT16 band, UINT16 width, UINT16 sub_ch)
{
	int retval, i;
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
...
int wlFwSetTxPowerClientScan(struct net_device *netdev, UINT8 flag, UINT32 powerLevel, UINT16 channel)
{
	...
	HostCmd_DS_802_11_RF_TX_POWER *pCmd = (HostCmd_DS_802_11_RF_TX_POWER *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	MIB_PHY_DSSS_TABLE *PhyDSSSTable = mib->PhyDSSSTable;
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_DS_802_11_RF_TX_POWER));
...
#endif
{
	...
	int retval = FAIL;
	unsigned int num = 0;
	HostCmd_DS_MAC_MULTICAST_ADR *pCmd = (HostCmd_DS_MAC_MULTICAST_ADR *) & wlpptr->pCmdBuf[0];
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_DS_MAC_MULTICAST_ADR));
...
static inline void DOT11_RESETPN(u8 * dst)
{
	(*(u16 *) & dst[0]) = 0;
	(*(u32 *) & dst[2]) = 0;
}

static int pn_replay_update_key_index(struct wlprivate *wlpptr, UINT8 * macAddr, UINT8 keyType, UINT8 keyIndex)
{
	rx_queue_t *rq;
	...
	if (*(mib->mib_STAMode))
		macAddr = GetParentStaBSSID(wlpptr->vmacSta_p->VMacEntry.phyHwMacIndx);

	pStaInfo = extStaDb_GetStaInfo(wlpptr->vmacSta_p, (IEEEtypes_MacAddr_t *) macAddr, STADB_DONT_UPDATE_AGINGTIME);
	...
	if (pStaInfo) {
		if (keyType == ENCR_KEY_FLAG_GTK_RX_KEY)
			...
			DOT11_RESETPN(pStaInfo->pn->ucMgmtRxQueues.RxPN[keyIndex]);
		}
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_FW_UPDATE_SECURITY_KEY *pCmd = (HostCmd_FW_UPDATE_SECURITY_KEY *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	...
	if ((StaInfo_p = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) pMacAddr, STADB_NO_BLOCK | STADB_SKIP_MATCH_VAP)) != NULL) {
		...
		if (StaInfo_p->pDefragSkBuff) {
			...
			wlexcept_p->cnt_defrag_drop_x[10]++;
		}
	}
	...
	pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_UPDATE_SECURITY_KEY));
	pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
	pCmd->CmdHdr.macid = wlpptr->vmacSta_p->VMacEntry.macId;
	pCmd->Action = ENDIAN_SWAP16(action);
	...
	if (action != ACT_DEL) {
		...
		case KEY_TYPE_ID_WEP:
			{
				...
				memcpy(pCmd->Key.Wep.KeyMaterial, pKey->KeyMaterial, keyLen);
			}
			break;
		case KEY_TYPE_ID_TKIP:
			{
				TKIP_TYPE_KEY *pKey = (TKIP_TYPE_KEY *) pKeyParam;

				memcpy(pCmd->Key.Tkip.KeyMaterial, pKey->KeyMaterial, sizeof(pCmd->Key.Tkip.KeyMaterial));
				memcpy(pCmd->Key.Tkip.TxMicKey, pKey->TxMicKey, sizeof(pCmd->Key.Tkip.TxMicKey));
				memcpy(pCmd->Key.Tkip.RxMicKey, pKey->RxMicKey, sizeof(pCmd->Key.Tkip.RxMicKey));

				pCmd->Key.Tkip.Rsc.low = 0;
				pCmd->Key.Tkip.Rsc.high = 0;
				pCmd->Key.Tkip.Tsc.low = ENDIAN_SWAP16(pKey->Tsc.low);
				pCmd->Key.Tkip.Tsc.high = ENDIAN_SWAP32(pKey->Tsc.high);
				keyLen += sizeof(pCmd->Key.Tkip.TxMicKey) + sizeof(pCmd->Key.Tkip.RxMicKey);
#ifdef RX_REPLAY_DETECTION
				pn_replay_update_key_index(wlpptr, pMacAddr, (keyInfo & ENCR_KEY_FLAG_GTK_RX_KEY), keyIndex);
#endif
			}
			break;
		...
		case KEY_TYPE_ID_GCMP:
			{
				...
				memcpy(pCmd->Key.Aes.KeyMaterial, pKey->KeyMaterial, keyLen);
#ifdef RX_REPLAY_DETECTION
				pn_replay_update_key_index(wlpptr, pMacAddr, (keyInfo & ENCR_KEY_FLAG_GTK_RX_KEY), keyIndex);
#endif
			}
		...
		case KEY_TYPE_ID_WAPI:
			{
				WAPI_TYPE_KEY *pKey = (WAPI_TYPE_KEY *) pKeyParam;

				memcpy(pCmd->Key.Wapi.PN, pKey->PN, WAPI_PN_LENGTH);
				memcpy(pCmd->Key.Wapi.KeyMaterial, pKey->KeyMaterial, keyLen);
				memcpy(pCmd->Key.Wapi.MicKeyMaterial, pKey->MicKeyMaterial, sizeof(pCmd->Key.Wapi.MicKeyMaterial));
			}
			break;
		default:
			MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
	...
	WLDBG_INFO(DBG_LEVEL_0, "HostCmd_CMD_UPDATE_SECURITY_KEY Action=%d KeyType=%d KeyLen=%d keyInfo=0x%08x", action, type, keyLen, keyInfo);

	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_UPDATE_SECURITY_KEY));
...
int wlFwSetWep(struct net_device *netdev, u_int8_t * staaddr)
{
	...
	for (keyIndex = 0; keyIndex < 4; keyIndex++) {
		...
		memcpy(param.KeyMaterial, mib->WepDefaultKeys[keyIndex].WepDefaultKeyValue, keyLen);
		retval += wlFwSetSecurityKey(netdev, ACT_SET, KEY_TYPE_ID_WEP, staaddr, keyIndex, keyLen, keyInfo, (UINT8 *) & param);
	}
#else
...
int wlFwSetWpaTkipMode(struct net_device *netdev, u_int8_t * staaddr)
{
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_UPDATE_ENCRYPTION));
	retval = wlexecuteCommand(netdev, HostCmd_CMD_UPDATE_ENCRYPTION);
...
int wlFwSetWpaWpa2PWK(struct net_device *netdev, extStaDb_StaInfo_t * StaInfo_p)
{
	...
	if ((StaInfo_p->keyMgmtStateInfo.RsnIEBuf[0] == 221 && StaInfo_p->keyMgmtStateInfo.RsnIEBuf[17] == 2)
		...
		keyInfo = ENCR_KEY_FLAG_PTK | ENCR_KEY_FLAG_TSC_VALID | ENCR_KEY_FLAG_MICKEY_VALID;

		memcpy(tkipParam.KeyMaterial, StaInfo_p->keyMgmtStateInfo.PairwiseTempKey1, MAX_ENCR_KEY_LENGTH);
		memcpy(tkipParam.TxMicKey, StaInfo_p->keyMgmtStateInfo.RSNPwkTxMICKey, MIC_KEY_LENGTH);
		memcpy(tkipParam.RxMicKey, StaInfo_p->keyMgmtStateInfo.RSNPwkRxMICKey, MIC_KEY_LENGTH);
		tkipParam.Rsc.low = 0;
		...
		tkipParam.Tsc.high = StaInfo_p->keyMgmtStateInfo.TxIV32;
		   ...
		   || (StaInfo_p->keyMgmtStateInfo.RsnIEBuf[0] == 48 && isAes4RsnValid(StaInfo_p->keyMgmtStateInfo.RsnIEBuf[13]))) {
		...
		memcpy(aesParam.KeyMaterial, StaInfo_p->keyMgmtStateInfo.PairwiseTempKey1, keyLen);
	} else {
		return retval;
	}

	wlFwSetSecurityKey(netdev, ACT_SET, keyType, StaInfo_p->Addr, 0, keyLen, keyInfo, pParam);
	...
	if ((StaInfo_p->keyMgmtStateInfo.RsnIEBuf[0] == 221 && StaInfo_p->keyMgmtStateInfo.RsnIEBuf[17] == 2)
		...
		pCmd->KeyParam.KeyInfo = ENDIAN_SWAP32(ENCR_KEY_FLAG_PAIRWISE | ENCR_KEY_FLAG_TSC_VALID | ENCR_KEY_FLAG_MICKEY_VALID);
		...
		memcpy(pCmd->KeyParam.Key.AesKey.KeyMaterial, StaInfo_p->keyMgmtStateInfo.PairwiseTempKey1, keyLen);
		memcpy(pCmd->KeyParam.Macaddr, StaInfo_p->Addr, 6);
		WLDBG_INFO(DBG_LEVEL_0, "HostCmd_FW_UPDATE_ENCRYPTION_KEY_SET WPA AES Key Type=%d Len=%d \n", keyTypeId, keyLen);
	} else {
		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
		return retval;
	}
...
int keyMgmtCleanGroupKey(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	HostCmd_FW_UPDATE_ENCRYPTION_SET_KEY *pCmd = (HostCmd_FW_UPDATE_ENCRYPTION_SET_KEY *) & wlpptr->pCmdBuf[0];
	...
	pCmd->KeyParam.KeyInfo = ENDIAN_SWAP32(ENCR_KEY_FLAG_RXGROUPKEY | ENCR_KEY_FLAG_TXGROUPKEY);
	pCmd->KeyParam.KeyIndex = 0;	// NA for wpa
	pCmd->KeyParam.KeyLen = 0;
	memcpy(pCmd->KeyParam.Macaddr, &vmacSta_p->macStaAddr[0], 6);

	WLDBG_INFO(DBG_LEVEL_0, "HostCmd_FW_UPDATE_ENCRYPTION_KEY_SET Remove GTK\n");
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_UPDATE_ENCRYPTION_SET_KEY));
...
int wlFwSetWpaTkipGroupK(struct net_device *netdev, UINT8 index)
{
	...
	pCmd->KeyParam.Length = ENDIAN_SWAP16(sizeof(pCmd->KeyParam));
	pCmd->KeyParam.KeyTypeId = ENDIAN_SWAP16(KEY_TYPE_ID_TKIP);
	pCmd->KeyParam.KeyInfo = ENDIAN_SWAP32(ENCR_KEY_FLAG_TXGROUPKEY | ENCR_KEY_FLAG_MICKEY_VALID | ENCR_KEY_FLAG_TSC_VALID);
...
int wlFwSetWpaGroupK_rx(struct net_device *netdev, wlreq_key * wk)
{
	...
	case WL_CIPHER_TKIP:
		pParam = (UINT8 *) & tkipParam;
		keyType = KEY_TYPE_ID_TKIP;
		memcpy(tkipParam.KeyMaterial, &wk->ik_keydata[0], MAX_ENCR_KEY_LENGTH);
	...
	wlFwSetSecurityKey(netdev, ACT_SET, keyType, wk->ik_macaddr, wk->ik_keyix, wk->ik_keylen, ENCR_KEY_FLAG_RXONLY, pParam);
#else
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	MIB_802DOT11 *mib = vmacSta_p->Mib802dot11;
	...
	case WL_CIPHER_TKIP:
		...
		memcpy(pCmd->KeyParam.Key.TkipKey.KeyMaterial, &wk->ik_keydata[0], MAX_ENCR_KEY_LENGTH);
		memcpy(pCmd->KeyParam.Key.TkipKey.TkipTxMicKey, &wk->ik_keydata[16], MIC_KEY_LENGTH);
		memcpy(pCmd->KeyParam.Key.TkipKey.TkipRxMicKey, &wk->ik_keydata[24], MIC_KEY_LENGTH);
		pCmd->KeyParam.Key.TkipKey.TkipRsc.low = 0;
		pCmd->KeyParam.Key.TkipKey.TkipRsc.high = 0;
	...
	case WL_CIPHER_CCMP:
		...
		memcpy(pCmd->KeyParam.Key.AesKey.KeyMaterial, &wk->ik_keydata[0], TK_SIZE);
		break;
	case WL_CIPHER_GCMP:
	...
	case WL_CIPHER_CCMP_256:
		...
		memcpy(pCmd->KeyParam.Key.AesKey.KeyMaterial, &wk->ik_keydata[0], TK_SIZE_MAX);
		break;
	case WL_CIPHER_GCMP_256:
	...
	pCmd->KeyParam.KeyTypeId = ENDIAN_SWAP16(KeyTypeId);
	pCmd->KeyParam.KeyInfo = ENDIAN_SWAP32(ENCR_KEY_FLAG_RXGROUPKEY | ENCR_KEY_FLAG_MICKEY_VALID | ENCR_KEY_FLAG_TSC_VALID);
...
#ifndef SOC_W906X
int wlFwSetWpaAesMode(struct net_device *netdev, u_int8_t * staaddr, UINT8 ouiType)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_FW_UPDATE_ENCRYPTION *pCmd = (HostCmd_FW_UPDATE_ENCRYPTION *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
...
int wlFwSetWpaAesGroupK(struct net_device *netdev, UINT8 index, UINT8 ouiType)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	MIB_802DOT11 *mib = vmacSta_p->Mib802dot11;
	...
	memcpy(pCmd->KeyParam.Key.AesKey.KeyMaterial, mib->mib_MrvlRSN_GrpKey->EncryptKey, keyLen);
	memcpy(pCmd->KeyParam.Macaddr, &vmacSta_p->macStaAddr[0], 6);

	WLDBG_INFO(DBG_LEVEL_0, "HostCmd_FW_UPDATE_ENCRYPTION_KEY_SET WPA AES Group Key Len=%d Type=%d", keyLen, keyTypeId);
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_UPDATE_ENCRYPTION_SET_KEY));
	retval = wlexecuteCommand(netdev, HostCmd_CMD_UPDATE_ENCRYPTION);
...
int wlFwSetWpaAesMode_STA(struct net_device *netdev, u_int8_t * staaddr, u_int8_t ouiType)
{
	...
	WLDBG_INFO(DBG_LEVEL_0, "wlFwSetWpaAesMode_STA::: set security mode %d\n", pCmd->ActionData[0]);
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_UPDATE_ENCRYPTION));
	retval = wlexecuteCommand(netdev, HostCmd_CMD_UPDATE_ENCRYPTION);
...
int wlFwSetApBeacon(struct net_device *netdev)
{
	...
	if (*(mib->mib_broadcastssid) && (ssid_len <= IEEEtypes_SSID_SIZE))
		memcpy(&pCmd->StartCmd.SsId[0], &(mib->StationConfig->DesiredSsId[0]), ssid_len);
#ifdef SOC_W906X
	if (*(mib->mib_broadcastssid) && (ssid_len <= IEEEtypes_SSID_SIZE)) {
		ssid_p = (IEEEtypes_SsIdElement_t *) wl_kmalloc(strlen(&(mib->StationConfig->DesiredSsId[0])) +
								sizeof(IEEEtypes_InfoElementHdr_t), GFP_ATOMIC);
		ssid_p->ElementId = IEEE80211_ELEMID_SSID;
		ssid_p->Len = strlen(&(mib->StationConfig->DesiredSsId[0]));
		memcpy(&ssid_p->SsId, &(mib->StationConfig->DesiredSsId[0]), ssid_len);
		macMgmtMlme_UpdateProbeRspBasicIes(vmacSta_p, (UINT8 *) ssid_p, sizeof(IEEEtypes_InfoElementHdr_t) + ssid_p->Len);
		update_nontxd_bssid_profile_ssid(vmacSta_p, ssid_p, ssid_p->Len);
		wl_kfree(ssid_p);
	}
#endif
	...
	cfParamSet->Len = sizeof(cfParamSet->CfpCnt) +
	    sizeof(cfParamSet->CfpPeriod) + sizeof(cfParamSet->CfpMaxDuration) + sizeof(cfParamSet->CfpDurationRemaining);
	...
	if (mib->PhyDSSSTable->Chanflag.FreqBand == FREQ_BAND_2DOT4GHZ) {
...
#ifdef SOC_W906X
		macMgmtMlme_UpdateProbeRspBasicIes(vmacSta_p, (UINT8 *) phyDsParamSet, sizeof(DsParams_t));
#endif
	}
...
#endif
	{
		...
		if (mib->Privacy->RSNEnabled) {
			...
			if (!mib->RSNConfigWPA2->WPA2OnlyEnabled) {
				if (vmacSta_p->Mib802dot11->thisStaRsnIE->PwsKeyCnt[0] > 1) {
					/* In mixed mode, if pairwise key > 1, Add mix WPA_IE in RsnMixedIE */
					AddRSN_IE(vmacSta_p, (IEEEtypes_RSN_IE_t *) & pCmd->RsnMixedIE);
#ifdef SOC_W906X
					macMgmtMlme_UpdateProbeRspBasicIes(vmacSta_p, (UINT8 *) & pCmd->RsnMixedIE, pCmd->RsnMixedIE.Len + 2);
#endif
				} else {
					IEEEtypes_RSN_IE_t tmp_RsnIE;

					memset(&tmp_RsnIE, 0, sizeof(IEEEtypes_RSN_IE_t));
					AddRSN_IE(vmacSta_p, (IEEEtypes_RSN_IE_t *) & tmp_RsnIE);
					memcpy(&pCmd->StartCmd.RsnIE, &tmp_RsnIE, sizeof(IEEEtypes_RSN_IE_t) - 4);	// minus Reserved field.
#ifdef SOC_W906X
					macMgmtMlme_UpdateProbeRspBasicIes(vmacSta_p, (UINT8 *) & pCmd->StartCmd.RsnIE, pCmd->StartCmd.RsnIE.Len + 2);
#endif
				}
			}
			capInfo |= HostCmd_CAPINFO_PRIVACY;
...
#ifdef SOC_W906X
		macMgmtMlme_UpdateProbeRspBasicIes(vmacSta_p, (UINT8 *) & pCmd->StartCmd.WMMParam, sizeof(WME_param_elem_t));
#endif
	}
	if (mib->PhyDSSSTable->Chanflag.FreqBand == FREQ_BAND_2DOT4GHZ) {
		ErpInfo.ElementId = ERP_INFO;
		ErpInfo.Len = 1;
		*((UINT8 *) & ErpInfo.ERPInfo) = wlpptr->wlpd_p->BcnErpVal;
		macMgmtMlme_UpdateProbeRspBasicIes(vmacSta_p, (UINT8 *) & ErpInfo, sizeof(IEEEtypes_ERPInfoElement_t));
	}
#ifdef AP_WPA2
...
#ifdef SOC_W906X
			macMgmtMlme_UpdateProbeRspBasicIes(vmacSta_p, (UINT8 *) & pCmd->StartCmd.Rsn48IE, pCmd->StartCmd.Rsn48IE.Len + 2);
#endif
		} else {
...
#endif
#ifdef SOC_W906X
	macMgmtMlme_UpdateProbeRspInfo(vmacSta_p, pCmd->StartCmd.BcnPeriod, pCmd->StartCmd.CapInfo);
#endif
#ifdef BRS_SUPPORT
	...
	if (mib->RSNConfigWPA2->WPA2Enabled || mib->RSNConfigWPA2->WPA2OnlyEnabled) {
		if (vmacSta_p->RSNXIE[1] != 0) {
			pCmd->StartCmd.sae_pwe = 1;
		}
	}

	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_DS_AP_BEACON));
...
int wlFwSetAid(struct net_device *netdev, u_int8_t * bssId, u_int16_t assocId)
{
	...
	WLDBG_ENTER_INFO(DBG_LEVEL_0, "bssid: %s, association ID: %i", mac_display(bssId), assocId);
...
void BFMRconfigChanBw(struct wlprivate *wlpptr, UINT8 ChnlWidth, UINT8 ExtChnlOffset, UINT8 channel)
{
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
...
int wlFwSetChannel(struct net_device *netdev, u_int8_t channel, u_int8_t secchannel, CHNL_FLAGS * Chanflag, u_int8_t initRateTable)
#else
...
#endif
{
	...
	HostCmd_FW_SET_RF_CHANNEL *pCmd = (HostCmd_FW_SET_RF_CHANNEL *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
#ifdef SOC_W906X
	UINT32 centerfreq;
	...
	if (pCmd->ChannelFlags.ChnlWidth == CH_80_MHz_WIDTH) {
		if (Chanflag->radiomode == RADIO_MODE_7x7p1x1)
			pCmd->ChannelFlags.ActPrimary = macMgmtMlme_Get80MHzPrimaryChannelOffset(secchannel);
		else
...
#else
	if (pCmd->ChannelFlags.ChnlWidth == CH_160_MHz_WIDTH)
		pCmd->ChannelFlags.ActPrimary = macMgmtMlme_Get160MHzPrimaryChannelOffset(channel);
#endif
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_SET_RF_CHANNEL));
	SMAC_RX_DISABLE(wlpptr, mib, vmacSta_p->VMacEntry.macId);
...
int wlFwCreateBAStream(struct net_device *dev, u_int32_t BarThrs, u_int32_t WindowSize, u_int8_t * Macaddr,
		       ...
		       u_int8_t * SrcMacaddr, UINT16 seqNo, UINT32 vhtrxfactor, UINT32 queueid, u_int16_t staid)
#else
int wlFwCreateBAStream(struct net_device *dev, u_int32_t BarThrs, u_int32_t WindowSize, u_int8_t * Macaddr,
		       u_int8_t DialogToken, u_int8_t Tid, u_int32_t ba_type, u_int32_t direction, u_int8_t ParamInfo,
		       u_int8_t * SrcMacaddr, UINT16 seqNo, UINT32 vhtrxfactor, UINT32 queueid)
#endif
{
	...
	UINT32 ampduBytes = vhtrxfactor ? ((1 << ((vhtrxfactor & 0x7) + 13)) - 4) : ((1 << ((ParamInfo & 0x3) + 13)) - 4);

	WLDBG_INFO(DBG_LEVEL_14, "before cap: ParamInfo %x, vhtrxfactor %x, WindowSize %x\n", ParamInfo, vhtrxfactor, WindowSize);
	...
	WLDBG_INFO(DBG_LEVEL_14, "after cap: ParamInfo %x, vhtrxfactor %x, WindowSize %x\n", ParamInfo, vhtrxfactor, WindowSize);
#endif				/* #ifdef SOC_W906X */
	...
	if (wlpptr->is_ba4roam == true) {
		pCmd->BaInfo.CreateParams.Flags.BaForRoamed = 1;
	} else {
		pCmd->BaInfo.CreateParams.Flags.BaForRoamed = 0;
	}
	printk("pCmd->BaInfo.CreateParams.Flags.BaForRoamed: %u\n", pCmd->BaInfo.CreateParams.Flags.BaForRoamed);
...
int wlFwGetSeqNoBAStream(struct net_device *dev, u_int8_t * Macaddr, uint8_t Tid, uint16_t * pbaseq_no)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
		  ...
		  PeerInfo_t * pPeerInfo, UINT8 Qosinfo, UINT8 isQosSta, UINT8 wds)
{
	...
	WLDBG_ENTER_INFO(DBG_LEVEL_0,
			 "staid: %s, association ID: %i stnId: %i action %i qosinfo %i qosSta %i\n", mac_display(staaddr), assocId, stnId, action,
			 Qosinfo, isQosSta);
#else
	WLDBG_ENTER_INFO(DBG_LEVEL_0, "bssid: %s, association ID: %i stnId: %i action %i", mac_display(bssId), assocId, stnId, action);
#endif
	...
	if (pPeerInfo) {
		if (wlpptr->retrycfgenable) {
			...
			if (PhyDSSSTable->Chanflag.FreqBand == FREQ_BAND_2DOT4GHZ || !pPeerInfo->HTRateBitMap) {
				printk("legacy:retry cfg \n");
				pPeerInfo->retrycntQoS.retrycntBK = wlpptr->retrycfgVAP.RetryLegacy[0];
				pPeerInfo->retrycntQoS.retrycntBE = wlpptr->retrycfgVAP.RetryLegacy[1];
				pPeerInfo->retrycntQoS.retrycntVI = wlpptr->retrycfgVAP.RetryLegacy[2];
				pPeerInfo->retrycntQoS.retrycntVO = wlpptr->retrycfgVAP.RetryLegacy[3];

			} else if (!pPeerInfo->vht_cap) {
				printk("11n:retry cfg \n");
				pPeerInfo->retrycntQoS.retrycntBK = wlpptr->retrycfgVAP.Retry11n[0];
				pPeerInfo->retrycntQoS.retrycntBE = wlpptr->retrycfgVAP.Retry11n[1];
				pPeerInfo->retrycntQoS.retrycntVI = wlpptr->retrycfgVAP.Retry11n[2];
				pPeerInfo->retrycntQoS.retrycntVO = wlpptr->retrycfgVAP.Retry11n[3];
			} else if (pPeerInfo->vht_cap) {
				printk("11ac: retry cfg \n");
				pPeerInfo->retrycntQoS.retrycntBK = wlpptr->retrycfgVAP.Retry11ac[0];
				pPeerInfo->retrycntQoS.retrycntBE = wlpptr->retrycfgVAP.Retry11ac[1];
				pPeerInfo->retrycntQoS.retrycntVI = wlpptr->retrycfgVAP.Retry11ac[2];
				pPeerInfo->retrycntQoS.retrycntVO = wlpptr->retrycfgVAP.Retry11ac[3];
			}
			printk(" BK %d BE %d VI %d VO %d \n", pPeerInfo->retrycntQoS.retrycntBK, pPeerInfo->retrycntQoS.retrycntBE,
			       pPeerInfo->retrycntQoS.retrycntVI, pPeerInfo->retrycntQoS.retrycntVO);
		} else {
		...
		if (wlpptr->wlpd_p->ldpcdisable) {
			...
			((IEEEtypes_VHT_Cap_Info_t *) & pCmd->PeerInfo.vht_cap)->RxLDPC = 0;
		}

		if (((pStaInfo = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) staaddr, STADB_DONT_UPDATE_AGINGTIME)) != NULL) &&
		    ((action == StaInfoDbActionAddEntry) || (action == StaInfoDbActionModifyEntry))) {
#ifdef STA_FULL_HTVHT_CAP
			memcpy((void *)&(pCmd->HtElem), (void *)&(pStaInfo->HtElem), sizeof(IEEEtypes_HT_Element_t));
			memcpy((void *)&(pCmd->vhtCap), (void *)&(pStaInfo->vhtCap), sizeof(IEEEtypes_VhtCap_t));
			if (wlpptr->wlpd_p->ldpcdisable) {
	...
	if ((pStaInfo = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) staaddr, STADB_DONT_UPDATE_AGINGTIME)) != NULL) {
		if ((pStaInfo->State == ASSOCIATED) && (action == StaInfoDbActionAddEntry))
			printk("wlFwSetNewStn pStaInfo->MultiAP_4addr = %d\n", pStaInfo->MultiAP_4addr);
		if ((pStaInfo->MultiAP_4addr == 1) && mib->multi_ap_attr)
			wds = 4;
	}
	...
	if (wfa_11ax_pf) {
		...
		if (ptf_arg->tf.common.tf_num_users > 0) {
			...
			if ((entries < last_stacnt) && (entries < ptf_arg->tf.common.tf_num_users)) {
				...
				wlFwSentTriggerFrameCmd(dev, 0, ptf_arg->type, ptf_arg->rate_info,
							ptf_arg->period, ptf_arg->pad_num, (void *)(&ptf_arg->tf));
...
int wlFwSetEbfMaxSs(struct net_device *dev, u_int8_t * staaddr, u_int8_t ebf_max_ss)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
	HostCmd_FW_SET_EBF_MAX_SS *pCmd = (HostCmd_FW_SET_EBF_MAX_SS *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;

	MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);

	memset(pCmd, 0x00, sizeof(HostCmd_FW_SET_EBF_MAX_SS));
	pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_SET_EBF_MAX_SS);
	pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_SET_EBF_MAX_SS));
	pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
	pCmd->CmdHdr.macid = wlpptr->vmacSta_p->VMacEntry.macId;
	pCmd->ebf_max_ss = ebf_max_ss;
	memcpy(&pCmd->MacAddr[0], staaddr, 6);
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_SET_EBF_MAX_SS));
	retval = wlexecuteCommand(dev, HostCmd_CMD_SET_EBF_MAX_SS);
...
int wlFwSetApMode(struct net_device *netdev, u_int8_t ApMode)
{
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_SET_APMODE));
...
int wlFwSetAPUpdateTim(struct net_device *netdev, u_int16_t assocId, Bool_e set)
{
	...
	WLDBG_ENTER_INFO(DBG_LEVEL_0, "association ID: %i %s", assocId, (set == WL_TRUE) ? "update" : "noact");
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_SSID_BROADCAST *pCmd = (HostCmd_SSID_BROADCAST *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;

	WLDBG_ENTER_INFO(DBG_LEVEL_0, "AP SSID broadcast %s", (facility == WL_ENABLE) ? "enable" : "disable");
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_SSID_BROADCAST));
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_FW_SetWMMMode *pCmd = (HostCmd_FW_SetWMMMode *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_FW_SET_G_PROTECT_FLAG *pCmd = (HostCmd_FW_SET_G_PROTECT_FLAG *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_SET_G_PROTECT_FLAG));
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_FW_SET_EDCA_PARAMS *pCmd = (HostCmd_FW_SET_EDCA_PARAMS *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_FW_SET_MAX_DELAY_BY_AC *pCmd = (HostCmd_FW_SET_MAX_DELAY_BY_AC *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
	...
	pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_SET_MAX_DELAY_BY_AC));
	pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
	pCmd->CmdHdr.macid = wlpptr->vmacSta_p->VMacEntry.macId;
	pCmd->action = action;
	pCmd->ac = ac;
...
void BFMRconfigVHT_HT(struct wlprivate *wlpptr, UINT8 * IeListHT, UINT8 * IeListVHT)
{
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;

	if (vmacSta_p->BFMRinitDone) {
		if (IeListHT && memcmp(vmacSta_p->BFMRconfig.ht_cap, IeListHT, 28)) {
			memcpy(vmacSta_p->BFMRconfig.ht_cap, IeListHT, 28);
			vmacSta_p->bBFMRconfigChanged = TRUE;
		}
		if (IeListVHT && memcmp(vmacSta_p->BFMRconfig.vht_cap_data, IeListVHT + 2, 12)) {
			memcpy(vmacSta_p->BFMRconfig.vht_cap_data, IeListVHT + 2, 12);
			vmacSta_p->bBFMRconfigChanged = TRUE;
		}
...
#ifdef CB_SUPPORT
extern UINT16 Build_IE_cb(vmacApInfo_t * vmacSta_p, UINT8 * pBcnBuf);
#endif				// CB_SUPPORT
int wlFwSetIEs(struct net_device *netdev)
{
	...
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	MIB_802DOT11 *mib_s = vmacSta_p->ShadowMib802dot11, *mib = vmacSta_p->ShadowMib802dot11;
	MIB_PHY_DSSS_TABLE *PhyDSSSTable = mib->PhyDSSSTable;
	HostCmd_FW_SetIEs *setFwIes = (HostCmd_FW_SetIEs *) & wlpptr->pCmdBuf[0];
	...
	if (*(vmacSta_p->ShadowMib802dot11->mib_HT40MIntoler)) {
		if (PhyDSSSTable->Chanflag.FreqBand == FREQ_BAND_2DOT4GHZ &&
			...
			if (PhyDSSSTable->Chanflag.ExtChnlOffset == EXT_CH_ABOVE_CTRL_CH)
				wlpptr->wlpd_p->BcnAddHtAddChannel = 5;
			else if (PhyDSSSTable->Chanflag.ExtChnlOffset == EXT_CH_BELOW_CTRL_CH)
				wlpptr->wlpd_p->BcnAddHtAddChannel = 7;
			else
				wlpptr->wlpd_p->BcnAddHtAddChannel = 0;
			if (wlpptr->wlpd_p->BcnAddHtAddChannel)
				*(mib->USER_ChnlWidth) = 1;
		}
	}
	printk("%s, BcnAddHtAddChanel: %u\n", netdev->name, wlpptr->wlpd_p->BcnAddHtAddChannel);
	/***************************************************************************************************/
	...
	if ((*(mib->mib_mbssid) == 1) && (mbssidGID < MAX_MBSSID_SET) && (*(mib->mib_ApMode) & AP_MODE_11AX) && (vmacSta_p->VMacEntry.macId == wlpptr->wlpd_p->mbssSet[mbssidGID].primbss)) {	//primary bssid
		...
		u32 idx = 0;
		struct wlprivate *wlp = NETDEV_PRIV_P(struct wlprivate, wlpptr->master);

		while (bitmap) {

			if (bitmap & 0x1) {
				if (idx != wlpptr->wlpd_p->mbssSet[mbssidGID].primbss) {
					struct wlprivate *wlp_ntxp = NETDEV_PRIV_P(struct wlprivate, wlp->vdev[idx]);
					vmacApInfo_t *vmacSta_ntxp = wlp_ntxp->vmacSta_p;

					//vmacSta_ntxp = wlp_ntxp->vmacSta_p;
					ie_len = Add_Mbssid_IE(vmacSta_ntxp, (IEEEtypes_Mbssid_Element_t *) tail);
					tail += ie_len;
					//printk("ie_len:%u\n",ie_len);
				}
			}
	...
	if (*(mib_s->QoSOptImpl)) {
		...
		ht_cap = tail;
		if (vmacSta_p->wtp_info.WTP_enabled && vmacSta_p->wtp_info.extHtIE == true) {
			memcpy(tail, vmacSta_p->wtp_info.HTCapIE, vmacSta_p->wtp_info.HTCapIE[1] + 2);
			tail += vmacSta_p->wtp_info.HTCapIE[1] + 2;
			memcpy(tail, vmacSta_p->wtp_info.addHTIE, vmacSta_p->wtp_info.addHTIE[1] + 2);
			tail += (vmacSta_p->wtp_info.addHTIE[1] + 2);
		} else {
			ie_len = AddHT_IE(wlpptr->vmacSta_p, (IEEEtypes_HT_Element_t *) tail);
			tail += ie_len;
			ie_len = AddAddHT_IE(wlpptr->vmacSta_p, (IEEEtypes_Add_HT_Element_t *) tail);
			tail += ie_len;
		}

		ie_len = AddChanReport_IE(wlpptr->vmacSta_p, (IEEEtypes_ChannelReportEL_t *) tail);
		tail += ie_len;

#ifdef COEXIST_20_40_SUPPORT
		if (*(mib_s->mib_HT40MIntoler) &&
		    !((*(mib_s->mib_ApMode) & AP_MODE_BAND_MASK) >= AP_MODE_A_ONLY || (*(mib_s->mib_ApMode) & AP_MODE_BAND_MASK) <= AP_MODE_MIXED)) {
			ie_len = AddOverlap_BSS_Scan_Parameters_IE(wlpptr->vmacSta_p, (IEEEtypes_OVERLAP_BSS_SCAN_PARAMETERS_Element_t *) tail);
			tail += ie_len;
			ie_len = AddExtended_Cap_IE(wlpptr->vmacSta_p, (IEEEtypes_Extended_Cap_Element_t *) tail);
			tail += ie_len;
			addedExtcap = TRUE;
		}
#endif
		/*Always add Extended Cap in beacon and probe rsp */
		if (!addedExtcap) {
			addedExtcap = TRUE;
			ie_len = AddExtended_Cap_IE(wlpptr->vmacSta_p, (IEEEtypes_Extended_Cap_Element_t *) tail);
			tail += ie_len;
		...
		/* shrink EXT Cap IE length to 8 for non-11AX mode */
		if (addedExtcap && !(*(mib->mib_ApMode) & AP_MODE_11AX)) {
			IEEEtypes_Extended_Cap_Element_t *ext_cap_ie = (IEEEtypes_Extended_Cap_Element_t *) (tail - ie_len);
			if (ext_cap_ie->Len == 11) {
				ext_cap_ie->Len = 8;
				tail -= (11 - 8);
				ie_len -= (11 - 8);
			}
		...
		if (*(mib_s->mib_ApMode) & AP_MODE_11AC) {
			vht_cap = tail;
			if (vmacSta_p->wtp_info.WTP_enabled && vmacSta_p->wtp_info.extVhtIE == true) {
				memcpy(tail, vmacSta_p->wtp_info.vhtCapIE, vmacSta_p->wtp_info.vhtCapIE[1] + 2);
				tail += (vmacSta_p->wtp_info.vhtCapIE[1] + 2);
				memcpy(tail, vmacSta_p->wtp_info.vhtInfoIE, vmacSta_p->wtp_info.vhtInfoIE[1] + 2);
				tail += (vmacSta_p->wtp_info.vhtInfoIE[1] + 2);
			} else {
				ie_len = Build_IE_191(wlpptr->vmacSta_p, tail, FALSE, 0);
				tail += ie_len;
		...
		if (*(mib->mib_ApMode) & AP_MODE_11AX) {
			if (vmacSta_p->wtp_info.WTP_enabled && vmacSta_p->wtp_info.extHeIE == true) {
				memcpy(tail, vmacSta_p->wtp_info.heCapIe, vmacSta_p->wtp_info.heCapIe[1] + 2);
				tail += (vmacSta_p->wtp_info.heCapIe[1] + 2);
				memcpy(tail, vmacSta_p->wtp_info.heOpIe, vmacSta_p->wtp_info.heOpIe[1] + 2);
				tail += (vmacSta_p->wtp_info.heOpIe[1] + 2);
			} else {
				ie_len = Build_IE_HE_CAP(wlpptr->vmacSta_p, tail);
				tail += ie_len;
				ie_len = Build_IE_HE_OP(wlpptr->vmacSta_p, tail, 1);
				tail += ie_len;
			}
...
#ifdef CB_SUPPORT
		ie_len = Build_IE_cb(wlpptr->vmacSta_p, tail);
		tail += ie_len;
#endif				// CB_SUPPORT
	...
	if (*(mib_s->mib_RptrMode)) {
		ie_len = AddM_Rptr_IE(wlpptr->vmacSta_p, (IEEEtypes_HT_Element_t *) tail);
		tail += ie_len;
	}
	//add Vendor Specific VHT IE
	if ((*(mib->mib_ApMode) & AP_MODE_11AX) || (*(mib->mib_ApMode) & AP_MODE_11AC)) {
		ie_len = Add_VendorSpec_VHT_IE(wlpptr->vmacSta_p, (IEEEtypes_HT_Element_t *) tail);
		tail += ie_len;
	}
#ifdef MULTI_AP_SUPPORT
	if (mib->multi_ap_attr) {
		//add Multi-AP IE
		ie_len = Add_MultiAP_IE(wlpptr->vmacSta_p, (IEEEtypes_InfoElementHdr_t *) tail, WL_WLAN_TYPE_AP);
		tail += ie_len;
	}
#endif				/*MULTI_AP_SUPPORT */

	if (vmacSta_p->wtp_info.WTP_enabled && vmacSta_p->wtp_info.extPropIE == true) {
		memcpy(tail, vmacSta_p->wtp_info.propIE, vmacSta_p->wtp_info.propIE[1] + 2);
		tail += (vmacSta_p->wtp_info.propIE[1] + 2);
	}
#ifdef IEEE80211K
	ie_len = AddRRM_Cap_IE(wlpptr->vmacSta_p, (IEEEtypes_RM_Enable_Capabilities_Element_t *) tail);
	tail += ie_len;
#endif				/* IEEE80211K */

#ifdef MRVL_80211R
	if (mib_s->RSNConfigWPA2->WPA2Enabled || mib_s->RSNConfigWPA2->WPA2OnlyEnabled) {
	...
	if ((vmacSta_p->RsnIESetByHost == 1) && (vmacSta_p->RSNXIE[1] != 0)) {
		memcpy(tail, vmacSta_p->RSNXIE, vmacSta_p->RSNXIE[1] + 2);
		tail += vmacSta_p->RSNXIE[1] + 2;
	}
	...
	if ((vmacSta_p->WPSOn || vmacSta_p->Mib802dot11->mib_mbo_enabled) &&
	    (tail - setFwIes->beacon_buf + vmacSta_p->thisprobeRespIEs.Len) <= MAX_BEACON_SIZE) {
		memcpy(tail, &vmacSta_p->thisprobeRespIEs.WSCData[0], vmacSta_p->thisprobeRespIEs.Len);
		tail += vmacSta_p->thisprobeRespIEs.Len;
	}
#endif
	macMgmtMlme_UpdateProbeRspExtraIes(vmacSta_p, setFwIes->beacon_buf, tail - setFwIes->beacon_buf);
	...
	if (*(mib->QoSOptImpl)) {
		...
		if (vmacSta_p->wtp_info.WTP_enabled && vmacSta_p->wtp_info.extHtIE == true) {
			memcpy(&(pCmd->IeListHT[retval]), vmacSta_p->wtp_info.HTCapIE, vmacSta_p->wtp_info.HTCapIE[1] + 2);
			retval = (pCmd->IeListLenHT += vmacSta_p->wtp_info.HTCapIE[1] + 2);
			memcpy(&(pCmd->IeListHT[retval]), vmacSta_p->wtp_info.addHTIE, vmacSta_p->wtp_info.addHTIE[1] + 2);
			retval = (pCmd->IeListLenHT += vmacSta_p->wtp_info.addHTIE[1] + 2);
		} else {
...
//#endif
		...
		if (*(vmacSta_p->ShadowMib802dot11->mib_HT40MIntoler)
...
#if 0				//optional IE, not require at this time
			{
				...
				pCmd->IeListLenHT +=
				    Add20_40_Coexist_IE(wlpptr->vmacSta_p, (IEEEtypes_20_40_BSS_COEXIST_Element_t *) & (pCmd->IeListHT[retval]));
				retval = pCmd->IeListLenHT;
			}
			{
				extern UINT16 Add20_40Interant_Channel_Report_IE(vmacApInfo_t * vmacSta_p,
										 IEEEtypes_20_40_INTOLERANT_CHANNEL_REPORT_Element_t * pNextElement);

				pCmd->IeListLenHT +=
				    Add20_40Interant_Channel_Report_IE(wlpptr->vmacSta_p,
								       (IEEEtypes_20_40_INTOLERANT_CHANNEL_REPORT_Element_t *) & (pCmd->
																  IeListHT[retval]));
				retval = pCmd->IeListLenHT;
			}
#endif
			{
				extern UINT16 AddOverlap_BSS_Scan_Parameters_IE(vmacApInfo_t * vmacSta_p,
										IEEEtypes_OVERLAP_BSS_SCAN_PARAMETERS_Element_t * pNextElement);

				pCmd->IeListLenHT +=
				    AddOverlap_BSS_Scan_Parameters_IE(wlpptr->vmacSta_p,
								      (IEEEtypes_OVERLAP_BSS_SCAN_PARAMETERS_Element_t *) & (pCmd->IeListHT[retval]));
				retval = pCmd->IeListLenHT;
			...
			}
			{
				...
				pCmd->IeListLenHT +=
				    AddExtended_Cap_IE(wlpptr->vmacSta_p, (IEEEtypes_Extended_Cap_Element_t *) & (pCmd->IeListHT[retval]));
				retval = pCmd->IeListLenHT;
			}
		}
		...
		if (*(vmacSta_p->Mib802dot11->mib_ApMode) >= AP_MODE_5GHZ_11AC_ONLY) {
			extern UINT16 AddExtended_Cap_IE(vmacApInfo_t * vmacSta_p, IEEEtypes_Extended_Cap_Element_t * pNextElement);
#ifdef WNM
			IEEEtypes_Extended_Cap_Element_t *pEC = FindIEWithinIEs(vmacSta_p->thisbeaconIEs.WSCData,
										vmacSta_p->thisbeaconIEs.Len, EXT_CAP_IE, NULL);
		...
		if (*(mib->mib_ApMode) & AP_MODE_11AC) {
			if (vmacSta_p->wtp_info.WTP_enabled && vmacSta_p->wtp_info.extVhtIE == true) {
				memcpy(&(pCmd->IeListVHT[retvalVHT]), vmacSta_p->wtp_info.vhtCapIE, vmacSta_p->wtp_info.vhtCapIE[1] + 2);
				retvalVHT = (pCmd->IeListLenVHT += vmacSta_p->wtp_info.vhtCapIE[1] + 2);
				memcpy(&(pCmd->IeListVHT[retvalVHT]), vmacSta_p->wtp_info.vhtInfoIE, vmacSta_p->wtp_info.vhtInfoIE[1] + 2);
				retvalVHT = (pCmd->IeListLenVHT += vmacSta_p->wtp_info.vhtInfoIE[1] + 2);
			} else {
				pCmd->IeListLenVHT += Build_IE_191(wlpptr->vmacSta_p, (UINT8 *) & (pCmd->IeListVHT[retvalVHT]), FALSE, 0);
		...
		pCmd->IeListLenProprietary +=
		    Add_Generic_HT_IE(wlpptr->vmacSta_p, (IEEEtypes_Generic_HT_Element_t *) & (pCmd->IeListProprietary[retvalProprietary]));
		retvalProprietary = pCmd->IeListLenProprietary;
		pCmd->IeListLenProprietary +=
		    Add_Generic_AddHT_IE(wlpptr->vmacSta_p, (IEEEtypes_Generic_Add_HT_Element_t *) & (pCmd->IeListProprietary[retvalProprietary]));
		/** For I_COMP only **/
		retvalProprietary = pCmd->IeListLenProprietary;
#endif
...

	{
		...
		if (*(mib->mib_RptrMode)) {
			pCmd->IeListLenProprietary +=
			    AddM_Rptr_IE(wlpptr->vmacSta_p, (IEEEtypes_HT_Element_t *) & (pCmd->IeListProprietary[retvalProprietary]));
			retvalProprietary = pCmd->IeListLenProprietary;
		}
	}
#ifdef MULTI_AP_SUPPORT
	if (mib->multi_ap_attr) {
		//add Multi-AP IE
		pCmd->IeListLenProprietary +=
		    Add_MultiAP_IE(wlpptr->vmacSta_p, (IEEEtypes_InfoElementHdr_t *) & (pCmd->IeListProprietary[retvalProprietary]), WL_WLAN_TYPE_AP);
		retvalProprietary = pCmd->IeListLenProprietary;
	}
#endif				/*MULTI_AP_SUPPORT */

	if (vmacSta_p->wtp_info.WTP_enabled && vmacSta_p->wtp_info.extPropIE == true) {
		memcpy(&(pCmd->IeListProprietary[retvalProprietary]), vmacSta_p->wtp_info.propIE, vmacSta_p->wtp_info.propIE[1] + 2);
		retvalProprietary = (pCmd->IeListLenProprietary += vmacSta_p->wtp_info.propIE[1] + 2);
	}
#ifdef MRVL_80211R
	if (mib->RSNConfigWPA2->WPA2Enabled || mib->RSNConfigWPA2->WPA2OnlyEnabled) {
		if (vmacSta_p->MDIE[1] != 0) {
			memcpy(&(pCmd->IeListProprietary[retvalProprietary]), vmacSta_p->MDIE, 5);
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_SET_POWERSAVESTATION *pCmd = (HostCmd_SET_POWERSAVESTATION *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_SET_POWERSAVESTATION));
...
extern int sprintf_hex_dump(u8 * pdest, u8 * psrc, UINT32 maxlen, UINT32 maxbuflen);
...
int wlexecuteCommand(struct net_device *netdev, unsigned short cmd)
{
	...
	if ((wlpd_p->smon.exceptionAbortCmdExec == 1) || ((wlpd_p->bfwreset == TRUE) &&
							  (cmd != HostCmd_CMD_GET_HW_SPEC) && (cmd != HostCmd_CMD_SET_HW_SPEC))) {
...
#endif
	{
		...
		WLDBG_FWCMD(DBG_LEVEL_1, "%s:FWcmd:0x%04x, CpuID:%u, PID:%i, ProcName:\"%s\"\n",
		...
		if (wlpd_p->smon.active && wlpd_p->smon.ready) {
			size = (UINT32) sprintf(&logbuf[0], "[%llu.%llu]: %s     0x%04x    %u       %i    %s    ", tsec, tms,
						netdev->name, cmd, smp_processor_id(), current->pid, current->comm);
			wlmon_log_buffer(netdev, logbuf, size);
			//copy max the first 512 bytes of the cmd 
			dbglen = ((pCmdHdr->Length > HM_CMDBUF_SIZE) ? HM_CMDBUF_SIZE : pCmdHdr->Length);
			memcpy((UINT8 *) (wlpd_p->smon.pLastCmdBuf), (UINT8 *) pCmdHdr, dbglen);
		}
#endif
		...
		if (wlwaitForComplete(netdev, 0x8000 | cmd)) {
			...
			if (wlpd_p->smon.active && wlpd_p->smon.ready) {
				u16 tmpcmd = (u16) (*(u16 *) (wlpd_p->smon.pLastCmdBuf));

				//printk("Cmd buffer :cmd:0x%04x, cmd in buf:0x%04x\n", cmd, tmpcmd);

				size = (UINT32) sprintf(&logbuf[0], "[Alarm]:FWcmd:0x%04x timeout\nFWdmd dump(the first 512B max):\n", cmd);
				wlmon_log_buffer(netdev, logbuf, size);
				...
				mwl_hex_dump((UINT8 *) (wlpd_p->smon.pLastCmdBuf), ((dbglen > 64) ? 64 : dbglen));

				size = (UINT32) sprintf_hex_dump(&logbuf[0], (UINT8 *) (wlpd_p->smon.pLastCmdBuf), dbglen, HM_CMDBUF_SIZE * 4);
		...
		if (wlpd_p->smon.active && wlpd_p->smon.ready) {
			...
			if (tdif > HM_CMD_RESP_THRESHOLD) {
				size = (UINT32) sprintf(&logbuf[0], "%u [Alarm]:long latency\n", tdif);
			} else {
...
static void wlsendCommand(struct net_device *netdev)
{
	...
	writel(MACREG_H2ARIC_BIT_DOOR_BELL, wlpptr->ioBase1 + reg_h2a_int_events);
#else
...
static UINT8 *getCmdRspErrorStr(UINT16 reason)
{
...
static int wlwaitForComplete(struct net_device *netdev, u_int16_t cmdCode)
{
	...
	if (currIteration == 0) {
		...
		printk("cmd timeout debug\n");
		print_hex_dump(KERN_INFO, "", DUMP_PREFIX_OFFSET, 16, 4, &((SMAC_CTRL_BLK_st *) wlpptr->ioBase0)->sfw, sizeof(SMAC_SFW_st), false);
		...
		if (result) {
			...
			WLDBG_FWCMD(DBG_LEVEL_1, "%s: cmd 0x%04x response seq# %d error code %d reason: %s\n", wlgetDrvName(netdev),
				    intCode, pCmdHdr->SeqNum, result, getCmdRspErrorStr(result));
			WLDBG_FWCMD_HEXDUMP(DBG_LEVEL_1, (void *)&wlpptr->pCmdBuf[0], length);
		}
	}
#endif
#endif				/* #ifdef SOC_W906X */
...
u_int8_t GetSecondChannel(u_int8_t Channel, u_int8_t bw)
{
	int i;
	...
	if (bw == CH_160_MHz_WIDTH) {
		for (i = 0; i < domainGetSizeOfGrpChList160Mhz() / sizeof(GRP_CHANNEL_LIST_160Mhz); i++) {
			if (channel_exists(Channel, GrpChList160Mhz[i].channelEntry, 8)) {
				if (Channel < GrpChList160Mhz[i].channelEntry[4])
					return GrpChList160Mhz[i].channelEntry[4];
				else
					return GrpChList160Mhz[i].channelEntry[0];
			}
		}
	} else {
		for (i = 0; i < domainGetSizeOfGrpChList80Mhz() / sizeof(GRP_CHANNEL_LIST_80Mhz); i++) {
			if (channel_exists(Channel, GrpChList80Mhz[i].channelEntry, 4) == FALSE) {
...
#ifdef SOC_W906X
	if (PhyDSSSTable->SecChan != 0 && PhyDSSSTable->Chanflag.radiomode == RADIO_MODE_7x7p1x1) {
		/* swap primary and 2nd channel according to fw design */
		CHNL_FLAGS chanflag_swp;
		memcpy(&chanflag_swp, &PhyDSSSTable->Chanflag, sizeof(CHNL_FLAGS));

		chanflag_swp.FreqBand = PhyDSSSTable->Chanflag.FreqBand2;
		chanflag_swp.FreqBand2 = PhyDSSSTable->Chanflag.FreqBand;
		chanflag_swp.ChnlWidth = PhyDSSSTable->Chanflag.ChnlWidth2;
		chanflag_swp.ChnlWidth2 = PhyDSSSTable->Chanflag.ChnlWidth;

		retval = wlchannelSet(netdev, PhyDSSSTable->SecChan, PhyDSSSTable->CurrChan, &chanflag_swp, 1);
	} else {
		retval = wlchannelSet(netdev, PhyDSSSTable->CurrChan, PhyDSSSTable->SecChan, &PhyDSSSTable->Chanflag, 1);
	}
	if (retval != SUCCESS) {
#else
...
int wlFwApplySettings(struct net_device *netdev)
{
	 ...
	 */
#ifdef SOC_W906X
	if ((PhyDSSSTable->CurrChan != PhyDSSSTable1->CurrChan) || (PhyDSSSTable->SecChan != PhyDSSSTable1->SecChan)) {
		bChannelChanged = TRUE;
		wlpptr->wlpd_p->privNdevStats.ch_start_time_sec = ktime_get_seconds();
		wlpptr->wlpd_p->privNdevStats.ch_change_count++;
	}
	if ((PhyDSSSTable->Chanflag.ChnlWidth != PhyDSSSTable1->Chanflag.ChnlWidth) ||
	    (PhyDSSSTable->Chanflag.ChnlWidth2 != PhyDSSSTable1->Chanflag.ChnlWidth2)) {
	...
	if ((*mib1->mib_autochannel == 1) && newDFSMode && !vmacSta_p->dfsCacExp) {
...
#ifdef SOC_W906X
	if ((PhyDSSSTable1->Chanflag.ChnlWidth == CH_160_MHz_WIDTH) && (PhyDSSSTable1->SecChan == 0)) {
		PhyDSSSTable1->Chanflag.radiomode = RADIO_MODE_80p80;
		PhyDSSSTable1->Chanflag.ChnlWidth2 = CH_80_MHz_WIDTH;
		PhyDSSSTable1->SecChan = GetSecondChannel(PhyDSSSTable1->CurrChan, CH_160_MHz_WIDTH);
		PhyDSSSTable1->Chanflag.FreqBand2 = PhyDSSSTable->Chanflag.FreqBand;

		if (PhyDSSSTable1->SecChan == 0) {
			PhyDSSSTable1->Chanflag.ChnlWidth = CH_80_MHz_WIDTH;
			PhyDSSSTable1->SecChan = GetSecondChannel(PhyDSSSTable->CurrChan, CH_80_MHz_WIDTH);

			printk("Can't find available secChan, change to 80+80 chan=%d secChan=%d bw=%d bw2=%d\n",
			       PhyDSSSTable1->CurrChan, PhyDSSSTable1->SecChan,
			       PhyDSSSTable1->Chanflag.ChnlWidth, PhyDSSSTable1->Chanflag.ChnlWidth2);
		}
	}
#endif
	...
	if (wlpptr->wlpd_p->ext_scnr_en && wlpptr->wlpd_p->pdfsApMain) {
		...
		mib->PhyDSSSTable->Chanflag.FreqBand2 = mib->PhyDSSSTable->Chanflag.FreqBand;
		mib->PhyDSSSTable->Chanflag.ChnlWidth2 = mib->PhyDSSSTable->Chanflag.ChnlWidth;
		...
		if (bBandWidthChanged)
			dfsDesc_p->CtlChanInfo.chanflag.ChnlWidth = dfsDesc_p->currChanInfo.chanflag.ChnlWidth =
			    mib->PhyDSSSTable->Chanflag.ChnlWidth;
#endif
		if (UpdateCurrentChannelInMIB(vmacSta_p, mib->PhyDSSSTable->CurrChan)) {
			mib_Update();

			if (wlchannelSet
			    (netdev, dfsDesc_p->CtlChanInfo.channel, dfsDesc_p->currChanInfo.channel, &dfsDesc_p->CtlChanInfo.chanflag, 1)) {
				WLDBG_EXIT_INFO(DBG_LEVEL_0, "setting channel");
				retval = FAIL;
			}
		}
...
#endif				/* CONCURRENT_DFS_SUPPORT */
	{
		if (PhyDSSSTable->SecChan != 0 && PhyDSSSTable->Chanflag.radiomode == RADIO_MODE_7x7p1x1) {
			...
			retval = wlchannelSet(netdev, PhyDSSSTable->SecChan, PhyDSSSTable->CurrChan, &chanflag_swp, 1);
		} else if (UpdateCurrentChannelInMIB(vmacSta_p, mib->PhyDSSSTable->CurrChan)) {
			mib_Update();
			retval = wlchannelSet(netdev, PhyDSSSTable->CurrChan, PhyDSSSTable->SecChan, &PhyDSSSTable->Chanflag, 1);
		}
		if (retval != SUCCESS) {
	...
	if ((*(vmacSta_p->Mib802dot11->mib_ApMode) & AP_MODE_BAND_MASK) < AP_MODE_A_ONLY)
		wlFwSet11N_20_40_Switch(vmacSta_p->dev, ((PhyDSSSTable1->Chanflag.ChnlWidth == CH_20_MHz_WIDTH) ? 0 : 1));
	...
	if (dfsDesc_p && dfsDesc_p->cac_complete == 1) {
		...
		if (DecideDFSOperation(netdev, bChannelChanged, bBandWidthChanged, currDFSMode, newDFSMode, mib)) {
	...
	if (model && strnstr(model, "G3", len)) {
		...
		struct wlprivate *wlp = NETDEV_PRIV_P(struct wlprivate, wlpptr->wlpd_p->rootdev);
	...
	if (mib->rxsop_ed_threshold1 || mib->rxsop_ed_threshold2) {
		if (wlFwNewDP_RxSOP(netdev, 2, mib->rxsop_ed_threshold1, mib->rxsop_ed_threshold2) == FAIL) {
			WLDBG_WARNING(DBG_LEVEL_0, "set RxSOP EDMAC fail");
			retval = FAIL;
		}
	}
	if (mib->rxsop_cck_threshold1) {
		if (wlFwNewDP_RxSOP(netdev, 4, mib->rxsop_cck_threshold1, 0) == FAIL) {
	...
	if ((mib_SpectrumMagament_p->spectrumManagement && (PhyDSSSTable->Chanflag.FreqBand == FREQ_BAND_5GHZ))
	    ) {
	...
	if (mib->StationConfig->bss_color == 0) {
		vmacSta_p->bss_color = prandom_u32() % 63;
	}
#endif

	/* AIRTIME_FAIRNESS */
	// Copy the whole atf information to firmware
	// including enable flag
	if (wlFwAtfCfgSet(netdev, mib->mib_atf_info)) {
		WLDBG_EXIT_INFO(DBG_LEVEL_0, "Set ATF config fail");
		retval = FAIL;
	}
...
int wlFwMultiBssApplySettings(struct net_device *netdev)
{
	...
	if ((*(mib->mib_autochannel) == 0) && Is2GNeedCoexScan(vmacSta_p->dev))
		scan_20_40_coex = 1;

	if (vmacSta_p->master &&
	    (((vmacSta_p->master->preautochannelfinished == 0) && (*(mib->mib_autochannel) != 0)) ||
	     (vmacSta_p->master->busyScanning && scan_20_40_coex))) {
		WLDBG_ERROR(DBG_LEVEL_0, "Don't enable AP bss when enable autochannel and in scanning");
		return FAIL;
	}

	if (wlpptr->wlpd_p->repeaterUpdateChannelWidth) {
		vmacSta_p->ShadowMib802dot11->PhyDSSSTable->Chanflag.ChnlWidth = wlpptr->wlpd_p->repeaterUpdateChannelWidth;
	}
#ifdef MRVL_DFS
...
#ifdef IEEE80211K
	if (vmacSta_p->master) {
		*(vmacSta_p->ShadowMib802dot11->mib_rrm) = *(vmacSta_p->master->Mib802dot11->mib_rrm);
		*(vmacSta_p->ShadowMib802dot11->mib_quiet) = *(vmacSta_p->master->Mib802dot11->mib_quiet);
	}
#endif				/* IEEE80211K */
	...
	/* power constraint ino IE should appear in beacon only on 5GHz */
	if (((mib_SpectrumMagament_p->spectrumManagement) && (Is5GBand(*(mib->mib_ApMode)) || (*(mib->mib_ApMode) == AP_MODE_N_ONLY)))
#ifdef IEEE80211K
	    || *(mib->mib_rrm)
#endif
	    ) {
	...
	if (combIE == NULL) {
		...
		if (vmacSta_p->thisbeaconIEs.Len != 0) {
			memcpy(&combIE->beaconIE, &vmacSta_p->thisbeaconIEs, sizeof(WSC_BeaconIEs_t));
		...
		if (vmacSta_p->thisprobeRespIEs.Len != 0) {
			memcpy(&combIE->probeRespIE, &vmacSta_p->thisprobeRespIEs, sizeof(WSC_ProbeRespIEs_t));
...
#ifdef SOC_W906X
	wlFwSetSecurityKey(netdev, ACT_DEL, 0, vmacSta_p->macStaAddr, 0, 0, 0, NULL);
#else
...
#endif
int wlFwGetAddrValue(struct net_device *netdev, UINT32 addr, UINT32 startIdx, UINT32 * val, UINT16 set)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_DS_MEM_ADDR_ACCESS *pCmd = (HostCmd_DS_MEM_ADDR_ACCESS *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
	UINT32 *dst = NULL;
	...
	if (set == 3) {
		dst = wl_dma_alloc_coherent(wlpptr->wlpd_p->dev, startIdx * sizeof(u32), &dst_phys_addr, wlpptr->wlpd_p->dma_alloc_flags);
		if (!dst)
			return retval;
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_DS_MEM_ADDR_ACCESS));
	retval = wlexecuteCommand(netdev, HostCmd_CMD_MEM_ADDR_ACCESS);
	if (!retval) {
		if (set == 3) {
			memcpy((void *)val, (void *)dst, startIdx * sizeof(u32));
			wl_dma_free_coherent(wlpptr->wlpd_p->dev, startIdx * sizeof(u32), dst, dst_phys_addr);
		} else
			memcpy((void *)val, (void *)pCmd->Value, sizeof(pCmd->Value));
	}
	MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
	return retval;
}
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_DS_MAC_REG_ACCESS *pCmd = (HostCmd_DS_MAC_REG_ACCESS *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_DS_MAC_REG_ACCESS));
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_DS_RF_REG_ACCESS *pCmd = (HostCmd_DS_RF_REG_ACCESS *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	...
	pCmd->Action = ENDIAN_SWAP16(flag);
	pCmd->Value = *val;
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_DS_RF_REG_ACCESS));
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_DS_BBP_REG_ACCESS *pCmd = (HostCmd_DS_BBP_REG_ACCESS *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
	...
	pCmd->Action = ENDIAN_SWAP16(flag);
	pCmd->Value = *val;
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_DS_BBP_REG_ACCESS));
...
static int wlFwSetMaxTxPwr(struct net_device *netdev)
{
	...
	UINT16 tmp;
	UINT16 chAutoWidth = ((mib->PhyDSSSTable->Chanflag.FreqBand == FREQ_BAND_2DOT4GHZ) ? CH_40_MHz_WIDTH : CH_80_MHz_WIDTH);
...
static int wlFwSetAdaptMode(struct net_device *netdev)
{
	...
	vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
	MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
	HostCmd_DS_SET_RATE_ADAPT_MODE *pCmd = (HostCmd_DS_SET_RATE_ADAPT_MODE *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
...
static int wlFwSetCSAdaptMode(struct net_device *netdev)
{
	...
	MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
	MIB_PHY_DSSS_TABLE *PhyDSSSTable = mib->PhyDSSSTable;
	HostCmd_DS_SET_LINKADAPT_CS_MODE *pCmd = (HostCmd_DS_SET_LINKADAPT_CS_MODE *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_FW_SET_N_PROTECT_FLAG *pCmd = (HostCmd_FW_SET_N_PROTECT_FLAG *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_SET_N_PROTECT_FLAG));
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_FW_SET_N_PROTECT_OPMODE *pCmd = (HostCmd_FW_SET_N_PROTECT_OPMODE *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_SET_N_PROTECT_OPMODE));
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_FW_SET_OPTIMIZATION_LEVEL *pCmd = (HostCmd_FW_SET_OPTIMIZATION_LEVEL *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_SET_OPTIMIZATION_LEVEL));
...
#ifdef SOC_W906X
int wlFwSetAcntStop(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_FW_SET_ACNT_STOP *pCmd = (HostCmd_FW_SET_ACNT_STOP *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_SET_ACNT_STOP));
...
int wlFwGetCalTable(struct net_device *netdev, UINT8 annex, UINT8 index)
{
	...
	HostCmd_FW_GET_CALTABLE *pCmd = (HostCmd_FW_GET_CALTABLE *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_FW_GET_CALTABLE));
...
int wlFwSetMimoPsHt(struct net_device *netdev, UINT8 * addr, UINT8 enable, UINT8 mode)
{
	...
	HostCmd_FW_SET_MIMOPSHT *pCmd = (HostCmd_FW_SET_MIMOPSHT *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_SET_MIMOPSHT));
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_FW_GET_REGION_CODE *pCmd = (HostCmd_FW_GET_REGION_CODE *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
	u32 status;
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_FW_GET_EEPROM_PWR_TBL *pCmd = (HostCmd_FW_GET_EEPROM_PWR_TBL *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
	u32 status;
	...
	pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_GET_EEPROM_PWR_TBL));
	...
	memcpy(EEPROM_CH_PwrTbl, &pCmd->channelPwrTbl, sizeof(channel_power_tbl_t));
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_FW_GET_BEACON *pCmd = (HostCmd_FW_GET_BEACON *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_FW_GET_BEACON));
...
#ifdef SOC_W906X
int wlFwGetRateTable(struct net_device *netdev, UINT8 * addr, UINT8 * pRateInfo, UINT32 size, UINT8 type, UINT16 staid)
#else
int wlFwGetRateTable(struct net_device *netdev, UINT8 * addr, UINT8 * pRateInfo, UINT32 size, UINT8 type)
#endif
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);

	HostCmd_FW_GET_RATETABLE *pCmd = (HostCmd_FW_GET_RATETABLE *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
/* Move outside of FW_cmd func, keep it pure to avoid lock issue */
#if 0
	extStaDb_StaInfo_t *pStaInfo;
	UINT16 index;

	if ((pStaInfo = extStaDb_GetStaInfo(wlpptr->vmacSta_p, (IEEEtypes_MacAddr_t *) addr, STADB_DONT_UPDATE_AGINGTIME)) == NULL) {
		//return retval;
		index = staid;
	} else {
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_FW_GET_RATETABLE));
 ...
 */
#ifdef SOC_W906X
int wlFwSetRateTable(struct net_device *netdev, UINT32 action, UINT8 * addr, UINT16 staid, UINT32 rateinfo)
#else
int wlFwSetRateTable(struct net_device *netdev, UINT32 action, UINT8 * addr, UINT32 rateinfo)
#endif
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);

	HostCmd_FW_SET_RATETABLE *pCmd = (HostCmd_FW_SET_RATETABLE *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
#ifdef SOC_W906X
	extStaDb_StaInfo_t *pStaInfo;
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_FW_SET_RATETABLE));
...
int wlFwMfgCmdIssue(struct net_device *netdev, char *pData, char *pDataOut)
{
	...
	if (!retval) {
		s64 time_elapsed = ktime_to_us(ktime_sub(ktime_get_real(), start_time));
		UINT8 *p_r = (UINT8 *) pCmd + 2;
		UINT16 CmdLen_r = ENDIAN_SWAP16(*(UINT16 *) p_r);

		memcpy((void *)pDataOut, (void *)pCmd, CmdLen_r);
		WLDBG_INFO(DBG_LEVEL_0, "Result from FW, time elapsed %lld us", time_elapsed);
...
extern UINT16 getCacTimerValue(UINT16 regionCode, UINT16 cacTimeout, UINT16 etsiTimeout, CHNL_FLAGS * pChanflag, UINT8 channel, UINT8 channel2);
int DecideDFSOperation(struct net_device *netdev, BOOLEAN bChannelChanged, BOOLEAN bBandWidthChanged,
		       UINT8 currDFSState, UINT8 newDFSState, MIB_802DOT11 * mib)
{
	...
	if (wlpptr->wlpd_p->pdfsApMain) {
		if (currDFSState == DFS_DISABLED) {
			...
			if (newDFSState == DFS_DISABLED) {
				...
				return SUCCESS;
#ifdef CONCURRENT_DFS_SUPPORT
			} else if ((wlpptr->wlpd_p->ext_scnr_en) &&
				   get_dfs_ctl_status(&wlpptr->wlpd_p->pdfsApMain->dfsApDesc, DFS_PATH_DEDICATED) == DFS_STATE_UNKNOWN) {
				DFSCtlInit(netdev);
#endif				/* CONCURRENT_DFS_SUPPORT */
			}
		}
	...
	mib_CACTimeOut = getCacTimerValue(*(mib->mib_regionCode), *(mib->mib_CACTimeOut), *(mib->mib_ETSICACTimeOut), pChanflag, chan, chan2);
	...
	if (bChannelChanged || noChannelChangeCheck || bBandWidthChanged) {
		...
		action = DFSGetCurrentRadarDetectionMode(wlpptr->wlpd_p->pdfsApMain, PhyDSSSTable->CurrChan,
							 PhyDSSSTable->SecChan, &PhyDSSSTable->Chanflag);
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_FW_SET_11N_20_40_SWITCHING *pCmd = (HostCmd_FW_SET_11N_20_40_SWITCHING *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
	...
	wlpptr->wlpd_p->BcnAddHtAddChannel = mode;

	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_SET_11N_20_40_SWITCHING));
		       ...
		       UINT8 bf_slp, UINT8 bf_power)
{
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_HT_BF_MODE));
...
int wlFwSetOfdma_Mode(struct net_device *netdev, UINT8 option, UINT8 ru_mode, UINT32 max_delay, UINT8 max_sta)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_FW_OFDMA_MODE *pCmd = (HostCmd_FW_OFDMA_MODE *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;

	WLDBG_ENTER_INFO(DBG_LEVEL_0, "Set ofdma mode %d %d %d %d", option, ru_mode, max_delay, max_sta);
	...
	WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_OFDMA_MODE));
...
int wlFwSetNoAck(struct net_device *netdev, UINT8 Enable, UINT8 be_enable, UINT8 bk_enable, UINT8 vi_enable, UINT8 vo_enable)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
...
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_FW_BCN_GPIO17_TOGGLE *pCmd = (HostCmd_FW_BCN_GPIO17_TOGGLE *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
	...
	pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_BCN_GPIO17_TOGGLE));
	pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
...
#ifndef SOC_W906X
int wlFwGetPHYBW(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
...
int wlFwSetAlphaTimingFc(struct net_device *netdev, UINT8 Enable, int Fc_Value)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	HostCmd_ALPHA_TIMING_FC *pCmd = (HostCmd_ALPHA_TIMING_FC *) & wlpptr->pCmdBuf[0];
	int retval = FAIL;
	unsigned long flags;
...
#ifdef NEWDP_ACNT_BA
void PrintBAHisto(struct net_device *netdev, UINT8 staid)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	...
	if (!IS_ERR(filp_bahisto)) {
		...
		if (pBA->pBAStats != NULL) {
			...
			data_p += sprintf(data_p, "BA histogram staid:%d, type:%s\n", pBA->Stnid, pBA->Type ? "MU" : "SU");
			...
			for (i = 0; i < ACNT_BA_SIZE && data; i++) {
				...
				if (!pBA->pBAStats[i].NoBA)
					bmap0flag = (pBA->pBAStats[i].BAHole == pBA->pBAStats[i].BAExpected) ? 1 : 0;
				else
					bmap0flag = 0;
				...
				if ((strlen(buff) + 16) >= 500) {
					__kernel_write(filp_bahisto, buff, strlen(buff), &filp_bahisto->f_pos);
					mdelay(2);
					memset(buff, 0, sizeof(buff));
					data_p = buff;
				...
				data_p += sprintf(data_p, "%3d,%3d,", pBA->pBAStats[i].BAHole, pBA->pBAStats[i].BAExpected);
				...
				if (bmap0flag) {
					...
					data_p += sprintf(data_p, "%3d,", bmap0flag);

				if (NoBAflag) {
					data_p += sprintf(data_p, "  *\n");
					NoBAcnt++;
				} else
					data_p += sprintf(data_p, "%3d\n", NoBAflag);
		...
		printk("Total BAhole:%u, BAExpected:%u, BAbitmap0:%u, NoBA:%u\n", (unsigned int)BAholecnt, (unsigned int)BAexpcnt,
		       (unsigned int)bmap0cnt, (unsigned int)NoBAcnt);
...
void PrintTxHisto(struct net_device *netdev, UINT16 indx, UINT8 type)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	int _11G_rate[IEEEtypes_MAX_DATA_RATES_G] = { 1, 2, 5, 11, 22, 6, 9, 12, 18, 24, 36, 48, 54, 72 };
	char *bwStr[4] = { "ht20", "ht40", "ht80", "ht160" };
	char *sgiStr[2] = { "lgi", "sgi" };
#ifdef SOC_W906X
	char *sgi11axStr[5] = { "1x0.8gi", "2x0.8gi", "2x1.6gi", "4x3.2gi", "4x0.8gi" };
#endif
	...
	if (type == SU_MIMO) {
		...
		pTxhisto = &(wlpptr->wlpd_p->txRateHistogram[indx]->custom_rate[0]);
	}
			...
			for (bw = 0; bw < QS_NUM_SUPPORTED_11AC_BW; bw++) {
				for (mcs = 0; mcs < QS_NUM_SUPPORTED_11AC_MCS; mcs++) {
					for (sgi = 0; sgi < QS_NUM_SUPPORTED_GI; sgi++) {
						cnt = ENDIAN_SWAP32(wlpptr->wlpd_p->txRateHistogram[indx]->MU_rate[nss][bw][sgi][mcs].cnt);
						rateinfo = ENDIAN_SWAP32(wlpptr->wlpd_p->txRateHistogram[indx]->MU_rate[nss][bw][sgi][mcs].rateinfo);

						if (cnt && (rateinfo > 0)) {
							total += cnt;
							per4 =
							    ENDIAN_SWAP32(wlpptr->wlpd_p->txRateHistogram[indx]->MU_rate[nss][bw][sgi][mcs].per[4]);
							per3 =
							    ENDIAN_SWAP32(wlpptr->wlpd_p->txRateHistogram[indx]->MU_rate[nss][bw][sgi][mcs].per[3]);
							per2 =
							    ENDIAN_SWAP32(wlpptr->wlpd_p->txRateHistogram[indx]->MU_rate[nss][bw][sgi][mcs].per[2]);
							per1 =
							    ENDIAN_SWAP32(wlpptr->wlpd_p->txRateHistogram[indx]->MU_rate[nss][bw][sgi][mcs].per[1]);
							per0 =
							    ENDIAN_SWAP32(wlpptr->wlpd_p->txRateHistogram[indx]->MU_rate[nss][bw][sgi][mcs].per[0]);

							if (printed == 0) {
								printk("%s %26s  <%2d       >=%2d       >=%2d       >=%2d       >=%2d\n", mode[type],
								       " PER%", TX_HISTO_PER_THRES[0], TX_HISTO_PER_THRES[0], TX_HISTO_PER_THRES[1],
								       TX_HISTO_PER_THRES[2], TX_HISTO_PER_THRES[3]);
							...
							if ((wlpptr->wlpd_p->txRateHistogram[indx]->CurRateInfo[type] & ratemask) ==
							    (rateinfo & ratemask))
								printk("*");	//mark as current rate
							else
								printk(" ");
							printk("%5s_%3s_%1dSS_MCS%2d : %10u, %9u, %9u, %9u, %9u, %9u\n",
							       bwStr[bw], sgiStr[sgi], (nss + 1), mcs, (unsigned int)cnt, (unsigned int)per0,
							       (unsigned int)per1, (unsigned int)per2, (unsigned int)per3, (unsigned int)per4);
		...
		for (j = 0; j < loopcnt; j++) {
			...
			if (cnt && (rateinfo > 0)) {
				...
				if (cnt && (rateinfo > 0)) {
					...
					if (printed == 0) {
						printk("%s %26s  <%2d       >=%2d       >=%2d       >=%2d       >=%2d\n", mode[type], " PER%",
						       TX_HISTO_PER_THRES[0], TX_HISTO_PER_THRES[0], TX_HISTO_PER_THRES[1],
						       TX_HISTO_PER_THRES[2], TX_HISTO_PER_THRES[3]);
					...
					if ((wlpptr->wlpd_p->txRateHistogram[indx]->CurRateInfo[type % SU_MU_TYPE_CNT] & ratemask) ==
					    (rateinfo & ratemask))
						printk("*");	//mark as current rate
					else
						printk(" ");
					if (format == 0) {
						if (mcs == 2)
							printk("5.5Mbps             : %10u, %9u, %9u, %9u, %9u, %9u\n", (unsigned int)cnt,
							       (unsigned int)per0, (unsigned int)per1, (unsigned int)per2, (unsigned int)per3,
							       (unsigned int)per4);
						else if (mcs < IEEEtypes_MAX_DATA_RATES_G)
							printk("%-3dMbps             : %10u, %9u, %9u, %9u, %9u, %9u\n", _11G_rate[mcs],
							       (unsigned int)cnt, (unsigned int)per0, (unsigned int)per1, (unsigned int)per2,
							       (unsigned int)per3, (unsigned int)per4);

					} else if (format == 1) {
						printk("%4s_%3s_MCS%2d	    : %10u, %9u, %9u, %9u, %9u, %9u\n",
						       bwStr[bw], sgiStr[sgi], mcs, (unsigned int)cnt, (unsigned int)per0, (unsigned int)per1,
						       (unsigned int)per2, (unsigned int)per3, (unsigned int)per4);
					} else {
						nss = (mcs >> 4);
						printk("%5s_%3s_%1dSS_MCS%2d : %10u, %9u, %9u, %9u, %9u, %9u\n",
#ifdef SOC_W906X
						       bwStr[bw], (format == 3) ? sgi11axStr[sgi] : sgiStr[sgi], (nss + 1), (mcs & 0xf),
						       (unsigned int)cnt, (unsigned int)per0, (unsigned int)per1, (unsigned int)per2,
						       (unsigned int)per3, (unsigned int)per4);
#else
						       bwStr[bw], sgiStr[sgi], (nss + 1), (mcs & 0xf), (unsigned int)cnt, (unsigned int)per0,
						       (unsigned int)per1, (unsigned int)per2, (unsigned int)per3, (unsigned int)per4);
#endif
...
#ifdef SOC_W906X
	void PrintSchHisto(struct net_device *netdev, UINT16 stnId, char *sysfs_buff) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		int i, j;

		if ((stnId >= sta_num) || (wlpptr->wlpd_p->scheHistogram[stnId] == NULL))
			return;

		for (i = 0; i < 2; i++) {
			Sysfs_Printk("%sAmpdu scheduler stats\n", i ? "" : "Non-");
			Sysfs_Printk("Delay\n          <1m         <10m        <100m        <300m        >300m\n");
			Sysfs_Printk("   %10u   %10u   %10u   %10u   %10u\n", wlpptr->wlpd_p->scheHistogram[stnId]->Delay[i][0]
				     , wlpptr->wlpd_p->scheHistogram[stnId]->Delay[i][1]
				     , wlpptr->wlpd_p->scheHistogram[stnId]->Delay[i][2]
				     , wlpptr->wlpd_p->scheHistogram[stnId]->Delay[i][3]
				     , wlpptr->wlpd_p->scheHistogram[stnId]->Delay[i][4]);
			Sysfs_Printk("\nBytes\n         <1500       <16k           <32k         <64k        <128k        >128k\n");
			Sysfs_Printk("   %10u   %10u   %10u   %10u   %10u   %10u\n\n", wlpptr->wlpd_p->scheHistogram[stnId]->NumBytes[i][0]
				     , wlpptr->wlpd_p->scheHistogram[stnId]->NumBytes[i][1]
				     , wlpptr->wlpd_p->scheHistogram[stnId]->NumBytes[i][2]
				     , wlpptr->wlpd_p->scheHistogram[stnId]->NumBytes[i][3]
				     , wlpptr->wlpd_p->scheHistogram[stnId]->NumBytes[i][4]
				     , wlpptr->wlpd_p->scheHistogram[stnId]->NumBytes[i][5]);
			for (j = 0; j < 65; j++) {
				if (wlpptr->wlpd_p->scheHistogram[stnId]->NumAmpdu[i][j])
					Sysfs_Printk("%sampdu[%2d] -  %10u\n",
						     i ? "" : "Non-", j, wlpptr->wlpd_p->scheHistogram[stnId]->NumAmpdu[i][j]);
			}
			Sysfs_Printk("\n");
		}
	}

	int wlFwGetQueueStats(struct net_device *netdev, int option, UINT8 fromHM, char *sysfs_buff)
#else
	int wlFwGetQueueStats(struct net_device *netdev, int option, char *sysfs_buff)
#endif
	{
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		int i;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_GET_QUEUE_STATS));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		...
		//request from HM
		if (fromHM == 1) {
			wlmon_log_pfw_schInfo(netdev, &(pCmd->QueueStats.qs_u.TxScheInfo));
			MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
			return retval;
		}
#endif
		...
		case QS_GET_TX_SCHEDULER_INFO:
			{
				QS_TX_SCHEDULER_INFO_t *pQS_Sched = &(pCmd->QueueStats.qs_u.TxScheInfo);
#ifdef SOC_W906X
				vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
				...
				Sysfs_Printk("\nPFW scheduler info\n");
				for (i = 0; i < 12; i++) {
					Sysfs_Printk("%s:\t%10u\n", s[i], ENDIAN_SWAP32((int)pQS_Sched->debug_scheduler[i]));
				}
				Sysfs_Printk("\tNumMpdu   \tNumBytes  \tTimeDelay\n");
				for (i = 0; i < 10; i++) {
					Sysfs_Printk("\t%10u\t%10u\t%10u\n", ENDIAN_SWAP32((int)pQS_Sched->debug_scheduler2[i][0]),
						     ENDIAN_SWAP32((int)pQS_Sched->debug_scheduler2[i][1]),
						     ENDIAN_SWAP32((int)pQS_Sched->debug_scheduler2[i][2]));
				}
#ifdef SOC_W906X
				entries = extStaDb_entries(vmacSta_p, 0);
				if (entries) {
					staBuf = wl_kmalloc(entries * sizeof(STA_INFO), GFP_ATOMIC);
					if (staBuf == NULL)
						break;
					...
					if (vmacSta_p->OpMode == WL_OP_MODE_STA || vmacSta_p->OpMode == WL_OP_MODE_VSTA) {
						Sysfs_Printk("\n");
						PrintSchHisto(netdev, 0, sysfs_buff);
					} else
						Sysfs_Printk("\nPer STA scheduler info\nNo STA connected.\n\n");
				}
#endif				/* #ifdef SOC_W906X */
		...
		case QS_GET_TX_COUNTER:
			{
				...
				printk
				    ("QID   Enqueued      TxDone  Total_Retry  QNotReady      QFull  DropNonBa   WrongQid     DropMc   FailHwEnQ\n");
				...
				for (i = 0; i < QS_NUM_STA_SUPPORTED; i++) {
					if (pQS_Counters->SwBAStats[i].SwBaPktEnqueued) {
						...
						memset((void *)&SBLTS, 0x00, sizeof(SBLTS));
						if (wlFwGetAddrValue(netdev, pQS_Counters->SwBAStats[i].pSBLTS,
				...
				for (j = 0; j < QS_NUM_STA_SUPPORTED; j++) {
					...
					printk("%02x:%02x:%02x:%02x:%02x:%02x%s ",
					       ...
					       pQS_Counters->StaCounters[j].addr[5], ba_str[wlCheckBa(netdev, pQS_Counters->StaCounters[j].addr)]);
				...
				for (j = 0; j < 4; j++) {
					if (pQS_Counters->rxStaCounters[j].valid) {
						printk("%02x:%02x:%02x:%02x:%02x:%02x\t",
						       ...
						       pQS_Counters->rxStaCounters[j].addr[4], pQS_Counters->rxStaCounters[j].addr[5]);
						printk("%10u\t%10u\t%10u\t%10u\n",
						       ENDIAN_SWAP32((int)pQS_Counters->rxStaCounters[j].rxPktCounts),
						       rxPktStats_sta[j].RxRecvPollCnt,
						       rxPktStats_sta[j].Rx80211InputCnt, rxPktStats_sta[j].RxfwdCnt);
					} else {
						break;
					}
				}
		...
		case QS_GET_RX_RATE_HIST:
			{
				...
				char *bwStr[4] = { "ht20", "ht40", "ht80", "ht160" };
				char *vhtbwStr[4] = { "vht20", "vht40", "vht80", "vht160" };
				char *sgiStr[2] = { "lgi", "sgi" };

				char *hebwStr[4] = { "he20", "he40", "he80", "he160" };

				{
...
#ifdef RXACNT_REC
					{
						DRV_RATE_HIST *prxRateHistogram = &wlpptr->wlpd_p->drvrxRateHistogram;
					...
					for (i = 0; i < QS_MAX_DATA_RATES_G; i++) {
						{
							if (ENDIAN_SWAP32(wlpptr->wlpd_p->rxRateHistogram.LegacyRates[i]) > 0) {
								if (i == 2) {
									printk("5.5Mbps \t: %10u\n",
									       ENDIAN_SWAP32((unsigned int)wlpptr->wlpd_p->rxRateHistogram.
											     LegacyRates[i]));
								} else {
									printk("%2dMbps  \t: %10u\n", _11G_rate[i]
									       ,
					...
					for (nss = 0; nss < QS_NUM_SUPPORTED_11AC_NSS_BIG; nss++) {
						for (bw = 0; bw < QS_NUM_SUPPORTED_11AC_BW; bw++) {
							for (i = 0; i < QS_NUM_SUPPORTED_11AC_MCS; i++) {
								for (sgi = 0; sgi < QS_NUM_SUPPORTED_GI; sgi++) {
									if (ENDIAN_SWAP32
									    (wlpptr->wlpd_p->drvrxRateHistogram.VHtRates[nss][bw][sgi][i]) > 0) {
										printk("%4s_%3s_%1dSS_MCS%2d  : %10u\n", vhtbwStr[bw], sgiStr[sgi],
										       (nss + 1), i,
										       ENDIAN_SWAP32(((int)wlpptr->wlpd_p->drvrxRateHistogram.
												      VHtRates[nss][bw][sgi][i])));
									}
								}
							}
						}
					}
					for (nss = 0; nss < QS_NUM_SUPPORTED_11AX_NSS; nss++) {
						for (bw = 0; bw < QS_NUM_SUPPORTED_11AX_BW; bw++) {
							for (i = 0; i < QS_NUM_SUPPORTED_11AX_MCS; i++) {
								for (sgi = 0; sgi < QS_NUM_SUPPORTED_11AX_GILTF_EXT; sgi++) {
									if (ENDIAN_SWAP32(wlpptr->wlpd_p->drvrxRateHistogram.HERates[nss][bw][sgi][i])
									    > 0) {
										printk("%4s_giltf[%d]_%1dSS_MCS%2d  : %10u\n", hebwStr[bw], sgi,
										       (nss + 1), i,
										       ENDIAN_SWAP32(((int)wlpptr->wlpd_p->drvrxRateHistogram.
												      HERates[nss][bw][sgi][i])));
									}
								}
							}
						}
	...
	int wlFwSetMacSa(struct net_device *netdev, int n, UINT8 * addr) {
		...
		HostCmd_QSTATS_SET_SA *pCmd = (HostCmd_QSTATS_SET_SA *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		...
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_QSTATS_SET_SA));
...
/*Get MAC addr to be kicked out when consecutive tx failure cnt > limit*/
	int wlFwGetConsecTxFailAddr(struct net_device *netdev, IEEEtypes_MacAddr_t * addr) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_GET_CONSEC_TXFAIL_ADDR *pCmd = (HostCmd_FW_GET_CONSEC_TXFAIL_ADDR *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		...
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_GET_CONSEC_TXFAIL_ADDR));
...
/*Set consective tx failure limit. When consecutive tx failure cnt > limit, client will be kicked out*/
	int wlFwSetConsecTxFailLimit(struct net_device *netdev, UINT32 value) {
		...
		HostCmd_FW_TXFAILLIMIT *pCmd = (HostCmd_FW_TXFAILLIMIT *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
...
#ifdef SOC_W906X
	int wlFwSetVHTOpMode(struct net_device *netdev, UINT16 staid, UINT8 vht_NewRxChannelWidth, UINT8 vht_NewRxNss)
#else
	int wlFwSetVHTOpMode(struct net_device *netdev, IEEEtypes_MacAddr_t * staaddr, UINT8 vht_NewRxChannelWidth, UINT8 vht_NewRxNss)
#endif
	{
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_VHT_OP_MODE *pCmd = (HostCmd_FW_VHT_OP_MODE *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		...
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_VHT_OP_MODE));
 ...
 */
	int wlFwGetCoreSniff(struct net_device *netdev, coredump_cmd_t * core_dump, char *buff) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_CORE_DUMP *pCmd = (HostCmd_FW_CORE_DUMP *) & wlpptr->pCmdBuf[0];
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_CORE_DUMP));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->cmd_data.coredump.context = ENDIAN_SWAP32(core_dump->context);
		pCmd->cmd_data.coredump.buffer = (UINT32) wlpptr->wlpd_p->pPhysCmdBuf + sizeof(HostCmd_FW_CORE_DUMP) - sizeof(HostCmd_FW_CORE_DUMP_);
		pCmd->cmd_data.coredump.buffer_len = ENDIAN_SWAP32(MAX_CORE_DUMP_BUFFER);
		...
		WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_FW_CORE_DUMP));
		retval = wlexecuteCommand(netdev, HostCmd_CMD_GET_FW_CORE_DUMP);
		if (retval == SUCCESS)
			retval = (ENDIAN_SWAP16(pCmd->CmdHdr.Result) == HostCmd_RESULT_OK) ? SUCCESS : FAIL;
		/*Update Core Dump Buffer */
		core_dump->context = ENDIAN_SWAP32(pCmd->cmd_data.coredump.context);
...
#else
		memcpy(buff, (const void *)((UINT32) pCmd + sizeof(HostCmd_FW_CORE_DUMP) - sizeof(HostCmd_FW_CORE_DUMP_)), MAX_CORE_DUMP_BUFFER);
#endif
		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
...
*/
	int wlFwGetCoreDumpAddrValue(struct net_device *netdev, UINT32 addr, UINT32 len, UINT32 * val, UINT16 set) {
		...
		MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);
		memset(pCmd, 0x00, sizeof(HostCmd_FW_CORE_DUMP));
		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_GET_FW_CORE_MEM_DUMP);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_CORE_DUMP));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		...
		WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_FW_CORE_DUMP));
		retval = wlexecuteCommand(netdev, HostCmd_CMD_GET_FW_CORE_MEM_DUMP);
		if (!retval) {
			if (set == 0) {	//get
				memcpy((void *)val, (void *)pCmd->Buffer, (sizeof(u32) * pdmCmd->val));
			}
		}
		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
		return retval;
	...
	int wlFwGetCoreDump(struct net_device *netdev, coredump_cmd_t * core_dump, char *buff) {
		...
		if (retval == SUCCESS) {
			...
			memcpy(buff, pCmd->Buffer, MAX_CORE_DUMP_BUFFER);
#else
			memcpy(buff, (const void *)((UINT32) pCmd + sizeof(HostCmd_FW_CORE_DUMP) - sizeof(HostCmd_FW_CORE_DUMP_)),
			       MAX_CORE_DUMP_BUFFER);
#endif
		} else {
			printk("Fail to get coredump: cmd:0x%x %s\n", HostCmd_CMD_GET_FW_CORE_DUMP, ((retval == TIMEOUT) ? "TIMEOUT" : "FAIL"));
		}
		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
	...
	int wlFwDiagMode(struct net_device *netdev, UINT16 status) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_DIAG_MODE *pCmd = (HostCmd_FW_DIAG_MODE *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;

		MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);
		memset(pCmd, 0x00, sizeof(HostCmd_FW_DIAG_MODE));
		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_CORE_DUMP_DIAG_MODE);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_DIAG_MODE));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->Status = ENDIAN_SWAP16(status);
		WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_FW_DIAG_MODE));
		retval = wlexecuteCommand(netdev, HostCmd_CMD_CORE_DUMP_DIAG_MODE);
		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
		return retval;
	}
	...
	int wlFwTxDropMode(struct net_device *netdev, UINT32 id, UINT16 flag, UINT16 enable) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_DEBUG_TXDROP_MODE *pCmd = (HostCmd_DEBUG_TXDROP_MODE *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_DEBUG_TXDROP_MODE));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->Flag = ENDIAN_SWAP16(flag);
		pCmd->Enable = ENDIAN_SWAP16(enable);
		pCmd->id = ENDIAN_SWAP32(id);
		WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_DEBUG_TXDROP_MODE));
	...
	int wlFw_SetFixedPe(struct net_device *netdev, UINT8 pe, UINT16 enable) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_FIXED_PE *pCmd = (HostCmd_FW_FIXED_PE *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_FIXED_PE));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->enabled = enable;
		pCmd->pe = pe;
...
#ifdef NEW_DP
	int wlFwNewDP_Cmd(struct net_device *netdev, UINT8 ch, UINT8 width, UINT8 rates, UINT8 rate_type, UINT8 rate_bw, UINT8 rate_gi, UINT8 rate_ss)
	{

		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_NEWDP_CTRL *pCmd = (HostCmd_FW_NEWDP_CTRL *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_CTRL));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->ch = ch;
		pCmd->width = width;
		pCmd->rates = rates;
	...
	int wlFwNewDP_RateDrop(struct net_device *netdev, UINT32 enabled, UINT32 rate_index, UINT32 staidx)
	{

		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_NEWDP_RATEDROP *pCmd = (HostCmd_FW_NEWDP_RATEDROP *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_RATEDROP));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
#ifdef CONFIG_MC_BC_RATE
		pCmd->CmdHdr.macid = wlpptr->vmacSta_p->VMacEntry.macId;
#endif
...
#define HAL_ACTSUBCH_BOTH   2
	...
	void getChanPrimarySub(u32 chan_num, u32 width, u32 * primary, u32 * sub) {
		if (width == HAL_CHANWIDTH_20MHZ) {
			...
			*primary = macMgmtMlme_Get80MHzPrimaryChannelOffset(chan_num);
			if (*primary & BIT(0)) {
				*sub = HAL_ACTSUBCH_LOWER;
			} else {
				*sub = HAL_ACTSUBCH_UPPER;
	...
	int wlFwOffChannel(struct net_device *netdev, u32 ch, u32 bw, u32 dwell, u8 req_type, struct sk_buff *skb, u16 * result) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_NEWDP_OFFCHANNEL *pCmd = (HostCmd_FW_NEWDP_OFFCHANNEL *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		u32 primary = 0;
		u32 sub = 0;
		unsigned long flags;
		...
		if (skb != NULL)
			pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_OFFCHANNEL) - (MAX_OFF_CHAN_PKT_SIZE - skb->len));
		else
			pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_OFFCHANNEL) - MAX_OFF_CHAN_PKT_SIZE);
		...
		if ((pCmd->OffChannel.req_type == OFF_CHAN_REQ_TYPE_TX) && (skb != NULL))
			memcpy(&(pCmd->Data[0]), skb->data, skb->len);

		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_NEWDP_OFFCHANNEL));
	...
	int wlFwOffChannel_dbg(struct net_device *netdev, u32 * fw_offchan_state) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_NEWDP_OFFCHANNEL_DBG *pCmd = (HostCmd_FW_NEWDP_OFFCHANNEL_DBG *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_OFFCHANNEL_DBG));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);

		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_NEWDP_OFFCHANNEL_DBG));
	...
	int wlFwNewDP_OffChannel_Start(struct net_device *netdev) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_NEWDP_OFFCHANNEL_START *pCmd = (HostCmd_FW_NEWDP_OFFCHANNEL_START *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		...
		if (wlpptr->wlpd_p->descData[0].pOffChReqRing == NULL) {
			...
			wlpptr->wlpd_p->descData[0].pOffChReqRing = (u_int8_t *)
			    wl_dma_alloc_coherent(wlpptr->wlpd_p->dev,
						  ...
						  &wlpptr->wlpd_p->descData[0].pPhysOffChReqRing, wlpptr->wlpd_p->dma_alloc_flags);

			wlpptr->wlpd_p->descData[0].pOffChDoneRing = (u_int8_t *)
			    wl_dma_alloc_coherent(wlpptr->wlpd_p->dev,
						  MAX_OFF_CHAN_DONE * sizeof(offchan_done_stat_t),
						  &wlpptr->wlpd_p->descData[0].pPhysOffChDoneRing, wlpptr->wlpd_p->dma_alloc_flags);
#else
			wlpptr->wlpd_p->descData[0].pOffChReqRing = (u_int8_t *) pci_alloc_consistent(wlpptr->pPciDev,
												      MAX_OFF_CHAN_REQ * sizeof(offchan_desc_t),
												      &wlpptr->wlpd_p->descData[0].pPhysOffChReqRing);

			wlpptr->wlpd_p->descData[0].pOffChDoneRing = (u_int8_t *) pci_alloc_consistent(wlpptr->pPciDev,
												       MAX_OFF_CHAN_DONE *
		...
		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_NEWDP_OFFCHAN_START);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_OFFCHANNEL_START));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->OffChanReqBase = ENDIAN_SWAP32(wlpptr->wlpd_p->descData[0].pPhysOffChReqRing);
		pCmd->OffChanDoneBase = ENDIAN_SWAP32(wlpptr->wlpd_p->descData[0].pPhysOffChDoneRing);
		pCmd->pPhyoffchanshared = ENDIAN_SWAP32(wlpptr->wlpd_p->descData[0].pPhyoffchanshared);

		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_NEWDP_OFFCHANNEL_START));
		retval = wlexecuteCommand(netdev, HostCmd_CMD_NEWDP_OFFCHAN_START);
		if (retval == SUCCESS) {
			wlpptr->wlpd_p->descData[0].OffChanReqHead = ENDIAN_SWAP32(pCmd->OffChanReqHead);
			wlpptr->wlpd_p->descData[0].OffChanReqTail = ENDIAN_SWAP32(pCmd->OffChanReqTail);
			wlpptr->wlpd_p->descData[0].OffChanDoneHead = ENDIAN_SWAP32(pCmd->OffChanDoneHead);
			wlpptr->wlpd_p->descData[0].OffChanDoneTail = ENDIAN_SWAP32(pCmd->OffChanDoneTail);
			printk("OffChanReqHead = %x\nOffChanReqTail = %x\nOffChanDoneHead = %x\nOffChanDoneTail = %x\n",
			       wlpptr->wlpd_p->descData[0].OffChanReqHead,
			       wlpptr->wlpd_p->descData[0].OffChanReqTail,
			       wlpptr->wlpd_p->descData[0].OffChanDoneHead, wlpptr->wlpd_p->descData[0].OffChanDoneTail);
		}
		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
		return retval;
	}
...
#ifdef SOC_W906X
	int wlFwNewDP_queue_OffChan_req(struct net_device *netdev, DOT11_OFFCHAN_REQ_t * pOffChan) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
		unsigned long flags;
		...
		if (pOffChan == NULL) {
			printk("%s offchan req is NULL\n", netdev->name);
			MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
			return FAIL;
		}
		//Check if interface is down. If down, do not accept new request
		if ((vmacSta_p->acs_mode != 1) && (netdev->flags & IFF_RUNNING) == 0) {
			printk("%s offchan interface is down\n", netdev->name);
			MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
			return FAIL;
		}
		...
		if ((offChanListItem_p = wl_kmalloc(sizeof(offChanListItem), GFP_ATOMIC)) == NULL) {
		...
		pOffChDesc->OffChannel.channel = ENDIAN_SWAP32(pOffChan->channel);
		pOffChDesc->OffChannel.lifetime = ENDIAN_SWAP32(pOffChan->lifetime);
		pOffChDesc->OffChannel.start_ts = ENDIAN_SWAP32(pOffChan->start_ts);
		pOffChDesc->OffChannel.dwell_time = ENDIAN_SWAP32(pOffChan->dwell_time);
		pOffChDesc->OffChannel.channel_width = ENDIAN_SWAP32(pOffChan->channel_width);
		...
		if (pOffChan->pak != NULL) {
			...
			UINT16 MrvlFrmLen;
			int retval;
			IEEEtypes_GenHdr_t *pHdr = (IEEEtypes_GenHdr_t *) pOffChan->pak;
			...
			retval = copy_from_user(txSkb_p->data, pOffChan->pak, MrvlFrmLen);
			...
			pOffChDesc->txDesc.Data = dma_map_single(wlpptr->wlpd_p->dev, txSkb_p->data, txSkb_p->len, DMA_TO_DEVICE);
#endif
#if 0
			...
			if (txSkb_p->len >= 8) {
				printk("data[0][1][2][3]/[%d][%d][%d][%d] = %x %x %x %x %x %x %x %x\n",
				       txSkb_p->len - 4, txSkb_p->len - 3, txSkb_p->len - 2, txSkb_p->len - 1,
				       txSkb_p->data[0], txSkb_p->data[1], txSkb_p->data[2], txSkb_p->data[3],
				       txSkb_p->data[txSkb_p->len - 4], txSkb_p->data[txSkb_p->len - 3], txSkb_p->data[txSkb_p->len - 2],
				       txSkb_p->data[txSkb_p->len - 1]);
			}
#endif
			/* TODO. Do we need off channel? Change to new TX descriptor. */
#if 0
			pOffChDesc->txDesc.Ctrl = ENDIAN_SWAP32(((0 & txring_Ctrl_QIDmask) << txring_Ctrl_QIDshift) |
								(((txSkb_p->len - (sizeof(struct ieee80211_frame)) -
								   2) & txring_Ctrl_LenMask) << txring_Ctrl_LenShift) | (txring_Ctrl_TAG_MGMT <<
		...
		/* FW can only deal with offch scan when vap is up or vap is down but scan type is RX_NF which used for ACS */
		if (wlpptr->wlpd_p->offchan_scan.init_flag == TRUE || pOffChDesc->OffChannel.req_type == OFFCHAN_TYPE_RX_NF) {
			/* Only init_flag == TRUE to put offchannel list item into queue */
			SPIN_LOCK_IRQSAVE(&wlpptr->wlpd_p->locks.offChanListLock, listflags);
			ListPutItem(&wlpptr->wlpd_p->offChanList, (ListItem *) offChanListItem_p);
			SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->locks.offChanListLock, listflags);
		} else
			return FAIL;
	...
	int wlFwNewDP_queue_OffChan_req(struct net_device *netdev, DOT11_OFFCHAN_REQ_t * pOffChan) {
		...
		int retval;
#ifdef SOC_W906X
		unsigned int reg_offchreq_head = wlpptr->wlpd_p->reg.offch_req_head;
		unsigned int reg_offchreq_tail = wlpptr->wlpd_p->reg.offch_req_tail;
#endif
		...
		pOffChDesc = (offchan_desc_t *) (wlpptr->wlpd_p->descData[0].pOffChReqRing + nextOff);
...
#endif
#else
			pOffChDesc->txDesc.Data = ENDIAN_SWAP32(pci_map_single(wlpptr->pPciDev, txSkb_p->data, txSkb_p->len, PCI_DMA_TODEVICE));
			pOffChDesc->txDesc.Ctrl = ENDIAN_SWAP32(((0 & txring_Ctrl_QIDmask) << txring_Ctrl_QIDshift) |
								(((txSkb_p->len - (sizeof(struct ieee80211_frame)) -
								   2) & txring_Ctrl_LenMask) << txring_Ctrl_LenShift) | (txring_Ctrl_TAG_MGMT <<
															 txring_Ctrl_TAGshift));
		...
		if ((ReqIdListItem_p = wl_kmalloc(sizeof(ReqIdListItem), GFP_ATOMIC)) == NULL) {
			if (txSkb_p)
				wl_free_skb(txSkb_p);
		...
		ListPutItem(&wlpptr->wlpd_p->ReqIdList, (ListItem *) ReqIdListItem_p);
...
#define OFFCHAN_COOLDOWN_TIME	2	//in 100ms unit. 10 = 1sec
	...
	int wlFwNewDP_handle_OffChan_event(struct net_device *netdev) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
		offChanListItem *offchan_current_req_p = wlpptr->wlpd_p->offchan_current_req_p;
		unsigned long listflags;
		int ret;
		u16 result;
		...
		case OFFCHAN_IDLE:
			{
				...
				SPIN_LOCK_IRQSAVE(&wlpptr->wlpd_p->locks.offChanListLock, listflags);
				wlOffChannelItemFree(wlpptr->wlpd_p->offchan_current_req_p);
				wlpptr->wlpd_p->offchan_current_req_p = (offChanListItem *) ListGetItem(&wlpptr->wlpd_p->offChanList);
				offchan_current_req_p = wlpptr->wlpd_p->offchan_current_req_p;
				SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->locks.offChanListLock, listflags);

				if (offchan_current_req_p != NULL) {
					//Save the offChanListItem. Will free it when OFFCHAN_DONE.
					switch (offchan_current_req_p->offchan_desc.OffChannel.req_type) {
					...
					case OFFCHAN_TYPE_RX_NF:
						{
							...
							u32 dwell = offchan_current_req_p->offchan_desc.OffChannel.dwell_time;
							u8 req_type = offchan_current_req_p->offchan_desc.OffChannel.req_type;

							wl_send_offchan_active_event(netdev, WLMGR_ID_DCS, WLMGR_CMD_DEV_OFFCH_STARTED,
										     offchan_current_req_p->offchan_desc.OffChannel.id);

							ret =
							    wlFwOffChannel(netdev, ch, bw, dwell, req_type, offchan_current_req_p->txSkb_p, &result);
							if (ret == SUCCESS) {
								switch (result) {
								case HostCmd_RESULT_OFFCHAN_BCN_GUARD:
									{
										ret =
										    wlFwOffChannel(netdev, ch, bw, dwell, req_type,
												   offchan_current_req_p->txSkb_p, &result);
								...
								case HostCmd_RESULT_OFFCHAN_IN_PROCESS:
									{
										printk
										    ("HostCmd_RESULT_OFFCHAN_IN_PROCESS, this request is canceled\n");
										break;
									}
								default:
									{
		...
		case OFFCHAN_STARTED:
			{
				/* In this State, FW start to do offchannel scan request, reset the completion count to 0 until receives OFFCHAN_DONE from FW */
				reinit_completion(&wlpptr->wlpd_p->offchan_scan.offchan_complete);
				break;
			}
		case OFFCHAN_CH_CHANGE:
			{
				...
				if (wlpptr->wlpd_p->offchan_current_req_p) {
					offch_id = wlpptr->wlpd_p->offchan_current_req_p->offchan_desc.OffChannel.id;
					if (OFFCHAN_GET_FEATURE_FROM_ID(offch_id) == OFFCHAN_BY_LIST) {
						/* If this OFFCHAN_DONE event belongs to OFFCHAN_BY_LIST, put "offchan_scan_mgt" into work queue to trigger next offchannel scan */
						wlpptr->wlpd_p->offchan_scan.status = OFFCHAN_CH_CHANGE;
		...
		case OFFCHAN_DONE:
			{
				...
				extern mvl_status_t CH_radio_status[IEEEtypes_MAX_CHANNELS + IEEEtypes_MAX_CHANNELS_A];
				UINT32 offch_id = 0;

 OFFCHAN_PROCESS_FAIL:
				/* Avoid kernel panic, after wdevreset, this pointer will become NULL, and offchan_state need to reset OFFCHAN_CH_CHANGE->OFFCHAN_DONE->OFFCHAN_IDLE */
				if (wlpptr->wlpd_p->offchan_current_req_p)
					offch_id = wlpptr->wlpd_p->offchan_current_req_p->offchan_desc.OffChannel.id;
				...
				SPIN_LOCK_IRQSAVE(&wlpptr->wlpd_p->locks.offChanListLock, listflags);
				wlOffChannelItemFree(wlpptr->wlpd_p->offchan_current_req_p);
				wlpptr->wlpd_p->offchan_current_req_p = NULL;
				SPIN_UNLOCK_IRQRESTORE(&wlpptr->wlpd_p->locks.offChanListLock, listflags);
				//2. Trigger a timer for cooldown
				TimerFireIn(&wlpptr->wlpd_p->offChanCooldownTimer, 1, &offChanCooldown_cb, (void *)netdev, OFFCHAN_COOLDOWN_TIME);
				...
				if (OFFCHAN_GET_FEATURE_FROM_ID(offch_id) == OFFCHAN_BY_LIST) {
					/* If this OFFCHAN_DONE event belongs to OFFCHAN_BY_LIST, put "offchan_scan_mgt" into work queue to trigger next offchannel scan */
					wlpptr->wlpd_p->offchan_scan.status = OFFCHAN_DONE;
					schedule_work(&wlpptr->wlpd_p->offchan_scan_mgt);
				}
				MSAN_unassocsta_offchan_done(netdev, UNASSOCSTA_TRACK_MODE_OFFCHAN);

				memset(&tmp_status, 0, sizeof(mvl_status_t));
				wlFwGetRadioStatus(netdev, &tmp_status);
				ch_idx = GetRegionChanIndx(GetDomainIndxIEEERegion(domainGetDomain()), tmp_status.channel);
				if (ch_idx >= 0 && ch_idx < IEEEtypes_MAX_CHANNELS + IEEEtypes_MAX_CHANNELS_A) {
					memcpy(&CH_radio_status[ch_idx], &tmp_status, sizeof(mvl_status_t));
				}

				if (ch_idx < IEEE_80211_MAX_NUMBER_OF_CHANNELS) {
					u32 *nf_dbm_p = wlpptr->wlpd_p->dcs_db[ch_idx].nf_bin;
					u32 ch_load, ch_nf;

					wlpptr->wlpd_p->dcs_db[ch_idx].channel = tmp_status.channel;
					memcpy(nf_dbm_p, tmp_status.nf_dbm, sizeof(u32) * 18);
					wlpptr->wlpd_p->dcs_db[ch_idx].nf_dbm_time = ktime_get();
					wlpptr->wlpd_p->dcs_db[ch_idx].score = ACS_channel_score_from_nf_reading(nf_dbm_p, &ch_load, &ch_nf);
					wlpptr->wlpd_p->dcs_db[ch_idx].ch_load = ch_load;
					wlpptr->wlpd_p->dcs_db[ch_idx].noise_floor = ch_nf;
				}
				wl_send_offchan_active_event(netdev, WLMGR_ID_DCS, WLMGR_CMD_DEV_OFFCH_COMPLETED, offch_id);

				/* if ACS mode is NF-reading and auto channel process is on-going, call timeout */
				if (IsACSOnoing(netdev) && (OFFCHAN_GET_FEATURE_FROM_ID(offch_id) == OFFCHAN_BY_ACS)) {
					memcpy(vmacSta_p->acs_db[vmacSta_p->ChanIdx].nf_bin, tmp_status.nf_dbm, MAX_NF_DBM_LEN * sizeof(u32));
					syncSrv_ScanActTimeOut((UINT8 *) vmacSta_p);
				}

				/* Increasing completion count so that when wdevreset, ifdown and wlstop will not be blocked */
				complete(&wlpptr->wlpd_p->offchan_scan.offchan_complete);
				break;
			}
	...
	int wlFwNewDP_handle_OffChan_event(struct net_device *netdev) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		UINT32 offchDoneHead = ((offchan_shared_t *) wlpptr->wlpd_p->descData[0].poffchanshared)->OffChanDoneHead;
		UINT32 offchDoneTail = ((offchan_shared_t *) wlpptr->wlpd_p->descData[0].poffchanshared)->OffChanDoneTail;
		...
		while (offchDoneHead != offchDoneTail) {
			pOffDone = (offchan_done_stat_t *) (wlpptr->wlpd_p->descData[0].pOffChDoneRing + offchDoneTail * sizeof(offchan_done_stat_t));
			pOffHead = (offchan_done_stat_t *) (wlpptr->wlpd_p->descData[0].pOffChDoneRing + offchDoneHead * sizeof(offchan_done_stat_t));

			if ((ENDIAN_SWAP32(pOffDone->status) == OFFCHAN_CH_CHANGE) && bReqTypeRx) {
				bStartOffChanRx = TRUE;
			} else if (ENDIAN_SWAP32(pOffDone->status) == OFFCHAN_DONE) {
				bStartOffChanRx = FALSE;
#ifdef IEEE80211K
				MSAN_update_neighbor_list(netdev);
#endif
			...
			if ((ENDIAN_SWAP32(pOffDone->status) == OFFCHAN_DONE) || (ENDIAN_SWAP32(pOffDone->status) == OFFCHAN_FAIL)) {
				while ((ReqIdListItem_p = (ReqIdListItem *) ListGetItem(&wlpptr->wlpd_p->ReqIdList)) != NULL) {
					if (ReqIdListItem_p->ReqId == ENDIAN_SWAP32(pOffDone->id)) {
			...
			sprintf(evBuf, "off-channel-status %d %d\n", ENDIAN_SWAP32(pOffDone->id), ENDIAN_SWAP32(pOffDone->status));
			WLSNDEVT(netdev, IWEVCUSTOM, (IEEEtypes_MacAddr_t *) & wlpptr->hwData.macAddr[0], evBuf);
		...
		((offchan_shared_t *) wlpptr->wlpd_p->descData[0].poffchanshared)->OffChanDoneTail = offchDoneTail;
	...
	int wlFwNewDP_config_prom(struct net_device *netdev, PROM_CNF_t * PromCnf) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		...
		if (op_mode != SMAC_OPMODE_NORMAL) {
			...
			*(u32 *) (&((SMAC_CTRL_BLK_st *) wlpptr->ioBase0)->config.opMode) = op_mode;
			SMAC_RX_ENABLE(wlpptr, mib, 0xFFFFFFFF);
			regval = readl(wlpptr->ioBase1 + BBRX_CFG);
			if ((regval & 0x01) == 0)
				writel((regval | 0x01), wlpptr->ioBase1 + BBRX_CFG);
		} else {
			*(u32 *) (&((SMAC_CTRL_BLK_st *) wlpptr->ioBase0)->config.opMode) = op_mode;
			SMAC_RX_DISABLE(wlpptr, mib, 0xFFFFFFFF);
		}
#else
		HostCmd_FW_NEWDP_PROM_CNF *pCmd = (HostCmd_FW_NEWDP_PROM_CNF *) & wlpptr->pCmdBuf[0];
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_PROM_CNF));
		pCmd->PromCnf.CnfType = PromCnf->CnfType;
		pCmd->PromCnf.PromDataMask = ENDIAN_SWAP16(PromCnf->PromDataMask);
		pCmd->PromCnf.PromDataTrunc = ENDIAN_SWAP16(PromCnf->PromDataTrunc);
		pCmd->PromCnf.PromMgmtMask = ENDIAN_SWAP16(PromCnf->PromMgmtMask);
		pCmd->PromCnf.PromMgmtTrunc = ENDIAN_SWAP16(PromCnf->PromMgmtTrunc);
		pCmd->PromCnf.PromCtrlMask = ENDIAN_SWAP16(PromCnf->PromCtrlMask);
		pCmd->PromCnf.PromCtrlTrunc = ENDIAN_SWAP16(PromCnf->PromCtrlTrunc);
		pCmd->PromCnf.PromRsvdMask = ENDIAN_SWAP16(PromCnf->PromRsvdMask);
		pCmd->PromCnf.PromRsvdTrunc = ENDIAN_SWAP16(PromCnf->PromRsvdTrunc);
		pCmd->PromCnf.PromPolNomWin = ENDIAN_SWAP32(PromCnf->PromPolNomWin);
		pCmd->PromCnf.PromPolNomPkts = ENDIAN_SWAP32(PromCnf->PromPolNomPkts);
		...
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_NEWDP_PROM_CNF));
		retval = wlexecuteCommand(netdev, HostCmd_CMD_NEWDP_CONFIG_PROM);
		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
#endif				/* #ifdef SOC_W906X */
		return retval;
...
#if defined(SOC_W906X) || defined(NEWDP_ACNT_CHUNKS)
	int wlFwNewDP_setAcntBufSize(struct net_device *netdev, u_int32_t * base, u_int32_t size, u_int32_t ActionType)
#else
	int wlFwNewDP_setAcntBufSize(struct net_device *netdev, u_int32_t base, u_int32_t size)
#endif
	{
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_SET_ACNT_BUF_SIZE *pCmd = (HostCmd_FW_SET_ACNT_BUF_SIZE *) & wlpptr->pCmdBuf[0];
		...
		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_NEWDP_SET_ACNT_BUF_SIZE);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_SET_ACNT_BUF_SIZE));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		...
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_NEWDP_PROM_CNF));
		retval = wlexecuteCommand(netdev, HostCmd_CMD_NEWDP_SET_ACNT_BUF_SIZE);
		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
		return retval;

	}
	int wlFwNewDP_sensorD_init(struct net_device *netdev, sensord_init_t * sensordinit, UINT8 action) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);

		HostCmd_FW_NEWDP_SENSORD_INIT *pCmd = (HostCmd_FW_NEWDP_SENSORD_INIT *) & wlpptr->pCmdBuf[0];
		...
		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_NEWDP_SENSORD_INIT);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_SENSORD_INIT));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);

		if (action) {
			pCmd->SensordInit_ext.mrvl_pri_mailbox = (UINT32) ENDIAN_SWAP32((UINT32) wlpptr->wlpd_p->MrvlPriSharedMem.dataPhysicalLoc);
			pCmd->SensordInit_ext.SensordInit.ca_mailbox =
			    (UINT32) ENDIAN_SWAP32((UINT32) wlpptr->wlpd_p->AllocSharedMeminfo.dataPhysicalLoc);
			pCmd->SensordInit_ext.SensordInit.buff_size = ENDIAN_SWAP32(sensordinit->buff_size);
			pCmd->SensordInit_ext.SensordInit.num_freq = ENDIAN_SWAP32(sensordinit->num_freq);
			pCmd->SensordInit_ext.SensordInit.instance = ENDIAN_SWAP32(sensordinit->instance);
			pCmd->SensordInit_ext.SensordInit.mvl_radio_id = ENDIAN_SWAP32(sensordinit->mvl_radio_id);
			pCmd->SensordInit_ext.SensordInit.enableSI = sensordinit->enableSI;
			memcpy(pCmd->SensordInit_ext.SensordInit.nsiKey, sensordinit->nsiKey, SENSORD_NSIKEY_LEN);
		} else {
			pCmd->SensordInit_ext.mrvl_pri_mailbox = (UINT32) ENDIAN_SWAP32((UINT32) wlpptr->wlpd_p->MrvlPriSharedMem.dataPhysicalLoc);
			pCmd->SensordInit_ext.SensordInit.ca_mailbox =
			    (UINT32) ENDIAN_SWAP32((UINT32) wlpptr->wlpd_p->AllocSharedMeminfo.dataPhysicalLoc);
		...
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_NEWDP_SENSORD_INIT));
		wlInterruptUnMask(netdev, MACREG_A2HRIC_NEWDP_SENSORD);
		retval = wlexecuteCommand(netdev, HostCmd_CMD_NEWDP_SENSORD_INIT);
		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
	...
	int wlFwNewDP_sensorD_cmd(struct net_device *netdev) {
		...
		HostCmd_FW_NEWDP_SENSORD_CMD *pCmd = (HostCmd_FW_NEWDP_SENSORD_CMD *) & wlpptr->pCmdBuf[0];
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_SENSORD_CMD));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_NEWDP_SENSORD_CMD));
		retval = wlexecuteCommand(netdev, HostCmd_CMD_NEWDP_SENSORD_CMD);
		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
		return retval;
	...
	int wlFwNewDP_DMAThread_start(struct net_device *netdev) {
		...
		HostCmd_FW_DMATHREAD_START_CMD *pCmd = (HostCmd_FW_DMATHREAD_START_CMD *) & wlpptr->pCmdBuf[0];
		...
		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_NEWDP_DMATHREAD_START);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_DMATHREAD_START_CMD));
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_DMATHREAD_START_CMD));
		retval = wlexecuteCommand(netdev, HostCmd_CMD_NEWDP_DMATHREAD_START);
		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
		return retval;
	}
#endif				/* #ifndef SOC_W906X */
	...
	int wlFwNewDP_wifiarb_post_req_intr(struct net_device *netdev) {
		...
		sfw_notify.notification_src = wifiarb_post_req_intr_notification;
		info.si_int = *(int *)&sfw_notify;
		rcu_read_lock();
		t = pid_task(find_pid_ns(wlpptr->wlpd_p->PostReqSiginfo.pid, &init_pid_ns), PIDTYPE_PID);
		if (t == NULL) {
			printk("no such pid\n");
			rcu_read_unlock();
			return -ENODEV;
	...
	int wlFwNewDP_sensord_set_blanking(struct net_device *netdev, u8 * blankingmask) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);

		HostCmd_FW_NEWDP_SENSORD_SET_BLANKING *pCmd = (HostCmd_FW_NEWDP_SENSORD_SET_BLANKING *) & wlpptr->pCmdBuf[0];
		...
		MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);
		memset(pCmd, 0x00, sizeof(HostCmd_FW_NEWDP_SENSORD_SET_BLANKING));
		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_NEWDP_SENSORD_SET_BLANKING);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_SENSORD_SET_BLANKING));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->blankingmask = *blankingmask;
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_NEWDP_SENSORD_SET_BLANKING));
		printk("blankingmask = %d \n", pCmd->blankingmask);
		retval = wlexecuteCommand(netdev, HostCmd_CMD_NEWDP_SENSORD_SET_BLANKING);
		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);

		return retval;
	}

	int wlFwNewDP_bfmr_config(struct net_device *netdev, bfmr_config_t * BFMRconfig, UINT8 action) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);

		HostCmd_FW_NEWDP_BFMR_CONFIG *pCmd = (HostCmd_FW_NEWDP_BFMR_CONFIG *) & wlpptr->pCmdBuf[0];
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_BFMR_CONFIG));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		memcpy((UINT8 *) & pCmd->BFMRconfig, (UINT8 *) BFMRconfig, sizeof(bfmr_config_t));
		...
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_NEWDP_BFMR_CONFIG));
		retval = wlexecuteCommand(netdev, HostCmd_CMD_NEWDP_BFMR_CONFIG);

		printk("chan =%d, bw =%d, rx_ant =%d, tx_ant =%d \n", pCmd->BFMRconfig.chan,
		       pCmd->BFMRconfig.bw, pCmd->BFMRconfig.rx_ant, pCmd->BFMRconfig.tx_ant);
		//printk("HT_cap\n");
	...
	int wlFwNewDP_bfmr_sbf_open(struct net_device *netdev, wlcfg_sbf_open_t * WlcfgsbfOpen) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
		extStaDb_StaInfo_t *pStaInfo;
		...
		HostCmd_FW_NEWDP_BFMR_SBF_OPEN *pCmd = (HostCmd_FW_NEWDP_BFMR_SBF_OPEN *) & wlpptr->pCmdBuf[0];
		unsigned long flags;
		int retval = FAIL;
		...
		if (WlcfgsbfOpen->reassign == 0) {
			if ((pStaInfo = extStaDb_GetStaInfo(vmacSta_p,
							    (IEEEtypes_MacAddr_t *) WlcfgsbfOpen->CurrentStaMac,
							    STADB_DONT_UPDATE_AGINGTIME)) == NULL) {
				printk("get current sta info fail\n");
				return retval;
			}
			memcpy(BFMRsbfOpen.addr, WlcfgsbfOpen->CurrentStaMac, 6);
			...
			if ((pStaInfo = extStaDb_GetStaInfo(vmacSta_p,
							    (IEEEtypes_MacAddr_t *) WlcfgsbfOpen->ReassignStaMac,
							    STADB_DONT_UPDATE_AGINGTIME)) == NULL) {
				printk("get reassign sta info fail\n");
				return retval;
			}
			memcpy(BFMRsbfOpen.addr, WlcfgsbfOpen->ReassignStaMac, 6);
			BFMRsbfOpen.sbf_slot = slotTmp;
		}
		memcpy(BFMRsbfOpen.vht_cap_data, ((UINT8 *) & pStaInfo->vhtCap) + 2, 12);
		if (pStaInfo->ClientMode == GONLY_MODE || pStaInfo->ClientMode == BONLY_MODE || pStaInfo->ClientMode == AONLY_MODE) {
			//legacy client?
			memset(BFMRsbfOpen.ht_cap, 0, 28);
		} else {
			memcpy(BFMRsbfOpen.ht_cap, (UINT8 *) & pStaInfo->HtElem, 28);
		}
		//BFMRsbfOpen.rate_map ????????????? to do
		MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);
		memset(pCmd, 0x00, sizeof(HostCmd_FW_NEWDP_BFMR_SBF_OPEN));
		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_NEWDP_BFMR_SBF_OPEN);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_BFMR_SBF_OPEN));
		memcpy((UINT8 *) & pCmd->BFMRsbfOpen, (UINT8 *) & BFMRsbfOpen, sizeof(bfmr_sbf_open_t));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		...
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_NEWDP_BFMR_SBF_OPEN));
	...
	int wlFwNewDP_bfmr_sbf_close(struct net_device *netdev, wlcfg_sbf_close_t * WlcfgsbfClose) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
		extStaDb_StaInfo_t *pStaInfo;
		bfmr_sbf_close_t BFMRsbfClose;
		HostCmd_FW_NEWDP_BFMR_SBF_CLOSE *pCmd = (HostCmd_FW_NEWDP_BFMR_SBF_CLOSE *) & wlpptr->pCmdBuf[0];
		unsigned long flags;
		int retval = FAIL;

		memset(&BFMRsbfClose, 0, sizeof(bfmr_sbf_close_t));
		if ((pStaInfo = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) WlcfgsbfClose->StaMac, STADB_DONT_UPDATE_AGINGTIME)) == NULL) {
		...
		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_NEWDP_BFMR_SBF_CLOSE);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_BFMR_SBF_CLOSE));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		memcpy((UINT8 *) & pCmd->BFMRsbfClose, (UINT8 *) & BFMRsbfClose, sizeof(bfmr_sbf_close_t));
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_NEWDP_BFMR_SBF_CLOSE));
		retval = wlexecuteCommand(netdev, HostCmd_CMD_NEWDP_BFMR_SBF_CLOSE);
		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
		return SUCCESS;
	}
	...
	int wlFwSetPowerPerRate(struct net_device *netdev) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_NEWDP_SET_POWER_PER_RATE *pCmd = (HostCmd_FW_NEWDP_SET_POWER_PER_RATE *) & wlpptr->pCmdBuf[0];
		unsigned long flags;
		int retval = FAIL;
#ifdef SOC_W8964
		MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);
		memset(pCmd, 0x00, sizeof(HostCmd_FW_NEWDP_SET_POWER_PER_RATE));
		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_NEWDP_SET_POWER_PER_RATE);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_SET_POWER_PER_RATE));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->pPhyInfoPwrTbl = (UINT32) ENDIAN_SWAP32(wlpptr->wlpd_p->descData[0].pPhyInfoPwrTbl);
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_NEWDP_SET_POWER_PER_RATE));
		retval = wlexecuteCommand(netdev, HostCmd_CMD_NEWDP_SET_POWER_PER_RATE);
		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
#else
		u32 len;
		u32 offset;
		...
		maxbuflen = (4096 - sizeof(HostCmd_FW_NEWDP_SET_POWER_PER_RATE));
		offset = 0;
		...
		while (offset < total_size) {
			...
			offset += len;
			pCmd->CmdHdr.Length = (len + sizeof(HostCmd_FW_NEWDP_SET_POWER_PER_RATE));
	...
	int wlFwMuUserPosition(struct net_device *netdev, u16 action, u8 gid, u8 usr_pos) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_MU_USER_POSIOTION *pCmd = (HostCmd_MU_USER_POSIOTION *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_MU_USER_POSIOTION));
		pCmd->Action = ENDIAN_SWAP16(action);
		pCmd->gid = gid;
	...
	int wlFwGetPowerPerRate(struct net_device *netdev, UINT32 RatePower, UINT16 * dBm, UINT8 * ant) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		...
		MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);
		memset(pCmd, 0x00, sizeof(HostCmd_FW_NEWDP_SET_POWER_PER_RATE));
		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_NEWDP_GET_POWER_PER_RATE);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_SET_POWER_PER_RATE) + sizeof(u32));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		//dralee
		//pCmd->pPhyInfoPwrTbl   = (UINT32)ENDIAN_SWAP32(wlpptr->wlpd_p->descData[0].pPhyInfoPwrTbl);
		*(u32 *) pCmd->payload = (u32) RatePower;

		WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_FW_NEWDP_SET_POWER_PER_RATE));
		retval = wlexecuteCommand(netdev, HostCmd_CMD_NEWDP_GET_POWER_PER_RATE);

		if (!retval) {
			u64 rpvalue = 0;

			memcpy((void *)&rpvalue, (void *)pCmd->payload, sizeof(u64));
			printk("rpvalue:%llx\n", rpvalue);
	...
	UINT32 wlFwSetProtectMode(struct net_device * netdev, UINT32 action, UINT32 * pMode) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_PROTECTION_MODE *pCmd = (HostCmd_PROTECTION_MODE *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_PROTECTION_MODE));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->CmdHdr.macid = wlpptr->vmacSta_p->VMacEntry.macId;
		pCmd->Action = ENDIAN_SWAP16(action);
		...
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_PROTECTION_MODE));
	...
	UINT32 wlFwSetMib(struct net_device * netdev, UINT32 action, UINT32 mibIdx, UINT32 * pValue, UINT32 * pNum) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_MIB_CFG *pCmd = (HostCmd_FW_MIB_CFG *) & wlpptr->pCmdBuf[0];
		int retval = FAIL, i;
		unsigned long flags;
		...
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_MIB_CFG));
	...
	UINT32 wlFwSetDbg(struct net_device * netdev, UINT32 action, UINT32 dbgIdx, UINT32 * pValue, UINT32 * pNum) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_DBG_CFG *pCmd = (HostCmd_FW_DBG_CFG *) & wlpptr->pCmdBuf[0];
		int retval = FAIL, i;
		unsigned long flags;
		...
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_DBG_CFG));
	...
	UINT32 wlFwSetSchedMode(struct net_device * netdev, UINT16 action, UINT32 mode_selected, void *pCfg, UINT16 len, UINT16 * pStatus) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_SCHED_MODE_CFG *pCmd = (HostCmd_SCHED_MODE_CFG *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_SCHED_MODE_CFG));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->CmdHdr.macid = wlpptr->vmacSta_p->VMacEntry.macId;
		pCmd->action = ENDIAN_SWAP16(action);
		...
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_SCHED_MODE_CFG));
	...
	UINT32 wlFwSetWfaTest(struct net_device * netdev, UINT32 action, UINT32 version, UINT32 testId, UINT32 stepId, void *cfg, UINT32 cfgLen) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_WFA_TEST_CMD *pCmd = (HostCmd_WFA_TEST_CMD *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_WFA_TEST_CMD) + cfgLen);
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->CmdHdr.macid = wlpptr->vmacSta_p->VMacEntry.macId;
		pCmd->action = ENDIAN_SWAP16(action);
		...
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_WFA_TEST_CMD) + cfgLen);
	...
	UINT32 wlFwSentTriggerFrameCmd(struct net_device * netdev, UINT8 action, UINT8 type,
				       UINT32 rateInfo, UINT32 period, UINT32 padNum, void *pData) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_DS_TRIGGER_FRAME *pCmd = (HostCmd_DS_TRIGGER_FRAME *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_DS_TRIGGER_FRAME));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->CmdHdr.macid = wlpptr->vmacSta_p->VMacEntry.macId;
		pCmd->action = action;
		pCmd->type = type;
		...
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_DS_TRIGGER_FRAME));
	...
	int wlFwGetPowerPerRate(struct net_device *netdev, UINT32 RatePower, UINT8 * trpcid, UINT16 * dBm, UINT16 * ant) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		...
		pratepower = (Info_rate_power_table_t *) wlpptr->wlpd_p->descData[0].pInfoPwrTbl;
		...
		if (!retval) {
			memcpy(trpcid, &pratepower->RatePwrTbl.RatePower[1], sizeof(UINT8));
			memcpy(dBm, &pratepower->RatePwrTbl.RatePower[2], sizeof(UINT16));
			memcpy(ant, &pratepower->RatePwrTbl.RatePower[3], sizeof(UINT16));
		}
	...
	int wlFwRadioStatusNotification(struct net_device *netdev, UINT32 action) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_NEWDP_RADIO_STATUS_NOTIFICATION *pCmd = (HostCmd_FW_NEWDP_RADIO_STATUS_NOTIFICATION *) & wlpptr->pCmdBuf[0];
		...
		MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);
		memset(pCmd, 0x00, sizeof(HostCmd_FW_NEWDP_RADIO_STATUS_NOTIFICATION));
		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_NEWDP_RADIO_STATUS_NOTIFICATION);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_RADIO_STATUS_NOTIFICATION));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->Action = ENDIAN_SWAP32(action);	//1: enable, 0: disable
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_NEWDP_RADIO_STATUS_NOTIFICATION));
		retval = wlexecuteCommand(netdev, HostCmd_CMD_NEWDP_RADIO_STATUS_NOTIFICATION);
		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
		return retval;
	}

/*Function to send a continuous fixed len pkt generated in fw using rate info supplied by user*/
	int wlFwSetTxContinuous(struct net_device *netdev, UINT8 mode, UINT32 rateinfo) {
		int retval = FAIL;
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_TXCONTINUOUS *pCmd = (HostCmd_FW_TXCONTINUOUS *) & wlpptr->pCmdBuf[0];
		unsigned long flags;

		MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);
		memset(pCmd, 0x00, sizeof(HostCmd_FW_TXCONTINUOUS));
		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_SET_TX_CONTINUOUS);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_TXCONTINUOUS));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);

		pCmd->mode = mode;
		pCmd->rate_info = ENDIAN_SWAP32(rateinfo);
		retval = wlexecuteCommand(netdev, HostCmd_CMD_SET_TX_CONTINUOUS);
		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
		return retval;
	}
	int wlFwNewDP_amsducfg(struct net_device *netdev, amsducfg_t * amsducfg) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_NEWDP_AMSDU_CFG *pCmd = (HostCmd_FW_NEWDP_AMSDU_CFG *) & wlpptr->pCmdBuf[0];
		unsigned long flags;
		int retval = FAIL;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_AMSDU_CFG));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		memcpy((UINT8 *) & pCmd->amsducfg, (UINT8 *) amsducfg, sizeof(amsducfg_t));
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_NEWDP_AMSDU_CFG));
...
/*Function to set Receiver Start of Packet Detection Threshold (Rx SOP) threshold in fw*/
	int wlFwNewDP_RxSOP(struct net_device *netdev, UINT8 params, UINT8 threshold1, UINT8 threshold2) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_NEWDP_RX_DETECT *pCmd = (HostCmd_FW_NEWDP_RX_DETECT *) & wlpptr->pCmdBuf[0];
		unsigned long flags;
		int retval = FAIL;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_RX_DETECT));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->rx_detect_params = params;
		pCmd->rx_detect_threshold1 = threshold1;
		pCmd->rx_detect_threshold2 = threshold2;

		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_NEWDP_RX_DETECT));
	...
	int wlFwNewDP_set_sku(struct net_device *netdev, UINT32 sku) {
		int retval = FAIL;
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_SET_SKU *pCmd = (HostCmd_FW_SET_SKU *) & wlpptr->pCmdBuf[0];
		unsigned long flags;
	...
	int wlFwNewDP_NDPA_UseTA(struct net_device *netdev, UINT32 enable) {
		int retval = FAIL;
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_NDPA_USETA *pCmd = (HostCmd_FW_NDPA_USETA *) & wlpptr->pCmdBuf[0];
		unsigned long flags;

		MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);
		memset(pCmd, 0x00, sizeof(HostCmd_FW_NDPA_USETA));
		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_NEWDP_NDPA_USETA);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_NDPA_USETA));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->Enable = ENDIAN_SWAP32(enable);

		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_NDPA_USETA));
	...
	int wlFwSetWtpMode(struct net_device *dev) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, dev);
		HostCmd_FW_SET_WTP_MODE *pCmd = (HostCmd_FW_SET_WTP_MODE *) & wlpptr->pCmdBuf[0];
		vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
		MIB_802DOT11 *mib = vmacSta_p->ShadowMib802dot11;
		int retval = FAIL;
		unsigned long flags;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_SET_WTP_MODE));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->CmdHdr.macid = vmacSta_p->VMacEntry.macId;
		...
		vmacSta_p->wtp_info.tunnel_mode = mib->mib_wtp_cfg->frame_tunnel_mode;
		...
		printk("WTP_nable=%d  macmode=%d tunnel_mode=%d RFID=%d WLANID=%d\n",
		       ...
		       vmacSta_p->wtp_info.mac_mode, vmacSta_p->wtp_info.tunnel_mode, vmacSta_p->wtp_info.RF_ID, vmacSta_p->wtp_info.WLAN_ID);

		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_SET_WTP_MODE));
	...
	int wlFwGetMUSet(struct net_device *netdev, UINT8 index) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		int j = 0;
		HostCmd_FW_GET_MU_SET *pCmd = (HostCmd_FW_GET_MU_SET *) & wlpptr->pCmdBuf[0];
		UINT8 *ptr;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_GET_MU_SET));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->CmdHdr.macid = 1;
		pCmd->index = index;
		...
		for (j = 0; j < MU_MIMO_MAX_USER; j++) {
			printk("RateCode : %x StnID%x\n", pCmd->muset[index].mustn[j].RateCode, pCmd->muset[index].mustn[j].StnId);
		}
	...
	int wlFwGetMUInfo(struct net_device *netdev, int groups_only) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_GET_MU_INFOT *pCmd = (HostCmd_FW_GET_MU_INFOT *) & wlpptr->pCmdBuf[0];
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_GET_MU_INFOT));
		pCmd->CmdHdr.macid = 1;
	...
	int wlFwSetMUConfig(struct net_device *netdev, u32 corr_thr_decimal, u16 sta_cep_age_thr, u16 period_ms) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_SET_MU_CONFIGT *pCmd = (HostCmd_FW_SET_MU_CONFIGT *) & wlpptr->pCmdBuf[0];
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_SET_MU_CONFIGT));
		pCmd->CmdHdr.macid = 1;
	...
	int wlFwMUGEnable(struct net_device *netdev, int enable) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_MUG_ENABLET *pCmd = (HostCmd_FW_MUG_ENABLET *) & wlpptr->pCmdBuf[0];
		//u8 * p_data = (u8 *)&(pCmd->groups);
		unsigned long flags;	/* NOTE: Used for MWL_SPIN_LOCK */
		u16 result;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_MUG_ENABLET));
		pCmd->CmdHdr.macid = 1;
	...
	int wlFwSetMUDma(struct net_device *netdev, u_int32_t base, u_int32_t size) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_SET_MU_DMAT *pCmd = (HostCmd_FW_SET_MU_DMAT *) & wlpptr->pCmdBuf[0];
		//u8 * p_data = (u8 *)&(pCmd->groups);
		unsigned long flags;	/* NOTE: Used for MWL_SPIN_LOCK */
		u16 result;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_SET_MU_DMAT));
		pCmd->CmdHdr.macid = 1;
...
#ifdef SOC_W906X
	int wlFwSetMUSet(struct net_device *netdev, UINT8 Option, UINT8 GID, UINT8 Setindex, UINT16 * Stn)
#else
	int wlFwSetMUSet(struct net_device *netdev, UINT8 Option, UINT8 GID, UINT8 Setindex, UINT16 Stn1, UINT16 Stn2, UINT16 Stn3)
#endif
	{
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_SET_MU_SET *pCmd = (HostCmd_FW_SET_MU_SET *) & wlpptr->pCmdBuf[0];
		vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
		MIB_802DOT11 *mib;
		unsigned long flags;
		u16 result;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_SET_MU_SET));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->CmdHdr.macid = 1;
		pCmd->Option = Option;
		pCmd->GID = GID;
...
#ifdef SOC_W906X
	int wlFwSetMBSSIDSet(struct net_device *netdev, UINT8 Option, UINT8 groupid, UINT8 primary, UINT32 members) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_SET_MBSSID_SET *pCmd = (HostCmd_FW_SET_MBSSID_SET *) & wlpptr->pCmdBuf[0];
		unsigned long flags;
		u16 result;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_SET_MBSSID_SET));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->CmdHdr.macid = 1;
		pCmd->Option = Option;
	...
	int wlFwOBW16_11b(struct net_device *netdev, u8 Enable) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_OBW16_11B *pCmd = (HostCmd_FW_OBW16_11B *) & wlpptr->pCmdBuf[0];
		unsigned long flags;
		u16 result;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_OBW16_11B));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->CmdHdr.macid = 1;
		pCmd->Enable = Enable;
...
/* AIRTIME_FAIRNESS */
	...
	int wlFwAtfEnable(struct net_device *netdev, UINT8 enable) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_ATF_ENABLE *pCmd = (HostCmd_FW_ATF_ENABLE *) & wlpptr->pCmdBuf[0];
		unsigned long flags;	/* NOTE: Used for MWL_SPIN_LOCK */
		u16 result;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_ATF_ENABLE));
		pCmd->CmdHdr.macid = 1;
	...
	int wlFwAtfCfgSet(struct net_device *netdev, atf_info_t * atf_info) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_ATF_CFG_SET *pCmd = (HostCmd_FW_ATF_CFG_SET *) & wlpptr->pCmdBuf[0];
		unsigned long flags;	/* NOTE: Used for MWL_SPIN_LOCK */
		u16 result;
		...
		memset(pCmd, 0x00, sizeof(HostCmd_FW_ATF_CFG_SET));

		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_ATF_CFG_SET);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_ATF_CFG_SET));
		pCmd->CmdHdr.macid = 1;

		memcpy(&(pCmd->atf_info), atf_info, sizeof(atf_info_t));

		if (wlexecuteCommand(netdev, HostCmd_CMD_ATF_CFG_SET)) {
			WLDBG_EXIT_INFO(DBG_LEVEL_0, "failed execution");
			MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
			return FAIL;
		}
	...
	int wlFwAtfCfgReset(struct net_device *netdev) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_ATF_CFG_RESET *pCmd = (HostCmd_FW_ATF_CFG_RESET *) & wlpptr->pCmdBuf[0];
		unsigned long flags;	/* NOTE: Used for MWL_SPIN_LOCK */
		u16 result;
		...
		memset(pCmd, 0x00, sizeof(HostCmd_FW_ATF_CFG_RESET));

		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_ATF_CFG_RESET);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_ATF_CFG_RESET));
		pCmd->CmdHdr.macid = 1;

		if (wlexecuteCommand(netdev, HostCmd_CMD_ATF_CFG_RESET)) {
			WLDBG_EXIT_INFO(DBG_LEVEL_0, "failed execution");
			MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
			return FAIL;
		}
	...
	int wlFwAtfCfgGet(struct net_device *netdev, atf_info_t * atf_info) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_ATF_CFG_GET *pCmd = (HostCmd_FW_ATF_CFG_GET *) & wlpptr->pCmdBuf[0];
		unsigned long flags;	/* NOTE: Used for MWL_SPIN_LOCK */
		u16 result;
		...
		memset(pCmd, 0x00, sizeof(HostCmd_FW_ATF_CFG_GET));

		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_ATF_CFG_GET);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_ATF_CFG_GET));
		pCmd->CmdHdr.macid = 1;

		if (wlexecuteCommand(netdev, HostCmd_CMD_ATF_CFG_GET)) {
			WLDBG_EXIT_INFO(DBG_LEVEL_0, "failed execution");
			MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
			return FAIL;
		}

		memcpy(atf_info, &(pCmd->atf_info), sizeof(atf_info_t));
...
/* end of AIRTIME_FAIRNESS */
	...
	int wlFwDFSParams(struct net_device *netdev, UINT16 Action, UINT8 * fcc_min_radar_num_pri, UINT8 * etsi_min_radar_num_pri,
			  UINT8 * jpn_w53_min_radar_num_pri, UINT8 * jpn_w56_min_radar_num_pri, UINT8 * false_detect_th,
			  UINT8 * fcc_zc_error_th, UINT8 * etsi_zc_error_th, UINT8 * jp_zc_error_th, UINT8 * jpw53_zc_error_th) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_DFS_PARAMS *pCmd = (HostCmd_FW_DFS_PARAMS *) & wlpptr->pCmdBuf[0];
		u16 result;
		u8 i;
		unsigned long flags;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_DFS_PARAMS));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->CmdHdr.macid = wlpptr->vmacSta_p->VMacEntry.macId;
		pCmd->Action = ENDIAN_SWAP16(Action);
		for (i = 0; i < (sizeof(pCmd->fcc_min_radar_num_pri) / sizeof(u8)); i++) {
			pCmd->fcc_min_radar_num_pri[i] = fcc_min_radar_num_pri[i];
		}
		for (i = 0; i < (sizeof(pCmd->etsi_min_radar_num_pri) / sizeof(u8)); i++) {
			pCmd->etsi_min_radar_num_pri[i] = etsi_min_radar_num_pri[i];
		}
		for (i = 0; i < (sizeof(pCmd->jpn_w53_min_radar_num_pri) / sizeof(u8)); i++) {
			pCmd->jpn_w53_min_radar_num_pri[i] = jpn_w53_min_radar_num_pri[i];
		}
		for (i = 0; i < (sizeof(pCmd->jpn_w56_min_radar_num_pri) / sizeof(u8)); i++) {
			pCmd->jpn_w56_min_radar_num_pri[i] = jpn_w56_min_radar_num_pri[i];
		}
		...
		pCmd->jpw53_zc_error_th = *jpw53_zc_error_th;
		...
		if (Action == DFS_GET_ALL) {
			for (i = 0; i < (sizeof(pCmd->fcc_min_radar_num_pri) / sizeof(u8)); i++) {
				fcc_min_radar_num_pri[i] = pCmd->fcc_min_radar_num_pri[i];
			}
			for (i = 0; i < (sizeof(pCmd->etsi_min_radar_num_pri) / sizeof(u8)); i++) {
				etsi_min_radar_num_pri[i] = pCmd->etsi_min_radar_num_pri[i];
			}
			for (i = 0; i < (sizeof(pCmd->jpn_w53_min_radar_num_pri) / sizeof(u8)); i++) {
				jpn_w53_min_radar_num_pri[i] = pCmd->jpn_w53_min_radar_num_pri[i];
			}
			for (i = 0; i < (sizeof(pCmd->jpn_w56_min_radar_num_pri) / sizeof(u8)); i++) {
				jpn_w56_min_radar_num_pri[i] = pCmd->jpn_w56_min_radar_num_pri[i];
			}
			...
			*jpw53_zc_error_th = pCmd->jpw53_zc_error_th;
		}
	...
	int wlFwDFSParamsLog(struct net_device *netdev, UINT16 Action, dfs_log_t * pLog) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_DFS_PARAMS *pCmd = (HostCmd_FW_DFS_PARAMS *) & wlpptr->pCmdBuf[0];
		unsigned long flags;
		...
		memcpy((void *)pLog, (void *)&pCmd->data, sizeof(dfs_log_t));

		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
		return SUCCESS;
	}

	int wlFwGetTLVSet(struct net_device *netdev, UINT8 act, UINT16 type, UINT16 len, UINT8 * tlvData, char *string_buff) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		unsigned long flags;
		int retval = FAIL;
		int i;

		HostCmd_FW_TLV_CONFIG *pCmd = (HostCmd_FW_TLV_CONFIG *) & wlpptr->pCmdBuf[0];

#ifdef WLS_FTM_SUPPORT
		if (type < FTM_MAX_CONFIG) {
			extern void wlsFTM_configureDevice(struct net_device *netdev, UINT8 act, UINT16 type, UINT16 len, UINT8 * tlvData,
							   char *string_buff);
			wlsFTM_configureDevice(netdev, act, type, len, tlvData, string_buff);
			return SUCCESS;
		}
#endif
		MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_TLV_CONFIG));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		if (act) {
			pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_SET_TLV_SET);
			pCmd->TlvType = ENDIAN_SWAP16(type);
			pCmd->TlvLen = ENDIAN_SWAP16(len);
			memcpy(&pCmd->TlvData, tlvData, len);
#ifdef PRD_CSI_DMA
			if (type == 12) {
				wlpptr->smacCfgAddr->prd_csi_dma_ddr_addr = (UINT32) wlpptr->wlpd_p->pPhysSsuBuf;
			}
#endif
			retval = wlexecuteCommand(netdev, HostCmd_CMD_SET_TLV_SET);
		} else {
			pCmd->TlvType = ENDIAN_SWAP16(type);

			if (type == 20)
				memcpy(&pCmd->TlvData, tlvData, len);
			retval = wlexecuteCommand(netdev, HostCmd_CMD_GET_TLV_SET);
		}
		if (retval) {
			WLDBG_EXIT_INFO(DBG_LEVEL_0, "failed execution");
			MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
		...
		if (!act) {
			...
			if (type == 3) {
				...
				AoA_Angle_INT = (SINT16) ((pCmd->TlvData[3] << 8) | (pCmd->TlvData[4]));
				...
				sprintf(string_buff, "\n%d.%02d m\n%d.%02d degrees\nMAC: %02x.%02x.%02x.%02x.%02x.%02x",
					WLS_Distance_INT, WLS_Distance_DEC, AoA_Angle_INT, AoA_Angle_DEC,
					pCmd->TlvData[6], pCmd->TlvData[7], pCmd->TlvData[8], pCmd->TlvData[9], pCmd->TlvData[10], pCmd->TlvData[11]);

				filp_wls = filp_open("/tmp/Distance_Output", O_RDWR | O_CREAT | O_TRUNC, 0);
				if (!IS_ERR(filp_wls)) {
					__kernel_write(filp_wls, string_buff, strlen(string_buff), &filp_wls->f_pos);
				...
				// WLS_Distance_INT = (pCmd->TlvData[0] << 8) | (pCmd->TlvData[1]);
				// WLS_Distance_DEC = pCmd->TlvData[2];
				AoA_Angle_INT = (SINT16) ((pCmd->TlvData[10] << 8) | (pCmd->TlvData[11]));
				AoA_Angle_DEC = pCmd->TlvData[12];
				Timestamp = (pCmd->TlvData[17] << 24) | (pCmd->TlvData[16] << 16) | (pCmd->TlvData[15] << 8) | (pCmd->TlvData[14]);
				LOS_Factor1 = (pCmd->TlvData[23] << 8) | (pCmd->TlvData[22]);
				LOS_Factor2 = (pCmd->TlvData[25] << 8) | (pCmd->TlvData[24]);
				// Pass back string to buffer for printing out at user space
				// Print back MAC address of distance / angle data
				sprintf(string_buff,
					"\n%d.%02d m\n%d.%01d degrees\n%d count\nTimestamp: %d\nRSSI_A: -%d dBm\nRSSI_B: -%d dBm\nRSSI_C: -%d dBm\nRSSI_D: -%d dBm\nLOS_Factor1: 0.%04d\nLOS_Factor2: 0.%04d\nMAC: %02x.%02x.%02x.%02x.%02x.%02x\nPacket_Type: %02x\nPacket_Subtype: %02x\n",
					WLS_Distance_INT, WLS_Distance_DEC, AoA_Angle_INT, AoA_Angle_DEC, pCmd->TlvData[13], Timestamp,
					pCmd->TlvData[18], pCmd->TlvData[19], pCmd->TlvData[20], pCmd->TlvData[21], LOS_Factor1, LOS_Factor2,
					pCmd->TlvData[2], pCmd->TlvData[3], pCmd->TlvData[4], pCmd->TlvData[5], pCmd->TlvData[6], pCmd->TlvData[7],
				...
				printk
				    ("AoA Parameters - User#, Enable, MAC_Address[6], Packet_Type, Packet_Subtype, AoA_Angle[3], Detection_Count\n");
 ...
 out:
	...
	int wlFwNewDP_eeprom(struct net_device *netdev, UINT32 offset, UINT8 * data, UINT32 len, UINT16 action) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		unsigned long flags;
		int retval = FAIL;

		HostCmd_FW_EEPROM_CONFIG *pCmd = (HostCmd_FW_EEPROM_CONFIG *) & wlpptr->pCmdBuf[0];

		MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);
		memset(pCmd, 0x00, sizeof(HostCmd_FW_EEPROM_CONFIG));
		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_EEPROM_SET);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_EEPROM_CONFIG));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		...
		if (pCmd->status == SUCCESS) {
			if (action == HostCmd_ACT_GEN_READ) {
				printk("EEPROM: read len=%d data=%02x %02x %02x %02x ...\n", (int)len, pCmd->data[0],
				       pCmd->data[1], pCmd->data[2], pCmd->data[3]);
	...
	int wlFwNewDP_EEPROM_access(struct net_device *netdev, UINT32 action) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_EEPROM_ACCESS *pCmd = (HostCmd_FW_EEPROM_ACCESS *) & wlpptr->pCmdBuf[0];
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_EEPROM_ACCESS));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->Action = ENDIAN_SWAP32(action);	//1: lock, 0: unlock
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_EEPROM_ACCESS));
...
/*Function to set antenna bitmap and trpc id derived from power for off channel*/
	int wlFwNewDP_Set_Offchanpwr(struct net_device *netdev, SINT8 pwr, UINT8 bitmap, UINT8 channel) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_NEWDP_OFFCHANNEL_PWR *pCmd = (HostCmd_FW_NEWDP_OFFCHANNEL_PWR *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_NEWDP_OFFCHANNEL_PWR));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->Pwr = pwr;
		pCmd->AntBitMap = bitmap;
		pCmd->Channel = channel;
		printk("pwr %d, bitmap %d, channel %d \n", pCmd->Pwr, pCmd->AntBitMap, pCmd->Channel);
		WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_FW_NEWDP_OFFCHANNEL_PWR));
		retval = wlexecuteCommand(netdev, HostCmd_CMD_NEWDP_OFFCHAN_PWR);
		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
		return retval;
	}
	...
	int wlFwGetSysLoad(struct net_device *netdev, radio_cpu_load_t * sys_load) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_GET_SYSLOAD *pCmd = (HostCmd_FW_GET_SYSLOAD *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_GET_SYSLOAD));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->CmdHdr.macid = 1;
		...
		if (pCmd->CmdHdr.Result == HostCmd_RESULT_OK) {
			memcpy((UINT8 *) sys_load, (UINT8 *) & pCmd->sysLoad, sizeof(radio_cpu_load_t));
			retval = SUCCESS;
		}
	...
	int wlFwSendFipsTest(struct net_device *netdev, UINT32 encdec, UINT32 alg,
			     DataEntry_t * pKey, DataEntry_t * pNounce, DataEntry_t * pAAD, DataEntry_t * pInData, DataEntry_t * pOutput) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		int len, retval = FAIL;
		UINT16 tmpLen;
		UINT8 *ptr;
		HostCmd_FIPS_TEST *pCmd = (HostCmd_FIPS_TEST *) & wlpptr->pCmdBuf[0];
		unsigned long flags;
		UINT8 *pOutLocation;
		...
		WLDBG_INFO(DBG_LEVEL_0, "wlFwSendFipsTest Command Request :::\n");
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, ENDIAN_SWAP16(pCmd->CmdHdr.Length));
		...
		WLDBG_INFO(DBG_LEVEL_0, "wlFwSendFipsTest Command Response :::\n");
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, ENDIAN_SWAP16(pCmd->CmdHdr.Length));
		...
		if (pCmd->CmdHdr.Result == HostCmd_RESULT_OK) {
			...
			memcpy(pOutput->Data, (pOutLocation + sizeof(tmpLen)), pOutput->Length);
			retval = SUCCESS;
		}
...
#define TEST_REC   8
	...
	UINT8 algorithm[TEST_REC] = { EncrTypeAes, EncrTypeCcmp256, EncrTypeGcmp128, EncrTypeGcmp256,
		EncrTypeAes, EncrTypeCcmp256, EncrTypeGcmp128, EncrTypeGcmp256
	};
	UINT8 algStrDef[TEST_REC][14] = { "aes-ccmp", "aes-ccmp-256", "aes-gcmp", "aes-gcmp-256",
		"aes-ccmp", "aes-ccmp-256", "aes-gcmp", "aes-gcmp-256"
	};
	DataEntry_t Key[TEST_REC] = { {16, {0xC6, 0x7E, 0x81, 0x6B, 0x4B, 0xFB, 0xE2, 0xFB,
	...
	DataEntry_t Nounce[TEST_REC] = { {13, {0x01, 0xBF, 0x31, 0xDE, 0x56, 0x72, 0x0F, 0x47,
	...
	DataEntry_t AAD[TEST_REC] = { {22, {0xEA, 0x56, 0x13, 0x7B, 0xD2, 0x85, 0xA1, 0xD8,
	...
	DataEntry_t expData[TEST_REC] = { {10, {0xF2, 0x40, 0xC9, 0x34, 0x89, 0x8B, 0xB4, 0xEB,
						0x7C, 0x27}
					   }
	,
	{36, {0x6d, 0x15, 0x5d, 0x88, 0x32, 0x66, 0x82, 0x56,
	...
	int wlFwSendFipsTestAll(struct net_device *netdev) {
		...
		for (i = 0; i < TEST_REC; i++) {
			status = wlFwSendFipsTest(netdev, encdec[i], algorithm[i], &Key[i], &Nounce[i], &AAD[i], &InData[i], &OutData);

			if (memcmp(OutData.Data, expData[i].Data, expData[i].Length)) {
				status |= 1 << i;
				printk("FIPS Failed on %s algorithm %s\n", encdec[i] == 0 ? "dec" : "enc", algStr(i));
			} else
	...
	int wlFwSetResetRateMode(struct net_device *netdev, u_int8_t ResetRateMode) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_SET_RESET_RATE_MODE *pCmd = (HostCmd_FW_SET_RESET_RATE_MODE *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;

		WLDBG_ENTER_INFO(DBG_LEVEL_0, "Reset Rate Mode = %d", ResetRateMode);

		MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);
		memset(pCmd, 0x00, sizeof(HostCmd_FW_SET_RESET_RATE_MODE));
		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_SET_RESET_RATE_MODE);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_SET_RESET_RATE_MODE));
		pCmd->CmdHdr.macid = wlpptr->vmacSta_p->VMacEntry.macId;
		pCmd->ResetRateMode = ResetRateMode;

		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_FW_SET_RESET_RATE_MODE));

		retval = wlexecuteCommand(netdev, HostCmd_CMD_SET_RESET_RATE_MODE);
		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
		return retval;
	}
	...
	int wlFwSetRateUpdateTicks(struct net_device *netdev, UINT32 * n_ticks, UINT8 is_set) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_FW_GEN_U32_ACCESS *pCmd = (HostCmd_FW_FW_GEN_U32_ACCESS *) & wlpptr->pCmdBuf[0];
		unsigned long flags;

		WLDBG_ENTER(DBG_LEVEL_0);
		MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);

		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HOSTCMD_CMD_SET_RATE_UPDATE_TICKS);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_FW_GEN_U32_ACCESS));
		pCmd->CmdHdr.macid = 1;
		pCmd->val = *n_ticks;
		pCmd->set = is_set;
	...
	int wlFwSetMcastCtsToSelf(struct net_device *netdev, u8 * enable) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_SET_MCAST_CTS_TO_SELF *pCmd = (HostCmd_SET_MCAST_CTS_TO_SELF *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		...
		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_SET_MCAST_CTS_TO_SELF);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_SET_MCAST_CTS_TO_SELF));
		pCmd->enable = ENDIAN_SWAP32(*enable);

		retval = wlexecuteCommand(netdev, HostCmd_CMD_SET_MCAST_CTS_TO_SELF);
		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
		if (retval != SUCCESS)
	...
	int wlFwGetQBSSLoad(struct net_device *netdev, UINT8 * ch_util, UINT16 * sta_cnt) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_GET_CCA_Busy_Fract *pCmd = (HostCmd_FW_GET_CCA_Busy_Fract *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_GET_CCA_Busy_Fract));

		WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_FW_GET_CCA_Busy_Fract));
		retval = wlexecuteCommand(netdev, HostCmd_CMD_CCA_BUSY_FRACTION);
		if (!retval) {
...
#include "ap8xLnxSwMimo.h"
	...
	int wlDspCmd(struct net_device *netdev, UINT8 index, UINT8 priority, UINT32 * result) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_DSP_CMD *pCmd = (HostCmd_DSP_CMD *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		...
		pCmd->ptrDstData = wlpptr->wlpd_p->pPhysDspBuf + DSP_BUF_SIZE - 0x1000;
		...
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, sizeof(HostCmd_DSP_CMD));
	...
	int wlDspTrig(struct net_device *netdev, UINT8 index, UINT8 priority, UINT8 muGID, UINT8 numUsers, UINT8 pkttype) {
		...
		// TBD, compose command
		pCmd->ptrSrcData = wlpptr->wlpd_p->pPhysDspBuf;
		pCmd->ptrDstData = wlpptr->wlpd_p->pPhysDspBuf + DSP_CMD_BUF_SIZE - 0x1000;

		pCmd->srcDataLen = createDspData((U8 *) wlpptr->pDspBuf, muGID, numUsers, pkttype);
	...
	int wlFwGetRadioStatus(struct net_device *netdev, mvl_status_t * radio_status) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_GET_MVL_RADIO_STATUS *pCmd = (HostCmd_FW_GET_MVL_RADIO_STATUS *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;

		MWL_SPIN_LOCK(&wlpptr->wlpd_p->locks.fwLock);
		memset(pCmd, 0x00, sizeof(HostCmd_FW_GET_MVL_RADIO_STATUS));
		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_GET_MVL_RADIO_STATUS);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_GET_MVL_RADIO_STATUS));

		WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_FW_GET_MVL_RADIO_STATUS));
		retval = wlexecuteCommand(netdev, HostCmd_CMD_GET_MVL_RADIO_STATUS);
		if (!retval) {
			memcpy(radio_status, &pCmd->radio_status, sizeof(mvl_status_t));
		}
		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
		return retval;
	}
	...
	int wlFwSetBeamChange(struct net_device *netdev, u8 enable) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_SET_BEAM_CHANGE *pCmd = (HostCmd_FW_SET_BEAM_CHANGE *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_SET_BEAM_CHANGE));
		pCmd->enable = enable;
	...
	int wlFwSendFrame(struct net_device *netdev, UINT16 staIdx, UINT8 reportId, UINT8 tid,
		...
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_DS_TX_FRAME_TEST *pCmd = (HostCmd_DS_TX_FRAME_TEST *) & wlpptr->pCmdBuf[0];
		unsigned long flags;
		...
		payloadLen = (UINT16) min((u_int32_t) payloadLen, (u_int32_t) (MAX_TX_FRAME_LEN - machdrLen));
		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_TX_FRAME_TEST);
		pCmd->CmdHdr.macid = wlpptr->vmacSta_p->VMacEntry.macId;
		pCmd->status = 0;
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->CmdHdr.Length = sizeof(HostCmd_DS_TX_FRAME_TEST) - sizeof(pCmd->data);
		...
		WLDBG_INFO(DBG_LEVEL_0, "wlFwSendTxTest Command Response :::\n");
		WLDBG_DUMP_DATA(DBG_LEVEL_0, (void *)pCmd, ENDIAN_SWAP16(pCmd->CmdHdr.Length));
	...
	int wlFwGetTsf(struct net_device *netdev, tsf_info_t * ptsf) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_GET_TSF *pCmd = (HostCmd_FW_GET_TSF *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		...
		WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_FW_GET_TSF));
		retval = wlexecuteCommand(netdev, HostCmd_CMD_PARENT_TSF);

		if (!retval) {
			memcpy((void *)ptsf, (void *)&pCmd->tsfInfo, sizeof(tsf_info_t));
		}
...
// New added CB ++
	...
	int wlFwSetApCBMode(struct net_device *netdev, u8 mode) {
		...
		HostCmd_AP_CBMODE *pCmd = (HostCmd_AP_CBMODE *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		u8 macid = wlpptr->vmacSta_p->VMacEntry.macId;
		...
		WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_AP_CBMODE));
		...
		parent_wlpptr->bssBasePtr = pCmd->bssBasePtr;
		parent_wlpptr->txqBasePtr = pCmd->txqBasePtr;
		parent_wlpptr->staBasePtr = pCmd->staBasePtr;
		printk("=> bcnBasePtr: %x\n", pCmd->bcnBasePtr);
		printk("=> bssBasePtr: %x\n", pCmd->bssBasePtr);
		printk("=> txqBasePtr: %x\n", pCmd->txqBasePtr);
		printk("=> staBasePtr: %x\n", pCmd->staBasePtr);
	...
	int wlFwSetStaCBNoAck(struct net_device *netdev, u8 staid, u8 mode) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_STA_CB_NOACK *pCmd = (HostCmd_STA_CB_NOACK *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		u8 macid = wlpptr->vmacSta_p->VMacEntry.macId;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_STA_CB_NOACK));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		...
		WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_STA_CB_NOACK));
	...
	int wlFwGetStaCBParam(struct net_device *netdev, HostCmd_STA_CB_PARAMS_SYNC * psta_cb_param) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_STA_CB_PARAMS_SYNC *pCmd = (HostCmd_STA_CB_PARAMS_SYNC *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		u8 macid = wlpptr->vmacSta_p->VMacEntry.macId;
		...
		pCmd->CmdHdr.macid = macid;
		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_GET_STA_CB_PARAMS_SYNC);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_STA_CB_PARAMS_SYNC));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);

		pCmd->staid = psta_cb_param->staid;

		WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_STA_CB_PARAMS_SYNC));
		retval = wlexecuteCommand(netdev, HostCmd_CMD_GET_STA_CB_PARAMS_SYNC);

		if (!retval) {
			//memcpy(psn, pCmd->sn, sizeof(u16)*8);
			memcpy(psta_cb_param, pCmd, sizeof(HostCmd_STA_CB_PARAMS_SYNC));
	...
	int wlFwSetStaCBParam(struct net_device *netdev, HostCmd_STA_CB_PARAMS_SYNC * psta_cb_param) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_STA_CB_PARAMS_SYNC *pCmd = (HostCmd_STA_CB_PARAMS_SYNC *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		...
		pCmd->CmdHdr.macid = macid;
		pCmd->CmdHdr.Cmd = ENDIAN_SWAP16(HostCmd_CMD_SET_STA_CB_PARAMS_SYNC);
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_STA_CB_PARAMS_SYNC));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		...
		WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_STA_CB_PARAMS_SYNC));
		retval = wlexecuteCommand(netdev, HostCmd_CMD_SET_STA_CB_PARAMS_SYNC);
...
//twt
	...
	int wlFwTwtParam(struct net_device *netdev, UINT8 action, UINT8 * mac, UINT8 agid, twt_param_t * param) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_DS_TWT_PARAM *pCmd = (HostCmd_FW_DS_TWT_PARAM *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_DS_TWT_PARAM));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		...
		if (action == WL_SET && param) {
			memcpy((void *)&pCmd->twtparam, (void *)param, sizeof(twt_param_t));
		}

		WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_FW_DS_TWT_PARAM));
		...
		if (!retval) {
			...
			if (action == WL_GET && param) {
				memcpy((void *)param, (void *)&pCmd->twtparam, sizeof(twt_param_t));
			}
#ifdef AP_TWT
			...
			if (wfa_twt_rxing_flag_addr == 0) {
				...
				if ((pCmd->IsTWTRxRunningAddr & 0xff000000) == 0x20000000) {
					wfa_twt_rxing_flag_addr = (pCmd->IsTWTRxRunningAddr) & 0xFFFFFF;
...
#else
				wfa_twt_rxing_flag_addr = pCmd->IsTWTRxRunningAddr;
				printk("[TWT]: WFA PF Rx Monitor flag Addr:0x%x\n", pCmd->IsTWTRxRunningAddr);
#endif
			}
#endif
...
/*Function to set spatial reuse param*/
	int wlFw_SetSR(struct net_device *netdev, UINT8 enable, SINT8 thresNonSrg, SINT8 thresSrg, UINT8 action, UINT8 * buf) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_SET_SR *pCmd = (HostCmd_FW_SET_SR *) & wlpptr->pCmdBuf[0];
		int retval = FAIL;
		unsigned long flags;
		...
		if (action == HAL_SFW_SR_SET_TESTMODE) {
			memcpy((U8 *) pCmd->macaddr, (U8 *) buf, 6);
			printk("enable flag:%d, %02x:%02x:%02x:%02x:%02x:%02x\n", pCmd->srEnable,
			       pCmd->macaddr[0], pCmd->macaddr[1], pCmd->macaddr[2], pCmd->macaddr[3], pCmd->macaddr[4], pCmd->macaddr[5]);
		} else if (action == HAL_SFW_SR_SET_SRPARAM) {
			printk("enable flag:%d, non-SRG thres %d, SRG thres %d\n", pCmd->srEnable, pCmd->thresNonSrg, pCmd->thresSrg);
		}

		WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_FW_SET_SR));

		retval = wlexecuteCommand(netdev, HostCmd_CMD_SR_PARAM);
		if (!retval) {
			if ((action == HAL_SFW_SR_GET_RSSI) && (buf != NULL)) {
				memcpy((U8 *) buf, (U8 *) & pCmd->intfRssiLog, (sizeof(pCmd->intfRssiLog[0]) * SR_RSSI_LOG_SIZE));
			}
		}
		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
		return retval;
	...
	int wlFwGetStaStats(struct net_device *netdev, int staId, SMAC_STA_STATISTICS_st * sta_stats) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_GET_STA_STATS *pCmd = (HostCmd_FW_GET_STA_STATS *) wlpptr->pCmdBuf;
		int retval = FAIL;
		unsigned long flags;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_GET_STA_STATS));
		pCmd->StaId = staId % sta_num;

		WLDBG_DUMP_DATA(DBG_LEVEL_1, (void *)pCmd, sizeof(HostCmd_FW_GET_STA_STATS));
		retval = wlexecuteCommand(netdev, HostCmd_CMD_GET_STA_STATS);
		if (!retval) {
			memcpy(sta_stats, &pCmd->StaStats, sizeof(SMAC_STA_STATISTICS_st));
		}
		MWL_SPIN_UNLOCK(&wlpptr->wlpd_p->locks.fwLock);
		return retval;
	}

	int wlFwSetOFDMASet(struct net_device *netdev, UINT8 enable, UINT8 sta_count, UINT16 * Stn) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_SET_OFDMA_SET *pCmd = (HostCmd_FW_SET_OFDMA_SET *) & wlpptr->pCmdBuf[0];
		vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
		MIB_802DOT11 *mib;
		unsigned long flags;
		u16 result;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_SET_OFDMA_SET));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->CmdHdr.macid = 1;
		pCmd->Option = enable;
	...
	int wlFwSetULMUSet(struct net_device *netdev, UINT8 Action, UINT32 RateInfo, UINT32 Flag, UINT8 GID, UINT8 Mode, UINT8 BandWidth,
			   UINT8 StaNum, ul_stnid_ru_t * StaList) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_SET_ULMU_SET *pCmd = (HostCmd_FW_SET_ULMU_SET *) & wlpptr->pCmdBuf[0];
		vmacApInfo_t *vmacSta_p = wlpptr->vmacSta_p;
		MIB_802DOT11 *mib;
		u16 result;
		unsigned long flags;
		...
		mib = vmacSta_p->Mib802dot11;
		if ((mib->UL_mimo_enable == 0) && Action == HostCmd_ACT_GEN_SET && Mode == 1) {
			/* Mode == 1 is MIMO */
			return SUCCESS;
		} else if ((mib->UL_ofdma_enable == 0) && Action == HostCmd_ACT_GEN_SET && Mode == 2) {
			/* Mode == 2 is OFDMA */
			return SUCCESS;
		}
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_SET_ULMU_SET) - (sizeof(ul_stnid_ru_t) * (MU_MAX_USERS - StaNum)));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->CmdHdr.macid = wlpptr->vmacSta_p->VMacEntry.macId;
		pCmd->Action = Action;
		pCmd->Version = 0;
		...
		if (Action == HostCmd_ACT_GEN_SET) {
			memcpy(pCmd->StaList, StaList, sizeof(ul_stnid_ru_t) * (StaNum));
		}
	...
	int wlFwSetAcntWithMu(struct net_device *netdev, UINT16 Action) {
		struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
		HostCmd_FW_SET_ANCT_WITH_MU *pCmd = (HostCmd_FW_SET_ANCT_WITH_MU *) & wlpptr->pCmdBuf[0];
		u16 result;
		unsigned long flags;
		...
		pCmd->CmdHdr.Length = ENDIAN_SWAP16(sizeof(HostCmd_FW_SET_ANCT_WITH_MU));
		pCmd->CmdHdr.SeqNum = GET_CMD_SEQ_NUM(wlpptr);
		pCmd->CmdHdr.macid = wlpptr->vmacSta_p->VMacEntry.macId;
		pCmd->Action = ENDIAN_SWAP16(Action);
