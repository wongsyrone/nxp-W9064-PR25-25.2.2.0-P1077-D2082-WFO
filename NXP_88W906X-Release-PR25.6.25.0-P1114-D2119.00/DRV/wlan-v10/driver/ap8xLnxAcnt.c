...
extern int wlFwNewDP_setAcntBufSize(struct net_device *netdev, u_int32_t * base, u_int32_t size, u_int32_t ActionType);
#else
extern int wlFwNewDP_setAcntBufSize(struct net_device *netdev, u_int32_t base, u_int32_t size);
#endif
#ifdef SOC_W906X
extern int wlFwGetRateTable(struct net_device *netdev, UINT8 * addr, UINT8 * pRateInfo, UINT32 size, UINT8 type, UINT16 staid);
#else
...
void dump_acnt_internal_stat(u32 entry)
{
	...
	memcpy(&(acnt_internal_stat_dump[0]), &(acnt_internal_stat[0]), sizeof(acnt_tx3_t) * NUM_INTERNAL_STAT);
	for (i = 0; i < entry; i++) {
		pos--;
		if (pos < 0) {
			pos = NUM_INTERNAL_STAT - 1;
		}
		print_hex_dump(KERN_INFO, "", DUMP_PREFIX_OFFSET, 32, 4, &(acnt_internal_stat_dump[pos]), sizeof(acnt_tx3_t), false);
	}
...
int wlAcntSetBufSize(struct net_device *netdev, SetAcntBufInfo_t * SetInfo)
{
	...
	if (SetInfo->ActionType == ACNT_PAUSE) {
...
#ifdef SOC_W906X
			wl_dma_free_coherent(wlpptr->wlpd_p->dev, oldSize / ACNT_NCHUNK,
					     wlpptr->wlpd_p->mmap_ACNTChunk[i].data, wlpptr->wlpd_p->descData[0].pPhysAcntRing[i]);
#else
			pci_free_consistent(wlpptr->pPciDev, oldSize / ACNT_NCHUNK, wlpptr->wlpd_p->mmap_ACNTChunk[i].data,
		...
		for (i = 0; i < ACNT_NCHUNK; i++) {
#ifdef SOC_W906X
			mem = wl_dma_alloc_coherent(wlpptr->wlpd_p->dev, size / ACNT_NCHUNK,
						    &wlpptr->wlpd_p->descData[0].pPhysAcntRing[i], wlpptr->wlpd_p->dma_alloc_flags);
#else
			mem = (UINT8 *) pci_alloc_consistent(wlpptr->pPciDev, size / ACNT_NCHUNK, &wlpptr->wlpd_p->descData[0].pPhysAcntRing[i]);
#endif
			if (mem == NULL) {
				UINT8 j;
				...
				for (j = 0; j < i; j++) {
#ifdef SOC_W906X
					wl_dma_free_coherent(wlpptr->wlpd_p->dev, size / ACNT_NCHUNK, (void *)memTmp[j],
							     wlpptr->wlpd_p->descData[0].pPhysAcntRing[i]);
#else
					pci_free_consistent(wlpptr->pPciDev, size / ACNT_NCHUNK, memTmp[j],
							    &wlpptr->wlpd_p->descData[0].pPhysAcntRing[i]);
#endif
				}
				bAllocFailed = TRUE;
		...
		head = readl(wlpptr->ioBase1 + reg_acnt_head);

		if (bAllocFailed) {
			//back to oldsize
			writel(head & (oldSize / ACNT_NCHUNK - 1), wlpptr->ioBase1 + reg_acnt_tail);
			writel(head & (oldSize / ACNT_NCHUNK - 1), wlpptr->ioBase1 + reg_acnt_head);
			for (i = 0; i < ACNT_NCHUNK; i++) {
#ifdef SOC_W906X
				mem = (UINT8 *) wl_dma_alloc_coherent(wlpptr->wlpd_p->dev,
								      oldSize / ACNT_NCHUNK,
								      &wlpptr->wlpd_p->descData[0].pPhysAcntRing[i], wlpptr->wlpd_p->dma_alloc_flags);
#else
				mem = (UINT8 *) pci_alloc_consistent(wlpptr->pPciDev,
								     oldSize / ACNT_NCHUNK, &wlpptr->wlpd_p->descData[0].pPhysAcntRing[i]);
#endif
				wlpptr->wlpd_p->mmap_ACNTChunk[i].data = mem;
			}
			wlpptr->wlpd_p->AcntChunkInfo.SizeOfChunk = oldSize / ACNT_NCHUNK;
			wlpptr->wlpd_p->descData[0].AcntRingSize = oldSize;
			wlFwNewDP_setAcntBufSize(netdev, (u_int32_t *) & wlpptr->wlpd_p->descData[0].pPhysAcntRing[0], oldSize, SetInfo->ActionType);
			return 0;
		} else {
			writel(head & (size / ACNT_NCHUNK - 1), wlpptr->ioBase1 + reg_acnt_tail);
			writel(head & (size / ACNT_NCHUNK - 1), wlpptr->ioBase1 + reg_acnt_head);
			wlpptr->wlpd_p->AcntChunkInfo.SizeOfChunk = size / ACNT_NCHUNK;
			wlpptr->wlpd_p->descData[0].AcntRingSize = size;
			wlFwNewDP_setAcntBufSize(netdev, (u_int32_t *) & wlpptr->wlpd_p->descData[0].pPhysAcntRing[0], size, SetInfo->ActionType);
			return size;
		}
...
int wlAcntSetBufSize(struct net_device *netdev, u_int32_t size)
{
	...
	wlpptr->wlpd_p->descData[0].pAcntRing = (UINT8 *) pci_alloc_consistent(wlpptr->pPciDev, size, &wlpptr->wlpd_p->descData[0].pPhysAcntRing);
	wlpptr->wlpd_p->ACNTmemInfo.data = wlpptr->wlpd_p->descData[0].pAcntRing;
	wlpptr->wlpd_p->ACNTmemInfo.dataPhysicalLoc = wlpptr->wlpd_p->descData[0].pPhysAcntRing;

	if (wlpptr->wlpd_p->descData[0].pAcntRing == NULL) {
		WLDBG_ERROR(DBG_LEVEL_12, "** newTestDP: Can not alloc acnt ring\n");
		return 0;
	}
	printk("acnt size = %d\n", size);
	wlpptr->wlpd_p->descData[0].AcntRingSize = size;

	wlFwNewDP_setAcntBufSize(netdev, wlpptr->wlpd_p->descData[0].pPhysAcntRing, size);
...
*/
void wltxRateDecode(dbRateInfo_t * pRateInfo, UINT32 ratetblindex, WLAN_TX_RATE_HIST * pRH, UINT8 type, UINT32 txcnt)
{
	...
	if (ratetblindex != 0xff) {
		if (type == MU_MIMO) {
			...
			if ((Nss_11ac < (QS_NUM_SUPPORTED_11AC_NSS - 1)) && (Rate_11ac < QS_NUM_SUPPORTED_11AC_MCS)) {
				pRH->MU_rate[Nss_11ac][pRateInfo->Bandwidth][pRateInfo->ShortGI][Rate_11ac].rateinfo = *(UINT32 *) pRateInfo;
				pRH->MU_rate[Nss_11ac][pRateInfo->Bandwidth][pRateInfo->ShortGI][Rate_11ac].cnt++;
			...
			if ((histo_p[ratetblindex].rateinfo & ratemask) == (*(UINT32 *) pRateInfo & ratemask)) {
		...
		if (type == MU_MIMO) {
			...
			for (i = 0; i < TX_RATE_HISTO_CUSTOM_CNT; i++) {
				if ((histo_p[i].rateinfo == 0) || ((histo_p[i].rateinfo & ratemask) == (*(UINT32 *) pRateInfo & ratemask))) {
					found = 1;
					break;
				}
			}
...
void wltxRatePERDecode(dbRateInfo_t * pRateInfo, UINT32 ratetblindex, UINT8 PER, WLAN_TX_RATE_HIST * pRH, UINT8 type)
{
...
#ifndef SOC_W906X
	if ((type == SU_MIMO) && (ratetblindex >= RATE_ADAPT_MAX_SUPPORTED_RATES) && (ratetblindex != 0xFF))
		return;
#endif
	...
	if (ratetblindex != 0xff) {
		...
		if (type == MU_MIMO) {
			...
			if ((Nss_11ac < (QS_NUM_SUPPORTED_11AC_NSS - 1)) && (Rate_11ac < QS_NUM_SUPPORTED_11AC_MCS)) {
				pRH->MU_rate[Nss_11ac][pRateInfo->Bandwidth][pRateInfo->ShortGI][Rate_11ac].rateinfo = *(UINT32 *) pRateInfo;
				pRH->MU_rate[Nss_11ac][pRateInfo->Bandwidth][pRateInfo->ShortGI][Rate_11ac].per[PER_index]++;
			}
...
#ifdef NEWDP_ACNT_BA
void wltxBADecode(WLAN_TX_BA_HIST * pBAStats, acnt_BA_stats_t * pBA)
{
	if ((!pBAStats->StatsEnable) || (pBAStats->pBAStats == NULL) || (pBA == NULL))
		return;

	if ((pBAStats->Index < ACNT_BA_SIZE) && (pBAStats->Stnid == pBA->StnId) && (pBAStats->Type == pBA->Type)) {
		pBAStats->pBAStats[pBAStats->Index].BAHole = pBA->BAhole;
		pBAStats->pBAStats[pBAStats->Index].BAExpected = pBA->BAexpected;
...
void wl_BF_Mimo_Ctrl_Field_Log_Decode(acnt_BF_Mimo_Ctrl_Field_Log_t * pBF_Mimo_Ctrl_Field_Log_Data)
{
	...
	sprintf(string_buff, "\nMAC: %02x.%02x.%02x.%02x.%02x.%02x\nSU_0_MU_1: %d\nMIMO_Ctrl_Field: 0x%x\nComp_BF_Report_First_8Bytes: 0x%llx\n",
		...
		pBF_Mimo_Ctrl_Field_Log_Data->Type, pBF_Mimo_Ctrl_Field_Log_Data->MIMO_Ctrl_Field, pBF_Mimo_Ctrl_Field_Log_Data->Comp_BF_Rep_8Bytes);

	filp_BF_MCF_Data = filp_open("/tmp/BF_MIMO_Ctrl_Field_Output.txt", O_RDWR | O_CREAT | O_TRUNC, 0);
	if (!IS_ERR(filp_BF_MCF_Data)) {
		__kernel_write(filp_BF_MCF_Data, string_buff, strlen(string_buff), &filp_BF_MCF_Data->f_pos);
	...
	case acnt_code_tx_enqueue:
		{
			...
			if ((acnt_tx_record = (acnt_tx_t *) wl_kmalloc_autogfp(size)) != NULL) {
				memset(acnt_tx_record, 0, size);
				priv->wlpd_p->acnt_tx_record = (void *)acnt_tx_record;
				priv->wlpd_p->acnt_tx_record_idx = 0;
			} else
	...
	case acnt_code_RA_stats:
		{
			...
			if ((acnt_RA_stats = (acnt_RA_stats_t *) wl_kmalloc_autogfp(size)) != NULL) {
				memset(acnt_RA_stats, 0, size);
				priv->wlpd_p->acnt_RA_stats = (void *)acnt_RA_stats;
				acnt_RA_stats_idx = 0;
			} else
...
void wl_disable_acnt_record_logging(struct net_device *netdev, UINT16 acnt_code)
{
	struct wlprivate *priv = NETDEV_PRIV_P(struct wlprivate, netdev);
...
void wl_copy_acnt_raw(void *acnt_records, void *recd, UINT16 acnt_code, UINT32 * record_idx)
{
	switch (acnt_code) {
	case acnt_code_tx_enqueue:
		{
			...
			memcpy(&acnt_tx_records[(*record_idx)++], acntRecd, sizeof(acnt_tx_t));

			if ((*record_idx) >= ACNT_TX_RECORD_MAX)
				(*record_idx) = 0;
			break;
		}
	case acnt_code_RA_stats:
		{
			acnt_RA_stats_t *acnt_RA_stats = (acnt_RA_stats_t *) acnt_records;
			...
			memcpy(&acnt_RA_stats[(*record_idx)++], acntRecd, sizeof(acnt_RA_stats_t));
...
void wl_write_acnt_tx_record(struct net_device *netdev, char *filename)
{
	struct wlprivate *priv = NETDEV_PRIV_P(struct wlprivate, netdev);
	UINT32 idx = 0;
	UINT32 currIdx = (priv->wlpd_p->acnt_tx_record_idx == 0) ? (ACNT_TX_RECORD_MAX - 1) : (priv->wlpd_p->acnt_tx_record_idx - 1);
	...
	if ((acnt_tx_record_write = (acnt_tx_t *) wl_kmalloc_autogfp(size)) != NULL) {
	...
	if ((string_buff = (char *)wl_kmalloc_autogfp(ACNT_MAX_STR_LEN * ACNT_TX_RECORD_MAX)) != NULL)
	...
	for (idx = 0; idx < ACNT_TX_RECORD_MAX; idx++) {
		...
		sprintf(tmp_buff, "%d 0x%x %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n",
			...
			acnt_tx_record_write[currIdx].PhyRate, acnt_tx_record_write[currIdx].revd1, acnt_tx_record_write[currIdx].revd2);
	...
	filp_acnt_tx_record = filp_open(filename, O_RDWR | O_CREAT | O_TRUNC, 0);
	if (!IS_ERR(filp_acnt_tx_record)) {
		__kernel_write(filp_acnt_tx_record, string_buff, strlen(string_buff), &filp_acnt_tx_record->f_pos);
...
void wl_dump_acnt_tx_record(struct net_device *netdev, UINT32 entry)
{
	...
	if ((acnt_tx_record_dump = (acnt_tx_t *) wl_kmalloc_autogfp(size)) != NULL) {
	...
	for (idx = 0; idx < entry; idx++) {
		printk
		    ("Ratetblindex: %d rateInfo: 0x%x StnId: %d Type: %d AggrType: %d Retries: %d Txcnt: %d NumAmpdu: %d NumBytes: %d DelayTime: %d SchedulePeriod: %d Qid: %d AirTime %d TimeStamp %d PhyRate %d revd1 %d revd2 %d\n",
		     acnt_tx_record_dump[currIdx].Ratetblindex, acnt_tx_record_dump[currIdx].rateInfo, acnt_tx_record_dump[currIdx].StnId,
		     acnt_tx_record_dump[currIdx].Type, acnt_tx_record_dump[currIdx].AggrType, acnt_tx_record_dump[currIdx].Retries,
		     acnt_tx_record_dump[currIdx].Txcnt, acnt_tx_record_dump[currIdx].NumAmpdu, acnt_tx_record_dump[currIdx].NumBytes,
		     acnt_tx_record_dump[currIdx].DelayTime, acnt_tx_record_dump[currIdx].SchedulePeriod, acnt_tx_record_dump[currIdx].Qid,
		     acnt_tx_record_dump[currIdx].AirTime, acnt_tx_record_dump[currIdx].TimeStamp, acnt_tx_record_dump[currIdx].PhyRate,
		     acnt_tx_record_dump[currIdx].revd1, acnt_tx_record_dump[currIdx].revd2);
...
void wl_write_acnt_RA_stats(struct net_device *netdev, char *filename)
{
	struct wlprivate *priv = NETDEV_PRIV_P(struct wlprivate, netdev);
	UINT32 idx = 0;
	UINT32 currIdx = (acnt_RA_stats_idx == 0) ? (ACNT_TX_RECORD_MAX - 1) : (acnt_RA_stats_idx - 1);
	acnt_RA_stats_t *acnt_RA_stats_write = NULL;
	acnt_RA_stats_t *acnt_RA_stats = (acnt_RA_stats_t *) priv->wlpd_p->acnt_RA_stats;
	size_t size = (sizeof(acnt_RA_stats_t) * ACNT_TX_RECORD_MAX);
	...
	if ((acnt_RA_stats_write = (acnt_RA_stats_t *) wl_kmalloc_autogfp(size)) != NULL) {
	...
	if ((string_buff = (char *)wl_kmalloc_autogfp(ACNT_RA_MAX_STR_LEN * ACNT_TX_RECORD_MAX)) != NULL)
		memset(string_buff, 0, (ACNT_RA_MAX_STR_LEN * ACNT_TX_RECORD_MAX));
	else {
	...
	for (idx = 0; idx < ACNT_TX_RECORD_MAX; idx++) {
		memset(tmp_buff, 0, sizeof(tmp_buff));
		sprintf(tmp_buff, "%d %d %d %d %d 0x%x %d %d %d %d %d %d %d %d %d %d %d\n",
			...
			acnt_RA_stats_write[currIdx].tx_success_cnt_raw, acnt_RA_stats_write[currIdx].tx_failure_cnt_raw);
	...
	if (!IS_ERR(filp_acnt_RA_stats)) {
		__kernel_write(filp_acnt_RA_stats, string_buff, strlen(string_buff), &filp_acnt_RA_stats->f_pos);
...
void wl_dump_acnt_RA_stats(struct net_device *netdev, UINT32 entry)
{
	...
	if ((acnt_RA_stats_dump = (acnt_RA_stats_t *) wl_kmalloc_autogfp(size)) != NULL) {
	...
	for (idx = 0; idx < entry; idx++) {
		printk
		    ("PER: %d TSF: %d StnId: %d Type: %d Ratetblindex: %d rateInfo: 0x%x Txattemptcnt: %d state: %d result: %d per_threshold: %d min_pkt_cnt_thres: %d time_for_decrease %d time_for_increase %d time_constant_for_increase %d tx_sent_cnt %d tx_success_cnt %d tx_failure_cnt %d\n",
		     acnt_RA_stats_dump[currIdx].PER, acnt_RA_stats_dump[currIdx].TSF, acnt_RA_stats_dump[currIdx].StnId,
		     acnt_RA_stats_dump[currIdx].Type, acnt_RA_stats_dump[currIdx].Ratetblindex, acnt_RA_stats_dump[currIdx].RateInfo,
		     acnt_RA_stats_dump[currIdx].Txattemptcnt, acnt_RA_stats_dump[currIdx].state, acnt_RA_stats_dump[currIdx].result,
		     acnt_RA_stats_dump[currIdx].per_threshold, acnt_RA_stats_dump[currIdx].min_pkt_cnt_thres,
		     acnt_RA_stats_dump[currIdx].time_for_decrease, acnt_RA_stats_dump[currIdx].time_for_increase,
		     acnt_RA_stats_dump[currIdx].time_constant_for_increase, acnt_RA_stats_dump[currIdx].tx_sent_cnt_raw,
...
UINT8 rate_GetRateID(UINT8 Rate)
{
	switch (Rate) {
...
void wlrxRateDecode(rx_info_t * pRxInfo, WLAN_RATE_HIST * pRH)
{
	u32 mcs;
	...
	case 2:
		...
		if ((pRxInfo->ht_sig1 & 0x3f) < 16) {
			pRH->HtRates[(pRxInfo->ht_sig1 >> 7) & 0x1][(pRxInfo->ht_sig2 >> 7) & 0x1][pRxInfo->ht_sig1 & 0xf]++;
		}
		break;
	case 4:
		//11ac; [NSS][bw][gi][mcs]
		...
		if (mcs < 10) {
			...
			if (idx < QS_NUM_SUPPORTED_11AC_NSS)
				pRH->VHtRates[idx][pRxInfo->ht_sig1 & 0x3][(pRxInfo->ht_sig2 & 0x1)][mcs]++;
		}
		break;
	}
}

BOOLEAN DecodeRecds(struct net_device *netdev, u8 * acntRecds, u32 bufSize)
{
	struct wlprivate *priv = NETDEV_PRIV_P(struct wlprivate, netdev);
		...
		case acnt_code_rx_ppdu:	// RXINFO for each PPDu (Acnt_rx_t)
			{
				...
				pStaInfo = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_MacAddr_t *) ((dot11_t *) (pAcntrx->RxINFO.Hdr))->addr2,
							       STADB_SKIP_MATCH_VAP);
				if (pStaInfo) {
					pStaInfo->RSSI = pAcntrx->RxINFO.rssi_x;
					pStaInfo->RSSI_path.a = pAcntrx->RxINFO.pm_rssi_dbm_a;
					pStaInfo->RSSI_path.b = pAcntrx->RxINFO.pm_rssi_dbm_b;
					pStaInfo->RSSI_path.c = pAcntrx->RxINFO.pm_rssi_dbm_c;
					pStaInfo->RSSI_path.d = pAcntrx->RxINFO.pm_rssi_dbm_d;
				} else {
					if (vmacSta_p->OpMode == WL_OP_MODE_STA || vmacSta_p->OpMode == WL_OP_MODE_VSTA) {
						vmacSta_p->RSSI_path.a = pAcntrx->RxINFO.pm_rssi_dbm_a;
						vmacSta_p->RSSI_path.b = pAcntrx->RxINFO.pm_rssi_dbm_b;
						vmacSta_p->RSSI_path.c = pAcntrx->RxINFO.pm_rssi_dbm_c;
						vmacSta_p->RSSI_path.d = pAcntrx->RxINFO.pm_rssi_dbm_d;
					}
				}

				priv->wlpd_p->NF_path.a = pAcntrx->RxINFO.pm_nf_dbm_a;
				priv->wlpd_p->NF_path.b = pAcntrx->RxINFO.pm_nf_dbm_b;
				priv->wlpd_p->NF_path.c = pAcntrx->RxINFO.pm_nf_dbm_c;
				priv->wlpd_p->NF_path.d = pAcntrx->RxINFO.pm_nf_dbm_d;

				wlrxRateDecode(&pAcntrx->RxINFO, &priv->wlpd_p->rxRateHistogram);
#ifndef SOC_W906X
		...
		case acnt_code_tx_enqueue:	// TXINFO when added to TCQ (Acnt_tx_t)
			{
				...
				/* ignore StnId > MAX_STNS record ? */
				if (priv->wlpd_p->acnt_tx_record && pAcnttx->StnId < sta_num)
					wl_copy_acnt_raw((acnt_tx_t *) priv->wlpd_p->acnt_tx_record,
							 pAcnttx, acnt_code_tx_enqueue, &priv->wlpd_p->acnt_tx_record_idx);
				...
				wltxRateDecode((dbRateInfo_t *) & pAcnttx->rateInfo, pAcnttx->Ratetblindex,
					       priv->wlpd_p->txRateHistogram[pAcnttx->StnId], pAcnttx->Type, pAcnttx->Txcnt);

				if (priv->wlpd_p->scheHistogram[pAcnttx->StnId] == NULL)
					goto error_skip;

				if (pAcnttx->NumAmpdu > 64) {
					priv->wlpd_p->scheHistogram[pAcnttx->StnId]->NumAmpdu[pAcnttx->AggrType][64]++;
				} else if (pAcnttx->NumAmpdu > 0) {
					priv->wlpd_p->scheHistogram[pAcnttx->StnId]->NumAmpdu[pAcnttx->AggrType][pAcnttx->NumAmpdu - 1]++;
				}
				if (pAcnttx->DelayTime < 1000) {
					priv->wlpd_p->scheHistogram[pAcnttx->StnId]->Delay[pAcnttx->AggrType][0]++;
				} else if (pAcnttx->DelayTime < 10000) {
					priv->wlpd_p->scheHistogram[pAcnttx->StnId]->Delay[pAcnttx->AggrType][1]++;
				} else if (pAcnttx->DelayTime < 100000) {
					priv->wlpd_p->scheHistogram[pAcnttx->StnId]->Delay[pAcnttx->AggrType][2]++;
				} else if (pAcnttx->DelayTime < 300000) {
					priv->wlpd_p->scheHistogram[pAcnttx->StnId]->Delay[pAcnttx->AggrType][3]++;
				} else
					priv->wlpd_p->scheHistogram[pAcnttx->StnId]->Delay[pAcnttx->AggrType][4]++;

				if (pAcnttx->NumBytes < 1500) {
					priv->wlpd_p->scheHistogram[pAcnttx->StnId]->NumBytes[pAcnttx->AggrType][0]++;
				} else if (pAcnttx->NumBytes < 16000) {
					priv->wlpd_p->scheHistogram[pAcnttx->StnId]->NumBytes[pAcnttx->AggrType][1]++;
				} else if (pAcnttx->NumBytes < 32000) {
					priv->wlpd_p->scheHistogram[pAcnttx->StnId]->NumBytes[pAcnttx->AggrType][2]++;
				} else if (pAcnttx->NumBytes < 64000) {
					priv->wlpd_p->scheHistogram[pAcnttx->StnId]->NumBytes[pAcnttx->AggrType][3]++;
				} else if (pAcnttx->NumBytes < 128000) {
					priv->wlpd_p->scheHistogram[pAcnttx->StnId]->NumBytes[pAcnttx->AggrType][4]++;
				} else
					priv->wlpd_p->scheHistogram[pAcnttx->StnId]->NumBytes[pAcnttx->AggrType][5]++;

				{
					extStaDb_StaInfo_t *pStaInfo = extStaDb_GetStaInfoStn(vmacSta_p, pAcnttx->StnId);
					if (pStaInfo && (pStaInfo->StnId == pAcnttx->StnId)) {
						memcpy(&pStaInfo->RateInfo, &pAcnttx->rateInfo, sizeof(u_int32_t));
					}
				}
#else
				acnt_tx_t *pAcnttx;
				extStaDb_StaInfo_t *pStaInfo = NULL;
				pAcnttx = (acnt_tx_t *) pStart;
				pStaInfo = extStaDb_GetStaInfo(vmacSta_p, (IEEEtypes_Addr_t(*)[6]) & (pAcnttx->Hdr.addr1[0]), 2);
				if (pStaInfo && pStaInfo->StnId) {
					memcpy(&pStaInfo->RateInfo, &pAcnttx->TxINFO.rateInfo, sizeof(u_int32_t));

					wltxRateDecode((dbRateInfo_t *) & pAcnttx->TxINFO.rateInfo, pAcnttx->Ratetblindex,
						       priv->wlpd_p->txRateHistogram[pStaInfo->StnId - 1], pAcnttx->Type, pAcnttx->Txcnt);
				}
#endif				/* SOC_W906X */
 error_skip:
		...
		case acnt_code_RA_stats:
			{
				...
				if (priv->wlpd_p->acnt_RA_stats)
					wl_copy_acnt_raw((acnt_RA_stats_t *) priv->wlpd_p->acnt_RA_stats,
							 pRAstats, acnt_code_RA_stats, &acnt_RA_stats_idx);

#ifndef SOC_W906X
				if ((pRAstats->StnId > 0) && (pRAstats->StnId < sta_num))
#endif
				{
					if (pRAstats->Type < 2) {
						if (pRAstats->Txattemptcnt >= 250)
							RA_TX_ATTEMPT[pRAstats->Type][5]++;
						else if (pRAstats->Txattemptcnt >= 100)
							RA_TX_ATTEMPT[pRAstats->Type][4]++;
						else if (pRAstats->Txattemptcnt >= 50)
							RA_TX_ATTEMPT[pRAstats->Type][3]++;
						else if (pRAstats->Txattemptcnt >= 15)
							RA_TX_ATTEMPT[pRAstats->Type][2]++;
						else if (pRAstats->Txattemptcnt >= 4)
							RA_TX_ATTEMPT[pRAstats->Type][1]++;
						else
							RA_TX_ATTEMPT[pRAstats->Type][0]++;
					}
#ifndef SOC_W906X
					SPIN_LOCK_IRQSAVE(&priv->wlpd_p->txRateHistoLock[pRAstats->StnId - 1], txRateHistoflags);
#endif

					wltxRatePERDecode((dbRateInfo_t *) & pRAstats->RateInfo, pRAstats->Ratetblindex, pRAstats->PER,
							  priv->wlpd_p->txRateHistogram[pRAstats->StnId], pRAstats->Type);

#ifndef SOC_W906X
					SPIN_UNLOCK_IRQRESTORE(&priv->wlpd_p->txRateHistoLock[pRAstats->StnId - 1], txRateHistoflags);
#endif
				}
		...
		case acnt_code_BA_stats:
			{
				...
				if (pBAstats->StnId < 10) {
					wltxBADecode(&priv->wlpd_p->txBAStats[BA_HISTO_STAID_MAP[pBAstats->StnId]], pBAstats);
				}
		...
		case acnt_code_BF_Mimo_Ctrl_Field_Log:
			{
				acnt_BF_Mimo_Ctrl_Field_Log_t *pBF_Mimo_Ctrl_Field_Log;

				pBF_Mimo_Ctrl_Field_Log = (acnt_BF_Mimo_Ctrl_Field_Log_t *) pStart;

				wl_BF_Mimo_Ctrl_Field_Log_Decode(pBF_Mimo_Ctrl_Field_Log);
		...
		case acnt_code_tx_getNewTxq:	//internal_stat[] from getNewTxq() (acnt_tx3_t)
			{
				...
				print_hex_dump(KERN_INFO, "", DUMP_PREFIX_OFFSET, 32, 4, pAcnttx3, sizeof(acnt_tx3_t), false);
#endif
				memcpy(&acnt_internal_stat[current_internal_stat], pAcnttx3, sizeof(acnt_tx3_t));
				current_internal_stat = (current_internal_stat + 1) % NUM_INTERNAL_STAT;
...
#else
	AcntRingBufSize = min(priv->wlpd_p->descData[0].AcntRingSize, (ACNT_NCHUNK * DEFAULT_SIZE_CHUNK));
		...
		while (tail != head) {
			...
			if (tail > head) {
				while (tail != (AcntRingBufSize)) {
					off = tail % (ChunkSize);
					size = ChunkSize - off;
					memcpy(dst, (u8 *) AcntBase[tail / ChunkSize] + off, size);
					dst += size;
					tail += size;
#ifdef SOC_W906X
					if (loopcnt++ > 1000) {
...
#endif
	tail = head;
	writel(tail, priv->ioBase1 + reg_acnt_tail);
}
...
void wlAcntProcess(struct net_device *netdev)
{
	...
	NumChunk = 1;
	AcntBufSize = (u_int32_t) min((u_int32_t) (NumChunk * ChunkSize), (u_int32_t) DEFAULT_ACNT_RING_SIZE);
	acntRecds = priv->wlpd_p->acntRecords;
	...
	if (tail > head) {
		...
		memcpy(acntRecds, (u8 *) priv->wlpd_p->ACNTmemInfo.data + tail, AcntBufSize - tail);
		memcpy(acntRecds + AcntBufSize - tail, (u8 *) priv->wlpd_p->ACNTmemInfo.data, head);
		DecodeRecds(netdev, acntRecds, ToBeReadAcntBufSize);
	} else {
		//printk("normal \n");
		ToBeReadAcntBufSize = head - tail;
		if (ToBeReadAcntBufSize > AcntBufSize)
			goto skip_process;
		DecodeRecds(netdev, (u8 *) priv->wlpd_p->ACNTmemInfo.data + tail, ToBeReadAcntBufSize);
		//free(acntRecds);
	}
...
void wlHandleAcnt(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	...
	WLSNDEVT(netdev, IWEVCUSTOM, (IEEEtypes_MacAddr_t *) & wlpptr->hwData.macAddr[0], "accounting record ready");

}

void wlAcntPeekRecds(struct net_device *netdev, u_int32_t * head, u_int32_t * tail)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
#ifdef SOC_W906X
	unsigned int reg_acnt_head = wlpptr->wlpd_p->reg.acnt_head;
	...
	*head = readl(wlpptr->ioBase1 + reg_acnt_head);
	*tail = readl(wlpptr->ioBase1 + reg_acnt_tail);
}

void wlAcntReadRecds(struct net_device *netdev, u_int32_t newTail, u_int8_t * pBuf, u_int32_t * bufSize)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	u_int32_t Head, tail;
	int temSize;
	...
	if (temSize < 0) {
		...
		if (temSize < temBufSize) {
			memcpy(pBuf, &wlpptr->wlpd_p->descData[0].pAcntRing[tail], temSize);
			pBuf += temSize;
			temBufSize -= temSize;
		} else {
			memcpy(pBuf, &wlpptr->wlpd_p->descData[0].pAcntRing[tail], temBufSize);
			writel((tail + temBufSize), wlpptr->ioBase1 + reg_acnt_tail);
			return;
		}
		if (temBufSize > Head) {
			memcpy(pBuf, &wlpptr->wlpd_p->descData[0].pAcntRing[0], Head);
			temBufSize -= Head;
			*bufSize = temBufSizeOrg - temBufSize;
			writel(Head, wlpptr->ioBase1 + reg_acnt_tail);
			return;
		} else {
			memcpy(pBuf, &wlpptr->wlpd_p->descData[0].pAcntRing[0], temBufSize);
			writel(temBufSize, wlpptr->ioBase1 + reg_acnt_tail);
			return;
		}
		...
		if (temSize < temBufSize) {
			memcpy(pBuf, &wlpptr->wlpd_p->descData[0].pAcntRing[tail], temSize);
			*bufSize = temSize;
			writel((tail + temSize), wlpptr->ioBase1 + reg_acnt_tail);
			return;
		} else {
			memcpy(pBuf, &wlpptr->wlpd_p->descData[0].pAcntRing[tail], temBufSize);
			writel((tail + temBufSize), wlpptr->ioBase1 + reg_acnt_tail);
...
*/
void wlAcntCopyRateTbl(struct net_device *netdev, UINT8 * sta_addr, UINT32 sta_id, UINT8 type)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
...
#ifdef SOC_W906X
	if ((pStaInfo =
	     extStaDb_GetStaInfo(wlpptr->vmacSta_p, (IEEEtypes_MacAddr_t *) sta_addr, STADB_DONT_UPDATE_AGINGTIME | STADB_NO_BLOCK)) != NULL) {
		sta_id = pStaInfo->StnId;
	}
	wlFwGetRateTable(netdev, sta_addr, (UINT8 *) pRateTable, size, type, sta_id);
	histo_p = (WLAN_TX_RATE_HIST_DATA *) & wlpptr->wlpd_p->txRateHistogram[sta_id]->SU_rate[0];
#else
	wlFwGetRateTable(netdev, sta_addr, (UINT8 *) pRateTable, size, type);
	histo_p = (WLAN_TX_RATE_HIST_DATA *) & wlpptr->wlpd_p->txRateHistogram[sta_id - 1]->SU_rate[0];
#endif
