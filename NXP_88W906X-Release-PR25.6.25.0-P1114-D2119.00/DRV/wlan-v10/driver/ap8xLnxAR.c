...
void wlTAcntBufInit(struct net_device *netdev)
{
	...
	wlpd_p->acntTxMsduRingSize = p_smac_cfg->acntTxMsduRingSize = SC5_TXQ_SIZE * sizeof(MSDU_RING_INFO_st);
	WLDBG_INFO(DBG_LEVEL_0, "%s(), acntTxMsduRingSize = %u\n", __func__, p_smac_cfg->acntTxMsduRingSize);
#ifdef USE_NCBUF
	wlpd_p->acntTxMsduRingBaseAddr_v = (MSDU_RING_INFO_st *) wl_dma_alloc_coherent(wlpd_p->dev,
										       p_smac_cfg->acntTxMsduRingSize,
										       &phys_addr, wlpd_p->dma_alloc_flags);
#else
	wlpd_p->acntTxMsduRingBaseAddr_v = (MSDU_RING_INFO_st *) wl_kmalloc(p_smac_cfg->acntTxMsduRingSize, GFP_KERNEL);
	phys_addr = dma_map_single(wlpd_p->dev, wlpd_p->acntTxMsduRingBaseAddr_v, p_smac_cfg->acntTxMsduRingSize, DMA_FROM_DEVICE);
#endif				//USE_NCBUF
	p_smac_cfg->acntTxMsduRingBaseAddr = (U32) phys_addr;
	wlpd_p->acntTxMsduRingBaseAddr_p = (U32) phys_addr;
	WLDBG_INFO(DBG_LEVEL_0, "%s(), acntTxMsduRingBaseAddr = %x\n", __func__, p_smac_cfg->acntTxMsduRingBaseAddr);

	// PPDU Tx Accounting Record
	ptxacntppdu->acntTxSize = p_smac_cfg->acntTxSize = TX_ACNT_PPDU_BUF_SIZE;

#ifdef USE_NCBUF
	// Using non-cacheable buffer
	ptxacntppdu->acntTxBaseAddr_v = (void *)wl_dma_alloc_coherent(wlpd_p->dev, p_smac_cfg->acntTxSize, &phys_addr, wlpd_p->dma_alloc_flags);
#else
	// Using cacheable buffer
	ptxacntppdu->acntTxBaseAddr_v = (char *)wl_kmalloc(p_smac_cfg->acntTxSize, GFP_KERNEL);
	phys_addr = dma_map_single(wlpd_p->dev, ptxacntppdu->acntTxBaseAddr_v, ptxacntppdu->acntTxSize, DMA_FROM_DEVICE);
#endif				//

	ptxacntppdu->acntTxBaseAddr_p = p_smac_cfg->acntTxBaseAddr = (U32) phys_addr;

	WLDBG_INFO(DBG_LEVEL_0, "%s(), acntTxBaseAddr (v, p)=(%p, %x), size=%x\n", __func__,
		   ptxacntppdu->acntTxBaseAddr_v, p_smac_cfg->acntTxBaseAddr, p_smac_cfg->acntTxSize);
	...
	p_smac_cfg->acntTxRdPtr = p_smac_cfg->acntTxWrPtr = p_smac_cfg->acntTxBaseAddr;
	return;
}

void wlTAcntBufCleanUp(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	txacntppdu *ptxacntppdu = &(wlpd_p->acntTxAcntPpdu);
...
#ifdef USE_NCBUF
		wl_dma_free_coherent(wlpd_p->dev, ptxacntppdu->acntTxSize, (void *)ptxacntppdu->acntTxBaseAddr_v, ptxacntppdu->acntTxBaseAddr_p);
#else
		dma_unmap_single(wlpd_p->dev, virt_to_phys(ptxacntppdu->acntTxBaseAddr_v), ptxacntppdu->acntTxSize, DMA_FROM_DEVICE);
...
#if defined(TXACNT_REC) && defined (SOC_W906X)
static void wlTAcntHdlr(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
...
irqreturn_t wlSC5MSIX_TAcntRec(int irq, void *dev_id)
{
...
extern unsigned int txacnt_idmsg;
inline void wlTxAcntDbgMsg(struct wlprivate_data *wlpd_p, void *recbuf)
{
	txacntppdu *ptxacntppdu = &(wlpd_p->acntTxAcntPpdu);
...
void wlTxPPDUHdrHndl(struct net_device *netdev, SMAC_ACNT_TX_PPDU_HDR_st * ptxppdu_hdr)
{
	if (txacnt_msg > 0) {
...
void wlTxPPDUUsrHndl(struct net_device *netdev, SMAC_ACNT_TX_HOST_USR_st * txppdu_usr)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	if (txacnt_msg > 0) {
...
/*
	...
	type:
		...
		ppdu_host_gen_ba
*/
void wlMacTxHostGenPPDUHndle(struct net_device *netdev, SMAC_ACNT_TX_MPDU_TID_INFO_st * txppdu_tidinfo,
			     MSDU_RING_INFO_st ** pptx_msdu_info, U16 usrid, U8 type)
{
	...
	for (mpduid = 0; mpduid < TX_ACNT_MAX_NUM_AGGR; mpduid++) {
		...
		for (msduid = 0; msduid < txppdu_tidinfo->numMsdu[mpduid]; msduid++, (*pptx_msdu_info)++) {
			if ((U8 *) * pptx_msdu_info >= ((U8 *) wlpd_p->acntTxMsduRingBaseAddr_v + wlpd_p->acntTxMsduRingSize)) {
				// Out of the buffer size => reset it back
				*pptx_msdu_info = (MSDU_RING_INFO_st *) wlpd_p->acntTxMsduRingBaseAddr_v;
			}
...
*/
inline void wlMoveAcntTxRdptr(txacntppdu * ptxacntppdu, S32 offset)
{
	...
	// Error checking
	if (((U32) (ptxacntppdu->acntTxRdPtr_v - ptxacntppdu->acntTxBaseAddr_v)) > ptxacntppdu->acntTxSize) {
		WLDBG_ERROR(DBG_LEVEL_0, "(wlpd_p->acntTxRdPtr, base)=(%p, %p)is over limit %x, & not gettint reset mark (%x) => force to reset\n",
			    ptxacntppdu->acntTxRdPtr_v, ptxacntppdu->acntTxBaseAddr_v, ptxacntppdu->acntTxSize, PPDUACNT_END);
...
*/
void wlMacTxDoneFreeMsdu(struct wlprivate_data *wlpd_p, U16 usrid, U16 tid, U8 bitmap[32])
{
	return;
}
...
*/
void wlMacTxDoneAckHndl(struct wlprivate_data *wlpd_p, SMAC_ACNT_MPDU_BA_INFO_st * txmpdu_bainfo, U16 usrid)
{
	...
	if (txmpdu_bainfo->numExpired == 0) {
		...
		if (wlpd_p->startSeq[usrid][tid] != txmpdu_bainfo->startSeqNum) {
			...
			printk("(send, ack)=(%u, %u)\n", wlpd_p->startSeq[usrid][tid], txmpdu_bainfo->startSeqNum);
			mwl_hex_dump(txmpdu_bainfo, sizeof(SMAC_ACNT_MPDU_BA_INFO_st));
			printk("%s(): Last (ppduinfo, txdone)=(%p, %p)\n", __func__, wlpd_p->lastppduinfo, wlpd_p->lasttxdoneinfo);
			printk("%s(), (%u, %u) (base, txmpdu_bainfo) = (%p, %p)\n", __func__, usrid, tid,
			       ptxacntppdu->acntTxBaseAddr_v, txmpdu_bainfo);
			wlTxAcntDbgMsg(wlpd_p, txmpdu_bainfo);
		}
...
*/
void wlMacTxDoneBAHndl(struct wlprivate_data *wlpd_p, SMAC_ACNT_MPDU_BA_INFO_st * txmpdu_bainfo, U16 usrid)
{
	U16 tid = txmpdu_bainfo->tid;
...
void wlMacTxDoneTimeoutHndl(struct wlprivate_data *wlpd_p, SMAC_ACNT_MPDU_BA_INFO_st * txmpdu_bainfo, U16 usrid)
{
	U16 tid = txmpdu_bainfo->tid;

	// Retrying of ACKed pkt
	if ((txmpdu_bainfo->startSeqNum == 4095) && (txmpdu_bainfo->numExpired == 0)) {
		// Retrying, nothing to do
		return;
	}
	if (tid >= DRV_TX_ACNT_MAX_NUM_TID) {	//==> Need to check
	...
	if (txmpdu_bainfo->startSeqNum != wlpd_p->startSeq[usrid][tid]) {
		printk("%s(), Error, Inconsist start_seq (%u, %u)\n", __func__, txmpdu_bainfo->startSeqNum, wlpd_p->startSeq[usrid][tid]);
		printk("%s(): Last (ppduinfo, txdone)=(%p, %p)\n", __func__, wlpd_p->lastppduinfo, wlpd_p->lasttxdoneinfo);
		wlTxAcntDbgMsg(wlpd_p, txmpdu_bainfo);
	}
	if (txacnt_idmsg > 0) {
...
char *ppdu_type_str(TX_PPDU_TYPE type)
{
	switch (type) {
...
char *mactxacnt_type_str(SMAC_ACNT_TXDONE_TYPE type)
{
	switch (type) {
...
U32 wlGetTotalMsduIn_ppdu_tid(SMAC_ACNT_TX_MPDU_TID_INFO_st * txppdu_tidinfo, U8 numTids)
{
	U16 tid;
	U32 mpduid;
	U32 totalNumMsdus = 0;
...
void wlTxPPDUAcntHndl(struct net_device *netdev)
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate *parent_wlpptr = GET_PARENT_PRIV(wlpptr);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	...
	wrptr = ptxacntppdu->acntTxBaseAddr_v + (psmac_cfg->acntTxWrPtr - ptxacntppdu->acntTxBaseAddr_p);

	if (txacnt_idmsg > 0) {
		printk("[TXACNT] ++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
		printk("[TXACNT] =>%s(), (base, rdptr, wrptr)=(%x, %x, %x) (%p, %p)\n", __func__,
		       psmac_cfg->acntTxBaseAddr, psmac_cfg->acntTxRdPtr, psmac_cfg->acntTxWrPtr, ptxacntppdu->acntTxRdPtr_v, wrptr);
	}
	...
	while (ptxacntppdu->acntTxRdPtr_v != wrptr) {
		...
		if ((*(U32 *) ptxacntppdu->acntTxRdPtr_v) == PPDUACNT_END) {
			...
			if (txacnt_msg > 0) {
				printk("=> Get %x, at (%p, %x) reset rdptr\n", PPDUACNT_END, ptxacntppdu->acntTxRdPtr_v, psmac_cfg->acntTxRdPtr);
			}
		...
		ptxppdu_info = (SMAC_ACNT_TX_PPDU_INFO_st *) (ptxacntppdu->acntTxRdPtr_v + usedlen);
		if (txacnt_idmsg > 0) {
			printk("=> Parsing at (%p), usedlen=%u\n", ptxppdu_info, usedlen);
		}
		...
		if ((ptxppdu_hdr->numUsers > 16) || (ptxppdu_hdr->numUsers == 0)) {
		...
		if (txacnt_idmsg > 0) {
			printk("ptxppdu_hdr->type: %u (%s)\n", ptxppdu_hdr->type, ppdu_type_str(ptxppdu_hdr->type));
		}
		...
		case ppdu_host_gen_ba:
			{
				SMAC_ACNT_TX_HOST_PKT_st *pAcntTxHostPkt = (SMAC_ACNT_TX_HOST_PKT_st *) (ptxacntppdu->acntTxRdPtr_v + usedlen);
				MSDU_RING_INFO_st *ptx_msdu_info = (MSDU_RING_INFO_st *) ((U8 *) wlpd_p->acntTxMsduRingBaseAddr_v +
											  pAcntTxHostPkt->msduWrPtr);
				usedlen += offsetof(SMAC_ACNT_TX_HOST_PKT_st, user);

				//DBG++
				wlpd_p->lastppduinfo = (U8 *) ptxppdu_hdr;
				//DBG--
				//printk("sizeof(wlpd_p->txacnt_skb)=%lu\n",sizeof(wlpd_p->txacnt_skb));
				for (usrid = 0; usrid < ptxppdu_hdr->numUsers; usrid++) {
					SMAC_ACNT_TX_HOST_USR_st *txppdu_usr;
					U32 totalNumMsdus = 0;

					txppdu_usr = (SMAC_ACNT_TX_HOST_USR_st *) (ptxacntppdu->acntTxRdPtr_v + usedlen);
					usedlen += offsetof(SMAC_ACNT_TX_HOST_USR_st, mpduTid);
					...
					if ((txppdu_usr->mpduHdr.numTids > 8) || (txppdu_usr->mpduHdr.numTids == 0)) {
					...
					if ((ptxppdu_hdr->type == ppdu_host_gen_noack) ||
					    (ptxppdu_hdr->type == ppdu_host_gen_ack) || (ptxppdu_hdr->type == ppdu_host_gen_ba)) {
						SMAC_ACNT_TX_MPDU_TID_INFO_st *txppdu_tidinfo;
						txppdu_tidinfo = (SMAC_ACNT_TX_MPDU_TID_INFO_st *) (ptxacntppdu->acntTxRdPtr_v + usedlen);
						totalNumMsdus = wlGetTotalMsduIn_ppdu_tid(txppdu_tidinfo, txppdu_usr->mpduHdr.numTids);
						...
						ptx_msdu_info = (MSDU_RING_INFO_st *) ((U8 *) wlpd_p->acntTxMsduRingBaseAddr_v +
										       ((pAcntTxHostPkt->msduWrPtr + wlpd_p->acntTxMsduRingSize -
											 totalNumMsdus * sizeof(MSDU_RING_INFO_st)) %
						...
						if (txacnt_idmsg > 0) {
							printk("[TXACNT] total: %u, [msduWrPtr, from]=[%u, %lu]\n",
							       ...
							       pAcntTxHostPkt->msduWrPtr,
							       ((pAcntTxHostPkt->msduWrPtr + wlpd_p->acntTxMsduRingSize -
								 totalNumMsdus * sizeof(MSDU_RING_INFO_st)) % wlpd_p->acntTxMsduRingSize));
						}
					}
					for (tid = 0; tid < txppdu_usr->mpduHdr.numTids; tid++) {
						SMAC_ACNT_TX_MPDU_TID_INFO_st *txppdu_tidinfo;
						txppdu_tidinfo = (SMAC_ACNT_TX_MPDU_TID_INFO_st *) (ptxacntppdu->acntTxRdPtr_v + usedlen);
						usedlen += sizeof(SMAC_ACNT_TX_MPDU_TID_INFO_st);
						if (txacnt_idmsg > 0) {
							printk("[TXACNT] (%p) [%u], tid:%u, , type=%s(%u), startSeqNum=%u\n", txppdu_tidinfo, tid,
							       txppdu_tidinfo->tid,
							       ppdu_type_str(ptxppdu_hdr->type), ptxppdu_hdr->type, txppdu_tidinfo->startSeqCtrl);

						}
						if ((U8 *) ptx_msdu_info >= ((U8 *) wlpd_p->acntTxMsduRingBaseAddr_v + wlpd_p->acntTxMsduRingSize)) {
							// Out of the buffer size => reset it back
							ptx_msdu_info = (MSDU_RING_INFO_st *) wlpd_p->acntTxMsduRingBaseAddr_v;
						}
						if (ptxppdu_hdr->type != ppdu_host_gen_noack) {
							wlMacTxHostGenPPDUHndle(netdev, txppdu_tidinfo, &ptx_msdu_info,
										txppdu_usr->mpduHdr.userId, ptxppdu_hdr->type);
						}
		...
		case ppdu_txdone:{
				for (usrid = 0; usrid < ptxppdu_hdr->numUsers; usrid++) {
					SMAC_ACNT_TX_DONE_USR_st *txdone_usr = (SMAC_ACNT_TX_DONE_USR_st *) (ptxacntppdu->acntTxRdPtr_v + usedlen);
					usedlen += offsetof(SMAC_ACNT_TX_DONE_USR_st, baTid);
					//DBG++
					wlpd_p->lasttxdoneinfo = (U8 *) ptxppdu_hdr;
					//DBG--
					if (txdone_usr->userHdr.userId >= TX_ACNT_USER_NUM) {
						printk("userId of ppdu_txdone is too big: %u\n", txdone_usr->userHdr.userId);
						wlTxAcntDbgMsg(wlpd_p, txdone_usr);
					}
					wlpd_p->txacnt_txdone_cnt[txdone_usr->userHdr.userId][txdone_usr->userHdr.type]++;
					if (txacnt_idmsg > 0) {
						printk("[TXACNT] TxDone, (%p) UsrID:%u, tidCnt=%u, type=%s(%u)\n",
						       txdone_usr,
						       txdone_usr->userHdr.userId, txdone_usr->userHdr.numTids,
						       mactxacnt_type_str(txdone_usr->userHdr.type), txdone_usr->userHdr.type);
					}
					if ((txdone_usr->userHdr.numTids > 8) || (txdone_usr->userHdr.numTids == 0)) {
						printk("[TXACNT] big tidCnt, SMAC_ACNT_TX_DONE_USR_st: (%p)\n", txdone_usr);
						mwl_hex_dump(txdone_usr, sizeof(SMAC_ACNT_TX_DONE_USR_st));
						printk("Last (ppduinfo, txdone)=(%p, %p)\n", wlpd_p->lastppduinfo, wlpd_p->lasttxdoneinfo);
						wlTxAcntDbgMsg(wlpd_p, txdone_usr);
					}

					for (tid = 0; tid < txdone_usr->userHdr.numTids; tid++) {
						SMAC_ACNT_MPDU_BA_INFO_st *txmpdu_bainfo;

						txmpdu_bainfo = (SMAC_ACNT_MPDU_BA_INFO_st *) (ptxacntppdu->acntTxRdPtr_v + usedlen);
						usedlen += sizeof(SMAC_ACNT_MPDU_BA_INFO_st);
						if (txacnt_idmsg > 0) {
							...
							mwl_hex_dump(txmpdu_bainfo->baBitmap, sizeof(U8) * 32);
						}

						switch (txdone_usr->userHdr.type) {
						case tacnt_txdone_ack:
							wlMacTxDoneAckHndl(wlpd_p, txmpdu_bainfo, txdone_usr->userHdr.userId);
							break;
						case tacnt_txdone_ba:
							wlMacTxDoneBAHndl(wlpd_p, txmpdu_bainfo, txdone_usr->userHdr.userId);
							break;
						case tacnt_txdone_timeout:
							wlMacTxDoneTimeoutHndl(wlpd_p, txmpdu_bainfo, txdone_usr->userHdr.userId);
							break;
						default:
			...
			break;
		case ppdu_sfw_gen:{
				usedlen += offsetof(SMAC_ACNT_TX_FW_PKT_st, user);
				for (usrid = 0; usrid < ptxppdu_hdr->numUsers; usrid++) {
					SMAC_ACNT_TX_FW_PKT_USR_st *txppdu_fw_usr;

					txppdu_fw_usr = (SMAC_ACNT_TX_FW_PKT_USR_st *) (ptxacntppdu->acntTxRdPtr_v + usedlen);
					usedlen += offsetof(SMAC_ACNT_TX_FW_PKT_USR_st, mpduInfo);

					{
						SMAC_ACNT_FW_PKT_INFO_st *txppdu_fwpkt_info;

						txppdu_fwpkt_info = (SMAC_ACNT_FW_PKT_INFO_st *) (ptxacntppdu->acntTxRdPtr_v + usedlen);
						//usedlen += sizeof(SMAC_ACNT_FW_PKT_INFO_st);
						usedlen += offsetof(SMAC_ACNT_FW_PKT_INFO_st, Pkt80211) + txppdu_fwpkt_info->length;
						//if (txacnt_idmsg > 0) {
	...
	if (txacnt_idmsg > 0) {
		printk("[TXACNT] <=%s(), (rd_v, wr_v)=(%p, %p), (rd_p, wr_p)=(%x, %x)\n", __func__,
		       ptxacntppdu->acntTxRdPtr_v, wrptr, psmac_cfg->acntTxRdPtr, psmac_cfg->acntTxWrPtr);
		printk("[TXACNT] ----------------------------------------------------\n\n");
		txacnt_idmsg--;
	}
	return;
...
/*
	...
	- offset: offset to move
		...
		<0 : Reset the pointer to base address
*/
inline void wlMoveAcntRxRdptr(rxacntppdu * prxacntppdu, S32 offset)
{
	if (offset == RESET_ACNTRDPTR) {
	...
	// Error checking
	if (((U32) (prxacntppdu->acntRxRdPtr_v - prxacntppdu->acntRxBaseAddr_v)) > prxacntppdu->acntRxSize) {
		WLDBG_ERROR(DBG_LEVEL_0, "(wlpd_p->acntRxRdPtr, base)=(%p, %p)is over limit %x, & not gettint reset mark (%x) => force to reset\n",
			    prxacntppdu->acntRxRdPtr_v, prxacntppdu->acntRxBaseAddr_v, prxacntppdu->acntRxSize, PPDUACNT_END);
...
void wlRxPPDUAcntHndl(struct net_device *netdev)
{
#ifdef RXACNT_REC
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate *parent_wlpptr = GET_PARENT_PRIV(wlpptr);
	...
	wrptr = prxacntppdu->acntRxBaseAddr_v + (psmac_cfg->acntRxWrPtr - prxacntppdu->acntRxBaseAddr_p);
...
// test--
	...
	if (rxacnt_idmsg > 0) {
		...
		printk("[RXACNT] =>%s(), (base, rdptr, wrptr)=(%x, %x, %x) (%p, %p)\n", __func__,
		       psmac_cfg->acntRxBaseAddr, psmac_cfg->acntRxRdPtr, psmac_cfg->acntRxWrPtr, prxacntppdu->acntRxRdPtr_v, wrptr);
		printk("[RXACNT] => lsmac: %x, %x\n", lsmac_cfg.acntRxRdPtr, lsmac_cfg.acntRxWrPtr);
	}
	...
	while (prxacntppdu->acntRxRdPtr_v != wrptr) {
		if ((*(U32 *) prxacntppdu->acntRxRdPtr_v) == PPDUACNT_END) {
			// The rest of the buffer is insufficient for next tx ppdu record => wr-ptr is resette
			if (rxacnt_idmsg > 0) {
				printk("=> Get %x, at (%p, %x) reset rdptr\n", PPDUACNT_END, prxacntppdu->acntRxRdPtr_v, psmac_cfg->acntRxRdPtr);
			}
...

		{
			...
			prxppdu_info = (SMAC_ACNT_RX_PPDU_INFO_st *) (prxacntppdu->acntRxRdPtr_v + usedlen);
			...
			if ((prxppdu_hdr->numUsers > 16) || (prxppdu_hdr->numUsers == 0)) {
				printk("[RXACNT] Error ==> numUsers == %d, SMAC_ACNT_RX_PPDU_HDR_st:(%p)\n", prxppdu_hdr->numUsers, prxppdu_hdr);
				mwl_hex_dump(prxppdu_info, sizeof(SMAC_ACNT_RX_PPDU_INFO_st));
				//printk("Last (ppduinfo, txdone)=(%p, %p)\n", wlpd_p->lastppduinfo, wlpd_p->lasttxdoneinfo);
				//wlTxAcntDbgMsg(wlpd_p, ptxppdu_hdr);
				...
				if (bgn_rdpt < wrptr) {
					mwl_hex_dump(bgn_rdpt, (wrptr - bgn_rdpt));
				}
				prxacntppdu->acntRxRdPtr_v = wrptr;
				*prxacntppdu->acntRxRdPtr_p = lsmac_cfg.acntRxWrPtr;
			...
			if (rxacnt_idmsg > 0) {
				...
				mwl_hex_dump(prxppdu_hdr, sizeof(SMAC_ACNT_RX_PPDU_HDR_st));
			}
			for (usrid = 0; usrid < prxppdu_hdr->numUsers; usrid++) {
				...
				case 0:{	// Mgmt
						SMAC_ACNT_RX_MGMT_MPDU_st *prxmpdu =
						    (SMAC_ACNT_RX_MGMT_MPDU_st *) (prxacntppdu->acntRxRdPtr_v + usedlen);
						//SMAC_ACNT_RX_MGMT_MPDU_st     *prxmpdu = &prxppdu_usr->u.mgmt;
						usedlen += sizeof(SMAC_ACNT_RX_MGMT_MPDU_st);
						prxRateHistogram->pkt_cnt[0]++;
						// TBD: Processing SMAC_ACNT_RX_MGMT_MPDU_st
						if (rxacnt_idmsg > 0) {
							printk("Mgmt, prxmpdu: %p\n", prxmpdu);

							mwl_hex_dump(prxmpdu, sizeof(SMAC_ACNT_RX_MGMT_MPDU_st));
						}
					}
					break;
				case 1:{	// Ctrl
						SMAC_ACNT_RX_CTRL_MPDU_st *prxmpdu =
						    (SMAC_ACNT_RX_CTRL_MPDU_st *) (prxacntppdu->acntRxRdPtr_v + usedlen);
						//SMAC_ACNT_RX_CTRL_MPDU_st     *prxmpdu = &prxppdu_usr->u.ctrl;
						usedlen += sizeof(SMAC_ACNT_RX_CTRL_MPDU_st);
						prxRateHistogram->pkt_cnt[1]++;
						// TBD: Processing SMAC_ACNT_RX_CTRL_MPDU_st
						if (rxacnt_idmsg > 0) {
							printk("Ctrl, prxmpdu: %p\n", prxmpdu);

							mwl_hex_dump(prxmpdu, sizeof(SMAC_ACNT_RX_CTRL_MPDU_st));
						}
					}
					break;
				case 2:{	// Data
						SMAC_ACNT_RX_DATA_MPDU_st *prxmpdu =
						    (SMAC_ACNT_RX_DATA_MPDU_st *) (prxacntppdu->acntRxRdPtr_v + usedlen);
						//SMAC_ACNT_RX_DATA_MPDU_st     *prxmpdu = &prxppdu_usr->u.data;
						usedlen += offsetof(SMAC_ACNT_RX_DATA_MPDU_st, mpduTid);
						prxRateHistogram->pkt_cnt[2]++;
						// TBD: Processing SMAC_ACNT_RX_DATA_MPDU_st
						for (tid = 0; tid < prxmpdu->numTids; tid++) {
							SMAC_ACNT_RX_MPDU_TID_INFO_st *prxmpdu_tid = &prxmpdu->mpduTid[tid];
							usedlen += sizeof(SMAC_ACNT_RX_MPDU_TID_INFO_st);

							// TBD: Processing SMAC_ACNT_RX_MPDU_TID_INFO_st
							if (rxacnt_idmsg > 0) {
								printk("Dat[%u], prxmpdu_tid: %p\n", tid, prxmpdu_tid);
								mwl_hex_dump(prxmpdu_tid, sizeof(SMAC_ACNT_RX_MPDU_TID_INFO_st));
							}
						}
					}
					break;
	...
	if (rxacnt_idmsg > 0) {
		printk("[RXACNT] <=%s(), (rd_v, wr_v)=(%p, %p), (rd_p, wr_p)=(%x, %x)\n", __func__,
		       prxacntppdu->acntRxRdPtr_v, wrptr, psmac_cfg->acntRxRdPtr, psmac_cfg->acntRxWrPtr);
		printk("[RXACNT] ----------------------------------------------------\n\n");
		rxacnt_idmsg--;
	}
#endif				//RXACNT_REC
...
void wlRxAcntPPDUBufInit(struct net_device *netdev)
{
#ifdef RXACNT_REC
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
	...
	prxacntppdu->acntRxSize = p_smac_cfg->acntRxSize = RX_ACNT_PPDU_BUF_SIZE;
#ifdef USE_NCBUF
	// Using non-cacheable buffer
	prxacntppdu->acntRxBaseAddr_v = (void *)wl_dma_alloc_coherent(wlpd_p->dev, p_smac_cfg->acntRxSize, &phys_addr, wlpd_p->dma_alloc_flags);
#else
	// Using cacheable buffer
	prxacntppdu->acntRxBaseAddr_v = (char *)wl_kmalloc(p_smac_cfg->acntRxSize, GFP_KERNEL);
	phys_addr = dma_map_single(wlpd_p->dev, prxacntppdu->acntRxBaseAddr_v, prxacntppdu->acntRxSize, DMA_FROM_DEVICE);
#endif				//
	prxacntppdu->acntRxBaseAddr_p = p_smac_cfg->acntRxBaseAddr = (U32) phys_addr;
	WLDBG_INFO(DBG_LEVEL_0, "%s(), acntRxBaseAddr (v, p)=(%p, %x), size=%x\n", __func__,
		   prxacntppdu->acntRxBaseAddr_v, p_smac_cfg->acntRxBaseAddr, p_smac_cfg->acntRxSize);

	prxacntppdu->acntRxRdPtr_v = prxacntppdu->acntRxBaseAddr_v;
	prxacntppdu->acntRxRdPtr_p = &(parent_wlpptr->smacCfgAddr->acntRxRdPtr);
	// Initialize the buffer pointer
	p_smac_cfg->acntRxRdPtr = p_smac_cfg->acntRxWrPtr = p_smac_cfg->acntRxBaseAddr;

#endif				//RXACNT_REC
	return;
}

void wlRxAcntPPDUCleanup(struct net_device *netdev)
{
#ifdef RXACNT_REC
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	struct wlprivate_data *wlpd_p = wlpptr->wlpd_p;
...
#ifdef USE_NCBUF
		// Using non-cacheable buffer
		wl_dma_free_coherent(wlpd_p->dev, p_smac_cfg->acntRxSize, prxacntppdu->acntRxBaseAddr_v, prxacntppdu->acntRxBaseAddr_p);
#else
		// Using cacheable buffer
		dma_unmap_single(wlpd_p->dev, prxacntppdu->acntRxBaseAddr_p, p_smac_cfg->acntRxSize, DMA_FROM_DEVICE);
...
#ifdef RXACNT_REC
irqreturn_t wlSC5MSIX_RAcntRec(int irq, void *dev_id)
{
	struct msix_context *ctx = (struct msix_context *)dev_id;
	struct net_device *netdev = ctx->netDev;
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
