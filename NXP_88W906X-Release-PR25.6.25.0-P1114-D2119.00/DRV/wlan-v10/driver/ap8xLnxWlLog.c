...
{
	unsigned char *debugString = NULL;	//Reduced from 1024 to 1020 to prevent frame size > 1024bytes warning during compilation
	int debugString_len = 1020;
	...
	/* wlsyslog called in many context, so use wl_kmalloc_autogfp to alloc mem will be more compatible */
	debugString = wl_kmalloc_autogfp(debugString_len);
	if (!debugString)
		return;
	strcpy(debugString, "");
