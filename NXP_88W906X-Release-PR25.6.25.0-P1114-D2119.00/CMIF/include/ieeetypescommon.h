...
typedef struct IEEEtypes_RSNX_IE_t {
	u8 ElemId;
	u8 Len;
	u8 Data;
} IEEEtypes_RSNX_IE_t;
...
typedef struct IEEEtypes_Bssid_info_t {
	...
	u32 ER_BSS:1;
	u32 Co_Located_AP:1;
	u32 Unsolicited_Probe_Responses_Active:1;
	u32 Member_Of_ESS_With_2dot4_5_GHz_Co_Located_AP:1;
	u32 OCT_Supported_With_Reporting_AP:1;
	u32 Co_Located_With_6_GHz_AP:1;
	u32 Reserved:11;
#else
	union {
		u32 u32_data;
...
typedef struct IEEEtypes_ActionField_t {
	...
	union {
		...
		IEEEtypes_Neighbor_Report_Element_t NeighborReqportElemes[NB_LIST_MAX_NUM];
#endif
...
typedef struct {
	...
	u8 hecap_ext[MAX_TX_RX_HE_MCS_SUPPORT_FIELD_LENGTH + MAX_PPE_THRESHOLD_INFO_LENGTH - 2 * sizeof(he_mcs_nss_support_t)];
}
...
#define MAX_SIZE_RSNX_IE_BUF 3	/* number of bytes */
