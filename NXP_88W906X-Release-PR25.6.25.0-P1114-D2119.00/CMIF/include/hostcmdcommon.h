...
#define HostCmd_CMD_SET_EBF_MAX_SS              0x1115
...
#define HostCmd_CMD_ATF_CFG_SET                 0x3102
#define HostCmd_CMD_ATF_CFG_RESET               0x3103
#define HostCmd_CMD_ATF_CFG_GET                 0x3104
...
typedef PACK_START struct tagHostCmd_FW_SET_PRESCAN {
	FWCmdHdr CmdHdr;
	u32 TsfTime;
} PACK_END HostCmd_FW_SET_PRESCAN, *PHostCmd_FW_SET_PRESCAN, HostCmd_FW_SET_DISCONNECT, *PHostCmd_FW_SET_DISCONNECT;
...
typedef PACK_START struct _rate_hist_t {
	...
	u32 HtRates[QS_NUM_SUPPORTED_11N_BW][QS_NUM_SUPPORTED_GI][QS_NUM_SUPPORTED_MCS];
	u32 VHtRates[QS_NUM_SUPPORTED_11AC_NSS][QS_NUM_SUPPORTED_11AC_BW][QS_NUM_SUPPORTED_GI][QS_NUM_SUPPORTED_11AC_MCS];
...
typedef struct _WLAN_TX_RATE_HIST {
	...
	WLAN_TX_RATE_HIST_DATA MU_rate[QS_NUM_SUPPORTED_11AC_NSS - 1][QS_NUM_SUPPORTED_11AC_BW][QS_NUM_SUPPORTED_GI][QS_NUM_SUPPORTED_11AC_MCS];
...
typedef PACK_START struct tagHostCmd_FW_SET_MAC {
	...
	u16 MacType;
	u8 MacAddr[6];
} PACK_END HostCmd_DS_SET_MAC, *PHostCmd_DS_SET_MAC, HostCmd_FW_SET_BSSID, *PHostCmd_FW_SET_BSSID, HostCmd_FW_SET_MAC, *PHostCmd_FW_SET_MAC;
...
typedef PACK_START struct tagHostCmd_FW_SET_EBF_MAX_SS {
	FWCmdHdr CmdHdr;
	u8 MacAddr[6];
	u8 ebf_max_ss;		/*MAX supported SS for explicit BF + 1 */

} PACK_END HostCmd_FW_SET_EBF_MAX_SS, *PHostCmd_FW_SET_EBF_MAX_SS;
...
typedef PACK_START struct _HostCmd_SET_POWER_CONSTRAINT {
	...
	IEEEtypes_VHTTransmitPowerEnvelopeElement_t VHTTransmitPowerEnvelopeElement;
...
typedef PACK_START struct tagHostCmd_FW_HT_STBC_MODE {
	...
	u32 Action;
	u32 Mode;
} PACK_END HostCmd_FW_HT_STBC_MODE, *PHostCmd_FW_HT_STBC_MODE, *PHostCmd_FW_HT_STBC_TX;
...
#define DFS_GET_LOG                         9
#define DFS_SET_JPW53_ZC_ERROR_TH          10
...
} PACK_END HostCmd_FW_NEWDP_OFFCHANNEL_START, *PHostCmd_FW_NEWDP_OFFCHANNEL_START;
...
typedef PACK_START struct tagHostCmd_FW_NEWDP_SENSORD_SET_BLANKING {
	FWCmdHdr CmdHdr;
	u8 blankingmask;
} PACK_END HostCmd_FW_NEWDP_SENSORD_SET_BLANKING, *PHostCmd_FW_NEWDP_SENSORD_SET_BLANKING;
...
typedef PACK_START struct tagHostCmd_FW_NEWDP_SET_POWER_PER_RATE {
	...
	u16 last:1;
	u8 payload[0];
} PACK_END HostCmd_FW_NEWDP_SET_POWER_PER_RATE, *PHostCmd_FW_NEWDP_SET_POWER_PER_RATE;
...
typedef PACK_START struct tagHostCmd_FW_NEWDP_RADIO_STATUS_NOTIFICATION {
	FWCmdHdr CmdHdr;
	u32 Action;
} PACK_END HostCmd_FW_NEWDP_RADIO_STATUS_NOTIFICATION, *PHostCmd_FW_NEWDP_RADIO_STATUS_NOTIFICATION;
...
#endif
	u8 GID;
	u8 flush;		// State of MU Set ownership (mu_own_*)
#if defined(SOC_W906X) || defined(SOC_W9068)
	u8 Schedule;
	u8 MaxUser;
...
typedef struct atf_info_s {
	u8 enable;
	u8 mode;
	u32 mode1_threshold_rate_hi;
	u32 mode1_min_airtime;
	u32 mode2_threshold_rate_lo;
	u8 mode2_airtime_percent;
	u8 mode3_threshold_mcs_lo;
	u8 mode3_threshold_mcs_hi;
	u8 mode3_airtime_percent;
	u8 mode4_bss_percent[SMAC_BSS_NUM];
	u8 mode4_sta_percent[SMAC_STA_NUM];
} atf_info_t;

typedef PACK_START struct tagHostCmd_FW_ATF_CFG_SET {
	FWCmdHdr CmdHdr;
	atf_info_t atf_info;
} PACK_END HostCmd_FW_ATF_CFG_SET, *PHostCmd_ATF_CFG_SET;

typedef PACK_START struct tagHostCmd_FW_ATF_CFG_RESET {
	FWCmdHdr CmdHdr;
} PACK_END HostCmd_FW_ATF_CFG_RESET, *PHostCmd_ATF_CFG_RESET;

typedef PACK_START struct tagHostCmd_FW_ATF_CFG_GET {
	FWCmdHdr CmdHdr;
	atf_info_t atf_info;
} PACK_END HostCmd_FW_ATF_CFG_GET, *PHostCmd_ATF_CFG_GET;
...
#define OFF_CHNL_PRIORITY_TYPE        5
	...
	u32 avail_slot_cnt;
	u32 cca_block;		// percentage
	u32 nav_block;		// percentage
} mvl_status_t;
...
typedef enum {
	...
	MIB_TF_RATEINFO,
	MIB_TF_MU_RTS_UL,
	MIB_DRA_PER_TH_UP_LOW_RATE,
	MIB_DRA_PER_TH_UP_HIGH_RATE,
	MIB_DRA_PER_TH_DOWN_LOW_RATE,
	MIB_DRA_PER_TH_DOWN_HIGH_RATE,
	MIB_DRA_PER_TH_UP_NOISY_RATE,
	MIB_DRA_PER_TH_DOWN_NOISY_RATE,
	MIB_TF_MU_BAR_STA_LIMIT,
	MIB_DRA_PER_STEADY_STATE_ROLLING_AVG_FACTOR,
	MIB_DRA_PER_NF_BIN_THRESH_LO_2G,
	MIB_DRA_PER_NF_BIN_THRESH_LO_5G,
	MIB_DRA_PER_NF_OFFSET_SCALING_FACTOR_2G,
	MIB_DRA_PER_NF_OFFSET_SCALING_FACTOR_5G,
	MIB_DRA_EN_DOWN_JMP_RATE_OFFSET,
	MIB_MAX,
} MIB_DEF;
...
typedef PACK_START struct tagHostCmd_FW_SET_SR {
	...
	s8 thresSrg;
	u8 action;		//2: set sr param, 1: test mode, 0: get interference RSSI

	union {
		s8 intfRssiLog[SR_RSSI_LOG_SIZE];	//For storing interference RSSI log from sfw to drv when action==0
		u8 macaddr[6];	//interference MAC addr for RSSI log
	};
...
#define DFS_LOC_IRAM   0
#define DFS_LOC_DMEM   1
typedef PACK_START struct dfs_log_s {
	u32 dfs_rdwr[MAX_DFS_NUM];
	u32 hal_dfsRadarDetected[MAX_DFS_NUM];
	u32 hal_dfsChirpRadarCnt[MAX_DFS_NUM];
	u32 hal_dfsFixRadarCnt[MAX_DFS_NUM];
	u32 hal_dfsFalseRadarCnt[MAX_DFS_NUM];
	u32 hal_dfsRecordProcessed[MAX_DFS_NUM];
	u32 false_chirp_radar_count[MAX_DFS_NUM];
	u32 false_det_count_history[MAX_DFS_NUM];
	//
	u8 dfs_curr_state[MAX_DFS_NUM];
	u8 dfs_curr_chan[MAX_DFS_NUM];
	u8 dfs_dbg_bw[MAX_DFS_NUM];
	u8 dfs_cac_state[MAX_DFS_NUM];
	u8 dfs_que_thread[MAX_DFS_NUM];
	u8 dfs_curr_region;
	u8 dfs_w53_flag;
	//
	u32 dfsRegDomain;
	...
	u16 loc_chirp_freq_data_save;
	u16 len_chirp_freq_data_save;
	u32 chirp_freq_data_save_addr;
	//
	u16 loc_dfs_dbg_cnt;
	u16 len_dfs_dbg_cnt;
	u32 dfs_dbg_cnt_addr;
